...
uint16 _rtk_rg_outerHdrIPv4Checksum(struct rgip *pip)
{
	...
	//if(csum == 0xffff)	csum = 0;		//FIXME:luke
 	pip->ip_sum=orgcsum;
	return((uint16)htons((~csum)&0xffff));
}
...
uint16 _rtk_rg_outerHdrUdpChecksum(struct rgip *pip, rgudpHdr_t *pudp, uint16 udpLen)
{
	...
	csum = _rtk_rg_cksum((uint16 *)tmp, 4U, 0U);
	memcpy(tmp,&pip->ip_dst,sizeof(pip->ip_dst));
	csum = _rtk_rg_cksum((uint16 *)tmp, 4U, csum);
...
uint32 _rtk_rg_hashTrapBandwidthSharing_get(uint32 SPA, uint64 SMAC, uint64 DMAC, uint32 innerSIP, uint32 innerDIP, uint32 innerSrcPort, uint32 innerDstPort)
{
	...
	//TRAP_HASH_SPA
	ASSERT_EQ(rtk_trap_cpuTrapHashMask_get(TRAP_HASH_SPA, &enabled),RT_ERR_OK);
	if(enabled == ENABLED)
	{
	...
	//TRAP_HASH_SMAC
	ASSERT_EQ(rtk_trap_cpuTrapHashMask_get(TRAP_HASH_SMAC, &enabled),RT_ERR_OK);
	if(enabled == ENABLED)
	{
		for(i=0U; i< 48U; i+=4U)
	...
	//TRAP_HASH_DMAC
	ASSERT_EQ(rtk_trap_cpuTrapHashMask_get(TRAP_HASH_DMAC, &enabled),RT_ERR_OK);
	if(enabled == ENABLED)
	{
		for(i=0U; i< 48U; i+=4U)
	...
	//TRAP_HASH_SIP_INNER
	ASSERT_EQ(rtk_trap_cpuTrapHashMask_get(TRAP_HASH_SIP_INNER, &enabled),RT_ERR_OK);
	if(enabled == ENABLED)
	{
		for(i=0U; i< 32U; i+=4U)
	...
	//TRAP_HASH_DIP_INNER
	ASSERT_EQ(rtk_trap_cpuTrapHashMask_get(TRAP_HASH_DIP_INNER, &enabled),RT_ERR_OK);
	if(enabled == ENABLED)
	{			
		for(i=0U; i< 32U; i+=4U)
	...
	ASSERT_EQ(rtk_trap_cpuTrapHashMask_get(TRAP_HASH_SPORT_INNER, &enabled),RT_ERR_OK);
	...
	ASSERT_EQ(rtk_trap_cpuTrapHashMask_get(TRAP_HASH_DPORT_INNER, &enabled),RT_ERR_OK);
	if(enabled == ENABLED)
	{
		for(i=0U; i< 16U; i+=4U)

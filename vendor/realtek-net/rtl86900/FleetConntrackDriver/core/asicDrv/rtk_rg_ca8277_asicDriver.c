...
asic_ret_t rtk_rg_asic_dmaAftFib_get(uint32_t idx, rtk_rg_asic_dmaAftFib_t *pDmaAftFib)
{
	...
	PARAM_CHK(idx >= DMA_AFT_FIB_SIZE, ASIC_RET_OUT_OF_RANGE);
	PARAM_CHK(pDmaAftFib == NULL, ASIC_RET_NULL_POINTER);
...
asic_ret_t rtk_rg_asic_dmaAftTpid_get(rtk_rg_asic_dmaAftTpid_t *pAftTpid)
{
	...
	PARAM_CHK(pAftTpid == NULL, ASIC_RET_NULL_POINTER);
...
asic_ret_t rtk_fc_asic_l2te_policer_enable_set(uint8_t enable)
{
	...
	ctrl.wrd = rtk_ne_reg_read(L2TM_L2TE_GLB_CTRL);
	ctrl.bf.pol_en = enable;
	rtk_ne_reg_write(ctrl.wrd, L2TM_L2TE_GLB_CTRL);
...
asic_ret_t rtk_rg_l3_policer_ifg_config_set(bool ifgInclude)
{
	...
	if(ifgInclude)
		...
		l3_pol_cfg.ipg = 0;
#else
	/*L3FE->L3TE: packet length does not include CRC, add 4 bytes in ipg*/
	if(ifgInclude)
		l3_pol_cfg.ipg = 24;
	else
		l3_pol_cfg.ipg = 4;
...
asic_ret_t rtk_rg_l3_flow_policer_rate_get(uint32_t pol_idx, uint32_t *committed_rate, uint32_t *peak_rate)
{
	...
	*committed_rate = l3_pol_profile_cfg.cir_m * 500 + l3_pol_profile_cfg.cir_k;
	rate_k =  l3_pol_profile_cfg.pir_k % 500; //The valid value is 0~499.
	*peak_rate = l3_pol_profile_cfg.pir_m * 500 + l3_pol_profile_cfg.pir_k;
...
asic_ret_t rtk_rg_l3_flow_policer2_rate_get(uint32_t pol_idx, uint32_t *committed_rate, uint32_t *peak_rate)
{
	...
	*committed_rate = l3_pol_profile_cfg.cir_m * 1000 + l3_pol_profile_cfg.cir_k;
	rate_k =  l3_pol_profile_cfg.pir_k % 1000; //The valid value is 0~999.s
...
asic_ret_t rtk_rg_asic_fb_init(void)
{
...
#if defined(CONFIG_CA8277B_SERIES)
	{
		...
		tpidConf.tpid_2 = 0;
		tpidConf.tpid_3 = 0;
		rtk_rg_asic_dmaAftTpid_set(0x3, tpidConf);
	}
#endif

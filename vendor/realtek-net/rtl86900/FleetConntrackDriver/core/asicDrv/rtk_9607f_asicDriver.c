...
asic_ret_t rtk_9607f_asic_netifTable_add(uint32_t idx, rtk_9607f_asic_netif_entry_t *pNetif)
{
	...
	// if
	{
		...
		// FPGA USAGE
#else		
		//default include padding len from SNAP L2 len
		eif.snap_tra_len_control=0;

		
#if 0	//	For wifi tx amsdu packet: the length in snap len should include pad_len
		...
		//	For wifi tx amsdu packet: the length in snap len should include pad_len, snap to snap also need to include padding
#if 0		
...
{
	if(asic_db.flow_profile_info[profileIdx].empty_miss_def_act_idx == asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP])
		*flow_empty_act = RTK_9607F_FLOW_DEFACT_TYPE_TRAP;
	else if(asic_db.flow_profile_info[profileIdx].empty_miss_def_act_idx == asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_DROP])
		*flow_empty_act = RTK_9607F_FLOW_DEFACT_TYPE_DROP;
	else if(asic_db.flow_profile_info[profileIdx].empty_miss_def_act_idx == asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP_WITH_METER])
		*flow_empty_act = RTK_9607F_FLOW_DEFACT_TYPE_TRAP_WITH_METER;
	else if(asic_db.flow_profile_info[profileIdx].empty_miss_def_act_idx == asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP_UMC_STORM])
		*flow_empty_act = RTK_9607F_FLOW_DEFACT_TYPE_TRAP_UMC_STORM;
	else
	{
		ERROR("%s: [ERROR] unknown profile[%u].empty_miss_def_act_idx (%u)\n", __func__, profileIdx, asic_db.flow_profile_info[profileIdx].empty_miss_def_act_idx);
		return ASIC_RET_FAIL;
	}

	if(asic_db.flow_profile_info[profileIdx].aging_miss_def_act_idx == asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP])
		*flow_aging_act = RTK_9607F_FLOW_DEFACT_TYPE_TRAP;
	else if(asic_db.flow_profile_info[profileIdx].aging_miss_def_act_idx == asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_DROP])
		*flow_aging_act = RTK_9607F_FLOW_DEFACT_TYPE_DROP;
	else if(asic_db.flow_profile_info[profileIdx].aging_miss_def_act_idx == asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP_WITH_METER])
		*flow_aging_act = RTK_9607F_FLOW_DEFACT_TYPE_TRAP_WITH_METER;
	else if(asic_db.flow_profile_info[profileIdx].aging_miss_def_act_idx == asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP_UMC_STORM])
		*flow_aging_act = RTK_9607F_FLOW_DEFACT_TYPE_TRAP_UMC_STORM;
	else
	{
		ERROR("%s: [ERROR] unknown profile[%u].aging_miss_def_act_idx (%u)\n", __func__, profileIdx, asic_db.flow_profile_info[profileIdx].aging_miss_def_act_idx);
		return ASIC_RET_FAIL;
	}
...
asic_ret_t rtk_9607f_flow_default_action_update(uint32_t profileIdx, rtk_9607f_asic_flow_defAct_type_t act)
{
	if(act == RTK_9607F_FLOW_DEFACT_TYPE_TRAP)
	{
		...
		asic_db.flow_profile_info[profileIdx].empty_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP];
		asic_db.flow_profile_info[profileIdx].aging_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP];
	}
	else if(act == RTK_9607F_FLOW_DEFACT_TYPE_DROP)
	{
		...
		asic_db.flow_profile_info[profileIdx].empty_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_DROP];
		asic_db.flow_profile_info[profileIdx].aging_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP];
	}
	else if(act == RTK_ASIC_FLOW_DEFACT_TYPE_TRAP_WITH_METER)
	{
		...
		aal_hash_profile_defAct_update(profileIdx, asic_db.flow_defAct_idx[RTK_ASIC_FLOW_DEFACT_TYPE_TRAP_WITH_METER], asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP]);
		asic_db.flow_profile_info[profileIdx].empty_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_ASIC_FLOW_DEFACT_TYPE_TRAP_WITH_METER];
		asic_db.flow_profile_info[profileIdx].aging_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP];
	}
	else if(act == RTK_ASIC_FLOW_DEFACT_TYPE_TRAP_UMC_STORM)
	{
		aal_hash_profile_defAct_update(profileIdx, asic_db.flow_defAct_idx[RTK_ASIC_FLOW_DEFACT_TYPE_TRAP_UMC_STORM], asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP]);
		asic_db.flow_profile_info[profileIdx].empty_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_ASIC_FLOW_DEFACT_TYPE_TRAP_UMC_STORM];
		asic_db.flow_profile_info[profileIdx].aging_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP];
	}
	else
	{
...
asic_ret_t rtk_asic_flow_default_action_meter_update(uint32_t profileIdx, uint32_t flow_meter_idx)
{
	...
	if((ret = aal_entry_get_by_idx(AAL_TABLE_HASH_DEFAULT_ACTION, asic_db.flow_defAct_idx[RTK_ASIC_FLOW_DEFACT_TYPE_TRAP_WITH_METER],  &default_action)) == AAL_E_OK ) {
		...
		ret = aal_entry_set(AAL_TABLE_HASH_DEFAULT_ACTION, asic_db.flow_defAct_idx[RTK_ASIC_FLOW_DEFACT_TYPE_TRAP_WITH_METER],  &default_action);
	}
...
asic_ret_t _rtk_9607f_asic_flow_key_gen(rtk_9607f_asic_pathValue_t in_path, void* flow_key, aal_hash_key_t *hash_key)
{
	...
	{
		rtk_9607f_asic_flow_config_key_pathVxlan_up_extra_tx_t *pPathVxlan_up_extra_tx_key = (rtk_9607f_asic_flow_config_key_pathVxlan_up_extra_tx_t *)flow_key;
	...
	{
		//rtk_9607f_asic_flow_config_key_path5_t *pPath5_key = (rtk_9607f_asic_flow_config_key_path5_t *)flow_key;
		rtk_9607f_asic_flow_config_key_esp_ds_spi_t *pPath_esp_ds_spi_key = (rtk_9607f_asic_flow_config_key_esp_ds_spi_t *)flow_key;
...
#if defined(CONFIG_REALTEK_BOARD_FPGA) || defined(CONFIG_REALTEK_BOARD_FPGA_V8)
		if(pPath_esp_ds_spi_key->l4_proto_mode)
			hash_key->ip_protocol = pPath_esp_ds_spi_key->l4proto_num;
		else
			hash_key->ip_protocol = pPath_esp_ds_spi_key->l4proto? 0x6 : 0x11;	// 1: TCP, 0: UDP
#else
		hash_key->ip_protocol = pPath_esp_ds_spi_key->l4proto_num;
#endif
...
asic_ret_t _rtk_9607f_asic_flow_action_gen(rtk_9607f_asic_pathValue_t in_path, void* flow_key, void* flow_act, aal_hash_action_t *hash_action, uint32_t profileIdx, uint32_t tupleIdx)
{
	...
	else if(in_path == FB_PATH_34)
	{
		...
		{
			hash_action->mdata_byte = pPath34_act->wifi_ssid; //wifi_ssid is wifi rx cahce index
			hash_action->mdata_byte_vld = 1;
	...
	else if(in_path == FB_PATH_5)
	{
		...
		// === action: others ===
		hash_action->l3_if_vld1 = pPath5_act->l3_if_vld1;
		...
		{
			hash_action->mdata_byte = pPath5_act->wifi_ssid; //wifi_ssid is rx cache index
			hash_action->mdata_byte_vld = 1;
	...
	{
		rtk_9607f_asic_flow_config_action_pathVxlan_up_extra_tx_t *pPathVxlan_up_extra_tx_act = (rtk_9607f_asic_flow_config_action_pathVxlan_up_extra_tx_t *)flow_act;
	...
	{
		rtk_9607f_asic_flow_config_key_esp_ds_spi_t *pPath_esp_ds_spi_key = (rtk_9607f_asic_flow_config_key_esp_ds_spi_t *)flow_key;
		rtk_9607f_asic_flow_config_action_esp_ds_spi_t *pPath_esp_ds_spi_act = (rtk_9607f_asic_flow_config_action_esp_ds_spi_t *)flow_act;
		...
		if(pPath_esp_ds_spi_act->ip_action_mode == RTK_9607F_IP_ACTION_MODE_V4_V6_NAPT)
		{
			hash_action->ip_addr_vld = TRUE;
		...
		else if(pPath_esp_ds_spi_act->ip_action_mode == RTK_9607F_IP_ACTION_MODE_V6_NPT)
		{
			int ret;
			hash_npt6_prefix_addr_entry_t prefix_addr_entry;
		...
		else if(pPath_esp_ds_spi_act->ip_action_mode == RTK_9607F_IP_ACTION_MODE_V4_NAT_LOOPBACK)
		{
			if(flow_act_mode == HASH_TYPE0_FLOW_ACTION_MODE_NORMAL)
			{
				ERROR("flow configuration do not support IPv4 NAT LOOPBACK HW acceleration.");
		...
		if(pPath_esp_ds_spi_act->l2format_act_vld)
		{
			hash_action->l2_format_vld1 = TRUE;
			hash_action->l2_format1 = pPath_esp_ds_spi_act->l2format_act;
		}
	}
#endif
...
asic_ret_t rtk_9607f_asic_flow_hash_crc_cal(rtk_9607f_asic_flow_hash_cal_info_t *flowHashCalInfo, rtk_9607f_asic_flow_hash_crc_t *flow_hash_crc)
{
	...
	{
		profileIdx = RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP;
		tupleIdx = HASH_DUAL_FRAG_TUPLE;
		/* hash key generation*/
		ret = _rtk_9607f_asic_flow_key_gen(FB_PATH_VXLAN_US_EXTRA_TX, &flowHashCalInfo->pathVxlan_up_extra_tx_key, &hash_key);
		if(ret != ASIC_RET_SUCCESS)
		{
	...
	else if(flowHashCalInfo->in_path == FB_PATH_ESP_SPI_DS)
	{
		...
		/* hash key generation*/
		ret = _rtk_9607f_asic_flow_key_gen(FB_PATH_ESP_SPI_DS, &flowHashCalInfo->path_esp_ds_spi_key, &hash_key);
		if(ret != ASIC_RET_SUCCESS)
		{
...
asic_ret_t rtk_9607f_asic_flow_add_with_no_crc_calulate(rtk_9607f_asic_flow_config_t *flowConfig, int32_t *mainHashIdx, rtk_9607f_asic_flow_hash_crc_t flow_hash_crc)
{
	...
	else if(flowConfig->in_path == FB_PATH_VXLAN_US_EXTRA_TX)
	{
		...
		/* hash key generation*/
		ret = _rtk_9607f_asic_flow_key_gen(FB_PATH_VXLAN_US_EXTRA_TX, &(flowConfig->pathVxlan_up_extra_tx.key), &hash_key);
		if(ret != ASIC_RET_SUCCESS)
		{
			DEBUG("generate hash key for pathVxlan_up_extra_tx failed!");
			return ret;
		...
		/* hash action generation*/
		ret = _rtk_9607f_asic_flow_action_gen(FB_PATH_VXLAN_US_EXTRA_TX, &(flowConfig->pathVxlan_up_extra_tx.key), &(flowConfig->pathVxlan_up_extra_tx.action), &hash_action, profileIdx, tupleIdx);
		if(ret != ASIC_RET_SUCCESS)
		{
	...
	else if(flowConfig->in_path == FB_PATH_ESP_SPI_DS)
	{
		...
		/* hash key generation*/
		ret = _rtk_9607f_asic_flow_key_gen(FB_PATH_ESP_SPI_DS, &(flowConfig->path_esp_ds_spi.key), &hash_key);
		if(ret != ASIC_RET_SUCCESS)
		{
			DEBUG("generate hash key for FB_PATH_ESP_SPI_DS failed!");
			return ret;
		...
		/* hash action generation*/
		ret = _rtk_9607f_asic_flow_action_gen(FB_PATH_ESP_SPI_DS, &(flowConfig->path_esp_ds_spi.key), &(flowConfig->path_esp_ds_spi.action), &hash_action, profileIdx, tupleIdx);
		if(ret != ASIC_RET_SUCCESS)
		{
			DEBUG("generate hash action for FB_PATH_ESP_SPI_DS failed!");
			return ret;
		}
	}
#endif
...
asic_ret_t rtk_asic_dmalso_backpressure_rule_add(rtk_asic_dmalso_bp_conf_t *pVP_bp_conf)
{
	...
	memcpy(&(asic_db.dmalso_bp_rule[pVP_bp_conf->rule_idx]), pVP_bp_conf, sizeof(rtk_asic_dmalso_bp_conf_t));
...
asic_ret_t rtk_asic_dmalso_backpressure_rule_del(rtk_asic_dmalso_bp_rule_t rule_idx)
{
	...
	memcpy(&(asic_db.dmalso_bp_rule[rule_idx]), &vp_bp_conf, sizeof(rtk_asic_dmalso_bp_conf_t));
...
{
	asic_ret_t ret = ASIC_RET_SUCCESS;
	rtk_asic_dmalso_bp_rule_t rule_idx;
	int i;

	for (rule_idx = ASIC_DMALSO_BP_RULE_1; rule_idx < ASIC_DMALSO_BP_RULE_END; rule_idx++) {
		printk("[%d] %s\n", rule_idx, (asic_db.dmalso_bp_rule_vld[rule_idx] ? "enable" : "disable"));

		if (asic_db.dmalso_bp_rule_vld[rule_idx]) {
			printk("\tthreshold hi/lo: %hu/%hu\n", asic_db.dmalso_bp_rule[rule_idx].threshold_hi, asic_db.dmalso_bp_rule[rule_idx].threshold_lo);

			for (i = 0; i < ASIC_DMALSO_BP_SPID_MAX; i++) {
				if (!asic_db.dmalso_bp_rule[rule_idx].lspid_vld[i] && !asic_db.dmalso_bp_rule[rule_idx].vpId_vld[i])
					continue;

				printk(KERN_CONT"\t[%d] ", i);
				if (asic_db.dmalso_bp_rule[rule_idx].lspid_vld[i]) {
					printk(KERN_CONT"lspid: %hhu\t", asic_db.dmalso_bp_rule[rule_idx].lspid[i]);
				}

				if (asic_db.dmalso_bp_rule[rule_idx].vpId_vld[i]) {
					printk(KERN_CONT"vp: %hhu\t", asic_db.dmalso_bp_rule[rule_idx].vpId[i]);
				}
...
asic_ret_t rtk_9607f_asic_dmaAftTpid_set(char tpid_set_msk, rtk_9607f_asic_dmaAftTpid_t aftTpid)
{
	...
	if(tpid_set_msk & (1 << 1)){
		...
		rtk_9607f_asic_dma_lso_reg_write(dma_lso_vlan_cfg0.wrd, DMA_SEC_DMA_GLB_DMA_LSO_VLAN_TAG_TYPE0);
	}
	if(tpid_set_msk & (1 << 2)){
		...
		rtk_9607f_asic_dma_lso_reg_write(dma_lso_vlan_cfg1.wrd, DMA_SEC_DMA_GLB_DMA_LSO_VLAN_TAG_TYPE1);
	}
...
asic_ret_t rtk_fc_asic_l3fe_pp_spcl_l3_offset_set(uint32_t l3_offset)
{
	...
	l3fe_pp_offset_hdr.wrd = rtk_9607f_asic_ne_reg_read(L3FE_PP_OFFSET_HDR);
	l3fe_pp_offset_hdr.bf.l3_prog_offset = l3_offset;
	rtk_9607f_asic_ne_reg_write(l3fe_pp_offset_hdr.wrd, L3FE_PP_OFFSET_HDR);
...
uint32_t rtk_9607f_flow_default_action_init(uint32_t actGrpMsk_default)
{
	...
	if(rtk_9607f_l3_flow_policer_burstSize_set(G3_FLOW_POLICER_IDXSHIFT_HASH_DFT_UMC_TRAP, 1, 1) != ASIC_RET_SUCCESS)
	...
	ASSERT_EQ(aal_hash_default_action_add(&default_action, &default_action_idx, actGrpMsk_default), AAL_E_OK);
	asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP_UMC_STORM] = default_action_idx;
...
uint32_t rtk_9607f_flow_hash_profile_init(uint32 actGrpMsk_flow,uint32 actGrpMsk_type1, rtk_9607f_flow_key_mask_t flowKeyMask)
{
	...
	******************************************************/
	...
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_FLOW_5TUPLE].empty_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP];
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_FLOW_5TUPLE].aging_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP];
	...
	******************************************************/
	...
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_FLOW_2TUPLE].empty_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP];
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_FLOW_2TUPLE].aging_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP];
	...
	******************************************************/
	...
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_FLOW_MC].empty_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_DROP];
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_FLOW_MC].aging_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP];
	// profile tuple add: 2 tuple
	// tuple 0
	aal_hash_mask_unmask(&hash_mask);
	hash_mask.ip_da = 128;
...
#else
	hash_mask.top_dei = (flowKeyMask.pall_vlan_deicfi)&0x1 ? mask : (~mask)&0x1;
	hash_mask.top_tpid_enc = mask&0x1;
#endif
	hash_mask.top_vid = (flowKeyMask.P345_vlanId) ? mask&0x1 : (~mask)&0x1;
	hash_mask.top_8021p = (flowKeyMask.P345_vlanPri) ? mask&0x1 : (~mask)&0x1;
	...
	******************************************************/
	...
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_FLOW_5TUPLE_TCP_FLAG0].empty_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP];
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_FLOW_5TUPLE_TCP_FLAG0].aging_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP];
	...
	******************************************************/
	...
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_DEFAULT_DROP].empty_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_DROP];
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_DEFAULT_DROP].aging_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_DROP];
	...
	******************************************************/
	...
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_MC_WIFI_TX_AMSDU_OFLD].empty_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_DROP];
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_MC_WIFI_TX_AMSDU_OFLD].aging_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_DROP];
	// profile tuple add: 5 tuple but no care TCP flag
	aal_hash_mask_unmask(&hash_mask);
	...
	******************************************************/
	// profile basic init: default action "TRAP", tuple number clear to 0, hash key algorithm selection
	aal_hash_profile_init(RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP, asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP], asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP], actGrpMsk_flow, actGrpMsk_type1);
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP].empty_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP];
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP].aging_miss_def_act_idx = asic_db.flow_defAct_idx[RTK_9607F_FLOW_DEFACT_TYPE_TRAP];
	...
	ASSERT_EQ(aal_hash_profile_tuple_add(RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP, TUPLE_TYPE_TYPE0, RTK_9607F_FLOW_TUPPLE_PRI_0, hash_mask, &hash_mask_idx, &tuple_index), AAL_E_OK);
	if(tuple_index != 0)
		ASICINITPRINT("%s: [ERROR] strange tuple_index (%d) of hash profile %d (%s)\n", __func__, RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP, tuple_index, string_of_hash_profile[RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP]);
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP].tuple_info[HASH_DUAL_FRAG_TUPLE].hash_mask_idx = hash_mask_idx;
	ASICINITPRINT("%s: ##### init hash profile %d (%s), profile 0 hash_mask_idx = %d\n", __func__, RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP, string_of_hash_profile[RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP], hash_mask_idx);
	...
	ASSERT_EQ(aal_hash_profile_tuple_add(RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP, TUPLE_TYPE_TYPE0, RTK_9607F_FLOW_TUPPLE_PRI_1, hash_mask, &hash_mask_idx, &tuple_index), AAL_E_OK);
	if(tuple_index != 1)
		ASICINITPRINT("%s: [ERROR] strange tuple_index (%d) of hash profile %d (%s)\n", __func__, RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP, tuple_index, string_of_hash_profile[RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP]);
	asic_db.flow_profile_info[RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP].tuple_info[HASH_ESP_SPI_TUPLE].hash_mask_idx = hash_mask_idx;
	ASICINITPRINT("%s: ##### init hash profile %d (%s), profile 1 hash_mask_idx = %d\n", __func__, RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP, string_of_hash_profile[RTK_9607F_FLOW_PROFILE_DUALFRAG_OR_ESP], hash_mask_idx);
...
uint32_t rtk_9607f_acl_init(void)
{
...
#if defined(CONFIG_CORTINA_BOARD_FPGA) || defined(CONFIG_REALTEK_BOARD_FPGA) || defined(CONFIG_REALTEK_BOARD_FPGA_V8) || defined(CONFIG_REALTEK_BOARD_MERCURY_ZEBU) || defined(CONFIG_RTK_L34_ASIC_MODELTEST)
	...
	if(ASICDRIVERVER >= CHIP_REV_ID_B){
		uint32 lspid_mask_cfg = (0x1 << 31);	//enable
		for(i = 0; i < 10; i++){	//default port 0~5 use bit 0~5, else invalid
			if(i < 6){	//port0_mask-port5_mask
				lspid_mask_cfg |= ((i) << (3*i));
				fc_db.acl_lan_portmask_hw |= (0x1 << i);
			}else
				lspid_mask_cfg |= ((7) << (3*i));
		}
		rtk_9607f_asic_ne_reg_write(lspid_mask_cfg, L3FE_CLS_LSPID_MSK);
	}

	//Update L2 Classifier per port lookup status 
	...
	pkt_len_cfg.wrd = rtk_9607f_asic_ne_reg_read(NI_HV_GLB_PKT_LEN_CFG);
...
uint32_t rtk_9607f_init(void)
{
	...
	uint32 asic_chip_id, chipSubtype;

	rtk_switch_version_get(&asic_chip_id, &ASICDRIVERVER, &chipSubtype);
	ASICINITPRINT("%s: #####ASICDRIVERVER: %c#####\n", __func__, (ASICDRIVERVER-1)+'A');

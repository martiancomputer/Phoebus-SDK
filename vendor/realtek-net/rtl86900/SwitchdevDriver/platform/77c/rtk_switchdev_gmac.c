...
int rtksw_gmac_sds_polarity_save(struct rtksw_priv *priv, int txPnInverse, int rxPnInverse)
{
	priv->cep.port_cfg.tx_pn_inverse = txPnInverse;
	priv->cep.port_cfg.rx_pn_inverse = rxPnInverse;
	return 0;
}
...
int rtksw_gmac_sds_polarity_set(struct rtksw_priv *priv, int txPnInverse, int rxPnInverse)
{
	...
	ret = aal_sds_txrx_polarity_set(0, priv->emac_port, txPnInverse, rxPnInverse);
	if (ret == CA_E_OK) {
		rtksw_gmac_sds_polarity_save(priv, txPnInverse, rxPnInverse);
	}

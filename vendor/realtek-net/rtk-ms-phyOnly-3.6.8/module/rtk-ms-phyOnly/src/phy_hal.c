...
extern u8 rtkmsphy_get_phy_addr_by_mac_id(u8 mac_id);
    ...
    uint32      *pData)
{
	...
	*/
	...
	ret = aal_mdio_read(unit, CA_AAL_MDIO_ST_CODE_C45, rtkmsphy_get_phy_addr_by_mac_id(port), (ca_uint8_t)mmdAddr, (ca_uint16_t)mmdReg , (ca_uint16_t *)pData);
	if (ret != CA_E_OK) {
    ...
    uint32      data)
{
	...
	*/
	...
	ret = aal_mdio_write(unit, CA_AAL_MDIO_ST_CODE_C45, rtkmsphy_get_phy_addr_by_mac_id(port), mmdAddr, mmdReg, data);
	if (ret != CA_E_OK) {

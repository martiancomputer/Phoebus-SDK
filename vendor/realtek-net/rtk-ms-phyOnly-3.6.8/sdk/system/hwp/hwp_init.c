...
hwp_parsedInfo_buildup(hwp_parsedInfo_t **p, hwp_hwProfile_t *hwp)
{
    ...
    if (_hwp_info_parse(info) != RT_ERR_OK)
    {
        RT_ERR(RT_ERR_FAILED, MOD_INIT, "hwp info parse fail");
    }
 ...
 *      pPhy_ctrl   - pointer to probed rt_phyctrl_t
 * Output:
 *      None
 * Return:
 *      int32       - update count
 ...
 */
int32
hwp_profilePhy_update(uint32 unit, uint32 baseport, rt_phyctrl_t *pPhy_ctrl)
{
    hwp_unitInfo_t      *unitInfo;
    hwp_swDescp_t       *sw;
    uint32              macId;
    ...
    for(j = 0 ; j < sw->port.count ; j++)
    {
        ...
        sw->phy.descp[phy_idx].chip = pPhy_ctrl->phyType;
        updateCnt++;
        break;
    } /* end for */

    /* update eth type for ports in the same PHY */
    for(j = 0 ; j < sw->port.count ; j++)
    {
        macId = sw->port.descp[j].mac_id;
        if (HWP_PHY_BASE_MACID(unit, macId) == baseport)
        {
            sw->port.descp[j].eth = pPhy_ctrl->pPhyInfo->eth_type;
        }
    }

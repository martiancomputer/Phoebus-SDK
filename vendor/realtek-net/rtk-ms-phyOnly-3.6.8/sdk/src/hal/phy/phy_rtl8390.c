...
phy_8390_enable_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
    ...
    RT_ERR_HDL(reg_field_write(unit, reg, CYPRESS_FR0_CFG_FIB_PDOWNf, &val), ERR_HDL, ret);

    if (ENABLED == enable)
    {
        val = 1;
        RT_ERR_HDL(reg_field_write(unit, rxReg, rxField, &val), ERR_HDL, ret);
...
phy_8390_duplex_set(uint32 unit, rtk_port_t port, rtk_port_duplex_t duplex)
{
    ...
    RT_ERR_CHK(reg_field_write(unit, reg, CYPRESS_FR0_CFG_FIB_FULLDUPf, &val), ret);

    RT_ERR_CHK(_phy_8390_reset(unit, port), ret);
    ...
    *pStatus = PHY_FIBER_RX_STATUS_0;
    ...
    if (errCnt >= 2)
    {
        *pStatus = PHY_FIBER_RX_STATUS_1;
        ...
        addr = (48 == port)? 0xb880: 0xb980;

        ioal_mem32_read(unit, addr, &regData);
        ...
        if (((regData >> 18) & 0x1) == 0)
        {
            ...
            regData &= ~(1<<6 | 1<<12);
            regData |= (1<<13);
            ioal_mem32_write(unit, addr, regData);

            osal_time_usleep(200);

            /* change back */
            ioal_mem32_write(unit, addr, ori);
        }
        else
        {
            if (HWP_10GE_PORT(unit, port))

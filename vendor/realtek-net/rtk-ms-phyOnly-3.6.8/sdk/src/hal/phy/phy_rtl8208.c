...
{
    int32   ret = RT_ERR_OK;
#if (defined(CONFIG_SDK_WA_EEE_COMPATIBLE) && defined(CONFIG_SDK_RTL8208D)) || defined(CONFIG_SDK_RTL8208L) || defined(CONFIG_SDK_RTL8208L_VB)
    uint32  phyData;
    ...
    if (   (RT_PHYDRV_RTL8208D == pHalCtrl->pPhy_ctrl[port]->pPhydrv->phydrv_index)
        && (HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_RTL8208D))
    {
...
#if defined(CONFIG_SDK_RTL8208L) || defined(CONFIG_SDK_RTL8208L_VB)
    if ((RT_PHYDRV_RTL8208D == pHalCtrl->pPhy_ctrl[port]->pPhydrv->phydrv_index)
        && ((HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_RTL8208L) || (HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_RTL8208L)))
    {
        /* SS-347 */
        if ((ret = hal_miim_read(unit, port, 5, 16, &phyData)) != RT_ERR_OK)
            return ret;
        if ((ret = hal_miim_write(unit, port, 5, 16, (phyData | (0x1 << 11)))) != RT_ERR_OK)
            return ret;
    }
#endif
...
phy_8208_speed_get(uint32 unit, rtk_port_t port, rtk_port_speed_t *pSpeed)
{
    ...
    bit_6_13_val = (phyData0 & SpeedSelection0_MASK) >> SpeedSelection0_OFFSET;
...
phy_8208_speed_set(uint32 unit, rtk_port_t port, rtk_port_speed_t speed)
{
    ...
    phyData0 = phyData0 | ((bit_6_13_val & 1) << SpeedSelection0_OFFSET);

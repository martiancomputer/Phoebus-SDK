...
phy_8380_autoNegoAbility_set(uint32 unit, rtk_port_t port, rtk_port_phy_ability_t *pAbility)
{
    ...
    if(24 == port)
    {
        reg4_idx = MAPLE_SDS4_FIB_REG4r;
        reg0_idx = MAPLE_SDS4_FIB_REG0r;
    }
    else if(26 == port)
    {
        reg4_idx = MAPLE_SDS5_FIB_REG4r;
        reg0_idx = MAPLE_SDS5_FIB_REG0r;
    }
    else
    {
        return RT_ERR_PORT_ID;
     ...
     if ((ret = reg_write(unit, reg4_idx
                     , &phy_data4)) != RT_ERR_OK)
     {
         RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PORT), "Error Code: 0x%X", ret);
         return ret;
     ...
     if (ENABLED == enable)
     {
                        ...
                        , &phy_data0)) != RT_ERR_OK)
        {
         ...
         if ((ret = reg_write(unit, reg0_idx
                             , &phy_data0)) != RT_ERR_OK)
         {
             RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PORT), "Error Code: 0x%X", ret);
             return ret;
...
phy_8380_polar_get(uint32 unit, rtk_port_t port, rtk_port_phyPolarCtrl_t *pPolarCtrl)
{
    ...
    RT_ERR_CHK(reg_read(unit, sds_reg0[sdsId], &data), ret);

    RT_ERR_CHK(reg_field_get(unit, sds_reg0[sdsId], MAPLE_INV_HSIf, &polar_rx, &data), ret); /* INV_HSI */
    RT_ERR_CHK(reg_field_get(unit, sds_reg0[sdsId], MAPLE_INV_HSOf, &polar_tx, &data), ret); /* INV_HSO */
...
phy_8380_polar_set(uint32 unit, rtk_port_t port, rtk_port_phyPolarCtrl_t *pPolarCtrl)
{
    ...
    RT_ERR_CHK(reg_field_set(unit, sds_reg0[sdsId], MAPLE_INV_HSIf, &polar_rx, &data), ret); /* INV_HSI */
    RT_ERR_CHK(reg_field_set(unit, sds_reg0[sdsId], MAPLE_INV_HSOf, &polar_tx, &data), ret); /* INV_HSO */

    RT_ERR_CHK(reg_write(unit, sds_reg0[sdsId], &data), ret);

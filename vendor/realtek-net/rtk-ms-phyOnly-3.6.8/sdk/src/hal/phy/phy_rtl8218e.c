...
phy_8218e_greenEnable_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
    ...
    {
        RT_ERR_CHK(hal_miim_write(unit, port, 0, 27, 0x806d), ret);
        RT_ERR_CHK(hal_miim_read(unit, port, 0, 28, &data0), ret);
        RT_ERR_CHK(hal_miim_write(unit, port, 0, 27, 0x8071), ret);
...
phy_8218e_eyeMonitorInfo_get(uint32 unit, uint32 port, uint32 sds, uint32 frameNum, rtk_sds_eyeMonInfo_t *pInfo)
{
    ...
    if (maxHeight >= RTK_EYE_MON_ARXIS_Y_MAX)
        pInfo->height = RTK_EYE_MON_ARXIS_Y_MAX;
    else
...
int32
_phy_8218e_ptpFullSec_get(uint32 unit, rtk_port_t port, uint64 partialSec,
    uint32 bitLen, uint64 *pFullSec)
{
    ...
    WAIT_COMPLETE(1000000)
    {
        ...
        if (!(reg_val & RTL8218E_PTP_PTP_TIME_EXEC_REG_FIELD_MASK))
            break;
    }

    if(WAIT_COMPLETE_IS_TIMEOUT())
    {
        RT_ERR(ret, (MOD_HAL|MOD_TIME), "unit %u port %u timeout.", unit, port);
 ...
 */
int32
phy_8218e_ptpFullSec_get(uint32 unit, rtk_port_t port, uint64 partialSec,
    uint32 bitLen, uint64 *pFullSec)
{
    int32   ret;
    ...
    if ((ret = _phy_8218e_ptpFullSec_get(unit, port, partialSec, bitLen, pFullSec)) != RT_ERR_OK)
    {
        RT_ERR(RT_ERR_FAILED, (MOD_HAL|MOD_TIME), "");
        _phy_8218e_ptpReg_access_end(unit, port);
        return ret;
    ...
    return RT_ERR_OK;
}   /* end of phy_8218e_ptpFullSec_get */
...
phy_8218e_ptpTxTimestampFifo_get(uint32 unit, rtk_port_t port, rtk_time_txTimeEntry_t *pTimeEntry)
{
    ...
    partialSec = (reg_val & RTL8218E_PTP_RD_TX_TIMESTAMP_SEC_H_REG_FIELD_MASK) << 2;
    ...
    partialSec |= (reg_val & RTL8218E_PTP_RD_TX_TIMESTAMP_SEC_L_REG_FIELD_MASK) >> 14;
    ...
    if ((ret = _phy_8218e_ptpFullSec_get(unit, port, partialSec, 8, &(pTimeEntry->txTime.sec))) != RT_ERR_OK)
    {
        _phy_8218e_ptpReg_access_end(unit, port);
        return ret;
    }
...
phy_8218e_serdes_linkdown_chk(uint32 unit, rtk_port_t port, uint32 *pStatus)
{
    ...
    uint32  phy_dataC = 0;
    uint32  p = 0;
    uint32  sds0_connected = FALSE;
    uint32  sds1_connected = FALSE;
    ...
    if (0xd0 == (data & 0xF0))
    {
        RT_ERR_HDL(hal_miim_read(unit, port, 0x507, 0x15, &phy_dataA), ERR, ret);
        RT_ERR_HDL(hal_miim_read(unit, port, 0x507, 0x15, &phy_dataB), ERR, ret);
        RT_ERR_HDL(hal_miim_read(unit, port, 0x507, 0x15, &phy_dataC), ERR, ret);
    }
    ...
    *pStatus = 0x0;
    ...
    if (0xd0 == (data & 0xF0))
    {
        ...
        {
            if (HWP_PORT_EXIST(unit, p) && (HWP_PHY_BASE_MACID(unit, p) == port) && (HWP_PORT_SDSID(unit, p) != HWP_NONE))
            {
                sds0_connected = TRUE;
                break;
            }
        }
        for (p=(port+4); p<(port+PORT_NUM_IN_8218E); p++)
        {
            if (HWP_PORT_EXIST(unit, p) && (HWP_PHY_BASE_MACID(unit, p) == port) && (HWP_PORT_SDSID(unit, p) != HWP_NONE))
            {
                sds1_connected = TRUE;
        ...
        if (TRUE == sds0_connected)
        {
            if (((phy_data1 & 0xff) != 0xff) || ((phy_data2 & 0xff) != 0xff))
                *pStatus = 0x1;     /* phy serdes had been linkdown */
        }
        if (TRUE == sds1_connected)
        {
            if (((phy_dataB & 0xff) != 0xff) || ((phy_dataC & 0xff) != 0xff))
                *pStatus = 0x1;     /* phy serdes had been linkdown */
        }
    }
    else
    {
        if (((phy_data1 & 0x1ff) != 0x1ff) || ((phy_data2 & 0x1ff) != 0x1ff))
            *pStatus = 0x1;         /* phy serdes had been linkdown */
    }
...
int32
_phy_8218e_green_init(uint32 unit, rtk_port_t port)
{
    int32   ret = RT_ERR_OK;
    uint32  basePort = 0;
    uint32  data0 = 0;
    ...
    if (HWP_UNIT_VALID_LOCAL(unit))
    {
        ...
        RT_ERR_CHK(hal_miim_write(unit, basePort, 0, 30, 0x8), ret);
        RT_ERR_CHK(hal_miim_write(unit, basePort, 0x360, 0x10, data0), ret);
        RT_ERR_CHK(hal_miim_write(unit, basePort, 0x360, 0x11, data1), ret);
        RT_ERR_CHK(hal_miim_write(unit, basePort, 0x360, 0x12, data2), ret);
        RT_ERR_CHK(hal_miim_write(unit, basePort, 0, 30, 0x0), ret);
    }
...
phy_8218e_init(uint32 unit, rtk_port_t port)
{
    ...
    if ((ret = _phy_8218e_green_init(unit, port)) != RT_ERR_OK)
        return ret;
...
phy_8218Edrv_ge_mapperInit(rt_phydrv_t *pPhydrv)
{
    ...
    pPhydrv->fPhydrv_ptpFullSec_get                     = phy_8218e_ptpFullSec_get;
#endif
}

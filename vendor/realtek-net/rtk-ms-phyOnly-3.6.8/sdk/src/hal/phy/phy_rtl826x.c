...
phy_826x_ctrl_set(uint32 unit, rtk_port_t port, rtk_phy_ctrl_t ctrl_type, uint32 value)
{
    ...
    switch (ctrl_type)
    {
        ...
        case RTK_PHY_CTRL_AN_ARBITRATION:
            if ((ret = phy_common_general_reg_mmd_get(unit, port, 31, 0xa008, &phyData)) != RT_ERR_OK)
                return ret;
            phyData &= (~BIT_13);
            phyData |= (value == 0) ? (0) : (BIT_13);
            if ((ret = phy_common_general_reg_mmd_set(unit, port, 31, 0xa008, phyData)) != RT_ERR_OK)
                return ret;
            break;

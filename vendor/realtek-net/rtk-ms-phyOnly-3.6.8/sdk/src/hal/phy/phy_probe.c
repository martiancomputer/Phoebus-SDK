    ...
    if (!RTK_PORTMASK_IS_ALL_ZERO(phyUpdatePmsk))
    {
        HWP_PORT_TRAVS_EXCEPT_CPU(unit, port)
        {
            if (RTK_PORTMASK_IS_PORT_SET(phyUpdatePmsk, port))
            {
                ...
                hwp_profilePhy_update(unit, port, pHalCtrl->pPhy_ctrl[port]);
                update_hwp = TRUE;
            }
        }
    }
...
_phy_c45_check(uint32 unit, uint32 port)
{
    ...
    if ((ret = MACDRV(pHalCtrl)->fMdrv_miim_mmd_read(unit, port, 30, 32771, &data)) == RT_ERR_OK)
    {
        /* 8295R's reg2 is 0, further check subid  */
        if (data == 0x8295)
        {
            return RT_ERR_OK;
        }

...
rt_phyInfo_t phy_8224_info =
{
    ...
    .eth_type   = HWP_2_5GE,
    .isComboPhy = { 0, 0, 0, 0 },
    .flags      = RTK_PHYINFO_FLAG_NONE | RTK_PHYINFO_FLAG_NO_RES_REG | RTK_PHYINFO_FLAG_1G_MMD_CFG,
...
uint32 port_tapbin_reg[RTK_MAX_NUM_OF_DEV][RTK_MAX_PORT_PER_UNIT];
uint32 port_tapbin_field[RTK_MAX_NUM_OF_DEV][RTK_MAX_PORT_PER_UNIT];
...
_phy_8224_interrupt_init(uint32 unit, rtk_port_t port)
{
    ...
    base_port = HWP_PHY_BASE_MACID(unit, port);
    if ((ret = hwp_get_offset_by_baseport_port(unit, base_port, port, &port_offset)) != RT_ERR_OK)
        return ret;
    ...
    if (port == base_port)
    {
        ...
        if ((ret = _phy_8224_intReg_set(unit, port, RTL8224_IMR_EXT_RLFD, RTL8224_INTER_REG_MSB, RTL8224_INTER_REG_LSB, 0)) != RT_ERR_OK)
            return ret;
        if ((ret = _phy_8224_intReg_set(unit, port, RTL8224_IMR_INT_RLFD, RTL8224_INTER_REG_MSB, RTL8224_INTER_REG_LSB, 0)) != RT_ERR_OK)
            return ret;
        /* Enable RLFD IMR */
...
int32
_phy_8224_led_init(uint32 unit, rtk_port_t port)
{
    int32   ret = RT_ERR_OK;
    uint32  phyData;
    rtk_port_t base_port;
    ...
    /* Set perPort IO pad to LED mode */
    for(led_idx = 0; led_idx <= RTL8224_LED_PAD_MAX_INDEX; led_idx++)
        if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_IO_MUX_SEL_0_R, led_idx, led_idx, 0x0)) != RT_ERR_OK)
            return ret;
    /* Set SYS LED IO pad to LED mode */
    if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_IO_MUX_SEL_0_R, RTL8224_GLB_SYS_LED_EN_OFFSET, RTL8224_GLB_SYS_LED_EN_OFFSET, 0x1)) != RT_ERR_OK)
        return ret;
    /* Set RLFD LED IO pad to LED mode */
    if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_IO_MUX_SEL_0_R, RTL8224_GLB_RLDP_LED_EN_OFFSET, RTL8224_GLB_RLDP_LED_EN_OFFSET, 0x1)) != RT_ERR_OK)
        return ret;

    /* Set all LED IO function to disanle at first */
    if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_LED_GLB_IO_EN_R, RTL8224_LED_PAD_EN_OFFSET, 0, 0x0)) != RT_ERR_OK)
        return ret;

    /* Reserve all LED PAD */
    phyData = 0x3fffffff;
    if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_LED_GLB_MUX_1, RTL8224_PAD_ALL_RESERVED_MSB, RTL8224_PAD_ALL_RESERVED_LSB, phyData)) != RT_ERR_OK)
        return ret;
    if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_LED_GLB_MUX_2, RTL8224_PAD_ALL_RESERVED_MSB, RTL8224_PAD_ALL_RESERVED_LSB, phyData)) != RT_ERR_OK)
        return ret;
    if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_LED_GLB_MUX_3, RTL8224_PAD_ALL_RESERVED_MSB, RTL8224_PAD_ALL_RESERVED_LSB, phyData)) != RT_ERR_OK)
        return ret;
    if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_LED_GLB_MUX_4, RTL8224_PAD_ALL_RESERVED_MSB, RTL8224_PAD_ALL_RESERVED_LSB, phyData)) != RT_ERR_OK)
        return ret;
    if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_LED_GLB_MUX_5, RTL8224_PAD_ALL_RESERVED_MSB, RTL8224_PAD_ALL_RESERVED_LSB, phyData)) != RT_ERR_OK)
        return ret;
    phyData = 0x0003ffff;
    if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_LED_GLB_MUX_6, RTL8224_PAD_ALL_RESERVED_MSB, RTL8224_PAD_ALL_RESERVED_LSB, phyData)) != RT_ERR_OK)
        return ret;

    /* Access LED 1/2/7/8 to P0/1/2/3 LED0 */
    phyData = 0x00;
    if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_LED_GLB_MUX_1, RTL8224_PAD_LED1_MUX_MSB, RTL8224_PAD_LED1_MUX_LSB, phyData)) != RT_ERR_OK)
        return ret;
    phyData = 0x04;
    if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_LED_GLB_MUX_1, RTL8224_PAD_LED2_MUX_MSB, RTL8224_PAD_LED2_MUX_LSB, phyData)) != RT_ERR_OK)
        return ret;
    phyData = 0x08;
    if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_LED_GLB_MUX_2, RTL8224_PAD_LED7_MUX_MSB, RTL8224_PAD_LED7_MUX_LSB, phyData)) != RT_ERR_OK)
        return ret;
    phyData = 0x0c;
    if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_LED_GLB_MUX_2, RTL8224_PAD_LED8_MUX_MSB, RTL8224_PAD_LED8_MUX_LSB, phyData)) != RT_ERR_OK)
        return ret;

    /* Enable all LED IO function after confgiure */
    if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_LED_GLB_IO_EN_R, RTL8224_LED_PAD_EN_OFFSET, 0, 0x7fffffff)) != RT_ERR_OK)
        return ret;
...
/* Function Name:
 *      phy_8224_sdsAllAm_set
 * Description:
 *      clear alignment marker0-3, and each AM has m0-3
 * Input:
 *      unit      - unit id
 *      port      - port id
 *      value     - m0/m1/m2 value
 * Output:
 *      NA
 * Return:
 *      RT_ERR_OK     - OK
...
/* Function Name:
 *      phy_8224_sdsExtra_set
 * Description:
 *      SerDes setting for Realtek MAC
 * Input:
 *      unit      - unit id
 *      port      - port id
 * Output:
...
/* Function Name:
 *      phy_8224_portPatch_backup
 * Description:
 *      Backup PHY port patch.
 * Input:
 *      unit    - unit id
 * Output:
 *      None
 ...
 */
int32
phy_8224_portPatch_backup(uint32 unit)
{
    uint32  port = 0, regVal = 0;
    uint32  field;
    ...
    HWP_PORT_TRAVS(unit, port)
    {
        switch (HWP_PHY_MODEL_BY_PORT(unit, port))
        {
            ...
            case RTK_PHYTYPE_RTL8224N:
                hal_miim_mmd_write(unit, port, 31, 0xa436, 0x8377);
                hal_miim_mmd_read(unit, port, 31, 0xa438, &regVal);
                port_tapbin_reg[unit][port] = regVal;
                field = ((regVal & RTL8224_TAPBIN_DEF_BITMASK) >> RTL8224_TAPBIN_DEF_OFFSET);
                port_tapbin_field[unit][port] = field;
                break;
            default:
                break;
        }
...
/* Function Name:
 *      phy_8224_portPatch_restore
 * Description:
 *      Restore PHY port patch.
 * Input:
 *      unit    - unit id
 * Output:
 *      None
 ...
 */
int32
phy_8224_portPatch_restore(uint32 unit)
{
    uint32  port = 0, regVal = 0;
    uint32  field;
    ...
    HWP_PORT_TRAVS(unit, port)
    {
        switch (HWP_PHY_MODEL_BY_PORT(unit, port))
        {
            ...
            case RTK_PHYTYPE_RTL8224N:
                hal_miim_mmd_write(unit, port, 31, 0xa436, 0x8377);
                hal_miim_mmd_write(unit, port, 31, 0xa438, port_tapbin_reg[unit][port]);
                field = port_tapbin_field[unit][port];
                regVal = ((field << 12) | (field << 8) | (field << 4) | (field)); 
                hal_miim_mmd_write(unit, port, 31, 0xbce0, regVal);
                break;
            default:
                break;
        }
...
phy_8224_init(uint32 unit, rtk_port_t port)
{
    ...
    /* Initial LED pad and mapping */
    if ((ret = _phy_8224_led_init(unit, port)) != RT_ERR_OK)
        return ret;
...
phy_8224_intrEnable_set(uint32 unit, rtk_port_t port, rtk_phy_intr_status_t phyIntr, rtk_enable_t enable)
{
    ...
    switch (phyIntr)
    {
        ...
        case RTK_PHY_INTR_STATUS_RLFD:
            bit_idx = ((RTL8224_IMR_EXT_RLFD_PORT_8_0_OFFSET + port_offset) + RTL8224_IMR_EXT_RLFD_PORT_SHIFT);
...
phy_8224_intrStatus_get(uint32 unit, rtk_port_t port, rtk_phy_intr_t phyIntr, rtk_phy_intrStatusVal_t *pStatus)
{
    ...
    switch (phyIntr)
    {
        ...
        case RTK_PHY_INTR_RLFD:
            ret = _phy_8224_intReg_get(unit, port, RTL8224_ISR_EXT_TM_RLFD_REG, (RTL8224_ISR_EXT_RLFD_PORT_SHIFT + 3), RTL8224_ISR_EXT_RLFD_PORT_SHIFT, &phyData);
            ret = _phy_8224_intReg_set(unit, port, RTL8224_ISR_EXT_TM_RLFD_REG, (RTL8224_ISR_EXT_RLFD_PORT_SHIFT + 3), RTL8224_ISR_EXT_RLFD_PORT_SHIFT, phyData);
            if (phyData != 0)
                status |= RTK_PHY_CTRL_INTR_RLFD;
            pStatus->statusType = RTK_PHY_INTR_STATUS_TYPE_PORT_BITMAP;
            pStatus->statusValue = phyData;
            break;
...
phy_8224_ptp_portRefTime_get(uint32 unit, rtk_port_t port, rtk_time_timeStamp_t *pTimeStamp)
{
    ...
    if ((ret = _phy_8224_intReg_set(unit, base_port, RTL8224_PTP_TIME_CTRL_R, RTL8224_PTP_TIME_CMD_MSB, RTL8224_PTP_TIME_CMD_LSB, RTL8224_PTP_TIME_CMD_READ)) != RT_ERR_OK)
        return ret;

    if ((ret = _phy_8224_intRegBit_set(unit, base_port, RTL8224_PTP_TIME_CTRL_R, RTL8224_PTP_TIME_EXEC_OFFSET, 1)) != RT_ERR_OK)
        return ret;

    WAIT_COMPLETE(1000000)
    {
        if ((ret = _phy_8224_intRegBit_get(unit, base_port, RTL8224_PTP_TIME_CTRL_R, RTL8224_PTP_TIME_EXEC_OFFSET, &rData)) != RT_ERR_OK)
            return ret;
        if (rData == 0)
            break;
    }
    ...
    if ((ret = _phy_8224_intReg_get(unit, base_port, RTL8224_PTP_TIME_SEC_RD0_R, RTL8224_RD_PTP_TIME_SEC_L_MSB, RTL8224_RD_PTP_TIME_SEC_L_LSB, &sec_l)) != RT_ERR_OK)
        return ret;
    if ((ret = _phy_8224_intReg_get(unit, base_port, RTL8224_PTP_TIME_SEC_RD1_R, RTL8224_RD_PTP_TIME_SEC_M_MSB, RTL8224_RD_PTP_TIME_SEC_M_LSB, &sec_m)) != RT_ERR_OK)
        return ret;
    if ((ret = _phy_8224_intReg_get(unit, base_port, RTL8224_PTP_TIME_SEC_RD2_R, RTL8224_RD_PTP_TIME_SEC_H_MSB, RTL8224_RD_PTP_TIME_SEC_H_LSB, &sec_h)) != RT_ERR_OK)
        return ret;
    if ((ret = _phy_8224_intReg_get(unit, base_port, RTL8224_PTP_TIME_NSEC_RD0_R, RTL8224_RD_PTP_TIME_NSEC_L_MSB, RTL8224_RD_PTP_TIME_NSEC_L_LSB, &nsec_l)) != RT_ERR_OK)
        return ret;
    if ((ret = _phy_8224_intReg_get(unit, base_port, RTL8224_PTP_TIME_NSEC_RD0_R, RTL8224_RD_PTP_TIME_NSEC_H_MSB, RTL8224_RD_PTP_TIME_NSEC_H_LSB, &nsec_h)) != RT_ERR_OK)
        return ret;
 ...
 */
int32
phy_8224_ptp_fullSec_get(uint32 unit, rtk_port_t port, uint64 partialSec,
    uint32 bitLen, uint64 *pFullSec)
{
    ...
    uint32     sec_m = 0;
    rtk_port_t base_port = 0;
    rtk_time_timeStamp_t curTime;
    uint64 upperSecMsk = 0;
    ...
    curTime.sec = ((uint64)sec_h << 32) | ((uint64)sec_m << 16) | ((uint64)sec_l & 0xFFFF);

    partialSec = partialSec & ((((uint64)1) << bitLen) - 1);
    upperSecMsk = 0xFFFFFFFFFFFFFFFF - ((((uint64)1) << bitLen) - 1);

    if((curTime.sec & (((uint64)1 << bitLen) - 1)) >= partialSec)
        *pFullSec = (curTime.sec & upperSecMsk) | partialSec;
    else
        *pFullSec = ((curTime.sec & upperSecMsk) | partialSec) - ((uint64)1 << bitLen);
...
phy_8224_ptp_portRefTimeFreq_set(uint32 unit, rtk_port_t port, uint32 freq, uint32 apply)
{
    ...
    if ((ret = _phy_8224_intRegBit_get(unit, port, RTL8224_PTP_CLK_CTRL_R, RTL8224_CFG_CLK_SRC_OFFSET, &rData)) != RT_ERR_OK)
        return ret;
    value = (rData == 0) ? RTK_PHY_CTRL_PTP_CLOCK_SRC_INT : RTK_PHY_CTRL_PTP_CLOCK_SRC_EXT;

    if(value == RTK_PHY_CTRL_PTP_CLOCK_SRC_INT)
    {
        if((freq < RTL8224_REFTIMEFREQ_INTCLK_MIN) || (freq > RTL8224_REFTIMEFREQ_INTCLK_MAX))
            return RT_ERR_INPUT;
    }else{
        if((freq < RTL8224_REFTIMEFREQ_EXTCLK_MIN) || (freq > RTL8224_REFTIMEFREQ_EXTCLK_MAX))
            return RT_ERR_INPUT;
    }
    ...
    WAIT_COMPLETE(1000000)
    {
        if ((ret = _phy_8224_intRegBit_get(unit, port, RTL8224_PTP_APPLY_FREQ_R, RTL8224_APPLY_FREQ, &rData)) != RT_ERR_OK)
            return ret;
        if (rData == 0)
            break;
    }
...
phy_8224_ptp_portPtpLatchTime_get(uint32 unit, rtk_port_t port, rtk_time_timeStamp_t *pLatchTime)
{
    ...
    if ((ret = _phy_8224_intReg_get(unit, port, RTL8224_PTP_TIME_NSEC_RD0_R, RTL8224_RD_PTP_TIME_NSEC_L_MSB, RTL8224_RD_PTP_TIME_NSEC_L_LSB, &nsec_l)) != RT_ERR_OK)
        return ret;
    if ((ret = _phy_8224_intReg_get(unit, port, RTL8224_PTP_TIME_NSEC_RD1_R, RTL8224_RD_PTP_TIME_NSEC_H_MSB, RTL8224_RD_PTP_TIME_NSEC_H_LSB, &nsec_h)) != RT_ERR_OK)
        return ret;
...
phy_8224_rtct_start(uint32 unit, rtk_port_t port)
{
    ...
    phy_common_c45_speedDuplexStatusResReg_get(unit, port, &speed, &duplex);
    if (phyData & BIT_2)
    {
...
phy_8224_ctrl_set(uint32 unit, rtk_port_t port, rtk_phy_ctrl_t ctrl_type, uint32 value)
{
    ...
    switch (ctrl_type)
    {
        ...
        case RTK_PHY_CTRL_LED_4_CFG_FORCE:
            ...
            /* active software led setting*/ 
            if ((ret = _phy_8224_intReg_set(unit, port, RTL8224_SW_LED_LOAD, RTL8224_SW_LED_LOAD_MSB, RTL8224_SW_LED_LOAD_LSB, 1)) != RT_ERR_OK)
                return ret;
            break;
...
phy_8224drv_mapperInit(rt_phydrv_t *pPhydrv)
{
    ...
    pPhydrv->fPhydrv_ptpFullSec_get                = phy_8224_ptp_fullSec_get;
#endif

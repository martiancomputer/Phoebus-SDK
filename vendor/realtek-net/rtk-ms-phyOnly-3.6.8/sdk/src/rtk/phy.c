    ...
    rtk_time_oper_t oper)
{
    ...
    return RT_MAPPER(unit)->time_portPtpBcastOper_set(unit, portmask, oper);
}   /* end of rtk_time_portPtpBcastOper_set */
    ...
    uint32 bitLen, uint64 *pFullSec)
{
    ...
    return RT_MAPPER(unit)->time_portPtpFullSec_get(unit, port, partialSec, bitLen, pFullSec);
}   /* end of rtk_time_portPtpFullSec_get */
...
rtk_phy_debug_patch_check(uint32 unit, rtk_port_t port)
{
    ...
    return RT_MAPPER(unit)->phy_debug_patch_check(unit, port);
}   /* end of rtk_phy_debug_patch_check */

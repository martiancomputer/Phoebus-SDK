...
dal_phyMapper_init(dal_mapper_t *pMapper)
{
    ...
    pMapper->time_portPtpFullSec_get = dal_phy_time_portPtpFullSec_get;
	pMapper->time_portPtpBcastOper_set = dal_phy_time_portPtpBcastOper_set;

    pMapper->phy_debug_patch_check = dal_phy_phy_debug_patch_check;
    return RT_ERR_OK;
}
...
dal_phy_init(uint32 unit)
{
    ...
    HWP_PORT_TRAVS_EXCEPT_CPU(unit, port)
    {
        ...
        if ((HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_RTL8261B)
            || (HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_RTL8261BE)
            || (HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_RTL8264B)
            || (HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_RTL8254B)
            || (HWP_PHY_MODEL_BY_PORT(unit, port) == RTK_PHYTYPE_RTL8251L_REV3))
        {
            pPhy_info[unit]->force_mode_speed[port] = PORT_SPEED_100M;
        }
    ...
    for(port_index = fiberRxOrder[unit]; (port_index < port_num) && (chk_flag < FIBER_RX_WATCHDOG_CHK_PORT_NUM); port_index++)
    {
        ...
        if ((HWP_COMBO_PORT(unit, port_index) || HWP_FIBER_PORT(unit, port_index)))
        {
                    ...
                    (phyModel == RTK_PHYTYPE_RTL8224QF))
            {
                ...
                ret = phy_media_get(unit, port_index, &portMedia);
                if(ret != RT_ERR_OK)
                    continue;
...
dal_phy_cableESDPhyIdNext_get(uint32 unit, int32 phyIdx, int32 *nextPhyIdx, uint32 wrapAround)
{
    ...
    for (i = 0; i < phyCnt; i++)
    {
        ...
        for(port = HWP_PHY_BASE_MACID_BY_IDX(unit, idx);
            port < RTK_MAX_PORT_PER_UNIT;
            port++)
        {
            /* consider port number of the PHY is discontinuous */
            if (!HWP_PORT_EXIST(unit, port))
            {
                continue;
            }
            if (HWP_PHY_BASE_MACID(unit, port) != HWP_PHY_BASE_MACID_BY_IDX(unit, idx))
            {
                break;
            }
...
dal_phy_portRefTime_set(uint32 unit, rtk_portmask_t portmask, rtk_time_timeStamp_t timeStamp, uint32 exec)
{
    ...
    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PHY), "unit=%d,portmask[1]=0x%x,portmask[0]=0x%x,timeStamp.sec=%llu,timeStamp.nsec=%d", unit, portmask.bits[1], portmask.bits[0], timeStamp.sec, timeStamp.nsec);
...
dal_phy_portRefTimeAdjust_set(uint32 unit, rtk_portmask_t portmask, uint32 sign, rtk_time_timeStamp_t timeStamp, uint32 exec)
{
    ...
    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PHY), "unit=%d,portmask=0x%x 0x%x,timeStamp.sec=%llu,timeStamp.nsec=%d", unit, portmask.bits[1], portmask.bits[0], timeStamp.sec, timeStamp.nsec);
...
dal_phy_portIntrStatus_get(uint32 unit, rtk_port_t port, rtk_phy_intr_t phyIntr, rtk_phy_intrStatusVal_t *pStatus)
{
    ...
    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PHY), "pStatus->statusType =%d, pStatus->statusValue =%d", pStatus->statusType, pStatus->statusValue);
...
dal_phy_portSdsLeq_set(uint32 unit, rtk_port_t port, uint32 sdsId, rtk_enable_t manual_en, uint32 leq_val)
{
    ...
    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PHY), "unit=%d, port=%d, sdsId=%d, enable=%d leq=%d",
...
_dal_phy_debug_batch_entry_print(uint32 idx, rtk_phy_batch_entry_t *entry)
{
    ...
    if(entry->para_num >= (sizeof(entry->para)/sizeof(uint32)))
        return;
...
/* Function Name:
 *      dal_phy_time_portPtpFullSec_get
 * Description:
 ...
 */
int32
dal_phy_time_portPtpFullSec_get(uint32 unit, rtk_port_t port, uint64 partialSec,
    uint32 bitLen, uint64 *pFullSec)
{
    int32   ret;

    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PHY), "unit=%d,port=%d,partialSec=%llu, bitLen=%d", unit, port, partialSec, bitLen);
    ...
    /* parameter check */
    RT_PARAM_CHK(!HWP_PORT_EXIST(unit, port), RT_ERR_PORT_ID);
    RT_PARAM_CHK((0xFFFFFFFFFFFF <= partialSec), RT_ERR_INPUT);
    RT_PARAM_CHK((NULL == pFullSec), RT_ERR_NULL_POINTER);
    /* function body */
    PHY_SEM_LOCK(unit);
    if ((ret = phy_ptpFullSec_get(unit, port, partialSec, bitLen, pFullSec)) != RT_ERR_OK)
    {
        PHY_SEM_UNLOCK(unit);
        RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PHY), "Error Code: 0x%X", ret);
        return ret;
    ...
    return RT_ERR_OK;
}   /* end of dal_phy_time_portPtpFullSec_get */

/* Function Name:
 *      dal_phy_phy_debug_patch_check
 * Description:
 *      Check chip initial patch, print mismatch registers
 * Input:
 *      unit        - unit id
 *      port        - port id
 * Output:
 ...
 */
int32
dal_phy_phy_debug_patch_check(uint32 unit, rtk_port_t port)
{
    int32   ret;
    uint8   port_offset = 0;
    ...
    RT_ERR_CHK(hwp_get_offset_by_baseport_port(unit, HWP_PHY_BASE_MACID(unit, port), port, &port_offset), ret);

    /* function body */
    PHY_SEM_LOCK(unit);

    if ((ret = phy_patch(unit, port, port_offset, PHY_PATCH_MODE_CMP)) != RT_ERR_OK)
    {
        PHY_SEM_UNLOCK(unit);
        RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PHY), "Error Code: 0x%X", ret);
        return ret;
    ...
    return RT_ERR_OK;
}   /* end of dal_phy_phy_debug_patch_check */

/* Function Name:
 *      dal_phy_time_portPtpBcastOper_set
 * Description:
 ...
 */
int32
dal_phy_time_portPtpBcastOper_set(uint32 unit, rtk_portmask_t portmask,
    rtk_time_oper_t oper)
{
    int32   ret;

    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PHY), "unit=%d,portmask=%x %x,oper=%d", unit, portmask.bits[1], portmask.bits[0], oper);
    ...
    RT_PARAM_CHK((oper >= TIME_OPER_END), RT_ERR_INPUT);

    /* function body */
    PHY_SEM_LOCK(unit);

    if ((ret = phy_ptpBcastOper_set(unit, portmask, oper)) != RT_ERR_OK)
    {
        PHY_SEM_UNLOCK(unit);
        RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PHY), "Error Code: 0x%X", ret);
        return ret;

 ...
 *
 * $Revision: 79443 $
 * $Date: 2017-06-06 20:35:43 +0800 (?▒ф?, 06 ?нц? 2017) $
 *
...
#if defined(CONFIG_FC_RTL8198F_SERIES)&&defined(CONFIG_RTK_DEV_AP)
//set igmp trap to cpu  in  8367 lan port 
int rtk_l2_8367_igmp_protocol_init(void)
{
	...
	// DIP + SIP lookup
	retVal = rtk_l2_ipMcastAddrLookup_set(LOOKUP_IP);
	if(retVal != RT_ERR_OK)
			return retVal;	
	
	for(i= 0; i< PROTOCOL_END; i++)
	{
		retVal = rtk_igmp_protocol_set(UTP_PORT0, i, IGMP_ACTION_TRAP2CPU);
		if(retVal != RT_ERR_OK)
			return retVal;	
		retVal = rtk_igmp_protocol_set(UTP_PORT1, i, IGMP_ACTION_TRAP2CPU);
		
		if(retVal != RT_ERR_OK)
			return retVal;	
		
		retVal = rtk_igmp_protocol_set(UTP_PORT2, i, IGMP_ACTION_TRAP2CPU);
		if(retVal != RT_ERR_OK)
			return retVal;	
	}
	printk("\n\n--- rtk_l2_8367_igmp_protocol_init success\n\n");
    return RT_ERR_OK;
}


EXPORT_SYMBOL(rtk_l2_8367_igmp_protocol_init);

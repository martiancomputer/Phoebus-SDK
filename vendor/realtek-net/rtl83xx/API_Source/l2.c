 ...
 *
 * $Revision: 82894 $
 * $Date: 2017-10-24 14:47:07 +0800 (?±‰?, 24 ?ÅÊ? 2017) $
 *
...
#if defined(CONFIG_FC_RTL8198F_SERIES)&&defined(CONFIG_RTK_DEV_AP)
int rtk_l2_8367_mcastAddr_add(char *Dmac,u32 portmask, u32 vid)
{
	int retVal;
	rtk_l2_mcastAddr_t McastAddr;
	rtk_portmask_t LogicalPmask;
	u32  pvid = 0, priority = 0 , membermsk = 0, untagmsk = 0, phy_port_msk = 0;
	unsigned short pri = 0, fid = 0;

	// if vid = 0, get pid from 8367 cpu port  pvid
	if(vid == 0)
	{
		if ((retVal = rtk_vlan_portPvid_get(EXT_PORT0, &pvid, &priority)) != 0)
		{
			//printk("---retVal = %d , pvid %d  EXT_PORT0=%d  fid %u\n",retVal, pvid, EXT_PORT0, fid);	
			return retVal ;
		}
		
		
		if ((retVal = rtl83xx_vlan_get(pvid, &membermsk, &untagmsk, &pri, &fid)) != 0)
		{
			//printk("---retVal =%d, pvid %d Mbrmsk 0x%x Untagmsk 0x%x EXT_PORT0=%d  fid %u\n",retVal, pvid, membermsk, untagmsk, EXT_PORT0, fid);	
			return retVal;
		}
	}
	//get pid from vlan id
	else
	{
		pvid = vid;
	...
	rtk_switch_portmask_P2L_get(portmask, &LogicalPmask);
	
	memset(&McastAddr, 0, sizeof(rtk_l2_mcastAddr_t));

	if(Dmac)
		memcpy((void*)&(McastAddr.mac.octet),Dmac ,sizeof(McastAddr.mac.octet));
	
	McastAddr.portmask = LogicalPmask ;
	McastAddr.fid = fid;

	retVal = rtk_l2_mcastAddr_add(&McastAddr);

    return retVal;
}

int rtk_l2_8367_McastAddr_get(char *Dmac ,rtk_portmask_t* portmask)
{
	int retVal;
	rtk_l2_mcastAddr_t mcastAddr;
	...
	retVal = rtk_l2_ipMcastAddr_get(&mcastAddr);

	if(RT_ERR_OK == retVal)
	{
		*portmask = mcastAddr.portmask ;
	}
	
	if(retVal == RT_ERR_L2_ENTRY_NOTFOUND)
		printk("not fund l2 mc entry ,DMAC =0x%x-%x-%x-%x-%x-%x \n",mcastAddr.mac.octet[0],mcastAddr.mac.octet[1],mcastAddr.mac.octet[2],mcastAddr.mac.octet[3],mcastAddr.mac.octet[4],mcastAddr.mac.octet[5]);


    return retVal;
}

int rtk_l2_8367_mcastAddr_del(char *Dmac, u32 vid)
{
	int retVal;
	rtk_l2_mcastAddr_t McastAddr;
	u32  pvid = 0, priority = 0 , membermsk = 0, untagmsk = 0;
	unsigned short pri = 0, fid = 0;
	
	memset(&McastAddr, 0, sizeof(rtk_l2_mcastAddr_t));

	if(Dmac)
		memcpy((void*)&(McastAddr.mac),Dmac ,sizeof(McastAddr.mac));
	...
	retVal = rtk_l2_mcastAddr_del(&McastAddr);

    return retVal;
}

int rtk_l2_8367_trapUnkownMC_froward(void)
{
	int retVal;
	rtk_portmask_t Flood_portmask;
	rtk_uint32 logicalPort ;
	rtk_uint32 physicalPort ;
	rtk_uint32 physicalPortmask;
	rtk_portmask_t LogicalPmask;

	physicalPort = 6 ;
	logicalPort =  rtk_switch_port_P2L_get(physicalPort);
	//printk("----phy 6 port ,get logical port = %d\n",logicalPort);
	
	retVal=rtk_trap_unknownMcastPktAction_set(logicalPort,1,1); 
	if(retVal != RT_ERR_OK)
		return retVal;

	physicalPortmask = 0x00;
	rtk_switch_portmask_P2L_get(physicalPortmask, &LogicalPmask);
	//printk("---physicalPortmask =0x%x get LogicalPmask =0x%x\n",physicalPortmask,LogicalPmask);

	rtk_l2_floodPortMask_set(FLOOD_UNKNOWNMC,&LogicalPmask);

	return retVal;
}

int rtk_l2_8367_ipMcastAddr_add(u32 dip, u32 sip, u32 portmask)
{
	int retVal;
	...
	//printk("rtk_l2_8367_ipMcastAddr_add, portmask =0x%x,LogicalPmask=0x%x\n",portmask,LogicalPmask.bits[0]);
	memset(&IpMcastAddr, 0, sizeof(IpMcastAddr));
	IpMcastAddr.dip = dip;
	IpMcastAddr.sip = sip;
	IpMcastAddr.portmask = LogicalPmask ;
	IpMcastAddr.fwd_pri_en = 1;
	IpMcastAddr.priority = 1;

	retVal = rtk_l2_ipMcastAddr_add(&IpMcastAddr);
	//printk("%s: dip = 0x%x,sip =0x%x ,portmask =0x%x ,retval =%d \n",__FUNCTION__,dip,sip,portmask,retVal);
    return retVal;
}

int rtk_l2_8367_ipMcastAddr_get(u32 dip, u32 sip ,rtk_portmask_t* portmask)
{
	int retVal;
	rtk_l2_ipMcastAddr_t IpMcastAddr;
	...
	retVal = rtk_l2_ipMcastAddr_get(&IpMcastAddr);
	
	//if(retVal == RT_ERR_L2_ENTRY_NOTFOUND)
		//printk("--not fund l2 mc entry ,SIP =0x%x, DIP =0x%x \n",sip,dip);

	if(RT_ERR_OK == retVal)
	{
		*portmask = IpMcastAddr.portmask ;
	}
    return retVal;
}


int rtk_l2_8367_ipMcastAddr_del(u32 dip, u32 sip)
{
	int retVal;
	rtk_l2_ipMcastAddr_t IpMcastAddr;
	memset(&IpMcastAddr, 0, sizeof(IpMcastAddr));
	...
	retVal = rtk_l2_ipMcastAddr_del(&IpMcastAddr);
	//if(RT_ERR_L2_ENTRY_NOTFOUND == retVal)
		//printk("%s:%d , not find l2 table entry , dip =0x%x, sip =0x%x\n",__FUNCTION__,__LINE__,dip,sip);

	return retVal;
}


EXPORT_SYMBOL(rtk_l2_8367_ipMcastAddr_del);
EXPORT_SYMBOL(rtk_l2_8367_ipMcastAddr_get);
EXPORT_SYMBOL(rtk_l2_8367_ipMcastAddr_add);
EXPORT_SYMBOL(rtk_l2_8367_trapUnkownMC_froward);
EXPORT_SYMBOL(rtk_l2_8367_mcastAddr_del);
EXPORT_SYMBOL(rtk_l2_8367_McastAddr_get);
EXPORT_SYMBOL(rtk_l2_8367_mcastAddr_add);
EXPORT_SYMBOL(rtk_l2_ipMcastGroupEntry_add);
EXPORT_SYMBOL(rtk_l2_ipMcastGroupEntry_del);

 ...
 *
 * $Revision: 82056 $
 * $Date: 2017-09-08 11:41:31 +0800 (?±‰?, 08 ‰πùÊ? 2017) $
 *
...
static rtk_api_ret_t _rtk_port_adminEnable_get(rtk_port_t port, rtk_enable_t *pEnable)
{
    ...
    if ((retVal = rtk_83xx_port_phyReg_get(port, PHY_CONTROL_REG, &data)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtk_api_ret_t rtk_83xx_port_phyAutoNegoAbility_set(rtk_port_t port, rtk_port_phy_ability_t *pAbility)
{
    rtk_api_ret_t retVal;
...
EXPORT_SYMBOL(rtk_83xx_port_phyAutoNegoAbility_set);
 ...
 */
rtk_api_ret_t rtk_83xx_port_phyAutoNegoAbility_get(rtk_port_t port, rtk_port_phy_ability_t *pAbility)
{
    rtk_api_ret_t retVal;
...
EXPORT_SYMBOL(rtk_83xx_port_phyAutoNegoAbility_get);
...
EXPORT_SYMBOL(rtk_port_macForceLink_set);
...
EXPORT_SYMBOL(rtk_port_macForceLink_get);
...
EXPORT_SYMBOL(rtk_port_macStatus_get);
 ...
 */
rtk_api_ret_t rtk_83xx_port_phyReg_set(rtk_port_t port, rtk_port_phy_reg_t reg, rtk_port_phy_data_t regData)
{
    rtk_api_ret_t retVal;
...
EXPORT_SYMBOL(rtk_83xx_port_phyReg_set);
 ...
 */
rtk_api_ret_t rtk_83xx_port_phyReg_get(rtk_port_t port, rtk_port_phy_reg_t reg, rtk_port_phy_data_t *pData)
{
    rtk_api_ret_t retVal;
...
EXPORT_SYMBOL(rtk_83xx_port_phyReg_get);

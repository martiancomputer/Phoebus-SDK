 ...
 *
 * $Revision: 79429 $
 * $Date: 2017-06-06 15:54:55 +0800 (?▒ф?, 06 ?нц? 2017) $
 *
...
#if defined(CONFIG_RTL_8226_SUPPORT) && defined(CONFIG_RTK_DEV_AP)
...
rtk_api_ret_t rtk_83xx_qos_init(rtk_queue_num_t queueNum)
{
	rtk_api_ret_t retVal;
	
    if ((retVal = rtk_qos_init(queueNum)) != RT_ERR_OK){
		goto done;
	}

#if defined(CONFIG_RTL_8226_SUPPORT) && defined(CONFIG_RTK_DEV_AP)
	rtk_get_8226_lan_dev_name();
#endif
...
EXPORT_SYMBOL(rtk_83xx_qos_init);


rtk_api_ret_t rtk_83xx_qos_schedulingQueue_get(rtk_port_t port, rtk_qos_queue_weights_t *pQweights)
{
	rtk_api_ret_t retVal;
	rtk_port_t logic_port = rtk_switch_port_P2L_get(port);

	if(pQweights == NULL)
		return RT_ERR_NULL_POINTER;
	
	if(logic_port == UNDEFINE_PORT){
		printk("phyport %d cannot convert to valid logical port\n", port);
		/* for diagshell checking all ports all queues status, return ok when fails to obtain valid logical port. */
		return RT_ERR_OK;
	}
	
    retVal = rtk_qos_schedulingQueue_get(logic_port, pQweights);

    return retVal;
}
EXPORT_SYMBOL(rtk_83xx_qos_schedulingQueue_get);


rtk_api_ret_t rtk_83xx_qos_schedulingQueue_set(rtk_port_t port, rtk_qos_queue_weights_t *pQweights)
{
	rtk_api_ret_t retVal;
	rtk_port_t logic_port = rtk_switch_port_P2L_get(port);
    ...
    retVal = rtk_qos_schedulingQueue_set(logic_port, pQweights);

    return retVal;
}
EXPORT_SYMBOL(rtk_83xx_qos_schedulingQueue_set);


rtk_api_ret_t rtk_83xx_qos_priSelGroup_get(rtk_qos_priDecTbl_t index, rtk_priority_select_t *pPriDec)
{
	rtk_api_ret_t retVal;
	RTK_API_LOCK();
	...
	if (index < PRIDECTBL_IDX0 || index >= PRIDECTBL_END){
	...
	if ((retVal = rtl8367c_getAsicPriorityDecision(index, PRIDEC_PORT, &pPriDec->port_pri)) != RT_ERR_OK)
	    goto done;

	if ((retVal = rtl8367c_getAsicPriorityDecision(index, PRIDEC_ACL, &pPriDec->acl_pri)) != RT_ERR_OK)
		goto done;

	if ((retVal = rtl8367c_getAsicPriorityDecision(index, PRIDEC_DSCP, &pPriDec->dscp_pri)) != RT_ERR_OK)
		goto done;

	if ((retVal = rtl8367c_getAsicPriorityDecision(index, PRIDEC_1Q, &pPriDec->dot1q_pri)) != RT_ERR_OK)
		goto done;

	if ((retVal = rtl8367c_getAsicPriorityDecision(index, PRIDEC_1AD, &pPriDec->svlan_pri)) != RT_ERR_OK)
		goto done;

	if ((retVal = rtl8367c_getAsicPriorityDecision(index, PRIDEC_CVLAN, &pPriDec->cvlan_pri)) != RT_ERR_OK)
		goto done;

	if ((retVal = rtl8367c_getAsicPriorityDecision(index, PRIDEC_DA, &pPriDec->dmac_pri)) != RT_ERR_OK)
		goto done;

	if ((retVal = rtl8367c_getAsicPriorityDecision(index, PRIDEC_SA, &pPriDec->smac_pri)) != RT_ERR_OK)
		goto done;
...
EXPORT_SYMBOL(rtk_83xx_qos_priSelGroup_get);


rtk_api_ret_t rtk_83xx_qos_priSelGroup_set(rtk_qos_priDecTbl_t index, rtk_priority_select_t *pPriDec)
{
	rtk_api_ret_t retVal;
	RTK_API_LOCK();
	...
	if (pPriDec == NULL || pPriDec->port_pri > 0xFF || pPriDec->dot1q_pri > 0xFF 
	|| pPriDec->acl_pri > 0xFF || pPriDec->dscp_pri > 0xFF || pPriDec->cvlan_pri > 0xFF 
	|| pPriDec->svlan_pri > 0xFF || pPriDec->dmac_pri > 0xFF || pPriDec->smac_pri > 0xFF){
		retVal = RT_ERR_QOS_SEL_PRI_SOURCE;
		goto done;
	}
	
	if ((retVal = rtl8367c_setAsicPriorityDecision(index, PRIDEC_PORT, pPriDec->port_pri)) != RT_ERR_OK)
	    goto done;

	if ((retVal = rtl8367c_setAsicPriorityDecision(index, PRIDEC_ACL, pPriDec->acl_pri)) != RT_ERR_OK)
		goto done;

	if ((retVal = rtl8367c_setAsicPriorityDecision(index, PRIDEC_DSCP, pPriDec->dscp_pri)) != RT_ERR_OK)
		goto done;

	if ((retVal = rtl8367c_setAsicPriorityDecision(index, PRIDEC_1Q, pPriDec->dot1q_pri)) != RT_ERR_OK)
		goto done;

	if ((retVal = rtl8367c_setAsicPriorityDecision(index, PRIDEC_1AD, pPriDec->svlan_pri)) != RT_ERR_OK)
		goto done;

	if ((retVal = rtl8367c_setAsicPriorityDecision(index, PRIDEC_CVLAN, pPriDec->cvlan_pri)) != RT_ERR_OK)
		goto done;

	if ((retVal = rtl8367c_setAsicPriorityDecision(index, PRIDEC_DA, pPriDec->dmac_pri)) != RT_ERR_OK)
		goto done;

	if ((retVal = rtl8367c_setAsicPriorityDecision(index, PRIDEC_SA, pPriDec->smac_pri)) != RT_ERR_OK)
		goto done;
...
EXPORT_SYMBOL(rtk_83xx_qos_priSelGroup_set);


rtk_api_ret_t rtk_83xx_qos_priMap_get(rtk_queue_num_t queue_num, rtk_qos_pri2queue_t *pPri2qid)
{
	rtk_api_ret_t retVal;
    ...
    retVal = rtk_qos_priMap_get(queue_num, pPri2qid);

    return retVal;
}
EXPORT_SYMBOL(rtk_83xx_qos_priMap_get);


rtk_api_ret_t rtk_83xx_qos_priMap_set(rtk_queue_num_t queue_num, rtk_qos_pri2queue_t *pPri2qid)
{
	rtk_api_ret_t retVal;
    ...
    retVal = rtk_qos_priMap_set(queue_num, pPri2qid);

    return retVal;
}
EXPORT_SYMBOL(rtk_83xx_qos_priMap_set);


rtk_api_ret_t rtk_83xx_qos_1pPriRemap_get(rtk_pri_t dot1p_pri, rtk_pri_t *pInt_pri)
{
	rtk_api_ret_t retVal;
...
EXPORT_SYMBOL(rtk_83xx_qos_1pPriRemap_get);


rtk_api_ret_t rtk_83xx_qos_1pPriRemap_set(rtk_pri_t dot1p_pri, rtk_pri_t int_pri)
{
	rtk_api_ret_t retVal;
...
EXPORT_SYMBOL(rtk_83xx_qos_1pPriRemap_set);


rtk_api_ret_t rtk_83xx_qos_1pRemark_get(rtk_pri_t int_pri, rtk_pri_t *pDot1p_pri)
{
	rtk_api_ret_t retVal;
    ...
    retVal = rtk_qos_1pRemark_get(int_pri, pDot1p_pri);

    return retVal;
}
EXPORT_SYMBOL(rtk_83xx_qos_1pRemark_get);


rtk_api_ret_t rtk_83xx_qos_1pRemark_set(rtk_pri_t int_pri, rtk_pri_t dot1p_pri)
{
	rtk_api_ret_t retVal;
...
EXPORT_SYMBOL(rtk_83xx_qos_1pRemark_set);


rtk_api_ret_t rtk_83xx_qos_dscpRemark_get(rtk_pri_t int_pri, rtk_dscp_t *pDscp)
{
	rtk_api_ret_t retVal;
    ...
    retVal = rtk_qos_dscpRemark_get(int_pri, pDscp);

    return retVal;
}
EXPORT_SYMBOL(rtk_83xx_qos_dscpRemark_get);


rtk_api_ret_t rtk_83xx_qos_dscpRemark_set(rtk_pri_t int_pri, rtk_dscp_t dscp)
{
	rtk_api_ret_t retVal;
...
EXPORT_SYMBOL(rtk_83xx_qos_dscpRemark_set);


rtk_api_ret_t rtk_83xx_qos_1pRemarkEnable_get(rtk_port_t port, rtk_enable_t *pEnable)
{
	rtk_api_ret_t retVal;
	rtk_port_t logic_port = rtk_switch_port_P2L_get(port);
	...
	if(logic_port == UNDEFINE_PORT){
		printk("phyport %d cannot convert to valid logical port\n", port);
		/* for diagshell checking all ports status, return ok when fails to obtain valid logical port. */
		return RT_ERR_OK;
	}

    retVal = rtk_qos_1pRemarkEnable_get(logic_port, pEnable);

    return retVal;
}
EXPORT_SYMBOL(rtk_83xx_qos_1pRemarkEnable_get);


rtk_api_ret_t rtk_83xx_qos_1pRemarkEnable_set(rtk_port_t port, rtk_enable_t enable)
{
	rtk_api_ret_t retVal;
	rtk_port_t logic_port = rtk_switch_port_P2L_get(port);
    ...
    retVal = rtk_qos_1pRemarkEnable_set(logic_port, enable);

    return retVal;
}
EXPORT_SYMBOL(rtk_83xx_qos_1pRemarkEnable_set);


rtk_api_ret_t rtk_83xx_qos_dscpRemarkEnable_get(rtk_port_t port, rtk_enable_t *pEnable)
{
	rtk_api_ret_t retVal;
	...
	/* aware that 8367 dscp remark is for the whole system, not per port */
    retVal = rtk_qos_dscpRemarkEnable_get(RTK_WHOLE_SYSTEM, pEnable);

    return retVal;
}
EXPORT_SYMBOL(rtk_83xx_qos_dscpRemarkEnable_get);


rtk_api_ret_t rtk_83xx_qos_dscpRemarkEnable_set(rtk_port_t port, rtk_enable_t enable)
{
	rtk_api_ret_t retVal;

	/* aware that 8367 dscp remark is for the whole system, not per port */
    retVal = rtk_qos_dscpRemarkEnable_set(RTK_WHOLE_SYSTEM, enable);

    return retVal;
}
EXPORT_SYMBOL(rtk_83xx_qos_dscpRemarkEnable_set);


rtk_api_ret_t rtk_83xx_setFlowControl(rtk_enable_t qosEnable)
{
	...
	if(!CPORT_TX_PAUSE && !CPORT_RX_PAUSE){
		printk("[%s:%d] no need to modify RTL83xx CPU port FC status for initialized as disable.\n", __FUNCTION__, __LINE__);
		return RT_ERR_OK;
	}

    if((retVal = rtk_port_macForceLinkExt_get(cpu_port, &mode, &mac_cfg)) != RT_ERR_OK)
		goto done;

	/* 	when qos is enabled, CPU port flow control should be disabled, 
		or traffic from 98F may not schedule well in 8367.	*/
	if((mac_cfg.txpause || mac_cfg.rxpause) && qosEnable == ENABLED)
	{
		mac_cfg.txpause = DISABLED;
		mac_cfg.rxpause = DISABLED;
		
		if((retVal = rtk_port_macForceLinkExt_set(cpu_port, mode, &mac_cfg)) != RT_ERR_OK)
			goto done;
	}
	else if(qosEnable == DISABLED)
	{
		if(CPORT_TX_PAUSE)
	...
	printk("[%s:%d] CPU txpause: %d, rxpause: %d\n", __FUNCTION__, __LINE__, mac_cfg.txpause, mac_cfg.rxpause);

done:
	return retVal;
}
EXPORT_SYMBOL(rtk_83xx_setFlowControl);

#if defined(CONFIG_RTL_83XX_QOS_SUPPORT)
unsigned int g83xxQosEnabled = 0;
EXPORT_SYMBOL(g83xxQosEnabled);

rtk_api_ret_t rtk_83xx_qos_enable(rtk_enable_t enable)
{
	rtk_api_ret_t retVal = RT_ERR_OK;
	rtk_uint32 pri = (enable == ENABLED) ? 0x80 : 0x02;		// make 1p-based the highest when qos is enabled, or back to init value in g_priorityDecision
	
	if((retVal = rtk_83xx_setFlowControl(enable)) != RT_ERR_OK)
		return retVal;
	
	RTK_API_LOCK();

	// for port0 - port4
	if((retVal = rtl83xx_setAsicPriorityDecision(PRIDECTBL_IDX0, PRIDEC_1Q, pri)) != RT_ERR_OK)		
		goto done;

	// for ext ports
	if((retVal = rtl83xx_setAsicPriorityDecision(PRIDECTBL_IDX1, PRIDEC_1Q, pri)) != RT_ERR_OK)		
		goto done;

	g83xxQosEnabled = enable>0 ? 1 : 0;
	
done:
	RTK_API_UNLOCK();
	return RT_ERR_OK;
}
EXPORT_SYMBOL(rtk_83xx_qos_enable);
#endif
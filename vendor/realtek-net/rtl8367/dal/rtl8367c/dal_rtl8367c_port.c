 ...
 *      None.
 */
rtksw_api_ret_t dal_rtl8367c_port_phyOCPReg_set(rtksw_port_t port, rtksw_uint32 ocpAddr, rtksw_uint32 ocpData )
{
    rtksw_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicPHYOCPReg(rtksw_switch_port_L2P_get(port), ocpAddr, ocpData)) != RT_ERR_OK)
        return retVal;
 ...
 *      None.
 */
rtksw_api_ret_t dal_rtl8367c_port_phyOCPReg_get(rtksw_port_t port, rtksw_uint32 ocpAddr, rtksw_uint32 *pRegData )
{
    rtksw_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicPHYOCPReg(rtksw_switch_port_L2P_get(port), ocpAddr, pRegData)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_port_phyMdx_set(rtksw_port_t port, rtksw_port_phy_mdix_mode_t mode)
{
    rtksw_uint32 regData;
    rtksw_api_ret_t retVal;
    ...
    switch (mode)
    {
        case PHY_AUTO_CROSSOVER_MODE:
            if ((retVal = rtl8367c_getAsicPHYReg(rtksw_switch_port_L2P_get(port), 24, &regData))!=RT_ERR_OK)
                return retVal;

            regData &= ~(0x0001 << 9);

            if ((retVal = rtl8367c_setAsicPHYReg(rtksw_switch_port_L2P_get(port), 24, regData))!=RT_ERR_OK)
                return retVal;
            break;
    ...
    /* Restart N-way */
    if ((retVal = rtl8367c_getAsicPHYReg(rtksw_switch_port_L2P_get(port), 0, &regData))!=RT_ERR_OK)
        return retVal;

    regData |= (0x0001 << 9);

    if ((retVal = rtl8367c_setAsicPHYReg(rtksw_switch_port_L2P_get(port), 0, regData))!=RT_ERR_OK)
        return retVal;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_port_phyMdx_get(rtksw_port_t port, rtksw_port_phy_mdix_mode_t *pMode)
{
    rtksw_uint32 regData;
    rtksw_api_ret_t retVal;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_port_phyMdxStatus_get(rtksw_port_t port, rtksw_port_phy_mdix_status_t *pStatus)
{
    rtksw_uint32 regData;
    rtksw_api_ret_t retVal;
    ...
    {
        if ((retVal = rtl8367c_getAsicPHYReg(rtksw_switch_port_L2P_get(port), 26, &regData))!=RT_ERR_OK)
            return retVal;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_port_phyTestMode_set(rtksw_port_t port, rtksw_port_phy_test_mode_t mode)
{
    ...
    if (RTKSW_PHY_TEST_MODE_NORMAL != mode)
    {
        ...
        RTKSW_SCAN_ALL_LOG_PORT(i)
        {
            if(rtksw_switch_isUtpPort(i) == RT_ERR_OK)
            {
                if(i != port)
                {
                    if ((retVal = rtl8367c_getAsicPHYReg(rtksw_switch_port_L2P_get(i), 9, &data)) != RT_ERR_OK)
                        return retVal;
    ...
    if (RTKSW_PHY_TEST_MODE_4 == mode)
    {
        ...
        if(regData == 0x6511)
        {
            if ((retVal = rtl8367c_setAsicPHYOCPReg(rtksw_switch_port_L2P_get(port), 0xa436, 0x8081)) != RT_ERR_OK)
                return retVal;

            if ((retVal = rtl8367c_setAsicPHYOCPReg(rtksw_switch_port_L2P_get(port), 0xa438, 0x7e00)) != RT_ERR_OK)
                return retVal;
        }
    }

    if ((retVal = rtl8367c_getAsicPHYReg(rtksw_switch_port_L2P_get(port), 9, &data)) != RT_ERR_OK)
        return retVal;

    data &= ~0xE000;
    data |= (mode << 13);
    if ((retVal = rtl8367c_setAsicPHYReg(rtksw_switch_port_L2P_get(port), 9, data)) != RT_ERR_OK)
        return retVal;
    ...
    if (RTKSW_PHY_TEST_MODE_4 == mode)
    {
        ...
        {
            if ((retVal = rtl8367c_setAsicPHYOCPReg(rtksw_switch_port_L2P_get(port), 0xbcc2, 0xF4F4)) != RT_ERR_OK)
                return retVal;
        }

        if( (regData == 0x6367) )
        {
            if ((retVal = rtl8367c_setAsicPHYOCPReg(rtksw_switch_port_L2P_get(port), 0xa436, 0x80c1)) != RT_ERR_OK)
                return retVal;

            if ((retVal = rtl8367c_setAsicPHYOCPReg(rtksw_switch_port_L2P_get(port), 0xa438, 0xfe00)) != RT_ERR_OK)
                return retVal;
        }
    }
 ...
 */
rtksw_api_ret_t dal_rtl8367c_port_phyTestMode_get(rtksw_port_t port, rtksw_port_phy_test_mode_t *pMode)
{
    rtksw_uint32      data;
    rtksw_api_ret_t   retVal;
 ...
 *      - ENABLE
 */
rtksw_api_ret_t dal_rtl8367c_port_phyGreenEthernet_set(rtksw_port_t port, rtksw_enable_t state)
{
    rtksw_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicGreenEthernet(rtksw_switch_port_L2P_get(port), state))!=RT_ERR_OK)
        return retVal;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_port_phyGreenEthernet_get(rtksw_port_t port, rtksw_enable_t *pState)
{
    rtksw_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicGreenEthernet(rtksw_switch_port_L2P_get(port), pState))!=RT_ERR_OK)
        return retVal;
 ...
 *      - ENABLE
 */
rtksw_api_ret_t dal_rtl8367c_port_phyLinkDownPowerSaving_set(rtksw_port_t port, rtksw_enable_t state)
{
    rtksw_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_setAsicPowerSaving(rtksw_switch_port_L2P_get(port), state))!=RT_ERR_OK)
        return retVal;        
 ...
 */
rtksw_api_ret_t dal_rtl8367c_port_phyLinkDownPowerSaving_get(rtksw_port_t port, rtksw_enable_t *pState)
{
    rtksw_api_ret_t retVal;
    ...
    if ((retVal = rtl8367c_getAsicPowerSaving(rtksw_switch_port_L2P_get(port), pState))!=RT_ERR_OK)
        return retVal; 

 ...
 * Data Declaration
 */
static rtksw_uint32 PatchChipData[210][2] =
{
...
static rtksw_api_ret_t _dal_switch_init_8367c(void)
{
    rtksw_port_t port;
    rtksw_uint32 retVal;
...
static rtksw_api_ret_t _dal_switch_init_8370b(void)
{
    ret_t retVal;
    rtksw_uint32 regData, tmp = 0;
    rtksw_uint32 i, prf, counter;
    rtksw_uint32 long_link[8] = {0x0210, 0x03e8, 0x0218, 0x03f0, 0x0220, 0x03f8, 0x0208, 0x03e0 };
...
static rtksw_api_ret_t _dal_switch_init_8364b(void)
{
    ret_t retVal;
    rtksw_uint32 regData;
...
static rtksw_api_ret_t _dal_switch_init_8363sc_vb(void)
{
 ...
 */
rtksw_api_ret_t dal_rtl8367c_switch_init(void)
{
    rtksw_int32  retVal;
    ...
    /* Max rate */
    if((retVal = rtl8367c_setAsicPortIngressBandwidth(rtksw_switch_port_L2P_get(EXT_PORT0), RTL8367C_QOS_RATE_INPUT_MAX_HSG>>3, RTKSW_DISABLED, RTKSW_ENABLED)) != RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicPortEgressRate(rtksw_switch_port_L2P_get(EXT_PORT0), RTL8367C_QOS_RATE_INPUT_MAX_HSG>>3)) != RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_setAsicPortEgressRateIfg(RTKSW_ENABLED)) != RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = rtl8367c_setAsicPortMirrorIsolationTxLeaky(RTKSW_ENABLED)) != RT_ERR_OK)
        return retVal;
 ...
 * Note:
 */
rtksw_api_ret_t dal_rtl8367c_switch_portMaxPktLen_set(rtksw_port_t port, rtksw_switch_maxPktLen_linkSpeed_t speed, rtksw_uint32 cfgId)
{
    ...
    if((retVal = rtl8367c_setAsicMaxLength(rtksw_switch_port_L2P_get(port), (rtksw_uint32)speed, cfgId)) != RT_ERR_OK)
        return retVal;
 ...
 * Note:
 */
rtksw_api_ret_t dal_rtl8367c_switch_portMaxPktLen_get(rtksw_port_t port, rtksw_switch_maxPktLen_linkSpeed_t speed, rtksw_uint32 *pCfgId)
{
    rtksw_api_ret_t retVal;
    ...
    if((retVal = rtl8367c_getAsicMaxLength(rtksw_switch_port_L2P_get(port), (rtksw_uint32)speed, pCfgId)) != RT_ERR_OK)
        return retVal;
 ...
 * Note:
 */
rtksw_api_ret_t dal_rtl8367c_switch_maxPktLenCfg_set(rtksw_uint32 cfgId, rtksw_uint32 pktLen)
{
    rtksw_api_ret_t retVal;
 ...
 * Note:
 */
rtksw_api_ret_t dal_rtl8367c_switch_maxPktLenCfg_get(rtksw_uint32 cfgId, rtksw_uint32 *pPktLen)
{
        rtksw_api_ret_t retVal;
 ...
 *      - ENABLE
 */
rtksw_api_ret_t dal_rtl8367c_switch_greenEthernet_set(rtksw_enable_t enable)
{
    rtksw_api_ret_t retVal;
    rtksw_uint32 port;
    ...
    RTKSW_SCAN_ALL_LOG_PORT(port)
    {
        if(rtksw_switch_isUtpPort(port) == RT_ERR_OK)
        {
            if ((retVal = rtl8367c_setAsicPowerSaving(rtksw_switch_port_L2P_get(port),enable))!=RT_ERR_OK)
                return retVal;

            if ((retVal = rtl8367c_setAsicGreenEthernet(rtksw_switch_port_L2P_get(port), enable))!=RT_ERR_OK)
                return retVal;
        }
    }
 ...
 */
rtksw_api_ret_t dal_rtl8367c_switch_greenEthernet_get(rtksw_enable_t *pEnable)
{
    rtksw_api_ret_t retVal;
    rtksw_uint32 port;
    rtksw_uint32 state;
    ...
    RTKSW_SCAN_ALL_LOG_PORT(port)
    {
        if(rtksw_switch_isUtpPort(port) == RT_ERR_OK)
        {
            if ((retVal = rtl8367c_getAsicPowerSaving(rtksw_switch_port_L2P_get(port), &state))!=RT_ERR_OK)
                return retVal;

            if(state == RTKSW_DISABLED)
            {
                *pEnable = RTKSW_DISABLED;
                return RT_ERR_OK;
            }

            if ((retVal = rtl8367c_getAsicGreenEthernet(rtksw_switch_port_L2P_get(port), &state))!=RT_ERR_OK)
                return retVal;

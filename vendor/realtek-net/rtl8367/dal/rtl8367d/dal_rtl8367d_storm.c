 ...
 *
 */
rtksw_api_ret_t dal_rtl8367d_rate_stormControlMeterIdx_set(rtksw_port_t port, rtksw_rate_storm_group_t stormType, rtksw_uint32 index)
{
 ...
 *
 */
rtksw_api_ret_t dal_rtl8367d_rate_stormControlMeterIdx_get(rtksw_port_t port, rtksw_rate_storm_group_t stormType, rtksw_uint32 *pIndex)
{
    rtksw_api_ret_t retVal;
    rtksw_uint32 phy_port;
 ...
 *
 */
rtksw_api_ret_t dal_rtl8367d_rate_stormControlPortEnable_set(rtksw_port_t port, rtksw_rate_storm_group_t stormType, rtksw_enable_t enable)
{
    rtksw_api_ret_t retVal;
    ...
    {
        case STORM_GROUP_UNKNOWN_UNICAST:
            if ((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_STORM_UNKOWN_UCAST, rtksw_switch_port_L2P_get(port), (enable == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
                return retVal;
            break;
        case STORM_GROUP_UNKNOWN_MULTICAST:
            if ((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_STORM_UNKOWN_MCAST, rtksw_switch_port_L2P_get(port), (enable == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
                return retVal;
            break;
        case STORM_GROUP_MULTICAST:
            if ((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_STORM_MCAST, rtksw_switch_port_L2P_get(port), (enable == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
                return retVal;
            break;
        case STORM_GROUP_BROADCAST:
            if ((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_STORM_BCAST, rtksw_switch_port_L2P_get(port), (enable == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
                return retVal;
            break;
        default:
            break;
 ...
 *
 */
rtksw_api_ret_t dal_rtl8367d_rate_stormControlPortEnable_get(rtksw_port_t port, rtksw_rate_storm_group_t stormType, rtksw_enable_t *pEnable)
{
    rtksw_api_ret_t retVal;
    ...
    {
        case STORM_GROUP_UNKNOWN_UNICAST:
            if ((retVal = rtl8367d_getAsicRegBit(RTL8367D_REG_STORM_UNKOWN_UCAST, rtksw_switch_port_L2P_get(port), &regData)) != RT_ERR_OK)
                return retVal;
            break;
        case STORM_GROUP_UNKNOWN_MULTICAST:
            if ((retVal = rtl8367d_getAsicRegBit(RTL8367D_REG_STORM_UNKOWN_MCAST, rtksw_switch_port_L2P_get(port), &regData)) != RT_ERR_OK)
                return retVal;
            break;
        case STORM_GROUP_MULTICAST:
            if ((retVal = rtl8367d_getAsicRegBit(RTL8367D_REG_STORM_MCAST, rtksw_switch_port_L2P_get(port), &regData)) != RT_ERR_OK)
                return retVal;
            break;
        case STORM_GROUP_BROADCAST:
            if ((retVal = rtl8367d_getAsicRegBit(RTL8367D_REG_STORM_BCAST, rtksw_switch_port_L2P_get(port), &regData)) != RT_ERR_OK)
                return retVal;
            break;
        default:
            break;
 ...
 *      - BYPASS_LLDP.
 */
rtksw_api_ret_t dal_rtl8367d_storm_bypass_set(rtksw_storm_bypass_t type, rtksw_enable_t enable)
{
    rtksw_api_ret_t retVal;
    rtksw_uint32 index;
    ...
    if (type >= 0 && type <= BYPASS_UNDEF_GARP_2F)
    {
        ...
        if((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_RMA_CTRL00 + index, RTL8367D_RMA_CTRL00_DISCARD_STORM_FILTER_OFFSET, (enable == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
            return retVal;
    }
    else if(type == BYPASS_IGMP)
    {
        if ((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_IGMP_MLD_CFG0, RTL8367D_IGMP_MLD_DISCARD_STORM_FILTER_OFFSET, (enable == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
            return retVal;
    }
    else if (type == BYPASS_CDP)
    {
        if((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_RMA_CTRL_CDP, RTL8367D_RMA_CTRL_CDP_DISCARD_STORM_FILTER_OFFSET, (enable == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
            return retVal;
    }
    else if (type  == BYPASS_CSSTP)
    {
        if((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_RMA_CTRL_CSSTP, RTL8367D_RMA_CTRL_CSSTP_DISCARD_STORM_FILTER_OFFSET, (enable == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
            return retVal;
    }
    else if (type  == BYPASS_LLDP)
    {
        if((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_RMA_CTRL_LLDP, RTL8367D_RMA_CTRL_LLDP_DISCARD_STORM_FILTER_OFFSET, (enable == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
            return retVal;
    }
    else
        return RT_ERR_INPUT;
 ...
 *      - BYPASS_LLDP.
 */
rtksw_api_ret_t dal_rtl8367d_storm_bypass_get(rtksw_storm_bypass_t type, rtksw_enable_t *pEnable)
{
    rtksw_api_ret_t retVal;
    rtksw_uint32 index;
    rtksw_uint32 regData;
 ...
 *
 */
rtksw_api_ret_t dal_rtl8367d_rate_stormControlExtPortmask_set(rtksw_portmask_t *pPortmask)
{
    rtksw_api_ret_t retVal;
    rtksw_uint32 pmask;
 ...
 *
 */
rtksw_api_ret_t dal_rtl8367d_rate_stormControlExtPortmask_get(rtksw_portmask_t *pPortmask)
{
    rtksw_api_ret_t retVal;
    rtksw_uint32 pmask;
 ...
 *
 */
rtksw_api_ret_t dal_rtl8367d_rate_stormControlExtEnable_set(rtksw_rate_storm_group_t stormType, rtksw_enable_t enable)
{
    rtksw_api_ret_t retVal;
    ...
    {
        case STORM_GROUP_UNKNOWN_UNICAST:
            if ((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_STORM_EXT_CFG, RTL8367D_STORM_UNKNOWN_UCAST_EXT_EN_OFFSET, (enable == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
                return retVal;
            break;
        case STORM_GROUP_UNKNOWN_MULTICAST:
            if ((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_STORM_EXT_CFG, RTL8367D_STORM_UNKNOWN_MCAST_EXT_EN_OFFSET, (enable == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
                return retVal;
            break;
        case STORM_GROUP_MULTICAST:
            if ((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_STORM_EXT_CFG, RTL8367D_STORM_MCAST_EXT_EN_OFFSET, (enable == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
                return retVal;
            break;
        case STORM_GROUP_BROADCAST:
            if ((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_STORM_EXT_CFG, RTL8367D_STORM_BCAST_EXT_EN_OFFSET, (enable == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
                return retVal;
            break;
        default:
            break;
 ...
 *
 */
rtksw_api_ret_t dal_rtl8367d_rate_stormControlExtEnable_get(rtksw_rate_storm_group_t stormType, rtksw_enable_t *pEnable)
{
    rtksw_api_ret_t retVal;
    rtksw_uint32 regData;
 ...
 *
 */
rtksw_api_ret_t dal_rtl8367d_rate_stormControlExtMeterIdx_set(rtksw_rate_storm_group_t stormType, rtksw_uint32 index)
{
    rtksw_api_ret_t retVal;
 ...
 *
 */
rtksw_api_ret_t dal_rtl8367d_rate_stormControlExtMeterIdx_get(rtksw_rate_storm_group_t stormType, rtksw_uint32 *pIndex)
{
    rtksw_api_ret_t retVal;

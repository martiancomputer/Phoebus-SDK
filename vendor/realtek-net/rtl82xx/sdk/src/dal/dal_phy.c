...
dal_phy_portRefTime_set(uint32 unit, rtk_portmask_t portmask, rtk_time_timeStamp_t timeStamp, uint32 exec)
{
    ...
    HWP_PHY_TRAVS(unit, phyIdx)
    {
        ...
        {
            if ((ret = phy_ptpRefTime_set(unit, baseMacId, timeStamp, exec)) != RT_ERR_OK)
            {
                PHY_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL | MOD_PHY), "");
                return ret;
...
dal_phy_portRefTimeAdjust_set(uint32 unit, rtk_portmask_t portmask, uint32 sign, rtk_time_timeStamp_t timeStamp, uint32 exec)
{
   ...
   rtk_port_t phyIdx, baseMacId;
    ...
    HWP_PHY_TRAVS(unit, phyIdx)
    {
        ...
        if ((ret = phy_ptpRefTimeAdjust_set(unit, baseMacId, sign, timeStamp, exec)) != RT_ERR_OK)
        {
            PHY_SEM_UNLOCK(unit);
            RT_ERR(ret, (MOD_DAL | MOD_PHY), "");
            return ret;
...
dal_phy_portPtpMacAddrRange_set(uint32 unit, rtk_port_t port, uint32 range)
{
    ...
    if ((ret = phy_ptpSwitchMacRange_set(unit, baseMacId, range)) != RT_ERR_OK)
    {
        PHY_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL | MOD_PHY), "");
        return ret;

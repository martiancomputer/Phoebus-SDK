...
//#include <hal/phy/nic_rtl8221/nic_rtl8221.h>
...
Rtl8221_api_ver(IN HANDLE hDevice)
{
	...
	osal_printf("RTL8226/RTL8221 Series API(function) Version v%d.%02x.%d\n", RELEASE_VER_M, phydata, _RELEASE_DATE);
    ...
    )
{
    ...
    if (MmdPhyRead(hDevice, MMD_AN, 0, &phydata) == SUCCESS)
    {
        if (phydata & BIT_12)   /* AN_ENABLE */
        {
            ...
            (void)MmdPhyWrite(hDevice, MMD_AN, 0, phydata);
        }
    }
    ...
    )
{
	...
	status = Rtl8221_autoNegoEnable_get(hDevice, &AnEnable);
	if (status != SUCCESS)
		goto exit;

	if (AnEnable == 1)
	{
		osal_printf("Please refer to Rtl8221_autoNegoAbility_set\n");
		status = FAILURE;
		goto exit;
	}
	else
    ...
    )
{
    ...
    status = Rtl8221_is_link(hDevice, &linkOK);
    if (status != SUCCESS)
        goto exit;
    ...
    )
{
    ...
    RTL8221B_INFO("%s phy (addr%d)", (Enable ? "Enable" : "Disable"), hDevice.port);
    ...
    )
{
    ...
    if (support)
    {
        ...
        status = Rtl8221_autoNegoEnable_set(hDevice, FALSE);
        if (status != SUCCESS)
            goto exit;
    }
    else
    ...
    )
{
	...
	if (AnEnable == 0)
	{
		osal_printf("Before setting the EEE, enable auto-negotiation.\n");
		status = FAILURE;
		goto exit;
	}
	else
	{
		...
		if ((phydata == 2) || (phydata == 3) || (phydata == 4))  //enable PHY mode EEE
		{
			if(pEeeEnable->EEE_100 == 1 || pEeeEnable->EEE_1000 == 1 || pEeeEnable->EEE_2_5G == 1)
			{
...
BOOLEAN
Rtl8221_PHYmodeEEE_set(IN HANDLE hDevice,int on_off)
{
    BOOL status = FAILURE;
    UINT16 phydata = 0;
	if(on_off){
		...
		status = MmdPhyRead(hDevice, MMD_VEND2, 0xA4A8, &phydata);
        if (status != SUCCESS)
            goto exit;

        phydata |= BIT_10;

        status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA4A8, phydata);
        if (status != SUCCESS)
            goto exit;
...
BOOLEAN
Rtl8221_10M_PHYmodeEEEP_set(IN HANDLE hDevice,int on_off)
{
    BOOL status = FAILURE;
    UINT16 phydata = 0; 
	if(on_off){
    ...
    )
{
    ...
    if (status != SUCCESS)
        goto exit;
    if (CrossOverMode != PHY_CROSSPVER_MODE_AUTO)
    {
        // RestartAutoNegotiation
  	 status = MmdPhyRead(hDevice, MMD_AN, 0, &phydata);
        if (status != SUCCESS)
    ...
    )
{
	...
	status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA436, 0x801D);
	if (status != SUCCESS)
		goto exit;

	status = MmdPhyRead(hDevice, MMD_VEND2, 0xA438, &phyreg0);
	if (status != SUCCESS)
		goto exit;
	
	if ((phyreg0&BIT_15) > 0)
	{
		osal_printf("This PHY DOES NOT support Giga-Lite.\n");
		status = FAILURE;
		goto exit;
	}
    ...
    )
{
	...
	UINT16 phydata1 = 0; 
	UINT16 phyreg0 = 0;
	UINT16 mdi_reverse = 0;
	UINT16 tx_polarity_swap = 0xa; //4'b1010
	...
	status = MmdPhyRead(hDevice, MMD_PCS, 0x0000, &phydata1);
	if (status != SUCCESS)
		goto exit;
	...
	if (phyreg0 == 0x00) 
	{
		status = MmdPhyRead(hDevice, MMD_VEND1, 0x6A21, &mdi_reverse);
		if (status != SUCCESS)
			goto exit;
		...
		status = MmdPhyRead(hDevice, MMD_VEND1, 0x6A20, &tx_polarity_swap);
		if (status != SUCCESS)
			goto exit;
		...
		if ((mdi_reverse != 0) || (tx_polarity_swap != 0xa))
		{
			status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA436, 0x81aa);
			if (status != SUCCESS)
				goto exit;
			...
			{
				phydata = phydata || ((tx_polarity_swap & BIT_3)<<5 ); //bit[8]
				phydata = phydata || ((tx_polarity_swap & BIT_2)<<7 ); //bit[8]
				phydata = phydata || ((tx_polarity_swap & BIT_1)<<9 ); //bit[8]
				phydata = phydata || ((tx_polarity_swap & BIT_0)<<11); //bit[8]
			}
	...
	if (phyreg0 == 0x81) 
	{
		status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA436, 0x8187);
		if (status != SUCCESS)
			goto exit;

		status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA438, 0x626E);
		if (status != SUCCESS)
			goto exit;

		status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA438, 0xE53C);
		if (status != SUCCESS)
			goto exit;

		status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA438, 0x0169);
		if (status != SUCCESS)
			goto exit;

		status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA438, 0xF630);
		if (status != SUCCESS)
			goto exit;
    ...
    if (linkOK)
    {
        status = Rtl8221_speed_get(hDevice, &Speed);
        if (status != SUCCESS)
            goto exit;
    ...
    )
{
       ...
       pRtctResult->linkType = Speed;
        ...
        case LINK_SPEED_2P5G:
			...
			if (phyreg0&BIT_7>0) 
			{
				...
				status = MmdPhyRead(hDevice, MMD_VEND2, 0xACBC, &phydata);
				if (status != SUCCESS)
					goto exit;
    ...
    else
    {
		...
		if (phyreg0 == 0x00) 
		{
			...
			status = Rtl8221_rtctResult_convert(phydata, &pRtctResult->channelAStatus);
			if (status != SUCCESS)
				goto exit;
			...
			status = Rtl8221_rtctResult_convert(phydata, &pRtctResult->channelBStatus);
			if (status != SUCCESS)
				goto exit;
			...
			status = Rtl8221_rtctResult_convert(phydata, &pRtctResult->channelCStatus);
			if (status != SUCCESS)
				goto exit;
			...
			status = Rtl8221_rtctResult_convert(phydata, &pRtctResult->channelDStatus);
			if (status != SUCCESS)
				goto exit;
		}
		if ((phyreg0&BIT_7)>0) 
		{
			...
			// channelALen = phyData * 100 / 80
			// channelALen (unit: cm)
			status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA436, 0x8028);
			if (status != SUCCESS)
				goto exit;
			...
			// channelBLen = phyData * 100 / 80
			// channelBLen (unit: cm)
			status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA436, 0x802C);
			if (status != SUCCESS)
				goto exit;
			...
			// channelCLen = phyData * 100 / 80
			// channelCLen (unit: cm)
			status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA436, 0x8030);
			if (status != SUCCESS)
				goto exit;
			...
			// channelDLen = phyData * 100 / 80
			// channelDLen (unit: cm)
			status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA436, 0x8034);
			if (status != SUCCESS)
				goto exit;
			...
			// channelA status
			status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA436, 0x8026);
			if (status != SUCCESS)
				goto exit;
			...
			// channelB status
			status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA436, 0x802A);
			if (status != SUCCESS)
				goto exit;
			...
			// channelC status
			status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA436, 0x802E);
			if (status != SUCCESS)
				goto exit;
			...
			// channelD status
			status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA436, 0x8032);
			if (status != SUCCESS)
				goto exit;
    ...
    )
{
	...
	if ((phyreg0&BIT_15) > 0)
	{
		osal_printf("This PHY DOES NOT support 2.5G-Lite.\n");
		status = FAILURE;
		goto exit;
	}
   ...
   case LINK_SPEED_10M:
    {
        ...
        if(pIEEEtestmode -> channel == TESTMODE_CHANNEL_A)
            Rtl8221_crossOverMode_set(hDevice,PHY_CROSSPVER_MODE_MDI);
        else if(pIEEEtestmode -> channel == TESTMODE_CHANNEL_B)
            Rtl8221_crossOverMode_set(hDevice,PHY_CROSSPVER_MODE_MDIX);
        ...
        if (pIEEEtestmode->TMFINISH)
        {
            //MMD31, reg 0xA430, bit[9:8]=0x0
             Rtl8221_crossOverMode_set(hDevice,PHY_CROSSPVER_MODE_AUTO);
    ...
    case LINK_SPEED_2P5G:
    {
        ...
        if (pIEEEtestmode->TM4)
        {
			...
			if ((phyreg0&BIT_15) > 0) 
			{
				if (pIEEEtestmode->channel == TESTMODE_CHANNEL_A)
				{
					//Write MMD31, reg 0xA444, bit[5:4]=0x0
					status = MmdPhyRead(hDevice, MMD_VEND2, 0xA444, &phydata);
					if (status != SUCCESS)
						goto exit;

					phydata &= (~(BIT_5 | BIT_4));

					status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA444, phydata);
					if (status != SUCCESS)
						goto exit;

					//Write MMD1, reg 0x0084, bit[15:12]=0x0
					status = MmdPhyWrite(hDevice, MMD_PMAPMD, 0x0084, 0x0400);
					if (status != SUCCESS)
						goto exit;
				}
				else if (pIEEEtestmode->channel == TESTMODE_CHANNEL_B)
				{
					//Write MMD31, reg 0xA444, bit[5:4]=0x1
					status = MmdPhyRead(hDevice, MMD_VEND2, 0xA444, &phydata);
					if (status != SUCCESS)
						goto exit;
				...
				{
					//Write MMD31, reg 0xA444, bit[5:4]=0x2
					status = MmdPhyRead(hDevice, MMD_VEND2, 0xA444, &phydata);
					if (status != SUCCESS)
						goto exit;
				...
				{
					//Write MMD31, reg 0xA444, bit[5:4]=0x3
					status = MmdPhyRead(hDevice, MMD_VEND2, 0xA444, &phydata);
					if (status != SUCCESS)
						goto exit;
        ...
        if (pIEEEtestmode->TMFINISH)
        {
			status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA444, 0x0000); 
			if (status != SUCCESS)
				goto exit;
...
{
    BOOL status = FAILURE, blinkOk = FAILURE;
    UINT16 phydata = 0, nTimeout = 100;
    if ((functioninput >= 0) && (functioninput <= 3))
    {
        status = MmdPhyRead(hDevice, MMD_VEND1, 0x75F3, &phydata);
        if (status != SUCCESS)
            goto exit;

        phydata &= ~BIT_0 ;
        status = MmdPhyWrite(hDevice, MMD_VEND1, 0x75F3, phydata);
        if (status != SUCCESS)
            goto exit;
        status = MmdPhyRead(hDevice, MMD_VEND1, 0x697A, &phydata);
        if (status != SUCCESS)
        ...
        if ((functioninput == 0) || (functioninput == 2))
        {
            ...
            if (status != SUCCESS)
                goto exit;
            status = MmdPhyWrite(hDevice, MMD_VEND1, 0x6F10, 0xD455);
            if (status != SUCCESS)
                goto exit;
        ...
        else if ((functioninput == 1) || (functioninput == 3))
        {
            ...
            if (status != SUCCESS)
                goto exit;
            status = MmdPhyWrite(hDevice, MMD_VEND1, 0x6F10, 0xD433);
            if (status != SUCCESS)
                goto exit;
            status = MmdPhyWrite(hDevice, MMD_VEND1, 0x6F11, 0x8020);
            if (status != SUCCESS)
        ...
        do {
            ...
            if (status != SUCCESS)
                goto exit;
            blinkOk = (phydata & BIT_2) ? (TRUE) : (FALSE);
            if (blinkOk) break;
            Sleep(100);
            nTimeout --;
            //osal_printf("blinkOk %x, nTimeout = %d\n",phydata,nTimeout);
        } while (nTimeout > 0);
        status = MmdPhyRead(hDevice, MMD_VEND2, 0xA400, &phydata);
        if (status != SUCCESS)
            goto exit;
    ...
    )
{
    ...
    Rtl8221_enable_set(hDevice, FALSE);
    Sleep (1);
    Rtl8221_enable_set(hDevice, TRUE);
...
#define R0_AUTO_NEGO    BIT_12
#define R5_ASM_DIR      BIT_11
#define R5_PAUSE        BIT_10
...
{
    BOOL status = FAILURE;
    UINT16 reg0, reg4, reg5;

    status = MmdPhyRead(hDevice, MMD_VEND2, 0xA400, &reg0);
    if (status != SUCCESS)
        goto exit;

    status = MmdPhyRead(hDevice, MMD_AN, 0x10, &reg4);
    if (status != SUCCESS)
        goto exit;

    status = MmdPhyRead(hDevice, MMD_AN, 0x13, &reg5);
    if (status != SUCCESS)
        goto exit;
    ...
    *rx_fc = 0;
    ...
    if (reg0 & R0_AUTO_NEGO) {
        ...
        if (reg4 & reg5 & R5_PAUSE) {
            *tx_fc = 1;
            *rx_fc = 1;
        }
        else if (reg4 & reg5 & R5_ASM_DIR) {
            if (reg4 & R5_PAUSE)
                *rx_fc = 1;
...
)
{
    ...
    if (status != SUCCESS)
        goto exit;
    if (mode == PHY_MASTER_MODE)
        phydata |= (BIT_10);
    else if(mode == PHY_SLAVE_MODE)
        phydata &= (~(BIT_10));
    else
        goto exit;
    status = MmdPhyWrite(hDevice, MMD_VEND2, 0xA412, phydata);

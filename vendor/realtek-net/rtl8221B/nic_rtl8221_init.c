...
static const MMD_REG Rtl8221_n0_ramcode[] =
{
	{ 31, 0xa436, 0XA016, },
    { 31, 0xa438, 0X0000, },
    { 31, 0xa436, 0XA012, },
...
static const MMD_REG Rtl8221_n1_ramcode[] =
{
    { 31, 0xa436, 0XA016, },
    { 31, 0xa438, 0X0010, },
    { 31, 0xa436, 0XA012, },
...
static const MMD_REG Rtl8221_n2_ramcode[] =
{
    { 31, 0xa436, 0XA016, },
    { 31, 0xa438, 0X0020, },
    { 31, 0xa436, 0XA012, },
...
static const MMD_REG Rtl8221_uc2_ramcode[] =
{
    { 31, 0xa436, 0Xb87c, },
    { 31, 0xa438, 0X8625, },
    { 31, 0xa436, 0Xb87e, },
...
static const MMD_REG Rtl8221_uc_ramcode[] =
{
	 { 31, 0xa436, 0X846e, },
    { 31, 0xa438, 0Xaf84, },
    { 31, 0xa438, 0X86af, },
...
static const MMD_REG Rtl8221_data_ramcode[] =
{
   
};

static const MMD_REG Rtl8221_isram_patch[] =
{
   
};

static BOOL
Rtl8221_wait_for_bit(
    IN HANDLE hDevice,
    IN UINT16 dev,
    IN UINT16 addr,
    IN UINT16 mask,
 ...
 */
void Rtl8221_led_init(HANDLE hDevice)
{
    ...
    (void)MmdPhyWrite(hDevice, MMD_VEND2, 0xD032, 0xA7);	/* Set LED 0 for 10/100/1g/2.5g/2.5glite */
    ...
    )
{
    ...
    // Polling PHY Status
    status = Rtl8221_wait_for_bit(hDevice, MMD_VEND2, 0xA420, 0x3, 1, 100);
    if (status != SUCCESS)
        goto exit;

    // Check phy id, no need to init 8221B_VB/VM/VD or 8221D Series.
    ...
    if ((rev_num == 8) &&(mod_num == 4))    {
        osal_printf("rtl8221b and go init flow...\n");
    }
    else{
        //osal_printf("Not rtl8221b and skip init flow...id = %x \n",phydata);
        RTL8221B_MSG("No need to init 8221B_VB/VM/VD or 8221D Series, and skip init flow...id = %x", phydata);
        status = MmdPhyRead(hDevice, MMD_VEND1, 0x75B5, &phydata);
        if (status == SUCCESS) {
            phydata = (phydata & ~0xF) | 0x4; // update IPG from 6+2 to 4+2
            (void)MmdPhyWrite(hDevice, MMD_VEND1, 0x75B5, phydata);
        }
        goto exit;
    }
    ...
    else
    {
        ...
        status = Rtl8221_wait_for_bit(hDevice, MMD_VEND2, 0xB800, BIT_6, 1, 100);
        if (status != SUCCESS)
            goto exit;
        ...
        // patch nc0
        for(i=0; i<sizeof(Rtl8221_n0_ramcode)/sizeof(MMD_REG); i++)
        {
            status = MmdPhyWrite(hDevice, Rtl8221_n0_ramcode[i].dev, Rtl8221_n0_ramcode[i].addr, Rtl8221_n0_ramcode[i].value);
            if (status != SUCCESS)
                goto exit;
        }

        // patch nc1
        for(i=0; i<sizeof(Rtl8221_n1_ramcode)/sizeof(MMD_REG); i++)
        {
            status = MmdPhyWrite(hDevice, Rtl8221_n1_ramcode[i].dev, Rtl8221_n1_ramcode[i].addr, Rtl8221_n1_ramcode[i].value);
            if (status != SUCCESS)
                goto exit;
        }
        ...
        // patch nc2
        for(i=0; i<sizeof(Rtl8221_n2_ramcode)/sizeof(MMD_REG); i++)
        {
            status = MmdPhyWrite(hDevice, Rtl8221_n2_ramcode[i].dev, Rtl8221_n2_ramcode[i].addr, Rtl8221_n2_ramcode[i].value);
            if (status != SUCCESS)
                goto exit;
        }

        // patch uc2
        for(i=0; i<sizeof(Rtl8221_uc2_ramcode)/sizeof(MMD_REG); i++)
        {
            status = MmdPhyWrite(hDevice, Rtl8221_uc2_ramcode[i].dev, Rtl8221_uc2_ramcode[i].addr, Rtl8221_uc2_ramcode[i].value);
        ...
        // patch uc
        for(i=0; i<sizeof(Rtl8221_uc_ramcode)/sizeof(MMD_REG); i++)
        {
            status = MmdPhyWrite(hDevice, Rtl8221_uc_ramcode[i].dev, Rtl8221_uc_ramcode[i].addr, Rtl8221_uc_ramcode[i].value);
            if (status != SUCCESS)
                goto exit;
        }
        ...
        // patch ram code
        for(i=0; i<sizeof(Rtl8221_data_ramcode)/sizeof(MMD_REG); i++)
        {
            status = MmdPhyWrite(hDevice, Rtl8221_data_ramcode[i].dev, Rtl8221_data_ramcode[i].addr, Rtl8221_data_ramcode[i].value);
            if (status != SUCCESS)
                goto exit;
        }
        ...
        status = Rtl8221_wait_for_bit(hDevice, MMD_VEND2, 0xB800, BIT_6, 0, 100);
        if (status != SUCCESS)
            goto exit;
       ...
       status = MmdPhyWrite(hDevice, MMD_VEND1, 0x75B5, 0xE086);
        ...
        if (singlephy)
        {
            for(i=0; i<sizeof(Rtl8221_isram_patch)/sizeof(MMD_REG); i++)
            {
                status = MmdPhyWrite(hDevice, Rtl8221_isram_patch[i].dev, Rtl8221_isram_patch[i].addr, Rtl8221_isram_patch[i].value);
                if (status != SUCCESS)
                    goto exit;
            }
        }

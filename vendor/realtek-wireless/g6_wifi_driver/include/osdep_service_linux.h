...
#define _rtw_spinlock_init  spin_lock_init
...
/*lock - mutex*/
	...
	typedef struct wfo_mutex		_mutex;
...
static __always_inline void _rtw_mutex_init(_mutex *pmutex)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 37))
...
static inline _thread_hdl_ rtw_thread_start_cpu(int (*threadfn)(void *data),
			void *data, const char namefmt[], unsigned int cpu)
{
	_thread_hdl_ _rtw_thread = NULL;
...
{
	return rtw_mem_pool_alloc_skb(sz);
}
#else /* !(CONFIG_RTW_MEMPOOL) */
static inline struct sk_buff *_rtw_skb_alloc(u32 sz)
{
#if defined(WKARD_98D) && \
	(defined(CONFIG_RTK_SOC_RTL8685XX) || defined(CONFIG_RTK_SOC_RTL9607C) || \
	 defined(CONFIG_RTK_SOC_RTL8198D) || defined(CONFIG_SOC_RTL8197F))
...
#define RTW_SMP_MB() smp_mb()
#define RTW_SMP_RMB() smp_rmb()
#define RTW_SMP_WMB() smp_wmb()

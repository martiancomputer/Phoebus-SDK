...
typedef enum _HT_IOT_PEER {
	...
	HT_IOT_PEER_QUALCOMM			= 20,
	HT_IOT_PEER_OCTOSCOPE			= 21,
	HT_IOT_PEER_QINGDAO_PDA			= 22,
	HT_IOT_PEER_FIBOCOM_PDA			= 23,
	HT_IOT_PEER_MAX
...
void issue_probersp(_adapter *padapter, unsigned char *da, u8 is_valid_p2p_probereq, u8 wmode);
...
int issue_om_with_nulldata(_adapter *padapter, struct sta_info *ptarget_sta, u16 tid, int try_cnt, int wait_ms);
...
#define RX_AMPDU_SIZE_MAX 0x100
...
u16 rtw_rx_ampdu_size_by_sta(_adapter *adapter, struct sta_info *psta, u16 addba_req_buf_size);
...
unsigned int OnAction_robust_av(_adapter *padapter, union recv_frame *precv_frame);
#endif
unsigned int on_action_vsp(_adapter *padapter, union recv_frame *precv_frame);
#ifdef CONFIG_RTW_80211R
...
void rtw_check_and_switch_color(_adapter *padapter, u64 bitmap);
void rtw_wnm_process_evt_rtp(_adapter *padapter,
...
void check_sa_query_mgmt_tx(_adapter *padapter, const u8 *buf, u32 len);
...
#ifdef CONFIG_QOS_MANAGEMENT
u8 add_scs_rsp_hdl(_adapter *padapter, unsigned char *pbuf);
u8 add_mscs_rsp_hdl(_adapter *padapter, unsigned char *pbuf);
#endif
...
#ifdef CONFIG_RTW_AP_BS_DECT
u8 set_bc_hdl(_adapter *padapter, unsigned char *pbuf);
#endif
...
struct rtw_cmd wlancmds[] = {
	...
	#ifdef CONFIG_RTW_AP_BS_DECT
	GEN_MLME_EXT_HANDLER("CMD_SET_COLOR", set_bc_hdl, NULL) /*CMD_SET_COLOR*/
	#endif
#ifdef CONFIG_QOS_MANAGEMENT
	GEN_MLME_EXT_HANDLER("CMD_ADD_SCSRSP", add_scs_rsp_hdl, NULL) /*CMD_ADD_SCSRSP*/
	GEN_MLME_EXT_HANDLER("CMD_ADD_MSCSRSP", add_mscs_rsp_hdl, NULL) /*CMD_ADD_SCSRSP*/
#endif
};
#endif
...
int proc_dump_env_stainfo_table(struct seq_file *m, void *v);
void rtw_record_env_stainfo(_adapter *padapter, union recv_frame *precv_frame);
struct env_sta_info *lookup_env_stainfo_table(_adapter *padapter, u8 *mac);
#endif

...
#define TX_RETRY_RECENT_CNT 100
...
struct	stainfo_stats	{
	...
	u32 tx_retry_ratio;
	u32 tx_retry_recent_total_pkts;
	u32 tx_retry_recent_retry_pkts;
	...
	u64 tx_bytes_per_sta;
	u64 rx_bytes_per_sta;
	u64 last_tx_bytes_per_sta;
	u64 last_rx_bytes_per_sta;
	u32 rt_sta_txtp;
	u32 rt_sta_rxtp;
};
...
struct rtw_delay_measure {
	u32 delay_num;
	u32 delay_total;
	u32 delay_max;
	u32 delay_min;
	u32 delay_avg;
	u32 delay_record[DELAY_RECORD_NUM + 1];
};

struct rtw_delay_measure_sta_entry {
	u8 mac_addr[MAC_ADDR_LEN];
	struct rtw_delay_measure tx_delay_measure;
	struct rtw_delay_measure rx_delay_measure;
};
#endif

enum SPEED_TEST_STATE {
	NOT_SPEEDTEST_STATE		= 0,
	SPEEDTEST_TX_STATE 		= 1,
	SPEEDTEST_RX_STATE 		= 2,
};

#define SPEEDTEST_TOLERANCE_PERIOD_TH	15
#define SPEEDTEST_TP_LOWER_BOUND		10
#define SPEEDTEST_NO_TP_UPPER_BOUND		8
#define UNDER_SPEEDTEST_STATE_TIMEOUT	150
#define SPEEDTEST_TX_PERIOD_TH			6
#define SPEEDTEST_RX_PERIOD_TH			5
#define SPEEDTEST_PERIOD_TOLERANCE		4

struct speedtest_pattern {
	enum SPEED_TEST_STATE state;
	u8 tx_period;
	u8 rx_period;
	u8 tolerance_period;
	u8 tx_cnt;	/* record current tx count after finish downlink state */
	u16 under_speed_test;
};
...
struct sta_info {
...
#ifdef FC_SBWC
	unsigned int egress_meter_idx;
	unsigned int ingress_meter_idx;
	unsigned int hostpolicer_idx;
#endif
	...
	u8 subpkt_addr1[NUM_RXSC_ENTRY][ETH_ALEN];
	u8 subpkt_idx;
	u8 subpkt_cnt;
#endif
#endif
...
#endif
	u32 akm_suite_type;
	u32 process_assoc_req_by_hapd;	/* use association response generated by hostapd */
...
#ifdef CONFIG_TX_MCAST2UNI
	u8 ipmc_num;
	u16 ipmc_aging_time[MAX_IP_MC_ENTRY];
...
//	u8 tbtx_timeslot;		/* This sta_info belong to which time slot.	*/
	...
	u32 txsc_entry_full;
	u8 txsc_active_entry;

#ifdef CONFIG_TXSC_AMSDU
	struct txsc_amsdu_swq amsdu_txq[8];
	u32 txsc_amsdu_timeout;
	u8 txsc_amsdu_num;
	u8 txsc_amsdu_max;
	u8 txsc_amsdu_zero;
	...
	_timer txsc_amsdu_tid_timer[8];
	u8 txsc_amsdu_tid_timeout_sts[8];
#endif /* CONFIG_TXSC_AMSDU */
	u8 txsc_sec_cam_idx;
#endif /* CONFIG_CORE_TXSC */
#ifdef CONFIG_ONE_TXQ
	u8 txq_rx_directly_deq;	/* bool */
	...
	u8 twt_update_ulmacid_cfg;
	struct rtw_twt_sta_config twt_config[MAX_NUM_TWT_CONFIG];
#endif
...
unsigned char multiap_profile;
	...
	u8 smooth_traffic_mode; /* refer enum TRAFFIC_MODE */
	...
	u8 xps_pe_id;	/* Transmit Packet Steering */
	u8 ppe_pe_id;
	u8 fc_staid;
	u8 pe_txsc_update;
	u8 is_btm_resp_recved;
	u8 twt_en;
	u16 wpseq_lmt;
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
	struct speedtest_pattern speedtest_pt;
	u16 sae_status;
	...
	u8 has_mscs;
	struct rtw_phl2_mscs_desc_data_s mscs_template;
	struct hlist_head mscs_tbl[QOS_MSCS_TBL_SZ];
#endif
};
...
static inline void rtw_presta_fill(struct rtw_presta_s *presta, struct rtw_phl_stainfo_t *phl_sta) {
	_rtw_memcpy(presta->mac_addr, phl_sta->mac_addr, ETH_ALEN);
#ifdef CONFIG_QOS_MANAGEMENT
	_rtw_memcpy(presta->scs_info, phl_sta->scs_info, sizeof(struct rtw_phl_scs_info) * MAX_SCS_DESCRIPTOR);
	_rtw_memcpy(presta->scs_temp, phl_sta->scs_temp, sizeof(struct rtw_phl_temp_scs_status) * MAX_TEMP_SCS_STATUS);
#endif
}

static inline void rtw_presta_ret(struct rtw_phl_stainfo_t *phl_sta, struct rtw_presta_s *presta) {
	_rtw_memcpy(phl_sta->mac_addr, presta->mac_addr, ETH_ALEN);
#ifdef CONFIG_QOS_MANAGEMENT
	_rtw_memcpy(phl_sta->scs_info, presta->scs_info, sizeof(struct rtw_phl_scs_info) * MAX_SCS_DESCRIPTOR);
	_rtw_memcpy(phl_sta->scs_temp, presta->scs_temp, sizeof(struct rtw_phl_temp_scs_status) * MAX_TEMP_SCS_STATUS);
#endif
}

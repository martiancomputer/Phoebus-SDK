...
struct SCSRspStatus_parm {
	u8 scsid;
	u16 status;
	u8 req_type;
};

struct addSCSRsp_parm {
	u8 addr[ETH_ALEN];
	u8 dialog_token;
	u8 scs_count;
	struct SCSRspStatus_parm scs_status[MAX_TEMP_SCS_STATUS];
};

struct addMSCSRsp_parm {
...
#ifdef CONFIG_QOS_MANAGEMENT
extern u8 rtw_addscsrsp_cmd(_adapter *padapter, struct sta_info *sta, u8 dialog_token);
extern u8 rtw_addmscsrsp_cmd(_adapter *padapter, struct addMSCSRsp_parm param);
#endif
...
extern u8 rtw_set_bc_cmd(_adapter *adapter);
#endif
...
enum rtw_cmd_id {
	...
	CMD_DELBA,
	CMD_SET_RUN_CMD_EN,
	CMD_SET_COLOR, // CONFIG_RTW_AP_BS_DECT
#ifdef CONFIG_QOS_MANAGEMENT
	CMD_ADD_SCSRSP,
	CMD_ADD_MSCSRSP,
#endif

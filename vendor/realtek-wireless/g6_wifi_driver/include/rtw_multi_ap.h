...
#define MULTI_AP_UNASSOC_STA_TIMEOUT (900)  /*unit: ms*/
...
enum rtw_map_ch_change_reason {
	MAP_CHANNEL_CHANGE_BY_ACS = 0,
	MAP_CHANNEL_CHANGE_BY_DFS,
	MAP_CHANNEL_CHANGE_BY_HAPD,
};
#endif
...
#define TUNNELED_MSG_DSCP_POLICY_QUERY	0x05
#define TUNNELED_MSG_DSCP_POLICY_REPSONSE	0x06
#define TUNNELED_MSG_RESERVED	0x07

#if defined(CONFIG_RTW_MULTI_AP) && defined(CONFIG_NSB_MULTI_AP_EVENT)
#define TUNNELED_MSG_ASSORES	0x10
#define TUNNELED_MSG_PROBEREQ	0x11
#define TUNNELED_MSG_PROBERES	0x12
#define TUNNELED_MSG_BTMRSP		0x13
#define TUNNELED_MSG_BCNREPORT	0x14

typedef struct multi_ap_cli_event{
	u8  tlv_type;
	u16 tlv_len;
	u8  macaddr[ETH_ALEN];
	u8  msg_type;
	...
	u16 frame_len;
	u8  frame[0];
}__attribute__ ((aligned(1), packed)) MULTI_AP_CLI_EVENT_STRU;
#endif
...
int core_map_update_bss(_adapter *padapter, u8 *tlv_content, u16 maximum_limit, int *processed_bytes);
...
void core_map_send_client_event(_adapter *adapter, u8* mac_address, u8 msg_type, u8 rssi, u16 payload_length, u8 *payload);

...
void rtw_sta_mbo_init(_adapter *padapter, struct sta_info *psta)
{
	struct mbo_priv *mbopriv = &psta->mbopriv;

	rtw_init_timer(&mbopriv->disassoc_imminent_timer, rtw_disassoc_imminent_timeout_handler, psta);
}

void rtw_sta_mbo_deinit(_adapter *padapter, struct sta_info *psta)
{
	struct mbo_priv *mbopriv = &psta->mbopriv;

	_cancel_timer_ex(&mbopriv->disassoc_imminent_timer);
}
...
void rtw_bss_termination_timeout_handler(void *ctx)
{
	...
	struct mbo_priv *mbopriv = &pmlmepriv->mbopriv;
	struct rtw_wifi_role_t *wrole = adapter->phl_role;
	void *phl = GET_HAL_INFO(adapter_to_dvobj(adapter));
	u32 dur = 250;

	if (mbopriv->bss_termination_phase == BSS_TERMINATION_NONE) {
		mbopriv->bss_termination_phase = BSS_TERMINATION_WAIT;

		rtw_sta_deauth_all(adapter);
		_set_timer(&mbopriv->bss_termination_timer, dur);
	}
	else if (mbopriv->bss_termination_phase == BSS_TERMINATION_WAIT) {
		...
		_set_timer(&mbopriv->bss_termination_dur_timer, mbopriv->bss_termination_dur*60*1000);
	}
}
...
void rtw_bss_termination_cancel_timer(_adapter *adapter)
{
	struct	mlme_priv *pmlmepriv = &adapter->mlmepriv;

	_cancel_timer_ex(&pmlmepriv->mbopriv.bss_termination_timer);
	_cancel_timer_ex(&pmlmepriv->mbopriv.bss_termination_dur_timer);
	pmlmepriv->mbopriv.bss_termination_phase = BSS_TERMINATION_NONE;
}

void rtw_disassoc_imminent_timeout_handler(void *ctx)
{
	...
	if (psta) {
		adapter = psta->padapter;
		mbopriv = &psta->mbopriv;

		if (DISASSOC_IMMINENT_STEP_NONE == mbopriv->disassoc_imminent_step) {
			issue_disassoc(adapter, psta->phl_sta->mac_addr, WLAN_REASON_DEAUTH_LEAVING);
			mbopriv->disassoc_imminent_step = DISASSOC_IMMINENT_STEP_SENT;
		}
		else if (DISASSOC_IMMINENT_STEP_SENT == mbopriv->disassoc_imminent_step) {
			ap_free_sta(adapter, psta, _TRUE, WLAN_REASON_DEAUTH_LEAVING, _TRUE, _TRUE);
			mbopriv->disassoc_imminent_step = DISASSOC_IMMINENT_STEP_DONE;
		}
		
		if (DISASSOC_IMMINENT_STEP_DONE != mbopriv->disassoc_imminent_step)
			_set_timer(&psta->mbopriv.disassoc_imminent_timer, DISASSOC_IMMINENT_TIMEOUT_WKARD);
	}
}

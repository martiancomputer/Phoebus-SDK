...
_adapter *adapter_led_share[CONFIG_IFACE_NUMBER];

void rtw_led_config(struct dvobj_priv *dvobj, u8 enable)
{
	u8 reg = rtw_phl_read8(dvobj->phl, 0x61) & 0xFE;

	if (!enable) {
		reg |= BIT(0);
	}

	rtw_phl_write8(dvobj->phl, 0x61, reg);
}

static void rtw_deinit_led_timer(_adapter *padapter)
{
	struct led_priv *ledpriv = adapter_to_led(padapter);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);

	_cancel_timer_ex(&ledpriv->led_timer);
	if (ledpriv->manual_ctrl == _FALSE)
...
void rtw_led_control(_adapter *adapter, LED_CTL_MODE mode)
{
	...
	ledpriv = adapter_to_led(adapter_led_share[RTW_SHARED_LED_ID]);
	...
	if (mode == LED_CTL_DOWN) {
		...
		if (is_iface_all_down == _TRUE) {
			RTW_INFO("[%s] deinit led timer\n", __func__);
			rtw_deinit_led_timer(adapter_led_share[RTW_SHARED_LED_ID]);
			ledpriv->mode = mode;
		} else
			return;
	}
...
void rtw_led_traffic_update(_adapter *adapter)
{
	...
	for (i = 0; i < CONFIG_IFACE_NUMBER; i++) {
		if (rtw_is_adapter_up(adapter_led_share[i]) == _TRUE) {
			pdvobj = adapter_to_dvobj(adapter_led_share[i]);
			delta += (pdvobj->traffic_stat.cur_tx_bytes + pdvobj->traffic_stat.cur_rx_bytes);
		}
	}

	ledpriv->traffic = (delta > 0) ? _TRUE : _FALSE;
}

static u8 rtw_led_get_toggle(struct led_priv *ledpriv)
{
	if (ledpriv->manual_ctrl)
		return ledpriv->manual_opt;

#ifdef CONFIG_WIFI_LED_SHARE_WITH_WPS
	if (ledpriv->last_mode)
		return (ledpriv->toggle + 1) % 2;
#endif

	if (ledpriv->mode == LED_CTL_DOWN)
		return RTW_LED_TOGGLE_OFF;

	if (ledpriv->mode == LED_CTL_UP_IDLE && !ledpriv->traffic)
		return RTW_LED_TOGGLE_ON;

	return (ledpriv->toggle + 1) % 2;
}

static void rtw_led_timeout_handler(void *ctx)
{
	...
	ledpriv->toggle = rtw_led_get_toggle(ledpriv);

	rtw_led_config(dvobj, ledpriv->toggle);

	_set_timer(&ledpriv->led_timer, ledpriv->interval);
}

static void rtw_set_sw_gpio_mode(_adapter *adapter)
{
	struct led_priv *ledpriv = adapter_to_led(adapter);
	...
	/* 0x4c[21] = 0, 0x60[16] = 1 */
	tmp = rtw_phl_read32(phl, 0x4c);
	tmp &= ~BIT21;
	rtw_phl_write32(phl, 0x4c, tmp);

	tmp = rtw_phl_read32(phl, 0x60);
	tmp |= BIT16;
	rtw_phl_write32(phl, 0x60, tmp);
}

void rtw_free_led_priv(_adapter *padapter)
{
	struct led_priv *ledpriv = adapter_to_led(padapter);
	...
	if (pdvobj->dev_id == RTW_SHARED_LED_ID)
		_cancel_timer_ex(&ledpriv->led_timer);
	adapter_led_share[pdvobj->dev_id] = NULL;
}
...
int	rtw_init_led_priv(_adapter *padapter)
{
	...
	adapter_led_share[pdvobj->dev_id] = padapter;

	RTW_INFO("[%s] dev_id:%d padapter:%px\n", __func__, pdvobj->dev_id, padapter);

	if (pdvobj->dev_id != RTW_SHARED_LED_ID)
		return 0;

	ledpriv->interval = RTW_LED_BLINK_INTERVAL;
	ledpriv->toggle = RTW_LED_TOGGLE_OFF;
	ledpriv->mode = LED_CTL_DOWN;
	ledpriv->traffic = _FALSE;
#ifdef CONFIG_WIFI_LED_SHARE_WITH_WPS
	ledpriv->last_mode = _FALSE;
#endif

	rtw_set_sw_gpio_mode(padapter);

	rtw_init_timer(&(ledpriv->led_timer), rtw_led_timeout_handler, padapter);
	_set_timer(&ledpriv->led_timer, ledpriv->interval);

	return 0;
}

#else /* CONFIG_RTW_SW_LED_SHARED */
void rtw_led_control(_adapter *adapter, LED_CTL_MODE mode)
{
	struct led_priv *ledpriv = adapter_to_led(adapter);
...
int	rtw_init_led_priv(_adapter *padapter)
{
	...
	/* set led defualt status to off */
	_rtw_led_control(padapter, RTW_LED_EVENT_SW_RF_OFF);

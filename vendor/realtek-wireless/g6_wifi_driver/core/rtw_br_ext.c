...
static void __nat25_db_print(_adapter *priv)
{
	...
	for (i = 0, j = 0; i < NAT25_HASH_SIZE; i++) {
		...
		while (db != NULL) {
#ifdef CL_IPV6_PASS
			RTW_PRINT("NAT25: DB(%d) H(%02d) C(%d) M:%02x%02x%02x%02x%02x%02x N:%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x"
				     "%02x%02x%02x%02x%02x%02x\n",
				     j,
				     i,
...
#ifdef BR_EXT_DEBUG
#ifdef CL_IPV6_PASS
						RTW_PRINT("NAT25 Expire H(%02d) M:%02x%02x%02x%02x%02x%02x N:%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x"
							"%02x%02x%02x%02x%02x%02x\n",
							     i,
							     f->macAddr[0],
							     f->macAddr[1],
...
int nat25_db_handle(_adapter *priv, struct sk_buff *skb, int method)
{
				...
				if (!__nat25_db_network_lookup_and_replace(priv, skb, networkAddr)) {
					if (*((unsigned char *)&iph->daddr + 3) == 0xff) {
						...
						if ((*(u32 *)priv->br_mac) == 0 && (*(u16 *)(priv->br_mac + 4)) == 0) {
							...
							RTW_PRINT("Re-init netdev_br_init() due to br_mac==0!\n");
	...
	/*                 Handle EAP frame                 */
	/*---------------------------------------------------*/
	else if (protocol == __constant_htons(ETH_P_EAPOL)) {
		switch (method) {
		case NAT25_CHECK:
			return -1;
...
int nat25_handle_frame(_adapter *priv, struct sk_buff *skb)
{
	...
	if (__constant_htons(ETH_P_EAPOL) == (*((unsigned short *)(skb->data + ETH_ALEN * 2))))
		return 0;

#ifdef BR_EXT_DEBUG
	if ((!priv->ethBrExtInfo.nat25_disable) && (!(skb->data[0] & 1))) {
		RTW_PRINT("NAT25: Input Frame: DA=%02x%02x%02x%02x%02x%02x SA=%02x%02x%02x%02x%02x%02x\n",
			     skb->data[0],
			     skb->data[1],
			     skb->data[2],
			     skb->data[3],

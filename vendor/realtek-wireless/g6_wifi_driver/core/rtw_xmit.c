...
{
	u8	sgi = _FALSE, sgi_20m = _FALSE, sgi_40m = _FALSE, sgi_80m = _FALSE, sgi_160m = _FALSE;
...
static s32 update_attrib_sec_info(_adapter *padapter, struct pkt_attrib *pattrib, struct sta_info *psta, enum eap_type eapol_type)
{
	...
	if (psta->ieee8021x_blocked == _TRUE ||
		...
		if ((pattrib->ether_type != ETH_P_EAPOL) && (check_fwstate(pmlmepriv, WIFI_MP_STATE) == _FALSE)) {
#ifdef DBG_TX_DROP_FRAME
			RTW_INFO("DBG_TX_DROP_FRAME %s psta->ieee8021x_blocked == _TRUE,  pattrib->ether_type(%04x) != ETH_P_EAPOL\n", __FUNCTION__, pattrib->ether_type);
#endif
			res = _FAIL;
			goto exit;
		}
...
#if defined(CONFIG_WAPI_SUPPORT) || defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
		if (pattrib->ether_type == ETH_P_WAPI)
		...
		if (((pattrib->encrypt == _WEP40_) || (pattrib->encrypt == _WEP104_)) && (pattrib->ether_type == ETH_P_EAPOL))
			pattrib->encrypt = _NO_PRIVACY_;
...
static void set_qos(_adapter *padapter, struct sk_buff *pkt, struct pkt_attrib *pattrib)
{
	...
	/* Get priority from pkt->cb[], which is obtained in rtw_core_tx() */
	UserPriority = pkt->cb[_SKB_CB_PRIORITY];

#ifdef CONFIG_ICMP_VOQ
	/* Use VO queue to send icmp packet */
	if (1 == pattrib->icmp_pkt)
		UserPriority = 7;
#endif

#ifdef CONFIG_IP_R_MONITOR
	if (ETH_P_ARP == pattrib->ether_type)
		UserPriority = 7;
#endif
...
static u8 _rtw_lps_chk_packet_type(struct pkt_attrib *pattrib)
{
	...
	#ifdef CONFIG_WAPI_SUPPORT
	if ((pattrib->ether_type == ETH_P_WAPI) || (pattrib->ether_type == ETH_P_ARP) ||
			(pattrib->ether_type == ETH_P_EAPOL) || (pattrib->dhcp_pkt == 1))
		pkt_type = LPS_PT_SP;
...
get_sta_info:
	...
	else if (ETH_P_EAPOL == XF_ETHTYPE)
	...
	if ((XF_ETHTYPE == ETH_P_EAPOL) || (XF_DHCP == 1))
...
static s32 update_xmitframe_security(_adapter *padapter, struct xmit_frame *pxframe) //rtw_phl_tx todo
{
	...
	if (XF_STA->ieee8021x_blocked == _TRUE) {
		...
		if ((XF_ETHTYPE != ETH_P_EAPOL) && (check_fwstate(pmlmepriv, WIFI_MP_STATE) == _FALSE)) {
#ifdef DBG_TX_DROP_FRAME
		...
		/* For WPS 1.0 WEP, driver should not encrypt EAPOL Packet for WPS handshake. */
		if (((XF_SEC_TYPE == _WEP40_) || (XF_SEC_TYPE == _WEP104_)) && (XF_ETHTYPE == ETH_P_EAPOL))
			XF_SEC_TYPE = _NO_PRIVACY_;
...
get_sta_info:
	...
	else if (ETH_P_EAPOL == pattrib->ether_type)
	...
	if ((pattrib->ether_type == ETH_P_EAPOL) || (pattrib->dhcp_pkt == 1)) {
		if (!rtw_is_scan_deny_bypass(padapter))
			rtw_mi_set_scan_deny(padapter, 3000);
		else
			RTW_WARN("Debug[%s]: bypass rtw_mi_set_scan_deny(3000)\n", __func__);
	}
...
//#endif
	...
	if (pkt->cb[_SKB_CB_ETH_AGG] == _PKT_TYPE_AGG_MULTI_PKT)
		pattrib->eth_agg_pkt =  _AGG_TYPE_MULTI_PKT;
	else
		pattrib->eth_agg_pkt =  0;

	if (pattrib->eth_agg_pkt) {
		pattrib->amsdu = 1;
	...
	if (padapter->swq_one_sta && pkt && _rtw_memcmp(padapter->swq_one_sta_mac, pkt->data, ETH_ALEN) == _TRUE)
		_rtw_memcpy(pkt->data + 3, &pkt->cb[_SKB_CB_MACADDR3], 3);
...
u16 rtw_get_sw_seq(_adapter *padapter, struct sta_info *sta, struct pkt_attrib *pattrib)
{
	u16 sw_seq = 0;
	u8 priority = (pattrib && pattrib->qos_en) ? pattrib->priority : RTW_PHL_RING_CAT_HIQ;

	sta->sta_xmitpriv.txseq_tid[priority]++;
	sta->sta_xmitpriv.txseq_tid[priority] &= 0xFFF;
	sw_seq = sta->sta_xmitpriv.txseq_tid[priority];

	RTW_DBG("tid = %d, seq = %d", priority, sw_seq);
	return sw_seq;
}
	...
	*/
	...
	if (pattrib->subtype & WIFI_DATA_TYPE) {
		...
		/* Update Seq Num will be handled by f/w */
		{
			...
			if (psta) {
				if (pattrib->qos_en) {
					pattrib->seqnum = rtw_get_sw_seq(padapter, psta, pattrib);
				} else {
					if (MLME_IS_STA(padapter)) 
						seq_psta = psta;
					else
						seq_psta = rtw_get_bcmc_stainfo(padapter);
					if (seq_psta)
						pattrib->seqnum = rtw_get_sw_seq(padapter, seq_psta, pattrib);
				}
...
s32 check_amsdu(struct xmit_frame *pxmitframe)
{
	...
	if (pattrib) {
		...
		if ((pattrib->ether_type == ETH_P_EAPOL) ||
			(pattrib->ether_type == ETH_P_ARP) ||
#if defined(CONFIG_WAPI_SUPPORT) || defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
			(pattrib->ether_type == ETH_P_WAPI) ||
#endif
			(pattrib->dhcp_pkt == 1))
			ret = _FALSE;
...
#define DBG_MGMT_XMIT_ENC_DUMP 0
	...
	if (bmcst) {
		if (subtype == WIFI_ACTION && CATEGORY_IS_GROUP_PRIVACY(category)) {
			...
			_rtw_memcpy(BIP_AAD + 2, pwlanhdr->addr1, ETH_ALEN);
			_rtw_memcpy(BIP_AAD + 2 + ETH_ALEN, pwlanhdr->addr2, ETH_ALEN);
			_rtw_memcpy(BIP_AAD + 2 + 2 * ETH_ALEN, pwlanhdr->addr3, ETH_ALEN);
...
void rtw_count_tx_wmm_stats(_adapter *padapter, u8 priority, u32 pkt_sz, u32 drop_pkts)
{
	struct xmit_priv	*pxmitpriv = &padapter->xmitpriv;
	u8 index = 4;
	...
	if (drop_pkts == 0) {
		padapter->wmm_stats[index].tx_pkts++;
		padapter->wmm_stats[index].tx_bytes += pkt_sz;
	} else {
		padapter->wmm_stats[index].tx_drops += drop_pkts;
	}
}

static void rtw_reverse_count_tx_wmm_stats(_adapter *padapter, u8 priority, u8 pkt_cnt, u32 pkt_sz)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	...
	padapter->wmm_stats[index].tx_pkts -= pkt_cnt;
	padapter->wmm_stats[index].tx_bytes -= pkt_sz;
}
...
void rtw_count_tx_stats_tx_req(_adapter *padapter, struct rtw_xmit_req *txreq, struct sta_info *psta)
{
	...
	//RT_TRX_TRAFFIC
	_adapter *pri_adapter = GET_PRIMARY_ADAPTER(padapter);
	struct xmit_priv *pri_pxmitpriv = &pri_adapter->xmitpriv;
	...
	if (txreq->mdata.type == RTW_PHL_PKT_TYPE_DATA) {
		...
		pri_pxmitpriv->tx_bytes_per_band += sz;//RT_TRX_TRAFFIC

		rtw_count_tx_wmm_stats(padapter, txreq->mdata.tid, sz, 0);

		if (psta) {
			pstats = &psta->sta_stats;
			pstats->tx_pkts += (txreq->pkt_cnt - 1);
			pstats->tx_bytes += sz;
			pstats->tx_bytes_per_sta += sz;//RT_TRX_TRAFFIC
		...
		if (txreq->mdata.bc && txreq->mdata.mc) {
			...
			pxmitpriv->tx_uc_pkts += (txreq->pkt_cnt - 1);
			pxmitpriv->tx_uc_bytes += sz;
			if (pstats)
				pstats->tx_uc_pkts += (txreq->pkt_cnt - 1);
		}
	}
}
...
void rtw_reverse_count_tx_stats_tx_req(_adapter *padapter, struct rtw_xmit_req *txreq, struct sta_info *psta)
{
	...
	if (txreq->mdata.type == RTW_PHL_PKT_TYPE_DATA) {
		...
		rtw_reverse_count_tx_wmm_stats(padapter, txreq->mdata.tid, (txreq->pkt_cnt - 1), sz);
...
void rtw_count_tx_stats(_adapter *padapter, struct xmit_frame *pxmitframe, int sz)
{
	...
	if (pxmitframe->xftype == RTW_TX_OS) {
		...
		rtw_count_tx_wmm_stats(padapter, pxmitframe->attrib.priority, sz, 0);
...
void rtw_reverse_count_tx_stats(_adapter *padapter, struct xmit_frame *pxmitframe, int sz)
{
	...
	if (pxmitframe->xftype == RTW_TX_OS) {
		...
		rtw_reverse_count_tx_wmm_stats(padapter, pxmitframe->attrib.priority, 1, sz);
...
s32 core_tx_free_xmitframe(_adapter *padapter, struct xmit_frame *pxframe)
{
	...
	rtw_delay_measure_done_tx(padapter, pxframe->pkt, psta);
#endif
...
void rtw_core_free_sta_xmit_queue(_adapter *padapter, struct sta_xmit_priv *pstaxmitpriv)
{
	...
	if (pstaxmitpriv->tx_pending_bitmap) {
		...
		for (q_idx = 0; q_idx < MAX_TXQ; q_idx++) {
			...
			cnt += txq->qhead.qlen;
			while((pskb = __skb_dequeue(&txq->qhead)) != NULL) {
...
#if defined(CONFIG_WAPI_SUPPORT) || defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
	if (pxmitframe->attrib.ether_type != ETH_P_WAPI) {
...
/* TXREQ_QMGT */
u8 *get_txreq_buffer(_adapter *padapter, u8 **txreq, u8 **pkt_list, u8 **head, u8 **tail, u8 txsc, struct sta_info *psta, u8 is_critical)
{
	struct xmit_txreq_buf *ptxreq_buf = NULL;
	_list *plist, *phead;
	...
	if ((padapter->dvobj->wfo_en & RTL_WFO_TX_OFLD) && psta->pe_txsc_update)
		is_critical = 1;
#endif
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	} else if ((padapter->dvobj->wfo_en & RTL_WFO_TX_OFLD) &&
		pfree_txreq_queue->qlen <= 1 && !psta->pe_txsc_update) {
		/* always reserve 1 txreq for PE TXSC update */
		padapter->txreq_full_cnt++;
#endif
	} else if (!is_critical &&
		pfree_txreq_queue->qlen <= 8) {
		/* always reserve 8 txreq for critical pkts(eapol) */
		padapter->txreq_full_cnt++;
	} else {
...
s32 core_tx_update_pkt(_adapter *padapter, struct xmit_frame *pxframe, struct sk_buff **pskb)
{
	...
	/* WFO: SA replaced by FC with VXD mac */
	if (padapter->registrypriv.wifi_mib.fc_nat25_en) {
		if(skb_orig->cb[_SKB_CB_ACC] != 0) {
			return SUCCESS;
		}
	}
			       ...
			       u32 frag_perfr, u32 wl_frags, struct sta_info *psta)
{
	...
	is_critical = (skb->cb[_SKB_CB_FLAGS] & _PKT_TYPE_EAPOL)?1:0;
	//get_txreq_resources(padapter, pxframe, &txreq, &list, &head, &tail);
	/* TXREQ_QMGT */
	txreq_buf = (struct xmit_txreq_buf *)get_txreq_buffer(padapter, &txreq, &list, &head, &tail, 0, psta, is_critical);
	if(txreq_buf == NULL){
...
void check_rts(_adapter *padapter, struct sta_info *psta, struct rtw_t_meta_data *mdata)
{
	...
	if (padapter->registrypriv.wifi_mib.cts2self == 1) {
		...
		mdata->rts_cca_mode = 0;
		return;
	} else if (padapter->registrypriv.wifi_mib.rtscts == 1) {
		mdata->rts_en = 1;
		mdata->cts2self = 0;
		mdata->rts_cca_mode = padapter->cca_rts_mode;
		mdata->hw_rts_en = (padapter->cca_rts_mode?0:1);
		return;
	}

#ifdef CONFIG_AUTOTEST_DETECT
	if (padapter->dvobj->autotest_case == AUTOTEST_5G_BE_VI_LATENCY) {
		mdata->hw_rts_en = 0;
		mdata->rts_en = 0;
		mdata->cts2self = 0;
		mdata->rts_cca_mode = 0;
...
#ifdef CONFIG_VW_REFINE
	if (1 == padapter->no_rts ||
		VW_MODE_LATENCY == padapter->vw_enable) {
		mdata->rts_en = 0;
		mdata->cts2self = 0;
		mdata->hw_rts_en = 0;
		mdata->rts_cca_mode = 0;
	...
	//_CCA_RTS_MODE_
	if (padapter->cca_rts_mode) {
		if(wmode == WLAN_MD_11B) {
			mdata->rts_en = 0;
			mdata->cts2self = 1;
		} else {
	...
	mdata->rts_en = psta->rtsen;
	mdata->hw_rts_en = psta->rtsen;
	mdata->cts2self = psta->cts2self;
	return;
...
extern u8 high_queue_debug_enable;
extern u8 high_queue_index;
...
void fill_txreq_mdata(_adapter *padapter, struct xmit_frame *pxframe)
{
...
#ifdef CONFIG_ETHER_PKT_AGG
	txreq->eth_agg_pkt = pxframe->attrib.eth_agg_pkt;
	if (txreq->eth_agg_pkt == 0 && pxframe->pkt && ((pxframe->pkt->len == (512-4)) || (pxframe->pkt->len == (512-8)) || (pxframe->pkt->len == (1518-4)) || (pxframe->pkt->len == (1518-8))))
		txreq->tc_non_agg_pkt = 1;
#endif
	...
	if (XF_SEC_TYPE == _NO_PRIVACY_ || XF_SWENC == _TRUE) {
		...
		if (mdata->bc || mdata->mc)
		{
			mdata->hw_sec_iv = 0;
			mdata->sw_sec_iv = 0;
			mdata->iv[0] = pxframe->attrib.iv[0];
			mdata->iv[1] = pxframe->attrib.iv[1];
			mdata->iv[2] = pxframe->attrib.iv[4];
			mdata->iv[3] = pxframe->attrib.iv[5];
			mdata->iv[4] = pxframe->attrib.iv[6];
			mdata->iv[5] = pxframe->attrib.iv[7];
		}
	...
	if (pxframe->attrib.ampdu_en == _TRUE) {
		...
		if ((padapter->mlmeextpriv.cur_channel < 36)
	...
	if ((pxframe->xftype != RTW_TX_DRV_MGMT) && (mdata->mc || mdata->bc)) {
		if (high_queue_debug_enable)
			mdata->cat = high_queue_index;
		else
...
#ifdef CONFIG_CORE_TXSC
	mdata->wp_offset = 56;
	mdata->wd_page_size = (mdata->wdinfo_en) ? 1 : 0;
...
u8 _txreq_lmt_check_sta_bcast(struct sta_info *psta)
{
	u8 ret = _TRUE;
	struct dvobj_priv *dvobj = adapter_to_dvobj(psta->padapter);

	if (dvobj->lmt_sta_bcast_txreq && ATOMIC_READ(&psta->num_pending_txreq) >= dvobj->lmt_sta_bcast_txreq) {
		psta->lmt_txreq_drop++;
		ret = _FALSE;
	}
...
{
	u8 ret = _TRUE;
	uint rtw_txreq_limit = padapter->dvobj->wmm_mode_txreq_lmt;
	static const u8 tid_2_qsel[8] = {0 ,1, 1, 0, 2, 2, 3, 3};
	...
	*acq = tid_2_qsel[txreq->mdata.tid];
	...
	if (psta && (padapter->lmt_txreq_enable == 1 || padapter->dvobj->lmt_txreq_enable == 1)) {
...
#ifdef CONFIG_AUTOTEST_DETECT
	if (padapter->dvobj->autotest_case == AUTOTEST_24G_LATENCY)
		rtw_txreq_limit = padapter->dvobj->autotest_txreq_lmt?padapter->dvobj->autotest_txreq_lmt:64;
#endif
		if ((padapter->dvobj->wmm_mode == 1 || padapter->dvobj->wmm_test) && rtw_txreq_limit
...
#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
		if (txreq->mdata.sec_hw_enc == 1 && txreq->mdata.sec_type == RTW_ENC_WAPI && txreq->pkt_cnt) {

			pkt_list = (struct rtw_pkt_buf_list *)txreq->pkt_list;
			//pwapiExt = pkt_list->vir_addr + txreq->mdata.hdr_len;

			#ifdef CONFIG_CORE_TXSC

			if (ptxsc_txreq) {

				wapi_wlan_hdr = rtw_zmalloc(pkt_list->length);

				if (wapi_wlan_hdr == NULL) {
					RTW_ERR("%s: allocate wapi wlan header failed.\n", __func__);
					goto abort_call_phl;
				}

				txreq->wapi_txsc = 1;

				_rtw_memcpy(wapi_wlan_hdr, pkt_list->vir_addr, pkt_list->length);

				pkt_list->vir_addr = wapi_wlan_hdr;

				pwapiExt = pkt_list->vir_addr + (txreq->mdata.hdr_len << 1);

				rtw_wapi_get_iv(padapter, pkt_list->vir_addr + 4, pwapiExt);
...
s32 rtw_core_tx(_adapter *padapter, struct sk_buff **pskb, struct sta_info *psta)
{
	...
	if(psta && psta == padapter->self_sta) {
		if (_txreq_lmt_check_sta_bcast(psta) == _FALSE)
			goto abort_core_tx;
	}
#endif
...
#if !defined(CONFIG_RTL_WIFI_OFFLOAD)
		if (psta != NULL)
			txsc_issue_addbareq_cmd(padapter, 0, psta, _TRUE);
#endif /* !defined(CONFIG_RTL_WIFI_OFFLOAD) */
	}
#endif
#endif
	...
	if (/*(pxframe->attrib.ether_type != ETH_P_ARP)
	    &&*/ (pxframe->attrib.ether_type != ETH_P_EAPOL)
...
core_txsc:
	...
	if (txsc_pkt.step == TXSC_AMSDU_APPLY && txsc_amsdu_enqueue(padapter, &txsc_pkt, &status) == _SUCCESS) {
		if (status == TXSC_AMSDU_ENQ_ABORT) {
			...
			if (txq->cnt >= MAX_AMSDU_ENQ_NUM) {
				txsc_release_entry_use_cnt(padapter, &txsc_pkt);
				_txsc_one_spinunlock_bh(&pxmitpriv->txsc_lock, NULL);
				txsc_amsdu_timeout_tx(psta, txsc_pkt.ac);
				return SUCCESS;
			}
			#endif

			already_enqueued = _TRUE;
			#ifdef CONFIG_WFA_OFDMA_Logo_Test_Statistic
	...
	if (0 != my_pskb->cb[_SKB_VW_FLAG]) {
		...
		if ( (1 == pxmitpriv->txsc_amsdu_enable) &&
			...
			printk_ratelimited(KERN_WARNING "txsc err code:0x%x cb:0x%x vw: 0x%x step: 0x%x\n",
				txsc_pkt.err, my_pskb->cb[_SKB_CB_FLAGS], my_pskb->cb[_SKB_VW_FLAG], txsc_pkt.step);
...
direct_call_phl:
	/* for slow path debug */
	print_txreq_pklist(pxframe, NULL, *pskb, __func__);
...
abort_core_tx:
	...
	if (ret == FAIL) {
		...
		rtw_count_tx_wmm_stats(padapter, my_pskb->cb[_SKB_CB_PRIORITY], 0, 1);
	}
	...
	if (pxframe == NULL){
		...
		{
#ifdef CONFIG_TXSC_AMSDU
			if (!txsc_pkt.amsdu || !already_enqueued)
#endif
		    	rtw_os_pkt_complete(padapter, *pskb);
		}
	} else {
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
s32 rtw_core_add_ofld_txsc(_adapter *adapter, struct sta_info *psta, u8 *mac)
{
	struct sk_buff *pskb;
	struct xmit_frame *pxframe;
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	...
	u8 wd_cache[MAX_WD_SIZE];
	u8 wd_len, wd_seq_offset;
	struct rtw_pkt_buf_list *pkt;
	s32 status = SUCCESS;

	if (!psta || !psta->phl_sta || !psta->phl_sta->active) {
		RTW_ERR("station is not ready.\n");
		return FAIL;
	}

	pskb = alloc_skb(128, GFP_ATOMIC);
	skb_put(pskb, ETH_HLEN);
	memcpy(pskb->data, mac, ETH_ALEN);
	memcpy(pskb->data+ETH_ALEN, adapter->mac_addr, ETH_ALEN);
	memcpy(pskb->data+ETH_ALEN*2, "\x08\x00", 2);
	if (core_tx_alloc_xmitframe(adapter, &pxframe, pskb) != SUCCESS) {
		RTW_ERR("fail to allocate xmitframe\n");
		kfree_skb(pskb);
		return FAIL;
	}

	psta->pe_txsc_update = true;
	if (core_tx_update_xmitframe(adapter, pxframe, &pskb, psta, RTW_TX_OS) ||
		core_tx_prepare_phl(adapter, pxframe) ||
		(RTW_PHL_STATUS_SUCCESS != phl_wfo_add_txsc(phl_info,
			pxframe->phl_txreq, wd_cache, &wd_len, &wd_seq_offset))) {
		RTW_ERR("fail to add txsc\n");
		status = FAIL;
	}
	psta->pe_txsc_update = false;

	if (SUCCESS == status) {
		pkt = (struct rtw_pkt_buf_list *)pxframe->phl_txreq->pkt_list;
		/* wd_len: wd_body_len + (wd_info_len) + seq_info_len + addr_info_len.
			Use wd_seq_offset as PE TXSC wd_len */
		wd_len = wd_seq_offset;
		rtl_wfo_ipc_update_txsc_info(adapter, psta, pkt->vir_addr,
			pkt->length, wd_cache, wd_len, wd_seq_offset);
	}
	core_tx_free_xmitframe(adapter, pxframe);

	return status;
}

enum rtw_phl_status rtw_core_process_pwr_state(void *drv_priv,
		u8 pwrbit, u16 macid, u16 seq)
{
	...
	if (pwrbit && !(psta->state & WIFI_SLEEP_STATE))
		stop_sta_xmit(padapter, psta);
	else if (!pwrbit && (psta->state & WIFI_SLEEP_STATE))
		wakeup_sta_to_xmit(padapter, psta);

	return RTW_PHL_STATUS_SUCCESS;
}

enum rtw_phl_status rtw_core_process_tim_indic(void *drv_priv, u16 macid)
{
	struct dvobj_priv *dvobj = (struct dvobj_priv *)drv_priv;
	_adapter *padapter;
	struct sta_info *psta = NULL;
	...
	if (!(MLME_IS_AP(padapter) || MLME_IS_MESH(padapter))) {
		RTW_ERR("%s: Adapter %pM is not AP\n", __func__, padapter->mac_addr);
		return RTW_PHL_STATUS_FAILURE;
	}
	if (psta == NULL) {
		RTW_ERR("%s: NULL STA (macid %d) !!\n", __func__, macid);
	...
	/* Set ALL tx_pending_bitmap for STA
	 * Do we need lock to protect tx_pending_bitmap ?
	 */
	for (i = 0; i < MAX_TXQ; i++)
		rtw_set_bit(i, &pstaxmitpriv->tx_pending_bitmap);
	...
	if (psta->state & WIFI_SLEEP_STATE) {
		if (psta->uapsd_bitmap == (BIT(TXQ_VO)|BIT(TXQ_VI)|BIT(TXQ_BE)|BIT(TXQ_BK)) ||
			...
			if (!rtw_tim_map_is_set(padapter, pstapriv->tim_bitmap, psta->phl_sta->aid)) {
				...
				RTW_DBG("%s: STA macid %u set TIM bitmap for aid %u\n",
						__func__, macid, psta->phl_sta->aid);
				//RTW_MAP_DUMP_SEL_ALWAYS(NULL, "tim_bitmap=", pstapriv->tim_bitmap,
				//		pstapriv->aid_bmp_len);
			}
		}
...
enum rtw_phl_status rtw_count_pe_tx_stats(void *drv_priv, void *append)
{
	struct dvobj_priv *dvobj = (struct dvobj_priv *)drv_priv;
	struct tx_stats_append *app = (struct tx_stats_append *)append;
	_adapter *padapter;
	struct sta_info *psta = NULL;
	...
	u8 i, wmm_idx;
	const static u8 wmm_tid_map[8] = {
		XMIT_BE_QUEUE, XMIT_BK_QUEUE, XMIT_BK_QUEUE, XMIT_BE_QUEUE,
		XMIT_VI_QUEUE, XMIT_VI_QUEUE, XMIT_VO_QUEUE, XMIT_VO_QUEUE
	};

	/* lookup all interfaces */
	for (i=0; i<dvobj->iface_nums; i++) {
		padapter = dvobj->padapters[i];
		psta = rtw_get_stainfo_by_macid(&padapter->stapriv, app->macid);
		if (psta)
			break;
	}
	if (psta == NULL) {
	...
	pmlmepriv->LinkDetectInfo.NumTxOkInPeriod += app->tx_pkts;
	pxmitpriv->tx_pkts += app->tx_pkts;
	pxmitpriv->tx_bytes += app->tx_bytes;
	pxmitpriv->tx_bytes_per_band += app->tx_bytes;//RT_TRX_TRAFFIC

	wmm_idx = wmm_tid_map[app->tid];
	padapter->wmm_stats[wmm_idx].tx_pkts += app->tx_pkts;
	padapter->wmm_stats[wmm_idx].tx_bytes += app->tx_bytes;

	pstats = &psta->sta_stats;
	pstats->tx_pkts += app->tx_pkts;
	pstats->tx_bytes += app->tx_bytes;
	pstats->tx_pkt_len = app->tx_pkt_len;
	pstats->tx_bytes_per_sta += app->tx_bytes;//RT_TRX_TRAFFIC

	switch (psta->dot118021XPrivacy) {
	case _WEP40_:
	case _WEP104_:
		psta->dot11txpn.val = (psta->dot11txpn.val + app->tx_pkts) & 0xffffff;
		break;
	case _TKIP_:
	case _TKIP_WTMIC_:
	case _AES_:
	...
	case _CCMP_256_:
	case _GCMP_256_:
		psta->dot11txpn.val = (psta->dot11txpn.val + app->tx_pkts) & 0xffffffffffffULL;
	default:
		break;
	}
	...
	if (pxmitpriv->txsc_amsdu_enable == 1 &&
		...
		if ((pstats->tx_pkts - pstats->last_tx_pkts) > padapter->registrypriv.wifi_mib.amsdu_pps) {
			...
			rtl_wfo_upd_sta_amsdu_info(psta);
	...
	pxmitpriv->tx_uc_pkts += app->tx_pkts;
	pxmitpriv->tx_uc_bytes += app->tx_bytes;

	return RTW_PHL_STATUS_SUCCESS;
}

enum rtw_phl_status rtw_core_pe_trx_pause(void *drv_priv)
{
	struct dvobj_priv *dvobj = (struct dvobj_priv *)drv_priv;
	...
	rtl_wfo_stop_pe(padapter);

	return RTW_PHL_STATUS_SUCCESS;
}

enum rtw_phl_status rtw_core_pe_trx_resume(void *drv_priv)
{
	struct dvobj_priv *dvobj = (struct dvobj_priv *)drv_priv;
	_adapter *padapter = dvobj_get_primary_adapter(dvobj);
	...
	if (dvobj->wfo_not_start_pe)
		dvobj->wfo_not_start_pe = 0;
	else if (padapter->netif_up && !dvobj->wfo_pe_stopped)
		rtl_wfo_start_pe(padapter);

	return RTW_PHL_STATUS_SUCCESS;
}

enum rtw_phl_status rtw_core_pe_tx_force_para(void *drv_priv)
{
	struct dvobj_priv *dvobj = (struct dvobj_priv *)drv_priv;
	_adapter *padapter = dvobj_get_primary_adapter(dvobj);
	...
	if (!RTL_WFO_IS_OFLD(dvobj))
		return RTW_PHL_STATUS_FAILURE;
#ifdef CONFIG_VW_REFINE
	apply_pe_tx_force_para(padapter);
#endif
	return RTW_PHL_STATUS_SUCCESS;
}
...
enum rtw_phl_status rtw_core_tx_recycle(void *drv_priv, struct rtw_xmit_req *txreq)
{
	...
	if (txreq->treq_type == RTW_PHL_TREQ_TYPE_CORE_TXSC
		...
		#ifdef CONFIG_VW_REFINE
		swq->vw_recycle_cnt[macid] += txreq->vw_cnt;
		#endif
		#ifdef RTW_PHL_DBG_CMD
...
inline bool xmitframe_hiq_filter(struct xmit_frame *xmitframe)
{
	...
	if (adapter->registrypriv.wifi_spec == 1)
		...
		if (attrib->ether_type == ETH_P_ARP
		    || attrib->ether_type == ETH_P_EAPOL
#ifdef CONFIG_WAPI_SUPPORT
		    || attrib->ether_type == ETH_P_WAPI
#endif
		    || attrib->dhcp_pkt
		   ) {
			if (0)
...
#ifdef CONFIG_RTW_MULTI_AP_R3
	core_map_deinit_service_prioritization(padapter);
#endif
}


#ifdef CONFIG_DELAY_MEASUREMENT
void rtw_delay_measure_record_tx(_adapter *padapter, struct sk_buff *pkt)
{
	...
	*(u32 *)&pkt->cb[_SKB_CB_TS] = 0;

	if (!padapter->registrypriv.wifi_mib.delay_measure)
		return;

	pstatus = rtl_phl_cmd_get_cur_tsf(padapter->dvobj->phl, &cur_tsf,
				padapter->phl_role,
				PHL_CMD_DIRECTLY, 0);

	if (pstatus == RTW_PHL_STATUS_SUCCESS)
		*(u32 *)&pkt->cb[_SKB_CB_TS] = cur_tsf.tsf_l;
}

void rtw_delay_measure_done_tx(_adapter *padapter, struct sk_buff *pkt, struct sta_info *psta)
{
	struct rtw_phl_port_tsf cur_tsf;
	u32 tsf_diff = 0;
	u32 tsf_start;
	...
	tsf_start = *(u32 *)&pkt->cb[_SKB_CB_TS];
	if (tsf_start == 0)
		return;
	...
	if (tsf_start > cur_tsf.tsf_l)
		tsf_diff = 0xffffffff - tsf_start + cur_tsf.tsf_l;
	else
		tsf_diff = cur_tsf.tsf_l - tsf_start;

	if (tsf_diff%1000 < 500)
		tsf_diff = tsf_diff/1000;
	else
		tsf_diff = tsf_diff/1000 + 1;

	if (tsf_diff <= DELAY_RECORD_SLOT[0]) {
		psta->tx_delay_measure.delay_record[0]++;
	} else {
		for (i = DELAY_RECORD_NUM - 1; i >= 0; i--) {
			if (tsf_diff > DELAY_RECORD_SLOT[i]) {
				psta->tx_delay_measure.delay_record[i + 1]++;
				break;
			}
		}
	}

	if (tsf_diff > psta->tx_delay_measure.delay_max)
		psta->tx_delay_measure.delay_max = tsf_diff;
	if (psta->tx_delay_measure.delay_min == 0 || tsf_diff < psta->tx_delay_measure.delay_min)
		psta->tx_delay_measure.delay_min = tsf_diff;

	psta->tx_delay_measure.delay_total += tsf_diff;
	psta->tx_delay_measure.delay_num ++;
}

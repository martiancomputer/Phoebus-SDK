		...
		struct nb_rpt_hdr *nb_report)
{
	...
	if (p && ie_len > 0) {
		if (rtw_parse_wpa2_ie(p, ie_len + 2, NULL, NULL, NULL, NULL, NULL, NULL) == _SUCCESS) {
...
struct dot11k_beacon_measurement_report * get_sta_rm_bcn_rpt(struct sta_info *psta, u8 *pframe, int len)
{
	...
	if (psta->rm_beacon_rpt_num < RTW_MAX_RADIO_RPT_NUM
	    && bssid_exist == 0) {
		#ifdef CONFIG_RTW_AP_EXT_SUPPORT
		if (psta->rm_beacon_rpt_list[psta->rm_beacon_rpt_num] == NULL) {
			psta->rm_beacon_rpt_list[psta->rm_beacon_rpt_num] = rtw_zmalloc(sizeof(struct dot11k_beacon_measurement_report));
			if (!psta->rm_beacon_rpt_list[psta->rm_beacon_rpt_num])
				return NULL;
		}
	...
	if (pframe[len + 3] == 0) {
		...
		while (report_copy_len < report_len + 2)
		{
			...
			if (subelement_id == _FRAME_BODY_SUBIE_) {
				...
				for (j= 0 ; j < psta->rm_beacon_rpt_num; j++) {
					if (psta->rm_beacon_rpt_list[j] == NULL)
						continue;
					if (_TRUE == _rtw_memcmp(psta->rm_beacon_rpt_list[j]->info.bssid, beacon_rep->info.bssid, ETH_ALEN)) {
		...
		if (beacon_rep) {
			...
			if (frame_fragment) {
				...
				if (beacon_rep->subelements_len + subelements_copy_len - 2 <= MAX_BEACON_LEN) {
					memcpy(beacon_rep->subelements + beacon_rep->subelements_len, pframe + subelements_begin + 2, subelements_copy_len - 2);
					beacon_rep->subelements_len += subelements_copy_len - 2;
				}
			} else {
				if (beacon_rep->subelements_len + subelements_copy_len <= MAX_BEACON_LEN) {
					memcpy(beacon_rep->subelements + beacon_rep->subelements_len, pframe + subelements_begin, subelements_copy_len);
	...
	union recv_frame *precv_frame, struct sta_info *psta)
{
	...
	struct dot11k_beacon_measurement_report *bcn_rpt;
	...
	while (len < frame_len) {
		...
		if (element_id == _MEAS_RSP_IE_) {
			if(pdiag_body[len + 4] == bcn_rep) {
				...
				bcn_rpt = get_sta_rm_bcn_rpt(psta, pdiag_body, len);
...
unsigned int rm_on_action(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	case RM_ACT_RADIO_MEAS_REP:
		...
		core_map_send_client_event(padapter, padapter->mac_addr, TUNNELED_MSG_BCNREPORT, precv_frame->u.hdr.attrib.phy_info.signal_strength, precv_frame->u.hdr.len, pframe);
#endif
...
static int retrieve_scan_result(struct rm_obj *prm)
{
	...
	queue = &padapter->dvobj->rm_scanned_queue;
#else
		...
		*pbuf = psta->rm_beacon_rpt_status;
		...
		if(psta->rm_beacon_rpt_num > 0 && psta->rm_beacon_rpt_num <= RTW_MAX_RADIO_RPT_NUM) {
			...
			for (i = 0; i < psta->rm_beacon_rpt_num; i++) {
				if (psta->rm_beacon_rpt_list[i] == NULL)
					continue;
				_rtw_memcpy(pbuf + len, psta->rm_beacon_rpt_list[i], sizeof(struct dot11k_beacon_measurement_report));
				len += sizeof(struct dot11k_beacon_measurement_report);
				bcn_rpt_num++;
			}
...
void rtw_rm_free_sta_beacon_report(struct sta_info *psta, u8 lock)
{
	int i;
	if (!psta)
		return;
	...
	if (lock)
		_rtw_spinlock_bh(&psta->lock);
	for (i = 0; i < RTW_MAX_RADIO_RPT_NUM; i++) {
		if (psta->rm_beacon_rpt_list[i]) {
			rtw_mfree(psta->rm_beacon_rpt_list[i], sizeof(struct dot11k_beacon_measurement_report));
			psta->rm_beacon_rpt_list[i] = NULL;
		}
	}
	if (lock)
		_rtw_spinunlock_bh(&psta->lock);
...
/* update BSS load IE periodically */
void rtw_rm_update_beacon(_adapter *padapter)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);

	if (is_primary_adapter(padapter))
		dvobj->update_qbss_ie_timeout++;
	if (dvobj->update_qbss_ie_timeout >= 10) {
		update_beacon(padapter, EID_QBSSLoad, NULL, _TRUE, 0);
		if (is_primary_adapter(padapter))
			dvobj->update_qbss_ie_timeout = 0;
	}
}

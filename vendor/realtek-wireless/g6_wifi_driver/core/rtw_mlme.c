...
sint	_rtw_init_mlme_priv(_adapter *padapter)
{
	...
	if (is_primary_adapter(padapter))
	{
		...
		_rtw_init_queue(&(d->rm_free_bss_pool));
		_rtw_init_queue(&(d->rm_scanned_queue));

		d->rm_max_bss_cnt = RM_MAX_BSS_CNT;

		pbuf = rtw_zvmalloc(d->rm_max_bss_cnt * (sizeof(struct wlan_network)));
		if (pbuf == NULL) {
			res = _FAIL;
			goto exit;
		}
		d->rm_free_bss_buf = pbuf;

		pnetwork = (struct wlan_network *)pbuf;
		for (i = 0; i < d->rm_max_bss_cnt; i++) {
			_rtw_init_listhead(&(pnetwork->list));
			rtw_list_insert_tail(&(pnetwork->list), &(d->rm_free_bss_pool.queue));
			pnetwork++;
		}
#endif
	}
...
#ifdef RADIO_MEASUREMENT_SPARE_SCAN_LIST
		_rtw_spinlock_free(&(d->rm_free_bss_pool.lock));
		_rtw_spinlock_free(&(d->rm_scanned_queue.lock));
#endif
	}
#else
...
#ifdef RADIO_MEASUREMENT_SPARE_SCAN_LIST
	u8 *rm_pfree_bss_buf = d->rm_free_bss_buf;
	u32 rm_max_bss_cnt = d->rm_max_bss_cnt;
#endif
#else
...
#ifdef RADIO_MEASUREMENT_SPARE_SCAN_LIST
		if (rm_pfree_bss_buf) {
			rtw_vmfree(rm_pfree_bss_buf, rm_max_bss_cnt * sizeof(struct wlan_network));
#ifdef RTW_MI_SHARE_BSS_LIST
			d->rm_free_bss_buf = NULL;
#endif
		}
#endif
	}
...
struct	wlan_network *_rtw_alloc_network(struct	mlme_priv *pmlmepriv, enum SCAN_LIST_TYPE scan_list_type) /* (_queue *free_queue) */
{
	struct	wlan_network	*pnetwork;
#ifdef RTW_MI_SHARE_BSS_LIST
	_adapter *adapter = mlme_to_adapter(pmlmepriv);
...
#ifdef RTW_MI_SHARE_BSS_LIST
#ifdef RADIO_MEASUREMENT_SPARE_SCAN_LIST
	if (scan_list_type == SCAN_LIST_RM)
		free_queue = &adapter->dvobj->rm_free_bss_pool;
	else
#endif
		free_queue = &adapter->dvobj->free_bss_pool;
#else
	free_queue = &(pmlmepriv->free_bss_pool);
#endif
...
#ifdef RTW_MI_SHARE_BSS_LIST
	if (scan_list_type == SCAN_LIST_REGULAR)
...
struct	wlan_network *rtw_alloc_network(struct	mlme_priv *pmlmepriv, enum SCAN_LIST_TYPE scan_list_type) /* (_queue	*free_queue) */
{
	struct	wlan_network	*pnetwork;
	pnetwork = _rtw_alloc_network(pmlmepriv, scan_list_type);
	return pnetwork;
}
...
u32 rtw_get_wmode_by_ie(u8 *pos, u32 left)
{
	struct rtw_ieee802_11_elems elems = {0};
	u32 wmode = 0;
	u8 channel = 0;
	...
	if (elems.he_capabilities_len)
		wmode |= WLAN_MD_11AX;

	if (elems.ht_capabilities_len)
		wmode |= WLAN_MD_11N;

	if (elems.ds_params_len)
		channel = *elems.ds_params;

	if (channel > 14) {
		if (elems.vht_capabilities_len)
			wmode |= WLAN_MD_11AC;
		wmode |= WLAN_MD_11A;
	} else {
		if (elems.supp_rates_len)
			wmode |= WLAN_MD_11G;
		wmode |= WLAN_MD_11B;
	}
...
*/
bool rtw_update_scanned_network(_adapter *adapter, WLAN_BSSID_EX *target, enum SCAN_LIST_TYPE scan_list_type)
{
	_list	*plist, *phead;
	u32	bssid_ex_sz;
	struct mlme_priv	*pmlmepriv = &(adapter->mlmepriv);
	...
	_queue *pfree_bss_pool;
	_queue *queue;

#ifdef RTW_MI_SHARE_BSS_LIST
#ifdef RADIO_MEASUREMENT_SPARE_SCAN_LIST
	if (scan_list_type == SCAN_LIST_RM) {
		pfree_bss_pool = &adapter->dvobj->rm_free_bss_pool;
		queue = &adapter->dvobj->rm_scanned_queue;
	}
	else
#endif
	{
		pfree_bss_pool = &adapter->dvobj->free_bss_pool;
		queue = &adapter->dvobj->scanned_queue;
	}
#else
	pfree_bss_pool = &(pmlmepriv->free_bss_pool);
	...
	if (!target_find) {
		if (_rtw_queue_empty(pfree_bss_pool) == _TRUE) {
			...
			pnetwork->network_type = rtw_get_wmode_by_ie(&target->IEs[_BEACON_IE_OFFSET_], target->IELength - _BEACON_IE_OFFSET_);
			pnetwork->aid = 0;
			pnetwork->join_res = 0;
			...
			pnetwork = rtw_alloc_network(pmlmepriv, scan_list_type); /* will update scan_time */
			if (pnetwork == NULL)
				goto unlock_scan_queue;
...
void rtw_add_network(_adapter *adapter, WLAN_BSSID_EX *pnetwork)
{
	...
	update_ie = rtw_update_scanned_network(adapter, pnetwork, SCAN_LIST_REGULAR);

	if (update_ie)
		update_current_network(adapter, pnetwork);

#ifdef RADIO_MEASUREMENT_SPARE_SCAN_LIST
	if (pnetwork->Configuration.DSConfig == adapter->mlmeextpriv.cur_channel)
		rtw_update_scanned_network(adapter, pnetwork, SCAN_LIST_RM);
#endif
...
void rtw_surveydone_event_callback(_adapter	*adapter, u8 *pbuf)
{
	...
	if(_rtw_queue_empty(queue) == _FALSE){
		_rtw_spinlock_bh(&queue->lock);
		phead = get_list_head(queue);
		RTW_INFO("%s: Sort the site survey list according to signal strength\n", __FUNCTION__);
	...
	if (parm->acs) {
		...
		if (adapter->multiap_acs_trigger) {
			acs_change_bss_chbw(adapter, _TRUE, 5);
			adapter->multiap_acs_trigger = 0;
		}
#endif
	}
#endif
...
u8 _rtw_sitesurvey_condition_check(const char *caller, _adapter *adapter, bool check_sc_interval)
{
	...
	if (!rtw_is_scan_deny_bypass(adapter) && rtw_is_scan_deny(adapter)) {
	 ...
	 */
	if ((MLME_IS_STA(adapter) ||
			(pmlmepriv->lastscantime &&
			(rtw_get_passing_time_ms(pmlmepriv->lastscantime) > registry_par->scan_interval_thr))) &&
	...
	RTW_PRINT("[%s]Caller: %s on %s\n", __func__, caller, NDEV_ARG(adapter->pnetdev));
_exit:
	return ss_condition;
}

void free_scanqueue(struct	mlme_priv *pmlmepriv, enum SCAN_LIST_TYPE scan_list_type)
{
#ifdef RTW_MI_SHARE_BSS_LIST
	_adapter *adapter = mlme_to_adapter(pmlmepriv);
#endif
	...
	if (scan_list_type == SCAN_LIST_RM) {
		free_queue = &adapter->dvobj->rm_free_bss_pool;
		scan_queue = &adapter->dvobj->rm_scanned_queue;
	}
	else
#endif
	{
		free_queue = &adapter->dvobj->free_bss_pool;
		scan_queue = &adapter->dvobj->scanned_queue;
	}
#else
	free_queue = &(pmlmepriv->free_bss_pool);
...
void rtw_free_assoc_resources(_adapter *adapter, u8 lock_scanned_queue)
{
	...
	if (MLME_IS_STA(adapter) && MLME_IS_ASOC(adapter)) {
		...
		if (psta && RTL_WFO_IS_OFLD(adapter->dvobj)) {
			/* Redirect pkts back to driver */
			if (adapter->dvobj->wfo_en & RTL_WFO_TX_OFLD) {
...
u32 rtw_scan_abort(_adapter *adapter, u32 timeout_ms)
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_UNDER_SURVEY) 
		...
		/* set timeout_ms to scan_timeout_ms, to avoid permanently waiting*/
		if (timeout_ms == 0)
			timeout_ms = pmlmeext->sitesurvey_res.scan_timeout_ms;
		RTW_INFO(FUNC_ADPT_FMT "- %s....scan_abort:%d, timeout_ms:%d\n",
			FUNC_ADPT_ARG(adapter), __func__, pmlmeext->scan_abort, timeout_ms);
...
static struct sta_info *rtw_joinbss_update_stainfo(_adapter *padapter, struct wlan_network *pnetwork)
{
	...
	if (psta) { /* update ptarget_sta */
...
#endif
		{
			...
			if (ie && ie_len > 0
				&& rtw_parse_wpa2_ie(ie, ie_len + 2, NULL, NULL, NULL, NULL, &mfp_opt, NULL) == _SUCCESS
			) {
...
void rtw_joinbss_event_prehandle(_adapter *adapter, u8 *pbuf, u16 status)
{
	...
	if (pnetwork->join_res > 0) {
		...
		if (check_fwstate(pmlmepriv, WIFI_UNDER_LINKING)) {
			...
			/* s6. Issue null data with OM ul_mu_disable field */
			if (MLME_IS_STA(adapter)) {
				if (ptarget_sta->hepriv.he_option == _TRUE) {
					issue_om_with_nulldata(adapter, ptarget_sta, 7, 0, 0);
				}
			}
		} else {
			_rtw_spinunlock_bh(&(queue->lock));
...
void rtw_stassoc_event_callback(_adapter *adapter, u8 *pbuf)
{
	...
	if (MLME_IS_AP(adapter) || MLME_IS_MESH(adapter)) {
		...
		if (VW_MODE_LATENCY == adapter->vw_enable) {
			...
			rtw_amsdu_pe_offload_mac_id_set(psta);
#endif
#ifdef CONFIG_RTL_WIFI_OFFLOAD
			if (!adapter->dvobj->wfo_pe_stopped &&
					RTL_WFO_IS_OFLD(adapter->dvobj)) {
				rtl_wfo_pe_id_decision(adapter, psta);
				/* Always add stainfo to PEs */
				rtl_wfo_ipc_cfg_sta_info(adapter, psta,
						(PE_ROLE_PPE | PE_ROLE_DOE | PE_ROLE_AOE),
						RTL_WFO_OP_ADD);
			}
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
#ifdef CONFIG_RTW_A4_STA
			/* Delete A4 entry if present */
			for (i = 0; i < dvobj->iface_nums; i++) {
				if (!is_primary_adapter(dvobj->padapters[i]) && (dvobj->padapters[i]->netif_up))
					a4_del_source_db_entry(dvobj->padapters[i], psta->phl_sta->mac_addr, _TRUE);
			}
#ifdef CONFIG_RTL_WIFI_OFFLOAD
			if (!RTL_WFO_IS_OFLD(adapter->dvobj)) {
				for (i = 0; i < BAND_MAX; i++) {
					struct dvobj_priv *dvobj2 = rtw_get_dvobj(i);
					if (dvobj2 && RTL_WFO_IS_OFLD(dvobj2) &&
						(GET_HAL_SPEC(dvobj)->band_cap !=
						 GET_HAL_SPEC(dvobj2)->band_cap)) {
						/* 2.4G STA roaming from agent to controller (5G backhaul) */
						for (i = 0; i < dvobj2->iface_nums; i++) {
							if (!is_primary_adapter(dvobj2->padapters[i]) &&
								(dvobj2->padapters[i]->netif_up)) {
								a4_del_source_db_entry(
									dvobj2->padapters[i],
			...
			if (adapter->registrypriv.wifi_mib.record_env_stainfo) {
				struct env_sta_info *entry;

				entry = lookup_env_stainfo_table(adapter, pstassoc->macaddr);
				if (entry)
					entry->conneted = _TRUE;
			}
#endif
#ifdef CONFIG_WLAN_EVENT_INDICATE_GENL
			if (psta->phl_sta->wmode == WLAN_MD_11BG ||
				psta->phl_sta->wmode == WLAN_MD_11B)
				genl_wlan_IndicateEvent(adapter, EVENT_WIFI_STA_BG_MODE, psta->phl_sta->mac_addr);
#endif
		}
		goto exit;
	}
...
void rtw_stadel_event_callback(_adapter *adapter, u8 *pbuf)
{
	...
	if (MLME_IS_AP(adapter)) {
...
#ifdef CONFIG_ETHER_PKT_AGG
		rtw_amsdu_pe_offload_mac_id_del(psta);
#endif
		...
		if (adapter->registrypriv.wifi_mib.record_env_stainfo) {
			...
			entry = lookup_env_stainfo_table(adapter, pstadel->macaddr);
			if (entry)
				entry->conneted = _FALSE;
		}
#endif
...
void rtw_join_timeout_handler(void *ctx)
{
	...
	{
		rtw_indicate_disconnect(adapter, pmlmepriv->join_status, _FALSE);
		free_scanqueue(pmlmepriv, SCAN_LIST_REGULAR);/* ??? */
...
static void collect_traffic_statistics(_adapter *padapter)
{
	...
	u16 tdma_off_thre = padapter->registrypriv.wifi_mib.tdma_off_thre;
	u16 tdma_on_thre = 0;
	u16 cur_total_tp = 0;

	/*_rtw_memset(&pdvobjpriv->traffic_stat, 0, sizeof(struct rtw_traffic_statistics));*/

	if (!netif_running(padapter->pnetdev) || !padapter->phl_role)
		return;
	...
	if (pdvobjpriv->traffic_stat.cur_tx_bytes > (1 << 10) &&
			pdvobjpriv->traffic_stat.cur_tx_bytes > (pdvobjpriv->traffic_stat.cur_rx_bytes << 3)) {
		pdvobjpriv->traffic_stat.traffic_mode = TRAFFIC_MODE_TX;
	} else if (pdvobjpriv->traffic_stat.cur_rx_bytes > (1 << 10) &&
			pdvobjpriv->traffic_stat.cur_rx_bytes > (pdvobjpriv->traffic_stat.cur_tx_bytes << 3)) {
		pdvobjpriv->traffic_stat.traffic_mode = TRAFFIC_MODE_RX;
	} else if (pdvobjpriv->traffic_stat.cur_tx_bytes > (10 << 10) && pdvobjpriv->traffic_stat.cur_rx_bytes > (10 << 10)) {
		pdvobjpriv->traffic_stat.traffic_mode = TRAFFIC_MODE_BIDIR;
	} else {
		;  // keep previous state
	}
	...
	// Turn off TDMA DIG when throughput > tdma_off_thre
	cur_total_tp = pdvobjpriv->traffic_stat.cur_tx_tp + pdvobjpriv->traffic_stat.cur_rx_tp;
	if (WIFI_ROLE_IS_ON_24G(padapter))
		tdma_on_thre = (tdma_off_thre - 25 > 0) ? (tdma_off_thre - 25) : 0;
	else
		tdma_on_thre = (tdma_off_thre - 50 > 0) ? (tdma_off_thre - 50) : 0;
	if(cur_total_tp > tdma_off_thre) {
		rtw_phl_set_dig_mode(phl, wrole, 0);
	}
	else if(cur_total_tp <= tdma_on_thre) {
		rtw_phl_set_dig_mode(phl, wrole, 1);
	}
...
static void collect_hw_statistics(_adapter *padapter)
{
	...
	if (GET_PRIMARY_ADAPTER(padapter)->registrypriv.wifi_mib.antdiv)
		pdvobjpriv->hw_stat.antdiv_targetant = rtw_phl_antdiv_get_targetant(pdvobjpriv->phl);
	else
		pdvobjpriv->hw_stat.antdiv_targetant = 0;
#endif
}
...
#if defined(CONFIG_AP_MODE) && defined (CONFIG_NATIVEAP_MLME)
#define RTW_DIFF(a,b) (((a) > (b)) ? (a-b) : (b-a))
void rtw_beacon_vsie_timeout_expire(_adapter *padapter)
{
	...
	for (vendor_ie_num = 0 ; vendor_ie_num < WLAN_MAX_VENDOR_IE_NUM ; vendor_ie_num++)
	{
		if (pmlmepriv->vendor_service_name_len[vendor_ie_num] && pmlmepriv->vendor_ielen[vendor_ie_num] > 0 && (pmlmepriv->vendor_ie_mask[vendor_ie_num] & WIFI_BEACON_VENDOR_IE_BIT))
		{
			...
			{
				passing_time = rtw_get_passing_time_ms(pmlmepriv->vendor_ie_settime[vendor_ie_num]);
				if((RTW_DIFF(passing_time, pmlmepriv->vendor_ie_duration[vendor_ie_num]*1000) <= 500) ||
					(passing_time >= pmlmepriv->vendor_ie_duration[vendor_ie_num]*1000))
				{
					RTW_INFO("[%s][%d]passing_time = %d(expected = %d), count = %d\n", __func__,
								vendor_ie_num,
								passing_time,
								pmlmepriv->vendor_ie_duration[vendor_ie_num]*1000,
...
inline bool rtw_is_scan_deny_bypass(_adapter *adapter)
{
	struct mlme_priv *mlmepriv = &adapter->mlmepriv;
	return (mlmepriv->scan_deny_bypass != 0) ? _TRUE : _FALSE;
}
...
int rtw_del_cached_pmkid(_adapter *adapter, u8 *bssid)
{
	struct security_priv *psecuritypriv = &adapter->securitypriv;
	int i = 0, bMatched = _FALSE;

	for (i = 0 ; i < NUM_PMKID_CACHE; i++) {
		if ((psecuritypriv->PMKIDList[i].bUsed) &&
		    (_rtw_memcmp(psecuritypriv->PMKIDList[i].Bssid, bssid, ETH_ALEN) == _TRUE)) {
		    	_rtw_memset(psecuritypriv->PMKIDList[i].Bssid, 0x00, ETH_ALEN);
			_rtw_memset(psecuritypriv->PMKIDList[i].PMKID, 0x00, WLAN_PMKID_LEN);
			psecuritypriv->PMKIDList[i].bUsed = _FALSE;
			bMatched = _TRUE;
			RTW_INFO("[%s]["MAC_FMT"]clear id:%hhu\n", __func__, MAC_ARG(bssid), i);
			break;
		}
	}

	if (_FALSE == bMatched) {
		RTW_INFO("[%s]["MAC_FMT"]do not have matched BSSID\n", __func__, MAC_ARG(bssid));
		return -1;
	}

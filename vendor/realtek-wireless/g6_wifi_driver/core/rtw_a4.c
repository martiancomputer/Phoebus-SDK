...
static void free_a4_entry(_adapter *adapter,  struct rtw_a4_db_entry *entry)
{
	...
	if (adapter->dvobj->wfo_en & RTL_WFO_TX_OFLD) {
		rtl_wfo_del_a4_staid_to_fc(entry);
		rtl_wfo_clear_a4_fc_staid(adapter, entry);
	}
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
}

void a4_del_source_db_entry(_adapter *adapter, u8 *mac, u8 ntfy_pe)
{
	...
	while (db != NULL) {
		if (!memcmp(db->mac, mac, ETH_ALEN)) {
			...
			if (RTL_WFO_IS_OFLD(adapter->dvobj)) {
				if (ntfy_pe)
					rtl_wfo_del_a4_db(adapter, mac, db->psta->rps_pe_id, db->fc_staid);
				else
					rtl_wfo_ipc_update_fc_sta(adapter, 0xff, db->fc_staid,
						PE_ROLE_PPE, db->psta->rps_pe_id);
				rtl_wfo_ipc_update_fc_sta(adapter, 0xff, db->fc_staid,
					PE_ROLE_AOE, 0);
			}
#endif
...
void a4_del_source(_adapter *adapter, u8 *mac)
{
	...
	a4_del_source_db_entry(adapter, mac, _FALSE);
}
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
struct sta_info *rtw_get_stainfo_from_adapters(_adapter *padapter, u8 *mac)
{
	int i;
	_adapter *padptr;
	struct sta_info *psta = NULL;
	...
	for (i = 0; i < CONFIG_IFACE_NUMBER; i++) {
		padptr = GET_ADAPTER(padapter, i);
		if (padptr) {
			if (!MLME_IS_AP(padptr) && !MLME_IS_MESH(padptr))
				continue;
			psta = rtw_get_stainfo(&padptr->stapriv, mac);
			if (psta) {
				RTW_INFO("%s: %s found sta:%pM\n",
					__func__, padptr->pnetdev->name, mac);
				break;
			}
		}
	}
...
struct sta_info *rtw_get_stainfo_from_dvobjs(u8 *mac)
{
	int i;
	struct sta_info *psta = NULL;
	...
	for (i = 0; i < BAND_MAX; i++) {
		dvobj = rtw_get_dvobj(i);
		if (dvobj) {
			psta = rtw_get_stainfo_from_adapters(dvobj->padapters[IFACE_ID0], mac);
			if (psta)
				break;
		}
	}
...
#endif
				RTW_PRINT("%s A4 STA Update emac:%pM, wmac:%pM\n",
					adapter->pnetdev->name, db->mac, psta->phl_sta->mac_addr);
			}
			db->psta = psta;
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	if (RTL_WFO_IS_OFLD(adapter->dvobj)) {
		struct sta_info *psta2 = rtw_get_stainfo_from_dvobjs(mac);

		if (psta2) {
			if (!psta2->is_btm_resp_recved) {
				RTW_INFO("Ignore mac:%pM, wmac:%pM roaming %d\n",
					mac, psta->phl_sta->mac_addr, roaming);
				return;
			}
			if (RTL_WFO_IS_OFLD(psta2->padapter->dvobj)) {
				/* reuse this fc_staid if possible */
				fc_staid = psta2->fc_staid;
				psta2->fc_staid = 0xFF;
			}
		}

		if (roaming) {
			printk("%s A4 STA Update, Roaming, emac:%pM, wmac:%pM\n",
				adapter->pnetdev->name, db->mac, psta->phl_sta->mac_addr);
			a4_del_source_db_entry(adapter, db->mac, _TRUE);
			return;
		}
	}
#endif
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	if (RTL_WFO_IS_OFLD(adapter->dvobj))
		rtl_wfo_cfg_a4_sta_info(adapter, db, fc_staid);
#endif /* CONFIG_RTL_WIFI_OFFLOAD */

#if 1
    printk("%s A4 STA Add emac:%pM, wmac:%pM\n",
	adapter->pnetdev->name, db->mac, db->psta->phl_sta->mac_addr);
#endif
}
...
bool core_a4_check_tx(_adapter *adapter, struct sk_buff **pskb)
{
	...
	struct pkt_file pktfile = {0};
	struct ethhdr etherhdr = {{0}};
...
void core_a4_init(_adapter *padapter)
{
	...
	//if (padapter->a4_enable) {
		...
		_rtw_spinlock_init(&padapter->a4_sta_list_lock);
		_rtw_spinlock_init(&padapter->a4_db_lock);

		if (MLME_IS_STA(padapter))
			core_a4_gptr_tbl_init(padapter);
	//}

	RTW_PRINT("[TXSC_INIT][%s] a4_enable:%d, a4_sta_list/a4_gptr_tbl init.\n",

...
sint _rtw_init_recv_com_priv(struct recv_com_priv *precvpriv)
{
	sint i;
...
void _rtw_free_recv_com_priv(struct recv_com_priv *precvpriv)
{
	sint i;
	union recv_frame *precvframe;
	precvframe = (union recv_frame*) precvpriv->precv_frame_buf;
...
sint _rtw_init_recv_priv(struct recv_priv *precvpriv, _adapter *padapter)
{
	...
	if (is_primary_adapter(padapter)) {
		precvframe = (union recv_frame *) padapter->dvobj->recv_compriv.precv_frame_buf;
		for (i = 0; i < NR_RECVFRAME; i++) {
...
union recv_frame *_rtw_alloc_recvframe(_queue *pfree_recv_queue)
{
	...
	if (_rtw_queue_empty(pfree_recv_queue) == _TRUE)
		...
		if (padapter != NULL) {
			if (pfree_recv_queue == &padapter->dvobj->recv_compriv.free_recv_queue)
				padapter->dvobj->recv_compriv.free_recvframe_cnt--;
		}
	}
...
sint _rtw_enqueue_recvframe(union recv_frame *precvframe, _queue *queue)
{
	...
	if (padapter != NULL) {
		if (queue == &padapter->dvobj->recv_compriv.free_recv_queue)
			padapter->dvobj->recv_compriv.free_recvframe_cnt++;
	}
...
u32 rtw_free_uc_swdec_pending_queue(_adapter *adapter)
{
	...
	while ((pending_frame = rtw_alloc_recvframe(&adapter->recvpriv.uc_swdec_pending_queue))) {
		rtw_free_recvframe(pending_frame, &adapter->dvobj->recv_compriv.free_recv_queue);
		cnt++;
	}
...
union recv_frame *decryptor(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (prxattrib->encrypt && !prxattrib->bdecrypted) {
		...
		if ((prxstat->icv == 1) && (prxattrib->encrypt != _AES_)) {
			...
			rtw_free_recvframe(precv_frame, &padapter->dvobj->recv_compriv.free_recv_queue);
...
sint check_pn_overflow(u64 pkt_pn, u64 curr_pn)
{
	if ((curr_pn > 0xffffffff0000) &&
		 (pkt_pn <= 0xffff)) {
		return 1;
	}
	return 0;
...
sint recv_bcast_pn_decache(union recv_frame *precv_frame)
{
	...
	if ((pattrib->encrypt == _AES_) && (MLME_IS_STA(padapter))) {
		...
		if (!VALID_PN_CHK(pkt_pn, curr_pn) &&
			!check_pn_overflow(pkt_pn, curr_pn))
			return _FAIL;
...
boolean is_om_comtrol(u32 ht_control, u8 *rx_ss, u8 *tx_ss, u8 *ch_bw, u8 *ul_mu_disable)
{
	u8 actrl_type = (ht_control & ACONTROL_ID_MASK) >> 2;
	boolean ret = _FALSE;
...
void process_om_control(_adapter *padapter, struct sta_info *psta, u8 rx_ss, u8 tx_ss, u8 ch_bw, u8 ul_mu_disable)
{
	void *phl = padapter->dvobj->phl;
	struct rtw_wifi_role_t *role = padapter->phl_role;
	u8 ori_phl_sta_nss_rx = psta->phl_sta->asoc_cap.nss_tx;
	u8 ori_phl_sta_bw = psta->phl_sta->chandef.bw;
	u8 ori_phl_sta_nss_tx = psta->phl_sta->asoc_cap.nss_rx;
	...
	if ((psta->phl_sta->chandef.bw != ori_phl_sta_bw) ||
		...
		rtw_phl_cmd_change_stainfo(phl, psta->phl_sta, STA_CHG_RAMASK, NULL, 0, PHL_CMD_DIRECTLY, 0);
	...
	if (((psta->phl_sta->chandef.bw != ori_phl_sta_bw) ||
		...
		if (padapter->registrypriv.wifi_mib.ofdma_enable & ul_fix_mode){
			...
			rtw_phl_write8(phl, 0xc600, 0x43);

			//rtw_clean_ofdma_grp(padapter);
			rtw_remove_grp(padapter);

			rtw_phl_write8(phl, 0xc600, 0x40);
			...
			rtw_phl_write32(phl, 0x9e48, 0x900000FF);
		}
		#endif
...
enum rtw_phl_status rtw_core_process_om_control(void *drv_priv, u16 macid, u32 htc)
{
	...
	if (is_om_comtrol(htc, &rx_ss, &tx_ss, &ch_bw, &ul_mu_disable))
		process_om_control(padapter, psta, rx_ss, tx_ss, ch_bw, ul_mu_disable);
...
void process_a_control_subfield(_adapter *padapter,  union recv_frame *precv_frame)
{
	...
	if ((ht_control & ACONTROL_MASK) == 0x03) {
		if (is_om_comtrol(ht_control, &rx_ss, &tx_ss, &ch_bw, &ul_mu_disable)) {
			if (pattrib->to_fr_ds == 3)
...
void process_wmmps_data(_adapter *padapter, union recv_frame *precv_frame, struct sta_info *psta)
{
	...
	if (psta->state & WIFI_SLEEP_STATE) {
		...
		/* notify fw to release a packet*/
		rtw_phl_host_getpkt(padapter->dvobj->phl, psta->phl_sta->macid, 0);
		...
		if (psta->uapsd_bitmap & BIT(q_idx)) {
			...
			if (pstaxmitpriv->tx_pending_bitmap & psta->uapsd_bitmap)
				pstaxmitpriv->ps_trigger_type |= (u8)((pstaxmitpriv->tx_pending_bitmap & psta->uapsd_bitmap) << 1);
			else
				pstaxmitpriv->ps_trigger_type |= BIT(q_idx+1);
...
void rtw_count_rx_wmm_stats(_adapter *padapter, int sz, u8 priority, u32 err_pkts, u32 drop_pkts)
{
	u8 index;
	const static u8 wmm_map[8] = {XMIT_BE_QUEUE, XMIT_BK_QUEUE, XMIT_BK_QUEUE, XMIT_BE_QUEUE,
					XMIT_VI_QUEUE, XMIT_VI_QUEUE, XMIT_VO_QUEUE, XMIT_VO_QUEUE};
	if (priority < 8) {
		index = wmm_map[priority];
		if (err_pkts) {
			padapter->wmm_stats[index].rx_errors += err_pkts;
		} else if (drop_pkts) {
			padapter->wmm_stats[index].rx_drops += drop_pkts;
		} else {
			padapter->wmm_stats[index].rx_bytes += sz;
			padapter->wmm_stats[index].rx_pkts++;
		}
	}
	
}

#ifdef CMCC_SHOW_STA_UNAMBIGUOUS_RX_RATE
u32 ignore_ambiguous_rx_rate(struct sta_info *psta, u16 rate)
{
	if ((psta->state & WIFI_ASOC_STATE)
		&& (psta->flags & (WLAN_STA_HT | WLAN_STA_VHT | WLAN_STA_HE))
		&& (rate < RTW_DATA_RATE_MCS0)
		&& (rtw_phl_get_sta_rssi(psta->phl_sta) > 15))
		return 1;
	return 0;
}
#endif
...
void count_rx_stats(_adapter *padapter, union recv_frame *prframe, struct sta_info *sta)
{
	...
	struct recv_priv *pri_precvpriv = &pri_adapter->recvpriv;

	sz = get_recvframe_len(prframe);
	precvpriv->rx_bytes += sz;
	pri_precvpriv->rx_bytes_per_band += sz;//RT_TRX_TRAFFIC
	precvpriv->rx_pkts++;
	rtw_count_rx_wmm_stats(padapter, sz, pattrib->priority, 0, 0);
	...
	if (psta) {
		...
		pstats->rx_data_pkts++;
		pstats->rx_bytes += sz;
		pstats->rx_bytes_per_sta += sz;//RT_TRX_TRAFFIC
...
#endif /* CONFIG_LPS */
#ifdef CMCC_SHOW_STA_UNAMBIGUOUS_RX_RATE
		if (!ignore_ambiguous_rx_rate(psta, pattrib->data_rate))
#endif
		{
	...
	if (check_fwstate(pmlmepriv, WIFI_AP_STATE) == _TRUE) {
		...
		*psta = rtw_get_stainfo(pstapriv, pattrib->ta);
		if (*psta == NULL) {
			...
			RTW_ERR("no STA A/H/S %pM / %pM / %pM\n",
				pattrib->ta, ptr + 10,
				(rx_req->tx_sta ? rx_req->tx_sta->mac_addr : dummy_mac));
...
bip_verify:
	...
	if (pattrib->pkt_len <= (pattrib->hdrlen + pattrib->iv_len + pattrib->icv_len)) {
		RTW_INFO(FUNC_ADPT_FMT" invalid mgmt frame length (%d)\n", FUNC_ADPT_ARG(adapter), pattrib->pkt_len);
		goto fail;
	}
...
sint validate_recv_data_frame(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if((pattrib->src[0] ==0xaa) && (pattrib->src[1] ==0xbb) && (pattrib->src[2] ==0xcc)
		...
		printk("\n");
#endif
		printk("[%s %d] receive WIFI_EZMESH_ARP_LOOP in driver on if:%s, send event!!!\n",__FUNCTION__,__LINE__,adapter->pnetdev->name);
...
pre_validate_status_chk:
	...
	if (psecuritypriv->dot11AuthAlgrthm == dot11AuthAlgrthm_WAPI &&
		(!rtw_validate_wapi_data_packet(adapter, precv_frame))) {
		ret = _FAIL;
		RTW_INFO("\n%s:%d Drop WAPI data packet!!!\n", __FUNCTION__, __LINE__);
		goto exit;
	}
#endif
	...
	case WIFI_DATA_TYPE: /* data */
		...
		if (retval == _FAIL) {
			...
			if (!pattrib->encrypt || pattrib->bdecrypted) {
				...
				if ((bDumpRxPkt == 4) && (eth_type == ETH_P_EAPOL))
					dump_rx_packet(ptr);
			}
			#endif
			...
			if (!pattrib->amsdu){
				if (EQ_MAC_ADDR(adapter->br_mac, pattrib->src)){
					if (adapter->registrypriv.wifi_mib.rx_loopdetect_drop) {
						retval = _FAIL;
						precvpriv->dbg_rx_drop_count++;
						if (psta)
							psta->sta_stats.rx_drops++;
...
#endif
#if defined(CONFIG_WLAN_EVENT_INDICATE_GENL)
					rtk_wlan_genl_indicate(adapter->pnetdev->name, WIFI_SA_EQUAL_BR0_MAC, adapter->mac_addr, 0);
#endif
#endif
				}
			}
...
union recv_frame *recvframe_defrag(_adapter *adapter, _queue *defrag_q)
{
	...
	curfragnum = 0;
	pfree_recv_queue = &adapter->dvobj->recv_compriv.free_recv_queue;
...
s32 recvframe_chk_defrag(_adapter *padapter, union recv_frame **pprecv_frame)
{
	...
	pfree_recv_queue = &padapter->dvobj->recv_compriv.free_recv_queue;
...
static int rtw_recv_indicatepkt_check(union recv_frame *rframe, u8 *ehdr_pos, u32 pkt_len)
{
	...
	if (ntohs(ehdr->h_proto) == ETH_P_EAPOL) {
												...
												union recv_frame *prframe)
{
	...
	u8 *ptr = NULL;
	unsigned char a1_addr[ETH_ALEN] = {0x00 ,0x00 ,0x00 , 0x00, 0x00 ,0x00};
...
u8 amsdu_check_subpkt_hdr_valid(_adapter *padapter, u8 *a1_addr, struct sta_info *psta)
{
	struct sta_priv *pstapriv = &padapter->stapriv;
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	struct wlan_network *cur_network = &(pmlmepriv->cur_network);
	...
	if (hit) {
		...
		if (relay_sta && !EQ_MAC_ADDR(relay_sta->phl_sta->mac_addr, cur_network->network.MacAddress)) {
			hit = 0;
		}
	}
...
/* CONFIG_RTW_A4_STA, todo: amsdu */
	...
	if (!prframe->u.hdr.rxsc_entry) {
		...
		if (rxsc_entry) {
			...
			/* cache WLAN header */
			_rtw_memcpy((void *)&rxsc_entry->rxsc_wlanhdr, get_recvframe_data(prframe), rattrib->hdrlen);

			/* Cache Rx Attribute */
			_rtw_memcpy((void *)&rxsc_entry->rxsc_attrib, rattrib, sizeof(rxsc_entry->rxsc_attrib));
			if (rxsc_entry->is_htc)
				rxsc_entry->rxsc_attrib.hdrlen -= 4;
		}
	} else
...
#ifdef CORE_RXSC_AMSDU
#ifdef CONFIG_RTW_BYPASS_DEAMSDU
	if (padapter->rx_deamsdu_by_nic 
			&& (!MLME_IS_AP(padapter) || rattrib->to_fr_ds != 1)
			&& rxsc_entry && rxsc_entry->forward_to == RXSC_FWD_KERNEL
			&& amsdu_check_subpkt_hdr_valid(padapter, prframe->u.hdr.rx_data, psta)
			&& !amsdu_check_subpkt_a1_mixed(padapter, prframe)) {
			...
			if (sub_pkt) {
...
#endif
				{
					...
					if(rxsc_entry && rxsc_entry->forward_to == RXSC_FWD_KERNEL) {
						...
						psta->subpkt_idx = (psta->subpkt_idx + 1) % NUM_RXSC_ENTRY;
...
#ifdef CONFIG_RTW_BYPASS_DEAMSDU
		if (bypass_deamsdu)
			padapter->core_logs.rxCnt_amsdu_bypass++;
		else
#endif
...
int recv_func(_adapter *padapter, union recv_frame *rframe)
{
	...
	if (ret == _SUCCESS) {
		...
		if (MLME_IS_STA(padapter) &&
			...
			if (padapter->dvobj->recv_compriv.free_recvframe_cnt < NR_RECVFRAME / 4) {
...
bypass_concurrent_hdl:
	...
	if (primary_padapter->registrypriv.mp_mode != 1) {
		...
		if (ra_is_bmc == _TRUE && GetFrameType(pbuf) == WIFI_DATA_TYPE
			...
			rtw_free_recvframe(precvframe, &precvframe->u.hdr.adapter->dvobj->recv_compriv.free_recv_queue);
			goto exit;
		}
	}
...
void core_update_recvframe_pkt(union recv_frame *prframe, void *req)
{
	...
	if (   (!rx_req->mdata.amsdu_cut && (frame != wlan_hdr))
	    || (rx_req->mdata.amsdu_cut && (frame == wlan_hdr)))
...
void core_update_recvframe_mdata(struct dvobj_priv *dvobj, union recv_frame *prframe, struct rtw_recv_pkt *rx_req)
{
	...
	prxattrib->phy_info.recv_signal_power = prxattrib->phy_info.signal_strength;
#if 0 //todo
...
#if defined(SBWC) && !defined(FC_SBWC)
                              ...
                              union recv_frame **prframes)
{
	...
	_queue			*pfree_recv_queue = &dvobj->recv_compriv.free_recv_queue;
                                     ...
                                     union recv_frame **prframes)
{
	...
	*/
	if (phlrx->mdata.icverr &&
		...
		rtw_wapi_check_frame_qos(whdr, phlrx->mdata.pktlen))
	...
	if (phlrx->mdata.icverr) {
		...
		if (get_tofr_ds(whdr) == 3)
			priority = GetPriority((whdr + WLAN_HDR_A4_LEN));
		else
			priority = GetPriority((whdr + WLAN_HDR_A3_LEN));
		rtw_count_rx_wmm_stats(primary_padapter, 0, priority, 1, 0);
		if (pstats)
			pstats->rx_errors++;
		goto exit;
	}
...
{
	_queue *pfree_recv_queue = &adapter->dvobj->recv_compriv.free_recv_queue;
	union recv_frame  *precvframe;
...
void rtw_delay_measure_done_rx(_adapter *padapter, union recv_frame *rframe, struct sta_info *psta)
{
	u32 freerun_diff = 0;
	u32 freerun_start;
	u32 cur_freerun_l, cur_freerun_h;
	int i;
	...
	rtw_phl_get_cur_freeruncnt(padapter->dvobj->phl, padapter->phl_role,
		&cur_freerun_h, &cur_freerun_l);

	rx_req = rframe->u.hdr.rx_req;
	freerun_start = rx_req->mdata.freerun_cnt;

	if (freerun_start > cur_freerun_l)
		freerun_diff = 0xffffffff - freerun_start + cur_freerun_l;
	else
		freerun_diff = cur_freerun_l - freerun_start;

	if (freerun_diff%1000 < 500)
		freerun_diff = freerun_diff/1000;
	else
		freerun_diff = freerun_diff/1000 + 1;

	if (freerun_diff <= DELAY_RECORD_SLOT[0]) {
		psta->rx_delay_measure.delay_record[0]++;
	} else {
		for (i = DELAY_RECORD_NUM - 1; i >= 0; i--) {
			if (freerun_diff > DELAY_RECORD_SLOT[i]) {
				psta->rx_delay_measure.delay_record[i + 1]++;
				break;
			}
		}
	}

	if (freerun_diff > psta->rx_delay_measure.delay_max)
		psta->rx_delay_measure.delay_max = freerun_diff;
	if (psta->tx_delay_measure.delay_min == 0 || freerun_diff < psta->rx_delay_measure.delay_min)
		psta->rx_delay_measure.delay_min = freerun_diff;

	psta->rx_delay_measure.delay_total += freerun_diff;
	psta->rx_delay_measure.delay_num ++;

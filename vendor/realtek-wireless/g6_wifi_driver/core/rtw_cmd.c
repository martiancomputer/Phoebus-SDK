...
#if 0 /*#ifdef CONFIG_CORE_CMD_THREAD*/
u32 _rtw_enqueue_cmd(_queue *queue, struct cmd_obj *obj, bool to_head)
{
...
#else
static u32 _rtw_enqueue_cmd(struct cmd_obj *obj, bool to_head)
{
	u32 res = _FAIL;
...
post_process:
	...
	if (pcmd_callback == NULL) {
		if (pcmd->cmdcode == CMD_SET_DRV_EXTRA) {
			struct drvextra_cmd_parm *extra_parm = (pcmdbuf ?
				((struct drvextra_cmd_parm *)pcmdbuf) :
	...
	for (i = 0; i < param->ch_num; i++) {
		...
		if (param->ch[i].flags & RTW_IEEE80211_CHAN_PASSIVE_SCAN) {
			...
			if (param->scan_type == RTW_SCAN_NORMAL && !param->acs
				...
				&& !(GET_MAP_BSS_TYPE(padapter) & MULTI_AP_BACKHAUL_STA)
#endif
			)
...
#ifdef CONFIG_RTW_MULTI_AP
		/* easymesh agent do extra scan to increase the possibility of finding backhual bss(hidden ssid) */
...
u8 rtw_sitesurvey_cmd(_adapter *padapter, struct sitesurvey_parm *pparm)
{
	...
	case RTW_SCAN_RRM:
		...
		scan_priv->rrm_token = pparm->token;
		if (pparm->ch_num > 13) {
...
static u8 rtw_createbss_cmd(_adapter  *adapter, int flags, bool adhoc,
		u16 ifbmp, u16 excl_ifbmp, s16 req_ch, s8 req_bw,
		s8 req_offset, u8 is_change_chbw)
{
	struct cmd_priv *pcmdpriv = &adapter_to_dvobj(adapter)->cmdpriv;
	struct cmd_obj *cmd = NULL;
	struct createbss_parm *parm = NULL;
	struct submit_ctx *sctx = NULL;
	u8 res = _FAIL;

	if (req_ch > 0 && req_bw >= 0 && req_offset >= 0) {
		if (!rtw_chset_is_chbw_valid(adapter_to_chset(adapter),
					req_ch, req_bw, req_offset, 0, 0)) {
			return _FAIL;
		}
	}

	/* prepare cmd parameter */
	parm = (struct createbss_parm *)rtw_zmalloc(sizeof(struct createbss_parm));
	if (NULL == parm)
		return _FAIL;
	...
	if (!rtw_mi_is_scan_deny(adapter)) {
		rtw_mi_scan_abort(adapter, _TRUE);
		rtw_mi_set_scan_deny(adapter, 500);
	}
	...
	init_h2fwcmd_w_parm_no_rsp(cmd, parm, CMD_CREATE_BSS);

	if (flags & RTW_CMDF_WAIT_ACK) {
		sctx = (struct submit_ctx *)rtw_zmalloc(sizeof(struct submit_ctx));
		if (NULL == sctx)
			goto _free_cmd_obj;
		cmd->sctx = sctx;
	...
	if (flags & RTW_CMDF_WAIT_ACK) {
		if (_SUCCESS == res) {
			...
			_rtw_mutex_lock_interruptible(&pcmdpriv->sctx_mutex);
			if (RTW_SCTX_SUBMITTED == sctx->status)
				cmd->sctx = NULL;
			_rtw_mutex_unlock(&pcmdpriv->sctx_mutex);
		}

		rtw_mfree((void *)sctx, sizeof(struct submit_ctx));
	}

	return res;

_free_cmd_obj:
	rtw_mfree((void *)cmd, sizeof(struct cmd_obj));

_free_createbss_parm:
	rtw_mfree((void *)parm, sizeof(struct createbss_parm));
...
u8 rtw_disassoc_cmd(_adapter *padapter, u32 deauth_timeout_ms, int flags)
{
	...
	/* prepare cmd parameter */
	param = (struct disconnect_parm *)rtw_zmalloc(sizeof(struct disconnect_parm));
	if (NULL == param)
		return _FAIL;
...
_free_disconnect_param:
	rtw_mfree((void *)param, sizeof(struct disconnect_parm));
...
u8 rtw_stop_ap_cmd(_adapter *adapter, u8 flags)
{
	...
	/* no need to enqueue, do the cmd hdl directly and free cmd parameter */
	if (flags & RTW_CMDF_DIRECTLY)
		return (H2C_SUCCESS == stop_ap_hdl(adapter)) ? _SUCCESS : _FAIL;
	...
	init_h2fwcmd_w_parm_no_rsp(cmd, parm, CMD_SET_DRV_EXTRA);
...
_free_cmd_parm:
	rtw_mfree((void *)parm, sizeof(struct drvextra_cmd_parm));
...
u8 rtw_setopmode_cmd(_adapter  *adapter, NDIS_802_11_NETWORK_INFRASTRUCTURE networktype, u8 flags)
{
	struct cmd_priv *pcmdpriv = &adapter_to_dvobj(adapter)->cmdpriv;
	struct cmd_obj *cmd = NULL;
	struct setopmode_parm *parm = NULL;
	struct submit_ctx *sctx = NULL;
	u8 res = _FAIL;

	/* prepare cmd parameter */
	parm = (struct setopmode_parm *)rtw_zmalloc(sizeof(struct setopmode_parm));
	if (NULL == parm)
		return _FAIL;
	...
	init_h2fwcmd_w_parm_no_rsp(cmd, parm, CMD_SET_OPMODE);
...
_free_setopmode_parm:
	rtw_mfree((void *)parm, sizeof(struct setopmode_parm));
...
u8 rtw_addscsrsp_cmd(_adapter *padapter, struct sta_info *sta, u8 dialog_token)
{
	struct cmd_priv *pcmdpriv = &adapter_to_dvobj(padapter)->cmdpriv;
	struct cmd_obj *cmd;
	struct addSCSRsp_parm *paddSCSRsp_parm;
	u8 res = _SUCCESS , scs_idx = 0, scs_count = 0;
	...
	cmd->padapter = padapter;
	paddSCSRsp_parm = (struct addSCSRsp_parm *)rtw_zmalloc(sizeof(struct addSCSRsp_parm));

	if (paddSCSRsp_parm == NULL) {
		rtw_mfree((unsigned char *)cmd, sizeof(struct cmd_obj));
		res = _FAIL;
		goto exit;
	}

	_rtw_memcpy(paddSCSRsp_parm->addr, sta->phl_sta->mac_addr, ETH_ALEN);
	paddSCSRsp_parm->dialog_token = dialog_token;

	for (scs_idx = 0; scs_idx < MAX_TEMP_SCS_STATUS; scs_idx++) {
		paddSCSRsp_parm->scs_status[scs_idx].scsid = sta->phl_sta->scs_temp[scs_idx].scsid;
		paddSCSRsp_parm->scs_status[scs_idx].status = sta->phl_sta->scs_temp[scs_idx].status;
		paddSCSRsp_parm->scs_status[scs_idx].req_type = sta->phl_sta->scs_temp[scs_idx].req_type;
		if (sta->phl_sta->scs_temp[scs_idx].scsid != 0) {
			scs_count++;
		}
	}
	paddSCSRsp_parm->scs_count = scs_count;
	init_h2fwcmd_w_parm_no_rsp(cmd, paddSCSRsp_parm, CMD_ADD_SCSRSP);
	res = rtw_enqueue_cmd(pcmdpriv, cmd);
...
u8 rtw_addmscsrsp_cmd(_adapter *padapter, struct addMSCSRsp_parm param)
{
	struct cmd_priv *pcmdpriv = &adapter_to_dvobj(padapter)->cmdpriv;
	struct cmd_obj *cmd;
	struct addMSCSRsp_parm *param2;
	u8 res = _SUCCESS , scs_idx = 0, scs_count = 0;
	...
	cmd->padapter = padapter;
	param2 = (struct addMSCSRsp_parm *)rtw_zmalloc(sizeof(struct addMSCSRsp_parm));
	if (!param2) {
		rtw_mfree((unsigned char *)cmd, sizeof(struct cmd_obj));
		res = _FAIL;
		goto exit;
	...
	memcpy(param2, &param, sizeof(struct addMSCSRsp_parm));

	init_h2fwcmd_w_parm_no_rsp(cmd, param2, CMD_ADD_MSCSRSP);
	res = rtw_enqueue_cmd(pcmdpriv, cmd);
...
u8 rtw_free_assoc_resources_cmd(_adapter *padapter, u8 lock_scanned_queue, int flags)
{
	...
	parm->ec_id = FREE_ASSOC_RESOURCES;
	parm->type = lock_scanned_queue;
	parm->size = 0;
	parm->pbuf = NULL;
...
u8 rtw_set_chbw_cmd(_adapter *padapter, u8 ch, u8 bw, u8 ch_offset, u8 flags)
{
	struct cmd_priv *pcmdpriv = &adapter_to_dvobj(padapter)->cmdpriv;
	struct cmd_obj *cmd = NULL;
	struct set_ch_parm *set_ch_parm = NULL;
	struct submit_ctx *sctx = NULL;
	u8 res = _FAIL;
	...
	/* prepare cmd parameter */
	set_ch_parm = (struct set_ch_parm *)rtw_zmalloc(sizeof(struct set_ch_parm));
	if (set_ch_parm == NULL)
		goto _exit;
	...
	init_h2fwcmd_w_parm_no_rsp(cmd, set_ch_parm, CMD_SET_CHANNEL);
...
_free_set_ch_parm:
	rtw_mfree((void *)set_ch_parm, sizeof(struct set_ch_parm));
...
u8 _rtw_set_chplan_cmd(_adapter *adapter, int flags, u8 chplan, u8 chplan_6g,
		const struct country_chplan *country_ent, u8 swconfig)
{
	struct cmd_priv *pcmdpriv = &adapter_to_dvobj(adapter)->cmdpriv;
	struct cmd_obj *cmd = NULL;
	struct SetChannelPlan_param *parm = NULL;
	struct submit_ctx *sctx = NULL;
	u8 res = _FAIL;

	/* check if allow software config */
	if (swconfig && _TRUE == rtw_hal_is_disable_sw_channel_plan(adapter))
		return _FAIL;
	...
	/* prepare cmd parameter */
	parm = (struct SetChannelPlan_param *)rtw_zmalloc(sizeof(struct SetChannelPlan_param));
	if (NULL == parm)
		return _FAIL;
	...
	if (flags & RTW_CMDF_DIRECTLY) {
		if (H2C_SUCCESS == set_chplan_hdl(adapter, (u8 *)parm))
			res = _SUCCESS;
		goto _free_setchannelplan_param;
	}
	...
	init_h2fwcmd_w_parm_no_rsp(cmd, parm, CMD_SET_CHANPLAN);
	...
	if (_SUCCESS == res) {
		...
		if (RTW_SCTX_DONE_SUCCESS != sctx->status)
			res = _FAIL;
		...
		parm = (struct SetChannelPlan_param *)
			rtw_zmalloc(sizeof(struct SetChannelPlan_param));
		if (NULL == parm)
			return _FAIL;
		...
		rtw_mfree((void *)parm, sizeof(struct SetChannelPlan_param));
	}

	rtw_mfree((struct submit_ctx *)sctx, sizeof(*sctx));
...
u8 rtw_set_bc_cmd(_adapter *adapter)
{
        ...
        init_h2fwcmd_w_parm_no_parm_rsp(cmdobj, CMD_SET_COLOR);
...
u8 rtw_set_run_cmd_en(_adapter *padapter, u8 value, u8 flags)
{
	struct cmd_priv *pcmdpriv = &adapter_to_dvobj(padapter)->cmdpriv;
	struct cmd_obj *cmd = NULL;
	struct SetRunCmdEn_param *param = NULL;
	struct submit_ctx *sctx = NULL;
	u8 res = _FAIL;
	...
	init_h2fwcmd_w_parm_no_rsp(cmd, param, CMD_SET_RUN_CMD_EN);

	if (flags & RTW_CMDF_WAIT_ACK) {
		sctx = (struct submit_ctx *)rtw_zmalloc(sizeof(struct submit_ctx));
		if (NULL == sctx)
			goto _free_runcmden_param;
		cmd->sctx = sctx;
		rtw_sctx_init(sctx, 2000);
	}
...
_free_runcmden_param:
	rtw_mfree((void *)param, sizeof(struct SetRunCmdEn_param));
...
#define TRAFFIC_PROTECT_PERIOD_MS 4500
	...
	pmlmepriv->scan_deny_bypass_chk_cnt++;

	RTW_DBG("Debug[%s]: scan_deny_bypass_chk_cnt=%u\n", __func__, pmlmepriv->scan_deny_bypass_chk_cnt);
	RTW_DBG("Debug[%s]: NumMgmtTxInMinute=%u\n", __func__, pmlmepriv->LinkDetectInfo.NumMgmtTxInMinute);

	if (pmlmepriv->scan_deny_bypass_chk_cnt == 30) {
		pmlmepriv->scan_deny_bypass_ready = (pmlmepriv->LinkDetectInfo.NumMgmtTxInMinute > 60) ? 1 : 0;
		pmlmepriv->scan_deny_bypass_chk_cnt = 0;
		pmlmepriv->LinkDetectInfo.NumMgmtTxInMinute = 0;
	}

	RTW_DBG("Debug[%s]: scan_deny_bypass_ready=%u\n", __func__, pmlmepriv->scan_deny_bypass_ready);

	if (padapter->registrypriv.wifi_mib.scan_deny_bypass_mode == 2) {
		pmlmepriv->scan_deny_bypass = 1;
	} else if (padapter->registrypriv.wifi_mib.scan_deny_bypass_mode == 1) {
		pmlmepriv->scan_deny_bypass = pmlmepriv->scan_deny_bypass_ready;
	} else {
		pmlmepriv->scan_deny_bypass = 0;
	}
	RTW_DBG("Debug[%s]: scan_deny_bypass_mode=%u\n", __func__, padapter->registrypriv.wifi_mib.scan_deny_bypass_mode);
	RTW_DBG("Debug[%s]: scan_deny_bypass=%u\n", __func__, pmlmepriv->scan_deny_bypass);

	if (pmlmepriv->scan_deny_bypass_chk_cnt == 0) {
		RTW_DBG("Debug[%s]: %s: scan_deny_bypass=%u\n", __func__, pmlmepriv->cur_network.network.Ssid.Ssid, pmlmepriv->scan_deny_bypass);
	}
		...
		|| check_fwstate(pmlmepriv, WIFI_UNDER_LINKING) == _TRUE))
	{
		if(!_rtw_memcmp(mybssid, "\x0\x0\x0\x0\x0\x0", ETH_ALEN))
		{
			...
			if(sta && sta->phl_sta && !is_broadcast_mac_addr(sta->phl_sta->mac_addr))
			{
				...
				sta->sta_stats.smooth_tx_tp_kbits =
					(sta->sta_stats.smooth_tx_tp_kbits * 6 / 10) + (sta->sta_stats.tx_tp_kbits * 4 / 10);/*Kbps*/
				sta->sta_stats.smooth_rx_tp_kbits =
					(sta->sta_stats.smooth_rx_tp_kbits * 6 / 10) + (sta->sta_stats.rx_tp_kbits * 4 / 10);/*Kbps*/
...
#define WMM_MODE_PKT_TH 500
#define WMM_MODE_STABLE_TIME 3
...
void rtw_switch_wmm_mode(_adapter *padapter)
{
	...
	u64 tx_wmm_pkts_diff[4] = {0};

	if (rtw_wmm_dm) {
		for (i = 0; i < 4; i++) {
			tx_wmm_pkts_diff[i] = dvobj->tx_wmm_pkts[i] - dvobj->tx_wmm_pkts_prev[i];
			if (tx_wmm_pkts_diff[i] > WMM_MODE_PKT_TH) /* 3Mbps for size 1518B pkts */
				stream++;
		}
	}
	...
	if (stream > 1) {
		wmm_mode = 1;
		if (dvobj->wmm_mode_to <= WMM_MODE_STABLE_TIME)
			dvobj->wmm_mode_to++;
		else
			dvobj->wmm_mode_to = 30 + WMM_MODE_STABLE_TIME;
		
		#ifdef CONFIG_LMT_TXREQ
#ifdef CONFIG_RTL_WIFI_OFFLOAD
		if ((!dvobj->lmt_txreq_enable || (stream != dvobj->wmm_mode_streams)) &&
			RTL_WFO_IS_OFLD(padapter->dvobj)) {
			rtl_wfo_ipc_set_mib(padapter, RTL_WFO_DOE_ID, "txreq_lmt", 400);
		}
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
		if (dvobj->wmm_mode_streams != stream) {
			dvobj->wmm_mode_streams = stream;
			dvobj->wmm_mode_txreq_lmt = padapter->max_tx_ring_cnt/stream;
		}
		dvobj->lmt_txreq_enable = 1;
		#endif
	} else {
		...
		/* fast leave when wmm_mode_to < 3 */
		if (dvobj->wmm_mode_to < WMM_MODE_STABLE_TIME) {
			dvobj->wmm_mode_to = 0;
			wmm_mode = 0;
			#ifdef CONFIG_LMT_TXREQ
			dvobj->wmm_mode_streams = 1;
			dvobj->wmm_mode_txreq_lmt = padapter->max_tx_ring_cnt;
#ifdef CONFIG_RTL_WIFI_OFFLOAD
			if (dvobj->lmt_txreq_enable && RTL_WFO_IS_OFLD(padapter->dvobj)) {
				rtl_wfo_ipc_set_mib(padapter, RTL_WFO_DOE_ID, "txreq_lmt", 0);
			}
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
			dvobj->lmt_txreq_enable = 0;
			#endif
		}
	}

#ifdef CONFIG_AUTOTEST_DETECT
	if (dvobj->autotest_case == AUTOTEST_5G_BE_VI_LATENCY) {
		dvobj->wmm_mode_to = 0;
		wmm_mode = 2;
		#ifdef CONFIG_LMT_TXREQ
		dvobj->wmm_mode_streams = 1;
	...
	if (wmm_mode != dvobj->wmm_mode) {
		...
		if (wmm_mode == 1) {
#ifdef CONFIG_AUTOTEST_DETECT
			if (!dvobj->autotest_ac_edca[0])
				dvobj->autotest_ac_edca[0] = rtw_phl_read32(padapter->dvobj->phl, 0xc300);
			if (!dvobj->autotest_ac_edca[1])
				dvobj->autotest_ac_edca[1] = rtw_phl_read32(padapter->dvobj->phl, 0xc304);

			if (dvobj->autotest_case == AUTOTEST_MULTI_STA_LATENCY) {
				rtw_hw_set_edca(padapter, 0, 0x5e642b);
				rtw_hw_set_edca(padapter, 1, 0x5e642b);
			} else
#endif
...
#endif /* CONFIG_RTW_MANUAL_EDCA */
			{
				if (padapter->stapriv.asoc_list_cnt >= padapter->registrypriv.wifi_mib.wmm_edca_sta_thres && dvobj->traffic_stat.cur_tx_tp >= padapter->registrypriv.wifi_mib.wmm_edca_amsdu_tp_th) {
					rtw_hw_set_edca(padapter, 0, 0x652b);
					rtw_hw_set_edca(padapter, 2, 0x5e4419);
				} else {
			...
			if (!dvobj->autotest_ac_edca[0])
				dvobj->autotest_ac_edca[0] = rtw_phl_read32(padapter->dvobj->phl, 0xc300);
			if (!dvobj->autotest_ac_edca[2])
				dvobj->autotest_ac_edca[2] = rtw_phl_read32(padapter->dvobj->phl, 0xc308);

			if (tx_wmm_pkts_diff[XMIT_BE_QUEUE] > tx_wmm_pkts_diff[XMIT_VI_QUEUE]) {
				rtw_hw_set_edca(padapter, 0, 0xa42b);
				rtw_hw_set_edca(padapter, 2, 0x5e3219);
			} else {
				rtw_hw_set_edca(padapter, 0, 0x2119);
				rtw_hw_set_edca(padapter, 2, 0x211f);
			}
		}
#endif
		else {
#ifdef CONFIG_AUTOTEST_DETECT
			if (dvobj->autotest_ac_edca[0])
				rtw_hw_set_edca(padapter, 0, dvobj->autotest_ac_edca[0]);
			if (dvobj->autotest_ac_edca[1])
				rtw_hw_set_edca(padapter, 1, dvobj->autotest_ac_edca[1]);
			if (dvobj->autotest_ac_edca[2])
				rtw_hw_set_edca(padapter, 2, dvobj->autotest_ac_edca[2]);
			dvobj->autotest_ac_edca[0] = 0;
			dvobj->autotest_ac_edca[1] = 0;
			dvobj->autotest_ac_edca[2] = 0;
#endif
		}
	}
}
...
void _atm_set_mode(struct dvobj_priv *dvobj, u8 atm_mode)
{
	if (dvobj->txq_manual_mode)
		return;

	if (dvobj->txq_mode == atm_mode)
		return;

	if (atm_mode == ATM_MODE_THROUGHPUT) {
		dvobj->txq_ts_factor = 128;
		dvobj->txq_deq_factor = 200;
	} else if (atm_mode == ATM_MODE_PRECISE) {
		dvobj->txq_ts_factor = 32;
		dvobj->txq_deq_factor = 0;
	}

	dvobj->txq_mode = atm_mode;
}
void _atm_switch_mode(_adapter *padapter)
{
	struct dvobj_priv *dvobj = padapter->dvobj;
	int i;
	u8 atm_mode = ATM_MODE_THROUGHPUT;

	if (dvobj->tx_mode != 2)
		return;

	for (i = 0; i < dvobj->iface_nums; i++) {
		if (rtw_is_adapter_up(dvobj->padapters[i]) &&
			dvobj->padapters[i]->atm_mode == ATM_MODE_PRECISE) {
			atm_mode = ATM_MODE_PRECISE;
			break;
		}
	}
	_atm_set_mode(dvobj, atm_mode);
}
#endif

#ifdef CONFIG_AUTOTEST_DETECT
static const char * _autotest_dump_casename(u8 autotest_case)
{
	switch (autotest_case) {
		case AUTOTEST_NONE:
			return "AUTOTEST_NONE";
		case AUTOTEST_24G_LATENCY:
			return "AUTOTEST_24G_LATENCY";
		case AUTOTEST_5G_BE_VI_LATENCY:
			return "AUTOTEST_5G_BE_VI_LATENCY";
		case AUTOTEST_5G_MULTI_STA_TP:
			return "AUTOTEST_5G_MULTI_STA_TP";
		case AUTOTEST_5G_MULTI_STA_TP_160M:
			return "AUTOTEST_5G_MULTI_STA_TP_160M";
		case AUTOTEST_MULTI_STA_LATENCY:
			return "AUTOTEST_MULTI_STA_LATENCY";
		case AUTOTEST_5G_160M_ATF:
			return "AUTOTEST_5G_160M_ATF";
		case AUTOTEST_5G_32STA_MIX_MODE_TP:
			return "AUTOTEST_5G_32STA_MIX_MODE_TP";
		case AUTOTEST_5G_32STA_ALL_AX_160M_TP:
			return "AUTOTEST_5G_32STA_ALL_AX_160M_TP";
		case AUTOTEST_MAX:
			return "AUTOTEST_MAX";
		default:
			return "undefined";
...
void _autotest_detect_setting(_adapter *padapter, _adapter *target_adapter)
{
	struct dvobj_priv *dvobj = padapter->dvobj;
	struct sta_priv *pstapriv;
	struct wifi_mib_priv *pmibpriv;
	_list	*phead, *plist;
	struct sta_info *psta = NULL;
	int i;
	enum wlan_mode wmode;

	if (dvobj->autotest_case == dvobj->autotest_case_prev)
		return;

	/* restore previous settings first */
	if (dvobj->autotest_case_prev == AUTOTEST_5G_MULTI_STA_TP_160M) {
		#ifdef SBWC
		for (i = 0; i < dvobj->iface_nums; i++) {
			if (!dvobj->padapters[i]->netif_up)
				continue;
			if (dvobj->padapters[i]->stapriv.asoc_sta_count == 0)
				continue;

			pstapriv = &dvobj->padapters[i]->stapriv;
			pmibpriv = &dvobj->padapters[i]->registrypriv.wifi_mib;
			...
			while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
				...
				if (is_supported_vht(wmode) &&
					!is_supported_he(wmode)) {
					psta->SBWC_mode &= ~SBWC_MODE_LIMIT_STA_TX;
					psta->SBWC_tx_limit = 0;
					psta->SBWC_tx_limit_byte = 0;
				} else if (is_supported_he(wmode) &&
...
#ifdef FC_SBWC
			rtw_fc_sbwc_update_adapter_host_policer(dvobj->padapters[i]);
#endif
		}
		#endif
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			dvobj->padapters[i]->xmitpriv.txsc_amsdu_mode = 1;
		}
		#endif
	}
#ifdef CONFIG_CTC_FEATURE
	else if (dvobj->autotest_case_prev == AUTOTEST_5G_32STA_MIX_MODE_TP) {
		#ifdef SBWC
		for (i = 0; i < dvobj->iface_nums; i++) {
			if (dvobj->padapters[i]->sbwc_setby_autotest) {
				dvobj->padapters[i]->SBWC_mode &= ~SBWC_MODE_LIMIT_STA_TX;
				dvobj->padapters[i]->SBWC_tx_limit_byte = 0;
				dvobj->padapters[i]->SBWC_tx_limit = 0;
				dvobj->padapters[i]->sbwc_length = SBWC_NUM_PKT_QUEUE;
				dvobj->padapters[i]->sbwc_setby_autotest = 0;
#ifdef FC_SBWC
				rtw_fc_sbwc_update_adapter_host_policer(dvobj->padapters[i]);
#endif
			}
		...
		#endif
		rtw_phl_write32(dvobj->phl, 0x1184, dvobj->autotest_reg_1184_bk);
		dvobj->txsc_amsdu_timeout_use_hw_timer = 1;
	}
#endif
	else if (dvobj->autotest_case_prev == AUTOTEST_5G_32STA_ALL_AX_160M_TP) {
		#ifdef SBWC
		for (i = 0; i < dvobj->iface_nums; i++) {
			if (dvobj->padapters[i]->sbwc_setby_autotest) {
				dvobj->padapters[i]->SBWC_mode &= ~SBWC_MODE_LIMIT_STA_TX;
	...
	/* apply new settings */
	if (dvobj->autotest_case == AUTOTEST_5G_MULTI_STA_TP_160M) {
		#ifdef SBWC
		for (i = 0; i < dvobj->iface_nums; i++) {
			if (!dvobj->padapters[i]->netif_up)
				continue;
			...
			while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
				...
				if (is_supported_vht(wmode) &&
					...
					/* AC STAs limit 80M */
					psta->SBWC_mode |= SBWC_MODE_LIMIT_STA_TX;
					psta->SBWC_tx_limit = 75000;
					psta->SBWC_tx_limit_byte = ((psta->SBWC_tx_limit * 1024 / 8) / (HZ / SBWC_TO));
				} else if (is_supported_he(wmode) &&
					psta->phl_sta->chandef.bw == CHANNEL_WIDTH_80) {
					/* AX 80M STAs limit 100M */
					psta->SBWC_mode |= SBWC_MODE_LIMIT_STA_TX;
					psta->SBWC_tx_limit = 100000;
					psta->SBWC_tx_limit_byte = ((psta->SBWC_tx_limit * 1024 / 8) / (HZ / SBWC_TO));
				} else if (is_supported_he(wmode) &&
					psta->phl_sta->chandef.bw == CHANNEL_WIDTH_160) {
					/* AX 80M STAs limit 100M */
					psta->SBWC_mode |= SBWC_MODE_LIMIT_STA_TX;
					psta->SBWC_tx_limit = 300000;
					psta->SBWC_tx_limit_byte = ((psta->SBWC_tx_limit * 1024 / 8) / (HZ / SBWC_TO));
				}
			}
			_rtw_spinunlock_bh(&pstapriv->asoc_list_lock);
		...
		#endif
	} else if (dvobj->autotest_case == AUTOTEST_5G_160M_ATF) {
		rtw_hw_set_edca(padapter, 0, 0x642b);
	} else if (dvobj->autotest_case == AUTOTEST_5G_BE_VI_LATENCY) {
		#ifdef CONFIG_TXSC_AMSDU
		for (i = 0; i < dvobj->iface_nums; i++) {
			dvobj->padapters[i]->xmitpriv.txsc_amsdu_mode = 0;
		}
		#endif
	}
#ifdef CONFIG_CTC_FEATURE
	else if (dvobj->autotest_case == AUTOTEST_5G_32STA_MIX_MODE_TP) {
		#ifdef SBWC
		if (target_adapter) {
			target_adapter->SBWC_mode |= SBWC_MODE_LIMIT_STA_TX;
			target_adapter->SBWC_tx_limit_byte = ((19000 * 1024 / 8) / (HZ / SBWC_TO));
			target_adapter->SBWC_tx_limit = 19000;
			target_adapter->sbwc_length = 4096;
			target_adapter->sbwc_setby_autotest = 1;
#ifdef FC_SBWC
			rtw_fc_sbwc_update_adapter_host_policer(target_adapter);
#endif
		} else {
			for (i = 0; i < dvobj->iface_nums; i++) {
				if (dvobj->padapters[i]->netif_up) {
					dvobj->padapters[i]->SBWC_mode |= SBWC_MODE_LIMIT_STA_TX;
					dvobj->padapters[i]->SBWC_tx_limit_byte = ((19000 * 1024 / 8) / (HZ / SBWC_TO));
					dvobj->padapters[i]->SBWC_tx_limit = 19000;
					dvobj->padapters[i]->sbwc_length = 4096;
					dvobj->padapters[i]->sbwc_setby_autotest = 1;
#ifdef FC_SBWC
					rtw_fc_sbwc_update_adapter_host_policer(
									dvobj->padapters[i]);
#endif
				}
			}
		...
		dvobj->autotest_reg_1184_bk = rtw_phl_read32(dvobj->phl, 0x1184);
		rtw_phl_write32(dvobj->phl, 0x1184, 0xc400f);
		/* amsdu use SW timer */
		dvobj->txsc_amsdu_timeout_use_hw_timer = 0;
		rtw_hw_set_edca(padapter, 0, 0x138611b);
	}
#endif /* CONFIG_CTC_FEATURE */
	else if (dvobj->autotest_case == AUTOTEST_5G_32STA_ALL_AX_160M_TP) {
		#ifdef SBWC
		if (target_adapter) {
			target_adapter->SBWC_mode |= SBWC_MODE_LIMIT_STA_TX;
			target_adapter->SBWC_tx_limit_byte = ((45000 * 1024 / 8) / (HZ / SBWC_TO));
			target_adapter->SBWC_tx_limit = 45000;
			target_adapter->sbwc_setby_autotest = 1;
		} else {
			for (i = 0; i < dvobj->iface_nums; i++) {
				if (dvobj->padapters[i]->netif_up) {
					dvobj->padapters[i]->SBWC_mode |= SBWC_MODE_LIMIT_STA_TX;
					dvobj->padapters[i]->SBWC_tx_limit_byte = ((45000 * 1024 / 8) / (HZ / SBWC_TO));
					dvobj->padapters[i]->SBWC_tx_limit = 45000;
					dvobj->padapters[i]->sbwc_setby_autotest = 1;
				}
			}
		}
		#endif
		rtw_hw_set_edca(padapter, 0, 0x1786119);
	}
}

void rtw_check_autotest_detect(_adapter *padapter)
{
	struct dvobj_priv *dvobj = padapter->dvobj;
	struct sta_priv *pstapriv;
	struct wifi_mib_priv *pmibpriv;
	...
	int sta_11ax_num = 0;
	int sta_160m_num = 0;
	int sta_txmode_num = 0;
	int sta_num = 0;
	int sta_11n_num_if[CONFIG_IFACE_NUMBER] = {0};
	int sta_11ac_num_if[CONFIG_IFACE_NUMBER] = {0};
	int sta_11ax_num_if[CONFIG_IFACE_NUMBER] = {0};
	int sta_txmode_num_if[CONFIG_IFACE_NUMBER] = {0};
	int sta_num_if[CONFIG_IFACE_NUMBER] = {0};
	enum wlan_mode wmode;
	int i, j;
	u32 stream = 0;
	_adapter *target_adapter = NULL;

	for (i = 0; i < dvobj->iface_nums; i++) {
		if (dvobj->padapters[i]->netif_up) {
			if (dvobj->padapters[i]->stapriv.asoc_sta_count) {
				...
				while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
					...
					wmode = pmibpriv->band & psta->phl_sta->wmode;
					if (is_supported_ht(wmode) && 
						!is_supported_vht(wmode) &&
						!is_supported_he(wmode)) {
						sta_11n_num ++;
						sta_11n_num_if[i]++;
					} else if (is_supported_vht(wmode) &&
						!is_supported_he(wmode)) {
						sta_11ac_num ++;
						sta_11ac_num_if[i]++;
					} else if (is_supported_he(wmode)) {
					...
					if (psta->phl_sta->chandef.bw == CHANNEL_WIDTH_160)
						sta_160m_num ++;

					if (psta->sta_stats.tx_tp_kbits > 4 * psta->sta_stats.rx_tp_kbits) {
						sta_txmode_num ++;
						sta_txmode_num_if[i]++;
					}
				}
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		if ((sta_txmode_num_if[i] == sta_txmode_num) &&
			(sta_num_if[i] == sta_num)) {
			target_adapter = dvobj->padapters[i];
			break;
		}
	}

	for (i = 0; i < 4; i++) {
		if (dvobj->tx_wmm_pkts[i] - dvobj->tx_wmm_pkts_prev[i] > WMM_MODE_PKT_TH)
			stream |= BIT(i);
	}

	dvobj->autotest_case_prev = dvobj->autotest_case;

	if (dvobj->autotest_force_case) {
		if (dvobj->autotest_force_case == 255)
			dvobj->autotest_case = AUTOTEST_NONE;
		else
			dvobj->autotest_case = dvobj->autotest_force_case;
		dvobj->autotest_case_cntdown = 0;
	} else if ((sta_11n_num == 4) &&
		(sta_num == sta_11n_num) &&
		WIFI_ROLE_IS_ON_24G(padapter)) {
		/* case 1: CTC 13.1.21 latency 2.4G 11N*/
		dvobj->autotest_case = AUTOTEST_24G_LATENCY;
		dvobj->autotest_case_cntdown = 0;
	} else if ((sta_num == 8) &&
		(stream == (BIT(XMIT_BE_QUEUE) | BIT(XMIT_VI_QUEUE))) &&
		WIFI_ROLE_IS_ON_5G(padapter)) {
		/* case 2: CTC/CU 4.27 latency 5G*/
		dvobj->autotest_case = AUTOTEST_5G_BE_VI_LATENCY;
		dvobj->autotest_case_cntdown = 0;
	} else if ((sta_num == 6) &&
		(sta_11ac_num == 4) &&
		(sta_11ax_num == 2) &&
		WIFI_ROLE_IS_ON_5G(padapter)){
		/* case 3: CTC 4.30.1 multi-STA TP 5G*/
		dvobj->autotest_case = AUTOTEST_5G_MULTI_STA_TP;
		dvobj->autotest_case_cntdown = 0;
	} else if ((sta_num == 6) &&
		(sta_11ac_num == 2) &&
		(sta_11ax_num == 4) &&
		(sta_160m_num == 2) &&
		WIFI_ROLE_IS_ON_5G(padapter)) {
		/* case 3: CTC 4.30.1 multi-STA TP 5G 160M*/
		dvobj->autotest_case = AUTOTEST_5G_MULTI_STA_TP_160M;
		dvobj->autotest_case_cntdown = 0;
	} else if ((sta_num == 4) &&
		stream == (BIT(XMIT_BE_QUEUE) | BIT(XMIT_BK_QUEUE) | BIT(XMIT_VI_QUEUE) | BIT(XMIT_VO_QUEUE))){
		/* case 4: CTC 13.1.10 multi-STA Latency*/
		dvobj->autotest_case = AUTOTEST_MULTI_STA_LATENCY;
		dvobj->autotest_case_cntdown = 0;
	} else if ((sta_num == 2) &&
		(sta_11ax_num == 2) &&
		(sta_160m_num == 2) &&
		(sta_txmode_num == 2)) {
		/* case 5: CTC 5G 160M airtime fairness */
		dvobj->autotest_case = AUTOTEST_5G_160M_ATF;
		dvobj->autotest_case_cntdown = 0;
	}
#ifdef CONFIG_CTC_FEATURE
	else if ((sta_num >= 32) &&
		...
		WIFI_ROLE_IS_ON_5G(padapter) &&
		target_adapter &&
		sta_160m_num && (sta_160m_num < 32) &&
		sta_11ax_num && (sta_11ax_num < 32)) {
		/* case 6: CTC FTTR 5G 32 STA Throughput */
		dvobj->autotest_case = AUTOTEST_5G_32STA_MIX_MODE_TP;
		dvobj->autotest_case_cntdown = AUTOTEST_CASE_CNTDOWN;
	}
#endif /* CONFIG_CTC_FEATURE */
	else if ((sta_num >= 32) &&
		(sta_txmode_num == 32) &&
		...
		(sta_160m_num == 32) &&
		(sta_11ax_num == 32)) {
		/* case 7: 5G 32 STA Throughput(All STA AX 160M)*/
		dvobj->autotest_case = AUTOTEST_5G_32STA_ALL_AX_160M_TP;
		dvobj->autotest_case_cntdown = AUTOTEST_CASE_CNTDOWN;
	}
	else {
		if (dvobj->autotest_case_cntdown > 0) {
			dvobj->autotest_case_cntdown--;
		} else {
			dvobj->autotest_case = AUTOTEST_NONE;
			dvobj->autotest_case_cntdown = 0;
		}
	...
	_autotest_detect_setting(padapter, target_adapter);
}
#endif

#ifdef CONFIG_RTW_HANDLE_SER_L2
void _rtw_recover_trx_path_check(_adapter *padapter)
{
	bool ret;

	if (!padapter->dvobj->ser_L2_cnt)
		return;

	if (padapter->dvobj->ser_L2_check_cnt >= 2)
		return;

	ret = rtw_phl_trx_force_resume(padapter->dvobj->phl);
	if (ret == true)
		padapter->dvobj->ser_L2_check_cnt++;
}
#endif
...
{
#ifdef CONFIG_AUTOTEST_DETECT
	rtw_check_autotest_detect(padapter);
#endif
	rtw_switch_wmm_mode(padapter);
...
#ifdef CONFIG_ONE_TXQ
	_atm_switch_mode(padapter);
#endif

#ifdef CONFIG_RTW_HANDLE_SER_L2
	_rtw_recover_trx_path_check(padapter);
#endif
}
		...
		u32 wait_ms, u8 flags)
{
	...
	struct cmd_obj *cmd = NULL;
	struct drvextra_cmd_parm *parm = NULL;
	struct mgnt_tx_parm *mgnt_parm = NULL;
	struct submit_ctx *sctx = NULL;
	u8 res = _FAIL;
...
_free_mgnt_parm:
	rtw_mfree((void *)mgnt_parm, sizeof(struct mgnt_tx_parm));
...
void _rtw_dfs_associated_clients_update_bw(struct dvobj_priv *dvobj)
{
	_adapter *padapter;
	...
	int i;
	enum channel_width cur_bw = rtw_get_oper_bw(pri_adapter);
	
	for (i = 0; i < dvobj->iface_nums; i++) {
		padapter = dvobj->padapters[i];
		if (!padapter || !(ifbmp_m & BIT(padapter->iface_id)))
			continue;
		...
		while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
			...
			if (psta->phl_sta->chandef.bw <= CHANNEL_WIDTH_160 &&
				psta->phl_sta->chandef.bw > cur_bw) {
				psta->phl_sta->chandef.bw = cur_bw;
				rtw_phl_cmd_change_stainfo(padapter->dvobj->phl,
						psta->phl_sta,
						STA_CHG_BW,
						NULL,
						0,
...
void rtw_ch_switch_hdl(struct dvobj_priv *dvobj)
{
	...
	if (ifbmp_m) {
		...
		rtw_change_bss_chbw_cmd(dvobj_get_primary_adapter(dvobj),
				0, /* just enqueue cmd, no wait */
				ifbmp_m, ifbmp_s, req_ch,
				req_bw, req_offset);
		_rtw_dfs_associated_clients_update_bw(dvobj);
...
void core_dump_record(_adapter *adapter, u8 dump_type, void *m)
{
	...
	RTW_PRINT_SEL(m, "rxCnt_data: orig=%d shortcut=%d(ratio=%d)\n",
		...
		log->rxCnt_data_shortcut/((log->rxCnt_data_orig+log->rxCnt_data_shortcut)/100));
#ifdef CORE_RXSC_AMSDU
	RTW_PRINT_SEL(m, "rxCnt_amsdu: orig=%d shortcut=%d(ratio=%d) \n",
		log->rxCnt_amsdu_orig, log->rxCnt_amsdu_shortcut,
		log->rxCnt_amsdu_shortcut/((log->rxCnt_amsdu_orig+log->rxCnt_amsdu_shortcut)/100));
#endif
#ifdef CONFIG_RTW_BYPASS_DEAMSDU
	RTW_PRINT_SEL(m, "rxCnt_amsdu: bypass=%d \n",
		log->rxCnt_amsdu_bypass);
#endif
...
static void activate_swq_one_sta(_adapter *adapter)
{
	struct sta_priv *pstapriv = &adapter->stapriv;
	struct sta_info *psta, *first_sta = NULL;
	int i;
	_list	*plist, *phead;

	if (!adapter->swq_one_sta) {
		_rtw_memset(adapter->swq_one_sta_mac, 0, ETH_ALEN);
		return;
	}
	...
	for (i = 0; i < NUM_STA; i++) {
		...
		while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
			...
			if (psta->phl_sta && psta->phl_sta->aid > 0) {
				if (first_sta == NULL)
					first_sta = psta;
				else if (psta->phl_sta->aid < first_sta->phl_sta->aid)
					first_sta = psta;
			}
		}
	}
	...
	if (first_sta)
		_rtw_memcpy(adapter->swq_one_sta_mac, first_sta->phl_sta->mac_addr, ETH_ALEN);
...
void core_cmd_swq(_adapter *adapter, void *cmd_para, u32 para_num)
{
	...
	if(!strcmp(para, "max_enq_len")) {
		...
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		adapter->set_max_deq_len = value;
	} else if(!strcmp(para, "sta_deq_len")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		adapter->set_sta_deq_len = value;
	...
	} else if(!strcmp(para, "amsdu_num")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		adapter->set_tx_amsdu = (u8)value;
	...
	} else if(!strcmp(para, "swq_one_sta")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		#ifdef WKARD_SWQ_MAP_ONE_STA
		adapter->swq_one_sta = value;
		activate_swq_one_sta(adapter);
		#endif
	} else if(!strcmp(para, "swq_limit_threshold")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		#ifdef CONFIG_SWQ_SKB_ARRAY_LIMIT
		adapter->swq_limit_threshold = value;
		#endif
	} else if(!strcmp(para, "vw_check_len")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		adapter->vw_check_len = value;
#ifdef CONFIG_VW_REFINE
	} else if(!strcmp(para, "force_vw_sta")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		adapter->force_vw_sta = value;
#endif
		...
		#ifdef CONFIG_SWQ_SKB_ARRAY_LIMIT
		#define RTL_CIRC_CNT2(head,tail,size) ((head>=tail)?(head-tail):(size-tail+head))
		printk("skb_array_enqueue_ptr:%d skb_array_dequeue_ptr:%d, diff = %d, drop = %d\n", \
		adapter->skb_array_enqueue_ptr, adapter->skb_array_dequeue_ptr,
		RTL_CIRC_CNT2(adapter->skb_array_enqueue_ptr, adapter->skb_array_dequeue_ptr, adapter->swq_limit_threshold),
		adapter->tx_logs.core_vw_swq_enq_fail_limit);
		#endif
		printk("adapter:%p hw_cnt:%u hw_tick:%u\n", \
		adapter, adapter->hw_swq_cnt, adapter->hw_irq_cnt);
#ifdef CONFIG_RTL_WIFI_OFFLOAD
		printk("PE: hw_pe_irq_cnt:%u\n", adapter->hw_pe_irq_cnt);
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
#ifdef CONFIG_SWQ_SKB_ARRAY
		printk("free_txq:%d amsdu_cnt:%d\n",
				pfree_txreq_queue->qlen, adapter->swq_amsdu_cnt);
#else
		printk("free_txq:%d amsdu_cnt:%d swq_cnt:%d\n",
				pfree_txreq_queue->qlen, adapter->swq_amsdu_cnt,
				ATOMIC_READ(&swq->skb_xmit_queue_len));
#endif
...
#ifdef CONFIG_SWQ_SKB_ARRAY
		printk("vw_swq_enq_fail_array:%d \n", adapter->tx_logs.core_vw_swq_enq_fail_array);
#ifdef CONFIG_SWQ_SKB_ARRAY_LIMIT
		printk("vw_swq_enq_fail_limit:%d \n", adapter->tx_logs.core_vw_swq_enq_fail_limit);
#endif
#endif
		...
		printk("vw_swq_enq_fail_not_handle:%d \n", adapter->tx_logs.core_vw_swq_enq_fail_not_handle);
		...
		printk("vw_total_retry:%d \n", adapter->vw_total_retry);
#endif
		for (i = 0; i < MAX_SKB_XMIT_QUEUE; i++) {
			if (i != adapter->self_sta->phl_sta->macid) {
				psta = rtw_get_stainfo_by_macid(&adapter->stapriv, i);
				if (!psta || (psta->padapter != adapter))
					continue;  // not adapter itself, nor a sta connecting to adapter
			}

			if (swq->vw_send_cnt[i]) {
				printk("m-id:%2d send:%d recycle:%d retry:%d\n", i,
				swq->vw_send_cnt[i], swq->vw_recycle_cnt[i], swq->vw_retry_cnt[i]);
			}
		}
		...
		for (i = 0; i < MAX_SKB_XMIT_QUEUE; i++) {
			...
			swq->vw_send_cnt[i] = 0;
			swq->vw_recycle_cnt[i] = 0;
			swq->vw_retry_cnt[i] = 0;
		}
		adapter->vw_total_retry = 0;
		...
		adapter->tx_logs.core_vw_swq_enq_fail_not_handle = 0;
		...
		printk("sta_deq_len: %d%s\n", adapter->sta_deq_len, (adapter->set_sta_deq_len > 0) ? "(M)" : "");
		printk("amsdu_num:   %d%s\n", adapter->tx_amsdu, (adapter->set_tx_amsdu > 0) ? "(M)" : "");
		printk("ring_lmt:    %d\n", adapter->ring_lmt);
		printk("dsr_time:    %d%s\n", adapter->xmit_dsr_time, (adapter->set_xmit_dsr_time > 0) ? "(M)" : "");
		printk("merge:       %d\n", adapter->amsdu_merge_cnt);
		printk("tx_lmt:      %d%s\n", adapter->tx_lmt, (adapter->set_tx_lmt > 0) ? "(M)" : "");
		printk("hw_time:     %d%s\n", adapter->hw_swq_timeout, (adapter->set_hw_swq_timeout > 0) ? "(M)" : "");
		printk("swq_time:    %d%s\n", adapter->swq_timeout, (adapter->set_swq_timeout > 0) ? "(M)" : "");
		printk("no_wdinfo:   %d\n", adapter->no_wdinfo);
		printk("with_bk:     %d\n", adapter->with_bk);
		printk("no_rts:      %d\n", adapter->no_rts);
		printk("sp_sz:       %d\n", adapter->spec_pktsz);
		...
		printk("vw_check_len:%d\n", adapter->vw_check_len);
		#ifdef WKARD_SWQ_MAP_ONE_STA
		printk("swq_one_sta:       %d\n", adapter->swq_one_sta);
		printk("swq_one_sta_mac:   %pM\n", adapter->swq_one_sta_mac);
		#endif
		#ifdef CONFIG_SWQ_SKB_ARRAY_LIMIT
		printk("swq_limit_threshold:   %d\n", adapter->swq_limit_threshold);
		#endif
		#ifdef CONFIG_VW_REFINE
		printk("force_vw_sta       :   %d\n", adapter->force_vw_sta);
		#endif
	}
}
...
void core_cmd_txq(_adapter *adapter, void *cmd_para, u32 para_num)
{
	...
	if (!strcmp(para, "max_enq_len")) {
	...
	} else if (!strcmp(para, "bcast_threshold")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		dvobj->txq_bcst_threshold = value;
	...
	} else if (!strcmp(para, "active_tp")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		dvobj->txq_active_tp = value;
	} else if (!strcmp(para, "active_tp_legacy")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		dvobj->txq_active_tp_legacy = value;
	} else if (!strcmp(para, "manual_mode")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		dvobj->txq_manual_mode = value;
	} else if (!strcmp(para, "txq_rx_enq_tp")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		dvobj->txq_rx_enq_tp = value;
	} else if (!strcmp(para, "dump")) {
		printk("max_enq_len=%u\n", dvobj->txq_max_enq_len);
		printk("bcast_threshold=%u\n", dvobj->txq_bcst_threshold);
		...
		printk("active_tp=%u\n", dvobj->txq_active_tp);
		printk("active_tp_legacy=%u\n", dvobj->txq_active_tp_legacy);
		printk("manual_mode=%u\n", dvobj->txq_manual_mode);
		printk("txq_rx_enq_tp=%u\n", dvobj->txq_rx_enq_tp);
	}
}
#endif
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	if(is_txforce_apply)
		rtl_wfo_upd_all_sta_info(adapter, _TRUE);
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
...
void cmd_a4_dump_sta(_adapter *adapter, void *m)
{
	...
	_rtw_spinlock_bh(&adapter->a4_sta_list_lock);
	...
	while ((plist != phead) && (plist != NULL)) {
		...
		if (psta_a4 && psta_a4->phl_sta) {
			RTW_PRINT_SEL(m, "[%d]a4_sta(%d):%pM \n", idx,
				psta_a4->phl_sta->macid, psta_a4->phl_sta->mac_addr);
		} else {
			if (psta_a4)
	...
	_rtw_spinunlock_bh(&adapter->a4_sta_list_lock);
}
...
void cmd_a4_dump_db(_adapter *adapter, void *m)
{
	...
	_rtw_spinlock_bh(&adapter->a4_db_lock);
	...
	for (i = 0 ; i < A4_STA_HASH_SIZE; i++) {
		...
		while (db != NULL) {
			RTW_PRINT_SEL(m, "[%d] ", i);
			RTW_PRINT_SEL(m, "source:%pM ", db->mac);
	...
	_rtw_spinunlock_bh(&adapter->a4_db_lock);
	...
	for (i = 0 ; i < A4_LOOP_HASH_SIZE; i++)
	{
		...
		{
			RTW_PRINT_SEL(m, "[%d]	mac: %pM, time: %u\n", i, entry->mac, rtw_systime_to_ms(entry->stime));
		}
	}
#endif
}
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
void cmd_a4_dump_fc_sta_table(_adapter *adapter, void *m)
{
	u8 i;
	...
	RTW_PRINT_SEL(m, "=== FC STA Table === \n");
	for (i = 0; i < FC_STA_NUM; i++) {
		if (wfo_priv->fc_sta[i].macid == 0xff)
			continue;
		RTW_PRINT_SEL(m, "FC_STA[%d]: macid %d band %d %s:%pM\n", i,
			wfo_priv->fc_sta[i].macid, wfo_priv->fc_sta[i].band,
			(wfo_priv->fc_sta[i].is_sta ? "STA" : "A4_DB"),
			wfo_priv->fc_sta[i].mac);
	}
}
#endif
	...
	if (!strcmp(para, "dump")) {
		...
		if (adapter->a4_enable) {
			...
			if (RTL_WFO_IS_OFLD(adapter->dvobj))
				cmd_a4_dump_fc_sta_table(adapter, RTW_DBGDUMP);
#endif
		}
	}
}
...
void core_cmd_dump_cnt(_adapter *adapter, void *cmd_para, u32 para_num)
{
	...
	if (!strcmp(para, "reset")) {
		...
		phl_com->tx_stats.phl_txreq_jumbo_frame_drop = 0;
		#endif
	...
	RTW_PRINT("	alloc_fail_cnt: %d\n", pxmitpriv->alloc_fail_xmitframe_cnt);
#endif
	RTW_PRINT("[XMITFRAME_EXT]\n");
	RTW_PRINT("	free_cnt_ext: %d\n", pxmitpriv->free_xframe_ext_cnt);
	RTW_PRINT("	full_cnt_ext: %d\n", pxmitpriv->full_xframe_ext_cnt);
#ifdef CONFIG_DYN_ALLOC_XMITFRAME
	RTW_PRINT("	alloc_fail_cnt: %d\n", pxmitpriv->alloc_fail_xmitframe_ext_cnt);
	RTW_PRINT("	alloc_txreq_fail_cnt: %d\n", pxmitpriv->alloc_fail_txreq_ext_cnt);
#endif
#ifdef CONFIG_TX_DEFER
	RTW_PRINT("	defer_tx_flag: %d\n", pxmitpriv->defer_tx_flag);
	RTW_PRINT("	defer_tx_cnt: %d\n", ATOMIC_READ(&pxmitpriv->defer_tx_cnt));
#endif

#if 0
	RTW_PRINT("[XMITFRAME_CMD]\n");
	...
	/* TXREQ */
	RTW_PRINT("[TXREQ]\n");
	RTW_PRINT("	free_cnt: %d\n", adapter->pfree_txreq_queue->qlen);
	RTW_PRINT("	full_cnt: %d\n", adapter->txreq_full_cnt);

	RTW_PRINT("	wp_tg_out_of_resource: %d\n", phl_com->tx_stats.wp_tg_out_of_resource);
	RTW_PRINT("	wp_tg_force_reuse: %d\n", phl_com->tx_stats.wp_tg_force_reuse);
	RTW_PRINT("	phl_txreq_sta_leave_drop: %d\n", phl_com->tx_stats.phl_txreq_sta_leave_drop);
	RTW_PRINT("	phl_txreq_jumbo_frame_drop: %d\n", phl_com->tx_stats.phl_txreq_jumbo_frame_drop);
	RTW_PRINT("	cnt_txsc_amsdu_dfree: %d\n", adapter->xmitpriv.cnt_txsc_amsdu_dfree);
#ifdef CONFIG_ETHER_PKT_AGG
	RTW_PRINT(" fc_registered_sta_num: %d\n", _os_atomic_read(adapter, &adapter_to_dvobj(adapter)->fc_registered_sta_num));
#endif

#ifdef CONFIG_WFA_OFDMA_Logo_Test
	RTW_PRINT("[core_xframe_macid_current]:\n");
	_rtw_spinlock_bh(&pstapriv->asoc_list_lock);
	plist = get_next(phead);
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		psta = LIST_CONTAINOR(plist, struct sta_info, asoc_list);
		plist = get_next(plist);
		RTW_PRINT(" macid[%d] core_current_xframe_cnt: %d\n",
				psta->phl_sta->macid, psta->core_current_xframe_cnt);
	}
	_rtw_spinunlock_bh(&pstapriv->asoc_list_lock);
	RTW_PRINT("\n");

	RTW_PRINT("[core_txreq_macid_current]:\n");
	_rtw_spinlock_bh(&pstapriv->asoc_list_lock);
	plist = get_next(phead);
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		psta = LIST_CONTAINOR(plist, struct sta_info, asoc_list);
		plist = get_next(plist);
		RTW_PRINT(" macid[%d] core_current_txreq_cnt: %d\n",
	...
	RTW_PRINT("======= [os tx] =======: \n");
	RTW_PRINT(" total os_tx_pkts: %lld\n", pxmitpriv->os_tx_pkts);
	RTW_PRINT(" total os_tx_drop: %lld\n", pxmitpriv->os_tx_drop);
#ifdef CONFIG_WFA_OFDMA_Logo_Test_Statistic
	_rtw_spinlock_bh(&pstapriv->asoc_list_lock);
	plist = get_next(phead);
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		psta = LIST_CONTAINOR(plist, struct sta_info, asoc_list);
		plist = get_next(plist);
		RTW_PRINT(" macid[%d] os_tx_cnt: %d (drop: %d) ==> %d\n",
	...
	RTW_PRINT("======= [core tx] =======: \n");
	RTW_PRINT(" core_tx_pkts: %d\n", pxmitpriv->core_tx_pkts);
	RTW_PRINT(" core_tx_drop: %d\n", pxmitpriv->core_tx_drop);
	for(i = 0; i < 10; i++)
		if(pxmitpriv->core_tx_abort[i] != 0)
			RTW_PRINT("  core_tx_abort[%d]: %d\n", i, pxmitpriv->core_tx_abort[i]);

#ifdef CONFIG_WFA_OFDMA_Logo_Test_Statistic
	if(!pxmitpriv->txsc_enable){
		RTW_PRINT("[slow path][core_xframe_macid]:	netif_drop_thd:%d\n", (NR_XMITFRAME/(adapter->stapriv.asoc_sta_count-1)));
		_rtw_spinlock_bh(&pstapriv->asoc_list_lock);
		plist = get_next(phead);
		while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
			psta = LIST_CONTAINOR(plist, struct sta_info, asoc_list);
			plist = get_next(plist);
			RTW_PRINT(" macid[%d] core_xframe_total_cnt: %d (abort:%d)\n",
	...
	if(pxmitpriv->txsc_enable){
		RTW_PRINT("[txsc][core_txsc_apply_macid]:	netif_drop_thd:%d\n", (adapter->max_tx_ring_cnt/(adapter->stapriv.asoc_sta_count-1)));
		RTW_PRINT(" macid[x] txsc_apply - (no_txreq[abort6] + fc abort + fail)	(amsdu need_enq/need_deq/abort[abort5]/amsdu to/amsdu deq) \n");
		_rtw_spinlock_bh(&pstapriv->asoc_list_lock);
		plist = get_next(phead);
		while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
			psta = LIST_CONTAINOR(plist, struct sta_info, asoc_list);
			plist = get_next(plist);
			RTW_PRINT(" macid[%d] core_txsc_apply_cnt: (%d + %d) - (%d + %d + %d)  (amsdu need_enq/need_deq/abort/to/deq: %d, %d, %d, %d, %d)\n",
	...
	RTW_PRINT("======= [add into phl tx tring] =======:\n");
	_rtw_spinlock_bh(&pstapriv->asoc_list_lock);
	plist = get_next(phead);
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		psta = LIST_CONTAINOR(plist, struct sta_info, asoc_list);
		plist = get_next(plist);
		RTW_PRINT(" macid[%d] core xframe ok/fail: %d, %d\n",
	...
	RTW_PRINT("======= [enquque WD pending] =======:\n");
	_rtw_spinlock_bh(&pstapriv->asoc_list_lock);
	plist = get_next(phead);
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		psta = LIST_CONTAINOR(plist, struct sta_info, asoc_list);
		plist = get_next(plist);
		RTW_PRINT(" macid[%d] first_deq_cnt: %d, enquque ok/fail: %d, %d\n",
	...
	RTW_PRINT(" update_txbd_ok: %d\n", phl_com->update_txbd_ok);
	RTW_PRINT(" update_txbd_fail: %d\n", phl_com->update_txbd_fail);
	RTW_PRINT(" trigger_txstart_ok: %d\n", phl_com->trigger_txstart_ok);
	RTW_PRINT(" trigger_txstart_fail: %d\n", phl_com->trigger_txstart_fail);
	RTW_PRINT("\n");
	...
	phl_get_wd_ring_wd_page_cnt(phl, RTW_DBGDUMP);
	RTW_PRINT("\n");
...
#endif
	/* FS/LS debug */
	RTW_PRINT("[RX PACKET DROP]: \n");
	RTW_PRINT("	RX FS/LS drop cnt: %d\n", adapter->FS_LS_cnt);
}
...
void core_cmd_add_latency_sta_by_macid(_adapter *adapter, void *cmd_para, u32 para_num)
{
	...
	//if(para_num<=1){
	//	DBGP("please enter cmd : add_latency_sta_2,<macid>,<0/1>\n");
	//	return;
	//}
...
void core_cmd_ru_cfg(_adapter *adapter, void *cmd_para, u32 para_num){
	...
	if(!strcmp(para, "4.29.1_5G")){
		...
		dl_ru_fix_grp->sta_info[6].mcs = 11;
		dl_ru_fix_grp->sta_info[7].mcs = 11;
	}else if(!strcmp(para, "4.44.1_1")){
		DBGP("cfg 4.44.1_1 APUT \n");
		...
		dl_ru_fix_grp->sta_info[0].mcs = 4;
		dl_ru_fix_grp->sta_info[1].mcs = 4;
		dl_ru_fix_grp->sta_info[2].mcs = 4;
		dl_ru_fix_grp->sta_info[3].mcs = 4;
		dl_ru_fix_grp->sta_info[4].mcs = 4;
		dl_ru_fix_grp->sta_info[5].mcs = 4;
		dl_ru_fix_grp->sta_info[6].mcs = 4;
		dl_ru_fix_grp->sta_info[7].mcs = 4;
	}else if(!strcmp(para, "4.44.1_2")){
		DBGP("cfg 4.44.1_2 APUT \n");
		...
		dl_ru_fix_grp->sta_info[6].mcs = 4;
		dl_ru_fix_grp->sta_info[7].mcs = 4;
	}else if(!strcmp(para, "4.44.1_3")){
		DBGP("cfg 4.44.1_3 APUT \n");
		dl_ru_fix_grp->gi_ltf = RTW_GILTF_LGI_4XHE32;
...
void vw_latency_set(_adapter *adapter)
{
	...
	#ifdef CONFIG_RTL_WIFI_OFFLOAD
	if (adapter->dvobj->wfo_en) {
		adapter->no_wdinfo = 0;
		adapter->no_rts = 1;
		rtl_wfo_ipc_set_mib(adapter, RTL_WFO_PEID_ALL, "log", 0);
		rtl_wfo_upd_all_sta_info(adapter, _TRUE);
		rtl_wfo_ipc_set_mib(adapter, RTL_WFO_PEID_ALL, "amsdu_to", 4);
	} else
		adapter->no_wdinfo = 1;
	#else
	adapter->no_wdinfo = 1;
	#endif /* CONFIG_RTL_WIFI_OFFLOAD */
	rtw_phl_write32(adapter->dvobj->phl, 0x9E14, 0xF0000801);
...
u8 high_queue_debug_enable = 0;
u8 high_queue_index = 0;
...
void core_cmd_debug(_adapter *adapter, void *cmd_para, u32 para_num)
{
	...
	if (!strcmp(para, "mdata"))
		...
		sscanf(para, "%d", &value);
		adapter->swdbg = value;
	}else if (!strcmp(para, "hq_debug_enable")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		high_queue_debug_enable = value;
	}else if (!strcmp(para, "hq_debug_index")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		high_queue_index = value;
	}else if (!strcmp(para, "hq_debug_dump")) {
		printk("hq_debug_enable:%d, high_queue_index: %d\n", high_queue_debug_enable, high_queue_index);
	...
	} else if(!strcmp(para, "phl_trx")) {
		u32 value2 = 0;
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		para = get_next_para_str(para);
		sscanf(para, "%d", &value2);
		printk("[phl_trx]trx = %d, pause = %d\n", value, value2);
		rtw_phl_set_trx_status(adapter->dvobj->phl, value, value2);
	...
	else if (!strcmp(para, "rttp")) {//RT_TRX_TRAFFIC
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		RTW_PRINT("set timeout:%dms\n", value);
		realtime_traffic_set_timer(adapter, value);
	}
#ifdef CONFIG_VW_REFINE
		...
		if (!adapter->vw_enable) {
		...
		} else if (VW_MODE_TP == adapter->vw_enable) {
			...
			// adapter->no_wdinfo = 1;
		} else if (VW_MODE_LATENCY == adapter->vw_enable) {
			vw_latency_set(adapter);
		}

		if (VW_MODE_DBG1 == adapter->vw_enable) {
		    adapter->vw_enable = VW_MODE_TP;
#ifdef CONFIG_DYNAMIC_THROUGHPUT_ENGINE
			primary_adapter->registrypriv.manual_edca = 1;
#endif
		...
		if (VW_MODE_DBG2 == adapter->vw_enable) {
		    rtw_phl_write32(adapter->dvobj->phl, 0x9E14, 0x0808);
		    //no_ra = 0;
		}
		...
		RTW_PRINT("is_map:%d (0:PHL 1:CORE)\n", adapter->is_map);
	}
	...
	else if (!strcmp(para, "cmcc_ld_sta_mac")) {
		para = get_next_para_str(para);
		sscanf(para, "%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx", 
			&adapter->cmcc_ld_sta_mac[0], &adapter->cmcc_ld_sta_mac[1], &adapter->cmcc_ld_sta_mac[2],
			&adapter->cmcc_ld_sta_mac[3], &adapter->cmcc_ld_sta_mac[4], &adapter->cmcc_ld_sta_mac[5]);
	}
...
void core_cmd_hfc(_adapter *adapter, void *cmd_para, u32 para_num)
{
	...
	u32 hfc_ctrl_a[3] = {0x8a10, 0x8a90, 0x8a94};
	u32 hfc_info_a[3] = {0x8a50, 0x8a8c, 0x8a98};
	u32 hfc_ctrl_c[3] = {0x1710, 0x1790, 0x1794};
	u32 hfc_info_c[3] = {0x1750, 0x178c, 0x1798};
	u32 *hfc_ctrl, *hfc_info;

	if(para_num<=0)
		return;

	if (adapter->dvobj->ic_id == RTL8852A) {
		hfc_ctrl = (u32 *)hfc_ctrl_a;
		hfc_info = (u32 *)hfc_info_a;
	} else if ((adapter->dvobj->ic_id == RTL8852C) ||
				(adapter->dvobj->ic_id == RTL8192XB) ||
				(adapter->dvobj->ic_id == RTL8832BR)) {
		hfc_ctrl = (u32 *)hfc_ctrl_c;
		hfc_info = (u32 *)hfc_info_c;
	} else {
		printk("unsupported ic\n");
		return;
	}

	if(strchr(para, '0') || strchr(para, 'a')) {
		ctrl_addr = hfc_ctrl[0];
		info_addr = hfc_info[0];
	...
	if(strchr(para, '1') || strchr(para, 'a')) {
		ctrl_addr = hfc_ctrl[0] + 0x10;
		info_addr = hfc_info[0] + 0x10;
	...
	if(strchr(para, '2') || strchr(para, 'a')) {
		ctrl_addr = hfc_ctrl[0] + 0x20;
		info_addr = hfc_info[0] + 0x20;
	...
	if(strchr(para, 'p') || strchr(para, 'a')) {
		ctrl = rtw_phl_read32(phl, hfc_ctrl[1]);
		info = rtw_phl_read32(phl, hfc_info[1]);
		info2 = rtw_phl_read32(phl, hfc_info[2]);
		ctrl2 = rtw_phl_read32(phl, hfc_ctrl[2]);
...
void core_cmd_amsdu(_adapter *adapter, void *cmd_para, u32 para_num)
{
	...
	if (!strcmp(para, "enable")) {
		...
		if (pxmitpriv->txsc_amsdu_enable)
			rtw_phl_write32(GET_HAL_INFO(dvobj), 0x9b00, 0);
		else
			rtw_phl_write32(GET_HAL_INFO(dvobj), 0x9b00, 0x7);
		#endif
	...
	} else if (!strcmp(para, "use_hw_timer")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		dvobj->txsc_amsdu_timeout_use_hw_timer = value;
		RTW_PRINT("[amsdu] set txsc_amsdu_timeout_use_hw_timer = %d\n", value);
	} else if(!strcmp(para, "timeout")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &i);
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);
		if (value >= HW_TIMER_MIN && i >= 0 && i < N_TIMEOUT) {
			dvobj->txsc_amsdu_timeout[i] = value;
			RTW_PRINT("[amsdu] set txsc_amsdu_timeout[%d] = %d(us)\n", i, value);
		} else {
			RTW_PRINT("[amsdu] INVALID!!!\n");
		}
	}
}
...
void core_cmd_lmt_txreq(_adapter *adapter, void *cmd_para, u32 para_num)
{
	...
	if (!strcmp(para, "dump")) {
		...
		u32 enable;
		para = get_next_para_str(para);
		sscanf(para, "%d", &enable);
		adapter->lmt_txreq_enable = enable;
		printk("lmt_txreq_enable = %d\n", enable);
	...
	} else if (!strcmp(para, "lmt_sta_bcast")) {
		u16 limit;
		para = get_next_para_str(para);
		sscanf(para, "%hu", &limit);
		adapter->dvobj->lmt_sta_bcast_txreq = limit;
		printk("limit pending bcast txreq to %d for each STA\n", limit);
	} else if (!strcmp(para, "info")) {
		printk("lmt_txreq_enable: %d\n", adapter->lmt_txreq_enable);
		printk("lmt_sta_bcast_txreq: %hu\n", adapter->dvobj->lmt_sta_bcast_txreq);
	}
}
#endif
...
void core_cmd_twt(_adapter *adapter, void *cmd_para, u32 para_num)
{
	...
	} else if(!strcmp(para, "entry")) {
		struct sta_info *psta = NULL;
		para=get_next_para_str(para);
		sscanf(para, "%d", &value);
		psta = rtw_get_stainfo_by_macid(&adapter->stapriv, (u16)value);
		rtw_core_twt_dump_local_setup_info(adapter, psta);
	} else if(!strcmp(para, "debug")) {
		para=get_next_para_str(para);
		sscanf(para, "%d", &value);
		DBGP("twt_debug = %d\n", value);
		adapter->twt_debug = value;
...
#ifdef CONFIG_AUTOTEST_DETECT
void core_cmd_autotest(_adapter *adapter, void *cmd_para, u32 para_num)
{
	char *para = (char *)cmd_para;
	u32 value = 0;
	int i;
	...
	if (!strcmp(para, "dump")) {
		printk("case: %d (%s)\n", adapter->dvobj->autotest_case,
			_autotest_dump_casename(adapter->dvobj->autotest_case));
		printk("count down: %d\n", adapter->dvobj->autotest_case_cntdown);
		printk("force_case: %d\n", adapter->dvobj->autotest_force_case);
		printk("txreq_lmt: %d\n", adapter->dvobj->autotest_txreq_lmt);
		printk("case_list:\n");
		for (i = 0; i <= AUTOTEST_MAX; i++)
			printk(" %2d - %s\n", i, _autotest_dump_casename(i));
	} else if (!strcmp(para, "force")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);

		adapter->dvobj->autotest_force_case = value;
	} else if (!strcmp(para, "lmt")) {
		para = get_next_para_str(para);
		sscanf(para, "%d", &value);

		adapter->dvobj->autotest_txreq_lmt = value;
	}
}
#endif

#ifdef CONFIG_RTL_WIFI_OFFLOAD
void core_dump_phl_entry(_adapter *adapter, u32 value)
{
	struct rx_logs *rx_logs = &adapter->rx_logs;
	struct xmit_priv *pxmitpriv = &adapter->xmitpriv;

	rtw_phl_dump_entry(adapter->dvobj->phl, value);

	if (2 == value) {
		PHL_PRINT("CORE RX: os_ind %u core_rx %u rx_err %u rx_net %u\n",
			rx_logs->os_indicate, rx_logs->core_rx,
			rx_logs->core_rx_err_stop, rx_logs->os_netif_ok);

		PHL_PRINT("\t os_mgmt %u core_ctrl %u rx_data = %u\n",
			adapter->rx_logs.core_rx_pre_mgmt,
			adapter->rx_logs.core_rx_pre_ctrl,
			adapter->rx_logs.core_rx_pre_data);

		PHL_PRINT("\t free_xframe_ext_cnt %u free_xmitframe_cnt %u\n",
			pxmitpriv->free_xframe_ext_cnt,
			pxmitpriv->free_xmitframe_cnt);
		PHL_PRINT("++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	}
...
void wfo_cmd_setmib(_adapter *adapter, void *cmd_para, u32 para_num)
{
	char *para = (char *)cmd_para, *name;
	int pe_id, value;

	para = get_next_para_str(para);
	name = para;
	para = get_next_para_str(para);
	if (para[0]=='0' && (para[1]== 'x' || para[2]== 'X'))
		sscanf(para, "%x", &value);
	else
		sscanf(para, "%d", &value);
	para = get_next_para_str(para);
	if (kstrtoint(para, 0, &pe_id))
		pe_id = MAX_PE_NUM;

	rtl_wfo_ipc_set_mib(adapter, pe_id, name, value);
}

void wfo_cmd_getmib(_adapter *adapter, void *cmd_para, u32 para_num)
{
	char *para = (char *)cmd_para, *name;
	...
	rtl_wfo_ipc_get_mib(adapter, MAX_PE_NUM, name);
}

void wfo_cmd_log_level(_adapter *adapter, void *cmd_para, u32 para_num)
{
	char *para = (char *)cmd_para;
	int pe_id, level;
	...
	rtl_wfo_ipc_set_mib(adapter, pe_id, "log", level);
}

void wfo_cmd_log_comp(_adapter *adapter, void *cmd_para, u32 para_num)
{
	char *para = (char *)cmd_para;
	int pe_id, value;

	para = get_next_para_str(para);
	kstrtoint(para, 0, &value);
	para = get_next_para_str(para);
	if (kstrtoint(para, 0, &pe_id))
		pe_id = MAX_PE_NUM;

	rtl_wfo_ipc_set_mib(adapter, pe_id, "log_comp", (s32)value);
}

void wfo_cmd_monitor(_adapter *adapter, void *cmd_para, u32 para_num)
{
	char *para = (char *)cmd_para;
	int pe_id, interval;

	para = get_next_para_str(para);
	sscanf(para, "%d", &interval);
	para = get_next_para_str(para);
	if (kstrtoint(para, 0, &pe_id))
		pe_id = MAX_PE_NUM;

	rtl_wfo_ipc_set_mib(adapter, pe_id, "mon", interval);
}


void wfo_cmd_rps_pe_id(_adapter *adapter, void *cmd_para, u32 para_num)
{
	char *para = (char *)cmd_para;
	int macid, rx_cpuid;

	para = get_next_para_str(para);
	sscanf(para, "%d", &rx_cpuid);
	if (rx_cpuid >= RTL_WFO_PEID_MAX) {
		RTW_PRINT("unknown rx_cpuid %d\n", rx_cpuid);
		return;
	}

	para = get_next_para_str(para);
	if (kstrtoint(para, 0, &macid))
		macid = U16_MAX;

	rtl_wfo_upd_sta_info(adapter, macid, rx_cpuid, U8_MAX, U16_MAX, _FALSE);
}

void wfo_cmd_fc_sta_cfg(_adapter *adapter, void *cmd_para, u32 para_num)
{
	char *para = (char *)cmd_para;
	int macid, to_amsdu;

	para = get_next_para_str(para);
	sscanf(para, "%d", &to_amsdu);
	para = get_next_para_str(para);
	if (kstrtoint(para, 0, &macid))
		macid = U16_MAX;

	rtl_wfo_upd_sta_info(adapter, macid, U8_MAX, to_amsdu, U16_MAX, _FALSE);
}

void wfo_cmd_wpseq_lmt_sta_cfg(_adapter *adapter, void *cmd_para, u32 para_num)
{
	_list *plist, *phead;
	struct sta_priv *pstapriv = &(adapter->stapriv);
	struct sta_info *psta;
	...
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		psta = LIST_CONTAINOR(plist, struct sta_info, asoc_list);
		plist = get_next(plist);
		if (psta && (macid == psta->phl_sta->macid)) {
			psta->wpseq_lmt = wpseq_lmt;
			rtl_wfo_ipc_cfg_sta_info(adapter, psta,
				PE_ROLE_DOE, RTL_WFO_OP_UPDATE);
			break;
		}
	}
	_rtw_spinunlock_bh(&pstapriv->asoc_list_lock);
...
void core_cmd_wfo(_adapter *adapter, void *cmd_para, u32 para_num)
{
	void *phl = adapter->dvobj->phl;
	char *para = (char *)cmd_para;
	int pe_id, i;
	...
	rtl_wfo_dev_t *wfo_dev = dvobj->wfo_dev;

	if (!RTL_WFO_IS_OFLD(dvobj)) {
		RTW_PRINT("not supported band.\n");
		return;
	}

	if (!strcmp(para, "start")) {
		/* wfo start - start DOE/PE and update all STA info */
		for (i = 0; i < BAND_CAP_BIT_NUM; i++)
			if ((wfo_priv->wfo_band_cap >> i) & BIT0)
				wfo_priv->dev[i].dvobj->wfo_pe_stopped = 0;

		dvobj->wfo_pe_stopped = 0;
		rtl_wfo_start_pe(adapter);
		rtl_wfo_upd_all_sta_info(adapter, _FALSE);
	} else if (!strcmp(para, "stop")) {
		/* wfo stop - use hb_timer_hdl to restart interface w/o starting PE*/
		for (i = 0; i < BAND_CAP_BIT_NUM; i++)
			if ((wfo_priv->wfo_band_cap >> i) & BIT0)
				wfo_priv->dev[i].dvobj->wfo_trg_stop_pe = 1;
	} else if (!strcmp(para, "setmib") || !strcmp(para, "sm")) {
		/* wfo setmib name value [peid] */
		wfo_cmd_setmib(adapter, cmd_para, para_num);
	} else if (!strcmp(para, "getmib") || !strcmp(para, "gm")) {
		/* wfo getmib name */
		wfo_cmd_getmib(adapter, cmd_para, para_num);
	} else if (!strcmp(para, "log")) {
		/* wfo log level [pe_id] - set log level */
		wfo_cmd_log_level(adapter, cmd_para, para_num);
	} else if (!strcmp(para, "log_comp")) {
		/* wfo log_comp value [peid] - set log trace components */
		wfo_cmd_log_comp(adapter, cmd_para, para_num);
	} else if (!strcmp(para, "mon")) {
		/* wfo mon interval [pe_id] - like rssi dump */
		wfo_cmd_monitor(adapter, cmd_para, para_num);
	} else if (!strcmp(para, "status")) {
		/* wfo status,[reset] */
		/* reset 1: clear basic counter, 2: reset basic & error counter */
		int reset = 0;
		...
		for (pe_id = 0; pe_id < MAX_PE_NUM; pe_id++)
			rtl_wfo_ipc_get_status(adapter, pe_id, reset);
	} else if (!strcmp(para, "wmm")) {
		/* wfo wmm,[reset] */
		int reset = 0;
		...
		/* get wmm status of DOE */
		rtl_wfo_ipc_get_wmm_status(adapter, reset);
	} else if (!strcmp(para, "debug")) {
		/* wfo debug */
		core_dump_phl_entry(adapter, 2);
	} else if (!strcmp(para, "res")) {
		/* wfo res - allocate resource to DOE/PE */
		if (wfo_priv->res_state == PE_RES_NOT_ALLOC) {
			RTW_PRINT("buffer resource is not allocated!\n");
			return;
		}

		for (i = 0; i < wfo_priv->res_num; i++) {
			rtl_wfo_ipc_set_resource(adapter, wfo_priv->resource[i].res_pe_id,
					&wfo_priv->resource[i]);
		}
	} else if (!strcmp(para, "resadd")) {
		/* wfo resadd,cat,order - manually allocate resource to DOE/PE */
		/* 		cat: 32-NIRX, 64-AMSDU_NIRX, order: 8-1M, 9-2M, 10-4M */
		int cat, order;
		...
		if (order > 10 || order < 8) /* 4M / 2M / 1M */
			order = 10;
		RTW_PRINT("add resource cat %d order %d\n", cat, order);

#ifdef FBM_USE_KMALLOC_BUF
		if (cat != RTL_WFO_RESOURCE_NIRX_BUF &&
			cat != RTL_WFO_RESOURCE_AMSDU_NIRX_BUF) {
			RTW_PRINT("cat %d is not suported!\n", cat);
			return;
		} else {
			rtl_wfo_res_t stage = {
				.sta_num = ADD_BUFINFO,
			};
			stage.buf_num = (1 << (order + PAGE_SHIFT))/
				((cat == RTL_WFO_RESOURCE_NIRX_BUF) ? \
				PE_NIRX_BUF_SZ : AMSDU_NIRX_BUF_SZ);
			rtl_wfo_add_res(adapter->dvobj,
				((cat == RTL_WFO_RESOURCE_NIRX_BUF) ? \
				RESOURCE_NIRX : RESOURCE_AMSDU_NIRX), &stage);
		}
#else /* !FBM_USE_KMALLOC_BUF  */
		rtl_wfo_add_res(adapter->dvobj, cat, order);
#endif /* FBM_USE_KMALLOC_BUF */
	} else if (!strcmp(para, "adap")) {
		/* wfo adap - update device and adapter info to DOE/PE */
		if (is_primary_adapter(adapter))
			rtl_wfo_ipc_send_device_info(adapter);
		rtl_wfo_ipc_cfg_adapter_info(adapter, RTL_WFO_OP_ADD);
	} else if (!strcmp(para, "usta")) {
		/* wfo usta - update all sta info to DOE/PE */
		rtl_wfo_upd_all_sta_info(adapter, _FALSE);
	} else if (!strcmp(para, "resd")) {
		/* wfo resd -- simple dump */
		/* wfo resd,buffer_category (bitmap) -- detailed dump */
		rtl_wfo_pe_resource_t resource = {0, 0, 0, RTL_WFO_RESOURCE_BUF_DUMP, 0, 0, 0, 0};
		int verbose = -1;

		para = get_next_para_str(para);
		sscanf(para, "%d", &verbose);
		if (verbose >= 0) {
			resource.res_type = RTL_WFO_RESOURCE_BUF_DUMP_VERBOSE | (1 << verbose);
		}

		for (pe_id = 0; pe_id < MAX_PE_NUM; pe_id++)
			rtl_wfo_ipc_set_resource(adapter, pe_id, &resource);

		PHL_PRINT("Resource: %d / %d\n", wfo_priv->res_num, RTL_WFO_RES_NUM);
		PHL_PRINT("    NIRX: %d / %d (next: %d)\n", wfo_priv->res_nirx_num,
					PE_NIRX_RES_NUM, wfo_priv->next_nirx_th);
		PHL_PRINT("   AMSDU: %d / %d (next: %d)\n", wfo_priv->res_amsdu_num,
					AMSDU_NIRX_RES_NUM, wfo_priv->next_amsdu_th);
		for (i = 0; i < wfo_priv->res_num; i++) {
			PHL_PRINT("[%2d] type 0x%08x size %4d KB buf sz %d num %d\n",
					i, wfo_priv->resource[i].res_type,
					wfo_priv->resource[i].res_size/1024,
					wfo_priv->resource[i].buf_size,
					wfo_priv->resource[i].buf_num);
		}
	} else if (!strcmp(para, "skbd")) {
		/* wfo skbd -- simple dump */
		/* wfo skbd,rtl_wfo_skbuf_class (bitmap) -- detailed dump */
		rtl_wfo_pe_resource_t resource = {0, 0, 0, RTL_WFO_RESOURCE_SKB_DUMP, 0, 0, 0, 0};
		int verbose = -1;

		para = get_next_para_str(para);
		sscanf(para, "%d", &verbose);
		if (verbose >= 0) {
			resource.res_type = RTL_WFO_RESOURCE_SKB_DUMP_VERBOSE | (1 << verbose);
		}

		for (pe_id = 0; pe_id < MAX_PE_NUM; pe_id++)
			rtl_wfo_ipc_set_resource(adapter, pe_id, &resource);
	} else if (!strcmp(para, "mem")) {
		/* wfo mem,pe_id,order - attach resource to DOE/PE's memory pool */
		/* pe_id: 1-PE0, 2-DOE */
		/* order: 0-4K, 1-8K, 2-16K, ..., 10-4M */
		int order;
		struct page *pages = NULL;
		rtl_wfo_pe_resource_t resource = {0, 0, 0, RTL_WFO_RESOURCE_MEMPOOL, 0, 0, 0, 0};
		u32 phy_addr_h = 0;
		...
		if (!pages) {
			RTW_ERR("alloc order %d failed\n", order);
			return;
		}
		resource.res_addr = __pa(page_address(pages));
		...
		/* wfo memd - dump memory pool info*/
		rtl_wfo_pe_resource_t resource = {0, 0, 0, RTL_WFO_RESOURCE_MEMPOOL_DUMP, 0, 0, 0, 0};

		for (pe_id = 0; pe_id < MAX_PE_NUM; pe_id++)
			rtl_wfo_ipc_set_resource(adapter, pe_id, &resource);
	} else if (!strcmp(para, "dump")) {
		/* for debug only !! */
		/* wfo dump,cpu_id,num,phy addr - dump DMEM/DRAM address */
		u32 addr, num, *ptr = NULL, addr_h = 0;
		u8 *u8_ptr;
		dma_addr_t dma_handle;
		struct pci_dev *pdev = dvobj_to_pci(adapter->dvobj)->ppcidev;
		...
		/* Request PE to dump the memory it sees.*/
		if (pe_id < RTL_WFO_ARM_ID) {
			rtl_wfo_ipc_dump_mem(adapter, pe_id, addr, num);
		} else {
			if (addr < 0x20000000) {
				ptr = __va(addr);
				...
				RTW_PRINT("DMEM addr 0x%08x size %d (ioremap %px)\n", addr, num, ptr);
			} else {
				RTW_PRINT("unknown address\n");
				return;
			}

			for (i = 0; i < num; i++) {
				if (i % 4 == 0)
					_RTW_PRINT("%08x -", (addr + (i * 4)));
				u8_ptr = (u8 *)&ptr[i];
				_RTW_PRINT(" %02x %02x %02x %02x ",
    ...
    } else if (!strcmp(para, "write") || !strcmp(para, "read")) {
		/* for debug only !! */
		/* wfo write,phy_addr,value */
		/* wfo read,phy_addr */
		u32 addr, val, *ptr = NULL;
		dma_addr_t dma_reg;
		struct pci_dev *pdev = dvobj_to_pci(adapter->dvobj)->ppcidev;
		int op = 0; /* 1: write, 2: read */
		...
		if (addr < 0x20000000) { /* DRAM address */
			...
			ptr = ioremap(addr, 4);
			RTW_PRINT("DMEM addr %08x (ioremap %px)\n", addr, ptr);
		} else {
			RTW_ERR("unknown address\n");
			return;
		}
		...
		rtl_wfo_ipc_cfg_sta_info(adapter, &macid,
				PE_ROLE_ALL, RTL_WFO_OP_DUMP);
	} else if (!strcmp(para, "pfs")) {
		/* wfo pfs[,pe_id] - start CPU profiling */
		para = get_next_para_str(para);
		if (kstrtoint(para, 0, &pe_id))
			pe_id = MAX_PE_NUM;

		rtl_wfo_ipc_start_profiling(adapter, pe_id);
	} else if (!strcmp(para, "pfd")) {
		/* wfo pfd[,pe_id] - dump CPU profiling info */
		para = get_next_para_str(para);
		if (kstrtoint(para, 0, &pe_id))
			pe_id = MAX_PE_NUM;

		rtl_wfo_ipc_stop_profiling(adapter, pe_id);
	} else if (!strcmp(para, "doed")) {
		/* wfo doed,ch_idx - dump DOE CH info */
		int ch_idx;

		para = get_next_para_str(para);
		sscanf(para, "%d", &ch_idx);

		rtl_wfo_ipc_dump_doe_chan(adapter, RTL_WFO_DOE_ID, ch_idx);
	} else if (!strcmp(para, "intf")) {
		/* phl wfo,intf */
		rtl_wfo_ipc_cfg_adapter_info(adapter, RTL_WFO_OP_DUMP);
	} else if (!strcmp(para, "peid") || !strcmp(para, "rps")) {
		/* wfo peid|rps rps_cpuid macid - set CPU ID for one STA */
		wfo_cmd_rps_pe_id(adapter, cmd_para, para_num);
	} else if (!strcmp(para, "fc")) {
		/* wfo fc to_amsdu macid - add/delete FC wifi AMSDU Ofld entry */
		wfo_cmd_fc_sta_cfg(adapter, cmd_para, para_num);
	} else if (!strcmp(para, "attr")) {
		/* wfo attr,<iface_id>,rpt[,<val>] - get/set FC client_mode attribute */
		/* wfo attr,<iface_id>,blk[,<val>] - get/set FC block_relay attribute */
		/* wfo attr,<iface_id>,mc2u,<val>  - set FC mc2uc_en attribute */
		_adapter *padptr = NULL;
		rtk_fc_wifi_dev_attr_t attr;
		char fc_attr[16];
		int fc_val = 0, iface_id;

		if (para_num < 3) {
			RTW_PRINT("Usage:\n");
			RTW_PRINT("    wfo attr,<iface_id>,rtp[,<val>]\n");
			RTW_PRINT("    wfo attr,<iface_id>,blk[,<val>]\n");
			RTW_PRINT("    wfo attr,<iface_id>,mc2u,<val>\n");
		} else {
			/* iface_id */
			para = get_next_para_str(para);
			sscanf(para, "%d", &iface_id);
			/* attribute name */
			para = get_next_para_str(para);
			sscanf(para, "%15s", fc_attr);
			/* set value */
			if (para_num > 3) {
				para = get_next_para_str(para);
				sscanf(para, "%d", &fc_val);
			}

			if (iface_id >= CONFIG_IFACE_NUMBER)
				iface_id = 0;
			padptr = GET_ADAPTER(adapter, iface_id);
			if ((para_num >= 3) && padptr) {
				if (!strcmp(fc_attr, "rpt")) {
					rtk_fc_wifi_dev_attr_get(padptr->pnetdev, &attr);
					if (para_num > 3) { /* set */
						attr.client_mode = !!fc_val;
						rtk_fc_wifi_dev_attr_set(padptr->pnetdev, attr);
						RTW_PRINT("set %s clnt_mode=%d\n", padptr->old_ifname, attr.client_mode);
					} else { /* get */
						RTW_PRINT("get %s clnt_mode=%d\n", padptr->old_ifname, attr.client_mode);
					}
				} else if (!strcmp(fc_attr, "blk")) {
					rtk_fc_wifi_dev_attr_get(padptr->pnetdev, &attr);
					if (para_num > 3) { /* set */
						attr.block_relay = !!fc_val;
						rtk_fc_wifi_dev_attr_set(padptr->pnetdev, attr);
						RTW_PRINT("set %s block_relay=%d\n", padptr->old_ifname, attr.block_relay);
					} else { /* get */
						RTW_PRINT("get %s block_relay=%d\n", padptr->old_ifname, attr.block_relay);
					}
				} else if (!strcmp(fc_attr, "mc2u")) {
					rtl_wfo_fc_mc2uc_enable(padptr->pnetdev, fc_val);
					RTW_PRINT("set FC mc2uc_en=%d\n", fc_val);
				} else {
					RTW_PRINT("unknown attribute name (%s)\n", fc_attr);
				}
			}
		}
	} else if (!strcmp(para, "map")) {
		/* wfo map - show DOE/PE memory layout */
		for (pe_id = 0; pe_id < MAX_PE_NUM; pe_id++)
			rtl_wfo_ipc_get_memory_map(adapter, pe_id);
	} else if (!strcmp(para, "uart")) {
		/* wfo uart,pe_id - change uart output */
		/* pe_id 0-PE0, 1-PE1, 2-DOE, others-disable */
		int uart_sel;

		para = get_next_para_str(para);
		sscanf(para, "%d", &uart_sel);

		rtl_wfo_ipc_set_uart_selection(adapter, uart_sel);
		if (uart_sel < RTL_WFO_ARM_ID)
			RTW_PRINT("set UART1 to CPU_RCPU%d\n", uart_sel);
		else
			RTW_PRINT("set UART1 disabled\n");
	} else if (!strcmp(para, "isr")) {
		/* wfo isr index mask dflt */
		/* ignore SER: wfo isr 2 00400000 00400000 */
		struct phl_info_t *phl_info = (struct phl_info_t *)(dvobj->phl);
		struct hal_info_t *hal_info = (struct hal_info_t *)phl_info->hal;
		struct rtw_hal_com_t *hal_com = hal_info->hal_com;
		int index = 2, mask = 0x00600000, dflt = 0;
		...
		if (para_num > 1) {
			...
			para = get_next_para_str(para);
			sscanf(para, "%x", &mask);
			_RTW_PRINT("set intr[%d] mask 0x%08x\n", index, mask);
			hal_com->_intr[index].mask = mask;
			if (para_num > 3) {
				sscanf(para, "%x", &dflt);
				_RTW_PRINT("set intr[%d] dflt 0x%08x\n", index, dflt);
				hal_com->_intr[index].mask_dflt = dflt;
			}
		}

		_RTW_PRINT("ind val %08x %08x %08x %08x\n",
			hal_com->_intr_ind[0].val, hal_com->_intr_ind[1].val, hal_com->_intr_ind[2].val, hal_com->_intr_ind[3].val);
		_RTW_PRINT("	msk %08x %08x %08x %08x\n",
			hal_com->_intr_ind[0].mask, hal_com->_intr_ind[1].mask, hal_com->_intr_ind[2].mask, hal_com->_intr_ind[3].mask);
		_RTW_PRINT("	dft %08x %08x %08x %08x\n",
			hal_com->_intr_ind[0].mask_dflt, hal_com->_intr_ind[1].mask_dflt, hal_com->_intr_ind[2].mask_dflt, hal_com->_intr_ind[3].mask_dflt);

		_RTW_PRINT("intr val %08x %08x %08x %08x\n",
			hal_com->_intr[0].val, hal_com->_intr[1].val, hal_com->_intr[2].val, hal_com->_intr[3].val);
		_RTW_PRINT("	 msk %08x %08x %08x %08x\n",
			hal_com->_intr[0].mask, hal_com->_intr[1].mask, hal_com->_intr[2].mask, hal_com->_intr[3].mask);
		_RTW_PRINT("	 dft %08x %08x %08x %08x\n",
			hal_com->_intr[0].mask_dflt, hal_com->_intr[1].mask_dflt, hal_com->_intr[2].mask_dflt, hal_com->_intr[3].mask_dflt);
#ifdef CONFIG_RTW_A4_STA
	} else if (!strcmp(para, "a4_dump")) {
		/* wfo a4_dump */
		for (pe_id = 0; pe_id < WFO_PPE_NUM; pe_id++)
			rtl_wfo_ipc_get_a4_info(adapter,
				RTL_WFO_PPE_ID + pe_id, RTL_WFO_OP_DUMP);
#endif
	} else if (!strcmp(para, "nat25")) {
		/* wfo nat25,iface_id - dump NAT2.5 table */
		int j, iface_id = 0;
		_adapter *padptr;
		...
		_rtw_spinlock_bh(&padptr->br_ext_lock);
		for (i = 0, j = 0; i < NAT25_HASH_SIZE; i++) {
			db = padptr->nethash[i];
			while (db != NULL) {
				RTW_PRINT("NAT25: DB(%02d) H(%02d) C(%04d) AGE(%d) MAC: "MAC_FMT" "
						"NET: %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n",
						 j, i, atomic_read(&db->use_count),
						 jiffies_to_msecs((long)jiffies - (long)db->ageing_timer),
						 MAC_ARG(db->macAddr),
						 db->networkAddr[0], db->networkAddr[1], db->networkAddr[2],
						 db->networkAddr[3], db->networkAddr[4], db->networkAddr[5],
						 db->networkAddr[6], db->networkAddr[7], db->networkAddr[8],
						 db->networkAddr[9], db->networkAddr[10], db->networkAddr[11],
						 db->networkAddr[12], db->networkAddr[13], db->networkAddr[14],
						 db->networkAddr[15], db->networkAddr[16]);
				j++;
				db = db->next_hash;
			}
		}
		_rtw_spinunlock_bh(&padptr->br_ext_lock);
	} else if (!strcmp(para, "pid")) {
		/* wfo pid - dump PI info */
		int j, qnum;
		rtl_wfo_pe_info_t *pe_info;
		u32 *prw, *pbd;
		u32 bd_addr, data_addr;
		struct phl_info_t *phl_info = (struct phl_info_t *)phl;
		struct hci_info_t *hci_info = (struct hci_info_t *)phl_info->hci;
		struct rtw_wd_page_ring *wd_ring =
							(struct rtw_wd_page_ring *)hci_info->wd_ring;
		pi_info_t *pi_info;
		pi_addr_t *pi_addr;

		if (adapter->dvobj->wfo_en) {
			/* PI Ring info */
			pi_info = wfo_dev->pi_info;
			pi_addr = wfo_dev->pi_addr;
			for (i = 0; i < MAX_PE_NUM; i++) {
				pe_info = &wfo_priv->pe_info[i];
				if (pe_info->pe_role & PE_ROLE_PPE)
					qnum = PI_RING_NUM;
				else if (pe_info->pe_role & PE_ROLE_DOE)
					qnum = DRV_PI_RING_NUM;
				else
					qnum = 0;

				if (qnum) {
					RTW_PRINT("[PE%d] PI rw %08x bd %08x\n",
						i, pi_addr->pi_ring_rw_ptr, pi_addr->pi_ring_addr);

					prw = ioremap(pi_addr->pi_ring_rw_ptr, (qnum * 4));
					for (j = 0; j < qnum; j++) {
						bd_addr = pi_addr->pi_ring_addr;
						data_addr = bd_addr + pi_info->pi_ring_num * \
									pi_info->pi_ring_size;
						RTW_PRINT("  PI%d rw %08x bd %08x data %08x "
									"num %3d size %2d data_size %3d\n",
							j, prw[j], bd_addr, data_addr,
							pi_info->pi_ring_num, pi_info->pi_ring_size,
							pi_info->pi_ring_dsize);
						pi_info++;
						pi_addr++;
					}
					iounmap(prw);
			...
			/* wp_seq */
			for (i = 0; i < 4; i++) {
				RTW_PRINT("[ch %d] wp_seq %d\n", i, wd_ring[i].wp_seq);
			}
		}
	} else if (!strcmp(para, "priv")) {
		/* dump wfo_priv info */
		RTW_PRINT("wfo_band_cap: 0x%x\n", wfo_priv->wfo_band_cap);
		RTW_PRINT("wfo_band_on: 0x%x\n", wfo_priv->wfo_band_on);
		RTW_PRINT("\n");

		/* device info */
		for (i = 0; i < BAND_MAX; i++) {
			if (wfo_priv->wfo_band_cap & BIT(i)) {
				RTW_PRINT("dev[%d]:\n", i);
				RTW_PRINT("    dvobj: %px\n",
					wfo_priv->dev[i].dvobj);
				RTW_PRINT("    wfo_pe_stopped: %u\n",
					wfo_priv->dev[i].dvobj->wfo_pe_stopped);
				RTW_PRINT("    sw_seq_addr: 0x%08x\n",
					wfo_priv->dev[i].sw_seq_addr);
				RTW_PRINT("    doe_sw_seq: %px\n",
					wfo_priv->dev[i].doe_sw_seq);
				RTW_PRINT("    pi_ring_rw_ptr: 0x%08x\n",
					wfo_priv->dev[i].pi_addr[0].pi_ring_rw_ptr);
				RTW_PRINT("    pi_ring_addr: 0x%08x\n",
					wfo_priv->dev[i].pi_addr[0].pi_ring_addr);
			}
		}
		RTW_PRINT("\n");
		...
		for (i = 0; i < MAX_PE_NUM; i++) {
			RTW_PRINT("pe_info[%d]:\n", i);
			RTW_PRINT("    state: %d\n", wfo_priv->pe_info[i].state);
			RTW_PRINT("    pe_role: 0x%x\n", wfo_priv->pe_info[i].pe_role);
			RTW_PRINT("    pe_priv_addr: 0x%08x (%px)\n",
				wfo_priv->pe_info[i].pe_priv_addr, wfo_priv->pe_info[i].pe_priv);
			RTW_PRINT("    last_hb: 0x%08x\n", wfo_priv->pe_info[i].last_hb);
		}

		/* issue IPC to PE to dump priv info */
		rtl_wfo_ipc_priv_info(adapter);
	} else if (!strcmp(para, "wpseq_lmt")) {
		/* wfo wpseq_lmt <wpseq_max> <macid> */
		wfo_cmd_wpseq_lmt_sta_cfg(adapter, cmd_para, para_num);
	} else {
		RTW_ERR("Invalid WFO cmd (%s).\n", para);
	}
}
#endif /* CONFIG_RTL_WIFI_OFFLOAD */

#ifdef DEBUG_REORDER_STAT
void core_cmd_reorder(_adapter *adapter, void *cmd_para, u32 para_num)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	struct rtw_phl_com_t *phl_com = GET_HAL_DATA(dvobj);
	int i, tid;
	...
	if (!strcmp((char *)cmd_para, "reset")){
		phl_com->rx_stats.max_reorder_sta = 0;
		for (i=1; i<NUM_STA; i++) {
			psta = rtw_get_stainfo_by_macid(&adapter->stapriv, (u16)i);
			if (psta && psta->phl_sta) {
				phl_sta = psta->phl_sta;
				for (tid = 0; tid < 8; tid++) {
					if (phl_sta->tid_rx[tid]) {
						phl_sta->tid_rx[tid]->max_stored_mpdu_num =
						phl_sta->tid_rx[tid]->acc_queue_time =
						phl_sta->tid_rx[tid]->max_queue_num =
						phl_sta->tid_rx[tid]->acc_queue_num = 0;
					}
				}
			}
		}
	...
	PHL_PRINT("reorder_sta %u max %u\n",
		phl_com->rx_stats.reorder_sta,
		phl_com->rx_stats.max_reorder_sta);
	...
	for (i=1; i<NUM_STA; i++) {
		...
		if (psta && psta->phl_sta) {
			...
			for (tid = 0; tid < 8; tid++)
				if (phl_sta->tid_rx[tid]) {
					PHL_PRINT("[%d] stored_mpdu_num %u max %u\n",
						tid,
						phl_sta->tid_rx[tid]->stored_mpdu_num,
						phl_sta->tid_rx[tid]->max_stored_mpdu_num);
					PHL_PRINT("    times %u max %u acc %u avg %u\n",
						phl_sta->tid_rx[tid]->acc_queue_time,
						phl_sta->tid_rx[tid]->max_queue_num,
						phl_sta->tid_rx[tid]->acc_queue_num,
						phl_sta->tid_rx[tid]->acc_queue_num / phl_sta->tid_rx[tid]->acc_queue_time);
				}
		}
	}
};
#endif /* DEBUG_REORDER_STAT */
...
struct test_cmd_list core_test_cmd_list[] = {
	...
	#if 0
	{"add_latency_sta",	core_cmd_add_latency_sta_by_addr,	CMD_PARA_STR},
	{"dump_latency",	core_cmd_dump_latency,	CMD_PARA_STR},
	#endif
	{"add_latency_sta_2",	core_cmd_add_latency_sta_by_macid,	CMD_PARA_DEC},
#endif
...
#endif
#ifdef CONFIG_AUTOTEST_DETECT
	{"autotest",	core_cmd_autotest,		CMD_PARA_STR},
#endif
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	{"wfo",	core_cmd_wfo, CMD_PARA_STR},
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
#ifdef DEBUG_REORDER_STAT
	{"reorder", core_cmd_reorder, CMD_PARA_STR},
#endif /* DEBUG_REORDER_STAT */
};
...
u8 rtw_mp_cmd(_adapter *adapter, u8 mp_cmd_id, u8 flags)
{
	...
	/* no need to enqueue, do the cmd hdl directly and free cmd parameter */
	if (flags & RTW_CMDF_DIRECTLY)
		return (H2C_SUCCESS == rtw_mp_cmd_hdl(adapter, mp_cmd_id)) ? _SUCCESS : _FAIL;
...
static u8 _rtw_run_in_thread_cmd(_adapter *adapter, void (*func)(void *), void *context, s32 timeout_ms)
{
	struct cmd_priv *pcmdpriv = &adapter_to_dvobj(adapter)->cmdpriv;
	struct cmd_obj *cmd = NULL;
	struct RunInThread_param *parm = NULL;
	struct submit_ctx *sctx = NULL;
	u8 res = _FAIL;
	...
	init_h2fwcmd_w_parm_no_rsp(cmd, parm, CMD_RUN_INTHREAD);

	if (timeout_ms >= 0) {
		sctx = (struct submit_ctx *)rtw_zmalloc(sizeof(struct submit_ctx));
		if (NULL == sctx)
			goto _free_thread_parm;
		cmd->sctx = sctx;
...
_free_thread_parm:
	rtw_mfree((void *)parm, sizeof(struct RunInThread_param));
...
void rtw_create_ibss_post_hdl(_adapter *padapter, int status)
{
	...
	{
		pwlan = _rtw_alloc_network(pmlmepriv, SCAN_LIST_REGULAR);
		_rtw_spinlock_bh(&(queue->lock));
		if (pwlan == NULL) {

...
sint core_rxsc_get_entry(_adapter *adapter, union recv_frame *prframe, u8 is_amsdu)
{
	...
	if (adapter->cached_sta &&
		adapter->cached_sta->phl_sta &&
		adapter->cached_sta->phl_sta->active &&
		(_rtw_memcmp(adapter->cached_sta->phl_sta->mac_addr,
		             get_addr2_ptr(whdr), ETH_ALEN)) == _TRUE) {
		psta = adapter->cached_sta;
		adapter->core_logs.rxsc_sta_get[1]++;
	}
	else {
		psta = rtw_get_stainfo(psta_priv, get_addr2_ptr(whdr));
		adapter->core_logs.rxsc_sta_get[0]++;
	}
...
u8 _core_rx_check_match_tclas_type_4(void *phl, struct rtw_phl_stainfo_t *phl_sta, union recv_frame *prframe)
{
	u8 ret =0;
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct rtw_phl_mscs_info *mscs_info = &phl_sta->mscs_info;
	struct rtw_phl_tclas_element *tclas = &phl_sta->mscs_info.tclas;
	struct rtw_recv_pkt *rx_req = prframe->u.hdr.rx_req;
	u8 tid = rx_req->mdata.tid;
	u32 ip;
	struct sk_buff *pkt = prframe->u.hdr.pkt;

	if (GET_TYPE_4_VERSION_PRESENT(&tclas->u.t4.mask)) {
		// check skb source ip
		if (GET_TYPE_4_SOURCE_IP_ADDRESS_PRESENT(&tclas->u.t4.mask))
		{
			ip = (tclas->u.t4.ip.v4.src_ip[0] << 24) + (tclas->u.t4.ip.v4.src_ip[1] << 16) +
				(tclas->u.t4.ip.v4.src_ip[2] << 8) + (tclas->u.t4.ip.v4.src_ip[3]);

			if (GET_IPV4_SRC(pkt->data+(ETH_HLEN)) == ip) {
				ret = 1;
			}
		}

		// check skb destation ip
		if (GET_TYPE_4_DESTINATION_IP_ADDRESS_PRESENT(&tclas->u.t4.mask))
		{
			ip = (tclas->u.t4.ip.v4.dst_ip[0] << 24) + (tclas->u.t4.ip.v4.dst_ip[1] << 16) +
				(tclas->u.t4.ip.v4.dst_ip[2] << 8) + (tclas->u.t4.ip.v4.dst_ip[3]);

			if ((GET_IPV4_DST(pkt->data+(ETH_HLEN)) == ip)) {
				ret = 1;
			}
		}
	}
	...
	if (ret) {
		if (mscs_info->up_control_bmp & BIT(tid))
		{
			phl_sta->mscs_target_up = (tid > mscs_info->up_control_limit)? mscs_info->up_control_limit : tid;
		}
	}
...
void _core_rx_check_mscs_classifier(void *phl, union recv_frame *prframe)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct rtw_recv_pkt *rx_req = prframe->u.hdr.rx_req;
	u16 macid = rx_req->mdata.macid;
	struct rtw_phl_stainfo_t *phl_sta = NULL;
	struct rtw_phl_tclas_element tclas;
	u8 i, classifier_type;

	phl_sta = rtw_phl_get_stainfo_by_macid(phl_info, macid);
	if (NULL == phl_sta)
		return;

	if (phl_sta->mscs_info.req_type == SCS_ADD) {
		tclas = phl_sta->mscs_info.tclas;
		classifier_type = tclas.type;

		switch (classifier_type) { /* TODO : check Frame Classifier filter (type 0~10) */
			case CLSFR_TYPE_0:
				break;
			case CLSFR_TYPE_1:
				break;
			...
			case CLSFR_TYPE_3:
				break;
			case CLSFR_TYPE_4:
				_core_rx_check_match_tclas_type_4(phl, phl_sta, prframe);
				break;
			case CLSFR_TYPE_5:
				break;
			case CLSFR_TYPE_6:
...
s32 core_rxsc_apply_check(_adapter *adapter, union recv_frame *prframe, void *rx_req)
{
	...
	if (psta == NULL)
		psta = rtw_get_stainfo(&adapter->stapriv,
				       get_addr2_ptr(whdr));
	if (psta && ((psta->SBWC_mode | adapter->SBWC_mode) & SBWC_MODE_LIMIT_STA_RX))
		return CORE_RX_CONTINUE;
#endif /* SBWC */
	...
	#ifdef CONFIG_QOS_MANAGEMENT
	_core_rx_check_mscs_classifier(phl, prframe);
	#endif
	...
	rxsc_entry->rxsc_attrib.sgi = phlrx->mdata.rx_gi_ltf;
	rxsc_entry->rxsc_attrib.first_ppdu = phlrx->mdata.ppdu_cnt_chg;
	wmb();
...
#ifdef CONFIG_RTW_A4_STA
	if (MLME_IS_AP(adapter) && (3 == pattrib->to_fr_ds))
		core_a4_upt_db(adapter, psta, pattrib->src);
#endif
...
/* Packet forward to OS */
	if (RXSC_FWD_KERNEL == rxsc_entry->forward_to && prframe->u.hdr.pkt
		&& is_equal_mac_addr(prframe->u.hdr.pkt->data, adapter_mac_addr(adapter))) {
		struct sk_buff *pkt = prframe->u.hdr.pkt;

#ifndef CONFIG_ETHER_PKT_AGG
#ifdef CONFIG_BR_EXT

...
s32 sdio_data_xmit(_adapter *adapter, struct xmit_frame *pxmitframe)
{
	...
	if ((pxmitframe->frame_tag == DATA_FRAMETAG)
		...
		&& (pxmitframe->attrib.ether_type != ETH_P_EAPOL)
...
static s32 sdio_recv_hdl(_adapter *adapter)
{
	...
	do {
		...
		do {
			precvframe = rtw_alloc_recvframe(&adapter->dvobj->recv_compriv.free_recv_queue);
			if (precvframe == NULL) {
			...
			if (!rtw_hal_rcr_check(adapter, BIT_ACRC32_8821C) && pattrib->crc_err) {
				...
				rtw_free_recvframe(precvframe, &adapter->dvobj->recv_compriv.free_recv_queue);
				break;
			}

...
void rtw_init_rxbuf_mempool(struct rtw_phl_com_t *phl_com)
{
	...
	snprintf(name_buf,sizeof(name_buf),"%s%d_rxq","wifi", phl_com->dev_id);
#ifdef CONFIG_RTW_RX_BUF_SHARING
	rtw_create_mem_pool_ex(name_buf,(phl_com->bus_sw_cap.rxbuf_num_effective/2)*4, phl_com->bus_sw_cap.rxbuf_num_effective/2, phl_com->bus_sw_cap.rxbuf_size_effective*2);
#else
	rtw_create_mem_pool_ex(name_buf,phl_com->bus_sw_cap.rxbuf_num_effective*4, phl_com->bus_sw_cap.rxbuf_num_effective, phl_com->bus_sw_cap.rxbuf_size_effective);
#endif

	snprintf(name_buf,sizeof(name_buf),"%s%d_rpq","wifi", phl_com->dev_id);
#ifdef CONFIG_RTW_RX_BUF_SHARING
	rtw_create_mem_pool_ex(name_buf,(phl_com->bus_sw_cap.rpbuf_num_effective/2)*4, phl_com->bus_sw_cap.rpbuf_num_effective/2, phl_com->bus_sw_cap.rpbuf_size_effective*2);
#else
	rtw_create_mem_pool_ex(name_buf,phl_com->bus_sw_cap.rpbuf_num_effective*4, phl_com->bus_sw_cap.rpbuf_num_effective, phl_com->bus_sw_cap.rpbuf_size_effective);
#endif
...
static void core_hdl_phl_evt(struct dvobj_priv *dvobj, struct phl_msg *msg)
{
	...
	else if (evt_id == MSG_EVT_BCCHG_COUNTDOWN_ZERO) {
		struct rtw_bcc_cntdown_rpt *bcc_rpt = (struct rtw_bcc_cntdown_rpt *)msg->inbuf;

		//RTW_PRINT("bc chg countdown msg, inbuf: %x, inlen: %u\n",
		//	(msg->inlen > 0) ? *(msg->inbuf) : 255, msg->inlen);

		if (bcc_rpt->band == 0 && bcc_rpt->port == 0) {
			// bnad0 root CSA C2H
			iface = dvobj_get_primary_adapter(dvobj);
				rfctl = adapter_to_rfctl(iface);
				if (rtw_set_bc_cmd(iface) != _SUCCESS) {
					rfctl->bcc_color = 0;
					rfctl->bcc_cntdown = 0;
					rfctl->bcc_set_ie = 0;
				}
...
#ifdef CONFIG_OPENSYNC
#ifdef CONFIG_80211N_HT
		ht_option = iface->mlmepriv.htpriv.ht_option;
#endif
		rtw_cfg80211_radar_detected_event(iface, iface->mlmeextpriv.cur_channel, iface->mlmeextpriv.cur_bwmode, iface->mlmeextpriv.cur_ch_offset, ht_option);
#endif
	}
	else {
		RTW_INFO("%s evt_id :%d\n", __func__, evt_id);
...
u8 rtw_hw_init(struct dvobj_priv *dvobj)
{
	...
	phl_status = rtw_phl_trx_alloc(dvobj->phl);
	...
	evt_ops->process_pwr_state = rtw_core_process_pwr_state;
	evt_ops->process_tim_indic = rtw_core_process_tim_indic;
	evt_ops->process_om_control = rtw_core_process_om_control;
	evt_ops->count_pe_tx_stats = rtw_count_pe_tx_stats;
	evt_ops->pe_trx_pause = rtw_core_pe_trx_pause;
	evt_ops->pe_trx_resume = rtw_core_pe_trx_resume;
	evt_ops->pe_tx_force_para = rtw_core_pe_tx_force_para;
...
int rtw_hw_set_ch_bw(struct _ADAPTER *a, u8 ch, enum channel_width bw,
		      u8 offset, u8 do_rfk, enum phl_cmd_type cmd_type)
{
	...
	if (mib->txpwr_lmt_index)
		rtw_phl_set_rf_regulation_idx(GET_HAL_INFO(a->dvobj), (mib->txpwr_lmt_index + 15));
#endif
...
u8 rtw_hw_iface_init(_adapter *adapter)
{
...
#ifdef TX_BEAMFORMING
	dvobj->phl_com->txbf_force_ng_cb = adapter->registrypriv.wifi_mib.txbf_force_ng_cb;
	...
	if (rtw_phl_snd_chk_in_progress(phl_info) == 0) {
		if(adapter->registrypriv.wifi_mib.txbf_auto_snd) {
			if(adapter->registrypriv.wifi_mib.txbf&&
				...
				int sounding_flag = 0;
				dvobj->phl_com->txbf_period_fw = adapter->registrypriv.wifi_mib.txbf_period_fw;
	...
	rtw_phl_sr_enable(phl, primary_adapter->registrypriv.wifi_mib.sr_enable);
...
void rtw_hw_iface_deinit(_adapter *adapter)
{
	...
	if (is_primary_adapter(adapter))
		rtw_tasklet_kill(&dvobj->txsc_amsdu_timeout_tasklet);
		   ...
		   enum phl_cmd_type cmd_type,  u32 cmd_timeout)
{
	...
	if (keyalgo == _SMS4_ || keyalgo == _GCM_SM4_) {
		...
		rtw_phl_del_key(phl, sta->phl_sta, &del_crypt, 1, NULL);
	}
#endif
			       ...
			       struct protocol_cap_t *cap)
{
	...
	RTW_PRINT("Updated %pM's A-MPDU number to %u\n",
	          sta->phl_sta->mac_addr, cap->num_ampdu);
#endif /* CONFIG_80211N_HT */
...
void rtw_adjust_mbssid_num(struct _ADAPTER *a, u32 bcnint)
{
	struct dvobj_priv *d;
	void *phl;
...
void rtw_adjust_bcn_hold_time(struct _ADAPTER *a, u32 hold_time)
{
	struct dvobj_priv *d;
	void *phl;
	struct rtw_wifi_role_t *wrole;
	...
	if (!wrole)
		return;
	param.cfg_id = CFG_BCN_HOLD_TIME;
	param.value = hold_time;
	status = rtw_phl_cmd_wrole_change(phl, a->phl_role,
				WR_CHG_AP_PARAM, (u8*)&param, sizeof(struct rtw_ap_param),
				PHL_CMD_DIRECTLY, 0);
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
			if (RTL_WFO_IS_OFLD(a->dvobj))
				rtl_wfo_ipc_update_rxofld_info(a, sta, 0, tid, 0);
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
...
u8 rtw_acs_get_nhm_ratio_by_idx(struct _ADAPTER *a, u8 acs_idx, u8 type)
{
	void *phl = GET_HAL_INFO(adapter_to_dvobj(a));
	struct rtw_acs_info_parm parm = {0};
	u8 nhm_ratio = 0;
	...
	channel = rtw_acs_get_channel_by_idx(a, acs_idx);
...
u16 rtw_acs_get_ch_util_cca_by_idx(struct _ADAPTER *a, u8 acs_idx)
{
	void *phl = GET_HAL_INFO(adapter_to_dvobj(a));
	struct rtw_acs_info_parm parm = {0};
	u16 chan_util_cca = 0;
	...
	parm.idx = acs_idx;
	parm.type = RTW_ACS_INFO_CHAN_UTIL_CCA;
	parm.info = &chan_util_cca;

	if (rtw_phl_get_acs_info(phl, &parm) == RTW_PHL_STATUS_SUCCESS)
		return chan_util_cca;
	else
		return 0;
}

u16 rtw_acs_get_ch_util_tx_by_idx(struct _ADAPTER *a, u8 acs_idx)
{
	void *phl = GET_HAL_INFO(adapter_to_dvobj(a));
	struct rtw_acs_info_parm parm = {0};
	u16 chan_util_tx = 0;
	...
	parm.idx = acs_idx;
	parm.type = RTW_ACS_INFO_CHAN_UTIL_TX;
	parm.info = &chan_util_tx;
...
u8 rtw_acs_get_clm_fa_ratio_by_idx(struct _ADAPTER *a, u8 acs_idx)
{
	void *phl = GET_HAL_INFO(adapter_to_dvobj(a));
	struct rtw_acs_info_parm parm = {0};
	u8 clm_fa_ratio = 0;
	...
	parm.idx = acs_idx;
	parm.type = RTW_ACS_INFO_CLM_FA_RATIO;
	parm.info = &clm_fa_ratio;
...
void rtw_set_nhm_mntr_result(struct _ADAPTER *a)
{
	void *phl = GET_HAL_INFO(adapter_to_dvobj(a));

	rtw_phl_set_nhm_mntr_result(phl);
}

s8 rtw_get_nhm_mntr_result(struct _ADAPTER *a, u8 type)
{
	void *phl = GET_HAL_INFO(adapter_to_dvobj(a));

	return rtw_phl_get_nhm_mntr_result(phl, type);
}
...
void rtw_core_set_gt3(_adapter *padapter, u8 enable, u32 timeout)
{
	//#ifndef CONFIG_RTW_LINK_PHL_MASTER
...
u8 rtw_get_sta_rssi(struct sta_info *sta)
{
	u8 rssi = rtw_phl_get_sta_rssi(sta->phl_sta);

...
/****************************gcm128_end**************************/
static u8 wapiBlockIn[16], wapiTempBlock[16];
static u32 wapiRK[32];
static u32 gcmsm4RK[32];
...
{
	SMS4KeyExt((u8 *)key,gcmsm4RK);
	
	SMS4Crypt((u8 *)Input,(u8 *)Output,gcmsm4RK);
}
								...
								u8 *Output, u16 *OutputLength)
{
	u32 blockNum, i, j;
	u16	remainder;
	u8 *pBlockIn, *pBlockOut, *pBlockTemp;
	...
	SMS4KeyExt(Key, wapiRK);

	pBlockIn = wapiBlockIn;
	pBlockOut = wapiTempBlock;
	for (i = 0; i < blockNum; i++) {
		SMS4Crypt(pBlockIn, pBlockOut, wapiRK);
										...
										u8 *Input2, u16 Input2Length, u8 *Output)
{
	...
	SMS4KeyExt(Key, wapiRK);
	SMS4Crypt(wapiBlockIn, BlockOut, wapiRK);
...
u32	rtw_sms4_decrypt(_adapter *padapter, u8 *precvframe)
{
	...
	if (_rtw_memcmp(mic_result, recvMic, WAPI_MIC_LEN) == _FALSE) {
		...
		rtw_dump_buff(pframe, prxattrib->hdrlen + 18, "rtw_sms4_decrypt: data before decrypt");
		return _FAIL;		
	} else {
...
u32	rtw_gcm_sm4_decrypt(_adapter *padapter, u8 *precvframe)
{
	...
	if(rtw_wapi_gcm_sm4_cryption(pDataKey, recvPN, mic_part1, part1_len, payload, payload_len, payload, recvMic, WAPI_DECRYPT) == _FAIL){
		RTW_INFO("%s: check MIC fail\n", __FUNCTION__);
		rtw_dump_buff(pframe, prxattrib->hdrlen + 18, "rtw_gcm_sm4_decrypt: data before decrypt");
		return _FAIL;
	}
	else{
		RTW_INFO("%s: check MIC success\n", __FUNCTION__);

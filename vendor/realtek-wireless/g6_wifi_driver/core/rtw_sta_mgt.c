...
void _rtw_init_stainfo(struct sta_info *psta, unsigned char is_first)
{
	...
	if (is_first) {
		...
		psta->process_assoc_req_by_hapd = ASSOC_REQ_DEFAULT_VALUE;
		...
		rtw_rm_free_sta_beacon_report(psta, 0);
#endif
...
u32	_rtw_init_sta_priv(struct	sta_priv *pstapriv)
{
	...
	if (!pstapriv->tim_bitmap)
		goto exit;
	pstapriv->tim_bitmap_set = rtw_zmalloc(pstapriv->aid_bmp_len);
	if (!pstapriv->tim_bitmap_set)
		goto exit;
	...
	/* CONFIG_QOS_MANAGEMENT */
	pstapriv->presta_tbl = rtw_vmalloc(sizeof(struct rhashtable));
...
u32	_rtw_free_sta_priv(struct	sta_priv *pstapriv)
{
	...
	if (pstapriv) {
		...
		if (pstapriv->tim_bitmap_set)
			rtw_mfree(pstapriv->tim_bitmap_set, pstapriv->aid_bmp_len);
		#endif

		/* CONFIG_QOS_MANAGEMENT */
		rtw_deinit_presta_tbl(pstapriv);
		rtw_vmfree(pstapriv->presta_tbl, sizeof(struct rhashtable));
	}
...
static struct sta_info *_rtw_alloc_core_stainfo(struct sta_priv *pstapriv, const u8 *hwaddr)
{
	...
	if (psta) {
		...
		psta->hostpolicer_idx = 0xFFFFFFFF;
		if (strncmp(hwaddr, pstapriv->padapter->mac_addr, 6)) {
			if (psta->SBWC_tx_limit)
				psta->SBWC_mode |= SBWC_MODE_LIMIT_STA_TX;
			if (psta->SBWC_rx_limit)
				psta->SBWC_mode |= SBWC_MODE_LIMIT_STA_RX;
			if(psta->SBWC_mode != SBWC_MODE_DISABLE)
				rtw_fc_sbwc_add_sta_host_policer(pstapriv->padapter, psta, hwaddr);
		}
#endif /* FC_SBWC */
		...
		for (i = 0; i < MAX_NUM_TWT_CONFIG; i++) {
			psta->twt_config[i].config_id = 255;
			psta->twt_config[i].acked = 0;
		}
#endif
		...
		rtw_sta_mbo_init(psta->padapter, psta);
#endif
	}
...
struct sta_info *rtw_alloc_stainfo(struct	sta_priv *stapriv, const u8 *hwaddr, enum phl_cmd_type cmd_type)
{
	...
	if (sta) {
		...
		sta->rps_pe_id = RTL_WFO_ARM_ID;
		sta->xps_pe_id = RTL_WFO_ARM_ID;
		sta->ppe_pe_id = RTL_WFO_ARM_ID;
		sta->fc_staid = 0xff;
		sta->twt_en = _FALSE;
...
u32 static _rtw_free_core_stainfo(_adapter *padapter , struct sta_info *psta)
{
...
#endif
#if (defined(RTW_STA_BWC) || defined (SBWC)) && !defined(FC_SBWC)
...
#ifdef CONFIG_RTW_MBO
	rtw_sta_mbo_deinit(padapter, psta);
#endif
...
#ifdef FC_SBWC
	/* Delete host policer if using FC SW shaping for bandwidth control */
...
#else
		_rtw_memset((u8 *)&psta->txsc_entry_cache[i].txsc_wlhdr, 0x0, CORE_TXSC_WLHDR_SIZE);
#endif
	}
#endif
	...
	if (pdefrag_q) {
		if (_rtw_queue_empty(pdefrag_q) == _FALSE)
			rtw_free_recvframe_queue(pdefrag_q, &padapter->dvobj->recv_compriv.free_recv_queue);
	}
...
u32 rtw_free_stainfo(_adapter *padapter, struct sta_info *psta)
{
	...
	psta->mscs_template = (struct rtw_phl2_mscs_desc_data_s){0};
	psta->has_mscs = 0;
...
u16 rtw_aid_alloc(_adapter *adapter, struct sta_info *sta)
{
	...
	if (aid) {
		...
		rtw_tim_map_set(adapter, stapriv->tim_bitmap_set, sta->phl_sta->aid);
	}

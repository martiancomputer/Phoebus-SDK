...
const u16 LEGACY_DATA_RATE_VALUE[12] =
{
	1, 2, 6, 11, 6, 9, 12, 18, 24, 36, 48, 54
};

const u16 MCS_DATA_RATE_VALUE[2][2][32] =
{
	{{7, 13, 20, 26, 39, 52, 59, 65,
	  13, 26, 39 ,52, 78, 104, 117, 130,
	  20, 39, 59, 78 ,117, 156, 176, 195,
	  26, 52, 78, 104, 156, 208, 234, 260},		// Long GI, 20MHz

	 {7, 14, 22, 29, 43, 58, 65, 72,
	  14, 29, 43, 58, 87, 116, 130, 145,
	  22, 43, 65, 87, 130, 173, 195, 217,
	  29, 58, 87, 116, 173, 231, 260, 289}},	// Short GI, 20MHz

	{{14, 27, 41, 54, 81, 108, 122, 135,
	  27, 54, 81, 108, 162, 216, 243, 270,
	  41, 81, 122, 162, 243, 324, 365, 405,
...
const u16 VHT_MCS_DATA_RATE_VALUE[4][2][40] =
{
...
const u16 HE_MCS_DATA_RATE_VALUE[4][3][48] =
{
...
void rtw_mi_status_by_ifbmp(struct dvobj_priv *dvobj, u16 ifbmp, struct mi_state *mstate)
{
	_adapter *iface;
	int i;
...
{
	return rtw_mi_status_by_ifbmp(adapter_to_dvobj(adapter), 0xFFFF, mstate);
}

inline void rtw_mi_status_no_self(_adapter *adapter, struct mi_state *mstate)
{
	return rtw_mi_status_by_ifbmp(adapter_to_dvobj(adapter), 0xFFFF & ~BIT(adapter->iface_id), mstate);
}
...
u16 get_current_rate_value(struct sta_info *psta, bool is_direction_tx)
{
	u8  current_bw = 0;
	u16 current_rate = 0;
	u8  current_giltf = 0;
	int band_with = 0, is_short_gi = 0, giltf_level = 0, idx = 0;
	...
	if(current_rate >= RTW_DATA_RATE_HE_NSS1_MCS0 && current_rate <= RTW_DATA_RATE_HE_NSS4_MCS11)
	{
		...
		return HE_MCS_DATA_RATE_VALUE[band_with][giltf_level][(idx < 48) ? idx : 0];
	}
	else if(current_rate >= RTW_DATA_RATE_VHT_NSS1_MCS0 && current_rate <= RTW_DATA_RATE_VHT_NSS4_MCS9)
	{
		...
		return VHT_MCS_DATA_RATE_VALUE[band_with][is_short_gi][(idx < 40) ? idx : 0];
	}
	else if (current_rate >= RTW_DATA_RATE_MCS0 && current_rate <= RTW_DATA_RATE_MCS31)
	{
		return MCS_DATA_RATE_VALUE[band_with <= 1 ? band_with : 1][is_short_gi][(current_rate - RTW_DATA_RATE_MCS0)];
	}
	else if(current_rate <= RTW_DATA_RATE_OFDM54)//legacy rate
	{
		return LEGACY_DATA_RATE_VALUE[current_rate];
	}
	else
...
static u8 _rtw_mi_traffic_statistics(_adapter *padapter , void *data)
{
	...
	for (i = 0; i < 4; i++)
		pdvobjpriv->tx_wmm_pkts[i] += padapter->wmm_stats[i].tx_pkts;
	...
	/* Rx info per STA */
	pdvobjpriv->traffic_stat.ratio_bss_rx = 0;
	...
	while (plist != phead) {
		...
		if (psta) {
			tmp_current_rx_byte = psta->sta_stats.rx_bytes - psta->sta_stats.last_rx_bytes;
			rate_value = get_current_rate_value(psta, 0);
			if (rate_value != 0) {
				pdvobjpriv->traffic_stat.ratio_bss_rx += (u32)((u32)tmp_current_rx_byte * 8 / rate_value / 10000);
			}
		}
		plist = plist->next;
	}
	_rtw_spinunlock_bh(&pstapriv->asoc_list_lock);
	if (pdvobjpriv->traffic_stat.ratio_bss_rx > 95) // calibrate abnormal value
		pdvobjpriv->traffic_stat.ratio_bss_rx = 95;

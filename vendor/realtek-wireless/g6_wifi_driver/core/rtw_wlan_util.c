...
#define OCTOSCOPE_OUI_NUM 1
unsigned char OCTOSCOPE_OUI[][3]= {
	{0x2C,0x27,0x9E}
};

#define QINGDAO_PDA_OUI_NUM 1
unsigned char QINGDAO_PDA_OUI[][3]= {
	{0x10,0xb2,0x32}
};

#define FIBOCOM_PDA_OUI_NUM 4
unsigned char FIBOCOM_PDA_OUI[][3]= {
	{0x64,0xf6,0xbb},
	{0x88,0x23,0x1f},
	{0xb0,0x5a,0x44},
	{0xb4,0x36,0xa9},
...
#define QUALCOMM_OUI_NUM 4
unsigned char QUALCOMM_OUI[][3]= {
	{0x00,0xa0,0xc6},
	{0x64,0x9c,0x81},
	{0x88,0x12,0x4e},
	{0x8c,0xfd,0xf0},
					...
					u8 do_rfk)
{
...
#ifdef CONFIG_DFS_MASTER
	{
		...
		rtw_hw_set_ch_bw(padapter, channel, (enum channel_width)bwmode,
				 channel_offset, do_rfk, PHL_CMD_DIRECTLY);
														...
														u8 do_rfk)
{
	...
	return set_bch_bwmode(padapter, band, channel, channel_offset, bwmode, do_rfk);
}
		     ...
		     struct beacon_keys *recv_beacon)
{
	...
	if (elems.rsn_ie && elems.rsn_ie_len) {
		...
		rtw_parse_wpa2_ie(elems.rsn_ie - 2, elems.rsn_ie_len + 2,
			...
			NULL, &recv_beacon->akm, &recv_beacon->mfp_opt, NULL);
	}
	/* checking WPA secon */
...
unsigned int get_dvobj_total_sta_num(struct dvobj_priv *dvobj)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		total_sta_num += padapter->stapriv.asoc_list_cnt;
	}
	return total_sta_num;
}
...
int update_network_type(_adapter *padapter)
{
...
*/
void update_priv_mib_band(_adapter *padapter, int network)
{
...
void update_wireless_mode(_adapter *padapter)
{
	...
	network_type = update_network_type(padapter);
	update_priv_mib_band(padapter, network_type);
...
{
	struct sta_info *psta;
	u16 tid, start_seq, param, buf_size, size;
	struct sta_priv *pstapriv = &padapter->stapriv;
	...
	tid = (param >> 2) & 0x0f;
	buf_size = (param >> 6) & 0x3ff;

	/* Intel 6205 will send addba-Req with buffer size = 0.
	   If APUT send addbaRsp with buffer size(0) will let NB to be "blue screen".
	   Avoid this case, set buf_size to 64 as basic. */
	if (buf_size == 0)
		buf_size = 64;

	if((psta->htpriv.ht_option == _TRUE) && (psta->htpriv.ampdu_enable == _TRUE)
		&& rtw_rx_ampdu_is_accept(padapter))
		accept = _TRUE;

	if (padapter->fix_rx_ampdu_size != RX_AMPDU_SIZE_INVALID) {
		size = padapter->fix_rx_ampdu_size;
	} else {
		size = rtw_rx_ampdu_size_by_sta(padapter, psta, buf_size);
...
u16 process_scs_req(_adapter *padapter,
		    u8 *scs_req,
		    u16 ele_len,
		    struct sta_info *psta)
{
	u8 status = INSUFFICIENT_TCLAS_PROCESSING_RESOURCES;
	struct rtw_presta_s *fakesta = _rtw_zmalloc(sizeof(struct rtw_presta_s));
	if (!psta) goto exit;

	rtw_presta_fill(fakesta, psta->phl_sta);
	status = rtw_parse_scs_descriptor(padapter, fakesta, scs_req, ele_len);
	rtw_presta_ret(psta->phl_sta, fakesta);
exit:
	rtw_mfree(fakesta, sizeof(struct rtw_presta_s));
	return status;
}

static u16 _process_mscs_req2(struct rtw_mscs_desc_s *mscs_req,
		    struct rtw_presta_s *psta)
{
	if (mscs_req->req_type == SCS_ADD || mscs_req->req_type == SCS_CHANGE) {
		if (mscs_req->req_type == SCS_ADD && psta->has_mscs) return REQUEST_DECLINED;
		if (mscs_req->template.tclas_mask_elem_len == 0) return REQUEST_DECLINED;
		psta->has_mscs = 1;
		mscs_htbl_reinit_table(psta->mscs_tbl, QOS_MSCS_TBL_BITL);
		rtw_phl2_mscs_desc_data_copy(&psta->mscs_template, &mscs_req->template);
		return SCS_REQ_SUCCESS;
	} else if (mscs_req->req_type == SCS_REMOVE) {
		psta->has_mscs = 0;
		mscs_htbl_reinit_table(psta->mscs_tbl, QOS_MSCS_TBL_BITL);
		return SCS_REQ_SUCCESS;
	}
...
static u16 _process_mscs_req(struct rtw_mscs_desc_s *mscs_req,
		    struct sta_info *psta)
{
	if (mscs_req->req_type == SCS_ADD || mscs_req->req_type == SCS_CHANGE) {
		if (mscs_req->req_type == SCS_ADD && psta->has_mscs) return REQUEST_DECLINED;
...
u16 process_mscs_req(_adapter *padapter,
		    u8 *mscs_req,
		    u16 ele_len,
		    struct sta_info *psta)
{
	u16 status = INSUFFICIENT_TCLAS_PROCESSING_RESOURCES;
	_adapter *primary_adapter = dvobj_get_primary_adapter(padapter->dvobj);
	struct rtw_mscs_desc_s mscs_desc = (struct rtw_mscs_desc_s){0};

	if (!psta) goto exit;
#ifdef CONFIG_RTW_MULTI_AP_R3
	if (_maclist_contain(
		primary_adapter->mscs_disallow_list,
		primary_adapter->mscs_disallow_list_count,
	...
	status = mscs_parse_descriptor(&mscs_desc, mscs_req, ele_len);
	if (status != SCS_REQ_SUCCESS) goto exit;
	status = _process_mscs_req(&mscs_desc, psta);
...
u16 process_scs_req_ctrl(_adapter *padapter,
		    u8 *scs_req,
		    u16 ele_len, u8 sta_mac[ETH_ALEN])
{
	u16 ret = INSUFFICIENT_TCLAS_PROCESSING_RESOURCES;
	struct sta_info *psta = NULL;
	struct sta_priv *pstapriv = &padapter->stapriv;
	struct rtw_presta_s *presta = rtw_get_presta(pstapriv, sta_mac);
	if (presta == NULL) goto exit;
	ret = rtw_parse_scs_descriptor(padapter, presta, scs_req, ele_len);

exit:
	return ret;
}

static u16 _process_mscs_req_ctrl(_adapter *padapter,
		    struct rtw_mscs_desc_s *mscs_req,
		    u8 sta_mac[ETH_ALEN])
{
	u16 status = INSUFFICIENT_TCLAS_PROCESSING_RESOURCES;
	struct sta_info *psta = NULL;
	struct sta_priv *pstapriv = &padapter->stapriv;
	struct rtw_presta_s *presta = NULL;

	presta = rtw_get_presta(pstapriv, sta_mac);
	if (presta == NULL) goto exit;

	psta = rtw_get_stainfo(pstapriv, sta_mac);
	if (psta) {
		status = _process_mscs_req(mscs_req, psta);
		if (status != SCS_REQ_SUCCESS) goto exit;
	}

	status = _process_mscs_req2(mscs_req, presta);

exit:
	return status;
}

u16 process_mscs_req_ctrl(_adapter *padapter,
		    u8 *mscs_req,
		    u16 ele_len, u8 sta_mac[ETH_ALEN])
{
	u16 status = INSUFFICIENT_TCLAS_PROCESSING_RESOURCES;
	...
	if ((status = mscs_parse_descriptor(&mscs_desc, mscs_req, ele_len))) goto exit;
	status = _process_mscs_req_ctrl(padapter, &mscs_desc, sta_mac);
exit:
	return status;
}
#endif
...
#if defined(CONFIG_RTW_PERSIST_IF) || defined(CPTCFG_RTW_PERSIST_IF) || defined(CONFIG_OPENSYNC)
...
int txpower_percent_to_level(u8 percentage)
{
	...
	const int pwrlevel_diff[RTW_PWR_DIFF_MAX] = {
		-40, -34, -30, -28, -26,
		-24, -23, -22, -21}; // for < 10% case
	...
	if (percentage >= RTW_PWR_DIFF_MAX)
		percentage = RTW_PWR_DIFF_MAX -1;
...
#endif
#ifdef FC_SBWC
		case EVENT_STA_BANDWIDTH_LIMIT:
		{
			/* data = [mac(6) + add/delete(4) + ifname(IFNAMSIZ)] */
			memcpy(mac, data, ETH_ALEN);
			/* info = [event(1) + mac(6) + add/delete(4) + ifname(IFNAMSIZ)] */
			info[0] = EVENT_STA_BANDWIDTH_LIMIT;
			memcpy(&info[1], data, (ETH_ALEN + 4 + IFNAMSIZ));
			len = 1 + ETH_ALEN + 4 + IFNAMSIZ;
			break;
		}
#endif
		default:
...
#ifdef REPORT_RADAR_DETECT
		case EVENT_RADAR_DETECT:
		{
			len = rtw_construct_radar_detect_info(padapter, info);
			break;
		}
#endif /* REPORT_RADAR_DETECT */
#ifdef CONFIG_CMCC_FEATURE
		case EVENT_CHANNEL_CHANGE:
		{
			struct channel_change_event *event = (struct channel_change_event *)info;
			struct channel_change_event *_event = (struct channel_change_event *)data; 

			event->event_id = EVENT_CHANNEL_CHANGE;
			event->pre_ch = _event->pre_ch;
			event->cur_ch = _event->cur_ch;
			event->reason = _event->reason;

			len = sizeof(struct channel_change_event);

			RTW_DBG("[%s][EVENT_CHANNEL_CHANGE] pre:%d cur:%d reason:%d\n", 
				__func__, event->pre_ch, event->cur_ch, event->reason);
			break;
		}
		case EVENT_NAME_CONFLICT:
		{
			struct name_conflict_event *event = (struct name_conflict_event *)info;
			NDIS_802_11_SSID *ssid = (NDIS_802_11_SSID *)data;

			event->event_id = EVENT_NAME_CONFLICT;
			_rtw_memcpy(event->ssid, ssid->Ssid, ssid->SsidLength);
			_rtw_memcpy(event->bssid, padapter->mac_addr, ETH_ALEN);

			len = sizeof(struct name_conflict_event);

			RTW_DBG("[%s][EVENT_NAME_CONFLICT] bssid="MAC_FMT"\n", __func__, MAC_ARG(event->bssid));

			break;
		}
		case EVENT_BSSID_CONFLICT:
		{
			struct bssid_conflict_event *event = (struct bssid_conflict_event *)info;
			struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
			struct wlan_network *cur_network = &(pmlmepriv->cur_network);
			WLAN_BSSID_EX *bssid = (WLAN_BSSID_EX *)data;

			event->event_id = EVENT_BSSID_CONFLICT;
			_rtw_memcpy(event->ssid, cur_network->network.Ssid.Ssid, cur_network->network.Ssid.SsidLength);
			_rtw_memcpy(event->conflict_bssid, bssid->MacAddress, ETH_ALEN);
			_rtw_memcpy(event->conflict_ssid, bssid->Ssid.Ssid, bssid->Ssid.SsidLength);

			len = sizeof(struct bssid_conflict_event);

			RTW_DBG("[%s][EVENT_BSSID_CONFLICT] conflict_bssid="MAC_FMT"\n", __func__, MAC_ARG(event->conflict_bssid));
			break;
		}
		case EVENT_WIFI_STA_BG_MODE:
		{
			struct sta_bg_mode_event *event = (struct sta_bg_mode_event *)info;
			u8 *sta_mac = (u8 *)data;

			event->event_id = EVENT_WIFI_STA_BG_MODE;
			_rtw_memcpy(event->mac_addr, sta_mac, MAC_ALEN);

			len = sizeof(struct sta_bg_mode_event);

			RTW_DBG("[%s][EVENT_WIFI_STA_BG_MODE] mac="MAC_FMT"\n", __func__, MAC_ARG(event->mac_addr));
			break;
		}
#endif /* CONFIG_CMCC_FEATURE */
		default:
			RTW_DBG("Not supported event : %d\n", event);
			event_supported = 0;
			break;
	}
...
#ifdef CONFIG_CMCC_FEATURE
void rtw_channel_change_indicate(_adapter *padapter, u8 pre_ch, u8 cur_ch, u8 reason)
{
	struct channel_change_event event;
	event.pre_ch = pre_ch;
	event.cur_ch = cur_ch;
	event.reason = reason;
	#ifdef CONFIG_WLAN_EVENT_INDICATE_GENL
	genl_wlan_IndicateEvent(padapter, EVENT_CHANNEL_CHANGE, &event);
...
int construct_rtl_link_roaming_sta(_adapter *padapter, ROAMING_INFO_RPT *proam_info)
{
	...
	unsigned char roaming_num_max = sizeof(proam_info->sta_info)/sizeof(proam_info->sta_info[0]);
	...
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		...
		if (psta && psta->phl_sta && !is_broadcast_mac_addr(psta->phl_sta->mac_addr)) {
			...
			roaming_num++;
			if (roaming_num == roaming_num_max)
				break;
		}
	}
...
int rtk_monitor_sta_info(_adapter *padapter, union recv_frame *prframe)
{
	...
	rssi = pattrib->phy_info.signal_strength;
...
void rtw_init_sbwc(_adapter *padapter)
{
	padapter->sbwc_length = SBWC_NUM_PKT_QUEUE;
...
int sbwc_tx(_adapter *padapter, struct sk_buff *skb)
{
	...
	u32 length = padapter->sbwc_length;
	u8 da[6];
	...
	if (length == 0)
		length = SBWC_NUM_PKT_QUEUE;

	_rtw_memcpy(da, skb->data, 6);
	pstat = rtw_get_stainfo(&padapter->stapriv, da);

	if (!pstat || !((pstat->SBWC_mode | padapter->SBWC_mode) & SBWC_MODE_LIMIT_STA_TX))
		return TX_CONTINUE;
	...
	if (pstat->SBWC_mode & SBWC_MODE_LIMIT_STA_TX)
		...
		tx_limit = padapter->SBWC_tx_limit_byte;

	if (tx_count > tx_limit) {
		if (pstat->SBWC_consuming_q) {
			return SBWC_FREE_SKB;
		} else {
			if (skb_queue_len(&pstat->SBWC_txq) < length) {
...
int sbwc_rx(_adapter *adapter, union recv_frame *prframe)
{
		...
		if (!pstat || !((pstat->SBWC_mode | adapter->SBWC_mode) & SBWC_MODE_LIMIT_STA_RX))
		...
		if (pstat->SBWC_mode & SBWC_MODE_LIMIT_STA_RX)
			...
			rx_limit = adapter->SBWC_rx_limit_byte;

		if (rx_count > rx_limit) {
			if (pstat->SBWC_consuming_q) {
				return CORE_RX_DROP;
			} else {
				if (skb_queue_len(&pstat->SBWC_rxq) < SBWC_NUM_PKT_QUEUE) {
					*(unsigned long *)&(skb->cb[_SKB_CB_BWC_]) = (unsigned long)prframe;	// backup prframe pointer
...
void sbwc_timer_handler(void *context)
{
	...
	_queue *pfree_recv_queue = &(padapter->dvobj->recv_compriv.free_recv_queue);
	...
	for (i = 0; i < NUM_STA; i++) {
		...
		while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
			...
			if (mode == SBWC_MODE_DISABLE && !padapter->SBWC_mode)
				continue;
			...
			while(skb_queue_len(&psta->SBWC_rxq)) {
				...
				if (!pskb)
					break;
				prframe = (union recv_frame *)*(unsigned long *)&(pskb->cb[_SKB_CB_BWC_]);
...
int sta_bwc_tx(_adapter *padapter, struct sk_buff *skb)
{
	...
	if (tx_count > tx_limit) {
		if (pstat->sta_bwc_consuming_q) {
			...
			if (skb_queue_len(&pstat->sta_bwc_txq) < STA_BWC_NUM_TXPKT_QUEUE) {
...
void sta_bwc_timer_handler(void *context)
{
...
#ifdef FC_SBWC
	// Do nothing since packets are queued in FC.
#else /* !(FC_SBWC) */
	for (i = 0; i < NUM_STA; i++) {
		phead = &(pstapriv->sta_hash[i]);
		plist = get_next(phead);
	...
	// Recalculate Tx tput limit
	if(((padapter->need_decide_tp_lmt == _TRUE) || (padapter->last_asoc_cnt != padapter->stapriv.asoc_list_cnt)) && (padapter->txduty))
			...
			(padapter->registrypriv.wifi_mib.gbwcmode == GBWC_MODE_LIMIT_IF_TRX))
		{
			if((padapter->GBWC_tx_count + skb->len) > ((padapter->registrypriv.wifi_mib.gbwcthrd_tx * 1024 / 8) / (1000 / GBWC_PERIOD)))
			{
				...
				else
				{
					...
					{
						if(pstat)
							*(unsigned long *)&(skb->cb[_SKB_CB_BWC_]) = (unsigned long)pstat;	// backup pstat pointer
						else
...
void gbwc_timer_handler(void *context){
	...
	if(padapter->registrypriv.wifi_mib.gbwcmode && padapter->GBWC_timer_alive)
	{
		...
		while(skb_queue_len(&padapter->GBWC_tx_queue)) {
			...
			if (!pskb)
				break;
			psta = (struct sta_info *)*(unsigned long *)&(pskb->cb[_SKB_CB_BWC_]);
...
void rtw_parse_vendor_info(struct sta_info *psta, u8 *start, sint left)
{
	...
	if (padapter && padapter->force_vw_sta) {
	...
	for (i = 0; i < QINGDAO_PDA_OUI_NUM; i++) {
		if (_rtw_memcmp(mac_addr, QINGDAO_PDA_OUI[i], WLAN_IEEE_OUI_LEN) == _TRUE) {
			psta->vendor = HT_IOT_PEER_QINGDAO_PDA;
			return;
		}
	}

	for (i = 0; i < FIBOCOM_PDA_OUI_NUM; i++) {
		if (_rtw_memcmp(mac_addr, FIBOCOM_PDA_OUI[i], WLAN_IEEE_OUI_LEN) == _TRUE) {
			psta->vendor = HT_IOT_PEER_FIBOCOM_PDA;
			return;
		}
	}
	...
	do {
		...
		for (i = 0; i < QUALCOMM_OUI_NUM; i++) {
			if (_rtw_memcmp(oui, QUALCOMM_OUI[i], WLAN_IEEE_OUI_LEN) == _TRUE) {
				psta->vendor = HT_IOT_PEER_QUALCOMM;
				return;
			}
		}

...
bool _is_new_freq_range(struct rf_ctl_t *rfctl, u32 hi_orig, u32 lo_orig, u32 hi_new, u32 low_new)
{
	bool needed = _FALSE;
	if (rtw_is_range_a_in_b(hi_new, low_new, hi_orig, lo_orig)) {
		/* request is in current detect range */
		goto exit;
	}

	/* check if request channel coverage has new range and the new range is in DFS range */
	if (!rtw_is_range_overlap(hi_new, low_new, hi_orig, lo_orig)) {
		/* request has no overlap with current */
		needed = _TRUE;
	} else if (rtw_is_range_a_in_b(hi_orig, lo_orig, hi_new, low_new)) {
		/* request is supper set of current */
		if ((hi_new != hi_orig && rtw_chset_is_dfs_range(rfctl->channel_set, hi_new, hi_orig))
			|| (low_new != lo_orig && rtw_chset_is_dfs_range(rfctl->channel_set, lo_orig, low_new)))
			needed = _TRUE;
	} else {
		/* request is not supper set of current, but has overlap */
		if ((low_new < lo_orig && rtw_chset_is_dfs_range(rfctl->channel_set, lo_orig, low_new))
			|| (hi_new > hi_orig && rtw_chset_is_dfs_range(rfctl->channel_set, hi_new, hi_orig)))
			needed = _TRUE;
	}
exit:
	return needed;
...
bool rtw_is_cac_reset_needed(struct rf_ctl_t *rfctl, u8 ch, u8 bw, u8 offset)
{
	...
	bool needed = _FALSE, needed_cur = _FALSE, needed_last = _FALSE;
	u32 cur_hi, cur_lo, hi, lo, last_hi, last_lo;
	...
	if (rfctl->radar_detect_ch == 0 && rfctl->radar_detect_ch_last == 0) {
		needed = _TRUE;
		goto exit;
	}
	...
	if (rfctl->radar_detect_ch != 0) {
		...
		if (cur_hi <= cur_lo) {
			RTW_ERR("cur_hi:%u, cur_lo:%u\n", cur_hi, cur_lo);
			rtw_warn_on(1);
		}
		needed_cur = _is_new_freq_range(rfctl, cur_hi, cur_lo, hi, lo);
	} else {
		needed_cur = _TRUE;
	}

	if (rfctl->radar_detect_ch_last != 0) {
		if (adapter->registrypriv.wifi_mib.dfs_bypass_cac == 1) {
			/* Do not re-do CAC only when primary channel/BW/offset are all same */
			if ((rfctl->radar_detect_ch_last != ch) ||
				(rfctl->radar_detect_bw_last != bw) ||
				(rfctl->radar_detect_offset_last != offset))
				needed_last = _TRUE;
		} else if (adapter->registrypriv.wifi_mib.dfs_bypass_cac == 2) {
			/* Do not re-do CAC when current freq coverage is smaller than the previous setting */
			if (rtw_chbw_to_freq_range(rfctl->radar_detect_ch_last, rfctl->radar_detect_bw_last, rfctl->radar_detect_offset_last, &last_hi, &last_lo) == _FALSE) {
				RTW_ERR("cur detection range ch:%u, bw:%u, offset:%u\n", rfctl->radar_detect_ch_last, rfctl->radar_detect_bw_last, rfctl->radar_detect_offset_last);
				rtw_warn_on(1);
			}
			if (last_hi <= last_lo) {
				RTW_ERR("last_hi:%u, last_lo:%u\n", last_hi, last_lo);
				rtw_warn_on(1);
			}
			needed_last = _is_new_freq_range(rfctl, last_hi, last_lo, hi, lo);
		} else {
			/* do CAC anyway */
			needed_last = _TRUE;
		}

		/* If previous CAC is not ended, do CAC*/
		if (!rfctl->radar_detect_last_cac_ended)
			needed_last = _TRUE;
		/* If previous interface down time is larger than 30s, do CAC */
		if (rtw_get_passing_time_ms(rfctl->radar_detect_last_change_ch_time) >= adapter->registrypriv.wifi_mib.dfs_bypass_cac_to*1000)
			needed_last = _TRUE;
	} else {
		needed_last = _TRUE;
	}
...
exit:
	RTW_INFO("[%s]needed=%d, needed_cur=%d, needed_last=%d\n", __func__, needed, needed_cur, needed_last);
	return needed;
}
...
u8 rtw_dfs_rd_hdl(_adapter *adapter)
{
	...
	if (adapter) {
		...
		genl_wlan_IndicateEvent(adapter, EVENT_RADAR_DETECT, NULL);
#endif
...
#ifdef CONFIG_OPENSYNC
	/* notify hostapd */
	if (IS_CAC_STOPPED(rfctl) && rfctl->cac_on_off) {
		u8 ht = 0;

		rfctl->cac_on_off = 0;

#ifdef CONFIG_80211N_HT
		ht = adapter->mlmepriv.htpriv.ht_option;
#endif

		rtw_cfg80211_cac_event(adapter, dvobj->oper_channel, dvobj->oper_bwmode, dvobj->oper_ch_offset, ht, NL80211_RADAR_CAC_FINISHED);
	}
#endif
	return H2C_SUCCESS;
}
...
static void rtw_dfs_rd_enable(struct rf_ctl_t *rfctl, u8 ch, u8 bw, u8 offset, bool bypass_cac)
{
	...
	if (bypass_cac)
		...
		ht = adapter->mlmepriv.htpriv.ht_option;
#endif
		rtw_cfg80211_cac_event(adapter, ch, bw, offset, ht, NL80211_RADAR_CAC_STARTED);
#endif
	}
	...
	rfctl->radar_detect_last_change_ch_time = rtw_get_current_time();
}
...
static void rtw_dfs_rd_disable(struct rf_ctl_t *rfctl, u8 ch, u8 bw, u8 offset, bool by_others)
{
	...
	if (rfctl->radar_detect_enabled) {
		...
		rfctl->radar_detect_offset_last = offset;
		if (IS_CH_WAITING(rfctl))
			rfctl->radar_detect_last_cac_ended = 0;
		else
			rfctl->radar_detect_last_cac_ended = 1;
		rfctl->cac_start_time = rfctl->cac_end_time = RTW_CAC_STOPPED;
...
apply:
	...
	if (!mib->disable_dfs && needed == _TRUE)
...
void rtw_dfs_rd_detect_onoff(_adapter *padapter, u8 enable)
{
	...
	if (!mib->disable_dfs && needed == _TRUE) {
		if (enable)
			...
			if ((rtw_mi_check_fwstate(padapter, WIFI_UNDER_LINKING|WIFI_UNDER_SURVEY) == _FALSE)
#ifdef CONFIG_RTW_HANDLE_SER_L2
				&& (!rfctl_to_dvobj(rfctl)->ser_L2_inprogress)
#endif
			) {
...
#define DBG_CHOOSE_SHORTEST_WAITING_CH 0
	...
	for (bw = CHANNEL_WIDTH_20; bw <= max_bw; bw++) {
		...
		for (i = 0; i < rfctl->max_chan_nums; i++) {
			...
			for (j = 0; j < rfctl->dfs_skip_ch_num; j++) {
				if (ch == rfctl->dfs_skip_ch_list[j]) {
					skip_ch = _TRUE;
					break;
				}
			}
	...
	if (ch_c != 0) {
		...
		*dec_offset = offset_c;
		...
		rtw_channel_change_indicate(dvobj->padapters[0], cur_ch, ch_c, AUTO_CHANNEL_SEL_INIT);
...
int rtw_construct_radar_detect_info(_adapter *padapter, unsigned char *data)
{
	struct mlme_ext_priv *pmlmeext = &padapter->mlmeextpriv;
	struct radar_detect_info *info = (struct radar_detect_info *)data;

	info->eventid = EVENT_RADAR_DETECT;
	info->channel = pmlmeext->cur_channel;

	return sizeof(struct radar_detect_info);
}
#endif

...
)
{
	...
	*(pbuf + 2) = ext_idx;
	...
	if (len > 0)
		_rtw_memcpy((void *)(pbuf + 3), (void *)source, len);

	if (frlen) {
#ifdef PLATFORM_ECOS
		tmp = get_unaligned(frlen) + (len + 3);
...
inline u8 *rtw_set_ie_ex_bc_change(u8 *buf, u32 *buf_len,
		u8 new_color, u8 countdown)
{
	u8 ie_data[2];

	ie_data[0] = countdown;
	ie_data[1] = (new_color & 0x3F);

	return rtw_set_ie_ex(buf, WLAN_EID_EXTENSION,
			WLAN_EID_EXTENSION_BCC_ANOCE,
			2, ie_data, buf_len);
}
#endif
...
int rtw_parse_wpa2_ie(u8 *rsn_ie, int rsn_ie_len, int *group_cipher,
	int *pairwise_cipher, int *gmcs, u32 *akm, u8 *mfp_opt, u16 *pmkid_cnt)
{
...
int rtw_parse_wapi_ie(u8 *wapi_ie, int wapi_ie_len, int *group_cipher,
	int *pairwise_cipher, int *gmcs, u32 *akm, u16 *cap)
{
	struct wapie_info info;
	int i, ret = _SUCCESS;

	memset(&info, 0, sizeof(struct wapie_info));
	...
	if (gmcs) {
		if (info.gmcs)
			...
			*gmcs = WPA_CIPHER_SMS4; /* default value when absent */
	}
...
u8 *rtw_get_owe_ie(const u8 *in_ie, uint in_len, u8 *owe_ie, uint *owe_ielen)
{
	...
	while (cnt + 1 + 4 < in_len) {
		...
		if ((eid == WLAN_EID_EXTENSION) &&
				(in_ie[cnt + 2] == WLAN_EID_EXTENSION_OWE_DH_PARAM)) {
			oweie_ptr = in_ie + cnt;
...
{
	u8 oui[4] = {0x50, 0x6f, 0x9a, 0x1c};

	return rtw_get_ie_ex(in_ie, in_len, EID_Vendor, oui, 4, owe_ie, owe_ielen);
}
#endif /* CONFIG_HAPD_OWE */
...
static int rtw_ieee802_11_parse_ext_elems(u8 *start, uint elen, struct rtw_ieee802_11_elems *elems)
{
	...
	case WLAN_EID_EXTENSION_HE_OPERATION:
		...
		return 0;
#ifdef CONFIG_HAPD_OWE
	case WLAN_EID_EXTENSION_OWE_DH_PARAM:
		elems->owe_dh_ie = pos;
		elems->owe_dh_ie_len = elen;
		return 0;
#endif
	default:
		return -1;

...
u8 rtw_set_acs_sitesurvey(_adapter *adapter)
{
	...
	for (band = BAND_ON_24G; band < BAND_MAX; band++) {
		...
		ch_num = acs->ch_list_num ? acs->ch_list_num : center_chs_num(CHANNEL_WIDTH_20);
		for (i = 0; i < ch_num && parm.ch_num < RTW_CHANNEL_SCAN_AMOUNT; i++) {
			if (acs->ch_list_num)
				parm.ch[parm.ch_num].hw_value = acs->ch_list[i];
			else
			...
			if (parm.ch[parm.ch_num].hw_value == adapter->mlmeextpriv.cur_channel) {
				free_scanqueue(&adapter->mlmepriv, SCAN_LIST_RM);
			}
#endif
			parm.ch_num++;
		}
...
u16 rtw_get_cur_max_rate(_adapter *adapter)
{
	...
	if (is_supported_ht(psta->phl_sta->wmode)) {
		...
		max_rate = rtw_he_mcs_to_data_rate(psta->phl_sta->chandef.bw
			, rtw_he_shortest_supported_giltf(adapter, psta)
			, &pmlmepriv->hepriv) * 10;
	}

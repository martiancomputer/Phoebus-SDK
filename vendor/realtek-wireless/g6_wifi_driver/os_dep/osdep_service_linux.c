...
#else
		rlen = kernel_read(fp, fp->f_pos, buf + sum, len - sum);
#endif
		if (rlen > 0)
			sum += rlen;
		else if (0 != rlen)
...
void rtw_init_os_handler(struct dvobj_priv *dvobj, struct rtw_os_handler *handler)
{
	...
	if (   (handler->type == RTW_OS_HANDLER_TASKLET)
		...
		smp_store_release(&handler->task_data.csd.node.u_flags, 0);
	}
}
                             ...
                             struct rtw_phl_handler *phl_handler)
{
	...
	case RTW_PHL_SER_HANDLER:
		os_handler->name = "PHL SER";
		break;
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	case RTW_PHL_PE_RX_HANDLER:
		os_handler->name = "PHL PE RX";
		break;
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
#if defined(CONFIG_PHL_TEST_SUITE)
	case RTW_PHL_TEST_HANDLER:
		os_handler->name = "MP Test";
		if (phl_handler->type != RTW_PHL_HANDLER_PRIO_LOW)
			break;
		os_handler->work_data.wq = alloc_workqueue("MP TEST",

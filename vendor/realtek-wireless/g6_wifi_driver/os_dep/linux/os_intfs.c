...
struct net_device *rtw_init_netdev(_adapter *old_padapter)
{
	...
	/* support MTU 2000 for Opensync */
	pnetdev->max_mtu = 2000;
#endif
...
int rtw_os_ndev_register(_adapter *adapter, const char *name)
{
	...
	else if (ndev) //Coverity thinks "ndev" maybe freed by register_netdevice
		RTW_INFO(FUNC_NDEV_FMT" if%d Failed!\n", FUNC_NDEV_ARG(ndev), (adapter->iface_id + 1));
	else
		RTW_INFO("if%d Failed!\n", (adapter->iface_id + 1));
...
#if defined(WIFI6_THER_CTRL) && defined(PLATFORM_LINUX)
	if (is_primary_adapter(adapter)) {
		extern void unreg_wifi6_ther_support(struct net_device *dev);
		unreg_wifi6_ther_support(netdev);
	}
#endif /* WIFI6_THER_CTRL && PLATFORM_LINUX */
...
#ifdef CONFIG_AP_MODE
#ifdef CTC_WIFI_DIAG
	rtw_init_timer(&(pdvobj->ctc_diag_tx_bcn_timer), rtw_ctc_diag_tx_bcn_timer_handlder, pdvobj);
#endif /* CTC_WIFI_DIAG */
#endif
	return pdvobj;
...
#endif
	{
		#ifdef CONFIG_SMP_SWQ_TX
		swq_xmit_task->type = RTW_OS_HANDLER_TASKLET;
		swq_xmit_task->cpu_id = CPU_ID_SWQ_TX;
		swq_xmit_task->name = "SWQ TX";
		swq_xmit_task->id = RTW_HANDLER_CORE_SWQ_TX;
		swq_xmit_task->task_data.func = rtl8192cd_swq_timeout;
		swq_xmit_task->task_data.data = (unsigned long)dvobj;
		rtw_init_os_handler(dvobj, swq_xmit_task);
		#else
...
static _adapter *fc_primary_adapters[BAND_MAX] = {NULL};
static bool wifi_event_cb_registered = _FALSE;

void _set_rx_deamsdu_by_nic(int val)
{
	_adapter *adapter = NULL;
	...
	for (band = 0; band < BAND_MAX; band++) {
		dvobj = (fc_primary_adapters[band]) ? fc_primary_adapters[band]->dvobj : NULL;

		for (iface = 0; dvobj && iface < CONFIG_IFACE_NUMBER; iface++) {
				adapter = dvobj->padapters[iface];
			if (adapter) {
				adapter->rx_deamsdu_by_nic = val;
				adapter->registrypriv.wifi_mib.fc_nat25_en = val;
			}
		}
	}
}

int rtw_fc_wifi_event_cb(rtk_fc_wifi_event_t *event)
{
	switch (event->code) {
	case RTK_FC_WIFI_EVENT_DISABLE_ACC:
		_set_rx_deamsdu_by_nic(0);
		break;
	case RTK_FC_WIFI_EVENT_ENABLE_ACC:
#ifndef CONFIG_OPENSYNC
		_set_rx_deamsdu_by_nic(1);
#endif
		break;
	default:
		RTW_ERR("%s: Event code not supported\n", __func__);
		break;
	}
...
void rtw_fc_register_wifi_event_proc(_adapter *padapter, u8 band)
{
	if (!wifi_event_cb_registered) {
		rtk_fc_wifi_event_handling_callback pfunc = rtw_fc_wifi_event_cb;
		rtk_fc_wifi_event_handling_cb_register(pfunc);
		wifi_event_cb_registered = _TRUE;
	}

	if (is_primary_adapter(padapter))
		fc_primary_adapters[band] = padapter;
}

void rtw_init_rx_deamsdu_by_nic(_adapter *padapter)
{
	if (is_supported_5g(padapter->registrypriv.band_type)
		|| is_supported_6g(padapter->registrypriv.band_type)) {

		/* because nat25, disable bypass_deamsdu in client mode but not a4 mode */
#if 0
		if (MLME_IS_STA(padapter)
			...
			padapter->rx_deamsdu_by_nic = 0;
		}
		else
#endif
			padapter->rx_deamsdu_by_nic = 1;
	} else {
		padapter->rx_deamsdu_by_nic = 1;
	}

	if (is_primary_adapter(padapter)) {
		rtw_fc_register_wifi_event_proc(padapter,
			padapter->phl_role->chandef.band);
	}
}
#endif /* CONFIG_RTW_BYPASS_DEAMSDU */
...
static _adapter *rtw_drv_add_vir_if(struct dvobj_priv *dvobj)
{
	...
	*/
	...
	rtw_init_presta_tbl(&padapter->stapriv);
	res = _SUCCESS;
...
{
#if defined(CONFIG_PHL_RX_PSTS_PER_PKT) && (defined(CONFIG_WIFI_DIAGNOSIS) || defined(DEBUG_MAP_UNASSOC))
...
static void _adapter_open_rtl_wfo_setting(struct net_device *pnetdev)
{
	_adapter *padapter = (_adapter *)rtw_netdev_priv(pnetdev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	...
	if (RTL_WFO_IS_OFLD(dvobj)) {
		...
		/* update resource & device info to DOE and PE */
		if (is_primary_adapter(padapter)) {
			/* ioremap sw_seq to sw_seq_addr in DOE's DMEM */
			if (wfo_dev->doe_sw_seq == NULL) {
				if (wfo_dev->sw_seq_addr)
					wfo_dev->doe_sw_seq = (u16 *)ioremap(
						wfo_dev->sw_seq_addr, 128 * 8 * 2);
				else
					RTW_ERR("DOE's sw_seq_addr is not ready!\n");
			}
			/* resource must be sent at first */
			if (wfo_priv->res_state == PE_RES_NOT_ALLOC) {
				RTW_ERR("%s: buffer resource not allocated!\n", __func__);
			} else if (wfo_priv->res_state == PE_RES_ALLOC) {
				for (i = 0; i < wfo_priv->res_num; i++) {
					rtl_wfo_ipc_set_resource(padapter,
							wfo_priv->resource[i].res_pe_id,
							&wfo_priv->resource[i]);
				}
				wfo_priv->res_state = PE_RES_ASSIGNED;
			}

			rtl_wfo_ipc_send_device_info(padapter);
			rtl_wfo_remap_hb_pe_vaddr(dvobj);
			_set_timer(&wfo_priv->hb_timer, HEARTBEAT_MON_INTERVAL_MS);
		}
		/* update adapter info to PE */
		rtl_wfo_ipc_cfg_adapter_info(padapter, RTL_WFO_OP_ADD);

		if (is_primary_adapter(padapter)) {
			/* update mc2u_disable to FC */
			rtl_wfo_fc_mc2uc_enable(pnetdev,
				!padapter->registrypriv.wifi_mib.mc2u_disable);
		}

		/* update default mib value */
		rtl_wfo_mib_init(dvobj, padapter);
	}

	/* enable FC callback hook */
	rtl_wfo_fc_callback_hook_update(pnetdev, 1);
}

static void _adapter_close_rtl_wfo_setting(struct net_device *pnetdev, bool sta_mode)
{
	_adapter *padapter = (_adapter *)rtw_netdev_priv(pnetdev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);

	/* disable FC callback hook */
	rtl_wfo_fc_callback_hook_update(pnetdev, 0);


	if (RTL_WFO_IS_OFLD(dvobj)) {
		/* delete adapter info to PE */
		rtl_wfo_ipc_cfg_adapter_info(padapter, RTL_WFO_OP_DELETE);
		/* stop WFO */
		if (dvobj->wfo_en && _is_all_other_iface_down(dvobj, padapter)) {
			rtl_wfo_flush_fc_staid();
			//PHL025+: _phl_sw_tx_pause() called in netdev_close()
...
static int _netdev_open(struct net_device *pnetdev)
{
	...
	struct acs_priv *acspriv = adapter_to_acs(padapter);
#endif
	...
	if (!rtw_hw_is_init_completed(dvobj)) {
		...
		if (padapter->registrypriv.wifi_mib.fw_ext_dyn_update) {
			//Debug and test mode
			dvobj->phl_com->dev_cap.fw_cap.fw_src = RTW_FW_SRC_EXTNAL;
			dvobj->phl_com->dev_cap.fw_cap.fw_path = RTW_FW_PATH_CONFIG;
		} else {
#ifdef CONFIG_RTW_FW_BUILTIN
			dvobj->phl_com->dev_cap.fw_cap.fw_src = RTW_FW_SRC_INTNAL;
#else
			dvobj->phl_com->dev_cap.fw_cap.fw_src = RTW_FW_SRC_EXTNAL;
#endif
			dvobj->phl_com->dev_cap.fw_cap.fw_path = RTW_FW_PATH_ROOTFS;
		}
#endif /* CONFIG_DYN_FW_UPDATE */
		...
		dvobj->phl_com->dev_sw_cap.antdiv_sup = padapter->registrypriv.wifi_mib.antdiv?1:0;
#endif
	...
	if (padapter->netif_up == _FALSE) {
		...
		padapter->up_time = 0;
		padapter->up_jiffies = _rtw_get_current_time();
		padapter->sta_dump_to = 0;
		padapter->sta_dump_bitmap = 0xffff;
		padapter->sta_ru_dump_bitmap = 0x1fff;
		...
		#ifdef CONFIG_RTW_SW_LED_SHARED
		if (adapter_led_share[RTW_SHARED_LED_ID]) {
			struct led_priv *ledpriv = adapter_to_led(adapter_led_share[RTW_SHARED_LED_ID]);
			_set_timer(&ledpriv->led_timer, ledpriv->interval);
			RTW_INFO("[%s] init led timer\n", __func__);
		}
		#endif
		if (is_primary_adapter(padapter))
...
#endif
		//RT_TRX_TRAFFIC
		realtime_traffic_timer_init(padapter);
	}
...
#ifndef CONFIG_OPENSYNC
#ifdef CONFIG_RTW_BYPASS_DEAMSDU
	rtw_init_rx_deamsdu_by_nic(padapter);
#endif
#endif
...
#ifdef CONFIG_ETHER_PKT_AGG
	/* update mc2u_disable to FC */
	rtl_wfo_fc_mc2uc_enable(pnetdev, !padapter->registrypriv.wifi_mib.mc2u_disable);
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	_adapter_open_rtl_wfo_setting(pnetdev);
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
...
static bool dev_sta_mode(_adapter *padapter)
{
	NDIS_802_11_NETWORK_INFRASTRUCTURE network_type;
	struct wireless_dev *rtw_wdev = padapter->rtw_wdev;

	network_type = nl80211_iftype_to_rtw_network_type(rtw_wdev->iftype);
	return (network_type == Ndis802_11Infrastructure);
}
...
int netdev_close(struct net_device *pnetdev)
{
	...
	bool sta_mode = dev_sta_mode(padapter);
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	_adapter_close_rtl_wfo_setting(pnetdev, sta_mode);
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
...
#ifdef CONFIG_RTW_MBO
	rtw_bss_termination_cancel_timer(padapter);
#endif
	...
	if (rtw_get_intf_type(padapter) == RTW_HCI_PCIE)
	{
		...
		while(skb_queue_len(&padapter->GBWC_rx_queue))
		{
			...
			if (!pskb)
				break;
			/* free skb */
			rtw_free_recvframe((union recv_frame *)*(unsigned long *)&(pskb->cb[_SKB_CB_BWC_]), pfree_recv_queue);
		}
		padapter->GBWC_consuming_Q = 0;
		padapter->GBWC_timer_alive = 0;
	}
...
#ifdef CONFIG_RTW_CROSSBAND_REPEATER_SUPPORT
	if(sta_mode && !is_primary_adapter(padapter))
	{
#ifdef CONFIG_CROSSBAND_REPEATER_SUPPORT_WIFI5_WIFI6
...
#ifdef CONFIG_RTW_BYPASS_DEAMSDU
	if (is_primary_adapter(padapter) && padapter->phl_role)
		fc_primary_adapters[padapter->phl_role->chandef.band];
#endif
	rtw_hw_iface_deinit(padapter);
...
#ifdef CONFIG_RTW_A4_STA
	if (sta_mode && padapter->a4_enable)
...
enum rtw_phl_status rtw_ch_bw_recover(struct dvobj_priv *dvobj)
{
...
u8 rtw_core_phl_restart(struct dvobj_priv *dvobj)
{
	...
	phl_status = rtw_phl_rf_off(dvobj->phl);
	if (phl_status != RTW_PHL_STATUS_SUCCESS)
		RTW_ERR("%s rtw_phl_rf_off return status fail %d!\n", __func__, phl_status);
	phl_status = rtw_phl_suspend_all_sta(dvobj->phl, PHL_CMD_DIRECTLY);
	if (phl_status != RTW_PHL_STATUS_SUCCESS)
		RTW_ERR("%s rtw_phl_suspend_all_sta return status fail %d!\n", __func__, phl_status);
	phl_status = rtw_phl_ser_l2_done_notify(dvobj->phl_com);
	if (phl_status != RTW_PHL_STATUS_SUCCESS)
		RTW_ERR("%s rtw_phl_ser_l2_done_notify return status fail %d!\n", __func__, phl_status);
	phl_status = rtw_phl_rf_on(dvobj->phl);
	if (phl_status != RTW_PHL_STATUS_SUCCESS)
		RTW_ERR("%s rtw_phl_rf_on return status fail %d!\n", __func__, phl_status);
	phl_status = rtw_phl_resume_all_sta(dvobj->phl, PHL_CMD_DIRECTLY);
	if (phl_status != RTW_PHL_STATUS_SUCCESS)
		RTW_ERR("%s rtw_phl_resume_all_sta return status fail %d!\n", __func__, phl_status);
	phl_status = rtw_ch_bw_recover(dvobj);
	if (phl_status != RTW_PHL_STATUS_SUCCESS)
		RTW_ERR("%s rtw_ch_bw_recover return status fail %d!\n", __func__, phl_status);
...
void rtw_recover_device(struct dvobj_priv *dvobj)
{
	...
	#ifdef CONFIG_RTL_WIFI_OFFLOAD
	if (RTL_WFO_IS_OFLD(dvobj) && !dvobj->wfo_pe_stopped)
		rtl_wfo_start_pe(pri_adapter);
	#endif

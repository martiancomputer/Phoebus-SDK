		 ...
		 struct iw_point *wrqu, char *extra)
{
	...
	if (_FAIL == rtw_pwr_wakeup(padapter)) {
		RTW_ERR("pwr wakeup fail !!!\n");
		return -EPERM;
	}

	if (!rtw_is_adapter_up(padapter)) {
		RTW_ERR("adapter_up fail !!!\n");
		return -EPERM;
	}
	...
	rtw_mp_phl_config_arg(padapter, MP_CONFIG_CMD_SET_PHY_INDEX);
	       ...
	       struct iw_point *wrqu, char *extra)
{
	...
	if (strncmp(input, "setbssid=", 8) == 0) {
			...
			sprintf(extra, "Mac Received packet OK:%d CRC error:%d",
				...
				u8 i = 0;
				int setflt = 0;
				u8 dummy_mac[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

				_rtw_memset((void *)&rx_arg, 0, sizeof(struct mp_rx_arg));

				if (7 == sscanf(input, "setflt=%d,ta=%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx", &setflt, &rx_arg.mp_fltr_mac_ta[0],
															          ...
															          &rx_arg.mp_fltr_mac_ta[5]))
				{
					...
					printk("\n");*/
					sprintf(extra , "set TA: %02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx\n",  rx_arg.mp_fltr_mac_ta[0],
																				rx_arg.mp_fltr_mac_ta[1],
																				rx_arg.mp_fltr_mac_ta[2],
																				rx_arg.mp_fltr_mac_ta[3],
																				rx_arg.mp_fltr_mac_ta[4],
																				rx_arg.mp_fltr_mac_ta[5]);
					if (setflt) {
						pmppriv->setflt = _TRUE;
						rx_arg.setflt = _TRUE;
					} else {
						pmppriv->setflt = _FALSE;
						rx_arg.setflt = _FALSE;
					}
				}
...
int rtw_mp_agc_fix_gain(struct net_device *dev,
	       struct iw_request_info *info,
	       struct iw_point *wrqu, char *extra)
{
	_adapter *padapter = rtw_netdev_priv(dev);
	struct mp_other_arg other_arg = {0};
	u8 en=0, elna_idx=0, lna_idx=0, tia_idx=0, rxbb_idx=0, path=0;
	...
	*(extra + wrqu->length) = '\0';
	...
	if (sscanf(extra, "%hhd,%hhd,%hhd,%hhd,%hhd,%hhd", &en, &elna_idx, &lna_idx, &tia_idx, &rxbb_idx, &path) == 6)
		RTW_INFO("get input: en=%hhd,elna_idx=%hhd,lna_idx=%hhd,tia_idx=%hhd,rxbb_idx=%hhd,path=%hhd\n", 
		en, elna_idx, lna_idx, tia_idx, rxbb_idx, path);
	else {
		RTW_PRINT("Input invalid format\n");
		return -EFAULT;
	}

	other_arg.mp_class = MP_CLASS_OTHER;
	other_arg.cmd = MP_OTHER_CMD_SET_AGC_FIX_GAIN;
	other_arg.cmd_ok = false;
	other_arg.en = en;
	other_arg.elna_idx = elna_idx;
	other_arg.lna_idx = lna_idx;
	other_arg.tia_idx = tia_idx;
	other_arg.rxbb_idx = rxbb_idx;
	other_arg.path = path;

	rtw_mp_set_phl_cmd(padapter, (void*)&other_arg, sizeof(struct mp_other_arg));
	
	return 0;
}

int rtw_mp_rssi_rx_k(struct net_device *dev,
	       struct iw_request_info *info,
	       struct iw_point *wrqu, char *extra)
{
	_adapter *padapter = rtw_netdev_priv(dev);
	...
	*(extra + wrqu->length) = '\0';
	...
	if (strncmp(extra, "rssia", 5) == 0)
		rssi_path = 0;
	else if (strncmp(extra, "rssib", 5) == 0)
		rssi_path = 1;

	RTW_INFO("%s:Query RSSI Path:%d\n", __func__, rssi_path);
	_rtw_memset((void *)&rx_arg, 0, sizeof(struct mp_rx_arg));

	rx_arg.rf_path = rssi_path;
	rx_arg.rx_path = rssi_path;

	rtw_mp_phl_rssi_rx_k(padapter, &rx_arg);
	{
		/*output floating point format*/
		s16 convert_rssi = (rx_arg.rssi * 100 / 2) - 110 * 100;
		s8 d = convert_rssi / 100;
		s8 f = convert_rssi % 100;
		if (rx_arg.cmd_ok)
			sprintf(extra, "Path%d RSSI=%d.%d\n", rssi_path, d, ((f >= 0) ? f : -f));
		else
			sprintf(extra, "Path%d, RSSI Fail\n", rssi_path);
	}
	wrqu->length = strlen(extra) + 1;
			...
			u8 path)
{
	...
	rtw_phl_tssi_current_slope_apply(GET_HAL_INFO(adapter_to_dvobj(padapter)), gain_diff, cw_diff, path);
			...
			struct iw_point *wrqu, char *extra)
{
	...
	_rtw_memset(extra, 0, wrqu->length + 1);
...
//	_enter_critical_mutex(&(adapter_to_dvobj(padapter)->ioctrl_mutex), NULL);
	...
	case MP_GET_TSSI_SLP_B:
		...
		break;
	case MP_AGC_FIX_GAIN:
		RTW_INFO("set MP_AGC_FIX_GAIN\n");
		status = rtw_mp_agc_fix_gain(dev, info, wrqu, extra);
		break;
	case MP_RSSI_RX_K:
		RTW_INFO("mp_get MP_RSSI_RX_K\n");
		status = rtw_mp_rssi_rx_k(dev, info, wrqu, extra);
		break;
	default:
		status = -EIO;
	}

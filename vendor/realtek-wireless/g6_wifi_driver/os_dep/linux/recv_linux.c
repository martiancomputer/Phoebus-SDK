...
int rtw_checkUDPandU2M(struct sk_buff *pskb)
{
	int MultiIP;
	int do_u2m = 0;
...
int rtw_checkV6UDPandU2M(struct sk_buff *pskb)
{
	struct ipv6hdr *iph;
	unsigned char *DDA;
	int do_u2m = 0;
...
void rtw_update_sta_mcast_entry(struct sta_priv *pstapriv, u8 *mac, u8 *mcmac, bool add)
{
	...
	if (found == _TRUE) {
		if (add == _FALSE) {
			...
			psta->ipmc_aging_time[i] = padapter->registrypriv.wifi_mib.mc2u_aging_time;
	} else {
		if (add == _TRUE) {
			if (psta->ipmc_num < MAX_IP_MC_ENTRY) {
				RTW_INFO("[%s] add %pM to sta=%pM mcmac entry\n", __func__, mcmac, mac);
				_rtw_memcpy(psta->ipmc[psta->ipmc_num].mcmac, mcmac, 6);
				psta->ipmc_aging_time[psta->ipmc_num] = padapter->registrypriv.wifi_mib.mc2u_aging_time;
				psta->ipmc_num++;
			}
		}
	}
...
void rtw_igmp3_report_check(struct sta_priv	*pstapriv, struct sk_buff *pkt, struct igmphdr *igmph)
{
	...
	while (rec_id < ntohs(igmpv3->ngrec)) {
		...
		if (op != 0) {
			...
			if (rtw_m2u_check(pstapriv->padapter, pkt, (unsigned char *)&group) != MC2U_FORBID)
#endif
...
void rtw_mld_icmpv6_report_check(struct sta_priv	*pstapriv, struct sk_buff *pkt, u8 *ptr)
{
	...
	if (ptr[0] == ICMPV6_MLD2_REPORT) {
		...
		for (i = 0; i < grpnum; i++) {
			...
			if (op != 0) {
#ifdef CONFIG_TX_MCAST2UNI
				if (rtw_m2u_check(pstapriv->padapter, pkt, (unsigned char *)&grec->grec_mca) != MC2U_FORBID)
#endif
...
void rtw_netif_rx_hdl(unsigned long data)
{
	...
	int ret;
	u16 next_idx;
	u32 batch_num = dvobj->netif_rx_batch, dequeue_num = 0;
	u8 issue_next = 0;

	next_idx = ring->read_idx;
	while (next_idx != ring->write_idx) {
		if (batch_num && dequeue_num >= batch_num) {
			issue_next = 1;
			break;
		}
		...
		ret = rtw_netif_rx(padapter->pnetdev, pskb);
	...
	if (batch_num) {
		if (issue_next) {
			dvobj->netif_rx_batch_part++;
			rtw_tasklet_schedule(&dvobj->netif_rx_task);
		} else {
			dvobj->netif_rx_batch_all++;
		}
	}
}
...
void rtw_netif_rx_init(struct dvobj_priv *dvobj)
{
	...
	_rtw_spinlock_init(&dvobj->netif_rx_ring.lock);
}
...
void rtw_netif_rx_deinit(struct dvobj_priv *dvobj)
{
	...
	_rtw_spinlock_free(&dvobj->netif_rx_ring.lock);
}
...
void rtw_rx_recycle_enq(struct dvobj_priv *dvobj, u8* recvpkt)
{
	...
	next_idx = (ring->write_idx + 1) & (MAX_RX_RECYCLE_RING_ENTRY - 1);
	if (next_idx == ring->read_idx) {
		ring->full++;
		_rtw_spinunlock_bh(&ring->lock);
...
void rtw_rx_recycle_hdl(unsigned long data)
{
	...
	next_idx = ring->read_idx;
	while (next_idx != ring->write_idx) {
		next_idx = (next_idx + 1) & (MAX_RX_RECYCLE_RING_ENTRY - 1);
...
sint rtw_os_rx_init(struct dvobj_priv *dvobj)
{
	sint ret = _SUCCESS;
#ifdef CONFIG_SMP_NETIF_RX
...
#endif
	ret = _rtw_init_recv_com_priv(&dvobj->recv_compriv);
	return ret;
}
...
#endif
	_rtw_free_recv_com_priv(&dvobj->recv_compriv);
}
						...
						union recv_frame *rframe)
{
	...
	if (pkt) {
		...
		rtw_delay_measure_done_rx(padapter, rframe, rframe->u.hdr.psta);
#endif
		...
		if (MLME_IS_AP(padapter)) {
			...
			if (_rtw_memcmp(ehdr->h_dest, adapter_mac_addr(padapter), ETH_ALEN) == _FALSE) {
				...
				/* Indicate to bridge, not forward by driver */
				psta = NULL;
#endif
...
#endif /* defined(CONFIG_ETHER_PKT_AGG) || defined(CONFIG_RTL_WIFI_OFFLOAD) */
		if (rtw_process_u2mc(padapter,pkt) == _FAIL)
			goto drop;
		...
		if (MLME_IS_STA(padapter) && (padapter->a4_enable == 1))
		{
			if(IS_MCAST(ehdr->h_dest))
			{
				...
				if(rtw_check_a4_loop_entry(padapter, ehdr->h_source))
				{
					...
					if(padapter->pkt_trace_enable)
						RTW_RX_TRACE(padapter,&pattrib->pktinfo);
#endif /* RTW_CORE_PKT_TRACE */
					goto drop;
				}
#else /* CONFIG_A4_LOOPBACK */
		...
		pkt->priority = rframe->u.hdr.attrib.priority;
		{
			struct rtw_presta_s *presta = NULL;
			struct sta_info *psta_src = NULL;
			if (padapter->stapriv.rtw_presta_s_cnt
				&& (presta = rtw_retrieve_presta(&padapter->stapriv, ehdr->h_source))
				&& presta->has_mscs) {
				mscs_rec_pkt(presta->mscs_tbl, QOS_MSCS_TBL_BITL, &presta->mscs_template, pkt);
			} else if ((psta_src = rtw_get_stainfo(pstapriv, ehdr->h_source))
				&& psta_src->has_mscs) {
				mscs_rec_pkt(psta_src->mscs_tbl, QOS_MSCS_TBL_BITL, &psta_src->mscs_template, pkt);
			}
		}
#endif

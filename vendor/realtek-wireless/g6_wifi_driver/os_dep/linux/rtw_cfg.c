...
MODULE_PARM_DESC(rtw_tx_mode, "wifi tx mode, 0: normal, 1: veriwave, 2: airtime");
#endif
...
MODULE_PARM_DESC(rtw_rx_ampdu_sz_limit_1ss, "RX AMPDU size limit for 1SS link of each BW, 0xFFFF: no limitation");
...
MODULE_PARM_DESC(rtw_rx_ampdu_sz_limit_2ss, "RX AMPDU size limit for 2SS link of each BW, 0xFFFF: no limitation");
...
MODULE_PARM_DESC(rtw_rx_ampdu_sz_limit_3ss, "RX AMPDU size limit for 3SS link of each BW, 0xFFFF: no limitation");
...
MODULE_PARM_DESC(rtw_rx_ampdu_sz_limit_4ss, "RX AMPDU size limit for 4SS link of each BW, 0xFFFF: no limitation");
...
u8 wlan0_efem_type = CONFIG_RTW_USING_EFEM;
u8 wlan1_efem_type = CONFIG_RTW_USING_EFEM;

module_param(wlan0_efem_type, byte, 0644);
MODULE_PARM_DESC(wlan0_efem_type, "wlan0 using: ifem:0x11, efem:0xee");

module_param(wlan1_efem_type, byte, 0644);
MODULE_PARM_DESC(wlan1_efem_type, "wlan1 using: ifem:0x11, efem:0xee");
#endif
...
static void rtw_ap_rfe_config(struct dvobj_priv *dvobj)
{
	...
	switch (phl_com->dev_sw_cap.band_sup){
		...
		case BAND_CAP_2G:
			...
			#ifdef CONFIG_WLAN0_RFE_TYPE_94
				RTW_PRINT("CONFIG_WLAN0_RFE_TYPE_94\n");
				set_rfe_type = 94;
			#endif
				break;
		...
		case BAND_CAP_2G:
			...
			#ifdef CONFIG_WLAN1_RFE_TYPE_94
				RTW_PRINT("CONFIG_WLAN1_RFE_TYPE_94\n");
				set_rfe_type = 94;
			#endif
				break;
			default:
...
#ifdef CONFIG_RTW_EFEM_IDENTIFY_WITH_FLASH
/* Should be call after read band & rfe type*/
u8 rtw_check_efem_type(struct dvobj_priv *dvobj)
{
	struct rtw_phl_com_t *phl_com = GET_HAL_DATA(dvobj);
	u8 ret = _SUCCESS;
	u8 efem_type = wlan0_efem_type;
	u8 wlanidx = 0;
...
#endif
#ifdef CONFIG_WLAN0_RFE_TYPE_50
			if (wlan0_efem_type != CONFIG_RTW_USING_IFEM)
#else
			if (wlan0_efem_type != CONFIG_RTW_USING_EFEM)
#endif
			{
				ret = _FAIL;
			}
...
#endif
#ifdef CONFIG_WLAN1_RFE_TYPE_50
			if (wlan1_efem_type != CONFIG_RTW_USING_IFEM)
#else
			if (wlan1_efem_type != CONFIG_RTW_USING_EFEM)
#endif
			{
				efem_type = wlan1_efem_type;
				wlanidx = 1;
				ret = _FAIL;
			}
	...
	{
		DBGP("\n\n!!!!!!!!!"DRV_NAME"!!!!!!!!\n");
		DBGP("wlan%d FEM setting is dismatch: flash setting(0x%02x), config rfe_type(%d)\n",
				wlanidx,
				efem_type,
				phl_com->dev_sw_cap.rfe_type);
		DBGP("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n\n");
	}
...
#ifdef CONFIG_LOAD_PHY_PARA_FROM_MODULE_PARA
uint rtw_rfe_type_5g = 53;
module_param(rtw_rfe_type_5g, uint, 0644);
MODULE_PARM_DESC(rtw_rfe_type_5g, "default 5G RFE type, init value:53");

char *rtw_fem_name_5g = "KCT8575HP";
module_param(rtw_fem_name_5g, charp, 0644);
MODULE_PARM_DESC(rtw_fem_name_5g, "default 5G Fem name KCT8575HP");

uint rtw_rfe_type_2g = 53;
module_param(rtw_rfe_type_2g, uint, 0644);
MODULE_PARM_DESC(rtw_rfe_type_2g, "default 2G RFE type, init value:53");

char *rtw_fem_name_2g = "VC5337";
module_param(rtw_fem_name_2g, charp, 0644);
MODULE_PARM_DESC(rtw_fem_name_2g, "default 2G Fem name VC5337");
#endif /* CONFIG_LOAD_PHY_PARA_FROM_MODULE_PARA */
...
*/
	...
	int rtw_load_phy_file = (BIT1 | BIT3 | BIT4 | BIT5 | BIT6 | BIT8);
#else
...
uint rtw_wfo_band = 0x3;
module_param(rtw_wfo_band, uint, 0644);
MODULE_PARM_DESC(rtw_wfo_band, "wfo_band_cap: b0 - 2G, b1 - 5G");
...
void rtw_core_update_default_setting (struct dvobj_priv *dvobj)
{
	...
	do {
		...
		if (phl_com->phy_sw_cap[0].band_sup & BAND_CAP_2G) {
			rx_buf_size = RX_BUF_SIZE_RXQ_1_CORE;
		} else {
			rx_buf_size = RX_BUF_SIZE_RXQ_0_CORE;
		}
		phl_com->proto_sw_cap[0].max_amsdu_len =   (rx_buf_size > 12000)
							 ? 2
							 : (  (rx_buf_size > 8000)
							    ? 1 : 0);
	} while (0);
	...
	/* Note: this rewrite default fw_src settings in phl_sw_cap_init() */
...
#endif
	dvobj->phl_com->dev_sw_cap.fw_cap.fw_path = RTW_FW_PATH_ROOTFS;

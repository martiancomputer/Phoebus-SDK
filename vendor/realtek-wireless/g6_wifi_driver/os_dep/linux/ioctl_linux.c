			     ...
			     union iwreq_data *wrqu, char *extra)
{
	...
	if (wrqu->data.length > (IFNAMSIZ-1))
		return -EFAULT;

	if (copy_from_user(new_ifname, wrqu->data.pointer, wrqu->data.length))
		return -EFAULT;
...
static int rtw_set_encryption(struct net_device *dev, struct ieee_param *param, u32 param_len)
{
	...
	if (!psta && check_fwstate(pmlmepriv, WIFI_AP_STATE)) /*  */ { /* group key */
		if (param->u.crypt.set_tx == 1) {
			...
			if (pbcmc_sta)
				rtw_phl_cmd_set_sta_seciv(padapter->dvobj->phl, padapter->phl_role, pbcmc_sta->phl_sta, pbcmc_sta->dot11txpn.val + 1, PHL_CMD_DIRECTLY, 0);
		}
...
_clear_path:
	...
	_update_beacon(padapter, 0xFF, NULL, _TRUE, 0, "clear vendor ie");
	return -EFAULT;
}
#endif
			...
			union iwreq_data *wdata, char *extra)
{
	...
	if (subcmd != MIB_SET && subcmd != PHL_TEST_SET &&
	...
	if (subcmd < MP_NULL) {
		...
		rtw_msleep_os(10); /* delay 5ms for sending pkt before exit adb shell operation */
			...
			case STA_NUM_GET:
				...
				status = rtw_sta_num_get(dev , info , wdata , extra, 1);
				break;
			case STA_MCAST_GET:
				RTW_INFO("get case STA_MCAST_GET\n");
				status = rtw_sta_mcast_get(dev , info , wdata , extra);
				break;
			case WMM_STATS_GET:
				RTW_INFO("get case WMM_STATS_GET\n");
				status = rtw_wmm_stats_get(dev , info , wdata , extra);
				break;
#ifdef CONFIG_DELAY_MEASUREMENT
			case STA_DELAY_GET:
				RTW_INFO("get case STA_DELAY_GET\n");
				status = rtw_sta_delay_measure_get(dev , info , wdata , extra);
				break;
#endif
			case MIB_GET:
 ...
 * --- sub-ioctls definitions --- */
	...
	{ STA_MCAST_GET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "get_sta_mcast" },
	{ WMM_STATS_GET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "get_wmm_stats" },
#ifdef CONFIG_DELAY_MEASUREMENT
	{ STA_DELAY_GET, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "get_sta_delay" },
#endif
	...
	{ MP_AGC_FIX_GAIN, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mp_agc_fix_gain" },
	{ MP_RSSI_RX_K, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
				IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "mp_rssi_rx_k" },

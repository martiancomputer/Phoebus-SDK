...
#if 0
	/* Remove according to CTC FTTR's requirement */
...
ssize_t proc_ctcwifi_diag_enable_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	if (en) {
		...
		_set_timer(&padapter->dvobj->ctc_diag_tx_bcn_timer, 100);
	}
	else {
		temp_diag_duration[idx] = 0;
		_cancel_timer(&padapter->dvobj->ctc_diag_tx_bcn_timer, &_cancelled);
	}
...
unsigned char rtw_ctcwifi_get_enable(_adapter *adapter)
{
	int n;
	if (rtw_get_oper_ch(adapter) > 14)
		n = CTCWIFI_5G;
	else
		n = CTCWIFI_2G;

	return diag_enable[n];
}
...
ssize_t proc_set_macaddr_acl(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	char tmp[18 * NUM_ACL + 32] = {0};  /* (macaddr + whitespace) * NUM_ACL + command */
	u8 period;
	char cmd[32] = {0};
	u8 mode;
	u8 addr[ETH_ALEN] = {0};
...
ssize_t proc_set_bcc_trigger(struct file *file, const char __user *buffer,
	size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	...
	char tmp[32] = {0};
	u8 bc, cntdown, i;
	u8 he_op_ie_ex = WLAN_EID_EXTENSION_HE_OPERATION;
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		int num = sscanf(tmp, "%hhu %hhu", &bc, &cntdown);

		if (num < 2)
			goto exit;

		rfctl->bcc_color = bc;
		rfctl->bcc_cntdown = cntdown;
		rfctl->bcc_set_ie = 1;
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			proto_cap = &(wrole->proto_role_cap);

			proto_cap->bc_disabled = 1;
			proto_cap->bsscolor = bc;

			update_beacon(padapter, WLAN_EID_EXTENSION, &he_op_ie_ex, _TRUE, 0);
		}
	}
...
void _parse_ch_list_str_to_table(u8 *ch_list, u8 *ch_list_num, char* str)
{
	char remain[128] = {0};
	int num;
	int ch = 0;

	num = sscanf(str, "%d,%s", &ch, remain);

	if (num < 1 || ch < 1)
		return;

	ch_list[*ch_list_num] = (u8) ch;
	(*ch_list_num)++;

	if (num == 2)
		_parse_ch_list_str_to_table(ch_list, ch_list_num, remain);
}
...
static int proc_get_mib_dfs(struct seq_file *m, void *v)
{
	...
	RTW_PRINT_SEL(m, "radar_detect_ch_last: %u\n", rfctl->radar_detect_ch_last);
	RTW_PRINT_SEL(m, "radar_detect_bw_last: %u\n", rfctl->radar_detect_bw_last);
	RTW_PRINT_SEL(m, "radar_detect_offset_last: %u\n", rfctl->radar_detect_offset_last);
	RTW_PRINT_SEL(m, "radar_detect_count: %u\n", rfctl->radar_detect_count);
	RTW_PRINT_SEL(m, "last change channel time: %u s\n", rtw_get_passing_time_ms(rfctl->radar_detect_last_change_ch_time)/1000);
	RTW_PRINT_SEL(m, "dfs_bypass_cac: %d\n", GET_PRIMARY_ADAPTER(adapter)->registrypriv.wifi_mib.dfs_bypass_cac);
	RTW_PRINT_SEL(m, "dfs_bypass_cac_to: %d s\n", GET_PRIMARY_ADAPTER(adapter)->registrypriv.wifi_mib.dfs_bypass_cac_to);
...
#ifdef CONFIG_OPENSYNC
			wdev->cac_start_time = rfctl->cac_start_time;
			wdev->cac_time_ms = rfctl->cac_end_time - rfctl->cac_start_time;
#endif
		}
	}
...
static int proc_get_dfs_skip_ch(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	...
	RTW_PRINT_SEL(m, "dfs_skip_ch_list (%d): ", rfctl->dfs_skip_ch_num);
	for (i = 0; i < rfctl->dfs_skip_ch_num; i++)
		RTW_PRINT_SEL(m, " %d", rfctl->dfs_skip_ch_list[i]);
	RTW_PRINT_SEL(m, "\n");

	return 0;
}

static ssize_t proc_set_dfs_skip_ch(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct rf_ctl_t *rfctl = adapter_to_rfctl(padapter);
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		memset(rfctl->dfs_skip_ch_list, 0, sizeof(rfctl->dfs_skip_ch_list));
		rfctl->dfs_skip_ch_num = 0;
		_parse_ch_list_str_to_table(rfctl->dfs_skip_ch_list, &rfctl->dfs_skip_ch_num, tmp);
	}
...
ssize_t proc_set_rx_ampdu_size_limit(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		int i;
		int num = sscanf(tmp, "%hhu %hu %hu %hu %hu"
...
static ssize_t proc_set_change_bss_chbw(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		if (ifbmp) {
			#ifdef CONFIG_CMCC_FEATURE
			rtw_channel_change_indicate(adapter, pmlmeext->cur_channel, ch, AUTO_CHANNEL_SEL_EXTERNAL);
...
#else
static int proc_get_tx_power_limit_wkard(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	bool power_limit = (bool)adapter->registrypriv.wifi_mib.power_limit;
	u8 chnl = adapter->mlmeextpriv.cur_channel;
	u8 lmt = 0, rate;

	if (power_limit) {
		if (chnl <= 14)
			rate = RTW_DATA_RATE_CCK11;
		else
			rate = RTW_DATA_RATE_OFDM54;

		lmt = rtw_phl_get_power_limit(dvobj->phl, HW_BAND_0, rate, 0, 0, 0, chnl);
	}
	else
		lmt = 0;
...
static void rtw_acs_info_dump(struct seq_file *m, _adapter *a)
{
	...
	RTW_PRINT_SEL(m, "[acs] nhm: %d\n", acs->use_nhm);
	RTW_PRINT_SEL(m, "[acs] scan_mode: %d\n", acs->scan_mode);
	RTW_PRINT_SEL(m, "[acs] duration: %d (ms)\n", acs->duration);
	RTW_PRINT_SEL(m, "[acs] dacs_interval: %d (s)\n", acs->dacs_interval);
	RTW_PRINT_SEL(m, "[acs] ch_list (%d): ", acs->ch_list_num);
	for (i = 0; i < acs->ch_list_num; i++)
		RTW_PRINT_SEL(m, " %d", acs->ch_list[i]);
	RTW_PRINT_SEL(m, "\n");
	RTW_PRINT_SEL(m, "[acs] candidate_ch (%d): ", acs->candidate_ch_num);
	for (i = 0; i < acs->candidate_ch_num; i++)
		RTW_PRINT_SEL(m, " %d", acs->candidate_ch[i]);
	RTW_PRINT_SEL(m, "\n");
	RTW_PRINT_SEL(m, "[acs] best_channel: %d\n", acs->best_channel);
	...
	RTW_PRINT_SEL(m, "%7s %3s %5s %5s %6s %7s %5s %5s %9s\n", "channel", "clm", "noise", "count", "clm_fa", "overlap", "ban", "score", "grp_score");

	for (idx = 0; idx < MAX_ACS_INFO; idx++) {
		if (!acs_r[idx].channel)
			break;

		RTW_PRINT_SEL(m, "%7d %3d %5d %5d %6d %7s %5s %5d %9d\n",
			acs_r[idx].channel,
			acs_r[idx].clm_ratio,
			acs_r[idx].noise,
			acs_r[idx].rx_count,
			acs_r[idx].clm_fa_ratio,
			(acs_r[idx].overlap) ? "TRUE" : "FALSE",
			is_acs_ban_channel(a, acs_r[idx].channel) ? "TRUE" : "FALSE",
			acs_r[idx].score,
			acs_r[idx].group_score);
	}
...
static int proc_get_chan_info(struct seq_file *m, void *v)
{
	...
	ssc_chk = rtw_sitesurvey_condition_check(adapter, _FALSE);

	if (ssc_chk != SS_ALLOW &&
		ssc_chk != SS_DENY_BUSY_TRAFFIC)
		return -EPERM;
...
static ssize_t proc_set_acs(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		int num = sscanf(tmp, "%s %s", cmd, val);
		...
		if (num == 2) {
			if (strcmp("nhm", cmd) == 0) {
				RTW_PRINT("[acs] set use_nhm:%d\n", rtw_atoi(val));
				acs->use_nhm = rtw_atoi(val);
			} else if (strcmp("scan_mode", cmd) == 0) {
				RTW_PRINT("[acs] set scan_mode:%d\n", rtw_atoi(val));
				acs->scan_mode = rtw_atoi(val);
			} else if (strcmp("duration", cmd) == 0) {
				RTW_PRINT("[acs] set duration:%d (ms)\n", rtw_atoi(val));
				acs->duration = rtw_atoi(val); // in ms
			} else if (strcmp("dacs_interval", cmd) == 0) {
				RTW_PRINT("[acs] set dacs_interval:%d (s)\n", rtw_atoi(val));
				acs->dacs_interval = rtw_atoi(val); // in s
			} else if (strcmp("ch_list", cmd) == 0) {
				RTW_PRINT("[acs] set ch_list: %s", val);
				memset(acs->ch_list, 0, sizeof(acs->ch_list));
				acs->ch_list_num = 0;
				_parse_ch_list_str_to_table(acs->ch_list, &acs->ch_list_num, val);
			} else if (strcmp("candidate_ch", cmd) == 0) {
				RTW_PRINT("[acs] set candidate_ch: %s", val);
				memset(acs->candidate_ch, 0, sizeof(acs->candidate_ch));
				acs->candidate_ch_num = 0;
				_parse_ch_list_str_to_table(acs->candidate_ch, &acs->candidate_ch_num, val);
			}
		} else if (num == 1) {
...
static int proc_get_wmm_dm(struct seq_file *m, void *v)
{
	...
	RTW_PRINT_SEL(m, "wmm_mode:       %u\n", dvobj->wmm_mode);
	RTW_PRINT_SEL(m, "wmm_mode_to:    %u\n", dvobj->wmm_mode_to);
	#ifdef CONFIG_LMT_TXREQ
	RTW_PRINT_SEL(m, "wmm_mode_streams = %d\n", dvobj->wmm_mode_streams);
	RTW_PRINT_SEL(m, "wmm_mode_txreq_lmt = %d\n", dvobj->wmm_mode_txreq_lmt);
	#endif
...
static int proc_get_dump_cnt(struct seq_file *m, void *v)
{
	...
	//core_cmd_dump_cnt(adapter, NULL, 0);
...
static int proc_get_a4(struct seq_file *m, void *v)
{
	...
	if (adapter->a4_enable) {
		...
		if (RTL_WFO_IS_OFLD(adapter->dvobj))
			cmd_a4_dump_fc_sta_table(adapter, m);
#endif
	}
...
static int proc_get_rx_debug(struct seq_file *m, void *v)
{
...
#ifdef DEBUG_CORE_RX
	RTW_PRINT_SEL(m, "num_rx_retry =%u\n", dvobj->num_rx_retry);
	...
	RTW_PRINT_SEL(m, "netif_rx_direct=%d\n", dvobj->netif_rx_direct);
	RTW_PRINT_SEL(m, "netif_rx_batch=%u\n", dvobj->netif_rx_batch);
	RTW_PRINT_SEL(m, "netif_rx_batch count all=%u, part=%u\n", dvobj->netif_rx_batch_all, dvobj->netif_rx_batch_part);
#endif
			...
			size_t count, loff_t *pos, void *data)
{
	...
	char *token;
	u32 argc = 0;
	char argv[MAX_PRC_CMD][PROC_CMD_LEN] = {{0}};
	int i, value;
	...
	else if (!strcmp(argv[0], "netif_rx_direct")) {
		if (argc > 1 && sscanf(argv[1], "%d", &value) == 1)
			dvobj->netif_rx_direct = value;
	}
	else if (!strcmp(argv[0], "netif_rx_batch")) {
		if (argc > 1 && sscanf(argv[1], "%d", &value) == 1)
			dvobj->netif_rx_batch = value;
	}
#endif
#ifdef CONFIG_SMP_PHL_RX_RECYCLE
...
static int proc_get_rx_deamsdu_by_nic(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);

	RTW_PRINT_SEL(m, "RTL_RX_DEAMSDU_BY_NIC = %d\n", padapter->rx_deamsdu_by_nic);

	return 0;
}

ssize_t proc_set_rx_deamsdu_by_nic(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	char tmp[32] = {0};
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		int rx_deamsdu_by_nic;
		int num = sscanf(tmp, "%d", &rx_deamsdu_by_nic);

		if (padapter && (num == 1)) {
			RTW_PRINT(FUNC_ADPT_FMT": RTL_RX_DEAMSDU_BY_NIC = %d\n",
			          FUNC_ADPT_ARG(padapter), rx_deamsdu_by_nic);
			padapter->rx_deamsdu_by_nic = rx_deamsdu_by_nic;
		}
	}
...
ssize_t proc_set_neighbor_rpt(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	int empty_slot;
	u8 error_code = 1;
	char tmp[NEIGHBOR_REPORT_PROC_LEN] = {0};
	char *tmpptr;
	char *tokptr;
	int command = 0;
	int i;
...
int proc_get_pe_ofld_dbg_cnt(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *padapter = (_adapter *) rtw_netdev_priv(dev);
	...
	struct swq_priv *swq = dvobj_to_swq(padapter->dvobj);
#endif
	int i = 0;

	if (!padapter->registrypriv.wifi_mib.pe_ofld_dbg) {
		RTW_PRINT_SEL(m, "Note: wifi_mib.pe_ofld_dbg is NOT enabled.\n");
		RTW_PRINT_SEL(m, "Cmd: iwpriv wlanX set_mib pe_ofld_dbg=1\n");
		return 0;
	}

	RTW_PRINT_SEL(m, "pe_ofld_tx_amsdus=%d\n", pxmitpriv->pe_ofld_tx_amsdus);
	RTW_PRINT_SEL(m, "pe_ofld_tx_amsdu_subframes=%d\n", pxmitpriv->pe_ofld_tx_amsdu_subframes);
	RTW_PRINT_SEL(m, "pe_ofld_tx_non_amsdu_frames=%d\n", pxmitpriv->pe_ofld_tx_non_amsdu);
	RTW_PRINT_SEL(m, "pe_ofld_tx_tc_pkts=%d\n", pxmitpriv->pe_ofld_tx_amsdu_subframes+pxmitpriv->pe_ofld_tx_non_amsdu);
	RTW_PRINT_SEL(m, "pe_ofld_tx_amsdu_bytes=%lld\n", pxmitpriv->pe_ofld_tx_amsdu_bytes);
	RTW_PRINT_SEL(m, "pe_ofld_tx_msdu_bytes=%lld\n", pxmitpriv->pe_ofld_tx_msdu_bytes);
	RTW_PRINT_SEL(m, "pe_ofld_tx_non_amsdu_bytes=%lld\n", pxmitpriv->pe_ofld_tx_non_amsdu_bytes);
	RTW_PRINT_SEL(m, "pe_ofld_hal_tx_amsdus=%d\n", phl_com->phl_stats.pe_ofld_amsdus);
	RTW_PRINT_SEL(m, "pe_ofld_hal_tx_amsdu_subframes=%d\n", phl_com->phl_stats.pe_ofld_amsdu_subframes);
	RTW_PRINT_SEL(m, "pe_ofld_hal_tx_non_amsdus=%d\n", phl_com->phl_stats.pe_ofld_non_amsdus);
	for (i = 0; i < AMSDU_SUBFRAME_DIST_LEN; i++) {
		if (pxmitpriv->pe_ofld_tx_amsdu_subframe_dist[i] == 0)
			continue;
		RTW_PRINT_SEL(m, "pe_ofld_tx_amsdu_subframe_dist[%d]=%d\n", i, pxmitpriv->pe_ofld_tx_amsdu_subframe_dist[i]);
	}
	RTW_PRINT_SEL(m, "pe_ofld_tx_mismatch_amsdus=%d\n", pxmitpriv->pe_ofld_tx_mismatch_amsdus);
	RTW_PRINT_SEL(m, "pe_ofld_tx_broken_amsdus=%d\n", pxmitpriv->pe_ofld_tx_broken_amsdus);
	RTW_PRINT_SEL(m, "pe_ofld_wd_tx_amsdus=%d\n", phl_com->phl_stats.pe_ofld_wd_amsdus);
	RTW_PRINT_SEL(m, "pe_ofld_wd_tx_non_amsdus=%d\n", phl_com->phl_stats.pe_ofld_wd_non_amsdus);
#ifdef CONFIG_VW_REFINE
	RTW_PRINT_SEL(m, "swq_dup_skb_cnt=%d\n", swq->swq_dup_skb_cnt);
#endif
	for (i = 0; i < 8; i++) {
		if (pxmitpriv->pe_ofld_tx_amsdu_priority_dist[i] == 0)
			continue;
		RTW_PRINT_SEL(m, "pe_ofld_tx_amsdu_priority_dist[%d]=%d\n", i, pxmitpriv->pe_ofld_tx_amsdu_priority_dist[i]);
	}
	return 0;
}

ssize_t proc_reset_pe_ofld_dbg_cnt(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	struct net_device *dev = data;
	_adapter *padapter = (_adapter *) rtw_netdev_priv(dev);
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	...
	if (buffer && !copy_from_user(cmd, buffer, count)) {
		...
		if (num == 1 && cnt == 0) {
			pxmitpriv->pe_ofld_tx_amsdus = 0;
			pxmitpriv->pe_ofld_tx_amsdu_subframes = 0;
			pxmitpriv->pe_ofld_tx_non_amsdu = 0;
			pxmitpriv->pe_ofld_tx_amsdu_bytes = 0;
			pxmitpriv->pe_ofld_tx_msdu_bytes = 0;
			pxmitpriv->pe_ofld_tx_non_amsdu_bytes = 0;
			phl_com->phl_stats.pe_ofld_amsdus = 0;
			phl_com->phl_stats.pe_ofld_amsdu_subframes = 0;
			phl_com->phl_stats.pe_ofld_non_amsdus = 0;
			_rtw_memset(pxmitpriv->pe_ofld_tx_amsdu_subframe_dist, 0, sizeof(pxmitpriv->pe_ofld_tx_amsdu_subframe_dist));
			pxmitpriv->pe_ofld_tx_mismatch_amsdus = 0;
			pxmitpriv->pe_ofld_tx_broken_amsdus = 0;
			phl_com->phl_stats.pe_ofld_wd_amsdus = 0;
			phl_com->phl_stats.pe_ofld_wd_non_amsdus = 0;
#ifdef CONFIG_VW_REFINE
			_rtw_memset(swq->swq_skb_cnt, 0, sizeof(swq->swq_skb_cnt));
			swq->swq_dup_skb_cnt = 0;
#endif
			_rtw_memset(pxmitpriv->pe_ofld_tx_amsdu_priority_dist, 0, sizeof(pxmitpriv->pe_ofld_tx_amsdu_priority_dist));
		}
	}
...
int proc_get_pe_ofld_id_mapping(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter->dvobj;
	...
	RTW_PRINT_SEL(m, "id     dev_id     macid\n");
	RTW_PRINT_SEL(m, "=======================\n");
	do {
		index = rtw_iterate_pe_entry(index, buf, sizeof(buf));
		if (buf[0] != 0)
			RTW_PRINT_SEL(m, "%s\n", buf);
	} while (index > 0);

	RTW_PRINT_SEL(m, "=======================\n");
	rtw_get_pe_entry_agg_sta_lmt(buf, sizeof(buf));
	if (buf[0] != 0)
		RTW_PRINT_SEL(m, "%s\n", buf);
	rtw_get_pe_entry_num_entry(buf, sizeof(buf));
	if (buf[0] != 0)
		RTW_PRINT_SEL(m, "%s\n", buf);
	rtw_get_pe_entry_pe_ofld_mode(buf, sizeof(buf));
	if (buf[0] != 0)
		RTW_PRINT_SEL(m, "%s\n", buf);
	return 0;
}
...
const struct rtw_proc_hdl adapter_proc_hdls[] = {
...
#ifdef RADIO_MEASUREMENT_SPARE_SCAN_LIST
	RTW_PROC_HDL_SSEQ("rm_survey_info", proc_get_rm_survey_info, NULL),
#endif
#ifdef CONFIG_RTW_MULTI_AP
	RTW_PROC_HDL_SSEQ("map_survey_info", proc_get_map_survey_info, NULL),
	RTW_PROC_HDL_SSEQ("map_block_list", proc_get_map_block_list, NULL),
#ifdef DEBUG_MAP_UNASSOC
	RTW_PROC_HDL_SSEQ("map_unassoc_sta", proc_get_map_unassoc_sta, NULL),
#endif
#endif
	...
	RTW_PROC_HDL_SSEQ("all_sta_info", proc_get_all_sta_info, proc_set_all_sta_info),
	...
	RTW_PROC_HDL_SSEQ("temperature", proc_get_temperature, proc_set_temperature),
	...
	RTW_PROC_HDL_SSEQ("total_sta_num_limit", proc_get_total_sta_num_limit, proc_set_total_sta_num_limit),
...
#endif
#ifdef SBWC
	RTW_PROC_HDL_SSEQ("mib_sbwc", proc_get_sbwc, proc_set_sbwc),
#endif
#ifdef GBWC
	...
	* 	RTW_PROC_HDL_SSEQ("rssi_disp",proc_get_rssi_disp, proc_set_rssi_disp), */
...
#ifdef CONFIG_RTW_AP_BS_DECT
	RTW_PROC_HDL_SSEQ("bcc_trig", NULL, proc_set_bcc_trigger),
#endif
	...
	RTW_PROC_HDL_SSEQ("dfs_skip_ch", proc_get_dfs_skip_ch, proc_set_dfs_skip_ch),
#if CONFIG_DFS_SLAVE_WITH_RADAR_DETECT
	...
	RTW_PROC_HDL_SSEQ("tx_power", proc_get_tx_power, NULL),
...
#else
	RTW_PROC_HDL_SSEQ("tx_power_limit", proc_get_tx_power_limit_wkard, NULL),
#endif
#ifdef GEORGIA_TODO_TX_PWR
...
#ifdef CONFIG_RTW_BYPASS_DEAMSDU
	RTW_PROC_HDL_SSEQ("rx_deamsdu_by_nic", proc_get_rx_deamsdu_by_nic, proc_set_rx_deamsdu_by_nic),
#endif
#ifdef CONFIG_CORE_TXSC
	...
	RTW_PROC_HDL_SSEQ("qos_mgmt", proc_get_qos_mgmt, proc_set_qos_mgmt),
#ifdef CONFIG_ONE_TXQ
	RTW_PROC_HDL_SSEQ("atm", proc_get_atm, proc_set_atm),
#endif
#ifdef CONFIG_DELAY_MEASUREMENT
	RTW_PROC_HDL_SSEQ("delay_measure", proc_get_delay_measure, NULL),
#endif
#ifdef CMCC_AUTO_DETECTION
	RTW_PROC_HDL_SSEQ("cmcc_detect", proc_get_cmcc_detect, NULL),
	RTW_PROC_HDL_SSEQ("cmcc_msta_info", proc_get_cmcc_msta_info, NULL),
#endif
#ifdef CONFIG_ETHER_PKT_AGG
	RTW_PROC_HDL_SSEQ("pe_ofld_dbg_cnt", proc_get_pe_ofld_dbg_cnt, proc_reset_pe_ofld_dbg_cnt),
	RTW_PROC_HDL_SSEQ("pe_ofld_id_mapping", proc_get_pe_ofld_id_mapping, NULL),
#endif /* CONFIG_ETHER_PKT_AGG */
#ifdef RECORD_ENV_STAINFO
	RTW_PROC_HDL_SSEQ("env_stainfo_table", proc_dump_env_stainfo_table, NULL),
#endif
};
                                 ...
                                 u32 input_num, char *output, u32 out_len)
{
	...
	if (input_num < 3) {
		for (i = 0; i < handler_num; i++) {
			...
			PHL_PROC_CMD_PRINTF(out_len, used, output + used, out_len - used,
					...
					"\n",
					i, os_handler->name,
					((os_handler->type==RTW_OS_HANDLER_TASKLET)?'T':'W'),
	...
	if (handler_id >= handler_num) {
		PHL_PROC_CMD_PRINTF(out_len, used, output + used, out_len - used,
				DEV_FMT": Invalid handler ID %u (%u)\n",
	...
	if (cpu_id > WORK_CPU_UNBOUND) {
		PHL_PROC_CMD_PRINTF(out_len, used, output + used, out_len - used,
				DEV_FMT": CPU ID %u is not within range %u.\n",
				DEV_ARG(dvobj), cpu_id, WORK_CPU_UNBOUND);
		return;
	}
	os_handler->cpu_id = cpu_id;
	PHL_PROC_CMD_PRINTF(out_len, used, output + used, out_len - used,
			DEV_FMT": Set handler %u \"%s\" run on CPU %u.\n",
				  ...
				  enum rtw_proc_cmd_type type)
{
	...
	if (buffer) {
		if (NULL == phydm_msg) {
			phydm_msg = rtw_zvmalloc(PHYDM_MSG_LEN);
			if (NULL == phydm_msg)
				return -ENOMEM;
		} else

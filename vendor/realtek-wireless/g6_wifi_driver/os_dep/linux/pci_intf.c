...
struct pci_device_id rtw_pci_id_tbl[] = {
	...
	{PCI_DEVICE(PCI_VENDER_ID_REALTEK, 0xb83e), .driver_data = RTL8852C},
#endif
...
static __inline__ void schedule_pe_isr_handler(struct dvobj_priv *dvobj)
{
	_adapter *padapter = dvobj_get_primary_adapter(dvobj);

	padapter->hw_pe_irq_cnt++;
	rtw_phl_pe_isr_notify(padapter->dvobj->phl);
}
...
#if defined(CONFIG_VW_REFINE) || defined(CONFIG_ONE_TXQ) || defined(CONFIG_TXSC_AMSDU)
	if (pstatus == RTW_PHL_STATUS_SH_TASK) {
#ifdef CONFIG_RTL_WIFI_OFFLOAD
		if (dvobj->wfo_en)
			schedule_pe_isr_handler(dvobj);
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
#ifdef CONFIG_VW_REFINE
		if (dvobj->tx_mode == 1) {
		...
		if (dvobj->tx_mode == 0 &&
			...
			rtw_tasklet_hi_schedule(&dvobj->txsc_amsdu_timeout_tasklet);
#endif
	} else
#endif
...
void rtw_init_txq(struct dvobj_priv *dvobj)
{
	...
	dvobj->txq_hw_timeout = 500; /* in us */
	dvobj->txq_pkt_timeout = 10; /* in ms */
	dvobj->txq_max_serv_time = 20; /* in ms */
	dvobj->txq_amsdu_merge = 0;
	dvobj->txq_ts_factor = 128;
	dvobj->txq_deq_factor = 200; /* decide by average timout measured */
	dvobj->txq_deq_loop = 8;
	dvobj->txq_debug = 0;
	dvobj->txq_serv_group_exp = 2;
	dvobj->txq_active_tp = 15000;
	dvobj->txq_active_tp_legacy = 1000;
	if (dvobj->phl_com->dev_sw_cap.band_sup == BAND_CAP_5G) {
		dvobj->txq_max_enq_len = 8192;
	} else if (dvobj->phl_com->dev_sw_cap.band_sup == BAND_CAP_2G) {
		dvobj->txq_max_enq_len = 4096;
	} else {
		dvobj->txq_max_enq_len = 4096;
	}
	dvobj->txq_bcst_threshold = 512;
	dvobj->txq_rx_enq_tp = 0;
...
void rtw_init_lmt_txreq(struct dvobj_priv *dvobj)
{
	if (dvobj->phl_com->dev_sw_cap.band_sup == BAND_CAP_5G)
		dvobj->lmt_sta_bcast_txreq = 400;
	else
		dvobj->lmt_sta_bcast_txreq = 200;
}
#endif

#ifdef CONFIG_VW_REFINE
u8 rtw_swq_priv_init(struct dvobj_priv *dvobj)
{
	struct swq_priv *swq = NULL;
	u8 i = 0;

	swq = rtw_zvmalloc(sizeof(struct swq_priv));
	if (swq == NULL)
		return _FAIL;

#ifdef CONFIG_SWQ_SKB_ARRAY
	_rtw_spinlock_init(&swq->lock);
	for (i = 0; i < MAX_SKB_XMIT_QUEUE; i++) {
		swq->skb_que_ts[i] = 10000;
		swq->swq_skb_array[i].wptr = 0;
		swq->swq_skb_array[i].rptr = 0;
	}

#ifdef CONFIG_ETHER_PKT_AGG
	swq->swq_dup_skb_cnt = 0;
#endif
#else  /* CONFIG_SWQ_SKB_ARRAY */
	ATOMIC_SET(&swq->skb_xmit_queue_len, 0);
	for (i = 0; i < MAX_SKB_XMIT_QUEUE; i++) {
		swq->skb_que_ts[i] = 10000;
		skb_queue_head_init(&swq->skb_xmit_queue[i]);
	}
#endif /* CONFIG_SWQ_SKB_ARRAY */
...
void rtw_swq_priv_deinit(struct dvobj_priv *dvobj)
{
	struct swq_priv *swq = dvobj_to_swq(dvobj);

	dvobj->swqpriv = NULL;
	if (swq) {
		rtw_vmfree(swq, sizeof(struct swq_priv));
	}
}
...
_adapter *rtw_pci_primary_adapter_init(struct dvobj_priv *dvobj, struct pci_dev *pdev)
{
	...
	overwrite_rtw_mib_default_tbl(dvobj, priv_mib);
#endif
...
#define DEF_THER_TP_LEVEL 21
static int def_ther_offset_5g = -35366;
static unsigned int def_ther_parameter_5g = 2517;
static unsigned int def_ther_tp_level_5g = 100;
static int def_ther_cps_5g[DEF_THER_TP_LEVEL] =
{
/*0*/0, /*100*/1,  /*200*/1,  /*300*/1, /*400*/2, /*500*/2, /*600*/2,
/*700*/2,  /*800*/2,  /*900*/4, /*1000*/4, /*1100*/4, /*1200*/4, /*1300*/4,
/*1400*/4, /*1500*/5, /*1600*/5, /*1700*/5, /*1800*/5, /*1900*/5, /*2000*/5,
};
static int def_ther_offset_2g = -23252;
static unsigned int def_ther_parameter_2g = 2553;
static unsigned int def_ther_tp_level_2g = 100;
static int def_ther_cps_2g[DEF_THER_TP_LEVEL] =
{
...
int rtw_ther_default_mib(struct dvobj_priv *dvobj)
{
	if (dvobj->phl_com->dev_sw_cap.band_sup == BAND_CAP_5G) {
		printk("load default 5g thermal parameters\n");
		dvobj->ther_offset = def_ther_offset_5g;
		dvobj->ther_parameter = def_ther_parameter_5g;
		dvobj->ther_tp_level = def_ther_tp_level_5g;
		memset(dvobj->ther_cps, 0, MAX_THER_TP_LEVEL*sizeof(s32));
		memcpy(dvobj->ther_cps, def_ther_cps_5g, DEF_THER_TP_LEVEL*sizeof(s32));
	} else if (dvobj->phl_com->dev_sw_cap.band_sup == BAND_CAP_2G) {
		printk("load default 2g thermal parameters\n");
		dvobj->ther_offset = def_ther_offset_2g;
		dvobj->ther_parameter = def_ther_parameter_2g;
		dvobj->ther_tp_level = def_ther_tp_level_2g;
		memset(dvobj->ther_cps, 0, MAX_THER_TP_LEVEL*sizeof(s32));
		memcpy(dvobj->ther_cps, def_ther_cps_2g, DEF_THER_TP_LEVEL*sizeof(s32));
	}
	return 0;
}
...
#define MAX_DYINGGASP_DVOBJ_NUM 3
static struct dvobj_priv *dyinggasp_dvobj[MAX_DYINGGASP_DVOBJ_NUM] = {NULL, NULL, NULL};
static int dyinggasp_dvobj_num = 0;
static _workitem rtw_dyinggasp_work;

void rtw_dyinggasp_handler(unsigned long data)
{
	int i;
	for (i = 0; i < MAX_DYINGGASP_DVOBJ_NUM; i++) {
		if (!dyinggasp_dvobj[i])
			continue;
		if (_is_all_other_iface_down(dyinggasp_dvobj[i], NULL))
			continue;
		if (!dyinggasp_dvobj[i]->phl)
			continue;

		rtw_phl_disable_interrupt(dyinggasp_dvobj[i]->phl);
		if (rtw_hw_is_init_completed(dyinggasp_dvobj[i]))
			rtw_phl_power_switch(dyinggasp_dvobj[i]->phl, 0);
	}
}

static int  rtw_dyinggasp_schedule(struct notifier_block *this, unsigned long event, void *unused)
{
	if (dyinggasp_dvobj_num)
		_set_workitem(&rtw_dyinggasp_work);
	return NOTIFY_OK;
}

static struct notifier_block rtw_dyinggasp_notifier = {
	.notifier_call  = rtw_dyinggasp_schedule,
	.next           = NULL,
	.priority       = 150   /* priority: INT_MAX >= x >= 0 */
};

void _rtw_register_dyinggasp_dvobj(struct dvobj_priv *dvobj)
{
	int i;
	if (dyinggasp_dvobj_num == MAX_DYINGGASP_DVOBJ_NUM) {
		printk("[%s]registered dvobj full\n", __func__);
		return;
	}

	if (dyinggasp_dvobj_num == 0) {
		rtw_dyinggasp_work.type = RTW_OS_HANDLER_WORK;
		rtw_dyinggasp_work.name = "dying gasp";
		rtw_dyinggasp_work.id = RTW_HANDLER_CORE_DYING_GASP;
		rtw_dyinggasp_work.task_data.func = rtw_dyinggasp_handler;
		rtw_dyinggasp_work.task_data.data = (unsigned long)dvobj;
		rtw_init_os_handler(dvobj, &rtw_dyinggasp_work);
		register_dyinggasp_notifier(&rtw_dyinggasp_notifier);
	}

	for (i = 0; i < MAX_DYINGGASP_DVOBJ_NUM; i++) {
		if (dyinggasp_dvobj[i] == NULL) {
			dyinggasp_dvobj_num++;
			dyinggasp_dvobj[i] = dvobj;
			break;
		}
	}
}

void _rtw_deregister_dyinggasp_dvobj(struct dvobj_priv *dvobj)
{
	int i;
	for (i = 0; i < MAX_DYINGGASP_DVOBJ_NUM; i++) {
		if (dyinggasp_dvobj[i] == dvobj) {
			dyinggasp_dvobj[i] = NULL;
			dyinggasp_dvobj_num--;
			break;
		}
	}
	if (i == MAX_DYINGGASP_DVOBJ_NUM)
		printk("[%s]deregistered dvobj fail\n", __func__);
	if (dyinggasp_dvobj_num == 0)
		rtw_deinit_os_handler(dvobj, &rtw_dyinggasp_work);
}

#endif /* CONFIG_RTK_DP_DIS_SOC */
...
struct dvobj_priv *g_dvobj[BAND_MAX] = {NULL, NULL, NULL};
...
struct dvobj_priv *rtw_get_dvobj(u8 band_on)
{
	if (band_on < BAND_MAX)
		return g_dvobj[band_on];
	else
		return NULL;
}
...
static int rtw_dev_probe(struct pci_dev *pdev, const struct pci_device_id *pdid)
{
	...
	if (dev_probe_num <= BAND_MAX)
		g_dvobj[dev_probe_num-1] = dvobj;
	...
	printk("[%s] dev_probe_num:%d padapter:0x%px dev_id:%d \n", __func__, dev_probe_num, padapter, dvobj->dev_id);
	...
	if (rtw_os_rx_init(dvobj) == _FAIL) {
		RTW_ERR("rtw_os_rx_init Failed!\n");
		goto free_hw;
	}

#ifdef CONFIG_RTW_EFEM_IDENTIFY_WITH_FLASH
	if (rtw_check_efem_type(dvobj) == _FAIL) {
		goto free_os_rx;
	}
#endif

	rtw_ther_default_mib(dvobj);
	/* CONFIG_ONE_TXQ */
	rtw_init_txq(dvobj);
#ifdef CONFIG_LMT_TXREQ
	rtw_init_lmt_txreq(dvobj);
#endif
#ifdef CONFIG_TXSC_AMSDU
	rtw_txsc_amsdu_timeout_init(dvobj);
#endif

#ifdef CONFIG_VW_REFINE
	if (rtw_swq_priv_init(dvobj) == _FAIL) {
		goto free_os_rx;
	}
#endif
...
#ifdef CONFIG_ETHER_PKT_AGG
	rtw_init_pe_entry(dvobj);
#endif

#ifdef CONFIG_RTK_DP_DIS_SOC
	_rtw_register_dyinggasp_dvobj(dvobj);
#endif
...
#endif
#ifdef CONFIG_RTK_DP_DIS_SOC
	_rtw_deregister_dyinggasp_dvobj(dvobj);
#endif
	pci_dvobj_deinit(pdev);

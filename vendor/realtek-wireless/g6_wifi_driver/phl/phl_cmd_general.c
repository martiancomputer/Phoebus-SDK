				     ...
				     struct phl_msg *msg)
{
	...
	case MSG_EVT_SET_TX_RTY_LMT:
		psts = phl_set_tx_retry_limit_hdl(phl_info, *phl_cmd->buf);
	break;

	case MSG_EVT_SET_PWR_LMT_EN:
		psts = phl_set_pwr_lmt_en_val_hdl(phl_info, phl_cmd->buf);
	break;

	case MSG_EVT_SET_EDCCA_MODE_EN:
		psts = phl_set_edcca_mode_en_val_hdl(phl_info, phl_cmd->buf);
	break;

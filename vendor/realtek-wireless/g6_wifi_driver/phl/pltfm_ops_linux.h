...
static void *_os_ioremap(u32 paddr, unsigned long size)
{
	return ioremap(paddr, size);
}

static void _os_iounmap(void *addr)
{
	iounmap(addr);
}
...
static inline void _os_convert_buf_to_pkt(u8 *pbuf, u8 *pe_buf, u32 buf_sz)
{
	_rtw_memcpy(pbuf, pe_buf, buf_sz);
}
						...
						void **os_priv)
{
	...
	if (alloc_data->pool_id < 1) {
		...
		//printk_ratelimited("%d pobj->pool_idx=%d skb=%p\n",__LINE__,pobj->pool_idx,skb);
		if(!skb)
			skb = rtw_skb_alloc(rxbuf_size);
		else
			skb->len=0;		//20220526LUKE: in case core_update_recvframe_pkt of rtw_recv.c directly call skb_put without clean skb->len.
#ifdef DBG_MEM_ALLOC
		rtw_mstat_update(
			MSTAT_TYPE_SKB
					   ...
					   void **data_2)
{
	...
	skb_1 = dev_alloc_skb_recy_wnic(rxbuf_size, alloc_data->pool_id);//printk_ratelimited("%d pobj->pool_idx=%d skb_1=%p\n",__LINE__,pobj->pool_idx,skb_1);
	if(!skb_1)
		skb_1 = rtw_skb_alloc(rxbuf_size);
	else
		skb_1->len=0;	//20220526LUKE: in case core_update_recvframe_pkt of rtw_recv.c directly call skb_put without clean skb->len.
#else
	skb_1 = rtw_skb_alloc(rxbuf_size);
#endif /* CONFIG_WNIC_RECYCLED_SKB */
	if (skb_1 == NULL)
...
#define _os_mutex_init(d, mutex) _rtw_mutex_init(mutex)
...
/* spinlock */
#define _os_spinlock_init(d, plock) _rtw_spinlock_init(plock)
...
static __inline u8 _os_thread_init_cpu(	void *drv_priv, _os_thread *thread,
					int (*call_back_func)(void * context),
					void *context,
					const char namefmt[],
					unsigned int cpu)
{
	RST_THREAD_STATUS(thread);
	thread->thread_handler = rtw_thread_start_cpu((int(*)(void*))call_back_func, context, namefmt, cpu);
	if (thread->thread_handler) {
		SET_THREAD_STATUS(thread, THREAD_STATUS_STARTED);
		return RTW_PHL_STATUS_SUCCESS;
	}

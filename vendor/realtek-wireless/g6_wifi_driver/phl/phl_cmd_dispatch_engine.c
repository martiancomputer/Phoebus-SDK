...
phl_disp_eng_init(struct phl_info_t *phl, u8 phy_num)
{
	...
	disp_eng->thread_cpu = CPU_ID_PHL_THREAD;
...
#ifdef CONFIG_PHL_THREAD_CPU_BALANCE
		_os_thread_init_cpu(d, &(disp_eng->share_thread), share_thread_hdl, phl,
				"disp_eng_share_thread", disp_eng->thread_cpu);
#else
		_os_thread_init(d, &(disp_eng->share_thread), share_thread_hdl, phl,
				"disp_eng_share_thread");
#endif
#ifdef RTW_WKAPD_AP_CMD_THREAD_USING_RT		
		_os_sched_setscheduler_rt(d, &(disp_eng->share_thread));
#endif

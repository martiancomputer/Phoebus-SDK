...
enum rtw_hal_status flash_set_hw_cap(struct flash_t *flash)
{
	...
	else if (hal_com->chip_id == CHIP_WIFI6_8192XB) {
		/* Leave to set xcap in share_xstal (or non_share_xstal) case*/
	}
									   ...
									   u8 size)
{
	...
	if(info_type <= FLASH_INFO_MAC_MAX)
		...
		hal_status = rtw_flash_get_tssi_slp_info(hal_com,
						flash_info,
						info_type,
						value,
						size);
...
enum rtw_hal_status rtw_flash_set_tssi_slp_info(void *flash, enum rtw_flash_info info_type, u8 *input_data, u8 len)
{
	struct flash_t *flash_info = flash;
	u8 i = 0, value = 0;
	u8 temp[3] = {0};
...
enum rtw_hal_status rtw_flash_get_tssi_slp_info(struct rtw_hal_com_t *hal_com, void *flash, enum rtw_flash_info info_type, void *value, u8 size)
{
	struct flash_t *flash_info = flash;
	u8 offset = 0;

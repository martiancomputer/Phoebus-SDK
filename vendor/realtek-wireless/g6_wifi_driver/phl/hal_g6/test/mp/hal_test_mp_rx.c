	...
	struct mp_context *mp, struct mp_rx_arg *arg)
{
	...
	PHL_INFO("%s: drv mp_fltr_rx_ok_cnt count = %d\n", __FUNCTION__, arg->mp_fltr_rx_ok_cnt);
	...
	struct mp_context *mp, struct mp_rx_arg *arg)
{
	...
	arg->mp_fltr_rx_err_cnt = trx_stat->mp_fltr_rx_err_cnt;

	PHL_INFO("%s: status = %d\n", __FUNCTION__, hal_status);
	PHL_INFO("%s: drv err count = %d\n", __FUNCTION__, arg->rx_err);
	PHL_INFO("%s: drv mp_fltr_rx_err_cnt count = %d\n", __FUNCTION__, arg->mp_fltr_rx_err_cnt);

	return hal_status;
}

enum rtw_hal_status _rtw_hal_setflt_get_rssi(struct hal_info_t *hal_info, enum rf_path rx_path, u8 *rssi)
{
	...
	*rssi =  trx_stat->rssi_raw_path[rx_path];

	PHL_INFO("%s: status = %d\n", __FUNCTION__, hal_status);
	PHL_INFO("%s: rssi_path[%d] = %d\n", __FUNCTION__, rx_path, *rssi);
	...
	struct mp_context *mp, struct mp_rx_arg *arg)
{
	...
	if (arg->setflt)
		hal_status = _rtw_hal_setflt_get_rssi(mp->hal, arg->rx_path, &arg->rssi);
	else
		...
		struct mp_context *mp, struct mp_rx_arg *arg)
{
	...
	trx_stat->setflt = arg->setflt;

	if (trx_stat->setflt) {
		_rtw_memcpy(&trx_stat->mp_fltr_ta, &arg->mp_fltr_mac_ta, 6);

		PHL_PRINT("set mac filter enable, ta %02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx\n", trx_stat->mp_fltr_ta[0],
		...
		_rtw_memset(&trx_stat->mp_fltr_ta, 0x00, 6);

		PHL_PRINT("set mac filter disable\n");
	}

	trx_stat->mp_fltr_rx_ok_cnt = 0;
	trx_stat->mp_fltr_rx_err_cnt = 0;
	for (i = RF_PATH_A; i < RTW_PHL_MAX_RF_PATH; i++)
		trx_stat->rssi_raw_path[i] = 0;
...
enum rtw_hal_status rtw_hal_mp_rx_get_rssi_rx_k(
	struct mp_context *mp, struct mp_rx_arg *arg)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	...
	hal_status = rtw_hal_bb_get_rssi_rx_k(mp->hal, arg->rx_path, &arg->rssi);

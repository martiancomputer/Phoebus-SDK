			...
			struct rtw_hal_com_t *hal_com)
{
	...
	if (phl_com->dev_cap.rx_amsdu_cut && phl_com->dev_cap.rx_hdr_conv) {
		/* RXBD NUM */
		bus_cap->rxbd_num = (bus_sw->msdu_rxbd_num) ?
			((bus_sw->msdu_rxbd_num > bus_hw->max_rxbd_num) ?
			 bus_hw->max_rxbd_num : bus_sw->msdu_rxbd_num) :
			bus_hw->max_rxbd_num;
		/* RXBUF NUM */
		bus_cap->rxbuf_num = (bus_sw->msdu_rxbuf_num < bus_cap->rxbd_num) ?
				 (bus_cap->rxbd_num + 32) : bus_sw->msdu_rxbuf_num;
		bus_sw->rxbuf_num_effective = bus_cap->rxbuf_num;
		/* RXBUF SIZE */
		bus_cap->rxbuf_size = (bus_sw->msdu_rxbuf_size) ?
			((bus_sw->msdu_rxbuf_size > bus_hw->max_rxbuf_size) ?
			 bus_hw->max_rxbuf_size : bus_sw->msdu_rxbuf_size) :
			bus_hw->max_rxbuf_size;
		bus_sw->rxbuf_size_effective = bus_cap->rxbuf_size;
...
void rtw_hal_fw_cap_pre_config(struct rtw_phl_com_t *phl_com, void *hal)
{
	...
	*/
	...
	dev_cap->fw_cap.fw_path = dev_sw_cap->fw_cap.fw_path;
}

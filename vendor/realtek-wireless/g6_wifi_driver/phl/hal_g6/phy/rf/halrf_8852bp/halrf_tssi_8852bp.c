					...
					enum phl_phy_idx phy, u32 tssi_de_offset, u8 path)
{
	...
	if (path == RF_PATH_A) {
		...
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8852bp(rf, phy, RF_PATH_A);
		...
		/*OFDM*/
		ofmd_de = _halrf_tssi_get_ofdm_efuse_tssi_de_8852bp(rf, phy, RF_PATH_A);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8852bp(rf, phy, RF_PATH_A);
		...
		/*CCK*/
		group_idx = _halrf_tssi_get_cck_efuse_group_8852bp(rf, phy);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8852bp(rf, phy, RF_PATH_B);
		...
		/*OFDM*/
		ofmd_de = _halrf_tssi_get_ofdm_efuse_tssi_de_8852bp(rf, phy, RF_PATH_B);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8852bp(rf, phy, RF_PATH_B);
		tssi_tmp = ofmd_de + tssi_trim + s_tssi_de_offset;
						...
						enum phl_phy_idx phy)
{
	...
	if (tssi_info->tssi_type[HW_PHY_0] == TSSI_CAL) {
		...
		for (i = 0; i < TSSI_PATH_MAX_8852BP; i++) {
			...
			tssi_info->curr_tssi_efuse_cck_de[i] = (s8)(tmp & 0xff);
			tssi_info->curr_tssi_efuse_ofdm_de[i] = (s8)(tmp & 0xff);

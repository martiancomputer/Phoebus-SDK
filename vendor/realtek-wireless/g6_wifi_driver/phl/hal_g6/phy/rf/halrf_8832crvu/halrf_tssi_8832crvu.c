...
void _halrf_tssi_set_alignment_default_8832crvu(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	...
	if (path == RF_PATH_A) {
		if (channel >= 36 && channel <= 64) {
			...
			halrf_wreg(rf, 0x5634, 0x3ff00000, 0x00d);
			halrf_wreg(rf, 0x5638, 0x000003ff, 0x048);
			halrf_wreg(rf, 0x5638, 0x000ffc00, 0x096);
		...
		if (channel >= 36 && channel <= 64) {
			...
			halrf_wreg(rf, 0x7634, 0x3ff00000, 0x005);
			halrf_wreg(rf, 0x7638, 0x000003ff, 0x044);
			halrf_wreg(rf, 0x7638, 0x000ffc00, 0x08e);
			...
			halrf_wreg(rf, 0x7634, 0x000003ff, 0x000);
			halrf_wreg(rf, 0x7634, 0x000ffc00, 0x000);
			halrf_wreg(rf, 0x7634, 0x3ff00000, 0x00d);
			halrf_wreg(rf, 0x7638, 0x000003ff, 0x048);
			halrf_wreg(rf, 0x7638, 0x000ffc00, 0x096);
			halrf_wreg(rf, 0x763c, 0x000003ff, 0x000);
			halrf_wreg(rf, 0x763c, 0x000ffc00, 0x000);
			halrf_wreg(rf, 0x763c, 0x3ff00000, 0x000);
			halrf_wreg(rf, 0x7640, 0x000003ff, 0x000);
...
//	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
	...
	for (i = path_tmp; i < path_max; i++) {
		...
		_halrf_tssi_set_alignment_default_8832crvu(rf, phy, i);
...
//	struct halrf_tssi_info *tssi_info = &rf->tssi;
	...
	for (i = path_tmp; i < path_max; i++) {
		...
		if (tssi_info->alignment_done[i][band] == true)
			...
			_halrf_tssi_alignment_default_8832crvu(rf, phy, i);
		}
#endif
	}
...
#if 0
			_halrf_tssi_alignment_default_8832crvu(rf, 0, path);
#endif
		}

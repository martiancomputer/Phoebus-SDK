...
static bool _iqk_check_cal_8832crvu(struct rf_info *rf, u8 path, u8 ktype)
{
	...
	RF_DBG(rf, DBG_RF_IQK, "[IQK]S%x, type= %x, cnt1= %d, cnt2= %d\n", path, ktype, cnt1, cnt2);
				 ...
				 u8 path, u8 ktype)
{
	...
	case ID_RXAGC:
		...
		iqk_cmd = 0x1708 | (1 << (4 + path)) | (path << 1);
		break;
	case ID_RXK:
...
__iram_func__
static bool _iqk_rxk_group_sel_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx, u8 path)
{

	struct halrf_iqk_info *iqk_info = &rf->iqk;
	
	u32 a_idxrxgain[4] = {0x18c, 0x198, 0x310, 0x318};
	u32 a_idxattc2[4] = {0x0, 0x1, 0x0, 0x1};
	...
	for(gp = 0; gp < 4; gp++){
		...
		notready = _iqk_one_shot_8832crvu(rf, phy_idx, path, ID_RXAGC);
		RF_DBG(rf, DBG_RF_IQK, "[IQK]S%x, RXAGC REG0x0 = 0x%x\n", path, halrf_rrf(rf, path, 0x00, MASKRF));
		halrf_wrf(rf, path, 0x1e, 0x0003f, 0x13);
		halrf_delay_us(rf, 200);		
		notready = _iqk_one_shot_8832crvu(rf, phy_idx, path, ID_NBRXK);
		iqk_info->nb_rxcfir[path] = halrf_rreg(rf, 0x813c + (path << 8), MASKDWORD);
		RF_DBG(rf, DBG_RF_IQK, "[IQK]S%x, NBRXK 0x8%x3c = 0x%x\n", path, 1 << path, iqk_info->nb_rxcfir[path]);
		notready = _iqk_one_shot_8832crvu(rf, phy_idx, path, ID_RXK);
	}
	halrf_wrf(rf, bkrf0, 0x0, 0x0c000, bkrf0);
	if (!notready)
...
__iram_func__
static bool _iqk_txk_group_sel_8832crvu(struct rf_info *rf, enum phl_phy_idx phy_idx,
				  u8 path)
{
	...
	u32 a_track_range[4] = {0x4, 0x4, 0x6, 0x6};
	u32 a_gain_bb[4] = {0x12, 0x12, 0x0a, 0x10};
	u32 a_itqt[4] = {0x12, 0x12, 0x12, 0x1b};
	
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
	halrf_wrf(rf, path, 0x00, 0xf0000, 0x4);
	for (gp = 0x1; gp < 0x4; gp++) {
		...
		halrf_wreg(rf, 0x8154 + (path << 8), 0x00000100, 0x1);
		halrf_wreg(rf, 0x8154 + (path << 8), 0x00000010, 0x1);
		...
		notready = _iqk_one_shot_8832crvu(rf, phy_idx, path, ID_NBTXK);
		iqk_info->nb_txcfir[path] = halrf_rreg(rf, 0x8138+ (path << 8), MASKDWORD);
		RF_DBG(rf, DBG_RF_IQK, "[IQK]S%x, NBTXK 0x8%x38 = 0x%x\n", path, 1 << path, iqk_info->nb_txcfir[path]);
		halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
		halrf_wreg(rf, 0x81cc + (path << 8), MASKDWORD, a_itqt[gp]);
		notready = _iqk_one_shot_8832crvu(rf, phy_idx, path, ID_TXK);
	}
...
static void _lok_res_table_8832crvu(struct rf_info *rf, u8 path, u8 ibias)
{
	...
	halrf_wrf(rf, path, 0x30, 0x00007, ibias);
	halrf_wrf(rf, path, 0xef, MASKRF, 0x0);
	RF_DBG(rf, DBG_RF_IQK, "[IQK]S%x, 0x30[5:3] = 0x%x\n", path, halrf_rrf(rf, path, 0x30, 0x00038));
			    ...
			    u8 path)
{
	...
	halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
	halrf_wreg(rf, 0x5670 | (path <<13), 0x00000002, 0x0);
	RF_DBG(rf, DBG_RF_IQK, "[IQK]S%x, rf0x%x = 0x%x\n", path, 0x5c, halrf_rrf(rf, path, 0x5c, MASKRF));
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]S%x, rf0x%x = 0x%x\n", path, 0x58, halrf_rrf(rf, path, 0x58, MASKRF));
				...
				u8 path)
{
	...
	//LOK
	for (i = 0; i < 3; i++) {		
		_lok_res_table_8832crvu(rf, path, 0x3 + i);
	...
	if (iqk_info->is_nbiqk) {
		...
		iqk_info->iqk_tx_fail[0][path] =
			_iqk_txk_group_sel_8832crvu(rf, phy_idx, path);
	}
	//RX
	...
	if (iqk_info->is_nbiqk) {
		...
		iqk_info->iqk_rx_fail[0][path] =
			_iqk_rxk_group_sel_8832crvu(rf, phy_idx, path);
	}
#endif

...
u8 halrf_get_ap_regulation_info(struct rf_info *rf, u8 band)
{
	struct halrf_pwr_info *pwr = &rf->pwr_info;
	...
	RF_DBG(rf, DBG_RF_POWER, "======>%s pwr->regulation_idx(0x%x) != 0xff\n",
		__func__, pwr->regulation_idx);
	/* TODO: adding category to get pwr_lmt_regu for 6G */
	return regulation_to_pw_lmt_regu_type(pwr->regulation_idx);
}
...
#ifdef RTW_FLASH_98D
	if (reg < REGULATION_MAX) {
		reg = halrf_get_ap_regulation_info(rf, band);
	} else {
		if (reg >= regd_name_max_size)
	...
	u8 beamforming, u8 channel)
{
	...
	u8 band, limit_rate = PW_LMT_RS_CCK, tx_num = PW_LMT_PH_1T, limit_ch,reg;
	u16 rate_tmp;

...
__iram_func__
static bool _iqk_rxk_group_sel_8832brvt(struct rf_info *rf, enum phl_phy_idx phy_idx,
				  u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u8 gp = 0x0;
	u32 a_idxrxgain[4] = {0x184, 0x198, 0x304, 0x318};
	u32 a_idxattc2[4] = {0x0, 0x3, 0x0, 0x3};
	...
	for(gp = 0; gp < 4; gp++){
		...
		notready = _iqk_one_shot_8832brvt(rf, phy_idx, path, ID_RXK);
	}
	...
	RF_DBG(rf, DBG_RF_IQK, "[IQK]S%x, kfail = 0x%x, [XYM]0x8%x3c = 0x%x\n", path, kfail, 1 << path, iqk_info->nb_rxcfir[path]);
...
__iram_func__
static bool _iqk_txk_group_sel_8832brvt(struct rf_info *rf, enum phl_phy_idx phy_idx,
				  u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	bool notready = false;
	...
	u32 a_track_range[4] = {0x0, 0x4, 0x5, 0x5};
	u32 a_gain_bb[4] = {0x0, 0x10, 0x0a, 0x10};
	u32 a_itqt[4] = {0x0, 0x12, 0x12, 0x1b};
	...
	for (gp = 0x1; gp < 0x4; gp++) {
		...
		notready = _iqk_one_shot_8832brvt(rf, phy_idx, path, ID_NBTXK);
		iqk_info->nb_txcfir[path] = halrf_rreg(rf, 0x8138, MASKDWORD);		
		...
		notready = _iqk_one_shot_8832brvt(rf, phy_idx, path, ID_TXK);
	}
	if (!notready)
		kfail = (bool)halrf_rreg(rf, 0x8008, BIT(26));
	...
	RF_DBG(rf, DBG_RF_IQK, "[IQK]S%x, kfail = 0x%x, [XYM]0x8%x38 = 0x%x\n", path, kfail, 1 << path, iqk_info->nb_txcfir[path]);
	return kfail;
}
...
//Init RF gain & tone idx= 8.25Mhz
	...
	halrf_wreg(rf, 0x802c, 0x00000fff, 0x021);
	halrf_wreg(rf, 0x8000, MASKDWORD, 0x00001709 |(1 << (4 + path)));
				...
				u8 path)
{
	...
	//LOK
	for (i = 0; i < 3; i++) {		
		_lok_res_table_8832brvt(rf, path, 0x3 + i);
	...
	if (iqk_info->is_nbiqk) {
		...
		iqk_info->iqk_tx_fail[0][path] =
			_iqk_txk_group_sel_8832brvt(rf, phy_idx, path);
	}
#endif
#if 1
	//RX
	...
	if (iqk_info->is_nbiqk) {
		...
		iqk_info->iqk_rx_fail[0][path] =
			_iqk_rxk_group_sel_8832brvt(rf, phy_idx, path);
	}
#endif
#endif

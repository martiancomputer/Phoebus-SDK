...
u32 _is_efem_8832brvt(struct rf_info *rf)
{
...
u32 _is_ifem_8832brvt(struct rf_info *rf)
{
	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
...
#if 0
		halrf_wreg(rf, 0x5804, 0xffffffff, 0x02b21000);
		halrf_wreg(rf, 0x5808, 0xffffffff, 0x02b21000);
#endif
...
#if 0
		halrf_wreg(rf, 0x7804, 0xffffffff, 0x02b21000);
		halrf_wreg(rf, 0x7808, 0xffffffff, 0x02b21000);
#endif
...
u32 _halrf_tssi_get_ofdm_efuse_bw_group_8832brvt(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 offset_index = 0;
...
u32 _halrf_tssi_get_6g_efuse_group_8832brvt(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 offset_index = 0;
					...
					enum phl_phy_idx phy, enum rf_path path)
{
	...
	if (band == BAND_ON_24G || band == BAND_ON_5G) {
		...
		group_idx = _halrf_tssi_get_6g_efuse_group_8832brvt(rf, phy);
...
s8 _halrf_tssi_get_ofdm_efuse_tssi_bw_diff_8832brvt(struct rf_info *rf,
					enum phl_phy_idx phy, enum channel_width bw, enum rf_path path)
{
	...
	if (band == BAND_ON_24G || band == BAND_ON_5G) {
		group_idx = _halrf_tssi_get_ofdm_efuse_bw_group_8832brvt(rf, phy);
...
u32 _halrf_tssi_get_tssi_trim_6g_efuse_group_8832brvt(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 group_index = 0;
					...
					enum phl_phy_idx phy, enum rf_path path)
{
	...
	if (band == BAND_ON_24G || band == BAND_ON_5G) {
		...
		group_idx = _halrf_tssi_get_tssi_trim_6g_efuse_group_8832brvt(rf, phy);
...
bool _halrf_tssi_alimentk_finish_bit_8832brvt(struct rf_info *rf,
					...
					enum phl_phy_idx phy, enum rf_path path)
{
	...
	/*get tssi cw*/
	if (!_halrf_tssi_alimentk_finish_bit_8832brvt(rf, phy, path, &power_cw[0])) {
	...
	/*get tssi cw*/
	if (!_halrf_tssi_alimentk_finish_bit_8832brvt(rf, phy, path, &power_cw[1])) {
		finish_time = _os_get_cur_time_us();
		tssi_info->tssi_alimk_time = tssi_info->tssi_alimk_time + (finish_time - start_time);
		return;
	}
	...
	/*get tssi cw*/
	if (!_halrf_tssi_alimentk_finish_bit_8832brvt(rf, phy, path, &power_cw[2])) {
		finish_time = _os_get_cur_time_us();
		tssi_info->tssi_alimk_time = tssi_info->tssi_alimk_time + (finish_time - start_time);
		return;
	}
	...
	/*get tssi cw*/
	if (!_halrf_tssi_alimentk_finish_bit_8832brvt(rf, phy, path, &power_cw[3])) {
		finish_time = _os_get_cur_time_us();
		tssi_info->tssi_alimk_time = tssi_info->tssi_alimk_time + (finish_time - start_time);
		return;
	}
					...
					enum phl_phy_idx phy)
{
	...
	u8 ch_idx = _halrf_ch_to_idx_8832brvt(rf, channel);
//	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
	u32 i, path_tmp = RF_PATH_A, path_max = TSSI_PATH_MAX_8832BRVT;
	...
	if (rf->hal_com->dbcc_en) {
		if (phy == HW_PHY_0) {
			...
			path_tmp = RF_PATH_B;
			path_max = TSSI_PATH_MAX_8832BRVT;
		}
	}
						...
						enum phl_phy_idx phy)
{
	...
	if (_is_efem_8832brvt(rf)) {
		...
		for (i = FLASH_INFO_RF_5G_BW20M_BW40M_TSSI_DIFF_A_8832BRVT_1, j = 0; i <= FLASH_INFO_RF_5G_BW20M_BW40M_TSSI_DIFF_A_8832BRVT_14; i++, j++) {
		...
		for (i = FLASH_INFO_RF_5G_BW80M_BW40M_TSSI_DIFF_A_8832BRVT_1, j = 0; i <= FLASH_INFO_RF_5G_BW80M_BW40M_TSSI_DIFF_A_8832BRVT_14; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		...
		for (i = FLASH_INFO_RF_5G_BW160M_BW40M_TSSI_DIFF_A_8832BRVT_1, j = 0; i <= FLASH_INFO_RF_5G_BW160M_BW40M_TSSI_DIFF_A_8832BRVT_14; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
	...
	if (_is_efem_8832brvt(rf)) {
		...
		for (i = FLASH_INFO_RF_5G_BW20M_BW40M_TSSI_DIFF_B_8832BRVT_1, j = 0; i <= FLASH_INFO_RF_5G_BW20M_BW40M_TSSI_DIFF_B_8832BRVT_14; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		...
		for (i = FLASH_INFO_RF_5G_BW80M_BW40M_TSSI_DIFF_B_8832BRVT_1, j = 0; i <= FLASH_INFO_RF_5G_BW80M_BW40M_TSSI_DIFF_B_8832BRVT_14; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		...
		for (i = FLASH_INFO_RF_5G_BW160M_BW40M_TSSI_DIFF_B_8832BRVT_1, j = 0; i <= FLASH_INFO_RF_5G_BW160M_BW40M_TSSI_DIFF_B_8832BRVT_14; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
					...
					enum phl_phy_idx phy, u32 tssi_de_offset, u8 path)
{
	...
	if (path == RF_PATH_A) {
		...
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832brvt(rf, phy, RF_PATH_A);
		...
		/*OFDM*/
		ofmd_de = _halrf_tssi_get_ofdm_efuse_tssi_de_8832brvt(rf, phy, RF_PATH_A);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832brvt(rf, phy, RF_PATH_A);
		...
		/*CCK*/
		group_idx = _halrf_tssi_get_cck_efuse_group_8832brvt(rf, phy);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832brvt(rf, phy, RF_PATH_B);
		...
		/*OFDM*/
		ofmd_de = _halrf_tssi_get_ofdm_efuse_tssi_de_8832brvt(rf, phy, RF_PATH_B);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832brvt(rf, phy, RF_PATH_B);
		tssi_tmp = ofmd_de + tssi_trim + s_tssi_de_offset;
						...
						enum phl_phy_idx phy)
{
	...
	u32 path_tmp = RF_PATH_A, path_max = TSSI_PATH_MAX_8832BRVT;
	...
	for (i = path_tmp; i < path_max; i++) {
		...
		/*TSSI BW 20M*/
		tssi_bw_diff = _halrf_tssi_get_ofdm_efuse_tssi_bw_diff_8832brvt(rf, phy, CHANNEL_WIDTH_20, i);
		...
		/*TSSI BW 40M*/
		tssi_bw_diff = _halrf_tssi_get_ofdm_efuse_tssi_bw_diff_8832brvt(rf, phy, CHANNEL_WIDTH_40, i);
		tmp = ofmd_de + tssi_trim + tssi_bw_diff;
		...
		/*TSSI BW 80M*/
		tssi_bw_diff = _halrf_tssi_get_ofdm_efuse_tssi_bw_diff_8832brvt(rf, phy, CHANNEL_WIDTH_80, i);
		tmp = ofmd_de + tssi_trim + tssi_bw_diff;
		...
		/*TSSI BW 1600M*/
		tssi_bw_diff = _halrf_tssi_get_ofdm_efuse_tssi_bw_diff_8832brvt(rf, phy, CHANNEL_WIDTH_160, i);
		tmp = ofmd_de + tssi_trim + tssi_bw_diff;

...
u8 halrf_get_thermal_ex_8852c(struct rf_info *rf, enum rf_path rf_path)
{
...
s8 halrf_get_ther_protected_threshold_8852c(struct rf_info *rf)
{
	...
	tmp_a = halrf_inn_get_thermal(rf, RF_PATH_A);
	tmp_b = halrf_inn_get_thermal(rf, RF_PATH_B);
...
void halrf_set_gpio_8852c(struct rf_info *rf, enum phl_phy_idx phy)
{
	...
	if ((rf->phl_com->id.id & 0xfff) == 0x109) {	/*HP*/
		halrf_gpio_setting(rf, 0, 0x1, false, HALRF_TRSW_RFM);	// GPIO0 pathA TRSW
		halrf_gpio_setting(rf, 2, 0x1, false, HALRF_GNT_BT);	// GPIO2 pathA BT GNT

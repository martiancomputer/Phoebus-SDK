...
u8 halrf_get_thermal_ex_8192xb(struct rf_info *rf, enum rf_path rf_path)
{
...
void halrf_op5k_trigger_8192xb(struct rf_info *rf)
{
	...
	while ((retry[RF_PATH_A] < 2 || retry[RF_PATH_B] < 2)) {
			...
			for (i = 0, k = 0; i < OP5K_RESET_CNT_DATA; i++) {
				if (zero_rst_cnt[j][k] == 0 && op5k->rst_cnt[j][i] == 0) {
					...
					zero_rst_cnt_end[j][k] = i;
		 			...
		 			if (i > 0 && op5k->rst_cnt[j][i - 1] == 0 && op5k->rst_cnt[j][i] == 0) {
						zero_rst_cnt[j][k]++;
						zero_rst_cnt_end[j][k] = i;
		...
		for (j = 0; j < OP5K_MAX_RF_PATH_8192XB; j++) {
			...
			op5k->thermal_op5k[j] = halrf_inn_get_thermal(rf, j);
...
void halrf_op5k_tracking_8192xb(struct rf_info *rf)
{
	...
	for (path = 0; path < 2; path++) {
		thermal_value[path] = halrf_inn_get_thermal(rf, path);

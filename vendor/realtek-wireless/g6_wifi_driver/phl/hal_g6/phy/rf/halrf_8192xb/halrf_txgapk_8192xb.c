	...
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	...
	halrf_wrf(rf, path, 0x05, MASKRF, 0x0);
					...
					enum phl_phy_idx phy)
{
	...
	for (path = 0; path < TXGAPK_RF_PATH_MAX_8192XB; path++) {
		_txgapk_backup_kip_8192xb(rf, kip_reg, kip_bkup, path, sizeof(kip_reg)/sizeof(kip_reg[0]));
		_txgapk_bkup_rf_8192xb(rf, rf_reg, rf_bkup, path, sizeof(rf_reg)/sizeof(rf_reg[0]));
	}
	...
	for (path = 0; path < TXGAPK_RF_PATH_MAX_8192XB; path++) {
		_txgapk_reload_kip_8192xb(rf, kip_reg, kip_bkup, path, sizeof(kip_reg)/sizeof(kip_reg[0]));
		_txgapk_reload_rf_8192xb(rf, rf_reg, rf_bkup, path, sizeof(rf_reg)/sizeof(rf_reg[0]));
		halrf_wrf(rf, path, 0x5, MASKRF, 0x1);

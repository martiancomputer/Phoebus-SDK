...
bool halrf_ch_setting_8192xb(struct rf_info *rf,   enum rf_path path, u8 central_ch, enum band_type band, bool is_dav)
{
	...
	/*==== [Set RF Reg 0x18] ====*/
	rf_reg18 &= ~0x3e3ff; /*[17:16],[9:8],[7:0],[15:13]*/
	...
	case BAND_ON_24G :
		...
		if(rf->hal_com->cv >= CAV){
			if(central_ch <= 0x4) { //ch4
				temp = 0x000a0;
				halrf_wrf(rf, path, 0xbe, MASKRF, temp);
		...
		if (path == RF_PATH_A) {
			c = 0;
			while (c < 30) {
				if (halrf_rrf(rf, path, MASKRF, 0xbe) == temp)
					break;
				c++;
				halrf_delay_us(rf, 5);
				halrf_wrf(rf, path, 0xbe, MASKRF, temp);
			...
			if (c == 30)
				RF_DBG(rf, DBG_RF_RFK, "halrf_ch_setting_8192xb: read 0xbe timeout!!!! Ta: %x BE: %x\n",temp, halrf_rrf(rf, path, MASKRF, 0xbe));
		}
...
//		halrf_wreg(rf, 0xc0d8 + (path << 8), 0x000001E0, 0x7); /*[8:5] for ADC 320M*/
		...
		halrf_wrf(rf, path, 0x8f, 0x00E00, 0x1); /*for DPK DC*/
		RF_DBG(rf, DBG_RF_RXDCK, "[RX_DCK] set 0x8f to 0x%x\n", halrf_rrf(rf, path, 0x8f, MASKRF));
		halrf_wreg(rf, 0x5670, BIT(1), 0x1); /*iqk_control_sw_si*/
		...
		halrf_wreg(rf, 0x5670, BIT(1), 0x0); /*iqk_control_sw_si*/
		halrf_wrf(rf, path, 0x8f, 0x00E00, 0x0); /*for DPK DC*/

...
void halrf_addck_wcheck_8832br(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u32 i = 0;
	...
	do {
		...
		if (i > 10)
			break;
	} while ((dack->addck_d[0][0] > 0x100 && dack->addck_d[0][0] < 0x300) ||
		(dack->addck_d[0][1] > 0x100 && dack->addck_d[0][1] < 0x300) ||
		(dack->addck_d[1][0] > 0x100 && dack->addck_d[1][0] < 0x300) ||
		(dack->addck_d[1][1] > 0x100 && dack->addck_d[1][1] < 0x300));
	dack->addck_retry_cnt = i - 1;
}
...
//	halrf_btc_rfk_ntfy(rf, phy_map, RF_BTC_DACK, RFK_ONESHOT_START);
	RF_DBG(rf, DBG_RF_DACK, "[DACK]b_ADDCK 0xc0c4 = 0x%x\n", halrf_rreg(rf, 0xc0c4, MASKDWORD));
	halrf_addck_wcheck_8832br(rf);
//	halrf_addck_ori_8832br(rf);

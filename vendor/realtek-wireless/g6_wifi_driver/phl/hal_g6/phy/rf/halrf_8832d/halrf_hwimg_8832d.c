...
#include "halrf_hwimg_raw_data_8832d.h"
#include "halrf_hwimg_nctl_raw_data_8832d.h"
#include "halrf_hwimg_raw_data_w_bt_8832d.h"

bool halrf_check_cond_8832d(struct rf_info *rf, u32 para_opt)
{
...
u32
halrf_get_8832d_nctl_reg_ver(void)
{
	return 0x13;
}

u32
halrf_get_8832d_radio_reg_ver(void)
{
	return RF_RELEASE_VERSION_8832D;
}

void halrf_config_8832d_store_radio_a_reg(struct rf_info *rf,
					u32 addr, u32 data)
{
...
void halrf_config_8832d_store_radio_b_reg(struct rf_info *rf,
					u32 addr, u32 data)
{
	struct halrf_radio_info *radio = &rf->radio_info;
...
void halrf_config_8832d_write_radio_a_reg_to_fw(struct rf_info *rf)
{
	struct halrf_radio_info *radio = &rf->radio_info;
...
void halrf_config_8832d_write_radio_b_reg_to_fw(struct rf_info *rf)
{
	struct halrf_radio_info *radio = &rf->radio_info;
...
void halrf_config_8832d_radio_to_fw(struct rf_info *rf)
{
	halrf_config_8832d_write_radio_a_reg_to_fw(rf);
	halrf_config_8832d_write_radio_b_reg_to_fw(rf);
}

void halrf_config_8832d_nctl_reg(struct rf_info *rf)
{
#if 1
	u32	i = 0;
	u32	array_len = 0x0;
	...
	array_len = sizeof(array_mp_8832d_nctl_reg) / sizeof(u32);
	array = (u32 *) &array_mp_8832d_nctl_reg;
	...
	while ((i + 1) < array_len) {
		v1 = array[i];
		v2 = array[i + 1];
		halrf_cfg_rf_nctl_8832d(rf, v1, MASKDWORD, v2);
		i += 2;
	}
#ifdef HALRF_CONFIG_FW_IO_OFLD_SUPPORT
	if (rf->phl_com->dev_cap.io_ofld)
...
bool halrf_sel_headline_8832d(struct rf_info *rf, u32 *array, u32 array_len,
			      u8 *headline_size, u8 *headline_idx)
{
	...
	*headline_size = 0;
	...
	compare_target = ((rfe_drv & 0xff) << 16) | (DONT_CARE_8832D & 0xff);
...
void halrf_flag_2_default_8832d(bool *is_matched, bool *find_target)
{
	*is_matched = true;
	*find_target = false;
}
...
void
halrf_config_8832d_radio_a_reg(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct rtw_hal_com_t *hal = rf->hal_com;
	struct halrf_radio_info *radio = &rf->radio_info;
	...
	if (is_form_folder) {
		...
		array_len = sizeof(array_mp_8832d_radioa) / sizeof(u32);
		array = (u32 *)array_mp_8832d_radioa;
	}

	RF_DBG(rf, DBG_RF_INIT, "RFCR_form_folder=%d, len=%d\n",
	       is_form_folder, array_len);

	if (!halrf_sel_headline_8832d(rf, array, array_len, &h_size, &h_idx)) {
	...
	halrf_flag_2_default_8832d(&is_matched, &find_target);
	while ((i + 1) < array_len) {
		v1 = array[i];
		v2 = array[i + 1];
		i += 2;
		...
		default:
			if (is_matched) {
				halrf_cfg_rf_radio_a_8832d(rf, v1, v2);
				halrf_config_8832d_store_radio_a_reg(rf, v1, v2);
			}
			break;
		}
	}
...
void
halrf_config_8832d_radio_b_reg(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct rtw_hal_com_t *hal = rf->hal_com;
	struct halrf_radio_info *radio = &rf->radio_info;
	struct rtw_para_info_t *phy_reg_info = NULL;
	...
	if (is_form_folder) {
		...
		array_len = sizeof(array_mp_8832d_radiob) / sizeof(u32);
		array = (u32 *)array_mp_8832d_radiob;
	}
	...
	while ((i + 1) < array_len) {
		...
		default:
			if (is_matched) {
				halrf_cfg_rf_radio_b_8832d(rf, v1, v2);
				halrf_config_8832d_store_radio_b_reg(rf, v1, v2);
			}
			break;
		}
	}
...
void
halrf_config_8832d_store_power_by_rate(struct rf_info *rf,
	enum phl_phy_idx phy)
{
	...
	if (is_form_folder) {
		...
		array_len = sizeof(array_mp_8832d_txpwr_byrate) / sizeof(u32);
		array = (u32 *)array_mp_8832d_txpwr_byrate;
	}
...
void
halrf_config_8832d_store_power_limit(struct rf_info *rf,
	enum phl_phy_idx phy)
{
	...
	if (is_form_folder) {
		...
		array_len = sizeof(array_mp_8832d_txpwr_lmt) / sizeof(struct halrf_tx_pw_lmt);
		array = array_mp_8832d_txpwr_lmt;
...
void
halrf_config_8832d_store_power_limit_6g(struct rf_info *rf,
	enum phl_phy_idx phy)
{
	const struct halrf_tx_pw_lmt *array = NULL;
	struct halrf_tx_pw_lmt *parray = NULL;
	...
	if (is_form_folder) {
		...
		array_len = sizeof(array_mp_8832d_txpwr_lmt_6g) / sizeof(struct halrf_tx_pw_lmt);
		array = array_mp_8832d_txpwr_lmt_6g;
...
void
halrf_config_8832d_store_power_limit_ru(struct rf_info *rf,
	enum phl_phy_idx phy)
{
	...
	if (is_form_folder) {
		...
		array_len = sizeof(array_mp_8832d_txpwr_lmt_ru) / sizeof(struct halrf_tx_pw_lmt_ru);
		array = array_mp_8832d_txpwr_lmt_ru;
...
void
halrf_config_8832d_store_power_limit_ru_6g(struct rf_info *rf,
	enum phl_phy_idx phy)
{
	const struct halrf_tx_pw_lmt_ru *array = NULL;
	struct halrf_tx_pw_lmt_ru *parray = NULL;
	...
	if (is_form_folder) {
		...
		array_len = sizeof(array_mp_8832d_txpwr_lmt_ru_6g) / sizeof(struct halrf_tx_pw_lmt_ru);
		array = array_mp_8832d_txpwr_lmt_ru_6g;
...
void
halrf_config_8832d_store_pwr_track(struct rf_info *rf,
	enum phl_phy_idx phy)
{
	...
	if (is_form_folder) {
		...
		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_2ga_p,
			(void *)delta_swingidx_mp_2ga_p_txpwrtrkssi_8832d,
			DELTA_SWINGIDX_SIZE);
		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_2ga_n,
			(void *)delta_swingidx_mp_2ga_n_txpwrtrkssi_8832d,
			DELTA_SWINGIDX_SIZE);
		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_2gb_p,
			(void *)delta_swingidx_mp_2gb_p_txpwrtrkssi_8832d,
			DELTA_SWINGIDX_SIZE);
		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_2gb_n,
			(void *)delta_swingidx_mp_2gb_n_txpwrtrkssi_8832d,
			DELTA_SWINGIDX_SIZE);

		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_2g_cck_a_p,
			(void *)delta_swingidx_mp_2g_cck_a_p_txpwrtrkssi_8832d,
			DELTA_SWINGIDX_SIZE);
		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_2g_cck_a_n,
			(void *)delta_swingidx_mp_2g_cck_a_n_txpwrtrkssi_8832d,
			DELTA_SWINGIDX_SIZE);
		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_2g_cck_b_p,
			(void *)delta_swingidx_mp_2g_cck_b_p_txpwrtrkssi_8832d,
			DELTA_SWINGIDX_SIZE);
		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_2g_cck_b_n,
			(void *)delta_swingidx_mp_2g_cck_b_n_txpwrtrkssi_8832d,
			DELTA_SWINGIDX_SIZE);

		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_5ga_p,
			(void *)delta_swingidx_mp_5ga_p_txpwrtrkssi_8832d,
			DELTA_SWINGIDX_SIZE * 3);
		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_5ga_n,
			(void *)delta_swingidx_mp_5ga_n_txpwrtrkssi_8832d,
			DELTA_SWINGIDX_SIZE * 3);
		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_5gb_p,
			(void *)delta_swingidx_mp_5gb_p_txpwrtrkssi_8832d,
			DELTA_SWINGIDX_SIZE * 3);
		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_5gb_n,
			(void *)delta_swingidx_mp_5gb_n_txpwrtrkssi_8832d,
			DELTA_SWINGIDX_SIZE * 3);

		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_6ga_p,
			(void *)delta_swingidx_mp_6ga_p_txpwrtrkssi_8832d,
			sizeof(pwr_trk->delta_swing_table_idx_6ga_p));
		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_6ga_n,
			(void *)delta_swingidx_mp_6ga_n_txpwrtrkssi_8832d,
			sizeof(pwr_trk->delta_swing_table_idx_6ga_n));
		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_6gb_p,
			(void *)delta_swingidx_mp_6gb_p_txpwrtrkssi_8832d,
			sizeof(pwr_trk->delta_swing_table_idx_6gb_p));
		hal_mem_cpy(hal, pwr_trk->delta_swing_table_idx_6gb_n,
			(void *)delta_swingidx_mp_6gb_n_txpwrtrkssi_8832d,
			sizeof(pwr_trk->delta_swing_table_idx_6gb_n));
	}
}

void
_halrf_config_rfe_xtal_track_table_8832d(struct rf_info *rf)
{
#if 0
	...
	hal_mem_cpy(hal, xtal_trk->delta_swing_xtal_table_idx_p,
		(void *)delta_swing_xtal_mp_p_txxtaltrack_8832d,
		DELTA_SWINGIDX_SIZE);
	hal_mem_cpy(hal, xtal_trk->delta_swing_xtal_table_idx_n,
		(void *)delta_swing_xtal_mp_n_txxtaltrack_8832d,
		DELTA_SWINGIDX_SIZE);
#endif
}

void
halrf_config_8832d_store_xtal_track(struct rf_info *rf,
	enum phl_phy_idx phy)
{
#if 0
	...
	if (is_form_folder) {
		...
		_halrf_config_rfe_xtal_track_table_8832d(rf);
	}
#else
	_halrf_config_rfe_xtal_track_table_8832d(rf);
#endif
...
void halrf_cfg_8832d_radio_b_w_bt_status(struct rf_info *rf, bool bt_connect)
{
	u32	i = 0;
	u32	array_len = 0x0;
	u32 *array = NULL;
	...
	if (bt_connect) {
		array_len = sizeof(array_mp_8832d_radiob_with_bt_con) / sizeof(u32);
		array = (u32 *) &array_mp_8832d_radiob_with_bt_con;
	} else {
		array_len = sizeof(array_mp_8832d_radiob_with_bt_discon) / sizeof(u32);
		array = (u32 *) &array_mp_8832d_radiob_with_bt_discon;
	}	

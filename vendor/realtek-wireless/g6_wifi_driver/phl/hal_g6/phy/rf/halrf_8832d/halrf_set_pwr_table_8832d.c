...
s8 _halrf_avg_power_8832d(struct rf_info *rf, enum phl_phy_idx phy, s8 *value, s8 n)
{
	u8 i;
	s16 total = 0;
...
void _halrf_bub_sort_8832d(struct rf_info *rf, enum phl_phy_idx phy, s8 *data, u32 n)
{
	s32 i, j, sp;
	s8 temp;
	u32 k;
...
bool halrf_set_power_by_rate_to_struct_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
...
void halrf_set_fix_power_to_struct_8832d(struct rf_info *rf, enum phl_phy_idx phy, s8 dbm)
{
...
void halrf_get_power_limit_to_struct_20m_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct rtw_tpu_pwr_imt_info *lmt = &rf->hal_com->band[phy].rtw_tpu_i.rtw_tpu_pwr_imt_i;
	struct rtw_hal_com_t *hal = rf->hal_com;
...
void halrf_get_power_limit_to_struct_40m_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct rtw_tpu_pwr_imt_info *lmt = &rf->hal_com->band[phy].rtw_tpu_i.rtw_tpu_pwr_imt_i;
	struct rtw_hal_com_t *hal = rf->hal_com;
	u8 ch = rf->hal_com->band[phy].cur_chandef.center_ch;
...
void halrf_get_power_limit_to_struct_80m_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct rtw_tpu_pwr_imt_info *lmt = &rf->hal_com->band[phy].rtw_tpu_i.rtw_tpu_pwr_imt_i;
	struct rtw_hal_com_t *hal = rf->hal_com;
	u8 ch = rf->hal_com->band[phy].cur_chandef.center_ch;
...
void halrf_get_power_limit_to_struct_160m_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct rtw_tpu_pwr_imt_info *lmt = &rf->hal_com->band[phy].rtw_tpu_i.rtw_tpu_pwr_imt_i;
	struct rtw_hal_com_t *hal = rf->hal_com;
	u8 ch = rf->hal_com->band[phy].cur_chandef.center_ch;
...
bool halrf_set_power_limit_to_struct_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct rtw_tpu_pwr_imt_info *lmt = &rf->hal_com->band[phy].rtw_tpu_i.rtw_tpu_pwr_imt_i;
	...
	if (bw == CHANNEL_WIDTH_20)
		halrf_get_power_limit_to_struct_20m_8832d(rf, phy);
	else if (bw == CHANNEL_WIDTH_40)
		halrf_get_power_limit_to_struct_40m_8832d(rf, phy);
	else if (bw == CHANNEL_WIDTH_80)
		halrf_get_power_limit_to_struct_80m_8832d(rf, phy);
	else
		halrf_get_power_limit_to_struct_160m_8832d(rf, phy);
...
void halrf_get_power_limit_ru_to_struct_20m_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct rtw_tpu_info *tpu = &rf->hal_com->band[phy].rtw_tpu_i;
	struct rtw_hal_com_t *hal = rf->hal_com;
	u8 ch = rf->hal_com->band[phy].cur_chandef.center_ch;
...
void halrf_get_power_limit_ru_to_struct_40m_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct rtw_tpu_info *tpu = &rf->hal_com->band[phy].rtw_tpu_i;
	struct rtw_hal_com_t *hal = rf->hal_com;
	u8 ch = rf->hal_com->band[phy].cur_chandef.center_ch;
...
void halrf_get_power_limit_ru_to_struct_80m_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct rtw_tpu_info *tpu = &rf->hal_com->band[phy].rtw_tpu_i;
	struct rtw_hal_com_t *hal = rf->hal_com;
	u8 ch = rf->hal_com->band[phy].cur_chandef.center_ch;
...
void halrf_get_power_limit_ru_to_struct_160m_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct rtw_tpu_info *tpu = &rf->hal_com->band[phy].rtw_tpu_i;
	struct rtw_hal_com_t *hal = rf->hal_com;
	u8 ch = rf->hal_com->band[phy].cur_chandef.center_ch;
...
bool halrf_set_power_limit_ru_to_struct_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct rtw_tpu_info *tpu = &rf->hal_com->band[phy].rtw_tpu_i;
	u8 bw = rf->hal_com->band[phy].cur_chandef.bw;
	u8 i, j, k;
	...
	if (bw == CHANNEL_WIDTH_20)
		halrf_get_power_limit_ru_to_struct_20m_8832d(rf, phy);
	else if (bw == CHANNEL_WIDTH_40)
		halrf_get_power_limit_ru_to_struct_40m_8832d(rf, phy);
	else if (bw == CHANNEL_WIDTH_80)
		halrf_get_power_limit_ru_to_struct_80m_8832d(rf, phy);
	else
		halrf_get_power_limit_ru_to_struct_160m_8832d(rf, phy);
...
void _halrf_set_tx_shape_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
...
bool _halrf_set_power_8832d(struct rf_info *rf, enum phl_phy_idx phy,
	enum phl_pwr_table pwr_table)
{
	...
	if (pwr_table & PWR_BY_RATE) {
		if (pwr->pwr_by_rate_switch == PW_BY_RATE_ALL_SAME) {
			halrf_set_fix_power_to_struct_8832d(rf, phy,
			...
			if (pwr->pwr_limit_switch == PWLMT_BY_EFUSE) {
				if (pwr->pwr_table_switch_efuse == 0) {
					halrf_set_power_by_rate_to_struct_8832d(rf, phy);
		...
		if (rf->dbg_component & DBG_RF_POWER) {
			...
			for (i = 0; i < TX_PWR_BY_RATE_NUM_MAC; i = i + 4)
				halrf_mac_get_pwr_reg_8832d(rf, phy, (0xd2c0 + i), 0xffffffff);
		}
	}

	if (pwr_table & PWR_LIMIT) {
		_halrf_set_tx_shape_8832d(rf, phy);
		if (!halrf_set_power_limit_to_struct_8832d(rf, phy)) {
			RF_DBG(rf, DBG_RF_POWER, "halrf_set_power_limit_to_struct_8832d return fail\n");
			return false;
		}
		...
		if (rf->dbg_component & DBG_RF_POWER) {
			...
			for (i = 0; i < TX_PWR_LIMIT_NUM_MAC; i = i + 4)
				halrf_mac_get_pwr_reg_8832d(rf, phy, (0xd2ec + i), 0xffffffff);
		}
	}

	if (pwr_table & PWR_LIMIT_RU) {
		/*_halrf_set_tx_shape_8832d(rf, phy);*/
		if (!halrf_set_power_limit_ru_to_struct_8832d(rf, phy)) {
			RF_DBG(rf, DBG_RF_POWER, "halrf_set_power_limit_ru_to_struct_8832d return fail\n");
			return false;
		}
		...
		if (rf->dbg_component & DBG_RF_POWER) {
			...
			for (i = 0; i < 48; i = i + 4)
				halrf_mac_get_pwr_reg_8832d(rf, phy, (0xd33c + i), 0xffffffff);
		}
	}
...
void _halrf_set_ext_power_diff_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
...
void halrf_set_ref_power_to_struct_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct rtw_tpu_info *tpu = &rf->hal_com->band[phy].rtw_tpu_i;
...
bool halrf_set_power_8832d(struct rf_info *rf, enum phl_phy_idx phy,
	enum phl_pwr_table pwr_table)
{
#if 0
	if (rf->is_coex == true) {
	...
	if (!_halrf_set_power_8832d(rf, phy, pwr_table)) {
		RF_DBG(rf, DBG_RF_POWER, "_halrf_set_power_8832d return fail\n");
		return false;
	}

	_halrf_set_ext_power_diff_8832d(rf, phy);
	halrf_set_bw160m_diff_8832d(rf, phy);
	halrf_set_bw160m_wa_8832d(rf, phy);

	return true;
}

void halrf_pwr_by_rate_info_8832d(struct rf_info *rf,
		char input[][16], u32 *_used, char *output, u32 *_out_len, enum phl_phy_idx phy)
{
	...
	cck_ref = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd200, 0x0007fc00);
	ofdm_ref = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd200, 0x0ff80000);
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd208, 0xffffffff);
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd2c0, 0xffffffff);
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd2c4, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-36s = %s%d.%d / %s%d.%d / %s%d.%d / %s%d.%d\n",
		 "OFDM 18M / 12M / 9M / 6M",
		 halrf_pwr_is_minus(rf, (reg_tmp & 0x7f000000) >> 24) ? "-" : "",
		 halrf_show_pwr_table(rf, (reg_tmp & 0x7f000000) >> 24) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd2c8, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-36s = %s%d.%d / %s%d.%d / %s%d.%d / %s%d.%d\n",
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd2cc, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-36s = %s%d.%d / %s%d.%d / %s%d.%d / %s%d.%d\n",
		"MCS3 / MCS2 / MCS1 / MCS0",
		halrf_pwr_is_minus(rf, (reg_tmp & 0x7f000000) >> 24) ? "-" : "",
		halrf_show_pwr_table(rf, (reg_tmp & 0x7f000000) >> 24) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd2d0, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-36s = %s%d.%d / %s%d.%d / %s%d.%d / %s%d.%d\n",
		 "MCS7 / MCS6 / MCS5 / MCS4",
		halrf_pwr_is_minus(rf, (reg_tmp & 0x7f000000) >> 24) ? "-" : "",
		halrf_show_pwr_table(rf, (reg_tmp & 0x7f000000) >> 24) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd2d4, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-36s = %s%d.%d / %s%d.%d / %s%d.%d / %s%d.%d\n",
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd2d8, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-36s = %s%d.%d / %s%d.%d / %s%d.%d / %s%d.%d\n",
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd2dc, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-36s = %s%d.%d / %s%d.%d / %s%d.%d / %s%d.%d\n",
		 "MCS3 / MCS2 / MCS1 / MCS0",
		 halrf_pwr_is_minus(rf, (reg_tmp & 0x7f000000) >> 24) ? "-" : "",
		 halrf_show_pwr_table(rf, (reg_tmp & 0x7f000000) >> 24) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd2e0, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-36s = %s%d.%d / %s%d.%d / %s%d.%d / %s%d.%d\n",
		 "MCS7 / MCS6 / MCS5 / MCS4",
		halrf_pwr_is_minus(rf, (reg_tmp & 0x7f000000) >> 24) ? "-" : "",
		halrf_show_pwr_table(rf, (reg_tmp & 0x7f000000) >> 24) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd2e4, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-36s = %s%d.%d / %s%d.%d / %s%d.%d / %s%d.%d\n",
		 "MCS11 / MCS10 / MCS9 / MCS8",
		 halrf_pwr_is_minus(rf, (reg_tmp & 0x7f000000) >> 24) ? "-" : "",
		 halrf_show_pwr_table(rf, (reg_tmp & 0x7f000000) >> 24) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd2e8, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-36s = %s%d.%d / %s%d.%d / %s%d.%d / %s%d.%d\n",
		 "DCM MCS4 / MCS3 / MCS1 / MCS0",
		halrf_pwr_is_minus(rf, (reg_tmp & 0x7f000000) >> 24) ? "-" : "",
		halrf_show_pwr_table(rf, (reg_tmp & 0x7f000000) >> 24) / 10,
...
void halrf_pwr_limit_info_8832d(struct rf_info *rf,
		char input[][16], u32 *_used, char *output, u32 *_out_len, enum phl_phy_idx phy)
{
	struct halrf_pwr_info *pwr = &rf->pwr_info;
	struct rtw_tpu_info *tpu = &rf->hal_com->band[phy].rtw_tpu_i;
	...
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s\n",
		 "Power Limit (Reg)",
		 halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd208, BIT(21)) ? "Enable Limit" : "Disable Limit");
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd2ec, 0xffffffff);
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd2f0, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s%d.%d\n",
		"OFDM NOBF",
		halrf_pwr_is_minus(rf, reg_tmp & 0x7f) ? "-" : "",
		halrf_show_pwr_table(rf, reg_tmp & 0x7f) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd300, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s%d.%d\n",
		"HT 40M NOBF",
		halrf_pwr_is_minus(rf, (reg_tmp & 0x7f0000) >> 16) ? "-" : "",
		halrf_show_pwr_table(rf, (reg_tmp & 0x7f0000) >> 16) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd308, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s%d.%d\n",
		"HT 80M NOBF",
		halrf_pwr_is_minus(rf, (reg_tmp & 0x7f0000) >> 16) ? "-" : "",
		halrf_show_pwr_table(rf, (reg_tmp & 0x7f0000) >> 16) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd30c, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s%d.%d\n",
		"HT 160M NOBF",
		halrf_pwr_is_minus(rf, (reg_tmp & 0x7f0000) >> 16) ? "-" : "",
		halrf_show_pwr_table(rf, (reg_tmp & 0x7f0000) >> 16) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd314, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s%d.%d\n",
		"CCK 20M NOBF",
		halrf_pwr_is_minus(rf, reg_tmp & 0x7f) ? "-" : "",
		halrf_show_pwr_table(rf, reg_tmp & 0x7f) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd318, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s%d.%d\n",
		"OFDM NOBF",
		halrf_pwr_is_minus(rf, reg_tmp & 0x7f) ? "-" : "",
		halrf_show_pwr_table(rf, reg_tmp & 0x7f) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd328, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s%d.%d / %s%d.%d\n",
		"HT 40M BF / NOBF",
		halrf_pwr_is_minus(rf, (reg_tmp & 0x7f000000) >> 24) ? "-" : "",
		halrf_show_pwr_table(rf, (reg_tmp & 0x7f000000) >> 24) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd330, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s%d.%d / %s%d.%d\n",
		"HT 80M BF / NOBF",
		halrf_pwr_is_minus(rf, (reg_tmp & 0x7f000000) >> 24) ? "-" : "",
		halrf_show_pwr_table(rf, (reg_tmp & 0x7f000000) >> 24) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd334, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s%d.%d / %s%d.%d\n",
		"HT 160M BF / NOBF",
		halrf_pwr_is_minus(rf, (reg_tmp & 0x7f000000) >> 24) ? "-" : "",
		halrf_show_pwr_table(rf, (reg_tmp & 0x7f000000) >> 24) / 10,
...
void halrf_pwr_limit_ru_info_8832d(struct rf_info *rf,
		char input[][16], u32 *_used, char *output, u32 *_out_len, enum phl_phy_idx phy)
{
	struct halrf_pwr_info *pwr = &rf->pwr_info;
	struct rtw_tpu_info *tpu = &rf->hal_com->band[phy].rtw_tpu_i;
	...
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s\n",
		 "Power Limit (Reg)",
		 halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd208, BIT(20)) ? "Enable Limit" : "Disable Limit");
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd33c, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s%d.%d\n",
		"RU26",
		halrf_pwr_is_minus(rf, reg_tmp & 0x7f) ? "-" : "",
		halrf_show_pwr_table(rf, reg_tmp & 0x7f) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd344, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s%d.%d\n",
		"RU52",
		halrf_pwr_is_minus(rf, reg_tmp & 0x7f) ? "-" : "",
		halrf_show_pwr_table(rf, reg_tmp & 0x7f) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd34c, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s%d.%d\n",
		"RU106",
		halrf_pwr_is_minus(rf, reg_tmp & 0x7f) ? "-" : "",
		halrf_show_pwr_table(rf, reg_tmp & 0x7f) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd354, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s%d.%d\n",
		"RU26",
		halrf_pwr_is_minus(rf, reg_tmp & 0x7f) ? "-" : "",
		halrf_show_pwr_table(rf, reg_tmp & 0x7f) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd35c, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s%d.%d\n",
		"RU52",
		halrf_pwr_is_minus(rf, reg_tmp & 0x7f) ? "-" : "",
		halrf_show_pwr_table(rf, reg_tmp & 0x7f) / 10,
	...
	reg_tmp = halrf_mac_get_pwr_reg_8832d(rf, phy, 0xd364, 0xffffffff);
	RF_DBG_CNSL(out_len, used, output + used, out_len - used, " %-30s = %s%d.%d\n",
		"RU106",
		halrf_pwr_is_minus(rf, reg_tmp & 0x7f) ? "-" : "",
		halrf_show_pwr_table(rf, reg_tmp & 0x7f) / 10,
...
void halrf_set_tx_shape_8832d(struct rf_info *rf, u8 tx_shape_idx)
{
	...
	if (tx_shape_idx == 255) {
		...
		_halrf_set_tx_shape_8832d(rf, 0);
		return;
	} else
...
void halrf_set_bw160m_diff_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
#if 0
	struct rtw_tpu_info *tpu = &rf->hal_com->band[phy].rtw_tpu_i;
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
...
void halrf_get_bw160m_wa_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
#ifdef PHL_PLATFORM_AP
	struct halrf_pwr_info *pwr = &rf->pwr_info;
	u32 i;
...
void halrf_set_bw160m_wa_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
#ifdef PHL_PLATFORM_AP
	struct halrf_pwr_info *pwr = &rf->pwr_info;

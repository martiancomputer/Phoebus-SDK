...
void _tssi_backup_bb_registers_8832d(
	struct rf_info *rf,
...
void _tssi_reload_bb_registers_8832d(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	u32 *reg,
	u32 *reg_backup,
...
u8 _halrf_ch_to_idx_8832d(struct rf_info *rf, u8 channel)
{
	u8	channelIndex;
...
u8 _halrf_idx_to_ch_8832d(struct rf_info *rf, u8 idx)
{
	u8 channelIndex;
...
u32 _is_efem_8832d(struct rf_info *rf)
{
...
u32 _is_ifem_8832d(struct rf_info *rf)
{
	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
...
void _halrf_tssi_hw_tx_8832d(struct rf_info *rf,
...
void _halrf_tssi_rf_setting_8832d(struct rf_info *rf,
...
void _halrf_tssi_set_sys_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	...
	halrf_wreg(rf, 0x0304, 0x0000ffff, 0x1313);
	halrf_wreg(rf, 0x0308, 0xff000000, 0x13);
	halrf_wreg(rf, 0x0314, 0xffff0000, 0x2041);
	halrf_wreg(rf, 0x0318, 0xffffffff, 0x00410041);
	halrf_wreg(rf, 0x0324, 0xffff0000, 0x0041);
	...
	if (path == RF_PATH_A) {
		/*TSSI ADC FIFO Clock follow RX ADC Clock by Ryan*/
		if (bw == CHANNEL_WIDTH_80)
			halrf_wreg(rf, 0x566c, 0x00030000, 0x1);
		else if (bw == CHANNEL_WIDTH_160 || bw == CHANNEL_WIDTH_80_80)
			halrf_wreg(rf, 0x566c, 0x00030000, 0x2);
		else
			halrf_wreg(rf, 0x566c, 0x00030000, 0x0);
		...
		/*TSSI ADC FIFO Clock follow RX ADC Clock by Ryan*/
		if (bw == CHANNEL_WIDTH_80)
			halrf_wreg(rf, 0x766c, 0x00030000, 0x1);
		else if (bw == CHANNEL_WIDTH_160 || bw == CHANNEL_WIDTH_80_80)
			halrf_wreg(rf, 0x766c, 0x00030000, 0x2);
		else
			halrf_wreg(rf, 0x766c, 0x00030000, 0x0);
...
void _halrf_tssi_ini_txpwr_ctrl_bb_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
...
void _halrf_tssi_ini_txpwr_ctrl_bb_he_tb_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   phy=%d   path=%d\n", __func__, phy, path);
...
void _halrf_tssi_set_dck_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{	
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
...
void _halrf_tssi_set_bbgain_split_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   phy=%d   path=%d\n", __func__, phy, path);
...
void _halrf_tssi_set_tmeter_tbl_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
...
void _halrf_tssi_set_tmeter_tbl_zere_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	u8 i;
	u32 thermal_offset_tmp = 0;
	u32 ftable_reg[TSSI_PATH_MAX_8832D] = {0x5c00, 0x7c00};
	u32 ftable_base_reg[TSSI_PATH_MAX_8832D] = {0x5810, 0x7810};
	u32 ftable_trigger_reg[TSSI_PATH_MAX_8832D] = {0x5864, 0x7864};
	s8 thermal_offset[64] = {0};
...
#if 0
void _halrf_tssi_set_dac_gain_tbl_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   path=%d\n", __func__, path);
...
void _halrf_tssi_slope_cal_org_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
...
#if 0
void _halrf_tssi_slope_cal_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	...
	u32 tssi_trigger[TSSI_PATH_MAX_8832D] = {0x5820, 0x7820};
	u32 tssi_cw_rpt_addr[TSSI_PATH_MAX_8832D] = {0x1c18, 0x3c18};
	...
	for (j = 0; j < 4; j++) {
		...
		_halrf_tssi_hw_tx_8832d(rf, phy, path, 100, 5000, power_high[j], rate, bw, true);
		...
		for (k = 0; halrf_rreg(rf, tssi_cw_rpt_addr[path], BIT(16)) == 0; k++) {
			...
			if (k > 100) {
				...
				RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI PA K] TSSI finish bit k > 100us path=%d\n",
					i);
				_halrf_tssi_hw_tx_8832d(rf, phy, path, 100, 5000, power_high[j], rate, bw, false);
				return;
			}
		}
		_halrf_tssi_hw_tx_8832d(rf, phy, path, 100, 5000, power_high[j], rate, bw, false);
		...
		_halrf_tssi_hw_tx_8832d(rf, phy, path, 100, 5000, power_low[j], rate, bw, true);
...
void _halrf_tssi_set_alignment_default_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
	...
	if (path == RF_PATH_A) {
		if (band == BAND_ON_24G) {
			...
			halrf_wreg(rf, 0x5640, 0x000003ff, 0x000);
			halrf_wreg(rf, 0x5640, 0x000ffc00, 0x000);
			halrf_wreg(rf, 0x5640, 0x3ff00000, 0x3e9);
			halrf_wreg(rf, 0x5644, 0x000003ff, 0x039);
			halrf_wreg(rf, 0x5644, 0x000ffc00, 0x080);
		} else {
			halrf_wreg(rf, 0x5604, 0x80000000, 0x1);
			halrf_wreg(rf, 0x5600, 0x3fffffff, 0x000000);
			halrf_wreg(rf, 0x5604, 0x003fffff, 0x312600);
			...
			halrf_wreg(rf, 0x5634, 0x000003ff, 0x000);
			halrf_wreg(rf, 0x5634, 0x000ffc00, 0x000);
			halrf_wreg(rf, 0x5634, 0x3ff00000, 0x3fc);
			halrf_wreg(rf, 0x5638, 0x000003ff, 0x04c);
			halrf_wreg(rf, 0x5638, 0x000ffc00, 0x090);
			halrf_wreg(rf, 0x563c, 0x3fffffff, 0x00000000);
			halrf_wreg(rf, 0x5640, 0x000003ff, 0x000);
			halrf_wreg(rf, 0x5640, 0x000ffc00, 0x000);
			halrf_wreg(rf, 0x5640, 0x3ff00000, 0x3fc);
			halrf_wreg(rf, 0x5644, 0x000003ff, 0x04c);
			halrf_wreg(rf, 0x5644, 0x000ffc00, 0x090);
		}
	} else {
		if (band == BAND_ON_24G) {
			halrf_wreg(rf, 0x7604, 0x80000000, 0x1);
			...
			halrf_wreg(rf, 0x7640, 0x000003ff, 0x000);
			halrf_wreg(rf, 0x7640, 0x000ffc00, 0x000);
			halrf_wreg(rf, 0x7640, 0x3ff00000, 0x3e9);
			halrf_wreg(rf, 0x7644, 0x000003ff, 0x039);
			halrf_wreg(rf, 0x7644, 0x000ffc00, 0x080);
		} else {
			halrf_wreg(rf, 0x7604, 0x80000000, 0x1);
			halrf_wreg(rf, 0x7600, 0x3fffffff, 0x000000);
			halrf_wreg(rf, 0x7604, 0x003fffff, 0x312600);
			...
			halrf_wreg(rf, 0x7634, 0x000003ff, 0x000);
			halrf_wreg(rf, 0x7634, 0x000ffc00, 0x000);
			halrf_wreg(rf, 0x7634, 0x3ff00000, 0x3fc);
			halrf_wreg(rf, 0x7638, 0x000003ff, 0x04c);
			halrf_wreg(rf, 0x7638, 0x000ffc00, 0x090);
			halrf_wreg(rf, 0x763c, 0x3fffffff, 0x00000000);
			halrf_wreg(rf, 0x7640, 0x000003ff, 0x000);
			halrf_wreg(rf, 0x7640, 0x000ffc00, 0x000);
			halrf_wreg(rf, 0x7640, 0x3ff00000, 0x3fc);
			halrf_wreg(rf, 0x7644, 0x000003ff, 0x04c);
			halrf_wreg(rf, 0x7644, 0x000ffc00, 0x090);
		}
	}
}

#if 0
void _halrf_tssi_alignment_default_8832d(struct rf_info *rf,
...
void _halrf_tssi_run_slope_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   phy=%d   path=%d\n",
		__func__, phy, path);
...
void _halrf_tssi_set_slope_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
...
void _halrf_tssi_set_tssi_track_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   path=%d\n", __func__, path);
...
void _halrf_tssi_set_txagc_offset_mv_avg_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======>%s   path=%d\n", __func__, path);
...
u32 _halrf_tssi_get_cck_efuse_group_8832d(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 offset_index = 0;
...
u32 _halrf_tssi_get_ofdm_efuse_group_8832d(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 offset_index = 0;
...
u32 _halrf_tssi_get_ofdm_efuse_bw_group_8832d(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 offset_index = 0;
...
u32 _halrf_tssi_get_6g_efuse_group_8832d(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 offset_index = 0;
...
s8 _halrf_tssi_get_ofdm_efuse_tssi_de_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
	...
	if (band == BAND_ON_24G || band == BAND_ON_5G) {
		group_idx = _halrf_tssi_get_ofdm_efuse_group_8832d(rf, phy);
		...
		group_idx = _halrf_tssi_get_6g_efuse_group_8832d(rf, phy);
...
s8 _halrf_tssi_get_ofdm_efuse_tssi_bw_diff_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum channel_width bw, enum rf_path path)
{
	...
	if (band == BAND_ON_24G || band == BAND_ON_5G) {
		group_idx = _halrf_tssi_get_ofdm_efuse_bw_group_8832d(rf, phy);
...
s8 _halrf_tssi_get_ofdm_efuse_slope_gain_diff_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 group_idx;
...
s8 _halrf_tssi_get_ofdm_efuse_slope_cw_diff_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 group_idx;
...
u32 _halrf_tssi_get_tssi_trim_efuse_group_8832d(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 group_index = 0;
...
u32 _halrf_tssi_get_tssi_trim_6g_efuse_group_8832d(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	u32 group_index = 0;
...
s8 _halrf_tssi_get_ofdm_tssi_trim_de_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
	...
	if (band == BAND_ON_24G || band == BAND_ON_5G) {
		group_idx = _halrf_tssi_get_tssi_trim_efuse_group_8832d(rf, phy);
		...
		group_idx = _halrf_tssi_get_tssi_trim_6g_efuse_group_8832d(rf, phy);
...
bool _halrf_tssi_alimentk_finish_bit_8832d(struct rf_info *rf,
	...
	for (i = 0; i < delay_500us_times; i++) {
		if (halrf_rreg(rf, 0x1c18 | (path << 13), BIT(16))) {
			*power_cw = halrf_rreg(rf, 0x1c18 | (path << 13), 0x000003ff);
			RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Alim K] 0x%x=0x%x\n", (0x1c18 | (path << 13)), *power_cw);
			RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Alim K] avg adc    0x%x=0x%x\n",
				0x1c0c | (path << 13), halrf_rreg(rf, 0x1c0c | (path << 13), 0xfff));
			RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Alim K] Dck_auto_avg   0x%x=0x%x\n",
				0x1c04 | (path << 13), halrf_rreg(rf, 0x1c04 | (path << 13), 0xfff000));
			break;
		} else {
...
void _halrf_tssi_alimentk_rfe_type_other_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	struct rtw_hal_com_t *hal = rf->hal_com;
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	...
	u32 band = rf->hal_com->band[phy].cur_chandef.band;
	s32 power_2g[4] = {32, 4, 8, -8};
	s32 power_5g[4] = {48, 40, 8, -8};
	s32 power[4] = {48, 40, 8, -8};
	u32 power_cw[4] = {0};
	s32 tssi_oft, s_tssi_final_oft;
	u32 tssi_final_oft, i, debug_times = 1;
	u32 bb_reg[4] = {0x12b8, 0x32b8, 0x030c, 0x032c};
	...
	/*BW160M PathB alimtk issue*/
	if (band != BAND_ON_24G)
		_tssi_backup_bb_registers_8832d(rf, phy, bb_reg, bb_reg_backup, backup_num);
	...
	if (band != BAND_ON_24G) {
		...
		halrf_wreg(rf, 0x032c, 0xffff0000, 0x41);
	}
	...
	halrf_wreg(rf, 0x5684 | (path << 13), 0x007f0000, 0x8);
	...
	halrf_delay_us(rf, 2000);

	halrf_write_fwofld_end(rf);		/*FW Offload End*/

	for (i = 0; i < debug_times; i++) {
		...
		/*get tssi cw*/
		if (!_halrf_tssi_alimentk_finish_bit_8832d(rf, phy, path, &power_cw[0])) {
	...
	for (i = 0; i < debug_times; i++) {
		...
		/*get tssi cw*/
		if (!_halrf_tssi_alimentk_finish_bit_8832d(rf, phy, path, &power_cw[1])) {
			finish_time = _os_get_cur_time_us();
			tssi_info->tssi_alimk_time = tssi_info->tssi_alimk_time + (finish_time - start_time);
			return;
		}
	...
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Alim K]   power[0]=%d   power_cw[0]=%d\n",
		power[0], power_cw[0]);
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Alim K]   power[1]=%d   power_cw[1]=%d\n",
		power[1], power_cw[1]);
	...
	/*CCK*/
	tssi_final_oft = halrf_rreg(rf, 0x5644 | (path << 13), 0x3ff);
	...
	halrf_wreg(rf, 0x5644 | (path << 13), 0x3ff, (s_tssi_final_oft & 0x3ff));

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Alim K] High Power Original 0x%x[9:0]=0x%x   Final 0x%x[9:0]=0x%x\n",
		0x5644 | (path << 13), tssi_final_oft,
		0x5644 | (path << 13), halrf_rreg(rf, 0x5644 | (path << 13), 0x3ff));
	...
	for (i = 0; i < debug_times; i++) {
		...
		/*get tssi cw*/
		if (!_halrf_tssi_alimentk_finish_bit_8832d(rf, phy, path, &power_cw[2])) {
			finish_time = _os_get_cur_time_us();
			tssi_info->tssi_alimk_time = tssi_info->tssi_alimk_time + (finish_time - start_time);
			return;
		}
	...
	for (i = 0; i < debug_times; i++) {
		...
		/*get tssi cw*/
		if (!_halrf_tssi_alimentk_finish_bit_8832d(rf, phy, path, &power_cw[3])) {
			finish_time = _os_get_cur_time_us();
			tssi_info->tssi_alimk_time = tssi_info->tssi_alimk_time + (finish_time - start_time);
			return;
		}
	...
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Alim K]   power[2]=%d   power_cw[2]=%d\n",
		power[2], power_cw[2]);
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Alim K]   power[3]=%d   power_cw[3]=%d\n",
		power[3], power_cw[3]);
	...
	/*CCK*/
	tssi_final_oft = halrf_rreg(rf, 0x5640 | (path << 13), 0x3ff00000);
	...
	halrf_wreg(rf, 0x5640 | (path << 13), 0x3ff00000, (s_tssi_final_oft & 0x3ff));

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Alim K]  Low Power Original 0x%x[29:20]=0x%x   Final 0x%x[29:20]=0x%x\n",
		0x5640 | (path << 13), tssi_final_oft,
		0x5640 | (path << 13), halrf_rreg(rf, 0x5640 | (path << 13), 0x3ff00000));

	/*Aligmnet K third wire, shift offset*/
	/*OFDM*/
	tssi_final_oft = halrf_rreg(rf, 0x5634 | (path << 13), 0x000ffc00);
	...
	halrf_wreg(rf, 0x5634 | (path << 13), 0x000ffc00, (s_tssi_final_oft & 0x3ff));

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Alim K]  Low Power Original 0x%x[19:10]=0x%x   Final 0x%x[19:10]=0x%x\n",
		0x5634 | (path << 13), tssi_final_oft,
		0x5634 | (path << 13), halrf_rreg(rf, 0x5634 | (path << 13), 0x000ffc00));

	/*CCK*/
	tssi_final_oft = halrf_rreg(rf, 0x5640 | (path << 13), 0x000ffc00);
	...
	halrf_wreg(rf, 0x5640 | (path << 13), 0x000ffc00, (s_tssi_final_oft & 0x3ff));

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Alim K]  Low Power Original 0x%x[19:10]=0x%x   Final 0x%x[19:10]=0x%x\n",
		0x5640 | (path << 13), tssi_final_oft,
		0x5640 | (path << 13), halrf_rreg(rf, 0x5640 | (path << 13), 0x000ffc00));
	...
	/*BW160M PathB alimtk issue reload*/
	if (band != BAND_ON_24G)
		_tssi_reload_bb_registers_8832d(rf, phy, bb_reg, bb_reg_backup, backup_num);
...
void _halrf_tssi_alimentk_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "[TSSI Alim K] ======> %s   rfe_type=%d   path=%d\n",
		__func__, rfe_type, path);

	_halrf_tssi_alimentk_rfe_type_other_8832d(rf, phy, path);
}

void _halrf_tssi_alimentk_done_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, enum rf_path path)
{
#if 0
	struct halrf_tssi_info *tssi_info = &rf->tssi;
...
void halrf_tssi_backup_txagc_8832d(struct rf_info *rf, enum phl_phy_idx phy, bool enable)
{
#if 0
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u8 channel = rf->hal_com->band[phy].cur_chandef.center_ch;
	...
	s16 xdbm;
	u32 i, j, tx_couter = 0;
	u8 ch_idx = _halrf_ch_to_idx_8832d(rf, channel);
	u8 channel_tmp;
	...
	_tssi_backup_bb_registers_8832d(rf, phy, bb_reg, bb_reg_backup,
					backup_num);
	...
	_halrf_tssi_hw_tx_8832d(rf, phy, RF_PATH_AB, 1000, 100, power_tmp, HT_MF_FMT, 0, true);

#if 0
	/*Path B*/
	tx_couter[RF_PATH_B] = halrf_rreg(rf, 0x1a40, 0xffff);
	
	_halrf_tssi_hw_tx_8832d(rf, phy, RF_PATH_B, 1000, 100, xdbm, HT_MF_FMT, 0, true);
	halrf_delay_ms(rf, 15);
	_halrf_tssi_hw_tx_8832d(rf, phy, RF_PATH_B, 1000, 100, xdbm, HT_MF_FMT, 0, false);
#endif
	...
	tssi_info->backup_txagc_oft_ther[RF_PATH_A][ch_idx] =
		halrf_get_thermal_8832d(rf, RF_PATH_A);

	tssi_info->backup_txagc_oft_ther[RF_PATH_B][ch_idx] =
		halrf_get_thermal_8832d(rf, RF_PATH_B);

	_halrf_tssi_hw_tx_8832d(rf, phy, RF_PATH_AB, 1000, 100, power_tmp, HT_MF_FMT, 0, false);
	...
	_tssi_reload_bb_registers_8832d(rf, phy, bb_reg, bb_reg_backup,
						backup_num);
...
void halrf_do_tssi_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, bool hwtx_en)
{
	...
	u8 ch_idx = _halrf_ch_to_idx_8832d(rf, channel);
//	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;
	u32 i, path_tmp = RF_PATH_A, path_max = TSSI_PATH_MAX_8832D;
	...
	halrf_tssi_disable_8832d(rf, phy);
	...
	for (i = path_tmp; i < path_max; i++) {
		...
		_halrf_tssi_set_sys_8832d(rf, phy, i);
		_halrf_tssi_ini_txpwr_ctrl_bb_8832d(rf, phy, i);
		_halrf_tssi_ini_txpwr_ctrl_bb_he_tb_8832d(rf, phy, i);
		_halrf_tssi_set_dck_8832d(rf, phy, i);
		_halrf_tssi_set_bbgain_split_8832d(rf, phy, i);
		_halrf_tssi_set_tmeter_tbl_8832d(rf, phy, i);
		/*_halrf_tssi_set_dac_gain_tbl_8832d(rf, phy, i);*/
		_halrf_tssi_slope_cal_org_8832d(rf, phy, i);

		_halrf_tssi_set_alignment_default_8832d(rf, phy, i);

		_halrf_tssi_set_slope_8832d(rf, phy, i);
		_halrf_tssi_run_slope_8832d(rf, phy, i);
		...
		if (hwtx_en == true)
			_halrf_tssi_alimentk_8832d(rf, phy, i);
	...
	if (phl_is_mp_mode(rf->phl_com)) {
		if (tssi_info->tssi_type[phy] == TSSI_ON || tssi_info->tssi_type[phy] == TSSI_CAL) {
			...
			halrf_tssi_enable_8832d(rf, phy);
			halrf_tssi_set_efuse_to_de_8832d(rf, phy);
		...
		if (rf->support_ability & HAL_RF_TX_PWR_TRACK) {
			...
			halrf_write_fwofld_end(rf);		/*FW Offload End*/
			//if (hwtx_en == true)
			//	halrf_tssi_backup_txagc_8832d(rf, phy, true);
		}
	}
...
void halrf_do_tssi_scan_8832d(struct rf_info *rf,
					enum phl_phy_idx phy)
{
//	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 i, path_tmp = RF_PATH_A, path_max = TSSI_PATH_MAX_8832D;
	...
	for (i = path_tmp; i < path_max; i++) {
		...
		if (tssi_info->alignment_done[i][band] == true)
			_halrf_tssi_alimentk_done_8832d(rf, phy, i);
		else {
			if (rf->hal_com->cv == CAV)
				_halrf_tssi_alignment_default_8832da(rf, phy, i);
			else
				_halrf_tssi_alignment_default_8832db(rf, phy, i, false);
		}
#endif
	}
...
void halrf_tssi_get_efuse_8832d(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	...
	if (_is_efem_8832d(rf)) {
		...
		for (i = FLASH_INFO_RF_5G_BW20M_BW40M_TSSI_DIFF_A_8832D_1, j = 0; i <= FLASH_INFO_RF_5G_BW20M_BW40M_TSSI_DIFF_A_8832D_14; i++, j++) {
		...
		for (i = FLASH_INFO_RF_5G_BW80M_BW40M_TSSI_DIFF_A_8832D_1, j = 0; i <= FLASH_INFO_RF_5G_BW80M_BW40M_TSSI_DIFF_A_8832D_14; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		...
		for (i = FLASH_INFO_RF_5G_BW160M_BW40M_TSSI_DIFF_A_8832D_1, j = 0; i <= FLASH_INFO_RF_5G_BW160M_BW40M_TSSI_DIFF_A_8832D_14; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		...
		for (i = FLASH_INFO_RF_2G_CCK_TSSI_GAIN_DIFF_A_8832D_1, j = 0; i <= FLASH_INFO_RF_2G_CCK_TSSI_GAIN_DIFF_A_8832D_6; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		...
		for (i = FLASH_INFO_RF_2G_TSSI_GAIN_DIFF_A_8832D_1, j = 0; i <= FLASH_INFO_RF_5G_TSSI_GAIN_DIFF_A_8832D_14; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
			tssi_info->tssi_efuse_slope_gain_diff[RF_PATH_A][EFUSE_TSSI_MCS][j] = flash_tmp;
			if (i <= FLASH_INFO_RF_2G_TSSI_GAIN_DIFF_A_8832D_5 && flash_tmp == 0x0)
				no_pg_count_2g++;
			else if (i > FLASH_INFO_RF_2G_TSSI_GAIN_DIFF_A_8832D_5 && flash_tmp == 0x0)
				no_pg_count_5g++;
...
#if 0
		for (i = FLASH_INFO_RF_6G_TSSI_GAIN_DIFF_A_8832D_1, j = 0; i <= FLASH_INFO_RF_6G_TSSI_GAIN_DIFF_A_8832D_32; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		...
		for (i = FLASH_INFO_RF_2G_CCK_TSSI_CW_DIFF_A_8832D_1, j = 0; i <= FLASH_INFO_RF_2G_CCK_TSSI_CW_DIFF_A_8832D_6; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		...
		for (i = FLASH_INFO_RF_2G_TSSI_CW_DIFF_A_8832D_1, j = 0; i <= FLASH_INFO_RF_5G_TSSI_CW_DIFF_A_8832D_14; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
			tssi_info->tssi_efuse_slope_cw_diff[RF_PATH_A][EFUSE_TSSI_MCS][j] = flash_tmp;
			if (i <= FLASH_INFO_RF_2G_TSSI_CW_DIFF_A_8832D_5 && flash_tmp == 0x0)
				no_pg_count_2g++;
			else if (i > FLASH_INFO_RF_2G_TSSI_CW_DIFF_A_8832D_5 && flash_tmp == 0x0)
				no_pg_count_5g++;
...
#if 0
		for (i = FLASH_INFO_RF_6G_TSSI_CW_DIFF_A_8832D_1, j = 0; i <= FLASH_INFO_RF_6G_TSSI_CW_DIFF_A_8832D_32; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
	...
	if (_is_efem_8832d(rf)) {
		...
		for (i = FLASH_INFO_RF_5G_BW20M_BW40M_TSSI_DIFF_B_8832D_1, j = 0; i <= FLASH_INFO_RF_5G_BW20M_BW40M_TSSI_DIFF_B_8832D_14; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		...
		for (i = FLASH_INFO_RF_5G_BW80M_BW40M_TSSI_DIFF_B_8832D_1, j = 0; i <= FLASH_INFO_RF_5G_BW80M_BW40M_TSSI_DIFF_B_8832D_14; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		...
		for (i = FLASH_INFO_RF_5G_BW160M_BW40M_TSSI_DIFF_B_8832D_1, j = 0; i <= FLASH_INFO_RF_5G_BW160M_BW40M_TSSI_DIFF_B_8832D_14; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		...
		for (i = FLASH_INFO_RF_2G_CCK_TSSI_GAIN_DIFF_B_8832D_1, j = 0; i <= FLASH_INFO_RF_2G_CCK_TSSI_GAIN_DIFF_B_8832D_6; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		...
		for (i = FLASH_INFO_RF_2G_TSSI_GAIN_DIFF_B_8832D_1, j = 0; i <= FLASH_INFO_RF_5G_TSSI_GAIN_DIFF_B_8832D_14; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
			tssi_info->tssi_efuse_slope_gain_diff[RF_PATH_B][EFUSE_TSSI_MCS][j] = flash_tmp;
			if (i <= FLASH_INFO_RF_2G_TSSI_GAIN_DIFF_B_8832D_5 && flash_tmp == 0x0)
				no_pg_count_2g++;
			else if (i > FLASH_INFO_RF_2G_TSSI_GAIN_DIFF_B_8832D_5 && flash_tmp == 0x0)
				no_pg_count_5g++;
...
#if 0
		for (i = FLASH_INFO_RF_6G_TSSI_GAIN_DIFF_B_8832D_1, j = 0; i <= FLASH_INFO_RF_6G_TSSI_GAIN_DIFF_B_8832D_32; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		...
		for (i = FLASH_INFO_RF_2G_CCK_TSSI_CW_DIFF_B_8832D_1, j = 0; i <= FLASH_INFO_RF_2G_CCK_TSSI_CW_DIFF_B_8832D_6; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
		...
		for (i = FLASH_INFO_RF_2G_TSSI_CW_DIFF_B_8832D_1, j = 0; i <= FLASH_INFO_RF_5G_TSSI_CW_DIFF_B_8832D_14; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
			tssi_info->tssi_efuse_slope_cw_diff[RF_PATH_B][EFUSE_TSSI_MCS][j] = flash_tmp;
			if (i <= FLASH_INFO_RF_2G_TSSI_CW_DIFF_B_8832D_5 && flash_tmp == 0x0)
				no_pg_count_2g++;
			else if (i > FLASH_INFO_RF_2G_TSSI_CW_DIFF_B_8832D_5 && flash_tmp == 0x0)
				no_pg_count_5g++;
...
#if 0
		for (i = FLASH_INFO_RF_6G_TSSI_CW_DIFF_B_8832D_1, j = 0; i <= FLASH_INFO_RF_6G_TSSI_CW_DIFF_B_8832D_32; i++, j++) {
			halrf_efuse_get_info(rf, i, &flash_tmp, 1);
...
bool halrf_tssi_check_efuse_data_8832d(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u32 i, j, countr = 0;
	...
	halrf_tssi_get_efuse_8832d(rf, phy);
...
void halrf_set_tssi_de_for_tx_verify_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, u32 tssi_de, u8 path)
{
	...
	tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832d(rf, phy, path);
...
void halrf_set_tssi_de_offset_8832d(struct rf_info *rf,
					enum phl_phy_idx phy, u32 tssi_de_offset, u8 path)
{
	...
	if (path == RF_PATH_A) {
		/*CCK*/
		group_idx = _halrf_tssi_get_cck_efuse_group_8832d(rf, phy);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832d(rf, phy, RF_PATH_A);
		...
		/*OFDM*/
		ofmd_de = _halrf_tssi_get_ofdm_efuse_tssi_de_8832d(rf, phy, RF_PATH_A);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832d(rf, phy, RF_PATH_A);
		...
		/*CCK*/
		group_idx = _halrf_tssi_get_cck_efuse_group_8832d(rf, phy);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832d(rf, phy, RF_PATH_B);
		...
		/*OFDM*/
		ofmd_de = _halrf_tssi_get_ofdm_efuse_tssi_de_8832d(rf, phy, RF_PATH_B);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832d(rf, phy, RF_PATH_B);
		tssi_tmp = ofmd_de + tssi_trim + s_tssi_de_offset;
...
void halrf_set_tssi_de_offset_zero_8832d(struct rf_info *rf,
							enum phl_phy_idx phy)
{
...
void halrf_tssi_enable_8832d(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	u32 i, path_tmp = RF_PATH_A, path_max = TSSI_PATH_MAX_8832D;
	...
	for (i = path_tmp; i < path_max; i++) {
		/*_halrf_tssi_set_slope_8832d(rf, phy, i);*/
		_halrf_tssi_set_tssi_track_8832d(rf, phy, i);
		_halrf_tssi_set_txagc_offset_mv_avg_8832d(rf, phy, i);
...
void halrf_tssi_disable_8832d(struct rf_info *rf,
					enum phl_phy_idx phy)
{
	u32 i, path_tmp = RF_PATH_A, path_max = TSSI_PATH_MAX_8832D;
...
s32 halrf_get_online_tssi_de_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,
				u8 path, s32 dbm, s32 puot)
{
	...
	tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832d(rf, phy_idx, path);
...
void halrf_tssi_set_efuse_to_de_8832d(struct rf_info *rf,
						enum phl_phy_idx phy)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	u8 bw = rf->hal_com->band[phy].cur_chandef.bw;
	...
	u32 path_tmp = RF_PATH_A, path_max = TSSI_PATH_MAX_8832D;

	RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======> %s\n", __func__);

	if (tssi_info->tssi_type[phy] == TSSI_CAL) {
		/*halrf_set_tssi_de_offset_zero_8832d(rf, phy);*/
		RF_DBG(rf, DBG_RF_TX_PWR_TRACK, "======> %s TSSI_CAL mode set TSSI Trim !!!\n", __func__);

		for (i = 0; i < TSSI_PATH_MAX_8832D; i++) {
			tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832d(rf, phy, i);
	...
	if (bw == CHANNEL_WIDTH_160 && rfe_type >= 50) {
		tssi_info->tssi_de_160m_adc_wa_20m = (s8)halrf_rreg(rf, 0x1a8, 0xff);
		tssi_info->tssi_de_160m_adc_wa_40m = (s8)halrf_rreg(rf, 0x1a8, 0xff00);
		tssi_info->tssi_de_160m_adc_wa_80m = (s8)halrf_rreg(rf, 0x1a8, 0xff0000);
	} else {
		tssi_info->tssi_de_160m_adc_wa_20m = 0;
		tssi_info->tssi_de_160m_adc_wa_40m = 0;
		tssi_info->tssi_de_160m_adc_wa_80m = 0;
	}
	...
	for (i = path_tmp; i < path_max; i++) {
		...
		/*OFDM*/
		ofmd_de = _halrf_tssi_get_ofdm_efuse_tssi_de_8832d(rf, phy, i);
		tssi_trim = _halrf_tssi_get_ofdm_tssi_trim_de_8832d(rf, phy, i);

		/*TSSI BW 20M*/
		tssi_bw_diff = _halrf_tssi_get_ofdm_efuse_tssi_bw_diff_8832d(rf, phy, CHANNEL_WIDTH_20, i);
		tmp = ofmd_de + tssi_trim + tssi_bw_diff + tssi_info->tssi_de_160m_adc_wa_20m;

		RF_DBG(rf, DBG_RF_TX_PWR_TRACK,
			"[TSSI] TSSI BW 20M tmp(0x%x) = tssi_efuse_ofdm(0x%x) + tssi_trim(0x%x) + tssi_bw_diff(0x%x) + tssi_de_160m_adc_wa_20m(0x%x)  path=%d\n",
			tmp, ofmd_de, tssi_trim, tssi_bw_diff, tssi_info->tssi_de_160m_adc_wa_20m, i);
		...
		/*TSSI BW 40M*/
		tssi_bw_diff = _halrf_tssi_get_ofdm_efuse_tssi_bw_diff_8832d(rf, phy, CHANNEL_WIDTH_40, i);
		tmp = ofmd_de + tssi_trim + tssi_bw_diff + tssi_info->tssi_de_160m_adc_wa_40m;

		RF_DBG(rf, DBG_RF_TX_PWR_TRACK,
			"[TSSI] TSSI BW 40M tmp(0x%x) = tssi_efuse_ofdm(0x%x) + tssi_trim(0x%x) + tssi_bw_diff(0x%x) + tssi_de_160m_adc_wa_40m(0x%x)  path=%d\n",
			tmp, ofmd_de, tssi_trim, tssi_bw_diff, tssi_info->tssi_de_160m_adc_wa_40m, i);
		...
		/*TSSI BW 80M*/
		tssi_bw_diff = _halrf_tssi_get_ofdm_efuse_tssi_bw_diff_8832d(rf, phy, CHANNEL_WIDTH_80, i);
		tmp = ofmd_de + tssi_trim + tssi_bw_diff + tssi_info->tssi_de_160m_adc_wa_80m;

		RF_DBG(rf, DBG_RF_TX_PWR_TRACK,
			"[TSSI] TSSI BW 80M tmp(0x%x) = tssi_efuse_ofdm(0x%x) + tssi_trim(0x%x) + tssi_bw_diff(0x%x) + tssi_de_160m_adc_wa_80m(0x%x)  path=%d\n",
			tmp, ofmd_de, tssi_trim, tssi_bw_diff, tssi_info->tssi_de_160m_adc_wa_80m, i);
		...
		/*TSSI BW 1600M*/
		tssi_bw_diff = _halrf_tssi_get_ofdm_efuse_tssi_bw_diff_8832d(rf, phy, CHANNEL_WIDTH_160, i);
...
void halrf_tssi_default_txagc_8832d(struct rf_info *rf,
...
//			halrf_do_tssi_8832d(rf, phy, false);
			return;
		}
	} else {
		/*SCAN_END*/
		...
		_halrf_tssi_alimentk_done_8832d(rf, phy, RF_PATH_A);
		_halrf_tssi_alimentk_done_8832d(rf, phy, RF_PATH_B);
...
void halrf_tssi_scan_ch_setting_8832d(struct rf_info *rf,
	enum phl_phy_idx phy, enum rf_path path)
{
...
void halrf_tssi_scan_ch_8832d(struct rf_info *rf, enum rf_path path)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
...
//	u32 tssi_mv_avg[2] = {0x58e4, 0x78e4};
	...
	if (!phl_is_mp_mode(rf->phl_com)) {
		...
		if (tssi_info->alignment_done[path][band] == true)
			_halrf_tssi_alimentk_done_8832d(rf, 0, path);
		else {
#if 0
			if (rf->hal_com->cv == CAV)
				_halrf_tssi_alignment_default_8832da(rf, 0, path);
			else
				_halrf_tssi_alignment_default_8832db(rf, 0, path, false);
#endif
		}
...
u32 halrf_tssi_get_final_8832d(struct rf_info *rf, enum rf_path path)
{
	u32 i;
...
void halrf_tssi_hw_tx_8832d(struct rf_info *rf,
			enum phl_phy_idx phy, u8 path, u16 cnt, s16 dbm, u32 rate, u8 bw,
			bool enable)
{
	_halrf_tssi_hw_tx_8832d(rf, phy, path, cnt, 5000, dbm, T_HT_MF, 0, enable);
}

void halrf_tssi_ant_open_8832d(struct rf_info *rf)
{
	struct halrf_tssi_info *tssi_info = &rf->tssi;
	...
	if ((halrf_rreg(rf, 0x1c60, 0xff000000) == 0x7f) ||
		...
		halrf_tssi_disable_8832d(rf, HW_PHY_0);
		halrf_tssi_enable_8832d(rf, HW_PHY_0);
	}
}

void halrf_get_tssi_info_8832d(struct rf_info *rf,
		char input[][16], u32 *_used, char *output, u32 *_out_len)
{
	...
	s32 delta_tssi;
	s32 diff_ther;
	s32 int_tmp[TSSI_PATH_MAX_8832D], float_tmp[TSSI_PATH_MAX_8832D];
	s8 txagc_offset[TSSI_PATH_MAX_8832D] = {0};

	RF_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used, " %-25s : DE(%d) = EFUSE(%d) + Trim(%d) + DIFF(%d) + ADCWA(%d)\n",
		"TSSI DE 20M A",
		...
		tssi_info->tssi_de_160m_adc_wa_20m);
	...
	RF_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used, " %-25s : DE(%d) = EFUSE(%d) + Trim(%d) + DIFF(%d) + ADCWA(%d)\n",
		...
		tssi_info->tssi_de_160m_adc_wa_80m);
...
void halrf_tssi_slope_onoff_8832d(struct rf_info *rf, u8 tssi_slope_type)
{
	u8 path;
	...
	for (path = RF_PATH_A; path < TSSI_PATH_MAX_8832D; path++) {
...
/*MP sloep K*/
void halrf_tssi_do_slope_8832d(struct rf_info *rf)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK,
		"======>%s\n", __func__);
...
/*MP sloep K*/
u32 halrf_tssi_get_cw_8832d(struct rf_info *rf, enum rf_path path)
{
...
/*MP sloep K*/
void halrf_tssi_slope_apply_8832d(struct rf_info *rf,
...
/*MP sloep K*/
void halrf_tssi_current_slope_apply_8832d(struct rf_info *rf,
...
/*MP sloep K*/
void halrf_tssi_finish_slope_8832d(struct rf_info *rf)
{
	RF_DBG(rf, DBG_RF_TX_PWR_TRACK,
		"======>%s\n", __func__);

...
void halrf_adc_fifo_rst_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx, u8 path)
{
...
bool halrf_ctl_band_ch_bw_8832d(struct rf_info *rf, enum phl_phy_idx phy, enum band_type band, u8 central_ch, enum channel_width bw)
{
		u8 kpath;
		u8 path = 0;
		u8 i = 0;
...
void halrf_set_lo_8832d(struct rf_info *rf, bool is_on, enum rf_path path)
{
	u32 rf18;
	bool not2g = true;
	...
	if (is_on) {
		if (not2g) {
			halrf_rf_direct_cntrl_8832d(rf, path, false);
		...
		halrf_rf_direct_cntrl_8832d(rf, path, true);
	}
}

void halrf_rf_direct_cntrl_8832d(struct rf_info *rf, enum rf_path path, bool is_bybb)
{
	if (is_bybb)
...
void halrf_drf_direct_cntrl_8832d(struct rf_info *rf, enum rf_path path, bool is_bybb)
{
	if (is_bybb)
...
void halrf_lo_test_8832d(struct rf_info *rf, bool is_on, enum rf_path path)
{
	switch (path) {
		case RF_PATH_A:
			halrf_set_lo_8832d(rf, is_on, RF_PATH_A);
			halrf_set_lo_8832d(rf, false, RF_PATH_B);
			break;
		case RF_PATH_B:
			halrf_set_lo_8832d(rf, false, RF_PATH_A);
			halrf_set_lo_8832d(rf, is_on, RF_PATH_B);
			break;
		case RF_PATH_AB:
			halrf_set_lo_8832d(rf, is_on, RF_PATH_A);
			halrf_set_lo_8832d(rf, is_on, RF_PATH_B);
...
u8 halrf_kpath_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx) {
...
void _rx_dck_info_8832d(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_afe)
{
...
void _rx_dck_value_rewrite_8832d(struct rf_info *rf, enum rf_path path, u8 addr, u8 val_i, u8 val_q)
{
	u32 ofst_val;;
...
bool _rx_dck_rek_check_8832d(struct rf_info *rf, enum rf_path path)
{
...
void _rx_dck_check_8832d(struct rf_info *rf, enum rf_path path, u8 addr,
				u8 val_i_bs, u8 val_q_bs, u8 val_i, u8 val_q)
{
	...
	_rx_dck_value_rewrite_8832d(rf, path, addr, val_i, val_q);
}

void _rx_dck_read_offset_8832d(struct rf_info *rf, enum rf_path path)
{
	u8 addr_bs[2] = {0x0, 0x10};
	u8 addr[2] = {0xc, 0x1c};
	u8 i_even_bs[2], q_even_bs[2]; /*0x0, 0x10*/
	...
	for (i = 0; i < 2; i++) {
		...
		RF_DBG(rf, DBG_RF_RXDCK, "[RX_DCK] Gain[0x%x] i_even/ q_even = 0x%x/ 0x%x\n",
			addr[i], i_even[i], q_even[i]);
		_rx_dck_check_8832d(rf, path, addr[i], i_even_bs[i], q_even_bs[i], i_even[i], q_even[i]);

		RF_DBG(rf, DBG_RF_RXDCK, "[RX_DCK] Gain[0x%x] i_odd_bs/ q_odd_bs = 0x%x/ 0x%x\n",
			addr_bs[i] + 1, i_odd_bs[i], q_odd_bs[i]);
		RF_DBG(rf, DBG_RF_RXDCK, "[RX_DCK] Gain[0x%x] i_odd/ q_odd = 0x%x/ 0x%x\n",
			addr[i] + 1, i_odd[i], q_odd[i]);
		_rx_dck_check_8832d(rf, path, addr[i] + 1, i_odd_bs[i], q_odd_bs[i], i_odd[i], q_odd[i]);
	}
}

void halrf_rx_dck_toggle_8832d(struct rf_info *rf, enum rf_path path)
{
	u32 cnt = 0;
...
void halrf_set_rx_dck_8832d(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_afe)
{
//	struct halrf_rx_dck_info *rx_dck = &rf->rx_dck;
...
//	u8 res;
	...
	_rx_dck_info_8832d(rf, phy, path, is_afe);
...
void halrf_rx_dck_8832d(struct rf_info *rf, enum phl_phy_idx phy, bool is_afe) 
{
	struct halrf_rx_dck_info *rx_dck = &rf->rx_dck;
	...
	for (path = 0; path < 2; path++) {
		if (kpath & BIT(path)) {
			...
			halrf_set_rx_dck_8832d(rf, phy, path, is_afe);

			if (!rf->is_chl_rfk)
				_rx_dck_read_offset_8832d(rf, path);
			...
			rx_dck->ther_rxdck[path] = halrf_get_thermal_8832d(rf, path);
...
void halrf_thermal_rx_dck_8832d(struct rf_info *rf, enum phl_phy_idx phy, bool is_afe) 
{
	struct halrf_rx_dck_info *rx_dck = &rf->rx_dck;
	...
	for (path = 0; path < 2; path++) {
		if (kpath & BIT(path)) {
			...
			is_fail = _rx_dck_rek_check_8832d(rf, path);
...
void halrf_scan_rx_dck_8832d(struct rf_info *rf, enum phl_phy_idx phy, enum rfk_tri_type tri_type)
{
	struct halrf_rx_dck_info *rx_dck = &rf->rx_dck;
	...
	if (tri_type == RFK_TYPE_SCAN_BK_OP) {
		...
		if (cur_band == BAND_ON_6G && rf->hal_com->dbcc_en == false && (cur_band != rx_dck->loc[RF_PATH_A].cur_band))
			do_rxdck = true;
		else { /*DBCC*/
			if (cur_band == BAND_ON_6G && phy == HW_PHY_0 && (cur_band != rx_dck->loc[RF_PATH_A].cur_band))
				do_rxdck = true;
			else if (cur_band == BAND_ON_6G && phy == HW_PHY_1 && (cur_band != rx_dck->loc[RF_PATH_B].cur_band))
				do_rxdck = true;
		}
	}
	...
	struct rf_info *rf)
{
	...
	if (!rf->hal_com->dbcc_en) {
		if (rx_dck->loc[RF_PATH_A].cur_band == 0) /*2.4G*/
			return;
		if (HALRF_ABS(rf->cur_ther_s0, rx_dck->ther_rxdck[RF_PATH_A]) >= ther_th ||
		    HALRF_ABS(rf->cur_ther_s1, rx_dck->ther_rxdck[RF_PATH_B]) >= ther_th)
		...
		if ((HALRF_ABS(rf->cur_ther_s1, rx_dck->ther_rxdck[RF_PATH_B]) >= ther_th) &&
		    (rx_dck->loc[RF_PATH_B].cur_band != 0))
			halrf_thermal_rx_dck_trigger(rf, HW_PHY_1, false); /*ReK S1 only*/
	}
}

void halrf_rck_8832d(struct rf_info *rf, enum rf_path path)
{
	u8 cnt = 0;
	u32 rf_reg5;
	u32 rck_val = 0;
...
void iqk_backup_8832d(struct rf_info *rf, enum rf_path path) 
{
	return;
}

void halrf_bf_config_rf_8832d(struct rf_info *rf)
{
...
bool halrf_rfk_reg_check_8832d(struct rf_info *rf)
{
	u32 i, reg, temp;
	bool fail = false;
...
void halrf_rfk_reg_reload_8832d(struct rf_info *rf)
{
	u32 i, reg;
...
void halrf_rfk_reg_backup_8832d(struct rf_info *rf)
{
	u32 i, reg;
...
bool halrf_rfk_reg_check_fail_8832d(struct rf_info *rf)
{
	bool fail = false;
	...
	halrf_wreg(rf, 0x81d8, MASKDWORD, 0x00010001);
	halrf_wreg(rf, 0x82d8, MASKDWORD, 0x00010001);
	fail = halrf_rfk_reg_check_8832d(rf);
	halrf_wreg(rf, 0x81d8, MASKDWORD, 0x00000000);
	halrf_wreg(rf, 0x81dc, MASKDWORD, 0x00010003);	
	halrf_wreg(rf, 0x81dc, MASKDWORD, 0x00000002);
	halrf_wreg(rf, 0x82d8, MASKDWORD, 0x00000000);
...
void halrf_rfc_reg_backup_8832d(struct rf_info *rf)
{
	u8 i;
...
bool halrf_rfc_reg_check_fail_8832d(struct rf_info *rf)
{
	u8 i;
	u32 temp;
	bool fail = false;
...
void halrf_by_rate_dpd_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
	halrf_mac_set_pwr_reg_8192xb(rf, phy, 0xd22c + (phy << 13), 0x1F800000, 0x29); /*[28:23]*/
	halrf_mac_set_pwr_reg_8192xb(rf, phy, 0xd230 + (phy << 13), 0x0003FFFF, 0x24924); /*[17:0]*/
}

void halrf_dpk_init_8832d(struct rf_info *rf)
{
	/*by rate dpd*/
	halrf_by_rate_dpd_8832d(rf, HW_PHY_0);
}

void halrf_set_rxbb_bw_8832d(struct rf_info *rf, enum channel_width bw, enum rf_path path)
{
	halrf_write_fwofld_start(rf);		/*FW Offload Start*/
...
void halrf_set_tia_bw_8832d(struct rf_info *rf, enum channel_width bw, enum rf_path path)
{
...
void halrf_rxbb_bw_8832d(struct rf_info *rf, enum phl_phy_idx phy, enum channel_width bw)
{
	u8 kpath, path;
	...
	for (path = 0; path < 2; path++) {
		if ((kpath & BIT(path)) && (rf->pre_rxbb_bw[path] != bw)) {
			halrf_set_tia_bw_8832d(rf, bw, path);
...
void halrf_disconnect_notify_8832d(struct rf_info *rf, struct rtw_chan_def *chandef) {
...
bool halrf_check_mcc_ch_8832d(struct rf_info *rf, struct rtw_chan_def *chandef) {
...
void halrf_fw_ntfy_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx) {
	u8 i = 0x0;
	struct halrf_mcc_info *mcc_info = &rf->mcc_info;
	u32 data_to_fw[6] = {0};
...
void halrf_before_one_shot_enable_8832d(struct rf_info *rf) {	
...
#ifdef HALRF_CONFIG_FW_IO_OFLD_SUPPORT
bool _halrf_one_shot_nctl_done_check_io_ofld_8832d(struct rf_info *rf, enum rf_path path) {	
	/* for check status */
	u32 r_bff8 = 0;
	u32 r_80fc = 0;
	bool is_ready = false;
...
bool _halrf_one_shot_nctl_done_check_default_8832d(struct rf_info *rf, enum rf_path path) {	
	/* for check status */
	u32 r_bff8 = 0;
	u32 r_80fc = 0;
	bool is_ready = false;
	...
	while (count < 2000) {	
		...
		if (r_80fc == NCTL_FINAL_LINE_8832D) {
			is_ready = true;
			break;
		}	
...
bool halrf_one_shot_nctl_done_check_8832d(struct rf_info *rf, enum rf_path path) {	

	bool is_ready = false;

#ifdef HALRF_CONFIG_FW_IO_OFLD_SUPPORT
	if (rf->phl_com->dev_cap.io_ofld)
		is_ready = _halrf_one_shot_nctl_done_check_io_ofld_8832d(rf, path);
	else 
#endif
	{
		is_ready = _halrf_one_shot_nctl_done_check_default_8832d(rf, path);
	}	
...
bool halrf_do_one_shot_8832d(struct rf_info *rf, enum rf_path path, u32 nctl_addr, u32 mask, u32 process_id)
{

	halrf_before_one_shot_enable_8832d(rf);
	halrf_wreg(rf, nctl_addr, mask, process_id); //set cal_path, process id

	return halrf_one_shot_nctl_done_check_8832d(rf, path);
}

bool halrf_set_ch_lck_8832d(struct rf_info *rf, u32 path, u32 val)
{
	u32 c = 0;
	bool timeout = false;
...
void halrf_lck_8832d(struct rf_info *rf)
{
	...
	if (!rf->hal_com->dbcc_en) {
		...
		halrf_set_ch_lck_8832d(rf, 0, temp18);
		...
		for(i = 0; i < 2; i++) {
			...
			halrf_set_ch_lck_8832d(rf, i, temp18);
...
void halrf_lck_tracking_8832d(struct rf_info *rf)
{
	u32 temp;
	...
	if(temp >= LCK_TH_8832D)		
		halrf_lck_trigger(rf);
}

static u32 check_rfc_reg_8832d[] = {0xfe, 0x0, 0x0, 0x0, 0x0,
	...
	0xb2, 0xc5};
static u32 check_dack_reg_8832d[] = {0x12a0, 0x32a0, 0x12b8, 0x32b8, 0x030c,
	...
	0xc164};
static u32 check_iqk_reg_8832d[] = {0x8000, 0x8004, 0x8008, 0x8080, 0x808c,
	...
	0x9f90, 0x9fa0};
static u32 check_dpk_reg_8832d[] = {0x80b0, 0x81bc, 0x82bc, 0x81b4, 0x82b4,
	0x81c4, 0x82c4, 0x81c8, 0x82c8, 0x58d4,
	0x78d4};
static u32 check_tssi_reg_8832d[] = {0x0304, 0x5818, 0x581c, 0x5820, 0x1c60,
...
void halrf_quick_checkrf_8832d(struct rf_info *rf)
{
	u32 path, temp, i;
	u32	len = sizeof(check_rfc_reg_8832d) / sizeof(u32);
	u32	*add = (u32 *)check_rfc_reg_8832d;
	...
	/*check DACK*/
	RF_TRACE("======DACK======\n");
	len = sizeof(check_dack_reg_8832d) / sizeof(u32);
	add = check_dack_reg_8832d;
	for (i = 0; i < len; i++ ) {
	...
	/*check IQK*/
	RF_TRACE("======IQK======\n");
	len = sizeof(check_iqk_reg_8832d) / sizeof(u32);
	add = check_iqk_reg_8832d;
	...
	/*check DPK*/
	RF_TRACE("======DPK======\n");
	len = sizeof(check_dpk_reg_8832d) / sizeof(u32);
	add = check_dpk_reg_8832d;
	for (i = 0; i < len; i++ ) {
		temp = halrf_rreg(rf, add[i], MASKDWORD);
		RF_TRACE("0x%x = 0x%x\n", add[i], temp);
	}
	/*check TSSI*/
	RF_TRACE("======TSSI======\n");
	len = sizeof(check_tssi_reg_8832d) / sizeof(u32);
	add = check_tssi_reg_8832d;
	for (i = 0; i < len; i++ ) {
		temp = halrf_rreg(rf, add[i], MASKDWORD);
		RF_TRACE("0x%x = 0x%x\n", add[i], temp);
	}
...
static u32 backup_mac_reg_8832d[] = {0x0};
static u32 backup_bb_reg_8832d[] = {0x2344, 0xc0c4, 0xc0d4, 0xc0d8, 0xc0e8, 0xc0ec, 0xc1c4, 0xc1d4, 0xc1d8, 0xc1e8, 0xc1ec};
static u32 backup_rf_reg_8832d[] = {0xdc, 0xef, 0xde, 0x0, 0x1e, 0x5, 0x10005};

static struct halrf_iqk_ops iqk_ops= {
    .iqk_kpath = halrf_kpath_8832d,
    .iqk_mcc_page_sel = iqk_mcc_page_sel_8832d,
    .iqk_get_ch_info = iqk_get_ch_info_8832d,
    .iqk_preset = iqk_preset_8832d,
    .iqk_macbb_setting = iqk_macbb_setting_8832d,
    .iqk_start_iqk = iqk_start_iqk_8832d,
    .iqk_restore = iqk_restore_8832d,
    .iqk_afebb_restore = iqk_afebb_restore_8832d,  
};

struct rfk_iqk_info rf_iqk_hwspec_8832d = {
  	.rf_iqk_ops = &iqk_ops,
	.rf_max_path_num = 2,
	.rf_iqk_version = iqk_version_8832d,
	.rf_iqk_ch_num = 2,
	.rf_iqk_path_num = 2,

#if 0
	.backup_mac_reg = backup_mac_reg_8832d,
	.backup_mac_reg_num = ARRAY_SIZE(backup_mac_reg_8832d),
#else
	.backup_mac_reg = backup_mac_reg_8832d,
	.backup_mac_reg_num = 0,
#endif

    	.backup_bb_reg = backup_bb_reg_8832d,
    	.backup_bb_reg_num = ARRAY_SIZE(backup_bb_reg_8832d),
    	.backup_rf_reg = backup_rf_reg_8832d,
    	.backup_rf_reg_num = ARRAY_SIZE(backup_rf_reg_8832d),
};

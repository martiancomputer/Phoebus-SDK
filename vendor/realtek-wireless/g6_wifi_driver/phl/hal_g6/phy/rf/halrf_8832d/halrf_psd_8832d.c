...
void _halrf_psd_backup_bb_registers_8832d(
	struct rf_info *rf,
...
void _halrf_psd_reload_bb_registers_8832d(
	struct rf_info *rf,
	enum phl_phy_idx phy,
	u32 *reg,
	u32 *reg_backup,
	...
	u8 path)
{
	...
	for (i = 0; i < PSD_RF_REG_NUM_8832D; i++) {
...
void _halrf_psd_reload_rf_8832d(
	struct rf_info *rf,
	u32 *rf_reg,
	u8 path)
{
...
void _halrf_psd_set_dac_off_adc_8832d(struct rf_info *rf, enum phl_phy_idx phy)
...
void halrf_psd_init_8832d(struct rf_info *rf, enum phl_phy_idx phy,
			u8 path, u8 iq_path, u32 avg, u32 fft)
{
	struct halrf_psd_data *psd_info = &rf->psd;
	u32 reg_tmp;
	u32 bb_reg[PSD_BACKUP_NUM_8832D] = {
			0x20fc, 0x5670, 0x7670, 0x12a0, 0x32a0,
			0x8080, 0x8008,	0x8088, 0x80d0, 0x8074,
			0x81dc, 0x82dc, 0x12b8, 0x32b8, 0x030c,
			0x032c, 0x8120, 0x8220, 0x8128, 0x8228,
			0x8154, 0x8254, 0x8140, 0x8240,	0x8144,
			0x8244, 0x801c, 0x8000, 0x800c, 0x8018,
			0x81cc,	0x82cc, 0x802c, 0x8034, 0x80d4,
			0x80fc};

	u32 rf_reg[PSD_RF_REG_NUM_8832D] = {0x0, 0x5, 0x8f, 0x10000, 0x10005};

	RF_DBG(rf, DBG_RF_PSD, "======> %s   phy=%d   path=%d   iq_path=%d   avg=%d   fft=%d\n",
		__func__, phy,  path, iq_path, avg, fft);

	if (psd_info->psd_progress == 0) {
		_halrf_psd_backup_bb_registers_8832d(rf, phy, bb_reg,
				psd_info->psd_reg_backup, PSD_BACKUP_NUM_8832D);

		_halrf_psd_bkup_rf_8832d(rf, rf_reg, RF_PATH_A);
		_halrf_psd_bkup_rf_8832d(rf, rf_reg, RF_PATH_B);
	}
	...
	_halrf_psd_set_dac_off_adc_8832d(rf, phy);
	...
	if (path == RF_PATH_A) {
		...
		halrf_wreg(rf, 0x81dc, MASKDWORD, 0x00000000);
	} else {	/*06a_S1_IQKPSD_Apply_RxCFIR_52D*/
...
void halrf_psd_restore_8832d(struct rf_info *rf, enum phl_phy_idx phy)
{
	struct halrf_psd_data *psd_info = &rf->psd;
	...
	} else {	/*98_S1_IQKPSD_Reg_Restore_52D*/
		halrf_wreg(rf, 0x8080, MASKDWORD, 0x00000002);
		halrf_delay_ms(rf, 1);
		halrf_wreg(rf, 0x8074, 0xff000000, 0x00);
		halrf_wreg(rf, 0x80d0, 0x00ff0000, 0x00);
	...
	if (psd_info->psd_progress == 1) {
		_halrf_psd_reload_bb_registers_8832d(rf, phy, bb_reg,
			psd_info->psd_reg_backup, PSD_BACKUP_NUM_8832D);
		_halrf_psd_reload_rf_8832d(rf, rf_reg, RF_PATH_A);
		_halrf_psd_reload_rf_8832d(rf, rf_reg, RF_PATH_B);
	}

	psd_info->psd_progress = 0;
}

u32 halrf_psd_get_point_data_8832d(struct rf_info *rf,
...
void halrf_psd_query_8832d(struct rf_info *rf, enum phl_phy_idx phy,
			u32 point, u32 start_point, u32 stop_point, u32 *outbuf)
{
	...
	while (i < stop_point) {
		...
		psd_info->psd_data[j] = halrf_psd_get_point_data_8832d(rf, phy, point_temp);

...
__iram_func__
static void _iqk_read_fft_8832d(struct rf_info *rf, u8 path)
{
#if 0
	u8 i = 0x0;
	u32 fft[6] = {0x0};
...
__iram_func__
static void _iqk_read_xym_8832d(struct rf_info *rf, u8 path)
{
...
__iram_func__
static void _iqk_read_txcfir_8832d(struct rf_info *rf, u8 path, u8 group)
{
...
__iram_func__
static void _iqk_read_rxcfir_8832d(struct rf_info *rf, u8 path, u8 group)
{
...
__iram_func__
static bool _iqk_check_nbiqc_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,  u8 ktype, u8 path)
{
	u32 x = 0;
	u32 y = 0;
...
__iram_func__
static void _iqk_sram_8832d(struct rf_info *rf, u8 path)
{
...
__iram_func__
static void _iqk_rxk_setting_8832d(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	//u32 tmp = 0x0;
	...
	case CHANNEL_WIDTH_40:		
		...
		halrf_rxck_force_8832d(rf, path, true, ADC_480M);
...
//		halrf_wreg(rf, 0xc0d8 + (path << 8), 0x000001e0, 0xf);
	...
	case CHANNEL_WIDTH_80:
		...
		halrf_wreg(rf, 0x5670 + (path << 13), 0x00002000, 0x1);
		halrf_rxck_force_8832d(rf, path, true, ADC_960M);
...
__iram_func__
static void _iqk_nto_rxcfir_mapping_8832d(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	...
	case BAND_ON_24G:		
		halrf_wreg(rf, 0x8150 + (path << 8), MASKDWORD, 0xa0a00000);
		break;
	case BAND_ON_5G:
	case BAND_ON_6G:
		halrf_wreg(rf, 0x8150 + (path << 8), MASKDWORD, 0xaaaa0000);
	break;
	default:
		break;
	}
...
__iram_func__
static bool _iqk_check_cal_8832d(struct rf_info *rf, u8 path, u8 ktype)
{
	...
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]HALRF_CONFIG_FW_IO_OFLD_SUPPORT = %x\n", HALRF_CONFIG_FW_IO_OFLD_SUPPORT);
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]rf->phl_com->dev_cap.io_ofld = %x\n", rf->phl_com->dev_cap.io_ofld);
	...
	RF_DBG(rf, DBG_RF_IQK, "[IQK]S%x, cnt1= %d, cnt2= %d, type= %x, fail = 0x%x \n", path, cnt1, cnt2, ktype, fail);

	return fail;
}

__iram_func__
static bool _iqk_one_shot_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,
				 u8 path, u8 ktype)
{
	...
	halrf_delay_us(rf, 1);
	fail = _iqk_check_cal_8832d(rf, path, ktype);

	switch(ktype ) {
	case ID_TXK:
		if (iqk_info->iqk_xym_en == true)
			_iqk_read_xym_8832d(rf, path);
		if (iqk_info->iqk_cfir_en == true) {
			_iqk_read_txcfir_8832d(rf, path, 0x1);
			_iqk_read_txcfir_8832d(rf, path, 0x2);
			_iqk_read_txcfir_8832d(rf, path, 0x3);
		}
		break;
	case ID_RXK:		
		if (iqk_info->iqk_sram_en == true)
			_iqk_sram_8832d(rf, path);
		if (iqk_info->iqk_xym_en == true)
			_iqk_read_xym_8832d(rf, path);
		if (iqk_info->iqk_cfir_en == true) {
			_iqk_read_rxcfir_8832d(rf, path, 0x0);
			_iqk_read_rxcfir_8832d(rf, path, 0x1);
			_iqk_read_rxcfir_8832d(rf, path, 0x2);
			_iqk_read_rxcfir_8832d(rf, path, 0x3);
		}
		break;
	default:
		break;
...
__iram_func__
static bool _rxk_group_sel_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,
				  u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u8 idx = 0;
	...
	u32 a6_idxrxgain[2] = {0x190, 0x284};
	u32 a6_idxattc2[2] = {0x0, 0x0};
	u32 a6_idxrxagc[2] = {0x4, 0x6};

	u32 a_idxrxgain[2] = {0x190, 0x304};
	u32 a_idxattc2[2] = {0x0, 0x1};
	...
	case BAND_ON_5G:
		halrf_wrf(rf, path, 0x00, 0xf0000, 0xc);
		halrf_wrf(rf, path, 0x00, 0x0c000, 0x0);
		halrf_wrf(rf, path, 0x89, 0x0000f, 0xa);
		gp_num = 2;
	break;
	case BAND_ON_6G:
		halrf_wrf(rf, path, 0x0, 0xf0000, 0xc);
		halrf_wrf(rf, path, 0x0, 0x0c000, 0x0);
		tmp = halrf_rrf(rf, path, 0x21, 0xfffff);
		halrf_wrf(rf, path, 0xdc, 0x00004, 0x1);
		halrf_wrf(rf, path, 0x21, 0xfffff, tmp);
		halrf_wrf(rf, path, 0x21, 0x03e00, 0x01);
		halrf_wrf(rf, path, 0x89, 0x0000f, 0xa);
		gp_num = 2;
	break;
	default:
	...
	for (idx = 0; idx < gp_num; idx++) {
		...
		fail = _iqk_one_shot_8832d(rf, phy_idx, path, ID_RXAGC);
		halrf_delay_us(rf,10);
		RF_DBG(rf, DBG_RF_IQK, "[IQK]S%x, after_agc 0x00 = %x\n", path, halrf_rrf(rf, path, 0x00, MASKDWORD));
		fail = _iqk_one_shot_8832d(rf, phy_idx, path, ID_RXK);
		kfail = kfail | fail;
	}
	halrf_wrf(rf, path, 0xdc, 0x00004, 0x0);
...
__iram_func__
static bool _iqk_nbrxk_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,
			      u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u8 idx = 0;
	...
	fail = _iqk_one_shot_8832d(rf, phy_idx, path, ID_NBRXK);
	kfail = _iqk_check_nbiqc_8832d(rf, phy_idx, ID_NBRXK, path);
...
__iram_func__
static bool _txk_group_sel_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,
				  u8 path)
{
...
//	bool kfail = false;
	...
	if (iqk_info->iqk_band[path] == BAND_ON_6G) {
		for (gp = 0x0; gp < 0x1; gp++) {
			...
			fail = _iqk_one_shot_8832d(rf, phy_idx, path, ID_TXK);
...
__iram_func__
static bool _iqk_nbtxk_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,
			      u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	bool fail = false;
	...
	if (iqk_info->iqk_band[path] == BAND_ON_6G) {
		for (gp = 0x0; gp < 0x1; gp++) {
			...
			fail = _iqk_one_shot_8832d(rf, phy_idx, path, ID_NBTXK);
			halrf_wreg(rf, 0x9fe0, BIT(8 + gp + path * 4), fail);
			kfail = kfail | fail;
			}
	} else {
			...
			kfail = _iqk_one_shot_8832d(rf, phy_idx, path, ID_NBTXK);
...
__iram_func__
static void _lok_res_table_8832d(struct rf_info *rf, u8 path, u8 ibias)
{
...
__iram_func__
static bool _lok_2g_finetune_check_8832d(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
static bool _lok_5g_finetune_check_8832d(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	//bool is_fail = false, is_fail1 = false,  is_fail2 = false;
	bool is_fail1 = false;
...
__iram_func__
static bool _iqk_lok_2g_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,
			    u8 path)
{
//	struct halrf_iqk_info *iqk_info = &rf->iqk;
	bool isfail;
	...
	isfail = _iqk_check_cal_8832d(rf, path, lok);
	halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
...
__iram_func__
static bool _iqk_lok_5g_bygain_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,
			    u8 path)
{
//	struct halrf_iqk_info *iqk_info = &rf->iqk;
	bool isfail = false;
...
//
	...
	if(i_dac < 0x2 || q_dac < 0x2 || i_dac > 0x3c || q_dac > 0x3c)
		lok_fail = true;
	else
		lok_fail = false;
//	
...
__iram_func__
static bool _iqk_lok_5g_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,
			    u8 path)
{
	bool isfail;
...
__iram_func__
static bool _iqk_lok_6g_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,
			    u8 path)
{
//	struct halrf_iqk_info *iqk_info = &rf->iqk;
	bool isfail;
...
__iram_func__
static bool _iqk_bw20_lok_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,
			    u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	//bool fail = false;
...
//============START : _lok_coarse_fine_wi_swap ========
	...
	tmp = _iqk_one_shot_8832d(rf, phy_idx, path, tmp_id);
...
//Step 2
	...
	tmp = _iqk_one_shot_8832d(rf, phy_idx, path, ID_FLOK_vbuffer);
...
#ifdef HALRF_CONFIG_FW_IO_OFLD_SUPPORT	/*NOT USB*/
	if (rf->phl_com->dev_cap.io_ofld == false) {
		halrf_delay_us(rf, 20);
		if(iqk_info->iqk_band[path]== BAND_ON_24G)
			isfail = _lok_2g_finetune_check_8832d(rf, path);
		else
			isfail = _lok_5g_finetune_check_8832d(rf, path);
	} else
		isfail = false;
#else 
	isfail = false;
...
__iram_func__
static bool _iqk_lok_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,
			    u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	bool isfail1 = false;
	...
	switch (iqk_info->iqk_band[path]) {
	case BAND_ON_24G:
		isfail1 = _iqk_lok_2g_8832d(rf, phy_idx, path);
		isfail2 = _lok_2g_finetune_check_8832d(rf, path);
		break;
	case BAND_ON_5G:
		if ((rf->phl_com->hci_type == RTW_HCI_USB) || (iqk_info->iqk_bw[path] == CHANNEL_WIDTH_20))
			isfail1 = _iqk_lok_5g_8832d(rf, phy_idx, path);			
		else		
			isfail1 = _iqk_lok_5g_bygain_8832d(rf, phy_idx, path);
		isfail2 = _lok_5g_finetune_check_8832d(rf, path);
		break;
	case BAND_ON_6G:		
		isfail1 = _iqk_lok_6g_8832d(rf, phy_idx, path);
		isfail2 = _lok_5g_finetune_check_8832d(rf, path);
		break;
	default:
		break;
...
__iram_func__
static void _iqk_txk_setting_8832d(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
static void _iqk_txclk_setting_8832d(struct rf_info *rf, u8 path)
{
#if 0
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
...
__iram_func__
static void _iqk_info_iqk_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,
				 u8 path)
{
	...
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
	//iqk_info->thermal[path] = halrf_get_thermal_8832d(rf, path);	
...
__iram_func__
static void _iqk_disable_rxagc_8832d(struct rf_info *rf,u8 path, u8 en_rxgac)
{
...
__iram_func__
static void _iqk_by_path_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,
				u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	...
	thermal = halrf_get_thermal_8832d(rf, path);
	...
	for (i = 0; i < 2; i++) {
		if (iqk_info->iqk_band[path] == BAND_ON_24G)
			...
			ibias = a_res[i];
		_lok_res_table_8832d(rf, path, ibias);
		_iqk_txk_setting_8832d(rf, path);
		lok_is_fail = _iqk_lok_8832d(rf, phy_idx, path);
		if (!lok_is_fail)			
			break;
	}
	...
	if (iqk_info->is_nbiqk) {
		iqk_info->iqk_tx_fail[0][path] =
			_iqk_nbtxk_8832d(rf, phy_idx, path);
	} else {
		iqk_info->iqk_tx_fail[0][path] =
			_txk_group_sel_8832d(rf, phy_idx, path);
	}
	//RX
	_iqk_rxk_setting_8832d(rf, path);
	_iqk_nto_rxcfir_mapping_8832d(rf, path);
	if (iqk_info->is_nbiqk) {
		iqk_info->iqk_rx_fail[0][path] =
			_iqk_nbrxk_8832d(rf, phy_idx, path);
	} else {
		iqk_info->iqk_rx_fail[0][path] =
			_rxk_group_sel_8832d(rf, phy_idx, path);
	}
#ifdef HALRF_CONFIG_FW_IO_OFLD_SUPPORT /*NOT USB*/
	if (rf->phl_com->dev_cap.io_ofld == false)
#endif
	{
		...
		_iqk_info_iqk_8832d(rf, phy_idx, path);
	}
	if(iqk_info->is_nbiqk) {
...
__iram_func__
void iqk_backup_rf0_8832d(
	struct rf_info *rf, u8 path,
	u32 backup_rf0[rf_reg_num_8832d],
	u32 backup_rf_reg0[rf_reg_num_8832d])
{
	u8 i;
...
__iram_func__
void iqk_backup_rf1_8832d(
	struct rf_info *rf, u8 path,
	u32 backup_rf1[rf_reg_num_8832d],
	u32 backup_rf_reg1[rf_reg_num_8832d])
{
	u8 i;
...
__iram_func__
void iqk_set_info_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx,
				 u8 path)
{
...
__iram_func__
bool iqk_mcc_page_sel_8832d(struct rf_info *rf, enum phl_phy_idx phy, u8 path)
{
#if 0
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	bool flag = false;
	...
	if (rf->hal_com->band[phy].cur_chandef.center_ch ==  iqk_info->iqk_mcc_ch[0][path]) {
		...
		_iqk_mcc_load_lok_8832d(rf, phy, path, iqk_info->lok_idac[0][path]);
		flag = true;
		...
		_iqk_mcc_load_lok_8832d(rf, phy, path, iqk_info->lok_idac[1][path]);
		flag = true;
	} else
		flag = false;
#endif	
...
__iram_func__
void iqk_get_ch_info_8832d(struct rf_info *rf, enum phl_phy_idx phy, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	...
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]S%x, thermal = %x\n", path, halrf_get_thermal_8832d(rf, path));

	halrf_wreg(rf, 0x9fe0, 0xff000000, iqk_version_8832d);
	//2G5G6G = 0/1/2
	...
	ver = (u8) halrf_get_8832d_nctl_reg_ver();
...
__iram_func__
void halrf_iqk_reload_8832d(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u32 tmp;
	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
...
__iram_func__
void iqk_start_iqk_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx, u8 path)
{

	//RF_DBG(rf, DBG_RF_IQK, "[IQK]===>%s\n", __func__);
	_iqk_by_path_8832d(rf, phy_idx, path);
	return;
}

__iram_func__
void iqk_restore_8832d(struct rf_info *rf, u8 path)
{
//	struct halrf_iqk_info *iqk_info = &rf->iqk;
	bool fail;
	//98_S0_IQK_Reg_Restore_52D_reg
	halrf_wreg(rf, 0x8000, MASKDWORD, 0x00001219 + (path << 4));
	halrf_delay_us(rf, 200);
	fail = _iqk_check_cal_8832d(rf, path, 0x12);
...
__iram_func__
void iqk_afebb_restore_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx, u8 path)
{
	RF_DBG(rf, DBG_RF_IQK, "===> %s\n", __func__);
	...
	//enable rxgac
	_iqk_disable_rxagc_8832d(rf, path, 0x1);

	return;
}

__iram_func__
void iqk_preset_8832d(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u8 idx = 0;
...
__iram_func__
void iqk_macbb_setting_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx, u8 path)
{
	RF_DBG(rf, DBG_RF_IQK, "[IQK]===> %s\n", __func__);
	...
	//disable rxgac
	_iqk_disable_rxagc_8832d(rf, path, 0x0);
...
__iram_func__
void halrf_iqk_toneleakage_8832d(struct rf_info *rf, u8 path)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	...
	iqk_get_ch_info_8832d(rf, HW_PHY_0, path);
	iqk_macbb_setting_8832d(rf, HW_PHY_0, path);	
	iqk_preset_8832d(rf, path);
...
__iram_func__
void halrf_iqk_dbcc_8832d(struct rf_info *rf, u8 path)
{
#if 0
	bool bkdbcc = false;	
	u8 phy_idx = 0x0;
	...
	//iqk_mcc_page_sel_8832d(rf,phy_idx, path);		
	iqk_get_ch_info_8832d(rf,phy_idx, path);
	iqk_macbb_setting_8832d(rf, phy_idx, path);
	iqk_preset_8832d(rf, path);
	iqk_start_iqk_8832d(rf, phy_idx, path);
	iqk_restore_8832d(rf, path);
	iqk_afebb_restore_8832d(rf, phy_idx, path);	
...
__iram_func__
void halrf_iqk_onoff_8832d(struct rf_info *rf, bool is_enable)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
void halrf_iqk_tx_bypass_8832d(struct rf_info *rf, u8 path)
{
...
__iram_func__
void halrf_iqk_rx_bypass_8832d(struct rf_info *rf, u8 path)
{
	if (path == RF_PATH_A) { //path A
		/*ch0*/
		halrf_wreg(rf, 0x8124, MASKDWORD, 0x0);
...
__iram_func__
void halrf_iqk_lok_bypass_8832d(struct rf_info *rf, u8 path)
{
...
__iram_func__
void halrf_nbiqk_enable_8832d(struct rf_info *rf, bool nbiqk_en)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
void halrf_iqk_xym_enable_8832d(struct rf_info *rf, bool iqk_xym_en)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
void halrf_iqk_fft_enable_8832d(struct rf_info *rf, bool iqk_fft_en)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
void halrf_iqk_sram_enable_8832d(struct rf_info *rf, bool iqk_sram_en)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
void halrf_iqk_cfir_enable_8832d(struct rf_info *rf, bool iqk_cfir_en)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	...
	struct rf_info *rf)
{
	...
	for (path = 0; path < 1; path++) {
		cur_ther = halrf_get_thermal_8832d(rf, path);
...
__iram_func__
bool halrf_iqk_get_ther_rek_8832d(struct rf_info *rf ) {
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__
u8 halrf_iqk_get_mcc_ch0_8832d(struct rf_info *rf ) {
...
__iram_func__
u8 halrf_iqk_get_mcc_ch1_8832d(struct rf_info *rf ) {
...
__iram_func__
bool halrf_check_fwiqk_done_8832d(struct rf_info *rf)
{
	u32 counter = 0x0;
	bool flag = false;
	bool isfail = false;
	...
	if(flag) {
		...
		halrf_iqk_tx_bypass_8832d(rf, RF_PATH_A);
		halrf_iqk_tx_bypass_8832d(rf, RF_PATH_B);
		halrf_iqk_rx_bypass_8832d(rf, RF_PATH_A);
		halrf_iqk_rx_bypass_8832d(rf, RF_PATH_B);
		halrf_iqk_lok_bypass_8832d(rf, RF_PATH_A);		
		halrf_iqk_lok_bypass_8832d(rf, RF_PATH_B);
	}
...
__iram_func__
void halrf_enable_fw_iqk_8832d(struct rf_info *rf, bool is_fw_iqk)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
...
__iram_func__ 
u8 halrf_get_iqk_times_8832d(struct rf_info *rf) {
	u8 times  = 0x0;
...
__iram_func__ 
u32 halrf_get_iqk_ver_8832d(void)
{
	return iqk_version_8832d;
}

__iram_func__
void iqk_init_8832d(struct rf_info *rf)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u8 idx, path;
	...
	if (!iqk_info->is_iqk_init) {
		...
		for (idx = 0; idx < 2; idx++) { //channel		
			...
			for (path = 0; path < ss_8832d; path++) {//path
...
u8 halrf_iqk_get_rxevm_8832d(struct rf_info *rf)
{
	u8 rxevm = 0x0;
#if 1
...
u32 halrf_iqk_get_rximr_8832d(struct rf_info *rf, u8 path, u32 idx)
{
	u32 rximr =0x0;
	u32 tone_idx = 0x0;
	u32 main_idx = 0x0;
	...
	// 06a_S0_IQKPSD_Apply_RxCFIR_52D
...
void halrf_lok_backup_dbcc_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u8 kpath, i;
...
void halrf_lok_reload_dbcc_8832d(struct rf_info *rf, enum phl_phy_idx phy_idx)
{
	struct halrf_iqk_info *iqk_info = &rf->iqk;
	u8 kpath, i;

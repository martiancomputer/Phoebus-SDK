...
void halrf_disconnect_notify_8852a(struct rf_info *rf, struct rtw_chan_def *chandef  ) {
    ...
    for (idx = 0; idx < 2; idx++) {
        for (path = 0; path < KPATH; path++) {
            RF_DBG(rf, DBG_RF_RFK, "[IQK] before clear mcc_ch[%x][%x]= %d\n", idx, path, iqk_info->iqk_mcc_ch[idx][path]);
        }
    }
    /*[IQK disconnect]*/
    if (false == rf->hal_com->dbcc_en)
        goto non_dbcc;
    for (idx = 0; idx< 2; idx++) {
        for (path = 0; path < KPATH; path++) {
            if (iqk_info->iqk_mcc_ch[idx][path] == chandef->center_ch)
                iqk_info->iqk_mcc_ch[idx][path] = 0x0;
        }
    }
non_dbcc:
    /* table0 and table1 is same ch and aslo is disconnection ch*/
    if(iqk_info->iqk_mcc_ch[0][0] == chandef->center_ch &&
       iqk_info->iqk_mcc_ch[1][0] == chandef->center_ch) {
        if (iqk_info->iqk_table_idx[0] == 0)
            delet_idx = 1;
        for (path = 0; path < KPATH; path++) {
            iqk_info->iqk_mcc_ch[delet_idx][path] = 0x0;
        }
    } else {
        for (idx = 0; idx< 2; idx++) {
            for (path = 0; path < KPATH; path++) {
    ...
    for (idx = 0; idx < 2; idx++) {
        for (path = 0; path < KPATH; path++) {
            RF_DBG(rf, DBG_RF_RFK, "[IQK] after clear mcc_ch[%x][%x]= %d\n", idx, path, iqk_info->iqk_mcc_ch[idx][path]);
        }
    }
}

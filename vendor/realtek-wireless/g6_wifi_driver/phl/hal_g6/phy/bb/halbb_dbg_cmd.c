...
void halbb_fwdbg_trace(struct bb_info *bb, u32 dbg_comp, u32 fw_trace_en)
{
	...
	bb_fwdbg->fw_dbg_trace = (u8)fw_trace_en;
			 ...
			 char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], help) == 0) {
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			    "{fw_dbg (dbg_trace=> 1:enable, 2:disable) (dbg_comp) (dbg_comp_trace_en)}\n");
		goto out;
	}
	for (i = 0; i < 5; i++) {
		if (input[i + 1])
	...
	if (val[0] == 1) {
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,
			    "[fw_dbg] Enable : dbg_comp = %x, dbg_trace_en = %x\n", val[1], val[2]);
		halbb_fwdbg_trace(bb, val[1], val[2]);
	} else if (val[0] == 2) {
		BB_DBG_CNSL(out_len, used, output + used, out_len - used,

...
#define PHYSTS_SS_MAX	4
#define PHYSTS_STS_MAX	4
#define PHYSTS_PATH_MAX	5
#define PHYSTS_NUM_SUB80_MAX 4
#define PHYSTS_CH_STB_LEN 32
...
enum bb_physts_hold_type {
	PSTS_HOLD_DISABLE 		= 0,
	PSTS_HOLD_WITH_PAR		= 1,
	PSTS_HOLD_STOP_PAR		= 2
};
...
struct bb_physts_rslt_20_info {
	...
	u8 user_idx;
	u8 tmax_idx_non_lgy_sts_path[HALBB_MAX_PATH][HALBB_MAX_PATH]; /*[sts][path]*/
	s8 snr_non_lgy_sts_path[HALBB_MAX_PATH][HALBB_MAX_PATH];/*[sts][path]*/
...
struct bb_physts_ch_stb_phy_info {
	u8 rssi_avg_ss[PHYSTS_SS_MAX];
	u8 snr;
	u8 channel;
	u8 ss;
	u8 cn_ss[PHYSTS_SS_MAX];
	#if 0
	u8 evm_ss[PHYSTS_SS_MAX];
	#endif
	u8 evm_max;
	u8 evm_min;
};

struct bb_physts_statics_info {
	u16 val;
	u16 record[PHYSTS_CH_STB_LEN];
	u16 sum;
	u16 mean;
	u16 var;
	u8 idx;
...
struct bb_physts_ch_stb_info {
	struct bb_physts_ch_stb_phy_info phy_info_recorder[PHYSTS_CH_STB_LEN];
	struct bb_physts_ch_stb_phy_info phy_info_avg;
	struct bb_physts_statics_info channel_change_ratio;
	#if 0
	struct bb_ra_accelerate_en_c2h_rpt ra_accel_rpt;
	#endif
	bool en;
	bool release_counter[PHYSTS_CH_STB_LEN];
	bool is_round_up;
	#if 0
	bool is_manual_accel;
	#endif
	u32 channel_change_idx;
	u32 channel_change_idx_backup;
	u32 channel_change_idx_cmn_rpt;
	#if 0
	u16 channel_change_ratio;
	u16 record_channel_change_ratio[PHYSTS_CH_STB_LEN];
	bool round_up_channel_change_ratio;
	u16 sum_channel_change_ratio;
	u16 var_channel_change_ratio;
	u8 idx_channel_change_ratio;
	u8 len_cal_var;
	#endif
	...
	u32 cn_change_cnt;
	u32 evm_change_cnt;
	u32 channel_change_cnt;

};

struct bb_physts_info {
	enum bb_physts_hold_type hold_type;
	...
	struct bb_physts_ch_stb_info	bb_physts_ch_stb_i;
};
...
void halbb_get_physts_avg_pdp_snr_sts_path(struct bb_info *bb, u8 *tmax_idx, s8 *snr, u8 sts, u8 path);

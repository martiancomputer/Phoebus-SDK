...
void halbb_bb_wrap_set_ant(struct bb_info *bb, u16 macid, u8 ant){

	struct rtw_hal_com_t *hal_com = bb->hal_com;
	u32 base_tx_path_macid = 0, cr = 0;
	u32 val = 0;

	switch (bb->ic_type) {

#if defined(BB_8852C_SUPPORT) || defined(BB_8852D_SUPPORT)
	case BB_RTL8852C:
		base_tx_path_macid = 0xDC00;
		val  = 0x03C8603E;
		break;
#endif
	default:
	...
	cr = base_tx_path_macid + (u32)(macid << 2);
	val |= (ant == MAIN_ANT)? 0 : BIT9;
	rtw_hal_mac_set_pwr_reg(hal_com, HW_PHY_0, cr, val);
}

...
void halbb_cfo_trk_init(struct bb_info *bb)
{
	...
	if ((bb->phl_com->wifi_roles[0].type != PHL_RTYPE_AP) && (bb->ic_type & (BB_RTL8852A | BB_RTL8852C)))
...
s32 halbb_multi_sta_avg_cfo_calc(struct bb_info *bb)
{
	...
	for (i = 0; i < PHL_MAX_STA_NUM; i++) {
		...
		if ((bb->phl_com->wifi_roles[0].type == PHL_RTYPE_AP) && (sta->macid == 0))
			continue;
...
void halbb_cfo_trk(struct bb_info *bb, s32 curr_cfo)
{
	...
	if ((!bb_link->is_one_entry_only) && (bb->phl_com->wifi_roles[0].type != PHL_RTYPE_AP)) {
		BB_DBG(bb, DBG_CFO_TRK, "rfe_type=%d, role_type=%d, is_one_entry_only=%d\n",
				     dev->rfe_type, bb->phl_com->wifi_roles[0].type, bb_link->is_one_entry_only);
...
void halbb_cfo_dm(struct bb_info *bb)
{
	...
	if (bb_link->is_linked) {
		if (bb_link->is_one_entry_only) {
			...
			/* To avoid NIC soft-AP going to multi-sta cfo until AP side is verified for a long time*/
			if (bb->phl_com->wifi_roles[0].type == PHL_RTYPE_AP) {
		       ...
		       struct physts_rxd *desc)
{
	...
	if ((bb->phl_com->wifi_roles[0].type == PHL_RTYPE_AP) && (bb_macid == 0)) /* No need to cnt AP Rx boardcast pkt*/
		return;

	if (rate_info->mode == BB_HE_MODE) {
		if ((fw_rate_idx - MAX_RATE_VHT - MAX_RATE_HT) >= (LEGACY_RATE_NUM + HE_RATE_NUM_4SS)) {
			BB_WARNING("[%s] fw_rate_idx = %d\n", __func__, fw_rate_idx);
			return;
		}
	} else if (rate_info->mode == BB_VHT_MODE) {
		if ((fw_rate_idx - MAX_RATE_HT) >= (LEGACY_RATE_NUM + HE_RATE_NUM_4SS)) {
			BB_WARNING("[%s] fw_rate_idx = %d\n", __func__, fw_rate_idx);
			return;
		}
	} else {
		if (fw_rate_idx >= (LEGACY_RATE_NUM + HE_RATE_NUM_4SS)) {
			BB_WARNING("[%s] fw_rate_idx = %d\n", __func__, fw_rate_idx);
			return;
		}
	}

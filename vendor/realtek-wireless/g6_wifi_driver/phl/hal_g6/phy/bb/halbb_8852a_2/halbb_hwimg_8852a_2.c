...
#include "halbb_hwimg_raw_data_8852a_2.h"

#ifdef BB_8852A_2_SUPPORT

bool halbb_sel_headline_8852a_2(struct bb_info *bb, u32 *array, u32 array_len,
			      u8 *headline_size, u8 *headline_idx)
{
	...
	*headline_size = 0;
	...
	compare_target = ((rfe_drv & 0xff) << 16) | (DONT_CARE_8852A_2 & 0xff);
	BB_DBG(bb, DBG_INIT, "[2] CHK {RFE:Match, CV:Dont care}\n");
	for (i = 0; i < *headline_size; i += 2) {
		if ((array[i] & 0x0fffffff) == compare_target) {
			*headline_idx = (u8)(i >> 1);
			return true;
...
void halbb_flag_2_default_8852a_2(bool *is_matched, bool *find_target)
{
	*is_matched = true;
	*find_target = false;
}

bool halbb_cfg_bbcr_ax_8852a_2(struct bb_info *bb, bool is_form_folder,
			  ...
			  enum phl_phy_idx phy_idx)
{
	...
	if (is_form_folder) {
		...
		array_len = sizeof(array_mp_8852a_2_phy_reg) / sizeof(u32);
		array = (u32 *)array_mp_8852a_2_phy_reg;
	}

	BB_DBG(bb, DBG_INIT, "form_folder=%d, len=%d, dbcc_en=%d, phy_idx=%d\n",
	       is_form_folder, array_len, bb->hal_com->dbcc_en, phy_idx);

	if (!halbb_sel_headline_8852a_2(bb, array, array_len, &h_size, &h_idx)) {
	...
	halbb_flag_2_default_8852a_2(&is_matched, &find_target);
	...
	while ((i + 1) < array_len) {
		...
		default:
			if (is_matched) 
				...
				#else
				halbb_cfg_bb_phy_8852a_2(bb, v1, v2, phy_idx);
				#endif
			break;
		}
	}
...
bool halbb_cfg_bb_gain_ax_8852a_2(struct bb_info *bb, bool is_form_folder,
				  u32 folder_len, u32 *folder_array)
{
	bool is_matched, find_target;
	u32 cfg_target = 0, cfg_para = 0;
	...
	if (is_form_folder) {
		...
		array_len = sizeof(array_mp_8852a_2_phy_reg_gain) / sizeof(u32);
		array = (u32 *)array_mp_8852a_2_phy_reg_gain;
	}
	...
	while ((i + 1) < array_len) {
		...
		default:
			if (is_matched)
				halbb_cfg_bb_gain_8852a_2(bb, v1, v2);
			break;
		}
	}
...
u32
halbb_get_8852a_2_phy_reg_ver(void)
{
		return (u32)BB_REG_RELEASE_VERSION_8852A_2;
}

...
void halbb_loop_filter_setting_8192xb(struct bb_info *bb, enum channel_width bw)
{
	BB_DBG(bb, DBG_DBG_API, "%s\n", __func__);
	if(phl_is_mp_mode(bb->phl_com))
		return;
	...
	if (bw == CHANNEL_WIDTH_20) {
		// pilot tracking loop filter
		halbb_set_reg(bb, 0x440c, 0xf8000, 0x2);
		halbb_set_reg(bb, 0x4df4, 0x1f, 0x2);
		// data tracking loop filter
		halbb_set_reg(bb, 0x4404, 0xf8000, 0x2);
		halbb_set_reg(bb, 0x4da0, 0x3e000000, 0x2);
	} else {
		// pilot tracking loop filter
		halbb_set_reg(bb, 0x440c, 0xf8000, 0xc);
		halbb_set_reg(bb, 0x4df4, 0x1f, 0xc);
		// data tracking loop filter
		halbb_set_reg(bb, 0x4404, 0xf8000, 0xc);
		halbb_set_reg(bb, 0x4da0, 0x3e000000, 0xc);
			   ...
			   enum phl_phy_idx phy_idx)
{
	...
	halbb_set_reg(bb, 0x0734, 0x0ff0000, ch_idx_encoded);
			      ...
			      enum phl_phy_idx phy_idx)
{
	...
	/*==== [loop filter setting] ====*/
	halbb_loop_filter_setting_8192xb(bb, bw);
...
void halbb_gpio_setting_init_8192xb(struct bb_info *bb)
{
	...
	} else if (rfe_idx == 53 || rfe_idx == 54 || rfe_idx == 94) {
...
void halbb_agc_fix_gain_8192xb(struct bb_info *bb, bool enable, enum rf_path path)
{
...
void halbb_agc_elna_idx_8192xb(struct bb_info *bb, bool elna_idx, enum rf_path path)
{
...
void halbb_agc_tia_shrink_8192xb(struct bb_info *bb, bool shrink_en,
...
void halbb_agc_step_en_8192xb(struct bb_info *bb, bool pre_pd_agc_en,
...
void halbb_get_efuse_ofst_init_8192xb(struct bb_info *bb)
{
...
void halbb_set_gain_error_8192xb(struct bb_info *bb, u8 central_ch, enum band_type band_type)
{
	...
	u32 lna_op1db_g[BB_PATH_MAX_8192XB][7] = {{0x4684, 0x4684, 0x4684,
						 0x4688, 0x4688, 0x4688,
						 0x4688}, {0x4768, 0x4768,
						 0x4768, 0x476C, 0x476C,
						 0x476C, 0x476C}};
	u32 tia_lna_op1db_g[BB_PATH_MAX_8192XB][7] = {{0x468C, 0x468C, 0x4690,
						     0x4690, 0x4690, 0x4690,
						     0x4694}, {0x4770, 0x4770,
						     0x4774, 0x4774, 0x4774,
						     0x4774, 0x4778}};
	u32 tia1_lna6_op1db_g[BB_PATH_MAX_8192XB] = {0x4694, 0x4778};
	...
	for (path = RF_PATH_A; path < BB_PATH_MAX_8192XB; path++) {
		...
		if (bb->phl_com->dev_cap.rfe_type >= 51) {
			...
			halbb_set_reg(bb, tia1_lna6_op1db_g[path], 0xff00, tmp);
			// Set wb_gidx
			// wb_gidx_elna

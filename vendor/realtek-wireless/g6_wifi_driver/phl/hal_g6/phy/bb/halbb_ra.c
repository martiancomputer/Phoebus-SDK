...
const u16 bb_phy_rate_table[LEGACY_RATE_NUM + HE_RATE_NUM_4SS] = {
	/*CCK*/
	1, 2, 5, 11,
	/*OFDM*/
...
u32 halbb_get_txsts_rpt(struct bb_info *bb, u16 len, u8 *c2h)
{
	...
	for (j = 0; j <= 1; j++) {
		...
		macid_rpt = (u16)txsts_i->rpt_macid_l + ((u16)txsts_i->rpt_macid_m << 8);
		...
		ra_sta_i = &hal_sta_i->ra_info;
		tx_ok[0] = (u16)txsts_i->tx_ok_be_l + ((u16)txsts_i->tx_ok_be_m << 8);
		tx_ok[1] = (u16)txsts_i->tx_ok_bk_l + ((u16)txsts_i->tx_ok_bk_m << 8);
		tx_ok[2] = (u16)txsts_i->tx_ok_vi_l + ((u16)txsts_i->tx_ok_vi_m << 8);
		tx_ok[3] = (u16)txsts_i->tx_ok_vo_l + ((u16)txsts_i->tx_ok_vo_m << 8);
		tx_retry[0] = (u16)txsts_i->tx_retry_be_l + ((u16)txsts_i->tx_retry_be_m << 8);
		tx_retry[1] = (u16)txsts_i->tx_retry_bk_l + ((u16)txsts_i->tx_retry_bk_m << 8);
		tx_retry[2] = (u16)txsts_i->tx_retry_vi_l + ((u16)txsts_i->tx_retry_vi_m << 8);
		tx_retry[3] = (u16)txsts_i->tx_retry_vo_l + ((u16)txsts_i->tx_retry_vo_m << 8);
		for (i = 0; i <= 3; i++) {
		...
		tx_total = (u16)txsts_i->tx_total_l + ((u16)txsts_i->tx_total_m << 8);
		rts_total_num = (u16)txsts_i->rts_total_num_l + ((u16)txsts_i->rts_total_num_m << 8);
		rts_ok_num = (u16)txsts_i->rts_ok_num_l + ((u16)txsts_i->rts_ok_num_m << 8);
		ppdu_num = (u16)txsts_i->ppdu_num_l + ((u16)txsts_i->ppdu_num_m << 8);
		if ((0xffffffff - ra_sta_i->tx_total_cnt) > tx_total)
			ra_sta_i->tx_total_cnt += tx_total;
		BB_DBG(bb, DBG_RA,
		       "====> GetTxSts : macid = %d, TxTotal = %d, rts_total_num = %d, rts_ok_num = %d, ppdu_num = %d\n",
		       macid_rpt, tx_total, rts_total_num, rts_ok_num, ppdu_num);
	}

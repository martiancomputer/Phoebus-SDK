...
void halbb_show_phy_hitogram_mu_cnsl(struct bb_info *bb, u32 *_used,
				     char *output, u32 *_out_len)
{
	struct bb_cmn_rpt_info	*cmn_rpt = &bb->bb_cmn_rpt_i;
	struct bb_pkt_cnt_mu_info *pkt_cnt = &cmn_rpt->bb_pkt_cnt_mu_i;
	...
	for (i = 0; i < 4; i++) {
		BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used, "[User idx : %d]", i);
		/*=== [EVM, SNR] =====================================================*/
		/*Threshold*/
				      ...
				      char *output, u32 *_out_len)
{
	...
	if (bb->bb_link_i.is_linked) {
		...
		halbb_show_phy_hitogram_mu_cnsl(bb, _used, output, _out_len);
	}
	BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
		    "\n");
	BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,

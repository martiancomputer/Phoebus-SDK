...
void halbb_physts_detail_dump_ie_20_gen2(struct bb_info *bb) {
	/*for 8852C*/
	struct bb_physts_info	*physts = &bb->bb_physts_i;
	...
	switch (rate_i->ss) {
		...
		case 4:
			BB_TRACE("[20] tmax_idx_non_lgy ==>\n");
			for (path = 0; path < HALBB_MAX_PATH; path ++){
				BB_TRACE("[20] sts0_path_%c : %d, sts1_path_%c : %d, sts2_path_%c : %d, sts3_path_%c : %d\n",
					'A' + path, psts_20->tmax_idx_non_lgy_sts_path[0][path],
					'A' + path, psts_20->tmax_idx_non_lgy_sts_path[1][path],
					'A' + path, psts_20->tmax_idx_non_lgy_sts_path[2][path],
					'A' + path, psts_20->tmax_idx_non_lgy_sts_path[3][path]);
			}
			BB_TRACE("[20] snr_non_lgy ==>\n");
			for (path = 0; path < HALBB_MAX_PATH; path ++){
				BB_TRACE("[20] sts0_path_%c : %d, sts1_path_%c : %d, sts2_path_%c : %d, sts3_path_%c : %d\n",
					'A' + path, psts_20->snr_non_lgy_sts_path[0][path] >> 1,
					'A' + path, psts_20->snr_non_lgy_sts_path[1][path] >> 1,
					'A' + path, psts_20->snr_non_lgy_sts_path[2][path] >> 1,
					'A' + path, psts_20->snr_non_lgy_sts_path[3][path] >> 1);
			}
			break;
		#endif
		...
		case 3:
			BB_TRACE("[20] tmax_idx_non_lgy ==>\n");
			for (path = 0; path < HALBB_MAX_PATH; path ++){
				BB_TRACE("[20] sts0_path_%c : %d, sts1_path_%c : %d, sts2_path_%c : %d\n",
					'A' + path, psts_20->tmax_idx_non_lgy_sts_path[0][path],
					'A' + path, psts_20->tmax_idx_non_lgy_sts_path[1][path],
					'A' + path, psts_20->tmax_idx_non_lgy_sts_path[2][path]);
			}
		...
		case 2:
			BB_TRACE("[20] tmax_idx_non_lgy ==>\n");
			for (path = 0; path < HALBB_MAX_PATH; path ++){
				BB_TRACE("[20] sts0_path_%c : %d, sts1_path_%c : %d\n",
					'A' + path, psts_20->tmax_idx_non_lgy_sts_path[0][path],
					'A' + path, psts_20->tmax_idx_non_lgy_sts_path[1][path]);
			}
			BB_TRACE("[20] snr_non_lgy ==>\n");
		...
		case 1:
			...
			break;
		default:
			BB_WARNING("[%s] Undefined ss number : %d\n", __func__, rate_i->ss);
			break;
	}
}
...
void halbb_physts_detail_dump(struct bb_info *bb, u32 bitmap, u32 bitmap_mask)
{
	...
	if (bitmap & BIT(IE20_DBG_OFDM_FD_USER_SEG_0)) {
		if (bb->ic_type == BB_RTL8852C)
			halbb_physts_detail_dump_ie_20_gen2(bb);
	}
}
...
bool halbb_physts_ie_20_gen2(struct bb_info *bb,
			     u8 *addr,
			     u16 ie_length,
			     struct physts_rxd *desc)

{
	...
	struct bb_physts_info	*physts = &bb->bb_physts_i;
	struct bb_physts_rslt_20_info *psts_20 = &physts->bb_physts_rslt_20_i;
	struct physts_ie_20_21_info_type1 *ie_20 = NULL;

	ie_20 = (struct physts_ie_20_21_info_type1 *)addr;
	psts_20->ie_20_addr = addr;
	psts_20->user_idx = (ie_20->user_idx_m << 4) | (ie_20->user_idx_l);

	psts_20->tmax_idx_non_lgy_sts_path[0][0] = ie_20->tmax_idx_non_lgy_sts0_patha;
	psts_20->snr_non_lgy_sts_path[0][0] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts0_patha, 8);
#ifdef HALBB_COMPILE_ABOVE_2SS
	psts_20->tmax_idx_non_lgy_sts_path[1][0] = (ie_20->tmax_idx_non_lgy_sts1_patha_m << 1) | (ie_20->tmax_idx_non_lgy_sts1_patha_l);
	psts_20->tmax_idx_non_lgy_sts_path[0][1] = ie_20->tmax_idx_non_lgy_sts0_pathb;
	psts_20->tmax_idx_non_lgy_sts_path[1][1] = (ie_20->tmax_idx_non_lgy_sts1_pathb_m << 1) | (ie_20->tmax_idx_non_lgy_sts1_pathb_l);
	psts_20->snr_non_lgy_sts_path[1][0] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts1_patha, 8);
	psts_20->snr_non_lgy_sts_path[0][1] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts0_pathb, 8);
	psts_20->snr_non_lgy_sts_path[1][1] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts1_pathb, 8);
#endif
#ifdef HALBB_COMPILE_ABOVE_3SS
	psts_20->tmax_idx_non_lgy_sts_path[2][0] = ie_20->tmax_idx_non_lgy_sts2_patha;
	psts_20->tmax_idx_non_lgy_sts_path[2][1] = ie_20->tmax_idx_non_lgy_sts2_pathb;
	psts_20->tmax_idx_non_lgy_sts_path[0][2] = ie_20->tmax_idx_non_lgy_sts0_pathc;
	psts_20->tmax_idx_non_lgy_sts_path[1][2] = (ie_20->tmax_idx_non_lgy_sts1_pathc_m << 1) | (ie_20->tmax_idx_non_lgy_sts1_pathc_l);
	psts_20->tmax_idx_non_lgy_sts_path[2][2] = ie_20->tmax_idx_non_lgy_sts2_pathc;
	psts_20->snr_non_lgy_sts_path[2][0] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts2_patha, 8);
	psts_20->snr_non_lgy_sts_path[2][1] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts2_pathb, 8);
	psts_20->snr_non_lgy_sts_path[0][2] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts0_pathc, 8);
	psts_20->snr_non_lgy_sts_path[1][2] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts1_pathc, 8);
	psts_20->snr_non_lgy_sts_path[2][2] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts2_pathc, 8);
#endif
#ifdef HALBB_COMPILE_ABOVE_4SS
	psts_20->tmax_idx_non_lgy_sts_path[3][0] = ie_20->tmax_idx_non_lgy_sts3_patha;
	psts_20->tmax_idx_non_lgy_sts_path[3][1] = ie_20->tmax_idx_non_lgy_sts3_pathb;
	psts_20->tmax_idx_non_lgy_sts_path[3][2] = ie_20->tmax_idx_non_lgy_sts3_pathc;
	psts_20->tmax_idx_non_lgy_sts_path[0][3] = ie_20->tmax_idx_non_lgy_sts0_pathd;
	psts_20->tmax_idx_non_lgy_sts_path[1][3] = (ie_20->tmax_idx_non_lgy_sts1_pathd_m << 1) | (ie_20->tmax_idx_non_lgy_sts1_pathd_l);
	psts_20->tmax_idx_non_lgy_sts_path[2][3] = ie_20->tmax_idx_non_lgy_sts2_pathd;
	psts_20->tmax_idx_non_lgy_sts_path[3][3] = ie_20->tmax_idx_non_lgy_sts3_pathd;
	psts_20->snr_non_lgy_sts_path[3][0] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts3_patha, 8);
	psts_20->snr_non_lgy_sts_path[3][1] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts3_pathb, 8);
	psts_20->snr_non_lgy_sts_path[3][2] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts3_pathc, 8);
	psts_20->snr_non_lgy_sts_path[0][3] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts0_pathd, 8);
	psts_20->snr_non_lgy_sts_path[1][3] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts1_pathd, 8);
	psts_20->snr_non_lgy_sts_path[2][3] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts2_pathd, 8);
	psts_20->snr_non_lgy_sts_path[3][3] = (s8)halbb_cnvrt_2_sign(ie_20->snr_non_lgy_sts3_pathd, 8);
#endif
...
void halbb_physts_ch_stability_update_phy_info_avg(struct bb_info *bb, u8 len)
{
	struct bb_physts_info	*physts = &bb->bb_physts_i;
	struct bb_physts_ch_stb_info *ch_stb = &physts->bb_physts_ch_stb_i;
	struct bb_physts_ch_stb_phy_info *recorder = ch_stb->phy_info_recorder;
	struct bb_physts_ch_stb_phy_info *phy_info_avg = &ch_stb->phy_info_avg;
	u16 rssi_ss_tmp[PHYSTS_SS_MAX] = {0}, snr_tmp = 0, ss = 0, cn_ss_tmp[PHYSTS_SS_MAX] = {0};
	u16 evm_max_tmp = 0, evm_min_tmp = 0;
	#if 0
	u16 evm_ss_tmp[PHYSTS_SS_MAX] = {0};
	#endif
	u16 channel_cnt[6] = {0};
	u8 i = 0, j = 0;
	u8 evm_invalid_cnt = 0, cnt_ss[PHYSTS_SS_MAX] = {0}, cnt_total = 0;

	if (len >= PHYSTS_CH_STB_LEN)
		return;
	...
	for (i = 0; i < len; i++) {
		...
		cnt_ss[ss - 1]++;
		cnt_total++;
		rssi_ss_tmp[ss - 1] += recorder[i].rssi_avg_ss[ss - 1];
		snr_tmp += recorder[i].snr;
		cn_ss_tmp[ss - 1] += recorder[i].cn_ss[ss - 1];
		if (recorder[i].evm_max == 192) {
			evm_invalid_cnt++;
		} else {
			evm_max_tmp += recorder[i].evm_max;
			evm_min_tmp += recorder[i].evm_min;
		}
		#if 0
		for (j = HALBB_SS0; j < ss; j++)
			evm_ss_tmp[j] += recorder[i].evm_ss[j];
		#endif
		if (recorder[i].channel < 6)
			channel_cnt[recorder[i].channel]++;
	}

	phy_info_avg->snr = (u8)HALBB_DIV(snr_tmp, len);

	for (i = HALBB_SS0; i < PHYSTS_SS_MAX; i++) {
		phy_info_avg->rssi_avg_ss[i] = (u8)HALBB_DIV(rssi_ss_tmp[i], cnt_ss[i]);
		phy_info_avg->cn_ss[i] = (u8)HALBB_DIV(cn_ss_tmp[i], cnt_ss[i]);
		#if 0
		phy_info_avg->evm_ss[i] = (u8)HALBB_DIV(evm_ss_tmp[i], cnt_total);
		#endif
		cnt_total -= cnt_ss[i];
	}
	phy_info_avg->evm_max = (u8)HALBB_DIV(evm_max_tmp, len - evm_invalid_cnt);
	phy_info_avg->evm_min = (u8)HALBB_DIV(evm_min_tmp, len - evm_invalid_cnt);
	phy_info_avg->channel = 0;
	for (i = 1; i < 6; i++) {
		if (channel_cnt[i] >= channel_cnt[phy_info_avg->channel])
			phy_info_avg->channel = i;
	}
	ch_stb->is_round_up = false;
	ch_stb->counter_idx = 0;
	ch_stb->diff_count = 0;
	ch_stb->channel_change_idx++;
}
#if 0
void halbb_physts_ch_stability_accel_h2c(struct bb_info *bb)
{
	struct rtw_hal_stainfo_t *hal_sta_i = NULL;
	struct bb_h2c_setting_phy_ch_rpt ra_phy_ch_rpt_i = {0};
	struct bb_physts_info	*physts = &bb->bb_physts_i;
	struct bb_physts_ch_stb_info *ch_stb = &physts->bb_physts_ch_stb_i;
	bool ret_val = true;
	bool accel_en = (bool)ch_stb->ra_accel_rpt.accel_en;
	u8 ch_diff_th = ch_stb->ra_accel_rpt.ch_diff_th;
	u8 rssi[4] = {0}, bcn_rssi[4] = {0};
	u16 macid = ch_stb->ra_accel_rpt.macid;
	u16 ra_macid = 0, path_cnt = 0;

	BB_DBG(bb, DBG_PHY_STS, "[ACCEL_H2C][%s]\n", __func__);

	if ((!ch_stb->en) || (!accel_en && !ch_stb->is_manual_accel))
		return;
	if ((ch_stb->channel_change_idx - ch_stb->channel_change_idx_backup) < ch_diff_th)
		return;
	...
	if (!hal_sta_i)
		return;
	if (!(hal_sta_i->ra_info.ra_registered))
		return;

	/* Only consider 1 client case*/
	ra_macid = bb->phl_sta_info[macid]->macid;
	BB_DBG(bb, DBG_PHY_STS, "[ACCEL_H2C] Add BB rssi info, macid=%d\n",  ra_macid);

	for (path_cnt = 0; path_cnt < 4; path_cnt++) {
		rssi[path_cnt] = (hal_sta_i->rssi_stat.rssi_ma_path[path_cnt] >> 5) & 0x7f;
		bcn_rssi[path_cnt] = (hal_sta_i->rssi_stat.rssi_bcn_ma_path[path_cnt] >> 5) & 0x7f;
		ra_phy_ch_rpt_i.rssi[path_cnt] = rssi[path_cnt];
		ra_phy_ch_rpt_i.bcn_rssi[path_cnt] = bcn_rssi[path_cnt];
	}
	ra_phy_ch_rpt_i.macid = (u8)(ra_macid & 0xff);
	ra_phy_ch_rpt_i.macid_msb = (u8)((ra_macid >> 8) & 0x3);
	ra_phy_ch_rpt_i.ch_change_idx = ch_stb->channel_change_idx;
	BB_DBG(bb, DBG_PHY_STS, "[ACCEL_H2C] macid={0x%x, 0x%x}, bcn_rssi={%d,%d,%d,%d}, data_rssi={%d,%d,%d,%d}, ch_change_idx=%d\n",
		ra_phy_ch_rpt_i.macid_msb, ra_phy_ch_rpt_i.macid,
		bcn_rssi[0], bcn_rssi[1], bcn_rssi[2], bcn_rssi[3],
		rssi[0], rssi[1], rssi[2], rssi[3], ch_stb->channel_change_idx);

	ch_stb->channel_change_idx_backup = ch_stb->channel_change_idx;

	/* Fill endcmd = 1 for last sta */
	ra_phy_ch_rpt_i.endcmd = 1;
	ret_val = halbb_fill_h2c_cmd(bb, sizeof(struct bb_h2c_setting_phy_ch_rpt),
				     RA_H2C_RA_PHY_CH_RPT, HALBB_H2C_RA, (u32 *)&ra_phy_ch_rpt_i);

	BB_DBG(bb, DBG_PHY_STS, "[ACCEL_H2C] RSSI cmd end\n");

	if (!ret_val) {
		BB_WARNING("[%s] H2C cmd error!!\n", __func__);
	} else {
		ch_stb->ra_accel_rpt.accel_en = false;
		halbb_cfg_timers(bb, BB_CANCEL_TIMER, &physts->physts_timer_i);
	}
}
#endif

void halbb_physts_ch_stability_ratio_statistic(struct bb_info *bb, u16 pkt_cnt)
{
	struct bb_physts_info	*physts = &bb->bb_physts_i;
	struct bb_physts_ch_stb_info *ch_stb = &physts->bb_physts_ch_stb_i;
	...
	temp = (u32)((ch_stb->channel_change_idx - ch_stb->channel_change_idx_backup) << 10);
	ratio->val = (u16)HALBB_DIV(temp * ch_stb->diff_count_th, pkt_cnt);

	if (ratio->num_sample > PHYSTS_CH_STB_LEN)
		ratio->num_sample = PHYSTS_CH_STB_LEN;

	if (ratio->idx >= ratio->num_sample) {
		ratio->idx = 0;
		ratio->round_up = true;
	}

	ratio->sum -= ratio->record[ratio->idx];
	ratio->record[ratio->idx] = ratio->val;
	ratio->sum += ratio->val;
	ratio->idx++;

	num_sample = ratio->round_up ? ratio->num_sample : ratio->idx;
	ratio->mean = HALBB_DIV(ratio->sum, num_sample);
	temp = 0;
	for (i = 0; i < num_sample; i++)
		temp += DIFF_2(ratio->record[i], ratio->mean);
	ratio->var = (u16)HALBB_DIV(temp, num_sample);

	ch_stb->channel_change_idx_cmn_rpt = ch_stb->channel_change_idx_backup;
	ch_stb->channel_change_idx_backup = ch_stb->channel_change_idx;
}

void halbb_physts_ch_stability(struct bb_info *bb, struct physts_rxd *desc)
{
	struct bb_physts_info	*physts = &bb->bb_physts_i;
	struct bb_rate_info *rate_i = &physts->bb_rate_i;
	struct bb_physts_ch_stb_info *ch_stb = &physts->bb_physts_ch_stb_i;
	struct bb_physts_ch_stb_phy_info *recorder = ch_stb->phy_info_recorder;
	struct bb_physts_ch_stb_phy_info *phy_info_avg = &ch_stb->phy_info_avg;
	struct bb_physts_rslt_hdr_info *psts_h = &physts->bb_physts_rslt_hdr_i;
	struct bb_physts_rslt_1_info *psts_1 = &physts->bb_physts_rslt_1_i;
	bool cn_change = false, evm_change = false;
	bool rssi_change = false, snr_change = false, channel_change = false;
	u8 agg_num = 0, i = 0;
	bool evm_change_en = ch_stb->diff_en & BIT(0);
	bool rssi_change_en = (ch_stb->diff_en & BIT(1)) >> 1;
	bool cn_change_en = (ch_stb->diff_en & BIT(2)) >> 2;
	bool snr_change_en = (ch_stb->diff_en & BIT(3)) >> 3;
	bool ch_change_en = (ch_stb->diff_en & BIT(4)) >> 4;
	#if 0
	u16 macid = ch_stb->ra_accel_rpt.macid;
	#endif
	...
	if (!ch_stb->en)
		return;
	if (psts_h->ie_map_type <= LEGACY_OFDM_PKT)
		return;
	if (rate_i->ss > PHYSTS_SS_MAX)
		return;
	if (desc->user_i[0].is_bcn)
		return;
#if 0
	if ((ch_stb->ra_accel_rpt.accel_en && macid != desc->user_i[0].macid) &&
	    !ch_stb->is_manual_accel)
		return;
#endif

	if (ch_stb->diff_count_max >= PHYSTS_CH_STB_LEN)
		ch_stb->diff_count_max = PHYSTS_CH_STB_LEN;
	if (ch_stb->diff_count_th > ch_stb->diff_count_max)
		ch_stb->diff_count_th = ch_stb->diff_count_max;

	if (ch_stb->counter_idx >= ch_stb->diff_count_max) {
		ch_stb->counter_idx = 0;
		ch_stb->is_round_up = true;
	}

	ch_stb->diff_count -= ch_stb->release_counter[ch_stb->counter_idx];

	recorder[ch_stb->counter_idx].rssi_avg_ss[rate_i->ss - 1] = psts_h->rssi_avg;
	recorder[ch_stb->counter_idx].snr = psts_1->snr_avg;
	recorder[ch_stb->counter_idx].cn_ss[rate_i->ss - 1] = psts_1->cn_avg;
	recorder[ch_stb->counter_idx].ss = rate_i->ss;
	#if 0
	recorder[ch_stb->counter_idx].channel = physts->bb_env_det.channel_model;
	#endif
	recorder[ch_stb->counter_idx].evm_max = psts_1->evm_max;
	recorder[ch_stb->counter_idx].evm_min = psts_1->evm_min;

	cn_change = (DIFF_2(phy_info_avg->cn_ss[rate_i->ss - 1], recorder[ch_stb->counter_idx].cn_ss[rate_i->ss - 1]) > ch_stb->cn_diff) ? true : false;

	if (rate_i->ss > 1) {
		if (DIFF_2(phy_info_avg->evm_min, recorder[ch_stb->counter_idx].evm_min) > ch_stb->evm_diff ||
		    DIFF_2(phy_info_avg->evm_max, recorder[ch_stb->counter_idx].evm_max) > ch_stb->evm_diff)
			evm_change = true;
	}
	snr_change = (DIFF_2(phy_info_avg->snr, recorder[ch_stb->counter_idx].snr) > ch_stb->snr_diff);
	rssi_change = (DIFF_2(phy_info_avg->rssi_avg_ss[rate_i->ss - 1], recorder[ch_stb->counter_idx].rssi_avg_ss[rate_i->ss - 1]) > ch_stb->rssi_avg_diff);
	channel_change = (phy_info_avg->channel != recorder[ch_stb->counter_idx].channel);

	ch_stb->snr_change_cnt += snr_change;
	ch_stb->cn_change_cnt += cn_change;
	ch_stb->channel_change_cnt += channel_change;
	ch_stb->evm_change_cnt += evm_change;
	ch_stb->rssi_change_cnt += rssi_change;

	if ((rssi_change && rssi_change_en) || (snr_change && snr_change_en) ||
	    (cn_change && cn_change_en) || (evm_change && evm_change_en) ||
	    (channel_change && ch_change_en)) {
		ch_stb->release_counter[ch_stb->counter_idx] = true;
	} else {
		ch_stb->release_counter[ch_stb->counter_idx] = false;
	}
	ch_stb->diff_count += ch_stb->release_counter[ch_stb->counter_idx];


	BB_DBG(bb, DBG_PHY_STS, "[%s] ===================== ch_stb INFO ==============================\n", __func__);
	halbb_print_rate_2_buff(bb, desc->data_rate, desc->gi_ltf, bb->dbg_buf, HALBB_SNPRINT_SIZE);
	BB_DBG(bb, DBG_PHY_STS, "[%s] [%d: %s] Counter_idx:%d\n", __func__,
	       psts_h->ie_map_type, bb_physts_bitmap_type_t[psts_h->ie_map_type], ch_stb->counter_idx);
	BB_DBG(bb, DBG_PHY_STS, "[%s] is_to_self=%d, su=%d, user_num=%d, phy_idx=%d\n", __func__,
	       desc->is_to_self, desc->is_su, desc->user_num, desc->phy_idx);
	BB_DBG(bb, DBG_PHY_STS, "[%s] Rate = %s (0x%x-%x), macid_su=%d\n", __func__,
	       bb->dbg_buf, desc->data_rate, desc->gi_ltf, desc->macid_su);
	BB_DBG(bb, DBG_PHY_STS, "[%s] diff_count_max:%d, diff_count_th:%d, PHYSTS_CH_STB_LEN:%d\n",
	       __func__, ch_stb->diff_count_max,
	       ch_stb->diff_count_th, PHYSTS_CH_STB_LEN);
	BB_DBG(bb, DBG_PHY_STS, "[%s] TH  {rssi, snr, cn, evm} = {%2d,%2d,%2d,%2d}\n",
	       __func__ ,ch_stb->rssi_avg_diff, ch_stb->snr_diff, ch_stb->cn_diff, ch_stb->evm_diff);
	BB_DBG(bb, DBG_PHY_STS, "[%s] avg {snr, channel} = {%2d,%2d}\n",
	       __func__, phy_info_avg->snr, phy_info_avg->channel);
	BB_DBG(bb, DBG_PHY_STS, "[%s] now {snr, channel, ss} = {%2d,%2d,%2d}\n",
	       __func__, recorder[ch_stb->counter_idx].snr,
	       recorder[ch_stb->counter_idx].channel, recorder[ch_stb->counter_idx].ss);
	BB_DBG(bb, DBG_PHY_STS, "[%s] avg_rssi_avg {1ss, 2ss, 3ss, 4ss} = {%2d,%2d,%2d,%2d}\n",
	       __func__,
	       phy_info_avg->rssi_avg_ss[HALBB_SS0], phy_info_avg->rssi_avg_ss[HALBB_SS1],
	       phy_info_avg->rssi_avg_ss[HALBB_SS2], phy_info_avg->rssi_avg_ss[HALBB_SS3]);
	BB_DBG(bb, DBG_PHY_STS, "[%s] now_rssi_avg {1ss, 2ss, 3ss, 4ss} = {%2d,%2d,%2d,%2d}\n",
	       __func__,
	       recorder[ch_stb->counter_idx].rssi_avg_ss[HALBB_SS0], recorder[ch_stb->counter_idx].rssi_avg_ss[HALBB_SS1],
	       recorder[ch_stb->counter_idx].rssi_avg_ss[HALBB_SS2], recorder[ch_stb->counter_idx].rssi_avg_ss[HALBB_SS3]);
	BB_DBG(bb, DBG_PHY_STS, "[%s] avg_cn {1ss, 2ss, 3ss, 4ss} = {%2d,%2d,%2d,%2d}\n",
	       __func__,
	       phy_info_avg->cn_ss[HALBB_SS0], phy_info_avg->cn_ss[HALBB_SS1],
	       phy_info_avg->cn_ss[HALBB_SS2], phy_info_avg->cn_ss[HALBB_SS3]);
	BB_DBG(bb, DBG_PHY_STS, "[%s] now_cn {1ss, 2ss, 3ss, 4ss} = {%2d,%2d,%2d,%2d}\n",
	       __func__,
	       recorder[ch_stb->counter_idx].cn_ss[HALBB_SS0], recorder[ch_stb->counter_idx].cn_ss[HALBB_SS1],
	       recorder[ch_stb->counter_idx].cn_ss[HALBB_SS2], recorder[ch_stb->counter_idx].cn_ss[HALBB_SS3]);
	#if 0
	BB_DBG(bb, DBG_PHY_STS, "[%s] avg_evm {max, min| ss0, ss1, ss2, ss3} = {%2d,%2d,%2d,%2d,%2d,%2d}\n",
	       __func__, phy_info_avg->evm_max, phy_info_avg->evm_min,
	       phy_info_avg->evm_ss[HALBB_SS0], phy_info_avg->evm_ss[HALBB_SS1],
	       phy_info_avg->evm_ss[HALBB_SS2], phy_info_avg->evm_ss[HALBB_SS3]);
	BB_DBG(bb, DBG_PHY_STS, "[%s] now_evm {max, min| ss0, ss1, ss2, ss3} = {%2d,%2d,%2d,%2d,%2d,%2d}\n",
	       __func__, recorder[ch_stb->counter_idx].evm_max, recorder[ch_stb->counter_idx].evm_min,
	       recorder[ch_stb->counter_idx].evm_ss[HALBB_SS0], recorder[ch_stb->counter_idx].evm_ss[HALBB_SS1],
	       recorder[ch_stb->counter_idx].evm_ss[HALBB_SS2], recorder[ch_stb->counter_idx].evm_ss[HALBB_SS3]);
	#endif
	BB_DBG(bb, DBG_PHY_STS, "[%s] rssi_change:%d, snr_change:%d, cn_change:%d, channel_change:%d, evm_change:%d\n",
	       __func__, rssi_change, snr_change, cn_change, channel_change, evm_change);
	BB_DBG(bb, DBG_PHY_STS, "[%s] is_diff:%d, channel_change_idx:%d\n",
	       __func__, ch_stb->release_counter[ch_stb->counter_idx], ch_stb->channel_change_idx);
	BB_DBG(bb, DBG_PHY_STS, "[%s] ================================================================\n", __func__);

	ch_stb->counter_idx++;
	if (ch_stb->diff_count >= ch_stb->diff_count_th) {
		for (i = 0; i < ch_stb->diff_count_max; i++)
			ch_stb->release_counter[i] = false;
		agg_num = ch_stb->is_round_up ? ch_stb->diff_count_max : ch_stb->counter_idx;
		BB_DBG(bb, DBG_PHY_STS, "[%s] New phy_info_avg agg_num:%d, round_up:%d, cout_max:%d, counter_idx:%d\n",
		       __func__, agg_num, ch_stb->is_round_up, ch_stb->diff_count_max, ch_stb->counter_idx);
		halbb_physts_ch_stability_update_phy_info_avg(bb, agg_num);
	}
...
{
	struct bb_physts_info	*physts = NULL;
	...
	if (physts->hold_type == PSTS_HOLD_STOP_PAR) {
	...
	for (i = 0; i < IE_PHYSTS_LEN_ALL; i++) {
		...
		if (curr_ie == IE00_CMN_CCK) {
			...
			if (bb->ic_type == BB_RTL8852C)
				halbb_physts_ie_20_gen2(bb, addr, ie_len, desc);
			else
...
PARSING_END:
	...
	if (desc->is_to_self && bb_rpt->is_pkt_with_data && !is_bt_polluted) {
		...
		halbb_physts_ch_stability(bb, desc);
...
void halbb_physts_ch_stability_init(struct bb_info *bb)
{
	struct bb_physts_info *physts = &bb->bb_physts_i;
	struct bb_physts_ch_stb_info *ch_stb = &physts->bb_physts_ch_stb_i;
	u8 i = 0;
	...
	ch_stb->en = true;
	ch_stb->evm_diff = 8; /*.25dB*/
	ch_stb->snr_diff = 2; /*dB*/
	ch_stb->rssi_avg_diff = 1; /*.5dB*/
	ch_stb->cn_diff = 2; /*.5dB*/
	ch_stb->is_round_up = false;
	ch_stb->channel_change_idx = 0;
	ch_stb->channel_change_idx_backup = 0;
	ch_stb->diff_count = 0;
	ch_stb->diff_count_max = 10;
	ch_stb->diff_count_th = 8;
	ch_stb->counter_idx = 0;
	ch_stb->diff_en = 0xf;
	ch_stb->channel_change_ratio.idx = 0;
	ch_stb->channel_change_ratio.round_up = false;
	ch_stb->channel_change_ratio.num_sample = 16;
	for (i = 0; i < PHYSTS_CH_STB_LEN; i++) {
		ch_stb->release_counter[i] = false;
		ch_stb->channel_change_ratio.record[i] = 0;
	}
	for (i = 0; i < PHYSTS_SS_MAX; i++) {
		ch_stb->phy_info_avg.cn_ss[i] = 127;
	}
}
...
void halbb_physts_parsing_init(struct bb_info *bb)
{
	...
	physts->hold_type = PSTS_HOLD_DISABLE;
	...
	for (i = 0; i < PHYSTS_BITMAP_NUM; i++) {
		...
		if (i >= CCK_PKT) {
			...
			for (j = 0; j < bb->num_rf_path; j++)
				u32_tmp |= BIT(IE04_CMN_EXT_PATH_A + j);
		}
		if (i >= HT_PKT) {
			u32_tmp |= BIT(IE20_DBG_OFDM_FD_USER_SEG_0);
			#if 0
			/*default open IE4/5*/
			if (bb->ic_type & BB_IC_2SS){
				for (j = 0; j < 2; j++)
					u32_tmp |= BIT(IE04_CMN_EXT_PATH_A + j);
			}
			#endif
	...
	halbb_physts_ie_bitmap_set(bb, TRIG_BASE_PPDU, BIT(13) | BIT(1));
	...
	halbb_physts_ch_stability_init(bb);
		  ...
		  char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[1], "-h") == 0) {
		...
		BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			    "hold {0:disable, 1:hold with parsing, 2:hold and stop parsing}\n");
		BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			    "ch_stb -h\n");
		return;
	}
	...
	if (_os_strcmp(input[1], "bypass_bt") == 0) {
		...
		BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			    "brk_rpt_en = %d\n",val[0]);
	} else if (_os_strcmp(input[1], "hold") == 0) {
		HALBB_SCAN(input[2], DCMD_DECIMAL, &val[0]);
		if (val[0] == PSTS_HOLD_WITH_PAR)
			halbb_cmn_info_rpt_reset(bb);
		physts->hold_type = (enum bb_physts_hold_type)val[0];
		BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			    "hold_type = %d\n",val[0]);
	} else if (_os_strcmp(input[1], "ch_stb") == 0) {
		if (_os_strcmp(input[2], "-h") == 0) {
			...
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "{evm_diff/snr_diff/rssi_avg_diff/cn_diff} {val}\n");
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "{diff_count_th/diff_count_max} {val (< %d)}\n", PHYSTS_CH_STB_LEN);
			return;
		} else if (_os_strcmp(input[2], "show") == 0) {
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "ch_stability en = %d\n", physts->bb_physts_ch_stb_i.en);
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "evm_diff      = %d(0.25dB)\n", physts->bb_physts_ch_stb_i.evm_diff);
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "rssi_avg_diff = %d(0.5dB)\n", physts->bb_physts_ch_stb_i.rssi_avg_diff);
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "cn_diff       = %d(0.5dB)\n", physts->bb_physts_ch_stb_i.cn_diff);
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "snr_diff      = %d(dB)\n", physts->bb_physts_ch_stb_i.snr_diff);
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "diff_en {channel, snr, cn, rssi, evm} = 0x%x\n",
				    physts->bb_physts_ch_stb_i.diff_en);
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "diff_count {threshold, max} = {%d, %d}\n",
				    physts->bb_physts_ch_stb_i.diff_count_th,
				    physts->bb_physts_ch_stb_i.diff_count_max);
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "num_sample = %d \n",
				    physts->bb_physts_ch_stb_i.channel_change_ratio.num_sample);
			return;
		}
		if (_os_strcmp(input[2], "diff_en") == 0) {
			HALBB_SCAN(input[3], DCMD_HEX, &val[0]);
			physts->bb_physts_ch_stb_i.diff_en = (u8)val[0];
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "diff_en {channel, snr, cn, rssi, evm} = 0x%x\n",
				    physts->bb_physts_ch_stb_i.diff_en);
			return;
		...
		if (_os_strcmp(input[2], "en") == 0) {
			physts->bb_physts_ch_stb_i.en = (bool)val[0];
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "Set en = %d\n", physts->bb_physts_ch_stb_i.en);
		} else if (_os_strcmp(input[2], "evm_diff") == 0) {
			physts->bb_physts_ch_stb_i.evm_diff = (u8)(val[0]);
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "Set evm_diff = %d(0.25dB)\n", physts->bb_physts_ch_stb_i.evm_diff);
		} else if (_os_strcmp(input[2], "snr_diff") == 0) {
			physts->bb_physts_ch_stb_i.snr_diff = (u8)val[0];
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "Set snr_diff = %d(dB)\n", physts->bb_physts_ch_stb_i.snr_diff);
		} else if (_os_strcmp(input[2], "rssi_avg_diff") == 0) {
			physts->bb_physts_ch_stb_i.rssi_avg_diff = (u8)(val[0]);
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "Set rssi_avg_diff = %d(0.5dB)\n", physts->bb_physts_ch_stb_i.rssi_avg_diff);
		} else if (_os_strcmp(input[2], "cn_diff") == 0) {
			physts->bb_physts_ch_stb_i.cn_diff = (u8)(val[0]);
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "Set cn_diff = %d(0.5dB)\n", physts->bb_physts_ch_stb_i.cn_diff);
		} else if (_os_strcmp(input[2], "diff_count_th") == 0) {
			if (val[0] > PHYSTS_CH_STB_LEN)
				val[0] = PHYSTS_CH_STB_LEN;
			physts->bb_physts_ch_stb_i.diff_count_th = (u8)val[0];
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "Set diff_count_th = %d\n", physts->bb_physts_ch_stb_i.diff_count_th);
		} else if (_os_strcmp(input[2], "diff_count_max") == 0) {
			if (val[0] > PHYSTS_CH_STB_LEN)
				val[0] = PHYSTS_CH_STB_LEN;
			physts->bb_physts_ch_stb_i.diff_count_max = (u8)val[0];
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "Set diff_count_max = %d\n", physts->bb_physts_ch_stb_i.diff_count_max);
		} else if (_os_strcmp(input[2], "num_sample") == 0) {
			physts->bb_physts_ch_stb_i.channel_change_ratio.num_sample = (u8)val[0];
			BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
				    "num_sample = %d\n", physts->bb_physts_ch_stb_i.channel_change_ratio.num_sample);
		}
	}

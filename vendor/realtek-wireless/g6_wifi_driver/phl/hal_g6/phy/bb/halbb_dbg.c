...
void halbb_dig_cmn_log(struct bb_info *bb)
{
	...
	if ((bb->ic_type == BB_RTL8852A && bb->hal_com->cv < CCV) ||
		...
		if ((bb_dig->dig_mode != DIG_TDMA) && (bb_dig->dig_mode != DIG_TDMA_ADV))
			...
			BB_DBG(bb, DBG_CMN, "[H]PD_low_bd(ofdm, cck) : (-%d, %d) dBm, [L]PD_low_bd(ofdm, cck) = (-%d, %d) dBm\n",
			       bb_dig->dig_state_h_i.pd_low_th,
			       bb_dig->dig_state_h_i.rssi_nocca_low_th,
			       bb_dig->dig_state_l_i.pd_low_th,
			       bb_dig->dig_state_l_i.rssi_nocca_low_th);
		#else
		BB_DBG(bb, DBG_CMN, "PD_low_bd_en(ofdm, cck) : (%d, %d), PD_low_bd(ofdm, cck) = (-%d, %d) dBm\n",
		       ofdm_pd_th_en, cck_pd_th_en, 102 - (ofdm_pd_th << 1),
		       cck_pd_th);
...
void halbb_basic_dbg_08_rssi_rate_hist_mu(struct bb_info *bb) {
	...
	BB_DBG(bb, DBG_CMN, "====[8. [MU] AVG RSSI/RxRate/HIST]\n");
	halbb_basic_dbg_08_rssi_rate_mu(bb);
	halbb_basic_dbg_08_hist_mu(bb);
}
...
void halbb_basic_dbg_09_dm_summary(struct bb_info *bb)
{
	...
	if (bb->support_ability & BB_ANT_DIV) {
		BB_DBG(bb, DBG_CMN,"[ANTDIV] rfe_type=%d, Target_ANT=%d\n",
...
void halbb_basic_dbg_message(struct bb_info *bb)
{
	...
	if (cmn_dbg->cmn_log_2_cnsl_en)
		return;
	if (cmn_dbg->cmn_log_2_drv_statistic_en)
		return;
	...
	BB_DBG(bb, DBG_CMN, "cmn_dbg_msg_component = 0x%x\n",bb->cmn_dbg_msg_component);
	...
	if (bb->bb_link_i.is_linked) {
		...
		halbb_basic_dbg_08_rssi_rate_hist_mu(bb);
	}
...
{
	u16 dbg_comp = (u16)(c2h[0] | (c2h[1] << 8));
	u16 dbg_num = (u16)(c2h[2] | (c2h[3] << 8));
	u16 content[5] = {0};
	u8 i = 0;

	for (i = 0; i < 5; i++)
		content[i] = (u16)(c2h[4 + (i << 1)] | (c2h[5 + (i << 1)] << 8));

	switch (dbg_comp) {
	case FWBBDBG_BF:
		if (dbg_num == BBDBG_NUM_BF_CHECK_BF_EXIST_0_0) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][0] BFentry exists macid:%d, txbf_entry_idx=%d\n",
				content[0], content[1]);
		} else if (dbg_num == BBDBG_NUM_BF_CHECK_BF_EXIST_0_1) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][0] TXBF_pause:%d, STBC_pause=%d, ss=%d, mcs=%d, bw=%d\n",
				content[0], content[1], content[2], content[3], content[4]);
		} else if (dbg_num == BBDBG_NUM_BF_CHECK_BF_EXIST_0_2) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][0] EnBF->DisBF=%d exists, ss=%d, mcs=%d, bw=%d\n",
				content[0], content[2], content[3], content[4]);
		} else if (dbg_num == BBDBG_NUM_BF_CHECK_BF_EXIST_0_3) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][0] Start one shot sounding\n");
		} else if (dbg_num == BBDBG_NUM_BF_CHECK_BF_EXIST_0_4) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][0][%d] Clear stbc pkt and start sounding,s tbc_lock=%d\n",
				   content[0], content[1]);
		} else if (dbg_num == BBDBG_NUM_BF_CHECK_BF_EXIST_0_5) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][0][%d] Sound done start try,bf_lock=%d\n",
				   content[0], content[1]);
		} else if (dbg_num == BBDBG_NUM_BF_CHECK_BF_EXIST_0_6) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][0][%d] BF etry backup=%x,magic_number=%x\n",
				   content[0], content[1], content[2]);
		} else if (dbg_num == BBDBG_NUM_BF_CHECK_BF_EXIST_0_7) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][0][%d] BF etry restore=%x\n",
				   content[0], content[1]);
		} else if (dbg_num == BBDBG_NUM_BF_CHECK_BF_EXIST_0_8) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][0][%d][Trydone] BF etry backup=%x,magic_number=%x\n",
				   content[0], content[1], content[2]);
		} else if (dbg_num == BBDBG_NUM_BF_CHECK_BF_EXIST_0_9) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][0][%d][Trydone] BF etry restore=%x\n",
				   content[0], content[1]);
		} else {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][others][0] dbg_num=%d, content0~4 = [0x%x,0x%x,0x%x,0x%x,0x%x]\n",
				   dbg_num, content[0], content[1], content[2], content[3],
				   content[4]);
		}
		break;
	case FWBBDBG_RA_STASTISTIC:
		if (dbg_num == BBDBG_NUM_RA_RATE_CHANGED_1_0) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][1] macid:%d, rate_changed:%d\n",
				   content[0], content[1]);
		} else if (dbg_num == BBDBG_NUM_RA_CAL_PARA_1_1) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][1] macid:%d, PER:%d, RDR:%d, R4:%d\n",
				   content[0], content[1], content[2], content[3]);
		} else {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][others][1] dbg_num=%d, content0~4 = [0x%x,0x%x,0x%x,0x%x,0x%x]\n",
				   dbg_num, content[0], content[1], content[2], content[3],
				   content[4]);
		}
		break;
	case FWBBDBG_RA_SEARCH_RATE:
		if (dbg_num == BBDBG_NUM_RA_SEARCH_RATE_RU_2_0) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][2][RU] macid:%d, rate_idx(mode,ss,mcs)=(%d,%d,%d), bw_idx:%d, multi_rate[0](mode,ss,mcs)=(%d,%d,%d), multi_bw[0]:%d,\n",
				   content[0], c2h[6], c2h[7], c2h[8], c2h[9],
				   c2h[10], c2h[11], c2h[12], c2h[13]);
		} else if (dbg_num == BBDBG_NUM_RA_SEARCH_RATE_RU_2_1) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][2][RU] multi_rate[1](mode,ss,mcs)=(%d,%d,%d), multi_bw[1]:%d\n",
				   c2h[6], c2h[7], c2h[8], c2h[9]);
		} else if (dbg_num == BBDBG_NUM_RA_SEARCH_RATE_RD_2_2) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][2][RD] macid:%d, rate_idx(mode,ss,mcs)=(%d,%d,%d), bw_idx:%d, multi_rate[0](mode,ss,mcs)=(%d,%d,%d), multi_bw[0]:%d,\n",
				   content[0], c2h[6], c2h[7], c2h[8], c2h[9],
				   c2h[10], c2h[11], c2h[12], c2h[13]);
		} else if (dbg_num == BBDBG_NUM_RA_SEARCH_RATE_RD_2_3) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][2][RD] multi_rate[1](mode,ss,mcs)=(%d,%d,%d), multi_bw[1]:%d\n",
				   c2h[6], c2h[7], c2h[8], c2h[9]);
		} else {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][others][2] dbg_num=%d, content0~4 = [0x%x,0x%x,0x%x,0x%x,0x%x]\n",
				   dbg_num, content[0], content[1], content[2], content[3],
				   content[4]);
		}
		break;
	case FWBBDBG_RA_RATE_STATE_DECISION:
		if (dbg_num == BBDBG_NUM_RA_D_O_PARA_3_0) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][3] macid:%d, rate_idx:%d, bw_idx:%d, PER_MA:%d, VAR:%d\n",
				   content[0], content[1], content[2], content[3],
				   content[4]);
		} else if (dbg_num == BBDBG_NUM_RA_UP_DOWN_TH_3_1) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][3] macid:%d, d_o_n:%d, d_o_p:%d, RD_TH:%d, RU_TH:%d\n",
				   content[0], content[1], content[2], content[3],
				   content[4]);
		} else if (dbg_num == BBDBG_NUM_RA_RATE_DOWN_3_2) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][3] macid:%d, rate down, rate_down_cnt:%d\n",
				   content[0], content[1]);
		} else if (dbg_num == BBDBG_NUM_RA_RATE_FORCE_DOWN_3_3) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][3] macid:%d, force rate down\n",
				   content[0]);
		} else if (dbg_num == BBDBG_NUM_RA_RATE_UP_3_4) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][3] macid:%d, rate up, rate_up_cnt:%d\n",
				   content[0], content[1]);
		} else if (dbg_num == BBDBG_NUM_RA_RATE_STAY_3_5) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][3] macid:%d, rate stay, rate_up_cnt:%d, rate_down_cnt:%d\n",
				   content[0], content[1], content[2]);
		} else {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][others][3] dbg_num=%d, content0~4 = [0x%x,0x%x,0x%x,0x%x,0x%x]\n",
				   dbg_num, content[0], content[1], content[2], content[3],
				   content[4]);
		}
		break;
	case FWBBDBG_RA_TRY_RESULT:
		if (dbg_num == BBDBG_NUM_RA_WRITE_CMAC_4_0) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][4] macid:%d, mode:%d, ss:%d, mcs:%d, bw:%d\n",
				   content[0], content[1], content[2], content[3],
				   content[4]);
		} else if (dbg_num == BBDBG_NUM_RA_TRY_RESULT_4_1) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][4] macid:%d, try_result=%d, try_bw:%d, try_rate_idx:%d, try_PER:%d, try_RDR:%d\n",
				   content[0], c2h[7], c2h[6], content[2], content[3],
				   content[4]);
		} else if (dbg_num == BBDBG_NUM_RA_TRY_RESULT_4_2) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][4] macid:%d, PER=%d, RDR=%d, R4=%d, ",
				   content[0], content[1], content[2], content[3]);
		} else if (dbg_num == BBDBG_NUM_RA_TRY_RESULT_4_3) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][4] pre_tp=%d,next_tp=%d\n",
				   (content[0] | content[1] << 16), (content[2] | content[3] << 16));
		} else if (dbg_num == BBDBG_NUM_RA_TRY_RESULT_4_4) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][4] (pre_rate,pre_pcr)=(%d,%d),(next_rate,next_pcr)=(%d,%d)\n",
				   content[0], content[1], content[2], content[3]);
		} else if (dbg_num == BBDBG_NUM_RA_TRY_FAIL_4_5) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][4] TryFail\n");
		} else if (dbg_num == BBDBG_NUM_RA_TRY_SUCCESS_4_6) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][4] TrySuccess\n");
		} else {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][others][4] dbg_num=%d, content0~4 = [0x%x,0x%x,0x%x,0x%x,0x%x]\n",
				   dbg_num, content[0], content[1], content[2], content[3],
				   content[4]);
		}
		break;
	case FWBBDBG_RA_TX_RPT:
		if (dbg_num == BBDBG_NUM_RA_TX_RPT_5_0) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][5] NoHESU\n");
		} else if (dbg_num == BBDBG_NUM_RA_TX_RPT_5_1) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][5] macid:%d, Txcnt=%d, Try=%d, BW=%d, wd_not_released=%d, total=%d, OK=%d,\n",
				   content[0], (content[1] & 0x3F), ((content[1] >> 6) & 0x1), ((content[1] >> 7) & 0x3), ((content[1] >> 9) & 0x1),
				   (content[2] & 0xff), ((content[2] >> 8) & 0xff));
		} else if (dbg_num == BBDBG_NUM_RA_TX_RPT_5_2) {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][RA][5] Initrate(mode,ss,mcs)=(%d,%d,%d), InitGiltf=%d, Finalrate(mode,ss,mcs)=(%d,%d,%d), FinalGiltf=%d\n",
				   c2h[6], c2h[7], c2h[8], c2h[9],
				   c2h[10], c2h[11], c2h[12], c2h[13]);
		} else {
			BB_DBG(bb, DBG_FW_DBG,
				   "[FWBB][others][5] dbg_num=%d, content0~4 = [0x%x,0x%x,0x%x,0x%x,0x%x]\n",
				   dbg_num, content[0], content[1], content[2], content[3],
				   content[4]);
		}
		break;

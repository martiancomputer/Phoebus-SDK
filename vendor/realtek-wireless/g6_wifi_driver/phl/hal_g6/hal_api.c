...
enum rtw_hal_status rtw_hal_get_freeruncnt(void *hal, u8 band, u32 *freeruncnt_h, u32 *freeruncnt_l)
{
	enum rtw_hal_status status = RTW_HAL_STATUS_FAILURE;
	status = rtw_hal_mac_get_freeruncnt(hal, band, freeruncnt_h, freeruncnt_l);

	if (status != RTW_HAL_STATUS_SUCCESS) {
		PHL_TRACE(COMP_PHL_MCC, _PHL_ERR_, "rtw_hal_get_freeruncnt(): get failed\n");
	}
	return status;
}
#endif
...
rtw_hal_set_macid_pkt_drop(void *hal, u16 macid, u8 sel, u8 port, u8 mbssid)
{
	...
	return rtw_hal_mac_set_macid_pkt_drop(hal_com, macid, sel, port, mbssid);
}
                                                ...
                                                void *hal)
{
	...
	if (phl_com->dev_cap.rx_hdr_conv && phl_com->dev_cap.rx_amsdu_cut)
		return rtw_hal_mac_enable_rx_amsdu_cut(hal_info);
                                                 ...
                                                 void *hal)
{
	...
	if (phl_com->dev_cap.rx_hdr_conv && phl_com->dev_cap.rx_amsdu_cut)
		return rtw_hal_mac_disable_rx_amsdu_cut(hal_info);
	return RTW_HAL_STATUS_FAILURE;
}
...
void rtw_hal_cfg_mac_dump_setting(void *hal, u8 cfg)
{
	...
	rtw_hal_mac_set_dbglog_cfg(hal_info, cfg);
}

enum rtw_hal_status rtw_hal_set_dig_mode(void *hal, u8 dig_op_mode)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;

	rtw_hal_bb_dig_mode_update(hal_info->hal_com, dig_op_mode);
...
rtw_hal_nvm_apply_dev_cap(void *hal, struct rtw_phl_com_t *phl_com)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;
	enum rtw_hal_status hal_sts = RTW_HAL_STATUS_SUCCESS;
...
u8 rtw_hal_antdiv_get_targetant(void *hal)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;

	return rtw_hal_bb_antdiv_get_targetant(hal_info);
}

enum rtw_hal_status
rtw_hal_antdiv_pause(void *hal, u8 target_ant, u8 pause)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;

	return rtw_hal_bb_antdiv_pause(hal_info, target_ant, pause);
}

enum rtw_hal_status
rtw_hal_sr_enable(void *hal, u8 sr_en)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;

	return rtw_hal_bb_sr_enable(hal_info, sr_en);
}

#ifdef CONFIG_RTL_WIFI_OFFLOAD
u32 rtw_hal_trx_bd_reinit(void *hal, void *txbd_buf, void *rxbd_buf)
{
	u32 ret;
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct mac_ax_adapter *mac = (struct mac_ax_adapter *)hal_info->mac;

	ret = rtw_hal_mac_trx_bd_reallocate(hal_info, txbd_buf, rxbd_buf);

	return ret;
}
#endif /* CONFIG_RTL_WIFI_OFFLOAD */

void rtw_hal_nhm_mntr_result(void *hal, void *rpt, enum phl_phy_idx phy_idx)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;

	return rtw_hal_bb_nhm_mntr_result(hal_info->hal_com, rpt, phy_idx);
}

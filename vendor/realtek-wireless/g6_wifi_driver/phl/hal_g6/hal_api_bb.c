...
#ifdef USE_TRUE_PHY
extern void realtime_traffic_set_timer(_adapter *padapter, u32 timeout);
void rtw_hal_bb_set_realtime_tp_timer(struct rtw_hal_com_t *hal_com, u32 timeout)
{
	void *drv_priv = hal_com->drv_priv;
	struct dvobj_priv *pobj = (struct dvobj_priv *)drv_priv;

	realtime_traffic_set_timer(pobj->padapters[0], timeout);
}

void rtw_hal_bb_get_realtime_tp(struct rtw_phl_com_t *phl_com, u32 *tx, u32 *rx)
{
	if (phl_com) {
		*tx = phl_com->phl_stats.rt_txtp;
		*rx = phl_com->phl_stats.rt_rxtp;
	}
}
...
enum rtw_hal_status rtw_hal_bb_adc_cfg(struct rtw_hal_com_t *hal_com,
	enum channel_width bw, enum rf_path path, enum phl_phy_idx phy_idx)
{
	struct hal_info_t *hal = (struct hal_info_t *)hal_com->hal_priv;

	halbb_adc_cfg(hal->bb, bw, path, phy_idx);
...
s8 rtw_hal_bb_get_tx_pwr_by_txrate (void *hal, enum channel_width bw, u8 mcs_idx)
{
    ...
    struct hal_info_t *hal_info = (struct hal_info_t *)hal;
    struct rtw_hal_com_t *hal_com = hal_info->hal_com;
    struct rtw_tpu_info *tpu = &hal_com->band[HW_PHY_0].rtw_tpu_i;
			 ...
			 struct rtw_phl_rx_pkt *phl_rx, u8 is_su)
{
	...
	if ((bb_rpt.rssi_avg != 0) || (bb_rpt.physts_rpt_valid == 1)) {
		...
		for (i = 0; i < RTW_PHL_MAX_RF_PATH; i++) {
			...
			phy_info->rssi_raw_path[i] = bb_rpt.rssi[i];
			PHL_TRACE(COMP_PHL_PSTS, _PHL_INFO_,
					"phy_info->rssi_path[%d] = %d, phy_info->rssi_raw_path[%d] = %d\n",
					i, phy_info->rssi_path[i], i, phy_info->rssi_raw_path[i]);
		}
		/* SNR from bb rpt */
...
enum rtw_hal_status rtw_hal_bb_acs_mntr_result(struct hal_info_t *hal_info, void *rpt)
{
	...
	if ((result & (CLM_SUCCESS | NHM_SUCCESS)) != (CLM_SUCCESS | NHM_SUCCESS)) {
		...
		acs_rpt->nhm_cca_ratio = mntr_rpt.nhm_cca_ratio;
		acs_rpt->nhm_tx_ratio = mntr_rpt.nhm_tx_ratio;
		acs_rpt->nhm_ratio = mntr_rpt.nhm_ratio;
		acs_rpt->nhm_idle_ratio = mntr_rpt.nhm_idle_ratio;
		hal_mem_cpy(hal_info->hal_com, acs_rpt->nhm_rpt, mntr_rpt.nhm_rpt, NHM_RPT_NUM);
		acs_rpt->nhm_tx_cnt = mntr_rpt.nhm_tx_cnt;
		acs_rpt->nhm_cca_cnt = mntr_rpt.nhm_cca_cnt;
		acs_rpt->nhm_idle_cnt = mntr_rpt.nhm_idle_cnt;
		if (result & IFS_CLM_SUCCESS)
			acs_rpt->clm_fa_ratio = mntr_rpt.ifs_clm_cck_fa_ratio + mntr_rpt.ifs_clm_ofdm_fa_ratio;
		return RTW_HAL_STATUS_SUCCESS;
	}
}
...
enum rtw_hal_status
rtw_hal_bb_set_agc_fix_gain(struct hal_info_t *hal_info, bool en,
				u8 elna_idx, u8 lna_idx, u8 tia_idx, u8 rxbb_idx, enum rf_path path)
{
	halbb_agc_tia_shrink(hal_info->bb, en, true, path);
	halbb_agc_fix_gain(hal_info->bb, en, path);
	halbb_set_igi(hal_info->bb, lna_idx, tia_idx, rxbb_idx, path);
	halbb_agc_elna_idx(hal_info->bb, elna_idx, path);

	return RTW_HAL_STATUS_SUCCESS;
}

enum rtw_hal_status
rtw_hal_bb_get_rssi_rx_k(struct hal_info_t *hal_info, enum rf_path rx_path, u8 *rssi)
{
	enum rtw_hal_status ret=RTW_HAL_STATUS_SUCCESS;

	PHL_INFO("[MP HAL API]%s\n", __FUNCTION__);

	*rssi = halbb_mp_get_rssi_td(hal_info->bb, rx_path);
...
void rtw_hal_bb_nhm_mntr_result(struct rtw_hal_com_t *hal_com, void *rpt, enum phl_phy_idx phy_idx)
{
	...
	nhm_rpt->nhm_cca_ratio = mntr_rpt.nhm_cca_ratio;
	nhm_rpt->nhm_tx_ratio = mntr_rpt.nhm_tx_ratio;
	nhm_rpt->nhm_idle_ratio = mntr_rpt.nhm_idle_ratio;
...
enum rtw_hal_status rtw_hal_bb_dig_mode_update(struct rtw_hal_com_t *hal_com, u8 dig_op_mode)
{
	struct hal_info_t *hal = hal_com->hal_priv;
	halbb_dig_mode_update(hal->bb, dig_op_mode);

	return RTW_HAL_STATUS_SUCCESS;
}

u8 rtw_hal_bb_antdiv_get_targetant(struct hal_info_t *hal_info)
{
	return halbb_antdiv_get_targetant(hal_info->bb);
}

enum rtw_hal_status rtw_hal_bb_antdiv_pause(struct hal_info_t *hal_info, u8 target_ant, u8 pause)
{
	struct rtw_hal_com_t *hal_com = hal_info->hal_com;
	struct bb_info *bb = hal_info->bb;
	enum halbb_pause_type type;
	u32 val[1] = {0};
	u8 ret = PAUSE_FAIL;
	...
	ret = halbb_pause_func(bb, F_ANT_DIV, type, HALBB_PAUSE_LV_3, 1, val);
	if (ret == PAUSE_FAIL)
		return RTW_HAL_STATUS_FAILURE;
	else
		return RTW_HAL_STATUS_SUCCESS;
...
enum rtw_hal_status rtw_hal_bb_sr_enable(struct hal_info_t *hal_info, u8 sr_en)
{
	struct bb_info *bb = hal_info->bb;

	halbb_spatial_reuse_en(bb, sr_en);
...
enum rtw_hal_status
rtw_hal_bb_set_agc_fix_gain(struct hal_info_t *hal_info, u8 en,
				u8 elna_idx, u8 lna_idx, u8 tia_idx, u8 rxbb_idx, u8 path)
{
	return RTW_HAL_STATUS_SUCCESS;
}

...
u32 rtw_hal_mac_set_tx_retry_limit_all_sta(struct rtw_hal_com_t *hal_com, u8 limit)
{
	struct hal_info_t *hal = hal_com->hal_priv;
	struct mac_ax_adapter *mac = hal_to_mac(hal);
	struct mac_ax_ops *ops = mac->ops;
	struct mac_ax_rty_lmt param;
	u32 result = 0;

	// convert retry limit to tx cnt limit
	if (limit >= 63)	// max: 63(5bit)
		limit = 63;
	else
		limit += 1;

	param.macid = MACID_NONE;	// whole band
	param.tx_cnt = limit;
	param.short_tx_cnt = limit;

	result = ops->set_hw_value(mac, MAC_AX_HW_SET_DATA_RTY_LMT, &param);
			...
			enum pcfg_type type, void *param)
{
	...
	case PCFG_BCN_DRP_ALL :
		...
		break;
	case PCFG_MBSSID_NUM :
		ptype = MAC_AX_PCFG_MBSSID_NUM;
		break;
	case PCFG_BCN_HOLD_TIME :
		ptype = MAC_AX_PCFG_BCN_HOLD_TIME;
		break;
	default :
...
#if defined(PHL_FEATURE_AP) && defined(CONFIG_PHL_VMALLOC_SUPPORT)
	if (fw_info->rom_buff) {
		_os_mem_free(hal_com->drv_priv, fw_info->rom_buff, RTW_MAX_FW_SIZE);
		fw_info->rom_buff = NULL;
	}
	if (fw_info->ram_buff) {
		_os_mem_free(hal_com->drv_priv, fw_info->ram_buff, RTW_MAX_FW_SIZE);
		fw_info->ram_buff = NULL;
	}
#endif /* PHL_FEATURE_AP && CONFIG_PHL_VMALLOC_SUPPORT */
...
hal_mac_ax_send_beacon(struct hal_info_t *hal, struct rtw_bcn_entry *bcn_entry)
{
	...
	if (bcn_cmn->ofst_bs_chg)
		info.bc_chg_ofst = (u16)((bcn_cmn->ofst_bs_chg << 1) | BIT(0));
#endif
...
/* FW Sounding Command */
/* 1. NDPA Content */
void _hal_max_ax_snd_cmd_ndpa(struct hal_info_t *hal_info,
			      ...
			      bool he, u8 sta_nr, bool mode)
{
	...
	struct phl_info_t *phl_info = (struct phl_info_t *)hal_info->phl_com->phl_priv;
	...
	if (he) {
		if(mode==1){
			for (i = 0; (i < 2)&&(i < HAL_MAX_HE_SND_STA_NUM); i++) {  //charlie sta_nr
				...
				if (hal_info->phl_com->is_HE_4_35_1) {
					mac_ndpa->he_para.sta_info[i].cb = 0;
				} else
#endif
#ifdef WIFI_LOGO_HE_4_35_3
				if (hal_info->phl_com->is_HE_4_35_3) {
					mac_ndpa->he_para.sta_info[i].cb = 0;
				} else
#endif
				{
		       ...
		       struct hal_ax_fwcmd_snd *hal_cmd)
{
	...
	/* 2. NDPA Content */
	_hal_max_ax_snd_cmd_ndpa(hal_info, &snd_cmd.pndpa, &hal_cmd->ndpa,
...
void hal_mac_process_bcn_stats(void *hal, struct mac_ax_bcn_cnt bcn_cnt)
{
	...
	if (bcn_entry) {
		...
		if (!(phl_is_mp_mode(phl_com)) && (bcn_cnt.ok_cnt == 0)) {
			if (bcn_cnt.ok_cnt < hal_info->hal_com->dbg_bcn_tx){
#ifdef CONFIG_RTW_DEBUG_BCN_STATS
...
#endif
#ifdef CONFIG_RTW_AP_BS_DECT
	case MSG_EVT_BCCHG_COUNTDOWN_ZERO:
		{
			struct rtw_bcc_cntdown_rpt *bcc_rpt = NULL;

			if (c2h->content_len == 1) {
				len = sizeof(struct rtw_bcc_cntdown_rpt);
				bcc_rpt = _os_kmem_alloc(drv, len);
				if (bcc_rpt == NULL)
					break;

				bcc_rpt->port = c2h->content[0] & 0x7f;
				bcc_rpt->band = (c2h->content[0] >> 7) & 0x1;

				// PHL_PRINT("content: %x, port: %u, band: %u\n",
				//		c2h->content[0], bcc_rpt->port, bcc_rpt->band);
...
enum rtw_hal_status
rtw_hal_mac_set_edcca_mode_en_val(struct rtw_hal_com_t *hal_com, u8 band, u8 en_val)
{
	struct hal_info_t *hal_info = hal_com->hal_priv;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	struct rtw_tpu_info *tpu = &hal_com->band[band].rtw_tpu_i;
	u32 mac_status = RTW_HAL_STATUS_FAILURE;
	struct mac_ax_resp_chk_cca param;

	mac_status = mac->ops->sifs_chk_cca_en(mac, band, en_val);
	if(mac_status != MACSUCCESS)
		return RTW_HAL_STATUS_FAILURE;

	param.band = band;
	param.resp_ack_chk_cca_en = en_val;
	mac_status = mac_set_hw_value(mac, MAC_AX_HW_SET_RESP_ACK_CHK_CCA, (void *)&param);
	if(mac_status != MACSUCCESS)
		return RTW_HAL_STATUS_FAILURE;
...
#ifdef CONFIG_PHL_FREERUNCNT_DBG
enum rtw_hal_status rtw_hal_mac_get_freeruncnt(struct hal_info_t *hal, u8 band,
					u32 *freeruncnt_h, u32 *freeruncnt_l)
{
	struct mac_ax_adapter *mac = hal_to_mac(hal);
	struct mac_ax_ops *hal_mac_ops = mac->ops;
	struct mac_ax_freerun freerun_cnt;

	freerun_cnt.band = band;
	if (hal_mac_ops->get_hw_value(mac, MAC_AX_HW_GET_FREERUN_CNT, &freerun_cnt))
		return RTW_HAL_STATUS_FAILURE;
	*freeruncnt_l = freerun_cnt.freerun_l;
	*freeruncnt_h = freerun_cnt.freerun_h;
...
rtw_hal_mac_disable_rx_amsdu_cut(struct hal_info_t *hal_info)
{
	...
	mret = mac->ops->enable_cut_hwamsdu(mac, 0, 14, 1600, MAC_AX_BYTE_ALIGNED_4);
	if (mret == MACSUCCESS)
		mret = mac->ops->hdr_conv_rx_en_driv_info_hdr(mac, &drv_info_cfg);
	if (mret == MACSUCCESS)
		mret = mac->ops->hdr_conv_rx_en(mac, &rx_hdr_conv_cfg);
	...
	PHL_PRINT("Disabled RX A-MSDU cut. (%u)\n", mret);
...
enum rtw_hal_status rtw_hal_mac_trx_bd_reallocate(struct hal_info_t *hal_info, void *txbd_buf, void *rxbd_buf)
{
	enum rtw_hal_status ret;
	struct mac_ax_adapter *mac = (struct mac_ax_adapter *)hal_info->mac;
	struct mac_ax_txbd_realloc_info txbd_realloc_info;
	struct mac_ax_txdma_ch_map txch_map;
	struct mac_ax_rxbd_realloc_info rxbd_realloc_info;
	struct mac_ax_rxdma_ch_map rxch_map;

	/* reallocate rxbd */
	rxch_map.rxq = MAC_AX_PCIE_ENABLE;
	rxch_map.rpq = MAC_AX_PCIE_ENABLE;
	rxbd_realloc_info.rxbd_buf = rxbd_buf;
	rxbd_realloc_info.rxch_map = &rxch_map;

	ret = mac->ops->intf_ops->rx_bd_reallocate(mac, &rxbd_realloc_info, _TRUE);
	if (ret != MACSUCCESS) {
		PHL_ERR("[%s]: pcie_rx_bd_reallocate() fails\n", __func__);
		return RTW_HAL_STATUS_FAILURE;
	}
	...
	txbd_realloc_info.txbd_buf = txbd_buf;
	txbd_realloc_info.txch_map = &txch_map;

	ret = mac->ops->intf_ops->tx_bd_reallocate(mac, &txbd_realloc_info, _FALSE);
	if (ret != MACSUCCESS) {
		PHL_ERR("[%s]: pcie_tx_bd_reallocate() fails\n", __func__);
		return RTW_HAL_STATUS_FAILURE;
	}

...
void _hal_mac_filter_crc_cnt(struct hal_info_t *hal_info, struct rtw_phl_rx_pkt *phl_rx){
	...
	if (setflt) {
		if (0 == _os_mem_cmp(hal_info->hal_com->drv_priv, meta->ta, trx_stat->mp_fltr_ta, 6)) {
			...
			PHL_INFO("Match, ta=%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx, OK:%d CRC error:%d\n",
					meta->ta[0], meta->ta[1], meta->ta[2],
					meta->ta[3], meta->ta[4], meta->ta[5],
					trx_stat->mp_fltr_rx_ok_cnt, trx_stat->mp_fltr_rx_err_cnt);
		} else {
			PHL_INFO("Mis-match, ta=%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx\n",
					meta->ta[0], meta->ta[1], meta->ta[2],
					meta->ta[3], meta->ta[4], meta->ta[5]);
		}
	}
...
void _hal_mac_filter_rssi(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
						struct rtw_phl_rx_pkt *phl_rx)
{
	...
	if (setflt) {
		...
		if (sts_ent->valid && 0 == _os_mem_cmp(hal_info->hal_com->drv_priv, sts_ent->src_mac_addr, trx_stat->mp_fltr_ta, 6)) {

			for (i = RF_PATH_A; i < RTW_PHL_MAX_RF_PATH; i++)
				trx_stat->rssi_raw_path[i] = sts_ent->phy_info.rssi_raw_path[i];

			PHL_INFO("Match, src_mac_addr=%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx, ppdu_cnt=%d, rssi_raw_path=(%d , %d , %d , %d)\n",
						sts_ent->src_mac_addr[0], sts_ent->src_mac_addr[1], sts_ent->src_mac_addr[2],
						sts_ent->src_mac_addr[3], sts_ent->src_mac_addr[4], sts_ent->src_mac_addr[5],
						meta->ppdu_cnt,
						trx_stat->rssi_raw_path[RF_PATH_A], trx_stat->rssi_raw_path[RF_PATH_B],
						trx_stat->rssi_raw_path[RF_PATH_C], trx_stat->rssi_raw_path[RF_PATH_D]);
		}
	}
}
                         ...
                         struct rtw_phl_rx_pkt *rxpkt)
{
	...
	if (phl_com->drv_mode != RTW_DRV_MODE_NORMAL){
		if (hstatus == RTW_HAL_STATUS_SUCCESS && mdata->rpkt_type == RTW_RX_TYPE_WIFI)
			_hal_mac_filter_crc_cnt(hal_info, rxpkt);
		else if (hstatus == RTW_HAL_STATUS_SUCCESS && mdata->rpkt_type == RTW_RX_TYPE_PPDU_STATUS)
			_hal_mac_filter_rssi(phl_com, hal_info, rxpkt);
	}
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	if (rxbuf->ofld_bd_sz &&
		(rxbuf->ofld_bd_sz == (_ALIGN(rxbuf->ofld_pld_sz, 4) + sizeof(struct rx_ppdu_append)))) {
		app = (struct rx_ppdu_append *)(rxbuf->vir_addr + _ALIGN(rxbuf->ofld_pld_sz, 4));
		if (app->max_user_id < MAX_PPDU_USER_CNT) {
			rtw_phl_update_stainfo_by_macid(phl_com->phl_priv,
						(app->max_user_id + 1), meta, app->user);
			ppdu_info = &phl_com->ppdu_sts_info;
			band = (meta->bb_sel > 0) ? HW_BAND_1 : HW_BAND_0;
			ppdu_info->cur_rx_ppdu_cnt[band] = app->ppdu_cnt;
			sts_ent = &(ppdu_info->sts_ent[band][meta->ppdu_cnt]);
			sts_ent->rx_rate = app->datarate;
			sts_ent->ppdu_type = app->ppdu_type;
			sts_ent->valid = false;
		}
	}
#endif /* CONFIG_RTL_WIFI_OFFLOAD */

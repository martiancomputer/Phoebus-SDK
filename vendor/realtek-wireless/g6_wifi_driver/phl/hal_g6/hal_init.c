...
void rtw_hal_enable_interrupt(struct rtw_phl_com_t *phl_com, void *h)
{
	...
	printk("mask: 0x%08X, 0x%08X, 0x%08X, 0x%08X, 0x%08X\n",
				...
				bool sort_en, u32 timeout)
{
	...
	cfg.count_en = (count_en & 0x1);
	cfg.mode = (mode & 0x1);
	cfg.gt3_en = (gt3_en & 0x1);
	cfg.sort_en = (sort_en & 0x1);
	cfg.timeout = (timeout & 0x0FFFFFFF);
...
static void _hal_send_hal_init_hub_msg(struct rtw_phl_com_t *phl_com, u8 init_ok)
{
	...
	u16 evt_id = (init_ok) ? MSG_EVT_HAL_START_OK : MSG_EVT_HAL_START_FAIL;
...
enum rtw_hal_status rtw_hal_start(struct rtw_phl_com_t *phl_com, void *hal)
{
	...
	if (RTW_HAL_STATUS_SUCCESS != rtw_hal_snd_proc_pre_cfg(hal_info)) {
		DBGP("warning!! rtw_hal_snd_proc_pre_cfg() fail\n");
	}

	/* Clear TX pause flags to ensure TX pause status is clean */
	for (band_idx = 0; band_idx < ARRAY_SIZE(hal_com->band); band_idx++) {
	...
	if(1)
	{
		...
		result = rtw_hal_mac_get_pwr_reg(hal_com, HW_PHY_0, 0xd200, &tmp);
		if (result) {
		...
		rtw_hal_mac_write_pwr_limit_en(hal_com, HW_PHY_0);
		...
		result = rtw_hal_mac_set_pwr_reg(hal_com, HW_PHY_0, 0xd200, tmp);
		if (result) {
...
enum rtw_hal_status rtw_hal_power_switch(struct rtw_phl_com_t *phl_com, void *hal, u8 on_off)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;

	hstatus = rtw_hal_mac_power_switch(phl_com,
			 hal_info,
			 on_off);
...
u8 _check_is_max_mbssid(void *hal, struct rtw_wifi_role_t *wrole)
{
	u8 i;
	u8 max_mbssid = 0;
	for (i = 0 ; i < MAX_WIFI_ROLE_NUMBER; i++)
	{
		if((wrole->phl_com->wifi_roles[i].type == PHL_RTYPE_VAP ||
		    wrole->phl_com->wifi_roles[i].type == PHL_RTYPE_AP) &&
			max_mbssid < wrole->phl_com->wifi_roles[i].hw_mbssid)
			max_mbssid = wrole->phl_com->wifi_roles[i].hw_mbssid;
	}
	if (max_mbssid == wrole->hw_mbssid)
		return _TRUE;
	else
		return _FALSE;
}

u32 _hal_get_max_bcnint(void *hal, struct rtw_wifi_role_t *wrole)
{
	u8 i;
	u32 max_bcnint = 100;
	for (i = 0 ; i < MAX_WIFI_ROLE_NUMBER; i++)
	{
		if((wrole->phl_com->wifi_roles[i].type == PHL_RTYPE_VAP ||
		    wrole->phl_com->wifi_roles[i].type == PHL_RTYPE_AP) &&
			max_bcnint < wrole->phl_com->wifi_roles[i].bcn_cmn.bcn_interval)
			max_bcnint = wrole->phl_com->wifi_roles[i].bcn_cmn.bcn_interval;
	}
...
rtw_hal_role_cfg(void *hal, struct rtw_wifi_role_t *wrole)
{
	...
	if (wrole->type == PHL_RTYPE_VAP) {
		u32	param = 1;
		/* Configure port for M-BSSID */
		if (_check_is_max_mbssid(hal, wrole)) {
			param = _hal_get_max_bcnint(hal, wrole);
			hal_status = rtw_hal_mac_port_cfg(hal_info, wrole,
							PCFG_MBSSID_NUM, &param);
		}
		param = 1;
		hal_status = rtw_hal_mac_port_cfg(hal_info, wrole,
		                                  PCFG_MBSSID_EN, &param);

...
void _hal_fill_wp_addr_info_offload_8852ce(u8 *addr_info, u16 length, u32 phy_offset, bool lastwp)
{
	SET_ADDR_INFO_LEN(addr_info, length);
	SET_ADDR_INFO_ADDR_LOW_LSB(addr_info, phy_offset & 0xFFFF);
	SET_ADDR_INFO_ADDR_LOW_MSB(addr_info, (phy_offset & 0xFFFF0000)>>16);
	SET_ADDR_INFO_ADDR_HIGH_SEL(addr_info, 0x0);
						...
						struct rtw_phl_pkt_req *req, u32 *wd_len)
{
	...
	u8 dma_ch;
	u16 agg_num = mdata->max_agg_num;
	u32 *wd_words;
	u32 w0, w1, w2, w3, w8, w9, w10, w12;
	...
	if (req->wd_seq_offset == 0) {
		...
		w9 = le32_to_cpu(wd_words[9])
				& ~(AX_TXD_DATA_RTY_LOWEST_RATE_MSK << AX_TXD_DATA_RTY_LOWEST_RATE_SH);
		w9 = le32_to_cpu(wd_words[9])
				& ~(AX_TXD_MAX_AGG_NUM_MSK << AX_TXD_MAX_AGG_NUM_SH);
		w10 = le32_to_cpu(wd_words[10])
        		& ~(AX_TXD_SEC_CAM_IDX_MSK << AX_TXD_SEC_CAM_IDX_SH);
		...
		if (agg_num > 0)
			agg_num-=1;
		w9 |= (agg_num
				& AX_TXD_MAX_AGG_NUM_MSK)
				<< AX_TXD_MAX_AGG_NUM_SH;
		wd_words[9] = cpu_to_le32(w9);

		w10 |= (mdata->sec_cam_idx & AX_TXD_SEC_CAM_IDX_MSK) << AX_TXD_SEC_CAM_IDX_SH;
...
#ifdef CONFIG_ETHER_PKT_AGG
static void hal_pe_ofld_dbg_get_next_pkt_list_indx_8852ce(struct hal_info_t *hal, struct rtw_pkt_buf_list *pkt_list, u8 pkt_list_num, u8 *pkt_list_idx, u32 skip, u32 *skip_frag_len, u32 *next_skip_frag_len)
{
	if (skip >= *next_skip_frag_len) {
		(*pkt_list_idx)++;
		*skip_frag_len = *next_skip_frag_len;
		if ((*pkt_list_idx) == pkt_list_num) {
			RTW_DBG("%s: no more pkt list, skip: %u, skip_frag_len: %u, next_skip_frag_len: %u\n", __func__, skip, *skip_frag_len, *next_skip_frag_len);
			return; /* no more pkt list */
		}
		*next_skip_frag_len = *skip_frag_len + pkt_list[*pkt_list_idx].length;
		RTW_DBG("%s: pkt_list[%u].length: %u, skip: %u, skip_frag_len: %u, next_skip_frag_len: %u\n", __func__, (*pkt_list_idx), pkt_list[*pkt_list_idx].length, skip, *skip_frag_len, *next_skip_frag_len);
	}
}

static int hal_pe_ofld_dbg_count_amsdu_subframes_8852ce(struct hal_info_t *hal, struct rtw_pkt_buf_list *pkt_list, u8 pkt_list_num)
{
	int count = 0;
	u8 pkt_list_idx = 0;
	u32 skip = 0;
	...
	pkt_list_idx = 0;
	hal_pe_ofld_dbg_get_next_pkt_list_indx_8852ce(hal, pkt_list, pkt_list_num, &pkt_list_idx, skip, &skip_frag_len, &next_skip_frag_len);
	while (pkt_list_idx < pkt_list_num) {
		/* skip da field */
		skip += ETH_ALEN;
		hal_pe_ofld_dbg_get_next_pkt_list_indx_8852ce(hal, pkt_list, pkt_list_num, &pkt_list_idx, skip, &skip_frag_len, &next_skip_frag_len);
		/* skip sa field */
		...
		len_field_high = pkt_list[pkt_list_idx].vir_addr + (skip - skip_frag_len);
		RTW_DBG("%s: len_field_high: %u\n", __func__, (u32) *len_field_high);
		skip += 1;
		hal_pe_ofld_dbg_get_next_pkt_list_indx_8852ce(hal, pkt_list, pkt_list_num, &pkt_list_idx, skip, &skip_frag_len, &next_skip_frag_len);
		len_field_low = pkt_list[pkt_list_idx].vir_addr + (skip - skip_frag_len);
...
void hal_pe_ofld_dbg_pkt_cnt_8852ce(struct hal_info_t *hal, struct rtw_pkt_buf_list *pkt_list, u8 pkt_list_num)
{
	...
	subframes = hal_pe_ofld_dbg_count_amsdu_subframes_8852ce(hal, pkt_list, pkt_list_num);
	phl_com->phl_stats.pe_ofld_amsdus++;
	phl_com->phl_stats.pe_ofld_amsdu_subframes += subframes;
}

static u8 hal_update_wp_addr_info_agg_8852ce(struct hal_info_t *hal, u8 *wd_page, u32 addr_info_ofst, struct rtw_pkt_buf_list *pkt_list, u8 pkt_list_num)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	struct bus_hw_cap_t *bus_hw_cap = &hal_com->bus_hw_cap;
	u16 msdu_len, len, msdu_ls;
	u32 phy_offset = 0;
	u8 wp_num = 0, i;
	...
	hal_pe_ofld_dbg_pkt_cnt_8852ce(hal, pkt_list, pkt_list_num);

	for (i=0 ; i<pkt_list_num; i++) {
		msdu_len = pkt_list[i].length;
		phy_offset = pkt_list[i].phy_addr_l;
		...
		do {
			if(msdu_len > MAX_SUB_PKT_SIZE_8852CE) {
				...
				len = msdu_len;
				msdu_ls = ((i+1)==pkt_list_num)?1:0;
				msdu_len = 0;
			}

			_hal_fill_wp_addr_info_offload_8852ce(wd_page + addr_info_ofst, len, phy_offset, msdu_ls);
...
#ifdef CONFIG_ETHER_PKT_AGG
		if (tx_req && tx_req->eth_agg_pkt == 1) {
			buf_len += hal_update_wp_addr_info_agg_8852ce(hal, req->wd_page, addr_info_ofst, pkt_list, tx_req->mdata.addr_info_num);
		} else
#endif
		{
#ifdef CONFIG_ETHER_PKT_AGG
			if (phl_com->dbg_cfg.pe_ofld_cfg.is_pe_ofld_dbg && tx_req && tx_req->tc_non_agg_pkt > 0) {
				phl_com->phl_stats.pe_ofld_non_amsdus += tx_req->tc_non_agg_pkt;
			}
#endif /* CONFIG_ETHER_PKT_AGG */
			for (i = 0; i < tx_req->mdata.addr_info_num; i++) {
				if (pkt_list[i].length > MAX_SUB_PKT_SIZE_8852CE) {
					...
					PHL_TRACE(COMP_PHL_XMIT, _PHL_ERR_,
						"packet type = %d, macid=%u, pktlen=%u, addr_info_num=%u, mc=%u, bc=%u\n",
						tx_req->mdata.type,
						tx_req->mdata.macid,
						tx_req->mdata.pktlen,
						tx_req->mdata.addr_info_num,
						tx_req->mdata.mc,
						tx_req->mdata.bc);
					debug_dump_data_header(pkt_list[i].vir_addr, pkt_list[i].length, "dump wp");
...
/**
 * the function update PE txbd
 * @hal: see struct hal_info_t
 * @txbd: the target txbd ring buffer going to update, see struct tx_base_desc
 * @wd_page: the wd page going to be filled in txbd, see struct rtw_wd_page
 */
static enum rtw_hal_status
hal_update_pe_txbd_8852ce(struct hal_info_t *hal,
		u8 *txbd, struct rtw_wd_page *wd_page)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;

	SET_TXBUFFER_DESC_LEN(txbd, wd_page->buf_len);
	SET_TXBUFFER_DESC_LS(txbd, wd_page->ls);
	SET_TXBUFFER_DESC_ADD_LOW(txbd,
		wd_page->phy_addr_l);
	SET_TXBUFFER_DESC_ADD_HIGH(txbd,
		wd_page->phy_addr_h);
...
#endif
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	ops.update_pe_txbd = hal_update_pe_txbd_8852ce;
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
}

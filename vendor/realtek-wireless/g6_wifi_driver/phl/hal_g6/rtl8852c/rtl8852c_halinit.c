	...
	char *ic_name, enum rtw_cv cv)
{
	...
	if (fw_path == RTW_FW_PATH_ROOTFS) {
		switch (cv) {
			...
			case CCV:
				cut_postfix = FW_FILE_CUT_C_POSTFIX;
				break;
			#endif
			default:
				return RTW_HAL_STATUS_FAILURE;
	...
	_os_snprintf(path, MAX_PATH_LEN, "%s%s%s%s%s%s%s", hal_phy_folder,
			ic_name, _os_path_sep, "RTL8852Cfw", filename_postfix,
			cut_postfix, ".bin");
...
static u32 _hal_cfg_extnal_fw_8852c(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
				     ...
				     enum rtw_fw_type fw_type)
{
	...
	if (fw_cap->fw_src == RTW_FW_SRC_EXTNAL) {
		...
		if (RTW_HAL_STATUS_SUCCESS != _hal_cfg_extnal_fw_8852c(phl_com, hal, fw_type,
			fw_info, ic_name))
			goto init_fw_fail;

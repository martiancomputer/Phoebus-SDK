...
static u32 _hal_read_fw_8852d(enum rtw_fw_type fw_type,
	u8 *tgt_buf, u32 *tgt_buf_size, enum rtw_fw_ext_path fw_path,
	char *ic_name, enum rtw_cv cv)
{
	char *hal_phy_folder = FW_FILE_CONFIG_PATH;
	char *filename_postfix = "";
	char *cut_postfix = "";
	...
	if (fw_path == RTW_FW_PATH_ROOTFS)
		hal_phy_folder = FW_FILE_ROOTFS_PATH;
	...
	if (fw_path == RTW_FW_PATH_ROOTFS) {
		switch (cv) {
			...
			case CAV:
				cut_postfix = FW_FILE_CUT_A_POSTFIX;
				break;
			#endif
			#ifdef MAC_FW_8852D_U2
			case CBV:
				cut_postfix = FW_FILE_CUT_B_POSTFIX;
				break;
			#endif
			default:
				return RTW_HAL_STATUS_FAILURE;
	...
	_os_snprintf(path, MAX_PATH_LEN, "%s%s%s%s%s%s%s", hal_phy_folder,
			ic_name, _os_path_sep, "RTL8852Dfw", filename_postfix,
			cut_postfix, ".bin");
...
static u32 _hal_cfg_extnal_fw_8852d(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
	enum rtw_fw_type fw_type, struct rtw_fw_info_t *fw_info, char *ic_name)
{
	...
	case RTW_FW_NIC:
		_hal_read_fw_8852d(RTW_FW_NIC, fw_info->buf,
			&fw_info->buf_size, fw_cap->fw_path,
			ic_name, hal_com->cv);
		/* preload wowlan fw */
		_hal_read_fw_8852d(RTW_FW_WOWLAN, fw_info->wow_buf,
			&fw_info->wow_buf_size, fw_cap->fw_path,
			ic_name, hal_com->cv);
		/* target is still NIC */
		tgt_buf = fw_info->buf;
	...
	if (tgt_buf && (0 != tgt_buf_size)) {
		...
		return _hal_read_fw_8852d(fw_type, fw_info->ram_buff,
			&fw_info->ram_size, fw_cap->fw_path,
			ic_name, hal_com->cv);
	}
				     ...
				     enum rtw_fw_type fw_type)
{
	...
	if (fw_cap->dlrom_en) {
		if (fw_info->rom_buff == NULL) {
			fw_info->rom_buff = hal_mem_alloc(hal->hal_com, RTW_MAX_FW_SIZE);
			if (fw_info->rom_buff == NULL) {
				PHL_TRACE(COMP_PHL_DBG, _PHL_ERR_, "%s : Allocate rom_buff fail.\n", __func__);
				goto init_fw_fail;
			}
		}
	...
	if (fw_cap->fw_src == RTW_FW_SRC_EXTNAL) {
		...
		if (RTW_HAL_STATUS_SUCCESS != _hal_cfg_extnal_fw_8852d(phl_com, hal, fw_type,
			fw_info, ic_name))
			goto init_fw_fail;
				   ...
				   struct hal_init_info_t *init_info)
{
	...
	/* antenna diversity: gpio control */
	if (phl_com->dev_cap.antdiv_sup == true)
		rtw_hal_mac_set_gpio_func(hal->hal_com, RTW_MAC_GPIO_WL_RFE_CTRL, 11);

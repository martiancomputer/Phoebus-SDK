 ...
 *
 *****************************************************************************/
#define _HAL_TRX_8852D_C_
#include "../hal_headers.h"
#include "rtl8852d_hal.h"

#if defined(RECORD_STA_CSI_INFO)
struct rtw_r_meta_data *cached_meta = NULL;
#endif

static void _hal_trx_8852d_dump_rxcnt(struct hal_ppdu_sts *ppdu_sts)
{
...
static void _hal_dump_rxdesc(u8 *buf, struct rtw_r_meta_data *mdata)
{
 ...
 hal_handle_chaninfo_8852d(struct hal_info_t *hal)
 {
...
#define WL_SEQ_MASK	0xfff
 ...
 /**
 * SW Parsing Rx Desc - hal_parsing_rx_wd_8852d
 * description:
 ...
 */
static enum rtw_hal_status
_hal_parsing_rx_wd_8852d(struct hal_info_t *hal,
#ifdef PHL_RXSC_AMPDU
	...
	u8 *desc, struct rtw_r_meta_data *mdata)
{
	...
	mdata->phy_rpt_en = GET_RX_AX_DESC_PHY_RPT_EN_8852D(desc);

	if ((mdata->rpkt_type == RX_8852D_DESC_PKT_T_WIFI) ||
	    (mdata->rpkt_type == RX_8852D_DESC_PKT_T_PPDU_STATUS)) {
	...
	if (RXSC_INVALID_MACID != phl_com->rxsc_entry.cached_rx_macid &&
		RX_8852D_DESC_PKT_T_WIFI == mdata->rpkt_type) {

		mdata->ampdu = GET_RX_AX_DESC_AMPDU_8852D(desc);

		if (mdata->long_rxd == 1) {
			mdata->tid = GET_RX_AX_DESC_TID_8852D(desc);
			mdata->macid = GET_RX_AX_DESC_MACID_8852D(desc);
			mdata->seq = GET_RX_AX_DESC_SEQ_8852D(desc);

			mdata->mc = GET_RX_AX_DESC_MC_8852D(desc);
			mdata->bc = GET_RX_AX_DESC_BC_8852D(desc);
			mdata->htc = GET_RX_AX_DESC_HTC_8852D(desc);
			mdata->frame_type = GET_RX_AX_DESC_TYPE_8852D(desc);
		}
		...
		if (phl_com->rxsc_entry.cached_rx_macid == mdata->macid &&
			...
			mdata->qos = GET_RX_AX_DESC_QOS_8852D(desc);
			mdata->addr_cam_vld = GET_RX_AX_DESC_CAM_VLD_8852D(desc);
		...
		mdata->get_ch_info = GET_RX_AX_DESC_CH_INFO_8852D(desc);
		mdata->rx_statistics = GET_RX_AX_DESC_STATISTICS_8852D(desc);

		mdata->pattern_idx = GET_RX_AX_DESC_PATTERN_IDX_8852D(desc);
		mdata->target_idc = GET_RX_AX_DESC_TARGET_IDC_8852D(desc);
		mdata->chksum_ofld_en = GET_RX_AX_DESC_CHKSUM_OFFLOAD_8852D(desc);
		mdata->with_llc = GET_RX_AX_DESC_WITH_LLC_8852D(desc);

		if (mdata->long_rxd == 1) {
			mdata->frame_type = GET_RX_AX_DESC_TYPE_8852D(desc);
			mdata->mc = GET_RX_AX_DESC_MC_8852D(desc);
			mdata->bc = GET_RX_AX_DESC_BC_8852D(desc);
			mdata->more_data = GET_RX_AX_DESC_MD_8852D(desc);
			mdata->more_frag = GET_RX_AX_DESC_MF_8852D(desc);
			mdata->pwr_bit = GET_RX_AX_DESC_PWR_8852D(desc);
			mdata->qos = GET_RX_AX_DESC_QOS_8852D(desc);

			mdata->tid = GET_RX_AX_DESC_TID_8852D(desc);
			mdata->eosp = GET_RX_AX_DESC_EOSP_8852D(desc);
			mdata->htc = GET_RX_AX_DESC_HTC_8852D(desc);
			mdata->q_null = GET_RX_AX_DESC_QNULL_8852D(desc);

			mdata->seq = GET_RX_AX_DESC_SEQ_8852D(desc);
			mdata->frag_num = GET_RX_AX_DESC_FRAG_8852D(desc);

			mdata->sec_cam_idx = GET_RX_AX_DESC_CAM_IDX_8852D(desc);
			mdata->addr_cam = GET_RX_AX_DESC_ADDR_CAM_8852D(desc);
			...
			mdata->rx_pl_id = GET_RX_AX_DESC_PL_ID_8852D(desc);

			mdata->addr_fwd_en = GET_RX_AX_DESC_FWD_EN_8852D(desc);
			mdata->rx_pl_match = GET_RX_AX_DESC_PL_MATCH_8852D(desc);

			_os_mem_cpy(hal_com->drv_priv,
				(void*)&mdata->mac_addr, (void*)(desc + 24), MAC_ALEN);
			mdata->sec_type = GET_RX_AX_DESC_SEC_TYPE_8852D(desc);
		}
	}
...
enum rtw_hal_status
hal_parsing_rx_wd_8852d(struct rtw_phl_com_t *phl_com,
				...
				struct rtw_r_meta_data *mdata)
{
	...
	do {
		...
		if (!halmac_rx) {
#ifdef PHL_RXSC_AMPDU
			hstatus = _hal_parsing_rx_wd_8852d(hal, phl_com, desc, mdata);
#else
			hstatus = _hal_parsing_rx_wd_8852d(hal, desc, mdata);
#endif
		} else {
		...
		desc_l = mdata->long_rxd ? RX_DESC_L_SIZE_8852D :
						RX_DESC_S_SIZE_8852D;
		...
		shift = (u8)((mdata->shift * 2)
			     + (mdata->drv_info_size * RX_DESC_DRV_INFO_UNIT_8852D)
			     + desc_l);

		if ((1 == mdata->mac_info_vld) &&
			(RX_8852D_DESC_PKT_T_PPDU_STATUS != mdata->rpkt_type))
			*pkt = desc + shift + RX_PPDU_MAC_INFO_SIZE_8852D;
		else
			*pkt = desc + shift;
...
static void
_hal_rx_wlanhdr_check_8852d(void *drvpriv, void *hdr, struct rtw_r_meta_data *mdata)
{
	/*Check Retry BIT*/
	u8 retry = 0;
	u16 seq_num = 0;
...
static void
_hal_rx_sts_8852d(struct hal_info_t *hal, struct rtw_r_meta_data *meta)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static void
_hal_rx_ampdu_cnt_8852d(struct hal_info_t *hal, struct rtw_r_meta_data *mdata)
{
...
void
_hal_rx_ppdu_sts_normal_data_8852d(struct rtw_phl_com_t *phl_com,
				   struct hal_info_t *hal,
				   void *hdr,
...
void
_hal_rx_ppdu_sts_8852d(struct rtw_phl_com_t *phl_com,
			    struct rtw_r_meta_data *meta,
...
void
_hal_parsing_rx_drvinfo_8852d(struct rtw_phl_com_t *phl_com,
			      struct hal_info_t *hal,
			      ...
			      u8 *buf)
{
	...
	do {
		...
		drv_info = desc + (r->mdata.long_rxd ? RX_DESC_L_SIZE_8852D : RX_DESC_S_SIZE_8852D);

		phl_rx->r.phy_info.signal_strength =
				rtw_hal_bb_drv_info_dbm2rssi(
							hal,
							GET_RX_AX_DRV_INFO_RX_PWR_DBM_8852D(drv_info),
							r->mdata.bw);
		phl_rx->r.phy_info.ch_idx =  GET_RX_AX_DRV_INFO_CENTRAL_CH_8852D(drv_info);
...
#endif
		phl_rx->r.phy_info.rxsc =  GET_RX_AX_DRV_INFO_RX_SC_8852D(drv_info);
		phl_rx->r.phy_info.is_drvinfo_vld = true;

		PHL_TRACE(COMP_PHL_PSTS, _PHL_DEBUG_,
			  "[8852DE][DrvInfo] DBM %d(signal_strength %d), RxSC %d, Central %d, BW 0x%x\n",
			  GET_RX_AX_DRV_INFO_RX_PWR_DBM_8852D(drv_info),
			  phl_rx->r.phy_info.signal_strength,
                           ...
                           struct rtw_phl_rx_pkt *phl_rx)
{
	...
	hstatus = hal_parsing_rx_wd_8852d(phl_com, hal, buf,
	                                  &frame, &frame_len,
	                                  &drv_info, mdata);

#ifdef TP_DEBUG
	_hal_rx_ampdu_cnt_8852d(hal, mdata);
#endif
	...
	case RX_8852D_DESC_PKT_T_WIFI :
	{
		...
		if (!mdata->hdr_conv) {
			...
			u32 hdr_offset = 0;
			u32 drv_info_sz = mdata->drv_info_size * RX_DESC_DRV_INFO_UNIT_8852D;
...
#endif
		{
			_hal_rx_wlanhdr_check_8852d(drv, wlan_hdr, mdata);
			_hal_rx_sts_8852d(hal, mdata);
		...
		if ((1 == mdata->phy_rpt_en) && (0 < mdata->drv_info_size)) {
			...
			_hal_parsing_rx_drvinfo_8852d(phl_com, hal, phl_rx, buf);
		}
	}
	break;
	case RX_8852D_DESC_PKT_T_TX_PD_RELEASE_HOST :
	{
		phl_rx->type = RTW_RX_TYPE_TX_WP_RELEASE_HOST;
	}
	break;
	case RX_8852D_DESC_PKT_T_PPDU_STATUS :
	{
		...
		PHL_TRACE(COMP_PHL_PSTS, _PHL_INFO_,
			  "==> RX_8852D_DESC_PKT_T_PPDU_STATUS :band %d ; ppdu_cnt  %d ; freerun_cnt %d \n",
		...
		if (ppdu_sts.rx_cnt_size != 0) {
			_hal_trx_8852d_dump_rxcnt(&ppdu_sts);
		}
		if (ppdu_sts.phy_st_size != 0) {
			if((mdata->ppdu_type == RX_8852D_DESC_PPDU_T_VHT_MU)||
				(mdata->ppdu_type == RX_8852D_DESC_PPDU_T_HE_MU)||
				(mdata->ppdu_type == RX_8852D_DESC_PPDU_T_HE_TB)) {
				is_su = 0;
			}
...
#if defined(RECORD_STA_CSI_INFO)
			if (cached_meta
				&& cached_meta->ppdu_cnt == mdata->ppdu_cnt) {
				cached_meta->rssi = phl_rx->r.phy_info.rssi;
				cached_meta->snr_avg = phl_rx->r.phy_info.snr_fd_avg;
				cached_meta->cfo = phl_rx->r.phy_info.cfo;
				cached_meta->ch_info_addr = phl_rx->r.phy_info.ch_info_addr;
				cached_meta->ch_info_len = phl_rx->r.phy_info.ch_info_len;
				cached_meta->rx_bw = phl_rx->r.phy_info.rx_bw;
				cached_meta->rxsc = phl_rx->r.phy_info.rxsc;
				for(i=0; i<RTW_PHL_MAX_RF_PATH; ++i){
					cached_meta->rssi_arr[i] = phl_rx->r.phy_info.rssi_path[i];
					cached_meta->snr_arr[i] = phl_rx->r.phy_info.snr_fd[i];
				}

				cached_meta = NULL; //avoid wrong record via ppdu_cnt
			}
#endif
		}
	}
	break;
	case RX_8852D_DESC_PKT_T_DFS_RPT :
	{
	...
	break;
	case RX_8852D_DESC_PKT_T_CHANNEL_INFO :
	{
	...
	break;
	case RX_8852D_DESC_PKT_T_F2P_TX_CMD_RPT :
	{
		/* DL MU Report ; UL OFDMA Trigger Report */
	}
	break;
	case RX_8852D_DESC_PKT_T_C2H :
	{
	...
	break;
	case RX_8852D_DESC_PKT_T_TX_RPT:
	{
                          ...
                          struct rtw_phl_rx_pkt *phl_rx)
{
	...
	ppdu_cnt = GET_RX_AX_DESC_PPDU_CNT_8852D(desc);
	if (ppdu_cnt != mdata->ppdu_cnt) {
		PHL_PRINT("R PPDU# mismatch %u/%u\n", ppdu_cnt, mdata->ppdu_cnt);
		return RTW_HAL_STATUS_FAILURE;
	}

	rpkt_type = GET_RX_AX_DESC_RPKT_TYPE_8852D(desc);
	if ((rpkt_type != RX_8852D_DESC_PKT_T_WIFI)) {
		PHL_PRINT("R type mismatch %u/%u\n", rpkt_type, RX_8852D_DESC_PKT_T_WIFI);
		return RTW_HAL_STATUS_FAILURE;
	}

	seq = GET_RX_AX_DESC_SEQ_8852D(desc);
	if (seq != mdata->seq){
		PHL_PRINT("R seq mismatch %u/%u\n", seq, mdata->seq);
		return RTW_HAL_STATUS_FAILURE;
	}

	amsdu = GET_RX_AX_DESC_AMSDU_8852D(desc);
	amsdu_cut = GET_RX_AX_DESC_AMSDU_CUT_8852D(desc);
	if (!amsdu || !amsdu_cut) {
	...
	shift = GET_RX_AX_DESC_SHIFT_8852D(desc);
	mac_info_vld = GET_RX_AX_DESC_MAC_INFO_VLD_8852D(desc);
	drv_info_size = GET_RX_AX_DESC_DRV_INFO_SIZE_8852D(desc);

	shift = (u8)(  (shift * 2)
		     + (drv_info_size * RX_DESC_DRV_INFO_UNIT_8852D)
		     + RX_DESC_L_SIZE_8852D);

	if (mac_info_vld)
		shift += RX_PPDU_MAC_INFO_SIZE_8852D;

	rxbuf->frame = desc + shift;
	rxbuf->frame_len = GET_RX_AX_DESC_PKT_LEN_8852D(desc);
	rxbuf->hdr = NULL;
	rxbuf->hdr_len = 0;

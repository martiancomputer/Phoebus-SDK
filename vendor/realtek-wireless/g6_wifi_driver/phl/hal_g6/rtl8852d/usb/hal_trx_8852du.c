 ...
 *
 *****************************************************************************/
#define _HAL_TRX_8852DU_C_
#include "../rtl8852d_hal.h"
#include "hal_trx_8852du.h"
...
/**
 * the function will initializing 8852du specific data and hw configuration
 */
enum rtw_hal_status hal_trx_init_8852du(struct hal_info_t *hal)
{
...
/**
 * the function will deinitializing 8852du specific data and hw configuration
 */
static void hal_trx_deinit_8852du(struct hal_info_t *hal)
{
}

static u8 hal_mapping_hw_tx_chnl_8852du(struct hal_info_t *hal,
			u16 macid, enum rtw_phl_ring_cat cat, u8 band)
{
	...
	if (0 == band) {
		...
		case RTW_PHL_RING_CAT_TID6:/*AC_VO*/
		case RTW_PHL_RING_CAT_TID7:
			dma_ch = ACH4_QUEUE_IDX_8852D;
			break;
		case RTW_PHL_RING_CAT_TID1:/*AC_BK*/
		case RTW_PHL_RING_CAT_TID2:
		case RTW_PHL_RING_CAT_TID4:/*AC_VI*/
		case RTW_PHL_RING_CAT_TID5:
			dma_ch = ACH6_QUEUE_IDX_8852D;
			break;
		case RTW_PHL_RING_CAT_MGNT:
		case RTW_PHL_RING_CAT_HIQ:
			dma_ch = MGQ_B1_QUEUE_IDX_8852D;
			break;
		default:
			dma_ch = ACH4_QUEUE_IDX_8852D;
			PHL_TRACE(COMP_PHL_DBG, _PHL_WARNING_, "[WARNING]unknown category (%d)\n",
...
static enum rtw_hal_status hal_query_info_8852du(struct hal_info_t *hal, u8 info_id, void *value)
{
...
static enum rtw_hal_status hal_pltfm_tx_8852du(void *hal,
...
u8 hal_get_bulkout_id_8852du(struct hal_info_t *hal, u8 dma_ch, u8 mode)
{
	return hal_mac_get_bulkout_id(hal, dma_ch, mode);
}

u8 hal_get_max_bulkout_wd_num_8852du(struct hal_info_t *hal)
{
	return hal_mac_usb_get_max_bulkout_wd_num(hal);
}

u32
hal_get_wd_len_8852du(struct hal_info_t *hal, struct rtw_xmit_req *tx_req)
{
 ...
 */
enum rtw_hal_status
hal_fill_wd_8852du(struct hal_info_t *hal,
...
enum rtw_hal_status
hal_usb_tx_agg_cfg_8852du(struct hal_info_t *hal, u8* wd_buf, u8 agg_num)
{
	return hal_mac_usb_tx_agg_cfg(hal, wd_buf, agg_num);
}

u8 hal_get_fwcmd_queue_idx_8852du(void)
{
	return FWCMD_QUEUE_IDX_8852D;
}

static void _hal_show_tx_failure_rsn_8852du(u8 txsts)
{
...
u16 hal_handle_rx_report_8852du(struct hal_info_t *hal, u8 *rp,
	...
	do {
		...
		*ac_queue = qsel_value % RTW_MAX_AC_QUEUE_NUM;
		...
		if (TX_STATUS_TX_DONE != *txsts) {
			...
			_hal_show_tx_failure_rsn_8852du(*txsts);
...
static struct hal_trx_ops ops= {
	.init = hal_trx_init_8852du,
	.deinit = hal_trx_deinit_8852du,
	.map_hw_tx_chnl = hal_mapping_hw_tx_chnl_8852du,
	.get_bulkout_id = hal_get_bulkout_id_8852du,
	.hal_get_wd_len = hal_get_wd_len_8852du,
	.hal_fill_wd = hal_fill_wd_8852du,
	.handle_rx_buffer = hal_handle_rx_buffer_8852d,
	.query_hal_info = hal_query_info_8852du,
	.usb_tx_agg_cfg = hal_usb_tx_agg_cfg_8852du,
	.usb_rx_agg_cfg = hal_usb_rx_agg_cfg,
	.get_fwcmd_queue_idx = hal_get_fwcmd_queue_idx_8852du,
	.get_max_bulkout_wd_num = hal_get_max_bulkout_wd_num_8852du,
	.handle_wp_rpt = hal_handle_rx_report_8852du,
};

u32 hal_hook_trx_ops_8852du(struct hal_info_t *hal_info)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;

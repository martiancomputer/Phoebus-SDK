...
enum rtw_hal_status _hal_set_pcicfg_8852de(struct hal_info_t *hal_info)
{
...
enum rtw_hal_status _hal_ltr_sw_init_state_8852de(struct hal_info_t *hal_info)
{
...
enum rtw_hal_status _hal_ltr_set_pcie_8852de(struct hal_info_t *hal_info)
{
...
static void _hal_tx_ch_config_8852de(struct hal_info_t *hal)
{
...
static void _hal_pre_init_8852de(struct rtw_phl_com_t *phl_com,
				 struct hal_info_t *hal_info,
				 struct hal_init_info_t *init_52de)
{
	struct mac_ax_trx_info *trx_info = &init_52de->trx_info;
	struct mac_ax_intf_info *intf_info = &init_52de->intf_info;
	...
	_hal_tx_ch_config_8852de(hal_info);
	...
	/* others */
	init_52de->ic_name = "rtl8852de";
}

void init_hal_spec_8852de(struct rtw_phl_com_t *phl_com,
					struct hal_info_t *hal)
{
	...
	init_hal_spec_8852d(phl_com, hal);
...
enum rtw_hal_status hal_get_efuse_8852de(struct rtw_phl_com_t *phl_com,
					 struct hal_info_t *hal_info)
{
	struct hal_init_info_t init_52de;
	_os_mem_set(hal_to_drvpriv(hal_info), &init_52de, 0, sizeof(init_52de));
	_hal_pre_init_8852de(phl_com, hal_info, &init_52de);

	return hal_get_efuse_8852d(phl_com, hal_info, &init_52de);
}


static u32 _hal_read32_8852de(struct rtw_hal_com_t *hal_com, u32 addr)
{
...
static u16 _hal_read16_8852de(struct rtw_hal_com_t *hal_com, u32 addr)
{
	u32 r_val = _hal_read32_8852de(hal_com, (addr & ~3));
	return (u16)(r_val >> (8 * (addr & 2)));
}

static u8 _hal_read8_8852de(struct rtw_hal_com_t *hal_com, u32 addr)
{
	u32 r_val = _hal_read32_8852de(hal_com, (addr & ~3));
	return (u8)(r_val >> (8 * (addr & 3)));
}

enum rtw_hal_status hal_init_8852de(struct rtw_phl_com_t *phl_com,
				    struct hal_info_t *hal_info)
{
...
void hal_deinit_8852de(struct rtw_phl_com_t *phl_com,
		       struct hal_info_t *hal_info)
{
	u8 txch_num = 0;
...
enum rtw_hal_status hal_start_8852de(struct rtw_phl_com_t *phl_com,
				    struct hal_info_t *hal_info)
{
	struct hal_init_info_t init_52de;
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	...
	hal_status = hal_start_8852d(phl_com, hal_info, &init_52de);
	if(RTW_HAL_STATUS_SUCCESS != hal_status) {

		PHL_ERR("hal_init_8852d: status = %u\n",hal_status);
		return hal_status;
	}

	hal_status = _hal_set_pcicfg_8852de(hal_info);
	if(RTW_HAL_STATUS_SUCCESS != hal_status) {
		PHL_ERR("_hal_set_pcicfg_8852de: status = %u\n",hal_status);
		return hal_status;
	}

	hal_status = _hal_ltr_set_pcie_8852de(hal_info);
	if(RTW_HAL_STATUS_SUCCESS != hal_status) {
		PHL_ERR("_hal_ltr_set_pcie_8852de: status = %u\n",hal_status);
		return hal_status;
	}

	hal_status = _hal_ltr_sw_init_state_8852de(hal_info);
	if(RTW_HAL_STATUS_SUCCESS != hal_status) {
		PHL_ERR("_hal_ltr_sw_init_state_8852de: status = %u\n",hal_status);
		return hal_status;
	}
...
enum rtw_hal_status hal_stop_8852de(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;

	hal_status = hal_stop_8852d(phl_com, hal);
	return hal_status;
}
...
enum rtw_hal_status
hal_wow_init_8852de(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
							struct rtw_phl_stainfo_t *sta)
{
	...
	*/
	...
	hal_status = hal_wow_init_8852d(phl_com, hal_info, sta, &init_52de);
...
enum rtw_hal_status
hal_wow_deinit_8852de(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info,
								struct rtw_phl_stainfo_t *sta)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_init_info_t init_52de;
	...
	*/
	...
	hal_status = hal_wow_deinit_8852d(phl_com, hal_info, sta, &init_52de);

	if (RTW_HAL_STATUS_SUCCESS != hal_status) {

		PHL_ERR("hal_wow_deinit_8852de: status = %u\n", hal_status);
		return hal_status;
	}

	if (RTW_HAL_STATUS_SUCCESS != _hal_set_pcicfg_8852de(hal_info))
		PHL_ERR("_hal_set_pcicfg_8852de: status = %u\n", hal_status);
...
u32 hal_hci_cfg_8852de(struct rtw_phl_com_t *phl_com,
...
static void _hal_dump_int_8852de(struct hal_info_t *hal, bool mask, bool imr, bool isr, const char *func)
{
...
#define HAL_DUMP_INT_8852DE(hal, mask, imr, isr) \
				_hal_dump_int_8852de(hal, mask, imr, isr, __func__)

void hal_init_default_value_8852de(struct hal_info_t *hal)
{
	init_default_value_8852d(hal);

	hal_init_int_default_value_8852de(hal, INT_SET_OPT_HAL_INIT);
}

void hal_init_int_default_value_8852de(struct hal_info_t *hal, enum rtw_hal_int_set_opt opt)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
void hal_enable_int_8852de(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	...
	HAL_DUMP_INT_8852DE(hal, false, true, true);
}

void hal_disable_int_8852de(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
 ...
 */
void hal_disable_int_isr_8852de(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
 ...
 * in function "hal_disable_int_isr_8852de".
 */
void hal_disable_int_rmn_8852de(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
void hal_clear_int_8852de(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
void hal_clear_mask_8852de(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
void hal_restore_int_8852de(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
bool hal_recognize_int_8852de(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	...
	/**
	 * We call hal_disable_int_isr_8852de only instead of hal_disable_int_8852de
	 ...
	 * in wlan mac.
	 */
	hal_disable_int_isr_8852de(hal);
...
#ifndef CONFIG_SYNC_INTERRUPT
	/* clear isr */
	hal_clear_int_8852de(hal);
	/* restore imr */
	hal_restore_int_8852de(hal);
#endif /* CONFIG_SYNC_INTERRUPT */

	/*if (!recognized)
		HAL_DUMP_INT_8852DE(hal, true, true, true);*/
...
static u32 hal_bcn_handler_8852de(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static u32 hal_rx_handler_8852de(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static u32 hal_tx_handler_8852de(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
	u32 handled0 = 0x0;
...
static u32 hal_cmd_handler_8852de(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static u32 hal_halt_c2h_handler_8852de(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
static u32 hal_gt3_handler_8852de(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
...
static u32 hal_watchdog_timer_handler_8852de(struct hal_info_t *hal, u32 *handled)
{
	u32 ret = 0;
	struct rtw_hal_com_t *hal_com = hal->hal_com;
...
u32 hal_int_hdler_8852de(struct hal_info_t *hal)
{
	...
	/* bit 1 : rx related */
	int_hdler_msk |= (hal_rx_handler_8852de(hal, handled) << 1);

	/* bit 2 : tx related */
	int_hdler_msk |= (hal_tx_handler_8852de(hal, handled) << 2);

	/* bit 4 : halt c2h */
	int_hdler_msk |= (hal_halt_c2h_handler_8852de(hal, handled) << 4);

	/* bit 5 : watchdog timeout */
	int_hdler_msk |= (hal_watchdog_timer_handler_8852de(hal, handled) << 5);

	/* <8> gt3 timer related; CONFIG_VW_REFINE || CONFIG_ONE_TXQ */
	int_hdler_msk |= (hal_gt3_handler_8852de(hal, handled) << 7);
...
void hal_rx_int_restore_8852de(struct hal_info_t *hal)
{
	struct rtw_hal_com_t *hal_com = hal->hal_com;

#ifdef CONFIG_RTL_WIFI_OFFLOAD
	if (hal_com->rxq_offload)
		hal_com->_intr[0].mask |= (B_AX_RDU_INT_EN | B_AX_RPQBD_FULL_INT_EN);
	else
...
enum rtw_hal_status
hal_mp_init_8852de(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_init_info_t init_52de;
	...
	hal_status = hal_mp_init_8852d(phl_com, hal_info, &init_52de);
...
enum rtw_hal_status
hal_mp_deinit_8852de(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal_info)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
	struct hal_init_info_t init_52de;
	...
	hal_status = hal_mp_deinit_8852d(phl_com, hal_info, &init_52de);

	if (RTW_HAL_STATUS_SUCCESS != hal_status) {

		PHL_ERR("hal_mp_deinit_8852de: status = %u\n", hal_status);
		return hal_status;
	}

                            ...
                            struct rtw_phl_rx_pkt *phl_rx)
{
	...
	phl_mirror_dump_rxd(phl_com->phl_priv, buf, (u32)(pkt->vir_addr - buf));
	#endif
	...
	case RX_8192XB_DESC_PKT_T_WIFI :
	{
		...
		rxbuf->frame = pkt->vir_addr;
		rxbuf->frame_len = pkt->length;
		...
		if (!mdata->hdr_conv)
			...
			if (  (mdata->drv_info_size * RX_DESC_DRV_INFO_UNIT_8192XB)
			    < (hdr_offset + 32)) {
				PHL_ERR("Driver info size is too small!\n");
				return RTW_HAL_STATUS_FAILURE;
			}
                           ...
                           struct rtw_phl_rx_pkt *phl_rx)
{
	...
	if ((rpkt_type != RX_8192XB_DESC_PKT_T_WIFI)) {
		PHL_PRINT("R type mismatch %u/%u\n", rpkt_type, RX_8192XB_DESC_PKT_T_WIFI);
		return RTW_HAL_STATUS_FAILURE;
	}

			  ...
			  enum hv_ax_pwr_seq_sel sel)
{
	...
	case HV_AX_PWR_SEQ_SEL_ON_NIC:
		tmp = adapter->mac_pwr_info.pwr_on_seq[0];
		adapter->mac_pwr_info.pwr_on_seq[0] = mac_pwron_nic_8852a;
		ret = adapter->ops->pwr_switch(adapter, 1);
		adapter->mac_pwr_info.pwr_on_seq[0] = tmp;
		break;
	case HV_AX_PWR_SEQ_SEL_OFF_NIC:
		tmp = adapter->mac_pwr_info.pwr_off_seq[0];
		adapter->mac_pwr_info.pwr_off_seq[0] = mac_pwroff_nic_8852a;
		ret = adapter->ops->pwr_switch(adapter, 0);
		adapter->mac_pwr_info.pwr_off_seq[0] = tmp;
		break;
	...
	case HV_AX_PWR_SEQ_SEL_ON_AP:
		tmp = adapter->mac_pwr_info.pwr_on_seq[0];
		adapter->mac_pwr_info.pwr_on_seq[0] = mac_pwron_ap_8852a;
		ret = adapter->ops->pwr_switch(adapter, 1);
		adapter->mac_pwr_info.pwr_on_seq[0] = tmp;
		break;
	case HV_AX_PWR_SEQ_SEL_OFF_AP:
		tmp = adapter->mac_pwr_info.pwr_off_seq[0];
		adapter->mac_pwr_info.pwr_off_seq[0] = mac_pwroff_ap_8852a;
		ret = adapter->ops->pwr_switch(adapter, 0);
		adapter->mac_pwr_info.pwr_off_seq[0] = tmp;
		break;
	default:

...
u32 cmd_mac_fw_swtx(struct mac_ax_adapter *adapter, char input[][MAC_MAX_ARGV], u32 input_num,
		    char *output, u32 out_len, u32 *used)
{
	struct mac_tx_mode_switch info = {0};
	...
	info.swtx = (u8)PLTFM_STRTOUL(input[1], 10);
	info.chain_count = (u8)PLTFM_STRTOUL(input[2], 10);
	info.swap_pri = (u8)PLTFM_STRTOUL(input[3], 10);
	mac_txmode_switch(adapter, &info);

...
u32 pcie_pre_init(struct mac_ax_adapter *adapter, void *param)
{
	...
	ret = p_ops->pcie_set_oobs(adapter);
	if (ret != MACSUCCESS) {
...
u32 pcie_tx_bd_reallocate(struct mac_ax_adapter *adapter,
			  struct mac_ax_txbd_realloc_info *intf_info,
			  bool int_en)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
	struct tx_base_desc *txbd = (struct tx_base_desc *)intf_info->txbd_buf;
	enum pcie_bd_ctrl_type type;
	enum mac_ax_pcie_func_ctrl *txch_ctrl_in;
	enum mac_ax_pcie_func_ctrl *txch_ctrl;
	struct mac_ax_txdma_ch_map txch_map;
	struct mac_ax_rxdma_ch_map rxch_map_dum;
	u32 reg, val32, bd_num, bd_addr_h_p2 = 0;
	u8 ch;
	u32 ret = MACSUCCESS;

	/* TX CH with MAC_AX_PCIE_ENABLE is regartded as "valid" */
	/* Prepare a copy of input txch_map and set valid CH to "disable" */
	txch_ctrl_in = (enum mac_ax_pcie_func_ctrl *)intf_info->txch_map;
	txch_ctrl = (enum mac_ax_pcie_func_ctrl *)&txch_map;
	for (ch = MAC_AX_DMA_ACH0; ch < MAC_AX_DMA_CH_NUM; ch++) {
		if (txch_ctrl_in[ch] == MAC_AX_PCIE_ENABLE)
			txch_ctrl[ch] = MAC_AX_PCIE_DISABLE;
		else
			txch_ctrl[ch] = MAC_AX_PCIE_IGNORE;
	}

	/* Disable TXDMA of valid TX CHs */
	ret = ops->ctrl_txdma_ch(adapter, &txch_map);
	if (ret != MACSUCCESS) {
		PLTFM_MSG_ERR("%s:stop txma ch fail %d\n", __func__, ret);
		return ret;
	}

	/* Poll TXDMA idle of valid TX CHs */
	ret = ops->poll_txdma_ch_idle(adapter, intf_info->txch_map);
	if (ret != MACSUCCESS) {
		PLTFM_MSG_ERR("%s:poll txdma idle fail %d\n", __func__, ret);
		return ret;
	}

	/* Prepare dummy rxch_map since that only tx ch should be executed here */
	rxch_map_dum.rxq = MAC_AX_PCIE_IGNORE;
	rxch_map_dum.rpq = MAC_AX_PCIE_IGNORE;
	/* Clear index of valid TX CHs */
	ret = p_ops->clr_idx_ch_pcie(adapter, intf_info->txch_map, &rxch_map_dum);
	if (ret != MACSUCCESS) {
		PLTFM_MSG_ERR("%s:clear txbd fail %d\n", __func__, ret);
		return ret;
	}
	...
	/* Get current setting of bd addr high part 2 (TRx bd common) */
	type = PCIE_BD_CTRL_DESC_H_P2;
	ret = p_ops->get_txbd_reg_pcie(adapter, MAC_AX_DMA_ACH0, &reg, type);
	if (ret != MACSUCCESS) {
		PLTFM_MSG_ERR("get bd addr high part 2 reg fail\n");
		return ret;
	}
	bd_addr_h_p2 = MAC_REG_R32(reg);
	...
	for (ch = MAC_AX_DMA_ACH0; ch < MAC_AX_DMA_CH_NUM; ch++) {
		...
		/* Only valid TX CHs should be executed */
		if (txch_ctrl_in[ch] != MAC_AX_PCIE_ENABLE)
			continue;
		...
		/* Input bd addr high part 2 should be the same as current setting */
		if (bd_addr_h_p2 != txbd[ch].phy_addr_h) {
			PLTFM_MSG_ERR("txbd%d addr high part2 mismatch\n", ch);
			return MACCMP;
		}
	...
	/* Disable DMA OK IMR of valid TX CHs */
	if (int_en) {
		ret = p_ops->set_dmaok_imr_ch_pcie(adapter, intf_info->txch_map,
						&rxch_map_dum);
		if (ret != MACSUCCESS) {
			PLTFM_MSG_ERR("%s:set dma ok imr fail %d\n", __func__, ret);
			return ret;
		}
	}

	/* Enable  TXDMA of valid TX CHs */
	ret = ops->ctrl_txdma_ch(adapter, intf_info->txch_map);
	if (ret != MACSUCCESS) {
		PLTFM_MSG_ERR("%s:stop txma ch fail %d\n", __func__, ret);
		return ret;
...
u32 pcie_rx_bd_reallocate(struct mac_ax_adapter *adapter,
			  struct mac_ax_rxbd_realloc_info *intf_info,
			  bool int_en)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
	struct rx_base_desc *rxbd = (struct rx_base_desc *)intf_info->rxbd_buf;
	enum pcie_bd_ctrl_type type;
	struct mac_ax_txdma_ch_map txch_map_dum;
	enum mac_ax_pcie_func_ctrl *txch_ctrl;
	enum mac_ax_pcie_func_ctrl *rxch_ctrl_in;
	u32 reg, bd_num, bd_addr_h_p2 = 0;
	u8 ch;
	u32 ret = MACSUCCESS;
	u8 cmac0_en = MAC_AX_FUNC_EN;
	u8 cmac1_en = MAC_AX_FUNC_EN;

	/* Stop RMAC RX in if CMAC is enabled */
	if (check_mac_en(adapter, MAC_AX_BAND_0, MAC_AX_CMAC_SEL))
		cmac0_en = MAC_AX_FUNC_DIS;
	if (check_mac_en(adapter, MAC_AX_BAND_1, MAC_AX_CMAC_SEL))
		cmac1_en = MAC_AX_FUNC_DIS;
	if (cmac0_en)
	...
	/* Disable RXHCI */
	ret = ops->ctrl_rxhci(adapter, MAC_AX_FUNC_DIS);
	if (ret != MACSUCCESS) {
		PLTFM_MSG_ERR("%s:stop rxhci fail %d\n", __func__, ret);
		return ret;
	}

	/* RX CH with MAC_AX_PCIE_ENABLE is regartded as "valid" */
	/* Poll RXDMA idle of valid RX CHs */
	ret = ops->poll_rxdma_ch_idle(adapter, intf_info->rxch_map);
	if (ret != MACSUCCESS) {
		PLTFM_MSG_ERR("%s:poll rxdma idle fail %d\n", __func__, ret);
		return ret;
	}

	/* Prepare dummy txch_map since that only rx ch should be executed here */
	txch_ctrl = (enum mac_ax_pcie_func_ctrl *)&txch_map_dum;
	for (ch = MAC_AX_DMA_ACH0; ch < MAC_AX_DMA_CH_NUM; ch++)
		txch_ctrl[ch] = MAC_AX_PCIE_IGNORE;

	/* Poll RXDMA idle of valid CHs */
	ret = p_ops->clr_idx_ch_pcie(adapter, &txch_map_dum,
				     intf_info->rxch_map);
	if (ret != MACSUCCESS) {
		PLTFM_MSG_ERR("%s:clear rxbd fail %d\n", __func__, ret);
		return ret;
	}
	...
	rxch_ctrl_in = (enum mac_ax_pcie_func_ctrl *)intf_info->rxch_map;
	for (ch = MAC_AX_RX_CH_RXQ; ch < MAC_AX_RX_CH_NUM; ch++) {
		/* Only valid TX CHs should be executed */
		if (rxch_ctrl_in[ch] != MAC_AX_PCIE_ENABLE)
			continue;
		...
		/* Input bd addr high part 2 should be the same as current setting */
		if (bd_addr_h_p2 != rxbd[ch].phy_addr_h) {
			PLTFM_MSG_ERR("rxbd%d addr high part2 mismatch\n", ch);
			return MACCMP;
		}
	...
	/* Disable DMA OK IMR of valid RX CHs */
	if (int_en) {
		ret = p_ops->set_dmaok_imr_ch_pcie(adapter, &txch_map_dum,
						intf_info->rxch_map);
		if (ret != MACSUCCESS) {
			PLTFM_MSG_ERR("%s:set dma ok imr fail %d\n", __func__, ret);
			return ret;
	...
	/* Enable RXHCI */
	ret = ops->ctrl_rxhci(adapter, MAC_AX_FUNC_EN);
	if (ret != MACSUCCESS) {
		PLTFM_MSG_ERR("%s:start rxhci fail %d\n", __func__, ret);
		return ret;
	}

	/* Start RMAC RX in if CMAC is enabled */
	if (cmac0_en)
		MAC_REG_W32(R_AX_RCR, (MAC_REG_R32(R_AX_RCR) & ~B_AX_STOP_RX_IN));
	if (cmac1_en)
		MAC_REG_W32(R_AX_RCR_C1, (MAC_REG_R32(R_AX_RCR_C1) & ~B_AX_STOP_RX_IN));

...
u32 mac_txmode_switch(struct mac_ax_adapter *adapter, struct mac_tx_mode_switch *info)
{
	u32 ret = 0;
	struct h2c_info h2c_info = {0};
	u32 write_ptr;

	u8 swtx = (u8)info->swtx;
	u8 chain_count = (u8)info->chain_count;
	u8 swap_pri    = (u8)info->swap_pri;
	...
	write_ptr |=
	cpu_to_le32(swap_pri ?
		    FWCMD_H2C_TXMODE_SWITCH_SWAP_PRIMARY_ENABLE : 0);

...
u32 mac_get_ru_size_from_allocation(u16 allocation, u32 *bw_idx)
{
#ifdef MAC_TXRPT_STATISTIC
	allocation = allocation >> MAC_RU_ALLOCATION_UPPER_80M_INDICATOR;
	*bw_idx = 0xff;

	if (allocation < PPDU_HE_26_tone_NUM)
		*bw_idx = PPDU_HE_26_tone;

	else if (allocation < PPDU_HE_26_tone_NUM + PPDU_HE_52_tone_NUM)
		*bw_idx = PPDU_HE_52_tone;

	else if (allocation < PPDU_HE_26_tone_NUM + PPDU_HE_52_tone_NUM + PPDU_HE_106_tone_NUM)
		*bw_idx = PPDU_HE_106_tone;
		...
		*bw_idx = PPDU_HE_20M;
		 ...
		 + PPDU_HE_20M_NUM + PPDU_HE_40M_NUM)
		*bw_idx = PPDU_HE_40M;

	else if (allocation < PPDU_HE_26_tone_NUM + PPDU_HE_52_tone_NUM + PPDU_HE_106_tone_NUM
		 + PPDU_HE_20M_NUM + PPDU_HE_40M_NUM + PPDU_HE_80M_NUM)
		*bw_idx = PPDU_HE_80M;
...
u32 mac_process_txrpt(struct mac_ax_adapter *adapter, u8 *frame, u32 len)
{
	...
	if (rate_gen == PPDU_GEN_OFDM_CCK) {
		...
		+= fail_packet;
	} else if (rate_gen == PPDU_GEN_HE) {
		if (ppdu_type == PPDU_TYPE_SU || ppdu_type == PPDU_TYPE_MU)
			bw = bw + PPDU_HE_20M;
		else if (ppdu_type == PPDU_TYPE_RU)
			mac_get_ru_size_from_allocation((u16)rpt->ru_allocation, &bw);

		txrpt_dbg_info->he_ok_fail[ppdu_type][bw]
		[rate_nss][rate_mcs][MPDU_SUCCESS]
		+= success_packet;
		txrpt_dbg_info->he_ok_fail[ppdu_type][bw]
		[rate_nss][rate_mcs][MPDU_FAIL]

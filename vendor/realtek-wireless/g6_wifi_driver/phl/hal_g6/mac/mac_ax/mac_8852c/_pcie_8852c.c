...
u32 pcie_set_oobs_8852c(struct mac_ax_adapter *adapter)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u8 link_speed;
	u16 val16, oobs_val = 0;
	u32 backup_aspm, ret = MACSUCCESS;
	u32 phy_offset = 0;

	if (adapter->env_info.env == DUT_ENV_FPGA || adapter->env_info.env == DUT_ENV_PXP)
		return MACSUCCESS;

	backup_aspm = MAC_REG_R32(R_AX_PCIE_MIX_CFG_V1);
	MAC_REG_W32(R_AX_PCIE_MIX_CFG_V1, backup_aspm & (~(PCIE_BIT_L1_V1 | PCIE_BIT_L0S_V1)));

	if ((MAC_REG_R16(RAC_DIRECT_OFFSET_G1 + RAC_ANA09 * RAC_MULT) & BAC_OOBS_SEL) &
	    (MAC_REG_R16(RAC_DIRECT_OFFSET_G2 + RAC_ANA09 * RAC_MULT) & BAC_OOBS_SEL))
		return MACSUCCESS;

	ret = ops->get_pcie_speed(adapter, &link_speed);
	if (ret) {
		PLTFM_MSG_ERR("[ERR]Get speed FAIL!\n");
		return ret;
	}

	if (link_speed == PCIE_GEN1_SPEED) {
		phy_offset = RAC_DIRECT_OFFSET_G1;
	} else if (link_speed == PCIE_GEN2_SPEED) {
		phy_offset = RAC_DIRECT_OFFSET_G2;
	} else {
		MAC_REG_W32(R_AX_PCIE_MIX_CFG_V1, backup_aspm);
		PLTFM_MSG_ERR("[ERR]PCIe PHY rate not support\n");
		return MACHWNOSUP;
	}

	val16 = MAC_REG_R16(phy_offset + RAC_ANA0D * RAC_MULT);
	MAC_REG_W16(phy_offset + RAC_ANA0D * RAC_MULT, val16 | BAC_RX_TEST_EN);

	MAC_REG_W16(phy_offset + RAC_ANA10 * RAC_MULT, ADDR_SEL_PINOUT_DIS_VAL);

	val16 = MAC_REG_R16(phy_offset + RAC_ANA19 * RAC_MULT);
	MAC_REG_W16(phy_offset + RAC_ANA19 * RAC_MULT, val16 | PCIE_BIT_RD_SEL);

	val16 = MAC_REG_R16(phy_offset + RAC_ANA1F * RAC_MULT);
	val16 = GET_FIELD(val16, OOBS_LEVEL);
	oobs_val = SET_CLR_WORD(oobs_val, val16, OOBS_SEN);

	MAC_REG_W16(RAC_DIRECT_OFFSET_G1 + RAC_ANA03 * RAC_MULT, oobs_val);

	val16 = MAC_REG_R16(RAC_DIRECT_OFFSET_G1 + RAC_ANA09 * RAC_MULT);
	MAC_REG_W16(RAC_DIRECT_OFFSET_G1 + RAC_ANA09 * RAC_MULT, val16 | BAC_OOBS_SEL);

	MAC_REG_W16(RAC_DIRECT_OFFSET_G2 + RAC_ANA03 * RAC_MULT, oobs_val);

	val16 = MAC_REG_R16(RAC_DIRECT_OFFSET_G2 + RAC_ANA09 * RAC_MULT);
	MAC_REG_W16(RAC_DIRECT_OFFSET_G2 + RAC_ANA09 * RAC_MULT, val16 | BAC_OOBS_SEL);
...
u32 set_dmaok_imr_ch_pcie_8852c(struct mac_ax_adapter *adapter,
				struct mac_ax_txdma_ch_map *txch_map,
				struct mac_ax_rxdma_ch_map *rxch_map)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	u32 reg_imr_trx = R_AX_HAXI_HIMR00;
	u32 reg_imr_tx1 = R_AX_HAXI_HIMR10;
	u32 val32 = 0;

	val32 = MAC_REG_R32(reg_imr_trx);
	if (txch_map->ch0 == MAC_AX_PCIE_ENABLE)
		val32 |= B_AX_TXDMA_ACH0_INT_EN;
	else if (txch_map->ch0 == MAC_AX_PCIE_DISABLE)
		val32 &= ~B_AX_TXDMA_ACH0_INT_EN;

	if (txch_map->ch1 == MAC_AX_PCIE_ENABLE)
		val32 |= B_AX_TXDMA_ACH1_INT_EN;
	else if (txch_map->ch1 == MAC_AX_PCIE_DISABLE)
		val32 &= ~B_AX_TXDMA_ACH1_INT_EN;

	if (txch_map->ch2 == MAC_AX_PCIE_ENABLE)
		val32 |= B_AX_TXDMA_ACH2_INT_EN;
	else if (txch_map->ch2 == MAC_AX_PCIE_DISABLE)
		val32 &= ~B_AX_TXDMA_ACH2_INT_EN;

	if (txch_map->ch3 == MAC_AX_PCIE_ENABLE)
		val32 |= B_AX_TXDMA_ACH3_INT_EN;
	else if (txch_map->ch3 == MAC_AX_PCIE_DISABLE)
		val32 &= ~B_AX_TXDMA_ACH3_INT_EN;

	if (txch_map->ch4 == MAC_AX_PCIE_ENABLE)
		val32 |= B_AX_TXDMA_ACH4_INT_EN;
	else if (txch_map->ch4 == MAC_AX_PCIE_DISABLE)
		val32 &= ~B_AX_TXDMA_ACH4_INT_EN;

	if (txch_map->ch5 == MAC_AX_PCIE_ENABLE)
		val32 |= B_AX_TXDMA_ACH5_INT_EN;
	else if (txch_map->ch5 == MAC_AX_PCIE_DISABLE)
		val32 &= ~B_AX_TXDMA_ACH5_INT_EN;

	if (txch_map->ch6 == MAC_AX_PCIE_ENABLE)
		val32 |= B_AX_TXDMA_ACH6_INT_EN;
	else if (txch_map->ch6 == MAC_AX_PCIE_DISABLE)
		val32 &= ~B_AX_TXDMA_ACH6_INT_EN;

	if (txch_map->ch7 == MAC_AX_PCIE_ENABLE)
		val32 |= B_AX_TXDMA_ACH7_INT_EN;
	else if (txch_map->ch7 == MAC_AX_PCIE_DISABLE)
		val32 &= ~B_AX_TXDMA_ACH7_INT_EN;

	if (txch_map->ch8 == MAC_AX_PCIE_ENABLE)
		val32 |= B_AX_TXDMA_CH8_INT_EN;
	else if (txch_map->ch8 == MAC_AX_PCIE_DISABLE)
		val32 &= ~B_AX_TXDMA_CH8_INT_EN;

	if (txch_map->ch9 == MAC_AX_PCIE_ENABLE)
		val32 |= B_AX_TXDMA_CH9_INT_EN;
	else if (txch_map->ch9 == MAC_AX_PCIE_DISABLE)
		val32 &= ~B_AX_TXDMA_CH9_INT_EN;

	if (txch_map->ch12 == MAC_AX_PCIE_ENABLE)
		val32 |= B_AX_TXDMA_CH12_INT_EN;
	else if (txch_map->ch12 == MAC_AX_PCIE_DISABLE)
		val32 &= ~B_AX_TXDMA_CH12_INT_EN;

	if (rxch_map->rxq == MAC_AX_PCIE_ENABLE)
		val32 |= (B_AX_RXDMA_INT_EN | B_AX_RXP1DMA_INT_EN);
	else if (rxch_map->rxq == MAC_AX_PCIE_DISABLE)
		val32 &= ~(B_AX_RXDMA_INT_EN | B_AX_RXP1DMA_INT_EN);

	if (rxch_map->rpq == MAC_AX_PCIE_ENABLE)
		val32 |= B_AX_RPQDMA_INT_EN;
	else if (rxch_map->rpq == MAC_AX_PCIE_DISABLE)
		val32 &= ~B_AX_RPQDMA_INT_EN;
	MAC_REG_W32(reg_imr_trx, val32);

	val32 = MAC_REG_R32(reg_imr_tx1);
	if (txch_map->ch10 == MAC_AX_PCIE_ENABLE)
		val32 |= B_AX_TXDMA_CH10_INT_EN_V1;
	else if (txch_map->ch10 == MAC_AX_PCIE_DISABLE)
		val32 &= ~B_AX_TXDMA_CH10_INT_EN_V1;

	if (txch_map->ch11 == MAC_AX_PCIE_ENABLE)
		val32 |= B_AX_TXDMA_CH11_INT_EN_V1;
	else if (txch_map->ch11 == MAC_AX_PCIE_DISABLE)
		val32 &= ~B_AX_TXDMA_CH11_INT_EN_V1;
	MAC_REG_W32(reg_imr_tx1, val32);

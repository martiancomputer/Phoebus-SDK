...
static void _phl_dump_stainfo(struct rtw_phl_stainfo_t *phl_sta)
{
	...
	PHL_INFO("\t\t[HAL STA] ra_mask:0x%016llx cur_ra_mask:0x%016llx, retry_ratio:%d\n",
...
__phl_free_stainfo_sw(struct phl_info_t *phl_info, struct rtw_phl_stainfo_t *sta)
{
	...
	phl_sta = rtw_phl_get_stainfo_by_addr(phl_info, wrole, sta->mac_addr);
	if (!phl_sta) {
		PHL_WARN("%s target sta(%02x:%02x:%02x:%02x:%02x:%02x) NOT exist\n",
		         __func__, sta->mac_addr[0], sta->mac_addr[1], sta->mac_addr[2],
			...
			u8 *sta_addr, bool is_connect)
{
	...
	if (is_connect == true && is_sta_linked == true) {
		...
		rtw_phl_cmd_change_stainfo(phl_info, sta, STA_CHG_NSS,
	...
	struct rtw_phl_stainfo_t *sta, enum sta_chg_id chg_id, u8 *chg_info, u8 chg_info_len)
{
	...
	case STA_CHG_AGG_NUM: {
		...
		PHL_PRINT("%s MACID:%02d AGG_NUM:%u\n",
			  __func__, sta->macid,
		...
		struct rtw_r_meta_data *meta, struct rtl_wfo_rx_sts *users)
{
	...
	struct rtl_wfo_rx_sts *user;
	u8 i;

	for (i=0; i<user_cnt; i++) {
		user = users + i;
		phl_sta = rtw_phl_get_stainfo_by_macid(phl, user->macid);
		if (phl_sta && phl_sta->core_data) {
			psta = (struct sta_info *)phl_sta->core_data;
			psta->cur_rx_data_rate = user->datarate;
			psta->cur_rx_gi_ltf = user->gi_ltf;
			psta->cur_rx_bw = user->bw;
			psta->sta_stats.rx_data_pkts += user->pkt_cnt;
			psta->sta_stats.rx_bytes += user->pkt_bytes;
			psta->sta_stats.rx_bytes_per_sta += user->pkt_bytes;//RT_TRX_TRAFFIC
			psta->sta_stats.last_rx_time = rtw_get_current_time();
			GET_PRIMARY_ADAPTER(psta->padapter)->recvpriv.rx_bytes_per_band += user->pkt_bytes;//RT_TRX_TRAFFIC
			psta->padapter->recvpriv.rx_bytes += user->pkt_bytes;
			psta->padapter->recvpriv.rx_pkts += user->pkt_cnt;
			psta->padapter->mlmepriv.LinkDetectInfo.NumRxOkInPeriod += user->pkt_cnt;

			phl_update_pe_rx_stats(&phl_sta->stats, user->pkt_bytes);
		}
	}
}

...
#define MAX_SCS_DESCRIPTOR 2
#define MAX_TCLAS_ELE 2
...
#endif
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	enum rtw_phl_status (*process_pwr_state)(void *drv_priv,
			u8 pwrbit, u16 macid, u16 seq);
	enum rtw_phl_status (*process_tim_indic)(void *drv_priv, u16 macid);
	enum rtw_phl_status (*process_om_control)(void *drv_priv, u16 macid, u32 htc);
	enum rtw_phl_status (*count_pe_tx_stats)(void *phl, void *app);
	enum rtw_phl_status (*pe_trx_pause)(void *drv_priv);
	enum rtw_phl_status (*pe_trx_resume)(void *drv_priv);
	enum rtw_phl_status (*pe_tx_force_para)(void *drv_priv);
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
};

#ifdef CONFIG_RTL_WIFI_OFFLOAD
#define MAX_PPDU_USER_CNT	8

struct rtl_wfo_rx_sts {
	u8 macid;
	u8 bw;
	u8 gi_ltf;
	u8 rsvd;
...
struct rx_ppdu_append {
	...
	u16 datarate;
	u16 rsvd;
	struct rtl_wfo_rx_sts user[MAX_PPDU_USER_CNT];
};
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
...
enum phl_msg_evt_id {
	...
	/* tx_retry_limit */
	MSG_EVT_SET_TX_RTY_LMT = 143,

	/* power limit enable */
	MSG_EVT_SET_PWR_LMT_EN = 144,

	/* power limit enable */
	MSG_EVT_SET_EDCCA_MODE_EN = 145,

	/* CONFIG_RTW_AP_BS_DECT */
	MSG_EVT_BCCHG_COUNTDOWN_ZERO = 150,
	/* tx power */
...
enum rtw_fw_ext_path {
	RTW_FW_PATH_CONFIG, /* 0 */
	RTW_FW_PATH_ROOTFS, /* 1 */
	RTW_FW_PATH_MAX
};
...
struct rtw_phl_dbg_pe_ofld {
	bool is_pe_ofld_dbg;
};
#endif /* CONFIG_ETHER_PKT_AGG */

struct rtw_phl_dbg_cfg_t {
	u8 placeholder; /* avoid empty struct */
#ifdef CONFIG_ETHER_PKT_AGG
	struct rtw_phl_dbg_pe_ofld pe_ofld_cfg;
#endif /* CONFIG_ETHER_PKT_AGG */
};
...
#ifdef CONFIG_ETHER_PKT_AGG
	u32 pe_ofld_amsdus;
	u32 pe_ofld_amsdu_subframes;
	u32 pe_ofld_non_amsdus;
	u32 pe_ofld_wd_amsdus;
	u32 pe_ofld_wd_non_amsdus;
#endif /* CONFIG_ETHER_PKT_AGG */
...
struct rtw_phl_mscs_info {
	u8 req_type;
	u8 up_control_bmp;
	u8 up_control_limit;
	u32 stream_to;
	/* TCLAS Mask Elements */
	struct rtw_phl_tclas_element tclas;
};
...
#ifdef CONFIG_LATENCY_TRACK

#ifdef CONFIG_LATENCY_TRACK_RAW_DATA
#define MAX_REC_NUM 512
#endif
#define MAX_DBG_TX_TRACK_POINT 5
//#define MAX_DBG_RX_TRACK_POINT 1
//#define LAT_QC_DP_NUM	0x4
//#define LAT_QC_MP_4_OFFSET 10
//#define LAT_QC_MP_5_OFFSET 12
//#define LAT_QC_DPID_4 4
//#define LAT_QC_DPID_5 5

struct rtw_phl_lat_txreq_tag
{
	u64 txmp[MAX_DBG_TX_TRACK_POINT];
	u32	tag[2];		/* tag[0] for normal record; tag[1] for raw data */
};

#ifdef CONFIG_LATENCY_TRACK_RAW_DATA
//struct rtw_phl_lat_dbg_rx_info {
//	u32 rec_cnt;
//	u64 rec_mp[MAX_DBG_RX_TRACK_POINT][MAX_REC_NUM];
//};

struct rtw_phl_lat_dbg_tx_info {
	u32 raw_rec_cnt;
	u64 rec_mp[MAX_DBG_TX_TRACK_POINT][MAX_REC_NUM];
	u8	amsdu_first[MAX_REC_NUM];
};
#endif

struct rtw_phl_lat_sta_rec {
	#ifdef CONFIG_LATENCY_TRACK_RAW_DATA
	struct rtw_phl_lat_dbg_tx_info lat_tx_info;
	//struct rtw_phl_lat_dbg_rx_info lat_rx_info;
	#endif
	...
	u32 invalid_rec_cnt;
	u32 valid_rec_cnt;
	u32 rec_mp[MAX_DBG_TX_TRACK_POINT-1];

	u32 avg_rec_mp[MAX_DBG_TX_TRACK_POINT-1];
	u32 avg_invalid_rec_cnt;
	u32 avg_valid_rec_cnt;
};

//struct rtw_phl_lat_ctrl_dp_info_qc {
//	u32 timestamp_pre_l;
//	u32 timestamp_pre_h;
//	u32 latency_diff_pre;
//	u32 entry_idx_pre;
//	u32 tx_retry;
//	u32 retry_time_pre;
//	u32 retry_latency_diff_pre;
//};

//struct rtw_phl_lat_ctrl_info_qc {
//	u32 lat_qc_diff_th;
//	u32 lat_qc_even_th;
//	u32 lat_qc_pkt_period;
//	struct rtw_phl_lat_ctrl_dp_info_qc lat_ctrl_dp_info_qc[LAT_QC_DP_NUM];
//};

struct rtw_phl_lat_ctrl_info {
	u8 ts_res;
	u8 analysis_tpye;
	u8 lat_start;
	u8 lat_dump_tx_raw_data;
	u32 lat_tr_tx_type;
	u32 lat_tr_tx_size_large;
	u32 lat_tr_tx_size_less;
	u32 lat_tr_tx_sip;
	u32 lat_tr_tx_dip;
	//struct rtw_phl_lat_ctrl_info_qc lat_ctrl_info_qc;
};

struct rtw_phl_lat_info {
	struct rtw_phl_lat_ctrl_info lat_ctrl_info;
};
#endif /* CONFIG_LATENCY_TRACK */
...
struct rtw_phl_stainfo_t {
	...
	struct rtw_phl_mscs_info mscs_info;
	u8 mscs_target_up;
	//struct rtw_phl_temp_mscs_status mscs_temp;
	#endif
...
struct rtw_phl_ppdu_phy_info {
	...
	u8 rssi_raw_path[RTW_PHL_MAX_RF_PATH];/*PATH A, PATH B ... PATH D*/
	u8 tx_bf;
...
struct phl_tx_stats {
	...
	u32 pe_tx_cnt;
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
	u32 phl_txreq_jumbo_frame_drop;
};
#endif /* DEBUG_PHL_TX */
...
struct phl_rx_stats {
	...
	u32 rx_drop_no_res;
	u32 rx_drop_tid[RTW_MAX_TID_NUM];
#ifdef PHL_RXSC_AMPDU
	u32 rxsc_ampdu[3];
#endif
...
struct phl_pwr_lmt_param {
	bool en_val;
	u8 band;
};

struct phl_edcca_mode_param {
	u8 band;

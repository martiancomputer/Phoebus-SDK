...
void phl_update_pe_rx_stats(struct rtw_stats *stats, u32 pkt_bytes)
{
	u32 diff_t = 0, cur_time = _os_get_cur_time_ms();
	u64 diff_bits = 0;

	stats->last_rx_time_ms = cur_time;
	stats->rx_byte_total += pkt_bytes;

	if (0 == stats->rxtp.last_calc_time_ms ||
		0 == stats->rxtp.last_calc_bits) {
		stats->rxtp.last_calc_time_ms = stats->last_rx_time_ms;
		stats->rxtp.last_calc_bits = stats->rx_byte_total * 8;
	} else {
		if (cur_time >= stats->rxtp.last_calc_time_ms) {
			diff_t = cur_time - stats->rxtp.last_calc_time_ms;
		} else {
		...
		if (diff_t > RXTP_CALC_DIFF_MS && stats->rx_byte_total != 0) {
			diff_bits = (stats->rx_byte_total * 8) -
				stats->rxtp.last_calc_bits;
			stats->rx_tp_kbits = (u32)_os_division64(diff_bits,
								 diff_t);
			stats->rxtp.last_calc_bits = stats->rx_byte_total * 8;
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
void free_pe_rx_buf(struct phl_info_t *phl_info, void *rxb)
{
	struct rtw_rx_buf *rxbuf = (struct rtw_rx_buf *)rxb;
#if 0
	struct rtw_phl_pe_rx_ring *pe_rx_free_ring = NULL;
	u32 phy_addr = rxbuf->pe_phy_addr;

	pe_rx_free_ring = &phl_info->phl_pe_rx_ring;
	pe_rx_free_ring->entry[pe_rx_free_ring->rw_ptr->core_idx].entry_addr = phy_addr;
	pe_rx_free_ring->rw_ptr->core_idx =
			( pe_rx_free_ring->rw_ptr->core_idx + 1 ) % pe_rx_free_ring->entry_num;
#endif
	rxbuf->pe_phy_addr = 0;
	rxbuf->ofld_bd_sz = 0;
}
				...
				struct rtw_phl_rx_pkt *phl_rx)
{
	...
	PHL_TRACE(COMP_PHL_RECV, _PHL_DEBUG_, "[4] %s:: [%p]\n",
	          __FUNCTION__,
	          list_first_entry_or_null(&phl_rx->r.rx_buf_lst_head,
				...
				struct rtw_phl_rx_pkt *phl_rx)
{
	...
	do {
		...
		PHL_TRACE(COMP_PHL_RECV, _PHL_DEBUG_, "[4] %s:: [%p]\n",
								__FUNCTION__, rx_buf);
#ifdef CONFIG_RTL_WIFI_OFFLOAD
		if (0 != rx_buf->pe_phy_addr) {
			free_pe_rx_buf(phl_info, rx_buf);
			rx_buf->pe_phy_addr = 0;
		}
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
		if (phl_rx->rxbuf_ptr) {
                         ...
                         _os_list *frames)
{
	...
	phl_list_for_loop_safe(pos, n, struct rtw_phl_rx_pkt, frames, list) {
		...
		if (RTW_PHL_STATUS_RESOURCE == status) {
			...
			rtw_phl_count_tid_drop(phl_info->phl_com, pos);
									...
									int index, _os_list *frames)
{
	...
	if (!r->stored_mpdu_num) {
		phl_info->phl_com->rx_stats.reorder_sta--;
		r->acc_queue_num += r->queue_num;
		if (r->queue_num > r->max_queue_num)
			r->max_queue_num = r->queue_num;
		r->queue_num = 0;
	}
										...
										_os_list *frames)
{
	...
	if (seq_less(mpdu_seq_num, head_seq_num)) {
		...
		rtw_phl_count_tid_drop(phl_info->phl_com, pkt);
#endif
	...
	r->queue_num++;
	if (phl_com->rx_stats.reorder_sta > phl_com->rx_stats.max_reorder_sta)
		phl_com->rx_stats.max_reorder_sta = phl_com->rx_stats.reorder_sta;
	if (r->stored_mpdu_num > r->max_stored_mpdu_num)
		r->max_stored_mpdu_num = r->stored_mpdu_num;
#endif /* DEBUG_REORDER_STAT */
                                   ...
                                   _os_list *frames)
{
	...
	while (phl_info->phl_com->append_fcs) {
		#ifdef CONFIG_RTW_HW_RX_AMSDU_CUT
		if (meta->amsdu_cut) {
			struct rtw_rx_buf_base *last_frame;
			/* Deduced FCS if last MSDU exists */
			if (!meta->last_msdu)
				break;
			last_frame = list_last_entry(&phl_rx->r.rx_buf_lst_head,
						     struct rtw_rx_buf_base,
						     list);
			last_frame->frame_len -= 4;
			break;
		}
...
#ifdef DEBUG_PHL_RX
	if(phl_info->phl_com->rx_reorder_disable)
		goto dont_reorder;
#endif
#ifdef PHL_RXSC_AMPDU
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
u8 phl_check_pe_ring_res(struct rtw_phl_pe_rx_ring *pe_rx_ring)
{
	struct rtw_phl_pe_rx_rw *ring = pe_rx_ring->rw_ptr;
	u16 avail = 0, wptr = 0, rptr = 0;
	...
	if (wptr > pe_rx_ring->entry_num || rptr > pe_rx_ring->entry_num)
		return false;

	avail = phl_calc_avail_rptr(rptr, wptr, pe_rx_ring->entry_num);
...
u32 rtw_phl_get_hw_cnt_rdu(void *phl)
{
	...
	return phl_com->rx_stats.rx_rdu_cnt;
}
#endif

#ifdef DEBUG_PHL_RX
void rtw_phl_count_tid_drop(struct rtw_phl_com_t *phl_com, struct rtw_phl_rx_pkt *phl_rx)
{
	struct rtw_r_meta_data *meta = &phl_rx->r.mdata;

	if (!meta->qos)
		return;
	/* current only consider data frames */
	if (meta->tid >= RTW_MAX_TID_NUM)
		return;

	phl_com->rx_stats.rx_drop_tid[meta->tid]++;
}
#endif

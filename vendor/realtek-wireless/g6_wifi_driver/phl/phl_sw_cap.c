	...
	if (IS_AP_ROLE_TYPE(rtype)) {
		...
		switch (phl_com->phy_cap[hw_band].rxss) {
			...
			case 2:
				...
				proto_role_cap->he_rx_mcs[1] = 0xff;
				proto_role_cap->he_rx_mcs[2] = 0xfa;
				proto_role_cap->he_rx_mcs[3] = 0xff;
				break;
		}
		switch (phl_com->phy_cap[hw_band].txss) {
			...
			case 2:
				...
				proto_role_cap->he_tx_mcs[1] = 0xff;
				proto_role_cap->he_tx_mcs[2] = 0xfa;
				proto_role_cap->he_tx_mcs[3] = 0xff;
				break;
		}
		...
		if ((1 == proto_role_cap->ht_su_bfme) ||
			...
			proto_role_cap->bfme_sts_greater_80mhz = 3;
		...
		if ((1 == proto_role_cap->he_su_bfmr) ||
			...
			proto_role_cap->num_snd_dim_greater_80mhz = 1;
		} else {
		      ...
		      struct role_sw_cap_t *sw_role_cap)
{
	...
	do {
		...
		if (IS_AP_ROLE_TYPE(wifi_role->type)) {
		  ...
		  struct rtw_wifi_role_t *wifi_role)
{
	...
	struct protocol_cap_t *proto_cap = &(wifi_role->proto_role_cap);

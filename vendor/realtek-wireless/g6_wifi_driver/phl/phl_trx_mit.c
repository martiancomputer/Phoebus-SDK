...
{
	static enum rtw_tfc_lvl rx_traffic_lvl[3] = { RTW_TFC_IDLE, RTW_TFC_IDLE, RTW_TFC_IDLE };

	struct rtw_stats *phl_stats = &phl_info->phl_com->phl_stats;
	u8 id = phl_info->dev_id;

	if (phl_info->hci->fixed_mitigation == 1)
		return;

	if (rx_traffic_lvl[id] == phl_stats->rx_traffic.lvl)
		return;

	rx_traffic_lvl[id] = phl_stats->rx_traffic.lvl;

	if (rx_traffic_lvl[id] == RTW_TFC_HIGH)

...
void debug_dump_data_header(u8 *buf, u32 buf_len, const char *prefix)
{
	...
	if (buf == NULL) {
		PHL_TRACE(COMP_PHL_DBG, _PHL_ERR_, "[debug dump] buf is NULL\n");
		return;
	}

	PHL_TRACE(COMP_PHL_DBG, _PHL_ERR_, "[debug dump] %s\n", prefix);
	for (i = 0; i < 40; i++) {
		if (!(i % 8))
			PHL_DATA(COMP_PHL_DBG, _PHL_ERR_, "\n");
		PHL_DATA(COMP_PHL_DBG, _PHL_ERR_, "%02X ", buf[i]);
		}
	PHL_DATA(COMP_PHL_DBG, _PHL_ERR_, "\n");
}

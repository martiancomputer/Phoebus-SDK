				...
				struct rtw_phl_tring_list *ring_list)
{
	...
	for (i = 0; i < MAX_PHL_RING_CAT_NUM; i++) {
		...
		_os_kmem_free(drv_priv, ring->entry, ring->entry_num * sizeof(u8 *));
		ring->entry = NULL;
	}
...
u16 tx_ring_num[MAX_PHL_RING_CAT_NUM] = {MAX_PHL_TX_RING_ENTRY_L1_NUM,
										MAX_PHL_TX_RING_ENTRY_L1_NUM,
										MAX_PHL_TX_RING_ENTRY_L1_NUM,
										MAX_PHL_TX_RING_ENTRY_L1_NUM,
										MAX_PHL_TX_RING_ENTRY_L3_NUM,
										MAX_PHL_TX_RING_ENTRY_L3_NUM,
										MAX_PHL_TX_RING_ENTRY_L3_NUM,
										MAX_PHL_TX_RING_ENTRY_L3_NUM,
										MAX_PHL_TX_RING_ENTRY_L3_NUM,
										MAX_PHL_TX_RING_ENTRY_L2_NUM};
			...
			u8 hw_band, u8 hw_wmm, u8 hw_port)
{
	...
	if (NULL != phl_tring_list) {
		...
		for (i = 0; i < MAX_PHL_RING_CAT_NUM; i++) {
			...
			phl_tring_list->phl_ring[i].entry_num = tx_ring_num[i];
			phl_tring_list->phl_ring[i].entry = (u8 **)_os_kmem_alloc(drv_priv, phl_tring_list->phl_ring[i].entry_num * sizeof(u8 *));
			if (phl_tring_list->phl_ring[i].entry == NULL) {
				PHL_DBG("cannot malloc %lu bytes, stop!", (unsigned long int)(phl_tring_list->phl_ring[i].entry_num * sizeof(u8 *)));
				goto exit;
			}
		}
			   ...
			   u8 is_first)
{
	...
	for (i = 0; i < MAX_PHL_RING_CAT_NUM; i++) {
		...
		ring_sts = _phl_check_ring_status(phl_info, ring, tring_list, is_first);
...
u8 phl_check_xmit_ring_resource(struct phl_info_t *phl_info, _os_list *sta_list)
{
	...
	phl_list_for_loop_safe(tring_list, t, struct rtw_phl_tring_list,
		...
		_phl_check_tring_list(phl_info, tring_list, sta_list, start);
...
void phl_datapath_deinit(struct phl_info_t *phl_info)
{
	...
	struct rtw_phl_handler *phl_pe_isr_handler = &phl_info->phl_pe_isr_handler;
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	phl_deregister_handler(phl_info->phl_com, phl_pe_isr_handler);
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
...
bool rtw_phl_trx_force_resume(void *phl)
{
	bool ret = true;
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct phl_hci_trx_ops *hci_trx_ops = phl_info->hci_trx_ops;

	if (phl_datapath_chk_trx_pause(phl_info, PHL_CTRL_TX)) {
		printk("[%s]phl tx is paused!\n", __func__);
		hci_trx_ops->trx_resume(phl_info, PHL_CTRL_TX);
		ret = false;
	}

	if (phl_datapath_chk_trx_pause(phl_info, PHL_CTRL_RX)) {
		printk("[%s]phl rx is paused!\n", __func__);
		hci_trx_ops->trx_resume(phl_info, PHL_CTRL_RX);
		ret = false;
	}
	return ret;
}
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
enum rtw_phl_status rtw_phl_pe_isr_notify(void *phl)
{
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;

	pstatus = phl_schedule_handler(phl_info->phl_com,
				&phl_info->phl_pe_isr_handler);
				...
				struct rtw_xmit_req *tx_req)
{
	...
	if (NULL != ring) {
		...
		{
			ring->core_idx =
				(ring->core_idx + 1) % ring->entry_num;
...
u16 rtw_phl_tring_rsc(void *phl, u16 macid, u8 tid)
{
	...
	if (tring_list != NULL) {
		...
		if (NULL != ring) {
			rptr = (u16)_os_atomic_read(drv_priv, &ring->phl_idx);
			ring_res = phl_calc_avail_rptr(rptr, ring->core_idx, ring->entry_num);
		}
	}
	_os_spinunlock(drv_priv, &phl_info->t_ring_list_lock, _bh, NULL);
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
		/* request PE to resume trx */
		evt_ops->pe_trx_resume(drv);
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	/* request PE to pause trx */
	evt_ops->pe_trx_pause(drv);
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
...
void rtw_phl_dump_tx_ring(void *phl, void *m)
{
	...
	RTW_PRINT_SEL(m, "total txreq enq/deq = %u/%u\n", phl_info->tx_req_enq, phl_info->tx_req_deq);
	...
	phl_list_for_loop_safe(tring_list, t, struct rtw_phl_tring_list,
		...
		avail = phl_calc_avail_rptr(phl_idx, ring->core_idx, ring->entry_num);

		RTW_PRINT_SEL(m, "	macid[%d] ring[0] avail entry: %d, tx_req_cnt: %d\n",
			tring_list->macid, avail, (u16)_os_atomic_read(drv_priv, &ring->tx_req_cnt));

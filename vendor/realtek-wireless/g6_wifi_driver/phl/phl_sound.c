	...
	else
	{
		...
		for (idx = 0; idx < grp->num_sta; idx++) {
			...
			if (NULL == sta || sta->active == false) {
				...
				if((grp->num_sta != 1) &&  (snd_param->test_flag & PHL_SND_TEST_F_ENABLE_MU_SND) &&((rtw_phl_get_sta_rssi(sta) < (phl_info->phl_com->txbf_mu_rssi_thres - 10)) || !_phl_snd_check_mu_tx_rate(sta))) {
...
phl_snd_func_grouping(struct phl_info_t *phl_info, u8 wroleidx)
{
	...
	if (rtw_phl_role_is_client_category(wrole) && wrole->mstate == MLME_LINKED) {
		...
		phl_list_for_loop(sta, struct rtw_phl_stainfo_t, &wrole->assoc_sta_queue.queue, list) {
			...
			if((snd_param->test_flag & PHL_SND_TEST_F_ENABLE_MU_SND) && (rtw_phl_get_sta_rssi(sta) >= phl_info->phl_com->txbf_mu_rssi_thres) && _phl_snd_check_mu_tx_rate(sta)) {
				u8 mu_grp_cnt = 0;
			...
			if (is_sta_exist_grp == false) {
				if (cnt == 0) { //TXBF or MU first
					...
					pstatus = phl_snd_func_add_snd_grp(
							phl_info,
							(sta->wmode & WLAN_MD_11AX) ? true : false,
							wrole->id, sta, gidx);
					if (pstatus != RTW_PHL_STATUS_SUCCESS)
						break;
				} else { //MU second
...
#ifdef TX_BEAMFORMING
					|| (sta_info->asoc_cap.trig_su_bfm_fb)
#endif
					)

...
void _phl_free_tx_req_pool(void *phl)
{
	...
	while (false == list_empty(&tx_req_pool->busy_list)) {
		...
		treq = list_first_entry(&tx_req_pool->busy_list, struct rtw_xmit_req, list);
		list_del(&treq->list);
		...
		for (ch = 0; ch < hci_info->total_txch_num; ch++) {
			...
			for (wp_seq = 0; wp_seq < WP_MAX_SEQ_NUMBER; wp_seq++) {
				if ((u8 *)treq == wd_ring[ch].wp_tag[wp_seq].ptr) {
					wd_ring[ch].wp_tag[wp_seq].ptr = NULL;
				}
			}
		}
	}

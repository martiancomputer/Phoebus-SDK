...
static enum rtw_phl_status phl_mp_rx_get_rssi_rx_k(
	struct mp_context *mp, struct mp_rx_arg *arg)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;

	hal_status = rtw_hal_mp_rx_get_rssi_rx_k(mp, arg);
...
enum rtw_phl_status mp_rx(struct mp_context *mp, struct mp_rx_arg *arg)
{
	...
	case MP_RX_CMD_FILTER_RX_RESET:
		...
		break;
	case MP_RX_CMD_GET_RSSI_RX_k:
		PHL_INFO("%s: CMD = MP_RX_CMD_GET_RSSI_RX_k\n", __FUNCTION__);
		phl_status = phl_mp_rx_get_rssi_rx_k(mp, arg);
		break;
	default:

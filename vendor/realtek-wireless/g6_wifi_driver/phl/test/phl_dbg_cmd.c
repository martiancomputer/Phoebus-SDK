			 ...
			 u32 input_num, char *output, u32 out_len)
{
	...
	do {
		...
		rtw_hal_set_ch_bw(phl_info->hal, (u8)band, &chdef, RFK_TYPE_FORCE_NOT_DO, false);
...
void _dump_phl_rx(struct phl_info_t *phl_info, char *output, u32 out_len)
{
	...
	PHL_DBG_MON_INFO(out_len, used, output + used, out_len - used,
		"drop@tid = %u %u %u %u %u %u %u %u\n",
		rx_stats->rx_drop_tid[0], rx_stats->rx_drop_tid[1], rx_stats->rx_drop_tid[2], rx_stats->rx_drop_tid[3],
		rx_stats->rx_drop_tid[4], rx_stats->rx_drop_tid[5], rx_stats->rx_drop_tid[6], rx_stats->rx_drop_tid[7]
		);
	PHL_DBG_MON_INFO(out_len, used, output + used, out_len - used,
		      ...
		      u32 input_num, char *output, u32 out_len)
{
	...
	u32 handler_id = 0;
	u32 cpu_id = 0;
	u8 *cpu_id_lst[RTW_PHL_MAX_HANDLER_ID];
	int i;
	...
	cpu_id_lst[3] =   (phl_info->phl_ser_handler.type == RTW_PHL_HANDLER_PRIO_LOW)
			? &phl_info->phl_ser_handler.os_handler.u.workitem.cpu_id
			: &phl_info->phl_ser_handler.os_handler.u.tasklet.cpu_id;
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	cpu_id_lst[4] =   (phl_info->phl_pe_isr_handler.type == RTW_PHL_HANDLER_PRIO_LOW)
			? &phl_info->phl_pe_isr_handler.os_handler.u.workitem.cpu_id
			: &phl_info->phl_pe_isr_handler.os_handler.u.tasklet.cpu_id;
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
	...
	if (input_num < 3) {
		for (i = 0; i < ARRAY_SIZE(handler_name); i++) {
			PHL_DBG_MON_INFO(out_len, used, output + used, out_len - used,
					"%d - %s: %c@%u\n", i, handler_name[i],
					phl_info->phl_tx_handler.type == RTW_PHL_HANDLER_PRIO_LOW
					? 'W' : 'T', *cpu_id_lst[i]);
		}
		return;
	}
	...
	if (handler_id >= ARRAY_SIZE(handler_name)) {
		PHL_DBG_MON_INFO(out_len, used, output + used, out_len - used,
				"Invalid handler ID %u\n", handler_id);
		return;
	}

	*cpu_id_lst[handler_id] = cpu_id;
	PHL_DBG_MON_INFO(out_len, used, output + used, out_len - used,
			"Set PHL %s run on CPU %u\n", handler_name[handler_id],
			*cpu_id_lst[handler_id]);
}
		       ...
		       u32 input_num, char *output, u32 out_len)
{
	...
	if (input_num >= 2) {
		...
		_get_dec_from_string(input[1], &ch);
	...
	PHL_DBG_MON_INFO(out_len, used, output + used, out_len - used,
		"Tx SW Pause: %d\n", phl_datapath_chk_trx_pause(phl_info, PHL_CTRL_TX));
	PHL_DBG_MON_INFO(out_len, used, output + used, out_len - used,
		"Rx SW Pause: %d\n", phl_datapath_chk_trx_pause(phl_info, PHL_CTRL_RX));
}
...
static void _phl_disp_eng_cpu(struct phl_info_t *phl_info, char input[][MAX_ARGV],
		      u32 input_num, char *output, u32 out_len)
{
	u32 used = 0;
	u32 cpu = 0;
	...
	if (1 == input_num) {
		PHL_DBG_MON_INFO(out_len, used, output + used, out_len - used,
			"\ndisp_eng thread cpu: %d(default: %d)\n", phl_info->disp_eng.thread_cpu, CPU_ID_PHL_THREAD);
	} else {
		_get_hex_from_string(input[1], &cpu);
		phl_info->disp_eng.thread_cpu = cpu;
		PHL_DBG_MON_INFO(out_len, used, output + used, out_len - used,
			"\nSET disp_eng thread cpu: %d\n", phl_info->disp_eng.thread_cpu);
	}
}
#endif /* CONFIG_PHL_THREAD_CPU_BALANCE */
...
#ifdef CONFIG_CMD_DISP
#ifdef CONFIG_PHL_THREAD_CPU_BALANCE
	case PHL_DBG_DISP_THREAD_CPU:
		_phl_disp_eng_cpu(phl_info, input, input_num, output, out_len);
	break;
#endif /* CONFIG_PHL_THREAD_CPU_BALANCE */
#endif /* CONFIG_CMD_DISP */
	default:

...
#define MAX_PHL_TX_RING_ENTRY_L1_NUM 2048 // 4096 tid: 0 ~ 3
#define MAX_PHL_TX_RING_ENTRY_L2_NUM (MAX_PHL_TX_RING_ENTRY_L1_NUM>>1) // high
#define MAX_PHL_TX_RING_ENTRY_L3_NUM (MAX_PHL_TX_RING_ENTRY_L1_NUM>>2) // tid: 4 ~ 7 & mgnt
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
/**
 * same with "ca-zephyr-system/rtl_wfo/include/rtl_wfo_def.h"
 */
enum pe_packet_type {
	PE_PKT_TYPE_DATA		= 0,
	PE_PKT_TYPE_DOE2DRV,
...
enum pe_packet_subtype {
	PE_PKT_SUBTYPE_PS			= 0,
	PE_PKT_SUBTYPE_TIM_INDIC,
	PE_PKT_SUBTYPE_OM_CONTROL,
	PE_PKT_SUBTYPE_MAX			= 0x3f,
};

/* RXBD */
#define PE_RXBD_TYPE_SH			30
#define PE_RXBD_TYPE_MSK		0x3
#define PE_RXBD_SUBTYPE_SH		24
#define PE_RXBD_SUBTYPE_MSK		0x3f

#define PE_GET_FIELD(_w, _f) (((_w) >> (_f##_SH)) & (_f##_MSK))

/* RXBD for WiFi RX or DOE2DRV, DOE2PE */
typedef union {
	u32 wrd;
	struct {
		u32 buf_sz			: 14;	/* bits 0:13 */
		u32 rsvd			: 8;	/* bits 14:21 */
		u32 buf_addr_h		: 8;	/* bits 22:29 */
		u32 type			: 2;	/* bits 30:31, always 00 */
	} wifi_rx;
	struct {
		u32 pwr				: 8;	/* bits 0:7 */
		u32 rsvd			: 16;	/* bits 8:23 */
		u32 subtype			: 6;	/* bits 24:29 */
		u32 type			: 2;	/* bits 30:31, 01 or 02 */
	} ps_cmd;
	struct {
		u32 macid			: 16;	/* bits 0:15 */
		u32 rsvd			: 8;	/* bits 16:23 */
		u32 subtype			: 6;	/* bits 24:29 */
		u32 type			: 2;	/* bits 30:31, 01 or 02 */
	} om_cmd;
} pe_rxbd_0_t;
...
typedef union {
	u32 wrd;
	struct {
		u32 buf_addr		: 32;	/* bits 0:31 */
	} wifi_rx;
	struct {
		u32 macid			: 16;	/* bits 0:15 */
		u32 seq				: 16;	/* bits 16:31 */
	} ps_cmd;
	struct {
		u32 ht_control;				/* bits 0:31 */
	} om_cmd;
} pe_rxbd_1_t;

struct rtw_phl_pe_rxbd {
	pe_rxbd_0_t word0;
	pe_rxbd_1_t word1;
};

/* TXBD */
#define PE_TXBD_TYPE_SH			16
#define PE_TXBD_TYPE_MSK		0x3
#define PE_TXBD_SUBTYPE_SH		18
#define PE_TXBD_SUBTYPE_MSK		0x3f

/* TXBD for WiFi TX or DRV2DOE, PE2DOE */
typedef union {
	u32 wrd;
	struct {
		u32 length			: 16;	/* bits 0:15 */
		u32 type			: 2;	/* bits 16:17, always 00 */
		u32 rsvd			: 4;	/* bits 18:21 */
		u32 buf_addr_h		: 8;	/* bits 22:29 */
		u32 ls				: 1;	/* bits 30 */
		u32 drop			: 1;	/* bits 31 */
	} wifi_tx;
...
struct rtw_phl_pe_txbd {
	pe_txbd_0_t word0;
	pe_txbd_1_t word1;
};

/* PE DRV RX */
struct rtw_phl_pe_rx_rw {
	u16 core_idx;
	u16 pe_idx;
};

struct rtw_phl_pe_rx_ring {
	u32 entry_num;
	u8 ch_idx;
	u16 buf_sz;
	struct rtw_phl_pe_rx_rw *rw_ptr;
	struct rtw_phl_pe_rxbd *entry;
};

/* PE DRV TX */
struct rtw_phl_pe_tx_rw {
	u16 pe_idx;
	u16 core_idx;
};

struct rtw_phl_pe_tx_ring {
	u32 entry_num;
	struct rtw_phl_pe_tx_rw *rw_ptr;
	struct rtw_phl_pe_txbd *entry;
};
#endif /* CONFIG_RTL_WIFI_OFFLOAD */

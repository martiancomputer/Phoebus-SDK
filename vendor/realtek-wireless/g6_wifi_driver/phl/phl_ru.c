					...
					struct rtw_phl_mac_ss_ul_grp_upd *info)
{
	...
	if (RTW_HAL_STATUS_SUCCESS == rtw_hal_mac_set_ul_grp_info(phl_info->hal, info)) {
		pstatus = RTW_PHL_STATUS_SUCCESS;
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "%s Success\n", __func__);
	} else {
		PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "%s Fail\n", __func__);
...
int rtw_phl_cal_ru_quota(struct phl_info_t *phl_info, int macid)
{
	...
	if(phl_info->phl_com->dev_cap.dlul_group_mode & BIT0){		// Dynamic quota
		...
		if(phl_sta){
			...
			if ((wd_ptr + pending_ptr) >= phl_wd_quota){
				quota = 0;
			}else{
		...
		if(phl_sta){
			...
			if ((wd_ptr + pending_ptr) >= phl_info->phl_com->phl_wd_quota){
				quota = 0;
			} else {
				quota = phl_info->phl_com->phl_wd_quota - (wd_ptr + pending_ptr);
			}
		}
	}
...
void rtw_phl_ru_cal_he_su_rx_info(struct rtw_stats *sta_stats)
{
	...
	if(sta_stats->HE_rx_cnt[idx] > 0){
		...
		sta_stats->average_HE_rx_rate_new = rate;
...
void rtw_phl_dump_wd_balance_status(void *phl, void *m){
	...
	RTW_PRINT_SEL(m, "macid[indx] cnt_current_wd: (enq busy, enq busy fail, deq busy), pendind: (enq pending, deq pending)\n");

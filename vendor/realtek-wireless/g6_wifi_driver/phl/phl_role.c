...
#ifdef RTW_WKARD_HW_WMM_FORCE_0
	wmm_num = 0;
				...
				enum wr_chg_id chg_id, void *chg_info)
{
	...
	case WR_CHG_AP_PARAM:
	{
		...
		else if (param->cfg_id == CFG_MBSSID_NUM) {
			u32 bcn_interval = param->value;

			rtw_hal_role_cfg_ex(phl_info->hal, wrole, PCFG_MBSSID_NUM, &bcn_interval);
		}
		else if (param->cfg_id == CFG_BCN_HOLD_TIME) {
			u32 bcn_hold_time = param->value;

			rtw_hal_role_cfg_ex(phl_info->hal, wrole, PCFG_BCN_HOLD_TIME, &bcn_hold_time);
		}
	...
	enum rf_path tx, enum rf_path rx)
{
	...
	_os_spinlock(phl_com->drv_priv, &role->assoc_sta_queue.lock, _bh, NULL);
	...
	_os_spinunlock(phl_com->drv_priv, &role->assoc_sta_queue.lock, _bh, NULL);
	...
	u32 *freeruncnt_h, u32 *freeruncnt_l)
{
	...
	hstatus = rtw_hal_get_freeruncnt(phl_info->hal,
			wifi_role->hw_band,
			freeruncnt_h,
			freeruncnt_l);
	if (RTW_HAL_STATUS_SUCCESS == hstatus)

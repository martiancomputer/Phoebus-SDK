...
u16 phl_pe_rx_res_query(u16 rptr, u16 wptr, u16 bndy);
u16 phl_pe_avail_res_query(u16 rptr, u16 wptr, u16 bndy);
					...
					struct phl_info_t *phl_info)
{
	...
	u32 buf_len = hal_com->bus_cap.rxbuf_size*RTW_RX_BUF_SHARE_FACTOR;
#else
	u32 buf_len = hal_com->bus_cap.rxbuf_size;
#endif

#ifdef CONFIG_WNIC_RECYCLED_SKB
	struct rtw_rx_buf_alloc_data alloc_data = {
		.drv_priv = drv_priv,
...
#ifdef WFO_DYNAMIC_RXBUF
		if (adapter->dvobj->wfo_en && !adapter->dvobj->wfo_not_start_pe)
...
#endif
#ifdef CONFIG_WNIC_RECYCLED_SKB
			if (ring[i].pool_idx > 0) {
				u16 rxbuf_num = rtw_hal_get_rxbuf_num(phl_info->hal, i);
				deinit_recycle_wnic_skb_buf(ring[i].pool_idx);
				ring[i].pool_idx = 0;
			}
#endif /* CONFIG_WNIC_RECYCLED_SKB */
		}
...
#ifdef CONFIG_WNIC_RECYCLED_SKB
#ifdef CONFIG_RTL_WIFI_OFFLOAD
			dvobj = (struct dvobj_priv *)phl_to_drvpriv(phl_info);
			if ((i == 0) && RTL_WFO_IS_OFLD(dvobj)) {
				rx_buf_ring[i].pool_idx = 0;
			} else
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
			{
				/* 1/2 extra packet in pool for RX process delay above driver */
				pkt_num_in_pool = ((i == 0) ? ((rxbuf_num * 3) / 2) : rxbuf_num);
				pkt_sz_in_pool = buf_len;
#ifdef CONFIG_RTW_RX_BUF_SHARING
				pkt_num_in_pool /= RTW_RX_BUF_SHARE_FACTOR;
				pkt_sz_in_pool *= RTW_RX_BUF_SHARE_FACTOR;
#endif /* CONFIG_RTW_RX_BUF_SHARING */
				pool_idx = init_recycle_wnic_skb_buf(pkt_num_in_pool, pkt_sz_in_pool);
				if (pool_idx > 0) {
					PHL_PRINT("%u: SKB pool %u created for channel %u of %uB with #%u (%u).\n",
						phl_info->dev_id, pool_idx, i, pkt_sz_in_pool, pkt_num_in_pool, rxbuf_num);
				} else {
...
#endif /* CONFIG_WNIC_RECYCLED_SKB */
			PHL_PRINT("[RXBUF][dev_id:%d][ch:%d] rx_buf: num=%d size=%d\n",
				phl_info->dev_id, i, rxbuf_num, buf_len);

			rx_buf = _phl_alloc_rxbuf_pcie(phl_info, &rx_buf_ring[i], i, buf_len);
			if (NULL == rx_buf) {
				pstatus = RTW_PHL_STATUS_RESOURCE;
				break;
			}
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
			if (phl_info->txq_offload == 2 && (i < 4))
				wd_page_ring[i].wp_seq = WP_MAX_SEQ_NUMBER - MAX_RSVD_SEQ;
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
			pstatus = RTW_PHL_STATUS_SUCCESS;
		}
	}
...
_phl_alloc_rxbd_pcie(struct phl_info_t *phl_info, u8 ch_num)
{
	...
	if (NULL != rxbd) {
		for (i = 0; i < ch_num; i++) {
			...
			PHL_PRINT("[RXBUF][dev_id:%d][ch:%d] rxbd_num:%d buf_len:%d\n",
						...
						struct phl_ring_status *ring_sts)
{
	...
	while (0 != ring_sts->req_busy) {
		...
		if ( (phl_sta == NULL || (phl_sta && !phl_sta->active)) && tring->cat != RTW_PHL_RING_CAT_MGNT)  {
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
void _phl_pe_isr_func(void *context)
{
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	struct rtw_phl_handler *phl_handler =
					(struct rtw_phl_handler *)phl_container_of(context,
						struct rtw_phl_handler, os_handler);
	struct phl_info_t *phl_info = (struct phl_info_t *)phl_handler->context;
	struct phl_hci_trx_ops *hci_trx_ops = phl_info->hci_trx_ops;
	...
	/* Handle PE RX */
	do {
		if (!phl_info->rxq_offload)
			break;

		do {
			if (false == phl_check_pe_ring_res(&phl_info->phl_pe_rx_ring))
				break;
			pstatus = hci_trx_ops->pe_rx(phl_info, &phl_info->phl_pe_rx_ring);
			if (RTW_PHL_STATUS_FAILURE == pstatus) {
				//PHL_TRACE(COMP_PHL_DBG, _PHL_WARNING_,
				//	"[WARNING] phl_pe_rx RXQ fail!\n");
			}
		} while (false);

		do {
			if (false == phl_check_pe_ring_res(&phl_info->phl_pe_rp_ring))
				break;
			pstatus = hci_trx_ops->pe_rx(phl_info, &phl_info->phl_pe_rp_ring);
			if (RTW_PHL_STATUS_FAILURE == pstatus) {
				//PHL_TRACE(COMP_PHL_DBG, _PHL_WARNING_,
				//	"[WARNING] phl_pe_rx RPQ fail!\n");
			}
		} while (false);

		phl_com->rx_stats.rx_pe_isr++;
	} while (false);

	rtw_hal_set_gt3(hal, true, 0, true, false, 1000);

	FUNCOUT_WSTS(pstatus);
}

#ifdef WFO_DYNAMIC_RXBUF
void rtw_phl_release_rx_buffer_pre(void *phl)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct rtw_phl_com_t *phl_com = phl_info->phl_com;
	struct hci_info_t *hci_info = (struct hci_info_t *)phl_info->hci;
	struct rtw_rx_buf_ring *ring = NULL;  /* handle only ch0 - RXQ */
	u16 release_num;
	struct rtw_rx_buf *rx_buf = NULL;
	...
	/* release idle buffer */
	release_num = ring->idle_rxbuf_cnt;
	while (release_num > 0) {
		rx_buf = query_idle_rx_buf(phl_info, ring);
		if (NULL == rx_buf)
			break;
		...
		enqueue_empty_rx_buf(phl_info, ring, rx_buf);
		release_num--;
	}
}

void rtw_phl_release_rx_buffer_post(void *phl)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct rtw_phl_com_t *phl_com = phl_info->phl_com;
	struct hci_info_t *hci_info = (struct hci_info_t *)phl_info->hci;
	...
	/* release busy buffer */
	release_num = ring->busy_rxbuf_cnt;
	while (release_num > 0) {
		rx_buf = query_busy_rx_buf(phl_info, ring);
		if (NULL == rx_buf)
			break;
...
void rtw_phl_refill_rx_buffer(void *phl)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct hci_info_t *hci_info = (struct hci_info_t *)phl_info->hci;
	struct rtw_rx_buf_ring *ring = NULL;
	...
	/* refill idle buffer */
	while (ring->empty_rxbuf_cnt) {
		rx_buf = query_empty_rx_buf(phl_info, ring);
		pstatus = _phl_alloc_dynamic_rxbuf_pcie(rx_buf, ring, phl_info);
		if (RTW_PHL_STATUS_SUCCESS != pstatus) {
			enqueue_empty_rx_buf(phl_info, ring, rx_buf);
			break;
		}
		enqueue_idle_rx_buf(phl_info, ring, rx_buf);
	}
}
#endif /* WFO_DYNAMIC_RXBUF */
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
                            ...
                            u16 packet_len)
{
	...
	tx_req->mdata.wd_page_size = (tx_req->mdata.wdinfo_en) ? 1 : 0;
...
enum rtw_phl_status phl_trx_init_pcie(struct phl_info_t *phl_info)
{
	...
	void *drv_priv = phl_to_drvpriv(phl_info);
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	struct rtw_phl_handler *pe_isr_handler = &phl_info->phl_pe_isr_handler;
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
	...
	do {
		...
		if (RTW_PHL_STATUS_SUCCESS != pstatus)
			break;
#ifdef CONFIG_RTL_WIFI_OFFLOAD
		pe_isr_handler->type = RTW_PHL_HANDLER_PRIO_HIGH;
		pe_isr_handler->callback = _phl_pe_isr_func;
		pe_isr_handler->context = phl_info;
		pe_isr_handler->drv_priv = drv_priv;

		#ifdef CONFIG_RTW_OS_HANDLER_EXT
		pe_isr_handler->id = RTW_PHL_PE_RX_HANDLER;
		#endif /* CONFIG_RTW_OS_HANDLER_EXT */

		pstatus = phl_register_handler(phl_info->phl_com, pe_isr_handler);
		if (RTW_PHL_STATUS_SUCCESS != pstatus)
			break;
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
		/* pcie tx sw resource */
		...
		#else /* !CONFIG_RTL9607F_SERIES */
		hci_info->fixed_mitigation = 1;
		#endif /* CONFIG_RTL9607F_SERIES */
		#endif /* PCIE_TRX_MIT_EN */

#ifdef CONFIG_RTW_RX_BUF_SHARING
		/* rx_buf_info_lock init */
		printk("[%s:%d] init lock rx_buf_info_lock \n", __FUNCTION__, __LINE__);
		_os_spinlock_init(phl_to_drvpriv(phl_info), &hci_info->rx_buf_info_lock);
#endif
		/* pcie rx sw resource */
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	if ((phl_info->txq_offload == 1 && (dma_ch == 0)) ||
		(phl_info->txq_offload == 2 && (dma_ch < 4)))
		wp_seq_start = WP_MAX_SEQ_NUMBER - MAX_RSVD_SEQ;
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
...
phl_prepare_tx_pcie(struct phl_info_t *phl_info, struct rtw_xmit_req *tx_req)
{
	...
	struct phl_txsc_entry *ptxsc = NULL;
	struct rtw_phl_stainfo_t *phl_sta = NULL;
	enum rtw_phl_status txsc_status;
#endif
#ifdef CONFIG_VW_REFINE
	u16 max_seq_nr = WP_MAX_SEQ_NUMBER;
#endif
	u16 wp_seq_start = 1;
	#ifdef CONFIG_WFA_OFDMA_Logo_Test
	void *drv_priv = phl_to_drvpriv(phl_info);
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
			if (phl_info->txq_offload)
				max_seq_nr = MAX_RSVD_SEQ;
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
			val = (phl_info->fr_ptr + 1) % max_seq_nr;
...
#ifdef CONFIG_ETHER_PKT_AGG
		if (phl_com->dbg_cfg.pe_ofld_cfg.is_pe_ofld_dbg) {
			if (tx_req->eth_agg_pkt)
				wd_page->is_pe_ofld_amsdu = 1;
			else if (tx_req->tc_non_agg_pkt)
				wd_page->pe_ofld_non_amsdu += tx_req->tc_non_agg_pkt;
		}
#endif /* CONFIG_ETHER_PKT_AGG */

		if ( phl_pkt_req.tx_req->mdata.type == RTW_PHL_PKT_TYPE_DATA){
...
#ifdef CONFIG_PHL_TXSC
			txsc_status = _phl_txsc_add_shortcut(phl_info, tx_req, &phl_pkt_req, ptxsc);
#endif
#ifdef CONFIG_VW_REFINE
			if (0 == dma_ch) {
				phl_info->free_wp[phl_info->fr_ptr] = WP_USED_SEQ;
				phl_info->fr_ptr = (phl_info->fr_ptr + 1) % max_seq_nr;
			}
#endif
			break;
		} else {
...
rtw_hal_update_pe_tx_ring(struct phl_info_t *phl_info,
			struct rtw_wd_page_list *wd_page_list,
			u8 ch_idx, u16 wd_num)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	u16 host_idx = 0, hw_idx = 0;
	u16 avail_txbd = 0, wcnt = 0;
	u16 txbd_num = phl_info->phl_pe_tx_ring.entry_num;
	struct rtw_wd_page *wd_page = NULL;
	u16 wd_page_index = 0;
	struct rtw_phl_com_t *phl_com = phl_info->phl_com;
	struct rtw_phl_pe_tx_rw *rw_ptr = phl_info->phl_pe_tx_ring.rw_ptr;
	struct rtw_phl_pe_txbd *entry = phl_info->phl_pe_tx_ring.entry;

	do {
		if (NULL == wd_page_list)
			break;

		host_idx = rw_ptr->core_idx;

		avail_txbd = phl_pe_avail_res_query(rw_ptr->pe_idx,
			rw_ptr->core_idx, txbd_num);
		...
		PHL_TRACE(COMP_PHL_XMIT, _PHL_DEBUG_,
			"hal_update_txbd => ch_idx %d, host_idx %d, "
			"hw_idx %d, avail_txbd %d, wcnt %d\n",
			ch_idx, rw_ptr->core_idx, rw_ptr->pe_idx,
			avail_txbd, wcnt);

		while (wcnt > 0) {
			u8 *txbd_buf = NULL;
			wd_page = wd_page_list->wd_page[wd_page_index];
			txbd_buf = (u8 *)&entry[host_idx];

			entry[host_idx].word0.wrd = 0;
			entry[host_idx].word1.wrd = 0;

			rtw_hal_update_pe_txbd(phl_info->hal, txbd_buf, wd_page);
	...
	phl_com->tx_stats.pe_tx_cnt += wd_page_index;
	rw_ptr->core_idx = host_idx;

	return hstatus;
}

static enum rtw_phl_status
phl_handle_pe_pending_wd(struct phl_info_t *phl_info,
	...
	while (txcnt > cnt) {
		...
		if (NULL == wd) {
			...
			PHL_TRACE(COMP_PHL_DBG, _PHL_INFO_,
				"query Tx pending WD fail!\n");
			break;
		}

		hstatus = rtw_hal_update_pe_tx_ring(phl_info, &phl_wd_list, ch, wd_num);
		if (RTW_HAL_STATUS_SUCCESS == hstatus) {
			for (j=0; j<wd_num; j++) {
...
enum rtw_phl_status
phl_wfo_add_txsc(void *phl, struct rtw_xmit_req *tx_req,
		u8 *wdpage, u8 *wd_len, u8 *wd_seq_offset)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;
	struct rtw_phl_pkt_req phl_pkt_req;
	...
	memset(&phl_pkt_req, 0, sizeof(phl_pkt_req));
	phl_pkt_req.wd_page = wdpage;
	phl_pkt_req.wp_seq = 1;
	phl_pkt_req.tx_req = tx_req;

	hstatus = rtw_hal_update_wd_page(phl_info->hal, &phl_pkt_req);
	if (RTW_HAL_STATUS_SUCCESS == hstatus) {
		if (phl_info->phl_com->tx_stats.flag_print_wdpage_once == 1) {
			debug_dump_buf(phl_pkt_req.wd_page,
					(u16)phl_pkt_req.wd_len, "dump wd page");
			phl_info->phl_com->tx_stats.flag_print_wdpage_once = 0;
		}

		*wd_len = phl_pkt_req.wd_len;
		*wd_seq_offset = phl_pkt_req.wd_seq_offset;
	} else {
		RTW_ERR("%s: update wd page fail! (%d)\n", __func__, hstatus);
	}

	return hstatus;
}

void phl_wfo_drv_trx_rcvy(void *phl)
{
	enum rtw_hal_status status = RTW_HAL_STATUS_FAILURE;
	void *txbd_buf = NULL, *rxbd_buf = NULL;
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct hal_info_t *hal_info = (struct hal_info_t *)(phl_info->hal);

	/* re-init driver's trx bd */
	txbd_buf = phl_info->hci_trx_ops->get_txbd_buf(phl_info);
	rxbd_buf = phl_info->hci_trx_ops->get_rxbd_buf(phl_info);
	rtw_hal_trx_bd_reinit(phl_info->hal, txbd_buf, rxbd_buf);

	/* trigger L1 recovery */
	status = rtw_hal_trigger_dmac_err(hal_info);
	RTW_PRINT("WFO drv trx recovery, status %d\n", status);
}
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
				...
				u16 txcnt, u8 ch)
{
	...
	if (wrole && wrole->active) {
		_os_spinlock(drv_priv, &wrole->assoc_sta_queue.lock, _bh, NULL);
		...
		_os_spinunlock(drv_priv, &wrole->assoc_sta_queue.lock, _bh, NULL);
	}
	...
	while (txcnt > cnt) {
		...
		if (RTW_HAL_STATUS_SUCCESS == hstatus) {
			for (j = 0; j < wd_num; j++) {
...
#ifdef CONFIG_ETHER_PKT_AGG
				if (phl_com->dbg_cfg.pe_ofld_cfg.is_pe_ofld_dbg) {
					if (wd->is_pe_ofld_amsdu) {
						wd->is_pe_ofld_amsdu = 0;
						phl_com->phl_stats.pe_ofld_wd_amsdus++;
					} else if (wd->pe_ofld_non_amsdu) {
						phl_com->phl_stats.pe_ofld_wd_non_amsdus += wd->pe_ofld_non_amsdu;
						wd->pe_ofld_non_amsdu = 0;
					}
				}
#endif /* CONFIG_ETHER_PKT_AGG */
				...
				#endif /* CONFIG_WFA_OFDMA_Logo_Test_Statistic */
				#ifdef CONFIG_WFA_OFDMA_Logo_Test
				if (wd->tid == 0 && phl_sta) {
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	if ((phl_info->txq_offload && (ch == 0)) ||
		(phl_info->txq_offload==2 && (ch < 4)))
		bndy = phl_info->phl_pe_tx_ring.entry_num;
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
static enum rtw_phl_status phl_tx_pe(struct phl_info_t *phl_info, u8 ch)
{
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_SUCCESS;
	struct hci_info_t *hci_info = (struct hci_info_t *)phl_info->hci;
	struct rtw_hal_com_t *hal_com = rtw_hal_get_halcom(phl_info->hal);
...
#endif
	hw_res = phl_pe_avail_res_query(rw_ptr->pe_idx,
		rw_ptr->core_idx, phl_info->phl_pe_tx_ring.entry_num);

	//printk("idx : pe_idx = %u, core_idx = %u, hw_res = %u\n",
	//	rw_ptr->pe_idx, rw_ptr->core_idx, hw_res);
	if (0 == hw_res)
		goto pe_tx_out;

	phl_handle_busy_wd(phl_info, &wd_ring[ch], ch, rw_ptr->pe_idx);
	...
	pstatus = phl_handle_pe_pending_wd(phl_info, &wd_ring[ch], txcnt, ch);
...
#ifdef CONFIG_VW_REFINE
	u8 hdl_ch0_wd = 0;
	u16 ch0_hw_res = 0, ch0_host_idx = 0, ch0_hw_idx = 0;
#endif
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	if (phl_info->txq_offload == 1) {
		phl_tx_pe(phl_info, 0);
		ch = 1;
	} else if (phl_info->txq_offload == 2) {
		for (ch = 0; ch < 4; ch++)
			phl_tx_pe(phl_info, ch);
	}
	else
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
		ch = 0;

	for (; ch < hci_info->total_txch_num; ch++) {
#ifndef RTW_WKARD_WIN_TRX_BALANCE
		/* if wd_ring is empty, do not read hw_idx for saving cpu cycle */
		if (wd_ring[ch].pending_wd_page_cnt == 0 && wd_ring[ch].busy_wd_page_cnt == 0){
			pstatus = RTW_PHL_STATUS_SUCCESS;
		...
		if(ch > 7) {
...
#ifdef CONFIG_VW_REFINE
			if (hdl_ch0_wd) {
				ch0_hw_res = rtw_hal_tx_res_query(phl_info->hal, 0, &ch0_host_idx, &ch0_hw_idx);
				txbd[0].hw_idx = ch0_hw_idx;
				phl_handle_busy_wd(phl_info, &wd_ring[0], 0, ch0_hw_idx);
			}
#endif
		} else {
			if(hw_res <  hal_com->bus_cap.txbd_num/4) {
				hw_res = rtw_hal_tx_res_query(phl_info->hal, ch, &host_idx,
								&hw_idx);
				txbd[ch].hw_idx = hw_idx;
				phl_handle_busy_wd(phl_info, &wd_ring[ch], ch, hw_idx);
			}
#ifdef CONFIG_VW_REFINE
			else if (phl_info->use_onetxring && ch == 0) {
				hdl_ch0_wd = 1;
				ch0_host_idx = host_idx;
				ch0_hw_idx = hw_idx;
			}
#endif
		}
					 ...
					 struct rtw_phl_rx_pkt **pphl_rx)
{
	...
	do {
		...
		phl_com->rx_stats.rx_chk_cnt++;

		if (true != rtw_hal_check_rxrdy(phl_com,
						phl_info->hal,
						rxbuf,
						ch)) {
#ifdef DEBUG_PHL_RX
			phl_com->rx_stats.rx_rdy_fail++;
#endif
		...
		if (phl_info->pending_amsdu) {
			...
			if (RTW_HAL_STATUS_SUCCESS == hstatus) {
				...
				if (phl_rx->r.mdata.last_msdu) {
					phl_info->pending_amsdu = NULL;
					#ifdef DEBUG_PHL_RX
					phl_com->rx_stats.amsdu_cut_ok++;
					phl_com->rx_stats.amsdu_cut_cur_num++;
					if (  phl_com->rx_stats.amsdu_cut_max_num
					    < phl_com->rx_stats.amsdu_cut_cur_num)
						phl_com->rx_stats.amsdu_cut_max_num
							= phl_com->rx_stats.amsdu_cut_cur_num;
					#endif /* DEBUG_PHL_RX */
					pstatus = RTW_PHL_STATUS_SUCCESS;
				} else {
				 ...
				 * frame as a new one */
				#ifdef DEBUG_PHL_RX
				phl_com->rx_stats.amsdu_cut_break++;
				phl_com->rx_stats.amsdu_cut_cur_num++;
				if (  phl_com->rx_stats.amsdu_cut_max_num
				    < phl_com->rx_stats.amsdu_cut_cur_num)
					phl_com->rx_stats.amsdu_cut_max_num
...
drop:
	...
	rtw_phl_count_tid_drop(phl_info->phl_com, phl_rx);
#endif
...
void _phl_wp_rpt_statistics(struct phl_info_t *phl_info, u16 wp_seq,
			    u8 txsts, struct rtw_xmit_req *treq)
{
	struct rtw_hal_com_t *hal_com = rtw_hal_get_halcom(phl_info->hal);
	struct rtw_wp_rpt_stats *rpt_stats = NULL;
	u32 diff_t = 0, cur_time = _os_get_cur_time_ms();
	u8 ch = treq->mdata.dma_ch;
...
void _phl_wp_rpt_chk_txsts(struct phl_info_t *phl_info, u16 wp_seq,
			    u8 txsts, struct rtw_xmit_req *treq)
{
	struct rtw_hal_com_t *hal_com = rtw_hal_get_halcom(phl_info->hal);
	struct rtw_wp_rpt_stats *rpt_stats = NULL;
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
void _phl_pe_tx_statistics(struct phl_info_t *phl_info,
	struct rtw_phl_rx_pkt *phl_rx)
{
	struct rtw_phl_evt_ops *ops = &phl_info->phl_com->evt_ops;
	struct rtw_rx_buf *rxbuf = (struct rtw_rx_buf *)phl_rx->rxbuf_ptr;
	struct tx_stats_append *app;
	u16 app_size = 0;
	struct rtw_xmit_req txreq;

	if (rxbuf->ofld_bd_sz && (rxbuf->ofld_bd_sz != rxbuf->ofld_pld_sz)) {
		app = (struct tx_stats_append *)(rxbuf->vir_addr + rxbuf->ofld_pld_sz);
		while (rxbuf->ofld_bd_sz > (rxbuf->ofld_pld_sz + app_size)) {
			txreq.mdata.macid = app->macid;
			txreq.mdata.tid = app->tid;
			txreq.tx_pkt_len = app->tx_pkt_len;
			txreq.total_len = app->tx_bytes;
			txreq.mdata.bc = txreq.mdata.mc = 0;
			txreq.mdata.type = RTW_PHL_PKT_TYPE_DATA;

			ops->count_pe_tx_stats(phl_to_drvpriv(phl_info), app);
			phl_tx_statistics(phl_info, &txreq);
			app_size += sizeof(struct tx_stats_append);
			app++;
		}
		ops->pe_tx_force_para(phl_to_drvpriv(phl_info));
	}
}

void phl_pe_recycle_payload(struct phl_info_t *phl_info, u8 dma_ch, u16 wp_seq,
			 u8 txsts, u16 macid)
{
	struct rtw_xmit_req treq;

	treq.tx_time = _os_get_cur_time_ms();
	treq.mdata.macid = macid;
	treq.mdata.dma_ch = dma_ch;

	_phl_wp_rpt_statistics(phl_info, wp_seq, txsts, &treq);
}
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
			 ...
			 u8 txsts, u8 code)
{
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
	if (phl_info->txq_offload == 2 && (dma_ch < 4))
		dma_ch = 0;
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
	...
	_phl_wp_rpt_statistics(phl_info, wp_seq, txsts, treq);
	_phl_wp_rpt_chk_txsts(phl_info, wp_seq, txsts, treq);
#ifdef CONFIG_VW_REFINE
	...
	if (RTW_PHL_STATUS_SUCCESS != sts) {
		...
		if (0 == dma_ch) {
			...
			phl_info->fw_ptr = (phl_info->fw_ptr + 1) % max_seq_nr;
		}
		if (TX_STATUS_TX_DONE == txsts)
...
#ifdef CONFIG_RTL_WIFI_OFFLOAD
		/* wp_seq #1 ~ #(WP_MAX_SEQ_NUMBER-MAX_RSVD_SEQ-1) is owned by WFO */
		if (((phl_info->txq_offload && (dma_ch == 0)) ||
			(phl_info->txq_offload == 2 && (dma_ch < 4))) &&
			wp_seq > 0 && wp_seq <= (WP_MAX_SEQ_NUMBER - 1 - MAX_RSVD_SEQ)) {
			phl_pe_recycle_payload(phl_info, dma_ch, wp_seq, txsts, macid);
		} else
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
		if (false == sw_retry) {
...
#endif
#ifdef CONFIG_RTL_WIFI_OFFLOAD
		_phl_pe_tx_statistics(phl_info, phl_rx);
#endif /* CONFIG_RTL_WIFI_OFFLOAD */
...
static enum rtw_phl_status phl_rx_pcie(struct phl_info_t *phl_info)
{
	...
	for (ch = 0; ch < hci_info->total_rxch_num; ch++) {
		...
		if (idle_rxbuf_cnt == 0) {
#ifdef DEBUG_PHL_RX
			phl_com->rx_stats.idle_rxbuf_empty++;
#else
...
#ifdef DEBUG_PHL_RX
			phl_com->rx_stats.idle_rxbuf_less++;
#else
		...
		for (i = 0; i < rxcnt; i++) {
			...
			#ifdef CONFIG_RTW_HW_RX_AMSDU_CUT
			/* Check pending AMSDU and process it */
			if (phl_info->pending_amsdu) {
				struct rtw_phl_rx_pkt *amsdu = phl_info->pending_amsdu;
				if (amsdu->r.mdata.last_msdu) {
					amsdu->r.mdata.dma_ch = ch;
					phl_rx_process_pcie(phl_info, amsdu);
					#ifdef DEBUG_PHL_RX
					phl_com->rx_stats.amsdu_cut_proc_pending++;
					#endif
				} else {
					PHL_ERR("Unfinished A-MSDU!\n");
					phl_recycle_rx_buf(phl_info, amsdu);
					#ifdef DEBUG_PHL_RX
					phl_com->rx_stats.amsdu_cut_unfinished++;
					#endif
				}
				phl_info->pending_amsdu = NULL;
			}
...
static void phl_handle_pe_d2d_pkt(struct phl_info_t *phl_info,
	struct rtw_phl_pe_rxbd *entry)
{
	struct rtw_phl_evt_ops *ops = &phl_info->phl_com->evt_ops;
	u8 subtype;

	subtype = PE_GET_FIELD(entry->word0.wrd, PE_RXBD_SUBTYPE);
	switch (subtype) {
	case PE_PKT_SUBTYPE_PS:
		ops->process_pwr_state(phl_to_drvpriv(phl_info),
				entry->word0.ps_cmd.pwr, entry->word1.ps_cmd.macid,
				entry->word1.ps_cmd.seq);
		break;
	case PE_PKT_SUBTYPE_TIM_INDIC:
		ops->process_tim_indic(phl_to_drvpriv(phl_info), entry->word1.ps_cmd.macid);
		break;
	case PE_PKT_SUBTYPE_OM_CONTROL:
		ops->process_om_control(phl_to_drvpriv(phl_info), entry->word0.om_cmd.macid,
			le32_to_cpu(entry->word1.om_cmd.ht_control));
		break;
	default:
		PHL_ERR("%s : Unknown PE pkt sub type (%d)!\n", __FUNCTION__, subtype);
		break;
	}
}
...
struct rtw_rx_buf *convert_to_rxbuf(struct phl_info_t *phl_info,
		struct rtw_rx_buf_ring *rx_buf_ring, u32 phy_addr,
		u32 bd_sz, u16 buf_len)
{
	struct rtw_phl_com_t *phl_com = phl_info->phl_com;
	...
	rxbuf = query_idle_rx_buf(phl_info, rx_buf_ring);
	if (NULL == rxbuf) {
		rxbuf = query_empty_rx_buf(phl_info, rx_buf_ring);
		if (NULL == rxbuf) {
			PHL_TRACE(COMP_PHL_DBG, _PHL_WARNING_,
				"[WARNING] there is no resource for rx bd refill setting\n");
			return NULL;
		}
		rxbuf->vir_addr = _os_pkt_buf_alloc_rx(mem_d,
							&rxbuf->phy_addr_l,
							&rxbuf->phy_addr_h,
							buf_len,
							&rxbuf->os_priv);
		if (NULL == rxbuf->vir_addr) {
			PHL_TRACE(COMP_PHL_DBG, _PHL_WARNING_,
				"[WARNING] there is no resource for rx skb\n");
			enqueue_empty_rx_buf(phl_info, rx_buf_ring, rxbuf);
			phl_com->rx_stats.rx_empty_buf_alloc_fail++;
			return NULL;
		}
		rxbuf->cache = true;
		rxbuf->buf_len = _os_pkt_get_rx_buf_sz(rxbuf->os_priv);
		rxbuf->dynamic = 1;
		rxbuf->reuse = false;
	}
#else
	...
	buf_ptr = (void *)phys_to_virt(phy_addr);
	rxbuf->pe_phy_addr = phy_addr;
	...
	_os_convert_buf_to_pkt(rxbuf->vir_addr, buf_ptr, bd_sz);
	rxbuf->ofld_bd_sz = bd_sz;

	return rxbuf;
}

enum rtw_phl_status phl_get_single_pe_rx(struct phl_info_t *phl_info,
		struct rtw_phl_pe_rx_ring *pe_rx_ring, u32 phy_addr, u32 bd_sz,
		struct rtw_phl_rx_pkt **pphl_rx)
{
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_FAILURE;
	...
	struct hci_info_t *hci_info = (struct hci_info_t *)phl_info->hci;
	struct rtw_phl_com_t *phl_com = phl_info->phl_com;
	/* always use RXQ buffer because RPQ buffer is too small */
	u16 buf_size = phl_info->phl_pe_rx_ring.buf_sz;
	u16 ch = 0;
	...
	if (bd_sz > buf_size)
	{
		RTW_ERR("WFO rx buffer too small (%d %d)\n", bd_sz, buf_size);
		goto drop;
	}

	phl_rx = rtw_phl_query_phl_rx(phl_info);
	rxbuf = convert_to_rxbuf(phl_info, rx_buf_ring, phy_addr, bd_sz, buf_size);
	...
	do {
		if (NULL == phl_rx) {
			PHL_TRACE(COMP_PHL_DBG, _PHL_INFO_,
				"%s(%d) phl_rx out of resource, ch=%d \n",
				__func__, __LINE__, ch);
			pstatus = RTW_PHL_STATUS_FAILURE;
			break;
		}
		...
		if (NULL == rxbuf) {
			PHL_TRACE(COMP_PHL_DBG, _PHL_INFO_,
				"%s(%d) [WARNING] queried NULL rxbuf, ch=%d \n",
				__func__, __LINE__, ch);
			pstatus = RTW_PHL_STATUS_FAILURE;
			break;
		}
		...
		if (true != rtw_hal_handle_rxbd_info(phl_info->hal,
				rxbuf->vir_addr, &buf_size))
		{
			phl_rx->rxbuf_ptr = (u8 *)rxbuf;
			goto drop;
		}

		phl_com->rx_stats.rx_bd_ok++;

		phl_rx->rxbuf_ptr = (u8 *)rxbuf;
		rxbuf->ofld_pld_sz = buf_size;
	...
	if (RTW_PHL_STATUS_SUCCESS != pstatus) {
		if (NULL != rxbuf) {
			// phl_recycle_rx_buf
			free_pe_rx_buf(phl_info, (void *)rxbuf);
			enqueue_idle_rx_buf(phl_info, rx_buf_ring, rxbuf);
		}
...
static enum rtw_phl_status phl_pe_rx(struct phl_info_t *phl_info,
		struct rtw_phl_pe_rx_ring *pe_rx_ring)
{
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
	struct hci_info_t *hci_info = (struct hci_info_t *)phl_info->hci;
	struct rtw_rx_buf_ring *rx_buf_ring = NULL;
	struct rtw_phl_rx_pkt *phl_rx = NULL;
	// struct rtw_phl_pe_rx_ring *pe_rx_ring = NULL;
	struct rtw_phl_pe_rx_rw *rw_ptr = NULL;
	struct rtw_phl_pe_rxbd *entry = NULL;
	u8 ch = 0, pkt_type;
	u16 rxcnt = 0, core_idx = 0;
	u32 bd_addr = 0, bd_sz = 0;
	#ifdef DEBUG_PHL_RX
	struct rtw_phl_com_t *phl_com = phl_info->phl_com;
	#endif /* DEBUG_PHL_RX */
	...
	rx_buf_ring = (struct rtw_rx_buf_ring *)hci_info->rxbuf_pool;
	// pe_rx_ring = &phl_info->phl_pe_rx_ring;
	rw_ptr = pe_rx_ring->rw_ptr;
	entry = pe_rx_ring->entry;

#ifdef CONFIG_DYNAMIC_RX_BUF
	/* The empty rxbuf (w/o available buffer) happen only on RTW_RX_TYPE_WIFI  */
#ifdef WFO_DYNAMIC_RXBUF
	if (phl_info->rxq_offload == 0)
#endif
		refill_empty_rx_buf(phl_info, &rx_buf_ring[0]);
#endif

	rxcnt = phl_pe_rx_res_query(rw_ptr->core_idx,
		rw_ptr->pe_idx, pe_rx_ring->entry_num);
#ifdef DEBUG_PHL_RX
	phl_com->rx_stats.rx_pe_cnt += rxcnt;
#endif

	core_idx = rw_ptr->core_idx;
	while (rxcnt) {
		pkt_type = PE_GET_FIELD(entry[core_idx].word0.wrd, PE_RXBD_TYPE);
		switch (pkt_type) {
		case PE_PKT_TYPE_DOE2DRV:
			phl_handle_pe_d2d_pkt(phl_info, &entry[core_idx]);
			break;
		case PE_PKT_TYPE_DATA:
			bd_sz = entry[core_idx].word0.wifi_rx.buf_sz;
			bd_addr = entry[core_idx].word1.wifi_rx.buf_addr;
			pstatus = phl_get_single_pe_rx(phl_info, pe_rx_ring, bd_addr, bd_sz, &phl_rx);
			if (RTW_PHL_STATUS_SUCCESS != pstatus) {
#ifdef DEBUG_PHL_RX
				phl_com->rx_stats.rx_pe_no_proc++;
#endif
				break;
			}
#ifdef DEBUG_PHL_RX
			phl_com->rx_stats.rx_pe_proc++;
#endif
			phl_rx_process_pcie(phl_info, phl_rx);
			break;
		default:
			PHL_ERR("%s : Unknown PE pkt type (%d)!\n", __FUNCTION__, pkt_type);
			break;
		}

		core_idx = (core_idx + 1) % pe_rx_ring->entry_num;
		rxcnt--;
	}

	rw_ptr->core_idx = core_idx;

#ifdef PHL_RX_BATCH_IND
	if (phl_info->rx_new_pending) {
		_phl_indic_new_rxpkt(phl_info);
	}
#endif

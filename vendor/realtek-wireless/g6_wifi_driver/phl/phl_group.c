...
void phl_grp_dump_info_ulru(void *phl, struct rtw_wifi_role_t *wrole)
{
	...
	for (i = 0; i < grp_obj->ulru.grp_num; i++) {
		printk("ulru_grp: %d (latency grp:%d)\n", grp_obj->ulru.grp[i].tbl_idx, grp_obj->ulru.grp[i].latency);
...
enum rtw_phl_status rtw_phl_ru_fill_ul_grp_info(struct phl_info_t *phl_info, u8 gidx, u8 id)
{
	enum rtw_phl_status pstatus;
	struct rtw_phl_com_t *phl_com = phl_info->phl_com;
	struct ru_grp_table *rugrptable = &phl_com->rugrptable[id];
...
enum rtw_phl_status rtw_phl_ru_fill_ru_fixmode_mib(struct phl_info_t *phl_info, struct rtw_phl_stainfo_t *psta, u8 id)
{
	struct rtw_phl_com_t *phl_com = phl_info->phl_com;
	struct ru_grp_table *rugrptable = &phl_com->rugrptable[id];
	struct ru_common_ctrl *ru_ctrl = &rugrptable->ru_ctrl;
...
rtw_phl_fill_dlru_grp_tbls(struct phl_info_t *phl_info, struct rtw_wifi_role_t *wrole)
{
	...
	for (gidx = 0; gidx < grp_obj->dlru.grp_num; gidx++)
	{
		...
		for(i=0;i<grp_obj->dlru.grp[gidx].sta_cnt;i++) {
			rtw_phl_ru_fill_ru_fixmode_mib(phl_info, grp_obj->dlru.grp[gidx].sta[i], wrole->id);
		...
		u8 gidx, struct rtw_phl_stainfo_t *psta)
{
	...
	if(ru_ctrl->ul_psd){
		...
		psd_e = rtw_phl_cal_psd_e(phl_info, avg_rssi_m, wrole->id);
	}
...
rtw_phl_fill_ulru_grp_tbls(struct phl_info_t *phl_info, struct rtw_wifi_role_t *wrole)
{
	...
	for (gidx = 0; gidx < grp_obj->ulru.grp_num; gidx++)
	{
		...
		pstatus = rtw_phl_ru_fill_ul_grp_info(phl_info, gidx, wrole->id);
		if(pstatus != RTW_PHL_STATUS_SUCCESS)
			return pstatus;
	}
	return pstatus;
...
enum rtw_phl_status rtw_phl_ru_fill_ul_dlru_tbl(struct phl_info_t *phl_info, struct rtw_wifi_role_t *wrole,
...
enum rtw_phl_status rtw_phl_ru_fill_ul_dl_grp_info(struct phl_info_t *phl_info, u8 gidx, u8 rugrpid, u8 id)
{
	enum rtw_phl_status pstatus;
	struct rtw_phl_com_t *phl_com = phl_info->phl_com;
	struct ru_grp_table *rugrptable = &phl_com->rugrptable[id];
...
enum rtw_phl_status
rtw_phl_fill_ul_dlru_grp_tbls(struct phl_info_t *phl_info, struct rtw_wifi_role_t *wrole)
{
	struct phl_grp_obj *grp_obj = phl_info->grp_obj;
	u8 gidx, i;
	enum rtw_phl_status pstatus;
	...
	{
		for(i=0;i<grp_obj->ulru.grp[gidx].sta_cnt;i++) {
			pstatus = rtw_phl_ru_fill_ul_dlru_tbl(phl_info, wrole, gidx, i, grp_obj->ulru.grp[gidx].sta[i]);
			if(pstatus != RTW_PHL_STATUS_SUCCESS)
				return pstatus;
		}
		...
		for(i=0;i<grp_obj->ulru.grp[gidx].sta_cnt;i++) {
			rtw_phl_ru_fill_ru_fixmode_mib(phl_info, grp_obj->ulru.grp[gidx].sta[i], wrole->id);
			//rtw_phl_ru_fill_ul_dl_grp_info(phl_info, gidx, grp_obj->ulru.grp[gidx].tbl_idx[i], i);
		}

		rtw_phl_ru_fill_ul_dl_grp_info(phl_info, gidx, grp_obj->ulru.grp[gidx].tbl_idx, wrole->id);
	 }
	 return pstatus;
}
	...
	if(!(phl_com->dev_cap.dlul_group_mode & BIT4)){
		if (ru_ctrl->GRP_UL_ON){
			...
			if (rtw_phl_check_ulru_grp_tbls_change(phl_info) || ru_ctrl->GRP_FORCE_FILL_UL_TBL) {
				...
				if (ru_ctrl->UL_mode & BIT(1))
					rtw_phl_fill_ul_dlru_grp_tbls(phl_info, wrole);
...
void phl_init_ru_ctrl(struct phl_info_t *phl_info, struct ru_common_ctrl *ru_ctrl){
	...
	ru_ctrl->DL_mode = 2; /* BIT0(1): SU-BA; BIT1(2): use trigger */
	ru_ctrl->trigger_THRD = 0x2;

...
{
	phl_com->dbg_cfg.placeholder = 0;
#ifdef CONFIG_ETHER_PKT_AGG
	phl_com->dbg_cfg.pe_ofld_cfg.is_pe_ofld_dbg = 0;
...
static enum rtw_phl_status phl_var_deinit(struct phl_info_t *phl_info)
{
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_SUCCESS;
					...
					struct rtw_ic_info *ic_info)
{
	...
	*phl = phl_info;
	...
	phl_info->nhm_info = _os_mem_alloc(drv_priv, sizeof(struct watchdog_nhm_report));
	if (phl_info->nhm_info == NULL) {
		phl_status = RTW_PHL_STATUS_RESOURCE;
		PHL_ERR("alloc phl_info->nhm_info failed\n");
		goto error_phl_mem;
	}
	_os_mem_set(drv_priv, phl_info->nhm_info, 0, sizeof(struct watchdog_nhm_report));
...
void rtw_phl_deinit(void *phl)
{
	...
	if (phl_info) {
		...
		phl_com_deinit(phl_info, phl_info->phl_com);
		phl_regulation_deinit(drv_priv, phl_info);
		_os_mem_free(drv_priv, phl_info->nhm_info, sizeof(struct watchdog_nhm_report));
...
static enum rtw_phl_status _phl_unmap_pkt_ring(struct phl_info_t *phl_info)
{
	enum rtw_phl_status sts = RTW_PHL_STATUS_SUCCESS;

	if (phl_info->phl_pe_rx_ring.rw_ptr)
		_os_iounmap(phl_info->phl_pe_rx_ring.rw_ptr);
	phl_info->phl_pe_rx_ring.rw_ptr = NULL;

	if (phl_info->phl_pe_rx_ring.entry)
		_os_iounmap(phl_info->phl_pe_rx_ring.entry);
	phl_info->phl_pe_rx_ring.entry = NULL;

	if (phl_info->phl_pe_rp_ring.rw_ptr)
		_os_iounmap(phl_info->phl_pe_rp_ring.rw_ptr);
	phl_info->phl_pe_rp_ring.rw_ptr = NULL;

	if (phl_info->phl_pe_rp_ring.entry)
		_os_iounmap(phl_info->phl_pe_rp_ring.entry);
	phl_info->phl_pe_rp_ring.entry = NULL;

	if (phl_info->phl_pe_tx_ring.rw_ptr)
		_os_iounmap(phl_info->phl_pe_tx_ring.rw_ptr);
	phl_info->phl_pe_tx_ring.rw_ptr = NULL;

	if (phl_info->phl_pe_tx_ring.entry)
		_os_iounmap(phl_info->phl_pe_tx_ring.entry);
	phl_info->phl_pe_tx_ring.entry = NULL;

	return sts;
}

enum rtw_phl_status _phl_map_pkt_ring(void *phl,
	u32 rx_rw_base, u32 rx_data_base, u32 rx_ring_sz,
	u32 rp_rw_base, u32 rp_data_base, u32 rp_ring_sz,
	u32 tx_rw_base, u32 tx_data_base, u32 tx_ring_sz)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	enum rtw_phl_status sts = RTW_PHL_STATUS_FAILURE;

	/* RXQ */
	phl_info->phl_pe_rx_ring.entry_num = rx_ring_sz;
	phl_info->phl_pe_rx_ring.ch_idx = 0;
	phl_info->phl_pe_rx_ring.buf_sz = rtw_hal_get_rxbuf_size(phl_info->hal, 0);
	phl_info->phl_pe_rx_ring.rw_ptr = (struct rtw_phl_pe_rx_rw *)
		_os_ioremap(rx_rw_base, sizeof(struct rtw_phl_pe_rx_rw));

	phl_info->phl_pe_rx_ring.entry = (struct rtw_phl_pe_rxbd *)
		_os_ioremap(rx_data_base,
				sizeof(struct rtw_phl_pe_rxbd) * rx_ring_sz);

	PHL_INFO("set rx_rw_base 0x%08X, rx_data_base 0x%08X, entry_num %d\n",
				rx_rw_base, rx_data_base, rx_ring_sz);

	/* RPQ */
	phl_info->phl_pe_rp_ring.entry_num = rp_ring_sz;
	phl_info->phl_pe_rp_ring.ch_idx = 1;
	phl_info->phl_pe_rp_ring.buf_sz = rtw_hal_get_rxbuf_size(phl_info->hal, 1);
	phl_info->phl_pe_rp_ring.rw_ptr = (struct rtw_phl_pe_rx_rw *)
		_os_ioremap(rp_rw_base, sizeof(struct rtw_phl_pe_rx_rw));

	phl_info->phl_pe_rp_ring.entry = (struct rtw_phl_pe_rxbd *)
		_os_ioremap(rp_data_base,
				sizeof(struct rtw_phl_pe_rxbd) * rp_ring_sz);

	PHL_INFO("set rp_rw_base 0x%08X, rp_data_base 0x%08X, entry_num %d\n",
				rp_rw_base, rp_data_base, rp_ring_sz);

	/* ACH0 */
	phl_info->phl_pe_tx_ring.entry_num = tx_ring_sz;
	phl_info->phl_pe_tx_ring.rw_ptr = (struct rtw_phl_pe_tx_rw *)
		 _os_ioremap(tx_rw_base, sizeof(struct rtw_phl_pe_tx_rw));

	phl_info->phl_pe_tx_ring.entry = (struct rtw_phl_pe_txbd *)
		_os_ioremap(tx_data_base,
				sizeof(struct rtw_phl_pe_txbd) * tx_ring_sz);

	PHL_INFO("set tx_rw_base 0x%08X, tx_data_base 0x%08X, entry_num %d\n",
				tx_rw_base, tx_data_base, tx_ring_sz);

	return RTW_PHL_STATUS_SUCCESS;
}

enum rtw_phl_status rtw_phl_check_ring_info(void *phl,
	u32 rx_rw_base, u32 rx_data_base, u32 rx_ring_sz,
	u32 rp_rw_base, u32 rp_data_base, u32 rp_ring_sz,
	u32 tx_rw_base,	u32 tx_data_base, u32 tx_ring_sz)
{
	...
	if (rx_rw_base == 0 || rp_rw_base == 0 || tx_rw_base == 0)
		return _phl_unmap_pkt_ring(phl_info);
	else
		return _phl_map_pkt_ring(phl_info,
			rx_rw_base, rx_data_base, rx_ring_sz,
			rp_rw_base, rp_data_base, rp_ring_sz,
			tx_rw_base, tx_data_base, tx_ring_sz);
}

void rtw_phl_set_offload(void *phl, u8 mode, u8 value)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	...
	if (mode & RTL_WFO_RX_OFLD) {
		phl_info->rxq_offload = value;
		hal_com->rxq_offload = value;
	}

	if (mode & RTL_WFO_TX_OFLD) {
		phl_info->txq_offload = value;
		hal_com->txq_offload = value;
		max_seq = (value) ? MAX_RSVD_SEQ : WP_MAX_SEQ_NUMBER;

		for (i=0; i<=max_seq; i++)
			phl_info->free_wp[i] = WP_MAX_SEQ_NUMBER - max_seq + i;

		phl_info->fr_ptr = 0;
		phl_info->fw_ptr = 0;

		if (value == 2) { /* WMM mode, reserve wp_seq from ACH0 ~ ACH3 */
			for (i=0; i<4; i++)
				wd_ring[i].wp_seq = (value) ? \
						(WP_MAX_SEQ_NUMBER - MAX_RSVD_SEQ) : 1;
		}
	}
}

void rtw_phl_dump_entry(void *phl, u32 value)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct rtw_phl_com_t *phl_com = phl_info->phl_com;
	struct phl_rx_stats *rx_stats = &phl_com->rx_stats;
	struct phl_tx_stats *tx_stats = &phl_com->tx_stats;
	struct rtw_phl_pe_rx_ring *rx_ring = &phl_info->phl_pe_rx_ring;
	struct rtw_phl_pe_rx_ring *rp_ring = &phl_info->phl_pe_rp_ring;
	struct rtw_phl_pe_tx_ring *tx_ring = &phl_info->phl_pe_tx_ring;

	if (2 == value) {
		PHL_PRINT("rxq_offload %u txq_offload %u\n",
			phl_info->rxq_offload, phl_info->txq_offload);

		PHL_PRINT("RX: all %u wifi %u wp %u ppdu %u c2h %u\n",
			rx_stats->rx_type_all,
			rx_stats->rx_type_wifi, rx_stats->rx_type_wp,
			rx_stats->rx_type_ppdu, rx_stats->rx_type_c2h);

		PHL_PRINT("\t rx_cnt %u rx_bd_ok %u rx_buf_ok %u "
			"rx_empty_buf_alloc_fail %u rdu %u\n",
			rx_stats->rx_cnt, rx_stats->rx_bd_ok,
			rx_stats->rx_buf_ok, rx_stats->rx_empty_buf_alloc_fail,
			rx_stats->rx_rdu_cnt);

		PHL_PRINT("\t pe_proc %u pe_no_proc %u\n",
			rx_stats->rx_pe_proc,
			rx_stats->rx_pe_no_proc);

		if (phl_info->rxq_offload) {
			PHL_PRINT("\t rxq: core_idx %u pe_idx %u pe_isr %u\n",
				rx_ring->rw_ptr->core_idx,
				rx_ring->rw_ptr->pe_idx,
				rx_stats->rx_pe_isr);
			PHL_PRINT("\t rpq: core_idx %u rp_pe_idx %u\n",
				rp_ring->rw_ptr->core_idx,
				rp_ring->rw_ptr->pe_idx);
		}

		if (phl_info->txq_offload) {
			PHL_PRINT("TX: core_idx %u pe_idx %u pe_isr %u\n",
				tx_ring->rw_ptr->core_idx,
				tx_ring->rw_ptr->pe_idx,
				tx_stats->tx_pe_isr);
		}

		PHL_PRINT("WD: bd_ok %u bd_err %u trigger_txstart_ok %u, pe_tx_cnt %u\n",
			phl_com->update_txbd_ok,
			phl_com->update_txbd_fail,
			phl_com->trigger_txstart_ok,
			tx_stats->pe_tx_cnt);

		PHL_PRINT("WP: fr_ptr %u fw_ptr %u\n",
			phl_info->fr_ptr,
			phl_info->fw_ptr);
	}
}
...
void rtw_phl_cmd_wd_info(void *phl, u32 value)
{
        ...
        if ( value == 1 ) {
		PHL_ERR("wd_info: idle:%d busy:%d pending:%d fw_ptr:%d fr_ptr:%d pretx_fail:%d phltx_cnt:%u\n", \
...
#endif
	phl_info->tx_req_enq = 0;
	phl_info->tx_req_deq = 0;
...
enum rtw_phl_status rtw_phl_rf_on(void *phl)
{
	...
	if (sts != RTW_PHL_STATUS_SUCCESS)
		PHL_ERR("[%s]phl_data_ctrler(PHL_DATA_CTL_SW_TX_RESUME) return %d\n", __func__, sts);
	...
	if (hal_status == RTW_HAL_STATUS_MAC_INIT_FAILURE) {
		phl_status = RTW_PHL_STATUS_HAL_INIT_FAILURE;
		PHL_ERR("[%s]rtw_hal_start return %d\n", __func__, hal_status);
		goto error_hal_start;
	} else if (hal_status == RTW_HAL_STATUS_BB_INIT_FAILURE) {
		phl_status = RTW_PHL_STATUS_HAL_INIT_FAILURE;
		PHL_ERR("[%s]rtw_hal_start return %d\n", __func__, hal_status);
	...
	printk("tx/rx pause = %d/%d\n", phl_datapath_chk_trx_pause(phl_info, PHL_CTRL_TX), phl_datapath_chk_trx_pause(phl_info, PHL_CTRL_RX));

	rtw_phl_trx_force_resume(phl_info);
...
enum rtw_phl_status rtw_phl_rf_off(void *phl)
{
	...
	sts = phl_data_ctrler(phl_info, &ctl, NULL);
	if (sts != RTW_PHL_STATUS_SUCCESS)
		PHL_ERR("[%s]phl_data_ctrler(PHL_DATA_CTL_SW_TX_PAUSE) return %d\n", __func__, sts);
...
enum rtw_phl_status rtw_phl_interrupt_handler(void *phl)
{
	...
	/* schedule tx process */
	if (phl_info->tx_req_enq != phl_info->tx_req_deq)
	...
	if (!from_flash){
		switch(rtw_phl_get_phyband_on_dev(phl_info->dev_id)){
			case BAND_ON_5G:
				...
				DBGP("BAND_ON_5G, set_rfe_type=%d\n", set_rfe_type);
				break;
			case BAND_ON_24G:
				set_rfe_type = rtw_rfe_type_2g;
				DBGP("BAND_ON_24G, set_rfe_type=%d\n", set_rfe_type);
				break;
			default:
...
void rtw_gen_fem_name(void *phl, u8* fem_name){
	...
	switch (band){
		case BAND_ON_5G:
			_os_strcpy(fem_name, rtw_fem_name_5g);
			break;
		case BAND_ON_24G:
			_os_strcpy(fem_name, rtw_fem_name_2g);
			break;
...
enum rtw_phl_status rtw_phl_set_trx_status(void *phl, u8 type, u8 is_pause)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct phl_hci_trx_ops *ops = phl_info->hci_trx_ops;
	enum rtw_phl_status phl_status = RTW_PHL_STATUS_FAILURE;
	...
	if (is_pause) {
		...
		ops->trx_resume(phl_info, type);

	return phl_status;
}

enum rtw_phl_status rtw_phl_power_switch(void *phl, u8 on_off)
{
	enum rtw_hal_status hstatus = RTW_HAL_STATUS_SUCCESS;
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;

	phl_module_stop(phl_info);
	hstatus = rtw_hal_power_switch(phl_info->phl_com, phl_info->hal, on_off);
	if (hstatus != RTW_HAL_STATUS_SUCCESS)
		return RTW_PHL_STATUS_FAILURE;

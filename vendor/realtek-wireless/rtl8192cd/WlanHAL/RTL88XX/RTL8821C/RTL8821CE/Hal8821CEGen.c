...
Abstract:
	Defined RTL8821CE HAL Function
...
)
{
    ...
    // TODO: Filen, check 8821C setting
...
//???			PWR_INTF_PCI_MSK, rtl8821C_card_enable_flow))
    {
...
printk("%s %d\n", __FUNCTION__, __LINE__);
    ...
    pHalFunc->ResetHWForSurpriseHandler     = ResetHWForSurprise8821CE;
...
//    pHalFunc->ReleaseShortCutTxDescHandler  = ReleaseShortCutTxDesc88XX;
    ...
    // TODO: 8821C Power Tracking should be done
    pHalFunc->TXPowerTrackingHandler        = TXPowerTracking_ThermalMeter_Tmp8821C;
    pHalFunc->PHYSSetRFRegHandler           = PHY_SetRFReg_88XX_AC; //config_phydm_write_rf_reg_8822b; 
    pHalFunc->PHYQueryRFRegHandler          = PHY_QueryRFReg_8821; //config_phydm_read_rf_reg_8822b;
    pHalFunc->IsBBRegRangeHandler           = IsBBRegRange88XX_V1;
    pHalFunc->PHYSetSecCCATHbyRXANT         = PHY_Set_SecCCATH_by_RXANT_8821C;
    pHalFunc->PHYSpurCalibration            = phy_SpurCalibration_8821C;
	...
	)
{
	...
	if (pDM_Odm->SupportInterface == ODM_ITRF_USB) {
		if (*pDM_Odm->hub_usb_mode == 2) {/*USB3.0*/
			if (pDM_Odm->RFType == RF_4T4R)
				Ntx = 3;
			else if (pDM_Odm->RFType == RF_3T3R)
				Ntx = 2;
			else
				Ntx = 1;
		} else if (*pDM_Odm->hub_usb_mode == 1)	/*USB 2.0 always 2Tx*/
			Ntx = 1;
		else
			Ntx = 1;
	} else
	...
	)
{
	...
	if (pDM_Odm->RFType == RF_4T4R)
		...
		Nrx = 2;
	else if (pDM_Odm->RFType == RF_2T2R)
		Nrx = 1;
	else if (pDM_Odm->RFType == RF_2T3R)
		Nrx = 2;
	else if (pDM_Odm->RFType == RF_2T4R)
		Nrx = 3;
	else if (pDM_Odm->RFType == RF_1T1R)
		Nrx = 0;
	else if (pDM_Odm->RFType == RF_1T2R)
		Nrx = 1;
	else
		Nrx = 0;
	...
	)
{
	...
	u1Cyte			Head_Page = 0x7FE;
	BOOLEAN			bSendBeacon = FALSE;
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
	u1Cyte			TxPageBndy = LAST_ENTRY_OF_TX_PKT_BUFFER_8814A; /*default reseved 1 page for the IC type which is undefined.*/
	...
	Adapter->HalFunc.GetHalDefVarHandler(Adapter, HAL_DEF_TX_PAGE_BOUNDARY, (pu1Cyte)&TxPageBndy);
	...
	PlatformEFIOWrite1Cyte(Adapter, REG_FIFOPAGE_CTRL_2_8814A, Head_Page);
	...
	/*0x204[11:0]	Beacon Head for TXDMA*/
	PlatformEFIOWrite1Cyte(Adapter, REG_FIFOPAGE_CTRL_2_8814A, TxPageBndy);
	...
	)
{
	...
	ODM_Write1Cyte(pDM_Odm, 0x1680 , 0); /*MU-MIMO Control as default value*/

	/* Set MU NDPA rate & BW source */
	/* 0x42C[30] = 1 (0: from Tx desc, 1: from 0x45F) */
	u1bTmp = ODM_Read1Byte(pDM_Odm, 0x42C);
	ODM_Write1Byte(pDM_Odm, REG_TXBF_CTRL_8821C, (u1bTmp|BIT6)); //eric-mu
	/* 0x45F[7:0] = 0x10 (Rate=OFDM_6M, BW20) */
	ODM_Write1Byte(pDM_Odm, REG_NDPA_OPT_CTRL_8821C, 0x10);
	...
	ODM_SetBBReg(pDM_Odm, 0x19E0 , BIT5|BIT4, 3); /*8821C grouping method*/

	if (pDM_Odm->RFType == RF_2T2R) { /*2T2R*/
		PHYDM_DBG(pDM_Odm, DBG_TXBF, "%s: RFType is 2T2R\n", __func__);
		config_phydm_trx_mode_8821c(pDM_Odm, 3, 3, 1);/*Tx2path*/
	}
}
	...
	)
{
	...
	PHYDM_DBG(pDM_Odm, DBG_TXBF, ODM_DBG_TRACE, ("[%s] BFerBFeeIdx=%d, BFerIdx=%d, BFeeIdx=%d\n", __func__, BFerBFeeIdx, BFerIdx, BFeeIdx));
	/*************SU BFer Entry Init*************/
	if ((pBeamformingInfo->beamformer_su_cnt > 0) && (BFerIdx < BEAMFORMER_ENTRY_NUM)) {
		PHYDM_DBG(pDM_Odm, DBG_TXBF, ODM_DBG_TRACE, ("[%s]SU BFer Entry Init\n", __func__));
		...
		ODM_Write1Byte(pDM_Odm, REG_SND_PTCL_CTRL_8821C, 0xDB);	
		...
		if (pBeamformerEntry->su_reg_index == 0) {
			for (i = 0; i < 6 ; i++)
				ODM_Write1Byte(pDM_Odm, (REG_ASSOCIATED_BFMER0_INFO_8821C+i), pBeamformerEntry->MacAddr[i]);
		} else {
			for (i = 0; i < 6 ; i++)
				ODM_Write1Byte(pDM_Odm, (REG_ASSOCIATED_BFMER1_INFO_8821C+i), pBeamformerEntry->MacAddr[i]);
		}

		/*CSI report parameters of Beamformer*/
		Nc_index = halTxbf8821C_GetNrx(pDM_Odm);	/*for 8814A Nrx = 3(4 Ant), min=0(1 Ant)*/
		...
		CSI_Param = (u1Cyte)((coefficientsize<<10)|(codebookinfo<<8)|(grouping<<6)|(Nr_index<<3)|(Nc_index));

		if (BFerIdx == 0)
			ODM_Write1Cyte(pDM_Odm, 0x6F4, CSI_Param);
		else
			ODM_Write1Cyte(pDM_Odm, 0x6F4+2, CSI_Param);
		/*ndp_rx_standby_timer, 8814 need > 0x56, suggest from Dvaid*/
		ODM_Write1Byte(pDM_Odm, REG_SND_PTCL_CTRL_8821C+3, 0x70);
	...
	if ((pBeamformingInfo->beamformee_su_cnt > 0) && (BFeeIdx < BEAMFORMEE_ENTRY_NUM)) {
		PHYDM_DBG(pDM_Odm, DBG_TXBF, ODM_DBG_TRACE, ("[%s]SU BFee Entry Init\n", __func__));
		...
		halTxbf8821C_RfMode(pDM_Odm, pBeamformingInfo, BFeeIdx);
		...
		if (pBeamformeeEntry->su_reg_index == 0) {	
			ODM_Write1Cyte(pDM_Odm, REG_TXBF_CTRL_8821C, STAid);	
			ODM_Write1Byte(pDM_Odm, REG_TXBF_CTRL_8821C+3, ODM_Read1Byte(pDM_Odm, REG_TXBF_CTRL_8821C+3)|BIT4|BIT6|BIT7);
		} else {
			ODM_Write1Cyte(pDM_Odm, REG_TXBF_CTRL_8821C+2, STAid | BIT14 | BIT15 | BIT12);
		}	

		/*CSI report parameters of Beamformee*/
		if (pBeamformeeEntry->su_reg_index == 0) {
			/*Get BIT24 & BIT25*/
			u1Byte	tmp = ODM_Read1Byte(pDM_Odm, REG_ASSOCIATED_BFMEE_SEL_8821C+3) & 0x3;
			
			ODM_Write1Byte(pDM_Odm, REG_ASSOCIATED_BFMEE_SEL_8821C + 3, tmp | 0x60);
			ODM_Write1Cyte(pDM_Odm, REG_ASSOCIATED_BFMEE_SEL_8821C, STAid | BIT9);
		} else		
			ODM_Write1Cyte(pDM_Odm, REG_ASSOCIATED_BFMEE_SEL_8821C+2, STAid | 0xE200);	/*Set BIT25*/
	...
	if ((pBeamformingInfo->beamformer_mu_cnt > 0) && (BFerIdx < BEAMFORMER_ENTRY_NUM)) {
		PHYDM_DBG(pDM_Odm, DBG_TXBF, ODM_DBG_TRACE, ("[%s]MU BFer Entry Init\n", __func__));
		pBeamformerEntry = &pBeamformingInfo->BeamformerEntry[BFerIdx];
		pBeamformerEntry->HwState = BEAMFORM_ENTRY_HW_STATE_ADDING;
		...
		/* Set partial AID */
		ODM_Write1Cyte(pDM_Odm, (REG_ASSOCIATED_BFMER0_INFO_8821C+6), pBeamformerEntry->P_AID);
		...
		u1bTmp = ODM_Read1Byte(pDM_Odm, REG_RXFLTMAP0_8821C+1);
		u1bTmp |= 0x40;
		ODM_Write1Byte(pDM_Odm, REG_RXFLTMAP0_8821C+1, u1bTmp);
		/* Set 0x6A2[5:4] = 1 to NDPA and BF report poll */
		u1bTmp = ODM_Read1Byte(pDM_Odm, REG_RXFLTMAP1_8821C);
		u1bTmp |= 0x30;
		ODM_Write1Byte(pDM_Odm, REG_RXFLTMAP1_8821C, u1bTmp);
		
		/*CSI report parameters of Beamformer*/
		Nc_index = halTxbf8821C_GetNrx(pDM_Odm);	/* Depend on RF type */
	...
	if ((pBeamformingInfo->beamformee_mu_cnt > 0) && (BFeeIdx < BEAMFORMEE_ENTRY_NUM)) {
		PHYDM_DBG(pDM_Odm, DBG_TXBF, ODM_DBG_TRACE, ("[%s]MU BFee Entry Init, RegMUTxCtrl=0x%x\n", __func__, pBeamformingInfo->RegMUTxCtrl));
		pBeamformeeEntry = &pBeamformingInfo->BeamformeeEntry[BFeeIdx];
		pBeamformeeEntry->HwState = BEAMFORM_ENTRY_HW_STATE_ADDING;
		...
		panic_printk("mu_reg[pBeamformeeEntry->mu_reg_index]0x%x = 0x%x\n",	
					mu_reg[pBeamformeeEntry->mu_reg_index], ODM_Read1Cyte(pDM_Odm, mu_reg[pBeamformeeEntry->mu_reg_index]));
		panic_printk("\n +++ \n");
#endif
		...
		PHYDM_DBG(pDM_Odm, DBG_TXBF, ODM_DBG_TRACE, ("@%s, RegMUTxCtrl = 0x%x, user_position_l = 0x%x, user_position_h = 0x%x\n", 
			__func__, pBeamformingInfo->RegMUTxCtrl, user_position_l, user_position_h));

		value16 = ODM_Read1Cyte(pDM_Odm, mu_reg[pBeamformeeEntry->mu_reg_index]);
		value16 &= 0xFE00; /*Clear PAID*/
		value16 |= BIT9; /*Enable MU BFee*/
		value16 |= pBeamformeeEntry->P_AID;
		ODM_Write1Cyte(pDM_Odm, mu_reg[pBeamformeeEntry->mu_reg_index] , value16);
...
#else
		/* 0x42C[30] = 1 (0: from Tx desc, 1: from 0x45F) */
		u1bTmp = ODM_Read1Byte(pDM_Odm, REG_TXBF_CTRL_8821C+3);
		u1bTmp |= 0xD0; // Set bit 28, 30, 31 to 3b'111

#if 0
		panic_printk("eric-mu [%s][%d] u1bTmp = 0x%x\n", __FUNCTION__, __LINE__, u1bTmp);
#else
		ODM_Write1Byte(pDM_Odm, REG_TXBF_CTRL_8821C+3, u1bTmp);
#endif

		/* Set NDPA to 6M*/
		ODM_Write1Byte(pDM_Odm, REG_NDPA_RATE_8821C, 0x4); // 6M

		u1bTmp = ODM_Read1Byte(pDM_Odm, REG_NDPA_OPT_CTRL_8821C);
		u1bTmp &= 0xFC; // Clear bit 0, 1
		ODM_Write1Byte(pDM_Odm, REG_NDPA_OPT_CTRL_8821C, u1bTmp);


		u4bTmp = ODM_Read4Byte(pDM_Odm, REG_SND_PTCL_CTRL_8821C);
		u4bTmp = ((u4bTmp & 0xFF0000FF) | 0x020200); // Set [23:8] to 0x0202
		ODM_Write4Byte(pDM_Odm, REG_SND_PTCL_CTRL_8821C, u4bTmp);	
...
#if 1
		{
			...
			/*Set Ctrl Info*/
			ODM_Write1Cyte(pDM_Odm, 0x140, 0x660);
			...
			PHYDM_DBG(pDM_Odm, DBG_TXBF, ODM_DBG_TRACE, ("@%s, MacId = %d, ctrl_info_offset = 0x%x, mu_reg_index = %x\n", 
	...
	)
{
	...
	if (pBeamformerEntry->BeamformEntryCap == BEAMFORMING_CAP_NONE) {
		ODM_Write1Byte(pDM_Odm, REG_SND_PTCL_CTRL_8821C, 0xD8);	
		if (pBeamformerEntry->is_mu_ap == 0) { /*SU BFer */
			if (pBeamformerEntry->su_reg_index == 0) {	
				ODM_Write4Byte(pDM_Odm, REG_ASSOCIATED_BFMER0_INFO_8821C, 0);
				ODM_Write1Cyte(pDM_Odm, REG_ASSOCIATED_BFMER0_INFO_8821C+4, 0);
				ODM_Write1Cyte(pDM_Odm, 0x6F4, 0);
			} else {
				ODM_Write4Byte(pDM_Odm, REG_ASSOCIATED_BFMER1_INFO_8821C, 0);
				ODM_Write1Cyte(pDM_Odm, REG_ASSOCIATED_BFMER1_INFO_8821C+4, 0);
				ODM_Write1Cyte(pDM_Odm, 0x6F4+2, 0);
			}
	...
	if (pBeamformeeEntry->BeamformEntryCap == BEAMFORMING_CAP_NONE) {
		halTxbf8821C_RfMode(pDM_Odm, pBeamformingInfo, Idx);
		if (pBeamformeeEntry->is_mu_sta == 0) { /*SU BFee*/
			if (pBeamformeeEntry->su_reg_index == 0) {	
				ODM_Write1Cyte(pDM_Odm, REG_TXBF_CTRL_8821C, 0x0);	
				ODM_Write1Byte(pDM_Odm, REG_TXBF_CTRL_8821C+3, ODM_Read1Byte(pDM_Odm, REG_TXBF_CTRL_8821C+3)|BIT4|BIT6|BIT7);
				ODM_Write1Cyte(pDM_Odm, REG_ASSOCIATED_BFMEE_SEL_8821C, 0);
			} else {
				ODM_Write1Cyte(pDM_Odm, REG_TXBF_CTRL_8821C+2, 0x0 | BIT14 | BIT15 | BIT12);

				ODM_Write1Cyte(pDM_Odm, REG_ASSOCIATED_BFMEE_SEL_8821C+2, 
				ODM_Read1Cyte(pDM_Odm, REG_ASSOCIATED_BFMEE_SEL_8821C+2) & 0x60);
			}
			...
			ODM_Write1Cyte(pDM_Odm, mu_reg[pBeamformeeEntry->mu_reg_index] , 0);
	...
	)
{
	...
	if(pSoundingInfo->State == SOUNDING_STATE_SU_SOUNDDOWN) {
		...
		PHYDM_DBG(pDM_Odm, DBG_TXBF, ODM_DBG_TRACE, ("@%s, BeamformEntry.BeamformEntryState = %d", __func__, pBeamformEntry->BeamformEntryState));

		if (pBeamformEntry->su_reg_index == 0) {
			BeamCtrlReg = REG_TXBF_CTRL_8821C;
		} else {
			BeamCtrlReg = REG_TXBF_CTRL_8821C+2;
		...
		if ((pBeamformEntry->BeamformEntryState == BEAMFORMING_ENTRY_STATE_PROGRESSED) && (priv->pshare->rf_ft_var.applyVmatrix)) {
			...
			PHYDM_DBG(pDM_Odm, DBG_TXBF, ODM_DBG_TRACE, ("@%s, Don't apply Vmatrix",  __func__));
			BeamCtrlVal &= ~(BIT9|BIT10|BIT11);
		}

		ODM_Write1Cyte(pDM_Odm, BeamCtrlReg, BeamCtrlVal);
		/*disable NDP packet use beamforming */
		tmpVal = ODM_Read1Cyte(pDM_Odm, REG_TXBF_CTRL_8821C);
		ODM_Write1Cyte(pDM_Odm, REG_TXBF_CTRL_8821C, tmpVal|BIT15);
	} else {
		//PHYDM_DBG(pDM_Odm, DBG_TXBF, ODM_DBG_TRACE, ("@%s, MU Sounding Done\n",  __func__));
		/*MU sounding done */
		if (1){//(pBeamformEntry->BeamformEntryState == BEAMFORMING_ENTRY_STATE_PROGRESSED) {
			//PHYDM_DBG(pDM_Odm, DBG_TXBF, ODM_DBG_TRACE, ("@%s, BEAMFORMING_ENTRY_STATE_PROGRESSED\n",  __func__));
			...
			//PHYDM_DBG(pDM_Odm, DBG_TXBF, ("@%s, is_sounding_success STA1:%d,  STA2:%d, STA3:%d, STA4:%d, STA5:%d, STA6:%d\n", 
			...
			{
				u1Cyte ptcl_gen_mu_cnt, wmac_tx_mu_cnt, wmac_rx_mu_ba_ok_cnt, wmac_tx_bar_cnt, wmac_rx_sta2_ba_ok_cnt, mu2su_tx_cnt; 
...
#else
				//PHYDM_DBG(pDM_Odm, DBG_TXBF, ("%s, ptcl_gen_mu_cnt = %d\n",  __func__, ptcl_gen_mu_cnt));
				//PHYDM_DBG(pDM_Odm, DBG_TXBF, ("%s, wmac_tx_mu_cnt = %d\n",  __func__, wmac_tx_mu_cnt));
				//PHYDM_DBG(pDM_Odm, DBG_TXBF, ("%s, wmac_rx_mu_ba_ok_cnt = %d\n",  __func__, wmac_rx_mu_ba_ok_cnt));
				//PHYDM_DBG(pDM_Odm, DBG_TXBF, ("%s, wmac_tx_bar_cnt = %d\n",  __func__, wmac_tx_bar_cnt));
				//PHYDM_DBG(pDM_Odm, DBG_TXBF, ("%s, wmac_rx_sta2_ba_ok_cnt = %d\n",  __func__, wmac_rx_sta2_ba_ok_cnt));
				//PHYDM_DBG(pDM_Odm, DBG_TXBF, ("%s, mu2su_tx_cnt = %d\n\n\n",  __func__, mu2su_tx_cnt));
#endif
	...
	)
{
	...
	PHYDM_DBG(pDM_Odm, DBG_TXBF, ("%s: STA0: gid_valid = 0x%x, user_position_l = 0x%x, user_position_h = 0x%x\n",
	...
	PHYDM_DBG(pDM_Odm, DBG_TXBF, ("%s: STA1: gid_valid = 0x%x, user_position_l = 0x%x, user_position_h = 0x%x\n",
		__func__, gid_valid, user_position_l, user_position_h));
	...
	)
{
	...
	if (pBeamEntry->BeamformEntryState == BEAMFORMING_ENTRY_STATE_PROGRESSING)
		halTxbf8821C_DownloadNDPA(Adapter, Idx);

	halTxbf8821C_FwTxBFCmd(Adapter);
#endif
}

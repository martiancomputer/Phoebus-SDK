...
)
{
	...
	if (!HalPwrSeqCmdParsing88XX(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
			PWR_INTF_SDIO_MSK, rtl8822B_card_enable_flow))
    {
...
)
{
	...
	//		PWR_INTF_SDIO_MSK, rtl8822B_enter_lps_flow)) {
	...
	//if (_FALSE == HalPwrSeqCmdParsing88XX(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
	//		PWR_INTF_SDIO_MSK, rtl8822B_card_disable_flow)) {
...
)
{
    ...
    pHalFunc->InitPONHandler        = InitPON88XX; //InitPON8822BS;
    pHalFunc->InitMACHandler        = InitMAC88XX;
    pHalFunc->InitFirmwareHandler   = InitMIPSFirmware88XX;
#if CFG_HAL_SUPPORT_MBSSID
    pHalFunc->InitMBSSIDHandler     = InitMBSSID_8822BS;
    ...
    //
#if CFG_HAL_SUPPORT_MBSSID
    pHalFunc->StopMBSSIDHandler     = StopMBSSID_8822BS;
#endif  //CFG_HAL_SUPPORT_MBSSID
    pHalFunc->StopHWHandler         = StopHW_8822BS;
    pHalFunc->StopSWHandler         = StopSW88XX;
    pHalFunc->DisableVXDAPHandler   = DisableVXDAP88XX;
    pHalFunc->ResetHWForSurpriseHandler     = ResetHWForSurprise8822BS;

    //
    //ISR Related
    //
    pHalFunc->InitIMRHandler                    = InitIMR_8822BS;
    pHalFunc->EnableIMRHandler                  = EnableIMR_8822BS;
    ...
    pHalFunc->SetMACIDSleepHandler          =   SetMACIDSleep_8822BS;
    ...
    pHalFunc->UpdateHalMSRRPTHandler        = UpdateHalMSRRPT_8822BS;
	...
	)
{
	...
	Nr_index = TxBF_Nr(halTxbf8822B_GetNtx(priv), BeamformeeEntry.CompSteeringNumofBFer);
	...
	if(bSelfBeamformer) {			// for 8814 19ac(idx 1), 19b4(idx 0), different Tx ant setting
		...
		if(idx == 0) {
			switch(Nr_index) {
				...
				PHY_SetBBReg(priv, REG_BB_TX_PATH_SEL_1, BIT23|BIT22|BIT21|BIT20, 0x6);	/*set TxPath selection for 8822B BFer bug refine*/
				...
				PHY_SetBBReg(priv, REG_BB_TX_PATH_SEL_1, BIT23|BIT22|BIT21|BIT20, 0xe);	/*set TxPath selection for 8822B BFer bug refine*/
				...
				PHY_SetBBReg(priv, REG_BB_TX_PATH_SEL_1, BIT23|BIT22|BIT21|BIT20, 0xf);	/*set TxPath selection for 8822B BFer bug refine*/
	...
	)
{
	...
	halTxbf8822B_GetBeamformCap(priv);
	...
	if ((pBeamformingInfo->BeamformCap & BEAMFORMEE_CAP) && (BFerIdx < BEAMFORMER_ENTRY_NUM)) {
		...
		Nc_index = halTxbf8822B_GetNrx(priv);	// for 8822B Nrx = 3(4 Ant), min=0(1 Ant)
	...
	if((pBeamformingInfo->BeamformCap & BEAMFORMER_CAP) && (BFeeIdx < BEAMFORMEE_ENTRY_NUM)) {
		...
		SetBeamformRfMode8822B(priv, pBeamformingInfo, BFeeIdx);
		...
		if(BFeeIdx == 0) {
			RTL_W16( REG_TXBF_CTRL, STAid|BIT14|BIT15);
	...
	)
{
	...
	*/
	if (BeamformeeEntry.BeamformEntryCap == BEAMFORMING_CAP_NONE) {
		if(Idx == 0) {
			RTL_W16( REG_TXBF_CTRL, BIT14|BIT15);

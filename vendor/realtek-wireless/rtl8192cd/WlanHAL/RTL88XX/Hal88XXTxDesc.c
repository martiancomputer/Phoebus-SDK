...
#if (IS_EXIST_RTL8192EE || IS_EXIST_RTL8192FE || IS_EXIST_RTL8814AE || IS_EXIST_RTL8197FEM || IS_EXIST_RTL8822BE || IS_EXIST_RTL8822CE || IS_EXIST_RTL8812FE)
...
#endif
#if IS_RTL8192E_SERIES || IS_RTL8192F_SERIES || IS_RTL8814A_SERIES || IS_RTL8197F_SERIES || IS_RTL8822B_SERIES || IS_RTL8822C_SERIES || IS_RTL8812F_SERIES
			if (IS_HARDWARE_TYPE_8192E(Adapter) || IS_HARDWARE_TYPE_8192F(Adapter) || IS_HARDWARE_TYPE_8814A(Adapter) || IS_HARDWARE_TYPE_8197F(Adapter)|| IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8822C(Adapter) || IS_HARDWARE_TYPE_8812F(Adapter)) {
...
original:
    ...
    for (q_num = 0; q_num < HCI_TX_DMA_QUEUE_MAX_NUM; q_num++)
    {
		...
		rtl_cache_sync_wback(Adapter,(unsigned long)(&(ptxbd[i].TXBD_ELE[0])-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), 
						sizeof(TX_BUFFER_DESCRIPTOR),DMA_TO_DEVICE);
#endif //CONFIG_ENABLE_CCI400
#else
			...
			sizeof(TXBD_ELEMENT),DMA_TO_DEVICE);
...
//    BOOLEAN TX_DESC_GF               = 0;
    ...
    struct  dm_struct *dm			 = GET_PDM_ODM(Adapter);
...
//3 FILL TXBD
	...
	if(pkt_info->amsdubuf_len[amsdu_i])
        ...
        rtl_cache_sync_wback(Adapter,((GET_DESC_FIELD((unsigned long)bus_to_virt(cur_txbd_amsdu->TXBD_ELE[amsdu_i].Dword1), TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH)) - CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL),
            (u4Byte)pkt_info->amsdubuf_len[amsdu_i],DMA_TO_DEVICE);
#endif  //CONFIG_ENABLE_CCI400
#else
		_dma_cache_wback(((GET_DESC_FIELD(cur_txbd_amsdu->TXBD_ELE[amsdu_i].Dword1, TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH)|0x80000000) - CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL),
            pkt_info->amsdubuf_len[amsdu_i]);
    ...
    TxPktFinalIO88XX(Adapter, cur_txbd, TxPktFinalIO88XX_WRITE, pkt_info->PSB_len);
        ...
        // With CCI400 ,Do nothing for cache coherent code
#else
        rtl_cache_sync_wback(Adapter,(unsigned long)((PVOID)(cur_txbd)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), sizeof(TX_BUFFER_DESCRIPTOR),DMA_TO_DEVICE);
        ...
        rtl_cache_sync_wback(Adapter,((GET_DESC_FIELD((unsigned long)bus_to_virt(cur_txbd->TXBD_ELE[1].Dword1), TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH)) - CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL),
            (u4Byte)pkt_info->hdr_len,DMA_TO_DEVICE);

#if CFG_HAL_TX_AMSDU
        if ( IS_SUPPORT_TX_AMSDU(Adapter) && pkt_info->amsdu_flag) {
            rtl_cache_sync_wback(Adapter,(unsigned long)((PVOID)(cur_txbd_amsdu)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), sizeof(TX_BUFFER_DESCRIPTOR_AMSDU),DMA_TO_DEVICE);
        }else
#endif // CFG_HAL_TX_AMSDU
        {
            if (pkt_info->hdr_len) {
            ...
            if (pkt_info->skbbuf_len) {
                rtl_cache_sync_wback(Adapter,((GET_DESC_FIELD((unsigned long)bus_to_virt(cur_txbd->TXBD_ELE[2].Dword1), TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH)) - CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL),
                    (u4Byte)pkt_info->skbbuf_len,DMA_TO_DEVICE);
            } 
            if (pkt_info->mic_icv_len && pkt_info->icv) {
                rtl_cache_sync_wback(Adapter,((GET_DESC_FIELD((unsigned long)bus_to_virt(cur_txbd->TXBD_ELE[3].Dword1), TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH)) - CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL),
                    pkt_info->mic_icv_len,DMA_TO_DEVICE);
            } else if (pkt_info->mic_icv_len && pkt_info->mic) {
                rtl_cache_sync_wback(Adapter,((GET_DESC_FIELD((unsigned long)bus_to_virt(cur_txbd->TXBD_ELE[3].Dword1), TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH)) - CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL),
                    pkt_info->mic_icv_len,DMA_TO_DEVICE);
            }
...
)
{
    ...
    u1Byte elm_idx;
        ...
        if(IS_HARDWARE_TYPE_8814BE(Adapter)){
            if(queueIndex > DATAQ_IDX_MAX)
                queueIndex = queueIndex - DATAQ_IDX_MAX;
        }
#endif
...
//}
    ...
    if (OFFLOAD_ENABLE(Adapter))
        BUG_ON(pdesc_data->aggreEn >= FG_AGGRE_MSDU_FIRST);
#endif
    ...
    {
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
        if (!(OFFLOAD_ENABLE(Adapter) && (hdrLen == 0))) 
#endif
        {
        SET_DESC_FIELD_NO_CLR(cur_txbd->TXBD_ELE[1].Dword0,
        ...
        pdesc_data->hdr_dma_addr = dma_addr;
        SET_DESC_FIELD_NO_CLR(cur_txbd->TXBD_ELE[1].Dword1,
                    dma_addr + CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL,
                    TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH);
        }
    ...
    // for FG_AGGRE_MSDU_FIRST, FG_AGGRE_MSDU_MIDDLE, and FG_AGGRE_MSDU_LAST
        ...
        SET_DESC_FIELD_NO_CLR(cur_txbd_amsdu->TXBD_ELE[i].Dword0,
        ...
        SET_DESC_FIELD_NO_CLR(cur_txbd_amsdu->TXBD_ELE[i].Dword1,
                dma_addr + CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL,
                TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH);
#if !(defined(CONFIG_NET_PCI) && defined(NOT_RTK_BSP))
	if(payloadLen){
        ...
        rtl_cache_sync_wback(Adapter,((GET_DESC_FIELD((unsigned long)bus_to_virt(cur_txbd_amsdu->TXBD_ELE[i].Dword1), TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH)) - CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL),
            (u4Byte)payloadLen,DMA_TO_DEVICE);
#endif //CONFIG_ENABLE_CCI400
#else
...
#endif
    {
        ...
        if (payloadLen) {
            dma_addr = HAL_VIRT_TO_BUS1(Adapter, (PVOID)pdesc_data->pBuf, payloadLen, HAL_PCI_DMA_TODEVICE);
            pdesc_data->buf_dma_addr = dma_addr;
...
#if defined(AP_SWPS_OFFLOAD)
        struct TX_DESC_88XX * ptx_desc_info = pdescinfo->ptxdesc;
...
#endif
#if (IS_EXIST_RTL8814AE || IS_EXIST_RTL8822BE || IS_EXIST_RTL8821CE|| IS_EXIST_RTL8198FEM || IS_EXIST_RTL8814BE || IS_EXIST_RTL8822CE || IS_EXIST_RTL8812FE) 
    if (IS_HARDWARE_TYPE_8814AE(Adapter) || IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8821C(Adapter) || IS_HARDWARE_TYPE_8198F(Adapter) || IS_HARDWARE_TYPE_8814B(Adapter) || IS_HARDWARE_TYPE_8822C(Adapter) || IS_HARDWARE_TYPE_8812F(Adapter)) {
        PSBLen   = (TotalLen%PBP_PSTX_SIZE_V1) == 0 ? (TotalLen/PBP_PSTX_SIZE_V1):((TotalLen/PBP_PSTX_SIZE_V1)+1);
    }
#endif
#if IS_EXIST_RTL8881AEM
    ...
    if (pdesc_data->aggreEn == FG_AGGRE_MSDU_LAST) {
        ...
        sizeof(TX_BUFFER_DESCRIPTOR_AMSDU),DMA_TO_DEVICE);
#endif //CONFIG_ENABLE_CCI400
#else
    	_dma_cache_wback((unsigned long)((PVOID)(cur_txbd_amsdu)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), sizeof(TX_BUFFER_DESCRIPTOR_AMSDU));
...
#endif // CFG_HAL_TX_AMSDU
    {
        ...
        // With CCI400 ,Do nothing for cache coherent code
			...
			if (pdesc_data->icv) {
                rtl_cache_sync_wback(Adapter,((GET_DESC_FIELD((unsigned long)bus_to_virt(cur_txbd->TXBD_ELE[elm_idx].Dword1), TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH)) - CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL),
		   		    pdesc_data->icv,DMA_TO_DEVICE);
			} else if (pdesc_data->mic) {

                rtl_cache_sync_wback(Adapter,((GET_DESC_FIELD((unsigned long)bus_to_virt(cur_txbd->TXBD_ELE[elm_idx].Dword1), TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH)) - CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL),
		   		    pdesc_data->mic,DMA_TO_DEVICE);
			}
...
)
{
...
#if (IS_EXIST_RTL8192EE || IS_RTL8192F_SERIES || IS_EXIST_RTL8814AE || IS_EXIST_RTL8197FEM || IS_EXIST_RTL8822BE || IS_EXIST_RTL8821CE || IS_EXIST_RTL8198FEM || IS_EXIST_RTL8814BE || IS_EXIST_RTL8822CE || IS_EXIST_RTL8812FE)
    if (IS_HARDWARE_TYPE_8192E(Adapter) || IS_HARDWARE_TYPE_8192F(Adapter) || IS_HARDWARE_TYPE_8814A(Adapter) || IS_HARDWARE_TYPE_8197F(Adapter) || IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8821C(Adapter) || IS_HARDWARE_TYPE_8198F(Adapter) || IS_HARDWARE_TYPE_8814B(Adapter) || IS_HARDWARE_TYPE_8822C(Adapter) || IS_HARDWARE_TYPE_8812F(Adapter)) {    
        TXBDBeaconOffset = TXBD_BEACON_OFFSET_V1;        
    }
    else
#endif  //(IS_EXIST_RTL8192EE || IS_EXIST_RTL8814AE || IS_EXIST_RTL8197FEM || IS_EXIST_RTL8822BE || IS_EXIST_RTL8198FEM)
...
#if (IS_EXIST_RTL8881AEM || IS_EXIST_RTL8192EE || IS_EXIST_RTL8192FE || IS_EXIST_RTL8814AE || IS_EXIST_RTL8197FEM || IS_EXIST_RTL8822BE || IS_EXIST_RTL8822CE || IS_EXIST_RTL8812FE)
    if (IS_HARDWARE_TYPE_8881A(Adapter) || IS_HARDWARE_TYPE_8192E(Adapter) || IS_HARDWARE_TYPE_8192F(Adapter) || IS_HARDWARE_TYPE_8814A(Adapter) || IS_HARDWARE_TYPE_8197F(Adapter) || IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8822C(Adapter) || IS_HARDWARE_TYPE_8812F(Adapter)) {    
        DMA_QUEUE_BCN = HCI_TX_DMA_QUEUE_BCN;  
    }
#endif  //(IS_EXIST_RTL8881AEM || IS_EXIST_RTL8192EE || IS_EXIST_RTL8814AE || IS_EXIST_RTL8197FEM || IS_EXIST_RTL8822BE)
    ...
    if ( IS_HARDWARE_TYPE_8814B(Adapter) ) {
        ...
        DMA_QUEUE_BCN = HCI_TX_DMA_QUEUE_BCN_PEOFLD; 
#else
...
) 
{
	...
	//3 Get TXBD PTR & Get TXDESC PTR
#if (IS_RTL8192E_SERIES || IS_RTL8192F_SERIES  || IS_RTL8814A_SERIES || IS_RTL8197F_SERIES || IS_RTL8822B_SERIES || IS_RTL8821C_SERIES || IS_RTL8198F_SERIES || IS_RTL8814B_SERIES || IS_RTL8822C_SERIES || IS_RTL8812F_SERIES)
    if (IS_HARDWARE_TYPE_8192E(Adapter) || IS_HARDWARE_TYPE_8192F(Adapter)  || IS_HARDWARE_TYPE_8814A(Adapter) || IS_HARDWARE_TYPE_8197F(Adapter) || IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8821C(Adapter) || IS_HARDWARE_TYPE_8198F(Adapter) || IS_HARDWARE_TYPE_8814B(Adapter) || IS_HARDWARE_TYPE_8822C(Adapter) || IS_HARDWARE_TYPE_8812F(Adapter)) {    	
		TXBDBeaconOffset = TXBD_BEACON_OFFSET_V1;		
	}
 ...
 #endif
    {
        ...
        rtl_cache_sync_wback(Adapter,(unsigned long)((PVOID)(&(pTXBD->TXBD_ELE[0].Dword0))-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), 
            sizeof(pTXBD->TXBD_ELE[0].Dword0),DMA_TO_DEVICE);
#endif //CONFIG_ENABLE_CCI400
#else
...
)
{
...
#endif //IS_EXIST_RTL8822BE || IS_EXIST_RTL8198FEM
...
#if IS_EXIST_RTL8814AE || IS_RTL8822B_SERIES || IS_RTL8821C_SERIES|| IS_EXIST_RTL8198FEM  || IS_EXIST_RTL8814BE || IS_RTL8822C_SERIES || IS_RTL8812F_SERIES
    if ( IS_HARDWARE_TYPE_8814AE(Adapter)|| IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8821C(Adapter) || IS_HARDWARE_TYPE_8198F(Adapter) || IS_HARDWARE_TYPE_8814B(Adapter) || IS_HARDWARE_TYPE_8822C(Adapter) || IS_HARDWARE_TYPE_8812F(Adapter)) {
        PSBLen   = (TotalLen%PBP_PSTX_SIZE_V1) == 0 ? (TotalLen/PBP_PSTX_SIZE_V1):((TotalLen/PBP_PSTX_SIZE_V1)+1);        
    }
    else
#endif //#if IS_EXIST_RTL8814AE || IS_RTL8822B_SERIES || IS_EXIST_RTL8198FEM 
    ...
    if(ki < 10)
    {
        ...
        // With CCI400 ,Do nothing for cache coherent code
#else
        rtl_cache_sync_wback(Adapter,(unsigned long)((PVOID)(pTXBD)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), sizeof(TX_BUFFER_DESCRIPTOR),DMA_TO_DEVICE);
...
)
{
    ...
    halQNum = GET_HAL_INTERFACE(Adapter)->MappingTxQueueHandler(Adapter, q_num);
    hwIdx = (BIT_MASK_QUEUE_IDX &
        (HAL_RTL_R32(ptx_dma->tx_queue[halQNum].reg_rwptr_idx)>>BIT_SHIFT_QUEUE_HW_IDX));    

#if defined(CONFIG_WLAN_HAL_8814BE)
    /* Work around: 8814B rarely get hw idx larger than txbd num, this will cause dead loop in tx recycle function,
        get hw idx again to avoid this */
    if (hwIdx >= ptx_dma->tx_queue[halQNum].total_txbd_num) {
	printk("Get HW idx error! qNum=%d hwIdx=%d txbd_num=%d\n", halQNum, hwIdx, ptx_dma->tx_queue[halQNum].total_txbd_num);
        hwIdx = (BIT_MASK_QUEUE_IDX &
            (HAL_RTL_R32(ptx_dma->tx_queue[halQNum].reg_rwptr_idx)>>BIT_SHIFT_QUEUE_HW_IDX));    
    }	
#endif
...
#if (IS_EXIST_RTL8822BE || IS_EXIST_RTL8197FEM || IS_EXIST_RTL8821CE || IS_EXIST_RTL8198FEM || IS_EXIST_RTL8814BE || IS_EXIST_RTL8822CE || IS_EXIST_RTL8812FE)
...
)
{
	...
	if ( IS_HARDWARE_TYPE_8192FE(Adapter) ||
			_GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v2_v3) {
...
{
    struct halmac_tx_desc *ptx_desc = (struct halmac_tx_desc *)pTxDesc;
...
)
{
    ...
    case _CCMP_PRIVACY_:
        ...
        break;
#ifdef WPA3_ENT
	case _GCMP256_PRIVACY_:
		IE1_SET_TX_DESC_SECTYPE(IE1_head,TXDESC_SECTYPE_GCMP256);				  
	break;
#endif
    default:
        break;
...
#ifdef WPA3_ENT
	case _GCMP256_PRIVACY_:
		SET_TX_DESC_SEC_TYPE(ptx_desc,TXDESC_SECTYPE_GCMP256);				  
	break;
#endif
    default:
#if 0
...
#if IS_EXIST_RTL8822BE || IS_EXIST_RTL8197FEM || IS_EXIST_RTL8198FEM || IS_EXIST_RTL8822CE || IS_EXIST_RTL8812FE
VOID
...
*/
    ...
    // With CCI400 ,Do nothing for cache coherent code
#else
    rtl_cache_sync_wback(Adapter,(unsigned long)((PVOID)(pdesc)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), TXDESCSize,DMA_TO_DEVICE);
#endif //CONFIG_ENABLE_CCI400
#else
    _dma_cache_wback((unsigned long)((PVOID)(pdesc)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), TXDESCSize);
#endif //CONFIG_RTL_8198F
...
#if (IS_EXIST_RTL8822BE || IS_EXIST_RTL8822CE || IS_EXIST_RTL8812FE)
	if ( IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8822C(Adapter) || IS_HARDWARE_TYPE_8812F(Adapter))  {
		if(TX_DESC_RTY_LMT_EN && (!TX_DESC_NDPA) && (queueIndex != HCI_TX_DMA_QUEUE_MGT)){
...
#if (IS_EXIST_RTL8197FEM || IS_EXIST_RTL8822BE || IS_EXIST_RTL8812FE) 
	if ( IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8197F(Adapter) || IS_HARDWARE_TYPE_8812F(Adapter)) {
...
)
{
...
#if (IS_EXIST_RTL8822BE || IS_EXIST_RTL8197FEM || IS_EXIST_RTL8812FE)
        if (IS_HARDWARE_TYPE_8197F(Adapter) || IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8812F(Adapter))		
    ...
    // With CCI400 ,Do nothing for cache coherent code
#else
    rtl_cache_sync_wback(Adapter,(unsigned long)((PVOID)ptx_desc-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), TXDESCSize,DMA_TO_DEVICE);
#endif //CONFIG_ENABLE_CCI400
#else
    _dma_cache_wback((unsigned long)((PVOID)ptx_desc-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), TXDESCSize);
#endif //CONFIG_RTL_8198F
...
#endif // #if IS_EXIST_RTL8822BE || IS_EXIST_RTL8197FEM || IS_EXIST_RTL8198FEM || IS_EXIST_RTL8814BE
...
#if IS_EXIST_RTL8814BE && defined(CONFIG_PE_ENABLE)
        if(IS_HARDWARE_TYPE_8814BE(Adapter)){
            if((queueIndex >= DATAQ_IDX_MIN) && (queueIndex <= DATAQ_IDX_MAX))
                printk("[%s][PE Enabled] unexpected queueIndex = 0x%x!!!\n",__FUNCTION__,queueIndex);

            if(queueIndex > DATAQ_IDX_MAX)
                dma_queueIndex = queueIndex - DATAQ_IDX_MAX;
        }else
#endif
        {
    ...
    if (IS_SUPPORT_WLAN_HAL_HW_TX_AMSDU(Adapter)) {
        ...
        if ( IS_HARDWARE_TYPE_8814B(Adapter)) {
       	    if (TX_DESC_NDPA) {
                ...
                if (Adapter->pshare->rf_ft_var.ndpa_retry != 0xff) {
                    IE0_SET_TX_DESC_RTY_LMT_EN(IE0_head, 1);
                    IE0_SET_TX_DESC_RTS_DATA_RTY_LMT(IE0_head, Adapter->pshare->rf_ft_var.ndpa_retry);
                }
       	    }
...
#if (CFG_HAL_MAC_LOOPBACK && CFG_HAL_WIFI_WMM) || defined(AP_SWPS_OFFLOAD_VERI)
    	case HCI_TX_DMA_QUEUE_BE_V2:
    		q_select = TXDESC_QSEL_TID0;
    		break;
        case HCI_TX_DMA_QUEUE_VO_V2:
...
{    
    ...
    PTX_DESC_88XX               pfwcmd_tx_desc;
        ...
        u4Byte TXBD_NUM_V3[HCI_TX_DMA_QUEUE_MAX_NUM_V2] =
        {
            TX_MGQ_TXBD_NUM_8814B,   TX_HI0Q_TXBD_NUM_8814B,  
            TX_HI1Q_TXBD_NUM_8814B,  TX_HI2Q_TXBD_NUM_8814B,    TX_HI3Q_TXBD_NUM_8814B,   TX_HI4Q_TXBD_NUM_8814B,  
            TX_HI5Q_TXBD_NUM_8814B,  TX_HI6Q_TXBD_NUM_8814B,    TX_HI7Q_TXBD_NUM_8814B,  TX_HI8Q_TXBD_NUM_8814B,  
            TX_HI9Q_TXBD_NUM_8814B,  TX_HI10Q_TXBD_NUM_8814B, TX_HI11Q_TXBD_NUM_8814B, TX_HI12Q_TXBD_NUM_8814B, 
...
#ifdef CONFIG_PE_ENABLE
        TotalTXBDNum_NoBcn  = TOTAL_NUM_TXBD_NO_BCN_PEOFLD_8814B;
        DMA_MAX_QUEUE_NUM = HCI_TX_DMA_QUEUE_MAX_NUM_PEOFLD;  
        DMA_QUEUE_BCN = HCI_TX_DMA_QUEUE_BCN_PEOFLD;      
        DMA_QUEUE_CMD = HCI_TX_DMA_QUEUE_CMD_PEOFLD;
        DMA_QUEUE_FWCMD = HCI_TX_DMA_QUEUE_FWCMD_PEOFLD;
...
original:
    ...
    if ( IS_SUPPORT_TX_AMSDU(Adapter) ) {
        ...
        rtl_cache_sync_wback(Adapter,(unsigned long)((PVOID)(desc_dma_buf_start_amsdu)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), 
            _GET_HAL_DATA(Adapter)->desc_dma_buf_len_amsdu,DMA_TO_DEVICE);
#endif  //CONFIG_ENABLE_CCI400
#else
        _dma_cache_wback((unsigned long)((PVOID)(desc_dma_buf_start_amsdu)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), _GET_HAL_DATA(Adapter)->desc_dma_buf_len_amsdu);
#endif
...
#endif // CFG_HAL_TX_AMSDU
    //No Beacon
#if 1//def NEW_BUUFER_ALLOCATION   // 88XX_V1 use desc_dma_buf to RX BD
    // TXBD allocation is separated from RXBD allocation
    ptxbd_head      =   _GET_HAL_DATA(Adapter)->desc_dma_buf;
    platform_zero_memory(ptxbd_head, _GET_HAL_DATA(Adapter)->desc_dma_buf_len);
#else
    ptxbd_head      = (PTX_BUFFER_DESCRIPTOR)(prx_dma->rx_queue[HCI_RX_DMA_QUEUE_MAX_NUM-1].pRXBD_head + \

...
)
{
	...
	RT_TRACE_F( COMP_INIT, DBG_LOUD, ("0: Reg0x0: 0x%x, Reg0x4: 0x%x, Reg0x1C: 0x%x\n", HAL_RTL_R32(0x0), HAL_RTL_R32(0x4), HAL_RTL_R32(0x1C)));
	HAL_RTL_W8(REG_RSV_CTRL, 0x00);
	RT_TRACE_F( COMP_INIT, DBG_LOUD, ("1: Reg0x0: 0x%x, Reg0x4: 0x%x, Reg0x1C: 0x%x\n", HAL_RTL_R32(0x0), HAL_RTL_R32(0x4), HAL_RTL_R32(0x1C)));
	HAL_RTL_W8(REG_RSV_CTRL, 0x00);
	RT_TRACE_F( COMP_INIT, DBG_LOUD, ("2: Reg0x0: 0x%x, Reg0x4: 0x%x, Reg0x1C: 0x%x\n", HAL_RTL_R32(0x0), HAL_RTL_R32(0x4), HAL_RTL_R32(0x1C)));
	...
	RT_TRACE_F( COMP_INIT, DBG_LOUD, ("3: Reg0x0: 0x%x, Reg0x4: 0x%x, Reg0x1C: 0x%x\n", HAL_RTL_R32(0x0), HAL_RTL_R32(0x4), HAL_RTL_R32(0x1C)));
	...
	RT_TRACE_F( COMP_INIT, DBG_LOUD, ("%s(%d): 0x1000:0x%x \n", __FUNCTION__, __LINE__, HAL_RTL_R32(0x1000)));
#endif
...

{
	...
	default:
		if(!(phydm_c2H_content_parsing(&(priv->pshare->_dmODM), c2hCmdId, c2hCmdLen, tmpBuf))) {
			panic_printk("[C2H], Unkown packet!! CmdId(%#X)!\n", c2hCmdId);
		}
		break;
	}
}
	...
	)
{
	...
	for (i= RF_PATH_A; i < MAX_RF_PATH; i++)
		odm_set_rf_reg(ODMPTR, i, RF_WE_LUT, 0x80000, 0x1); // RF Mode table write enable
	
	while(1) {
		counter++;
		for (i= RF_PATH_A; i < MAX_RF_PATH; i++)
			odm_set_rf_reg(ODMPTR, i, RF_RCK_OS, 0xfffff, 0x18000); // Select Rx mode

		ODM_delay_us(2);
		
		for (i= RF_PATH_A; i < MAX_RF_PATH; i++)
			rf_mode[i] = odm_get_rf_reg(ODMPTR, i, RF_RCK_OS, 0xfffff); // Select Rx mode
		
		if ((rf_mode[0] == 0x18000) && (rf_mode[1] == 0x18000) && (rf_mode[2] == 0x18000) && (rf_mode[3] == 0x18000))
			break;
		else if (counter == 100) {
			PHYDM_DBG(ODMPTR, ODM_PHY_CONFIG, "iqgen setting fail:8814A \n");
			for (i= RF_PATH_A; i < MAX_RF_PATH; i++)	/*RF Mode table write disable*/
				odm_set_rf_reg(ODMPTR, i, RF_WE_LUT, 0x80000, 0x0);
			return FALSE;
		}
	}

	for (i= RF_PATH_A; i < MAX_RF_PATH; i++) {
		odm_set_rf_reg(ODMPTR, i, RF_TXPA_G1, 0xfffff, 0xBE77F); // Set Table data
		odm_set_rf_reg(ODMPTR, i, RF_TXPA_G2, 0xfffff, 0x226BF); // Enable TXIQGEN in Rx mode
	}
	odm_set_rf_reg(ODMPTR, RF_PATH_A, RF_TXPA_G2, 0xfffff, 0xE26BF); // Enable TXIQGEN in Rx mode
	

	while(1) {
		counter++;
		for (i= RF_PATH_A; i < MAX_RF_PATH; i++)
			odm_set_rf_reg(ODMPTR, i, RF_RCK_OS, 0xfffff,0x8000); // Select Standby mode

		ODM_delay_us(2);
		
		for (i= RF_PATH_A; i < MAX_RF_PATH; i++)
			rf_mode[i] = odm_get_rf_reg(ODMPTR, i, RF_RCK_OS, 0xfffff); // Select Standby mode
	...
	for (i= RF_PATH_A; i < MAX_RF_PATH; i++) {
		odm_set_rf_reg(ODMPTR, i, RF_TXPA_G1, 0xfffff,0xFE441);/*Set Table data*/
		odm_set_rf_reg(ODMPTR, i, RF_TXPA_G2, 0xfffff,0xE20BF);/*Enable TXIQGEN in Standby mode*/
	}

	for (i= RF_PATH_A; i < MAX_RF_PATH; i++)
		odm_set_rf_reg(ODMPTR, i, RF_WE_LUT, 0x80000, 0x0);	/*RF Mode table write disable*/
	...
	)
{
	...
	if (bSelfBeamformer) {		/*for 8814 19ac(idx 1), 19b4(idx 0), different Tx ant setting*/
		phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF1, BIT28|BIT29, 0x2);			/*enable BB TxBF ant mapping register*/
		phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF1, BIT30, 0x1);			/*enable BB TxBF ant mapping register*/
		phy_set_bb_reg(priv, REG_BB_TX_PATH_SEL_1, BIT23|BIT22|BIT21|BIT20, 0xe);	/*set TxPath selection for 8814a BFer bug refine*/
		phy_set_bb_reg(priv, REG_BB_TX_PATH_SEL_1, 0xff000000, 0x93);				/*if Bfer enable, always use 3Tx for all Spatial stream*/
		phy_set_bb_reg(priv, REG_BB_TX_PATH_SEL_2, 0xffffffff, 0x93e93e0);
		...
		if (idx == 0) {
			switch(Nr_index) {			
				...
				case 1:		// Nsts = 2	BC
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF0, BIT3|BIT2|BIT1|BIT0, 0x6);	//1ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF0, BIT7|BIT6|BIT5|BIT4, 0x6);	//2ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF0, 0x0000ff00, 0x10);			//BC
				break;

				case 2:		//Nsts = 3	BCD
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF0, BIT3|BIT2|BIT1|BIT0, 0xe);	//1ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF0, BIT7|BIT6|BIT5|BIT4, 0xe);	//2ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF0, 0x0000ff00, 0x90);			//BCD
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF0, BIT19|BIT18|BIT17|BIT16, 0xe);	//3ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF0, 0xff00000, 0x90);			//bcd

				break;
			
				default:		//Nr>3, same as Case 3
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF0, BIT3|BIT2|BIT1|BIT0, 0xf);	//1ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF0, BIT7|BIT6|BIT5|BIT4, 0xf);	//2ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF0, 0x0000ff00, 0x93);			//aBCd
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF0, BIT19|BIT18|BIT17|BIT16, 0xf);	//4ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF0, 0xff00000, 0x93);			//abcd
				phy_set_bb_reg(priv, REG_BB_TX_PATH_SEL_1, BIT23|BIT22|BIT21|BIT20, 0xf);	/*set TxPath selection for 8814a BFer bug refine*/
				phy_set_bb_reg(priv, REG_BB_TX_PATH_SEL_1, 0xff000000, 0x93);				/*if Bfer enable, always use 3Tx for all Spatial stream*/
				phy_set_bb_reg(priv, REG_BB_TX_PATH_SEL_2, 0xffffffff, 0x93f93f0);
				break;
			}
		} else {		// IDX =1
			switch(Nr_index) {
				...
				case 1:		// Nsts = 2	BC
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF1, BIT3|BIT2|BIT1|BIT0, 0x6);	//1ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF1, BIT7|BIT6|BIT5|BIT4, 0x6);	//2ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF1, 0x0000ff00, 0x10);			//BC
				break;

				case 2:		//Nsts = 3	BCD
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF1, BIT3|BIT2|BIT1|BIT0, 0xe);	//1ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF1, BIT7|BIT6|BIT5|BIT4, 0xe);	//2ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF1, 0x0000ff00, 0x90);			//BC
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF1, BIT19|BIT18|BIT17|BIT16, 0xe);	//3ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF1, 0xff00000, 0x90);			//bcd
				break;
			
				default:		//Nr>3, same as Case 3
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF1, BIT3|BIT2|BIT1|BIT0, 0xf);	//1ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF1, BIT7|BIT6|BIT5|BIT4, 0xf);	//2ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF1, 0x0000ff00, 0x93);			//BC
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF1, BIT19|BIT18|BIT17|BIT16, 0xf);	//3ss
				phy_set_bb_reg(priv, REG_BB_TXBF_ANT_SET_BF1, 0xff00000, 0x93);			//bcd
				phy_set_bb_reg(priv, REG_BB_TX_PATH_SEL_1, BIT23|BIT22|BIT21|BIT20, 0xf);	/*set TxPath selection for 8814a BFer bug refine*/
				phy_set_bb_reg(priv, REG_BB_TX_PATH_SEL_1, 0xff000000, 0x93);				/*if Bfer enable, always use 3Tx for all Spatial stream*/
				phy_set_bb_reg(priv, REG_BB_TX_PATH_SEL_2, 0xffffffff, 0x93f93f0);
				break;

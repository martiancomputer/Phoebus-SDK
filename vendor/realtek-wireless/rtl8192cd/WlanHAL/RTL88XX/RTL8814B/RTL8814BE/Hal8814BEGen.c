...
)
{
	...
	if(Adapter->pshare->rf_ft_var.manual_pe_enable){
		pHalFunc->MappingTxQueueHandler 		= MappingTxQueue8814B_PEOFLD;
	}else
#endif
	{
...
#ifdef CONFIG_PE_ENABLE
	pHalFunc->MappingTxQueueHandler 		= MappingTxQueue88XX_PEOFLD;
#else
	pHalFunc->MappingTxQueueHandler 		= MappingTxQueue88XX_V2;
#endif    
...
//    pHalFunc->ReleaseShortCutTxDescHandler  = ReleaseShortCutTxDesc88XX;
    ...
    if(Adapter->pshare->rf_ft_var.manual_pe_enable){
        ...
        pHalFunc->QueryRxDescHandler            = HalGeneralDummy;
	...
	)
{
	...
	*/
	...
	if (p_dm_odm->rf_type == RF_4T4R) { /*2T2R*/
		...
		if (priv->pmib->dot11RFEntry.adaptive_ant)
			config_phydm_trx_mode_8814b(p_dm_odm, BB_PATH_AB, BB_PATH_AB);	
#endif		
	}
	
	config_bf_tx_path_ext_8814b(p_dm_odm, BB_PATH_ABCD, BB_PATH_A, BB_PATH_AB, BB_PATH_ABC);
	
	if(AC_SIGMA_MODE == AC_SIGMA_APUT) {
		priv->pshare->rf_ft_var.force_mu = 1;
		priv->pshare->rf_ft_var.force_ndpa_rate = 44;
	}
}
	...
	)
{
	...
	if ((pBeamformingInfo->beamformee_mu_cnt > 0) && (BFeeIdx < BEAMFORMEE_ENTRY_NUM)) {
		...
		if((pBeamformeeEntry->BeamformEntryCap & BEAMFORM_CAP_VHT_MU_BFER) && pBeamformeeEntry->mu_reg_index == 0xFF)
		{
			...
			{
				if (VERSION_8814C) {
					for (i = 1; i <= MAX_NUM_BEAMFORMEE_MU; i++) {
...
#else
            {
				...
				u4Byte ctrl_info_offset, val32;
				u1Byte STA_ANTNUM = 0;

				ctrl_info_offset = RPT_CTRL_INFO + 64 * pBeamformeeEntry->MacId;

                INDIRECT_READ(ctrl_info_offset, val32);
                val32 = (val32 & 0xff00ffff) | ((pBeamformeeEntry->mu_reg_index + 1) << 16);
                INDIRECT_WRITE(ctrl_info_offset, val32);
                ...
                INDIRECT_READ(ctrl_info_offset + 40, val32);
                val32 = (val32 & 0xffcfffff) | (STA_ANTNUM << 16);
                INDIRECT_WRITE(ctrl_info_offset + 40, val32);
				...
				if(pBeamformingInfo->beamformee_mu_cnt > 0) {
					...
					if (pBeamformingInfo->beamformee_mu_cnt < 2) {
						if (priv->mu_bkup_agg_num) {
							...
							RTL_W16(0x63e, priv->mu_bkup_reg_63e);
							config_phydm_mu_anapar_8814b(ODMPTR, pBeamformingInfo->beamformee_mu_cnt);

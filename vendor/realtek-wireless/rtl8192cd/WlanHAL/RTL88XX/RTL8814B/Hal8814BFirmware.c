...
--*/
#define SUPPORT_EXTERNAL_WLANFW
#ifdef SUPPORT_EXTERNAL_WLANFW
#include <linux/firmware.h>
#endif
#include "HalPrecomp.h"
u32 GLOBAL_dbg_print = PRN_ALWAYS;
struct fwdl_ctx_t GLOBAL_fwdl_ctx;
struct fwdl_ctx_t GLOBAL_fwdl_ctx1;

#define GLOBAL_FWDL_CTX_IF (Adapter->pshare->wlandev_idx == (0^WLANIDX)?GLOBAL_fwdl_ctx:GLOBAL_fwdl_ctx1)
#define ASSIGN_GLOBAL_FWDL_CTX(fwdl_ctx) { \
    if(Adapter->pshare->wlandev_idx == (0^WLANIDX)) \
        fwdl_ctx = &GLOBAL_fwdl_ctx; \
    else \
        fwdl_ctx = &GLOBAL_fwdl_ctx1; \
    }
                              ...
                              struct fwdl_dl_section_t *conf)
{
	...
	HAL_PADAPTER Adapter = (HAL_PADAPTER)fwdl_ctx->priv;
    ...
    fwconf_set(NULL, FWCONF_ITEM_RX_RAW, Adapter->pshare->rf_ft_var.mp_specific ? 1 : 0);
    ...
    fwconf_set(NULL, FWCONF_ITEM_TX_LOOPBACK_MODE, Adapter->pshare->rf_ft_var.loopback);
                        /* 0:disable, 1:short loopback, 2:long loopback */
    fwconf_set(NULL, FWCONF_ITEM_TX_FLOWCTRL, Adapter->pshare->rf_ft_var.flowctrl);
                        /* 0:Disable, 1:HW, 2:SW */

    fwconf_set(NULL, FWCONF_ITEM_WMAC_MACID_MODE, !!(Adapter->pmib->dot11OperationEntry.opmode & WIFI_AP_STATE));
    fwconf_set(NULL, FWCONF_ITEM_DBG_LVL, Adapter->pshare->rf_ft_var.dbglevel);
    fwconf_set(NULL, FWCONF_ITEM_DEBUG_BOOT, 0);
    fwconf_set(NULL, FWCONF_ITEM_UNIT_TEST, Adapter->pshare->rf_ft_var.unittest);
    ...
    if (hil_dnld_info->atomic_cnt) {
        ...
        dnld_htxbd->HTXBD_BD[0].word0 = SET_DESC(HTXBD_BD_BIT_OWN |     /* own to HW */
                                         ...
                                         OFFSET_VALUE(*txdesc_txie_len,
                                                      HTXBD_BD_TXBUFFSIZE));
        dnld_htxbd->HTXBD_BD[0].word1 = SET_DESC(ctx_info_dma+(unsigned long)(*txdesc)-(unsigned long)ctx_info);

        /* CONTENT */
        dnld_htxbd->HTXBD_BD[1].word0 = SET_DESC(OFFSET_VALUE(*content_len,
                                                     HTXBD_BD_TXBUFFSIZE));
        dnld_htxbd->HTXBD_BD[1].word1 = SET_DESC(ctx_info_dma+(unsigned long)(*content)-(unsigned long)ctx_info);

#if defined(NOT_RTK_BSP)
        rtl_cache_sync_wback(Adapter, *txdesc, *txdesc_txie_len, DMA_TO_DEVICE);
        rtl_cache_sync_wback(Adapter, *content, *content_len, DMA_TO_DEVICE);
        rtl_cache_sync_wback(Adapter, dnld_htxbd, sizeof(*dnld_htxbd), DMA_TO_DEVICE);
#endif
...
{
    struct fwdl_ctx_t *fwdl_ctx;
    ASSIGN_GLOBAL_FWDL_CTX(fwdl_ctx);
    int ret = 0;
...
)
{
    ...
    pu1Byte pWFWRealStart=NULL, pDFWRealStart=NULL;
    u4Byte WFWRealLen, DFWRealLen;
#endif
...
#ifdef SUPPORT_EXTERNAL_WLANFW
#define DEF_WFW_FILENAME "rtl8814BfwMP.bin"
#define DEF_DFW_FILENAME "fw_target_RTL8814B.bin"
    if (Adapter->pshare->rf_ft_var.extfw & BIT(0)) { /* Wlan FW */
        const struct firmware *file_entry;
        int ret;
        unsigned long flags;

        SMP_UNLOCK(flags);
        ret = request_firmware(&file_entry, DEF_WFW_FILENAME, &Adapter->dev->dev);
        SMP_LOCK(flags);
        if (ret) {
            printk("open file failed, use hardcode fw\n");
        } else {
            pWFWRealStart = kmalloc(file_entry->size, GFP_KERNEL);
            if (pWFWRealStart == NULL) {
                printk("fail to allocate buffer for wfw (size %d)\n", file_entry->size);
            } else {
                memcpy(pWFWRealStart, file_entry->data, file_entry->size);
                pFWStart = pWFWRealStart;
                imageSZ = file_entry->size;
                printk(" >>> W %d %d %d %d <<< \n", pFWStart[12], pFWStart[13], pFWStart[14], pFWStart[15]);
            }
            release_firmware(file_entry);
        }
    }
    if (Adapter->pshare->rf_ft_var.extfw & BIT(1)) { /* DataCPU FW */
        const struct firmware *file_entry;
        int ret;
        unsigned long flags;

        SMP_UNLOCK(flags);
        ret = request_firmware(&file_entry, DEF_DFW_FILENAME, &Adapter->dev->dev);
        SMP_LOCK(flags);
        if (ret) {
            printk("open file failed, use hardcode fw\n");
        } else {
            pDFWRealStart = kmalloc(file_entry->size, GFP_KERNEL);
            if (pDFWRealStart == NULL) {
                printk("fail to allocate buffer for DFW (size %d)\n", file_entry->size);
            } else {
                memcpy(pDFWRealStart, file_entry->data, file_entry->size);
                pFWStartExt = pDFWRealStart;
                imageSZExt = file_entry->size;
                printk(" >>> D %d %d %d %d <<< \n", pFWStartExt[12], pFWStartExt[13], pFWStartExt[14], pFWStartExt[15]);
            }
            release_firmware(file_entry);
        }
    }
...
int _sys_ready(HAL_PADAPTER Adapter, struct cil_pkt_t *cil)
{
    ...
    if (cil_ofldVer != CIL_VERSION) {
        ...
        if (cil_d2h_sys_ready->sys_feature0 & CIL_D2H_SYS_READY_FLAGS_MCU_FW) {
            ...
            /* TODO: read endian? */
            _PRINT(PRN_ALWAYS, "WLCPU-FW info sig 0x%04x cat %d func %d v%d.%d.%d SVN%d %d/%d-%d:%d@%d",
                            *((u16 *)(fwhdr + 0)),
                            *(fwhdr + 2),
                            *(fwhdr + 3),
                            *((u16 *)(fwhdr + 4)),
...
void _query_seq_done(HAL_PADAPTER Adapter, struct cil_pkt_t *cil)
{
	struct rtl8192cd_priv *priv_intf = Adapter;
	struct cil_d2h_seq_report_t *cil_d2h_seq_report = (struct cil_d2h_seq_report_t *)cil->buf;
	...
	ifid = cil_d2h_seq_report->if_id;
	if (ifid > 0) {
#ifdef UNIVERSAL_REPEATER
		ifid_offset = 2;
		if (ifid == 1)
			priv_intf = Adapter->pvxd_priv;
#endif
#ifdef MBSSID
		if (ifid >= ifid_offset)
			priv_intf = Adapter->pvap_priv[ifid - ifid_offset];
#endif
	}
	pstat = get_macidinfo(priv_intf, cil_d2h_seq_report->macid);
	if (NULL == pstat)
		return;

	tid = cil_d2h_seq_report->tid;
	seq = (le32_to_cpu(cil_d2h_seq_report->seq) + pstat->ADDBA_accu[tid]) & 0xfff;

	pstat->AC_seq[tid] = seq;

	issue_ADDBAreq(priv_intf, pstat, tid);
}
...
)
{
	...
	case CIL_D2H_C_WMAC:
		...
		break;
	case CIL_D2H_C_PE:
		if (id == CIL_D2H_I_PE_SEQ)
			_query_seq_done(Adapter, cil);
		break;
	default:
...
u32 get_ext_fw_version(HAL_PADAPTER Adapter, HALMAC_FW_VERSION *fw_version)
{
    fw_version->version = FWHDR_GET_FIELD(GLOBAL_FWDL_CTX_IF.fwVersion, FWHDR_FW_VERSION);
    fw_version->sub_version = FWHDR_GET_FIELD(GLOBAL_FWDL_CTX_IF.fwVersion, FWHDR_FW_SUB_VERSION);
    fw_version->sub_index = FWHDR_GET_FIELD(GLOBAL_FWDL_CTX_IF.fwVersion, FWHDR_FW_PATCH_VERSION);
    fw_version->build_time.year= GLOBAL_FWDL_CTX_IF.fwYear;
    fw_version->build_time.month = GLOBAL_FWDL_CTX_IF.fwMonth;
    fw_version->build_time.date = GLOBAL_FWDL_CTX_IF.fwDay;
    fw_version->build_time.hour = GLOBAL_FWDL_CTX_IF.fwHour;
    fw_version->build_time.min = GLOBAL_FWDL_CTX_IF.fwMinute;
    return GLOBAL_FWDL_CTX_IF.fwRevision;
}

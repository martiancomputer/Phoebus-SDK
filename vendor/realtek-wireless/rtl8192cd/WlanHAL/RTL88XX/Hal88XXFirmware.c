...
#if (IS_EXIST_RTL8822BE || IX_EXIST_RTL8821CE || IS_EXIST_RTL8822CE || IS_EXIST_RTL8812FE)
        if ( IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8821C(Adapter) || IS_HARDWARE_TYPE_8822C(Adapter) || IS_HARDWARE_TYPE_8812F(Adapter))  {
            TXDESCSize = SIZE_TXDESC_88XX_V1;
        }
        else
...
{	
		...
		(WirelessMode & WIRELESS_MODE_AC_5G) )
	{
		...
		if ((pEntry->ht_cap_buf.ht_cap_info & HAL_cpu_to_le16(_HTCAP_SHORTGI_20M_)
				|| (Adapter->pshare->rf_ft_var.sgi_vw_force && pEntry->ht_cap_len)) //for tp-link 822 v2
	...
	{
		{
			if((( HAL_cpu_to_le32(pEntry->vht_cap_buf.vht_cap_info) & BIT(SHORT_GI80M_E))
				|| (Adapter->pshare->rf_ft_var.sgi_vw_force && pEntry->vht_cap_len))
...
)
{
	...
	for (i=0; i<=HALMAC_VHT_NSS4_MCS9; i++) {
		...
		if (Adapter->pshare->rf_ft_var.initial_pa - level * 10 > 5)
			txduty_table[i].duty = Adapter->pshare->rf_ft_var.initial_pa - level * 10;
		else
			txduty_table[i].rate = 5;
#else
...
)
{
    ...
    *(loc_page) = (u1Byte)(frlen /PBP_PSTX_SIZE);
	...
	if ( IS_HARDWARE_TYPE_8814A(Adapter) || IS_HARDWARE_TYPE_8822B(Adapter) || IS_HARDWARE_TYPE_8821C(Adapter) || IS_HARDWARE_TYPE_8814B(Adapter) || IS_HARDWARE_TYPE_8822C(Adapter) || IS_HARDWARE_TYPE_8812F(Adapter)) {
...
)
{
    ...
    switch(sub_id)
    {
		...
		case EXTEND_C2H_88XX_DETECT_THERMAL:
			// Do nothing to avoid warning message for recommended by kkbomb
		break;
        default:
            printk("[C2H EXT], Unkown packet!! CmdId(%#X)!sub_id(%#X)\n", c2hCmdId, sub_id);
        break;
...
)
{
	...
	memset(para, 0, sizeof(struct halmac_cfg_mumimo_para));

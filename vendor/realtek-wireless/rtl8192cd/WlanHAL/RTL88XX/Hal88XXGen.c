	...
	case HW_VAR_MAC_CONFIG: {
		...
		if ( _GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v1 ) {
			...
			if (IS_HARDWARE_TYPE_8192F(Adapter)) {
				HAL_RTL_W8(REG_AMPDU_MAX_TIME, 0x70);
			}
			else
...
/*
            ...
            if ( RT_OP_MODE_AP == pMacCfgPara->OP_Mode ) {
                ...
                HAL_RTL_W16(REG_RXFLTMAP1, BIT_CTRLFLT10EN);
                #if (IS_RTL8192E_SERIES || IS_RTL8197F_SERIES || IS_RTL8198F_SERIES || IS_RTL8814A_SERIES || IS_RTL8881A_SERIES)
                if(IS_HARDWARE_TYPE_8192E(Adapter) || IS_HARDWARE_TYPE_8197F(Adapter) || IS_HARDWARE_TYPE_8198F(Adapter) || IS_HARDWARE_TYPE_8814A(Adapter) || IS_HARDWARE_TYPE_8881A(Adapter))
    ...
    if( _GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v1_v2 ) {
        ...
        if(_GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v4)
        {
            ...
            HAL_RTL_W8(REG_ATIM_DTIM_CTRL_SEL, (HAL_RTL_R8(REG_ATIM_DTIM_CTRL_SEL) &~BIT_MASK_ATIM_DTIM_SEL) | BIT_ATIM_DTIM_SEL(0));
...
#if (IS_RTL8197F_SERIES || IS_RTL8822B_SERIES || IS_RTL8198F_SERIES || IS_RTL8822C_SERIES || IS_RTL8812F_SERIES)
            if (IS_HARDWARE_TYPE_8198F(Adapter) || IS_HARDWARE_TYPE_8822B(Adapter)||IS_HARDWARE_TYPE_8197F(Adapter) || IS_HARDWARE_TYPE_8822C(Adapter) || IS_HARDWARE_TYPE_8812F(Adapter)) {
                tmpU1Byte |= BIT_EN_BCN_FUNCTION | BIT_P0_EN_TXBCN_RPT;
            }
...
if (Adapter->hci_type == RTL_HCI_USB) {
...
#endif //(IS_RTL8197F_SERIES || IS_RTL8198F_SERIES)
#if IS_RTL8814B_SERIES
        if (IS_HARDWARE_TYPE_8814B(Adapter)){
	...
	case HW_VAR_HIQ_NO_LMT_EN: {
		...
		HAL_RTL_W8(REG_HIQ_NO_LMT_EN, HiQNoLMTEn2);
	}
    ...
    case HW_VAR_BMC_RTS0_INVALID:
		...
		{
			if ( IS_HARDWARE_TYPE_8192E(Adapter) || IS_HARDWARE_TYPE_8192F(Adapter) || IS_HARDWARE_TYPE_8881A(Adapter) )
			{
				u1Byte RegVal = *((pu1Byte)(val));
...
#if IS_RTL8192F_SERIES
		if ( IS_HARDWARE_TYPE_8192F(Adapter) ) {
			printk("[TXPWR_LMT_92FE_new]\n");
			*((unsigned long*)(val)) = (unsigned long)data_TXPWR_LMT_92FE_new_start;
		}
#endif
#endif
    ...
    if ( IS_HARDWARE_TYPE_8814B(Adapter) ) {
		...
		if(HAL_VAR_use_ext_pa){
			if(HAL_RFE_TYPE == 1) {
				...
				*((unsigned long*)(val)) = (unsigned long)data_TXPWR_LMT_8814Bmp_Type1_start;
			}else if(HAL_RFE_TYPE == 4) {
				panic_printk("[%s][TXPWR_LMT_8814Bmp_Type4]\n",__FUNCTION__);
				*((unsigned long*)(val)) = (unsigned long)data_TXPWR_LMT_8814Bmp_Type4_start;
			}else if(HAL_RFE_TYPE == 5) {
				panic_printk("[%s][TXPWR_LMT_8814Bmp_Type5]\n",__FUNCTION__);
				*((unsigned long*)(val)) = (unsigned long)data_TXPWR_LMT_8814Bmp_Type5_start;
			}else {
				panic_printk("[%s][TXPWR_LMT_8814Bmp]\n",__FUNCTION__);
				*((unsigned long*)(val)) = (unsigned long)data_TXPWR_LMT_8814Bmp_start;
			}
		}else
#endif
		{
			if(HAL_RFE_TYPE == 2) {
				...
				*((unsigned long*)(val)) = (unsigned long)data_TXPWR_LMT_8814Bmp_Type2_start;
			}else if(HAL_RFE_TYPE == 7) {
				panic_printk("[%s][TXPWR_LMT_8814Bmp_Type7]\n",__FUNCTION__);
				*((unsigned long*)(val)) = (unsigned long)data_TXPWR_LMT_8814Bmp_Type7_start;
			}else if(HAL_RFE_TYPE == 8) {
				panic_printk("[%s][TXPWR_LMT_8814Bmp_Type8]\n",__FUNCTION__);
				*((unsigned long*)(val)) = (unsigned long)data_TXPWR_LMT_8814Bmp_Type8_start;
			}else{
				panic_printk("[%s][TXPWR_LMT_8814Bmp]\n",__FUNCTION__);
				*((unsigned long*)(val)) = (unsigned long)data_TXPWR_LMT_8814Bmp_start;
			}
...
#if IS_RTL8192E_SERIES
		if ( IS_HARDWARE_TYPE_8192E(Adapter) ) {
			printk("[TxPowerTrack_AP_8192E]\n");
			*((unsigned long*)(val)) = (unsigned long)data_TxPowerTrack_AP_8192E_start;
		}
...
#if IS_RTL8192E_SERIES
		if ( IS_HARDWARE_TYPE_8192E(Adapter) ) {
			*((pu4Byte)(val)) = (u4Byte)(data_TxPowerTrack_AP_8192E_end - data_TxPowerTrack_AP_8192E_start);
		}
#endif // #if IS_RTL8192E_SERIES
#if IS_RTL8814A_SERIES
		if ( IS_HARDWARE_TYPE_8814A(Adapter) ) {
...
#endif
#if (IS_RTL8192E_SERIES || IS_RTL8192F_SERIES || IS_RTL8197F_SERIES || IS_RTL8822B_SERIES || IS_RTL8822C_SERIES || IS_RTL8812F_SERIES)
		if ( IS_HARDWARE_TYPE_8192E(Adapter) || IS_HARDWARE_TYPE_8192F(Adapter) || IS_HARDWARE_TYPE_8197F(Adapter) || IS_HARDWARE_TYPE_8822B(Adapter)|| IS_HARDWARE_TYPE_8822C(Adapter)|| IS_HARDWARE_TYPE_8812F(Adapter) ) {
...
)
{
    ...
    if (IS_SUPPORT_AP_SWPS_OFFLOAD(Adapter)) {
        ...
        Adapter->SWPS_RPT_macid_num = 1; //tmp test set 1
        HAL_RTL_W8(REG_SWPS_CTRL+1,(Adapter->SWPS_RPT_macid_num+1) & BIT_MASK_SWPS_RPT_LENGTH);

        SWPS_RPT_init_size = MaxMacIDNum*SWPSRPT_ENTRY_SIZE+SWPSRPT_BITMAP_SIZE+sizeof(RX_DESC_88XX); //16 is one report size,64 is MACID bitmap, 24 is RXDESC size
            
        HAL_RTL_W16(REG_PKTBUF_DBG_CTRL, 0x800);
        
        for(init_i=0;init_i<(SWPS_RPT_init_size/4);init_i++)
            HAL_RTL_W32(0x8000+(init_i*4), 0x0);

        //set SWPS_RPT bit in RXDESC
        HAL_RTL_W8(0x800a, BIT(1));

        //set gen report threshold
        HAL_RTL_W16(REG_SWPS_PKT_TH,0x100); //just random set for test
        HAL_RTL_W32(REG_SWPS_TIME_TH,BIT_SWPS_PSTIME_TH(0x40) | BIT_SWPS_TIME_TH(0x3E8)); //just random set for test
...
)
{
	...
	if (Adapter->hci_type == RTL_HCI_SDIO) {
		...
		if (IS_HARDWARE_TYPE_8814B(Adapter)) {
			...
			result = FirmwareDownload8814B(Adapter);
			priv->pshare->fw_download_status = result;
	...
	#if CFG_HAL_MAC_LOOPBACK
		result = GET_MACHALAPI_INTERFACE(Adapter)->halmac_init_mac_cfg(Adapter->pHalmac_adapter,HALMAC_TRX_MODE_LOOPBACK);
	#else // !(CFG_HAL_MAC_LOOPBACK)
	#ifdef CTC_WMM_LATENCY
		if ((priv->pshare->rf_ft_var.ctc_spec_wmm) && (HAL_VAR_PHYBANDSELECT == PHY_BAND_5G))
			result = GET_MACHALAPI_INTERFACE(Adapter)->halmac_init_mac_cfg(Adapter->pHalmac_adapter,HALMAC_TRX_MODE_CTC_SPEC_WMM);
		else
	#endif
		result = GET_MACHALAPI_INTERFACE(Adapter)->halmac_init_mac_cfg(Adapter->pHalmac_adapter,HALMAC_TRX_MODE_NORMAL);
...
GET_MACHALAPI_INTERFACE(Adapter)->halmac_cfg_drv_info(Adapter->pHalmac_adapter,HALMAC_DRV_INFO_PHY_STATUS);
	...
	if ((IS_HARDWARE_TYPE_8822B(priv)) || (IS_HARDWARE_TYPE_8821C(priv)) || (IS_HARDWARE_TYPE_8814B(priv)) || (IS_HARDWARE_TYPE_8822C(priv)) || (IS_HARDWARE_TYPE_8812F(priv))) {
        ...
        if(_GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v1_v2){
			...
			if (IS_HARDWARE_TYPE_8822B(Adapter))
				HAL_RTL_W32(REG_INT_MIG, 0x000000);
			else
#endif
            HAL_RTL_W32(REG_INT_MIG, 0x160000);
        }
	...
	if (Adapter->hci_type == RTL_HCI_USB) {
			...
			_InitQueuePriority_8192EU(Adapter);
...
)
{
	...
	//	| BIT_ACRC32 | BIT_AB | BIT_AM | BIT_APM | BIT_AAP);
 	    ...
 	    HAL_RTL_W32(REG_RCR, BIT_APP_FCS | BIT_APP_MIC | BIT_APP_ICV | BIT_APP_PHYSTS | BIT_VHT_DACK |  BIT_ACRC32 | BIT_AB | BIT_AM | BIT_APM | BIT_AAP);
...
#if CFG_HAL_SUPPORT_MBSSID
				if (Adapter->pmib->miscEntry.vap_enable) {
...
)
{
	...
	if (HAL_IS_PCIBIOS_TYPE(Adapter)) {
		...
		if (IS_HARDWARE_TYPE_8814B(Adapter)) {
			...
			_GET_HAL_DATA(Adapter)->h2d_fwdl_dma_handle = dma_handle_desc+(unsigned long)_GET_HAL_DATA(Adapter)->h2d_fwdl_cpu_addr-(unsigned long)cpu_addr_desc;
			printk("OFLD: cpu_addr %p, dma_handle %p, size %d, virt_to_phys=%08x\n", 
				_GET_HAL_DATA(Adapter)->h2d_fwdl_cpu_addr, 
				_GET_HAL_DATA(Adapter)->h2d_fwdl_dma_handle, 
				dma_len, (unsigned long)HAL_VIRT_TO_BUS(_GET_HAL_DATA(Adapter)->h2d_fwdl_cpu_addr));
		}
#endif
	}
...
)
{
    ...
    if ( IS_HARDWARE_TYPE_8822BE(Adapter) || IS_HARDWARE_TYPE_8197F(Adapter)|| IS_HARDWARE_TYPE_8198F(Adapter) || IS_HARDWARE_TYPE_8822CE(Adapter)|| IS_HARDWARE_TYPE_8812FE(Adapter)) {
...
#if (IS_EXIST_RTL8197FEM || IS_EXIST_RTL8197FEM)
    if (IS_HARDWARE_TYPE_8197F(Adapter) || IS_HARDWARE_TYPE_8198F(Adapter)) {
        value32 = HAL_RTL_R32(REG_HCI_CTRL);
        value32 = value32 & (~0x3fff);
        HAL_RTL_W32(REG_HCI_CTRL, value32);
    }
#endif //#if (IS_EXIST_RTL8197FEM || IS_EXIST_RTL8197FEM)
...
)
{
    ...
    if(IS_HARDWARE_TYPE_8814BE(Adapter)&& Adapter->pshare->rf_ft_var.manual_pe_enable){
        //release DataQ DMA in PE
    }else
#endif
    {
...
)
{
	...
	if ( IS_HARDWARE_TYPE_8197F(Adapter) || IS_HARDWARE_TYPE_8822C(Adapter) || IS_HARDWARE_TYPE_8812F(Adapter))
	{
		...
		IS_HARDWARE_TYPE_8881A(Adapter) ||IS_HARDWARE_TYPE_8814A(Adapter) ||
...
#endif
#if (IS_RTL8192E_SERIES || IS_RTL8192F_SERIES || IS_RTL8881A_SERIES || IS_RTL8814A_SERIES )
        if( IS_HARDWARE_TYPE_8192E(Adapter) || IS_HARDWARE_TYPE_8192F(Adapter) || IS_HARDWARE_TYPE_8881A(Adapter) ||IS_HARDWARE_TYPE_8814A(Adapter) )
...
)
{
    ...
    if(byteLen) {
        for (i = 0; i < byteLen; i++) {
            WriteVal = *(val + i);
            HAL_RTL_W8((macID * ctrlInfoSZ_8198 + i + byteOffset) | RWCtrlBit, WriteVal);

            //printk("address = %x content = %x \n",(macID*ctrlInfoSZ_8814 + i + byteOffset)|RWCtrlBit,WriteVal);
        }
    } else    {
        ReadVal =  HAL_RTL_R8((macID * ctrlInfoSZ_8198 + byteOffset) | RWCtrlBit);
        WriteVal =  ((ReadVal & (~(Mask << Shift))) | ((*(val)&Mask) << Shift));

        // printk("((*(val)&Mask)<<Shift)=%x \n",((*(val)&Mask)<<Shift));
        //  printk("ReadVal =%x WriteVal = %x \n",ReadVal,WriteVal);
        HAL_RTL_W8((macID * ctrlInfoSZ_8198 + byteOffset) | RWCtrlBit, WriteVal);
    }
...
)
{
	...
	// avoid stopping HIMR failed probably on SMP platform

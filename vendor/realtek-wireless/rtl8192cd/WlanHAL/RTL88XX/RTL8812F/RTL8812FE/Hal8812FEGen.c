	...
	)
{
	...
	if (p_dm_odm->priv->pshare->rf_ft_var.txbf_iqgen_keep_on)
		return;
	...
	)
{
	...
	odm_write_1byte(p_dm_odm, REG_TXBF_CTRL_8812F+3, (u1bTmp|BIT6)); //eric-mu
	...
	if ((pBeamformingInfo->beamformee_su_cnt > 0) && (BFeeIdx < BEAMFORMEE_ENTRY_NUM)) {
			...
			pBeamformeeEntry->su_reg_index == 0xFF)
		{
			...
			if (pBeamformeeEntry->su_reg_index == 0) {
				odm_write_2byte(p_dm_odm, REG_TXBF_CTRL_8812F, STAid|BIT14|BIT15);
	...
	if(pBeamformeeEntry && pBeamformeeEntry->BeamformEntryCap == BEAMFORMING_CAP_NONE) { //eric-txbf
		...
		{ /*SU BFee*/
			if(pBeamformeeEntry->su_reg_index != 0xFF) {
				...
				if (pBeamformeeEntry->su_reg_index == 0) {
					odm_write_2byte(p_dm_odm, REG_TXBF_CTRL_8812F, BIT14|BIT15);
					odm_write_1byte(p_dm_odm, REG_TXBF_CTRL_8812F+3, odm_read_1byte(p_dm_odm, REG_TXBF_CTRL_8812F+3)|BIT4|BIT6|BIT7);
			...
			if(pBeamformeeEntry->mu_reg_index < MAX_NUM_BEAMFORMEE_MU ) {
				...
				if(pBeamformingInfo->beamformee_mu_cnt > 0){
					...
					if (pBeamformingInfo->beamformee_mu_cnt < 2) {
						//use original RA mode
						//printk("[%s] use original RA mode \n", __func__);
						phydm_ra_mode_selection(ODMPTR,  0 );
						if(pBeamformeeEntry->pSTA->IOTPeer == HT_IOT_PEER_REALTEK){
							//close reg and do IQK, DPK
							//printk("[%s] close reg and do Tssi_On_Flow \n", __func__);
							RTL_W32(0x1e7c, RTL_R32(0x1e7c) | BIT30);
							Tssi_Enable_Flow(priv);
						}
					}
				}else

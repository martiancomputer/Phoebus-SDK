...
#if (CFG_HAL_DELAY_REFILL_RX_BUF == 0)
    // Free allocated skbs
    while (rxbd_idx != 0) {
        if (GET_HW(Adapter)->rx_infoL[--rxbd_idx].pbuf == NULL)
            continue;
...
#endif // (CFG_HAL_DELAY_REFILL_RX_BUF == 0)
...
#endif
        platform_zero_memory((void*)_GET_HAL_DATA(Adapter)->alloc_dma_buf, DESC_DMA_PAGE_SIZE_MAX_HAL_IF(Adapter));
...
#if IS_EXIST_RTL8822BE || IS_EXIST_RTL8197FEM || IS_EXIST_RTL8821CE || IS_EXIST_RTL8198FEM || (IS_EXIST_RTL8814BE && !defined(CONFIG_RTL_OFFLOAD_DRIVER)) || IS_EXIST_RTL8822CE || IS_EXIST_RTL8812FE
HAL_IMEM
enum rt_status
QueryRxDesc88XX_V1 (
    IN      HAL_PADAPTER    Adapter,

...
#if IS_RTL88XX_AC
#if (IS_RTL8814A_SERIES || IS_RTL8822B_SERIES || IS_RTL8822C_SERIES || IS_RTL8821C_SERIES || IS_RTL8198F_SERIES || IS_RTL8814B_SERIES || IS_RTL8812F_SERIES)
void
set_current_tx_agc(
...
#endif // (IS_RTL8814A_SERIES || IS_RTL8822B_SERIES || IS_RTL8822C_SERIES || IS_RTL8821C_SERIES || IS_RTL8198F_SERIES || IS_RTL8814B_SERIES || IS_RTL8812F_SERIES)
#endif // IS_RTL88XX_AC
...
#if (IS_RTL8198F_SERIES || IS_RTL8814B_SERIES || IS_RTL8812F_SERIES)
void TxLMT_OFDM_Jaguar_NEW(IN  HAL_PADAPTER	Adapter)
{
	...
	char	max_lmt_idx_ofdm = 0;
	char	max_lmt_idx_ht1s = 0, max_lmt_idx_ht2s = 0;
	char	max_lmt_idx_vht1s = 0, max_lmt_idx_vht2s = 0;
	char	max_lmt_idx_ht3s = 0, max_lmt_idx_ht4s = 0;
	char	max_lmt_idx_vht3s = 0, max_lmt_idx_vht4s = 0;
...
#endif // (IS_RTL8198F_SERIES || IS_RTL8814B_SERIES || IS_RTL8812F_SERIES)
#endif // IS_RTL88XX_AC
#if IS_RTL88XX_N
...
#if IS_RTL8814A_SERIES
		if (GET_CHIP_VER(Adapter) == VERSION_8814A) {
			if (RegAddr == 0x18 && eRFPath == 3)
				pathD_value = New_Value;
		}
#endif
	} else {
...
#if IS_RTL8814A_SERIES
	if (GET_CHIP_VER(Adapter) == VERSION_8814A) {
		// Work around: Read pathD rf reg 0x18 and check again to avoid previous write error
		if (RegAddr == 0x18 && eRFPath == 3) {
			Original_Value = GET_HAL_INTERFACE(priv)->PHYQueryRFRegHandler(Adapter, 3, RegAddr, BIT_MASK_SET_MASK20BITS_COMMON); 
			if (Original_Value != pathD_value) {
				phy_RFSerialWrite_88XX_AC(Adapter, 3, RegAddr, pathD_value);
				printk("!!!!!! Path D 0x18 Recovery !!!! RF 3 18=0x%x\n", 
					GET_HAL_INTERFACE(priv)->PHYQueryRFRegHandler(Adapter, 3, RegAddr, BIT_MASK_SET_MASK20BITS_COMMON));
			}
		}
	}
#endif
...
#if (IS_RTL8198F_SERIES)||(IS_RTL8814B_SERIES) || (IS_RTL8822C_SERIES) || (IS_RTL8812F_SERIES)
void TxPG_CCK_NEW
(
	IN  HAL_PADAPTER                Adapter
)
...
)
{
    ...
    if (GetChipIDMIMO88XX(Adapter) == RF_1T1R) {
        ...
        bPathXEn[2] = 0;
        bPathXEn[3] = 0;
        if(HAL_VAR_pwrlevelCCK_A(ch_idx) == 0  && (Adapter->pmib->dot11RFEntry.tssi_enable == 0)) {
        ...
        if ((HAL_VAR_pwrlevelCCK_A(ch_idx) == 0)||(HAL_VAR_pwrlevelCCK_B(ch_idx) == 0)		
			...
			if ( IS_HARDWARE_TYPE_8814B(Adapter) && (IS_HAL_TEST_CHIP(Adapter)==FALSE)) {
				...
				for (i=RF_PATH_A; i<RF_PATH_MAX;i++){		
					config_phydm_write_txagc_ref_8814b((&(Adapter->pshare->_dmODM)),def_power_value,i,PDM_CCK);					
					for (rate=ODM_RATE1M; rate<=ODM_RATE11M; rate++){
        ...
        for(i = 0; i < 4; i++) {
            ...
            if(bPathXEn[i]) {
				...
				if ( (IS_HARDWARE_TYPE_8814B(Adapter) && (IS_HAL_TEST_CHIP(Adapter)==FALSE))) {
						...
						config_phydm_write_txagc_ref_8814b((&(Adapter->pshare->_dmODM)),HAL_VAR_pwrlevelCCK_X[i]?HAL_VAR_pwrlevelCCK_X[i]:0x12,i,PDM_CCK);					
						for (rate=ODM_RATE1M; rate<=ODM_RATE11M; rate++){
							phydm_write_txagc_1byte_8814b((&(Adapter->pshare->_dmODM)),0,rate);
							set_current_tx_agc(Adapter,i,rate,HAL_VAR_pwrlevelCCK_X[i]?HAL_VAR_pwrlevelCCK_X[i]:0x12);
						}
...
)
{
	...
	if (Adapter->pmib->dot11RFEntry.tssi_enable) {
#if IS_RTL8198F_SERIES || IS_RTL8814B_SERIES || IS_RTL8822C_SERIES || IS_RTL8812F_SERIES
		if(IS_HARDWARE_TYPE_8198F(Adapter)||IS_HARDWARE_TYPE_8814B(Adapter)||IS_HARDWARE_TYPE_8822C(Adapter)||IS_HARDWARE_TYPE_8812F(Adapter)){
...
#endif			
#if IS_RTL8198F_SERIES || IS_RTL8814B_SERIES || IS_RTL8812F_SERIES
			if(IS_HARDWARE_TYPE_8198F(Adapter)||IS_HARDWARE_TYPE_8814B(Adapter)||IS_HARDWARE_TYPE_8812F(Adapter))
				TxLMT_OFDM_Jaguar_NEW(Adapter);
			else
#endif	
...
)
{
    ...
    for (i = 0; i < 4; i++) {
		...
		if ( IS_HARDWARE_TYPE_8812F(Adapter) && (IS_HAL_TEST_CHIP(Adapter)==FALSE)){
			if(Adapter->pmib->dot11RFEntry.tssi_enable){
					pwr_40_1s = halrf_get_tssi_codeword_for_txindex((&(Adapter->pshare->_dmODM)));
					diff_ofdm_1t= 0;
				}
...
#if (IS_RTL8198F_SERIES)
	u1Byte pwrlevelCCK_C = HAL_VAR_pwrlevelCCK_C(channel-1);
	u1Byte pwrlevelCCK_D = HAL_VAR_pwrlevelCCK_D(channel-1);
#endif
#if CFG_HAL_POWER_PERCENT_ADJUSTMENT
	s1Byte pwrdiff_percent = HAL_PwrPercent2PwrLevel(HAL_VAR_power_percent);
#endif
	...
	if (HAL_cck_pwr_max) {
		...
		if ( IS_HARDWARE_TYPE_8198F(Adapter) && (IS_HAL_TEST_CHIP(Adapter)==FALSE)){
			...
			if (pwrlevelCCK_A == 0 && pwrlevelCCK_B == 0
				...
				&& pwrlevelCCK_C == 0&& pwrlevelCCK_D == 0){

byte = 0x32; //def power

				for (path=RF_PATH_A; path<RF_PATH_MAX;path++){		
					config_phydm_write_txagc_ref_8198f((&(Adapter->pshare->_dmODM)),byte,path,PDM_CCK);					
					for (rate=ODM_RATE1M; rate<=ODM_RATE11M;rate++){ 	
						phydm_write_txagc_1byte_8198f((&(Adapter->pshare->_dmODM)),0,rate);
						set_current_tx_agc(Adapter,path,rate,byte);
					}
				...
				for (path=RF_PATH_A; path<RF_PATH_MAX;path++){
					config_phydm_write_txagc_ref_8198f((&(Adapter->pshare->_dmODM)),HAL_VAR_pwrlevelCCK_X[path]?HAL_VAR_pwrlevelCCK_X[path]:byte,path,PDM_CCK);
				
				}
					/*98F has new rule for power by rate and power limit */
					// need to implement

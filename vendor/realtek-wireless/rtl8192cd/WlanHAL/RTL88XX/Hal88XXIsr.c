...
)
{
    ...
    u4Byte bit_hisr1_ind_int = 0, bit_hisr2_ind_mask = 0, bit_hisr3_ind_mask = 0;
	...
	// 8812F & 8822C do not provide BIT_HISR2_IND_INT_MSK & BIT_HISR3_IND_INT_MSK
#if (IS_RTL8814A_SERIES || IS_RTL8197F_SERIES || IS_RTL8822B_SERIES || IS_RTL8821C_SERIES || IS_RTL8198F_SERIES )
    ...
    if (_GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v2_v3_v4) {
		...
		if ((GET_CHIP_VER(Adapter) != VERSION_8822C) && (GET_CHIP_VER(Adapter) != VERSION_8812F)) {
			if (pHalData->IntMask[2] == 0) {
				pHalData->IntMask[0] &= ~bit_hisr2_ind_mask;
			}
			if (pHalData->IntMask[3] == 0) {
				pHalData->IntMask[0] &= ~bit_hisr3_ind_mask;
			}
		}
    }
...
//       if ( _GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v2_v3_v4) {
#if (IS_RTL8814A_SERIES || IS_RTL8197F_SERIES || IS_RTL8822B_SERIES || IS_RTL8821C_SERIES || IS_RTL8198F_SERIES || IS_RTL8814B_SERIES )//IS_RTL88XX_MAC_V2_V3
        if ((GET_CHIP_VER(Adapter) == VERSION_8814A) || (GET_CHIP_VER(Adapter) == VERSION_8197F) || (GET_CHIP_VER(Adapter) == VERSION_8822B) || (GET_CHIP_VER(Adapter) == VERSION_8821C) || (GET_CHIP_VER(Adapter) == VERSION_8198F) || (GET_CHIP_VER(Adapter) == VERSION_8814B)) {
	...
	)
{
    ...
    if (_GET_HAL_DATA(Adapter)->MacVersion.is_MAC_v3_v4) {

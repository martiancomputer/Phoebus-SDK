...
)
{
	...
	if (!HalPwrSeqCmdParsing88XX(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
			PWR_INTF_SDIO_MSK, rtl8822C_card_enable_flow))
    {
...
{   
	...
	//		PWR_INTF_SDIO_MSK, rtl8822C_enter_lps_flow)) {
	...
	//if (_FALSE == HalPwrSeqCmdParsing88XX(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
	//		PWR_INTF_SDIO_MSK, rtl8822C_card_disable_flow)) {
...
)
{
    ...
    pHalFunc->InitPONHandler        = InitPON88XX; //InitPON8822CS;
    pHalFunc->InitMACHandler        = InitMAC88XX;
    pHalFunc->InitFirmwareHandler   = InitMIPSFirmware88XX;  
#if CFG_HAL_SUPPORT_MBSSID    
    pHalFunc->InitMBSSIDHandler     = InitMBSSID_8822CS;
    ...
    //
#if CFG_HAL_SUPPORT_MBSSID        
    pHalFunc->StopMBSSIDHandler     = StopMBSSID_8822CS;
#endif  //CFG_HAL_SUPPORT_MBSSID
    pHalFunc->StopHWHandler         = StopHW_8822CS;
    pHalFunc->StopSWHandler         = StopSW88XX;
    pHalFunc->DisableVXDAPHandler   = DisableVXDAP88XX;
    pHalFunc->ResetHWForSurpriseHandler     = ResetHWForSurprise8822CS;

    //
    //ISR Related
    //
    pHalFunc->InitIMRHandler                    = InitIMR_8822CS;
    pHalFunc->EnableIMRHandler                  = EnableIMR_8822CS;
    ...
    pHalFunc->SetMACIDSleepHandler          =   SetMACIDSleep_8822CS;
    ...
    pHalFunc->UpdateHalMSRRPTHandler        = UpdateHalMSRRPT_8822CS;
	...
	)
{
	...
	Nr_index = TxBF_Nr(halTxbf8822C_GetNtx(priv), BeamformeeEntry.CompSteeringNumofBFer);
	...
	if(bSelfBeamformer) {			// for 8814 19ac(idx 1), 19b4(idx 0), different Tx ant setting
		...
		if(idx == 0) {
			switch(Nr_index) {			
				...
				PHY_SetBBReg(priv, REG_BB_TX_PATH_SEL_1, BIT23|BIT22|BIT21|BIT20, 0x6);	/*set TxPath selection for 8822C BFer bug refine*/
				...
				PHY_SetBBReg(priv, REG_BB_TX_PATH_SEL_1, BIT23|BIT22|BIT21|BIT20, 0xe);	/*set TxPath selection for 8822C BFer bug refine*/
				...
				PHY_SetBBReg(priv, REG_BB_TX_PATH_SEL_1, BIT23|BIT22|BIT21|BIT20, 0xf);	/*set TxPath selection for 8822C BFer bug refine*/
	...
	)
{
	...
	halTxbf8822C_GetBeamformCap(priv);
	...
	if ((pBeamformingInfo->BeamformCap & BEAMFORMEE_CAP) && (BFerIdx < BEAMFORMER_ENTRY_NUM)) {
		...
		Nc_index = halTxbf8822C_GetNrx(priv);	// for 8822C Nrx = 3(4 Ant), min=0(1 Ant)
	...
	if((pBeamformingInfo->BeamformCap & BEAMFORMER_CAP) && (BFeeIdx < BEAMFORMEE_ENTRY_NUM)) {	
		...
		SetBeamformRfMode8822C(priv, pBeamformingInfo, BFeeIdx);	

...
Abstract:
	Defined RTL8822CE HAL Function
...
)
{
    ...
    // TODO: Filen, check 8822C setting
	...
	if (!HalPwrSeqCmdParsing88XX(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK,
			PWR_INTF_PCI_MSK, rtl8822C_card_enable_flow))
    {
...
)
{
    ...
    pHalFunc->ResetHWForSurpriseHandler     = ResetHWForSurprise8822CE;
...
//    pHalFunc->ReleaseShortCutTxDescHandler  = ReleaseShortCutTxDesc88XX;
    ...
    // TODO: 8822C Power Tracking should be done
    pHalFunc->TXPowerTrackingHandler        = TXPowerTracking_ThermalMeter_Tmp8822C;
    pHalFunc->PHYSSetRFRegHandler           = PHY_SetRFReg_88XX_AC; //config_phydm_write_rf_reg_8822C; 
    pHalFunc->PHYQueryRFRegHandler          = PHY_QueryRFReg_8822C; //config_phydm_read_rf_reg_8822C;
    pHalFunc->IsBBRegRangeHandler           = IsBBRegRange88XX_V1;
    pHalFunc->PHYSetSecCCATHbyRXANT         = PHY_Set_SecCCATH_by_RXANT_8822C;
    pHalFunc->PHYSpurCalibration            = phy_SpurCalibration_8822C;
	...
	)
{
	...
	odm_write_1byte(p_dm_odm, REG_TXBF_CTRL_8822C, (u1bTmp|BIT6)); //eric-mu
	/* 0x45F[7:0] = 0x10 (Rate=OFDM_6M, BW20) */
	odm_write_1byte(p_dm_odm, REG_NDPA_OPT_CTRL_8822C, 0x10);
	...
	odm_set_bb_reg(p_dm_odm, 0x19E0 , BIT5|BIT4, 3); /*8822C grouping method*/

	if (p_dm_odm->rf_type == RF_2T2R) { /*2T2R*/
		PHYDM_DBG(p_dm_odm, DBG_TXBF, "%s: rf_type is 2T2R\n", __func__);
		//config_phydm_trx_mode_8822C(p_dm_odm, 3, 3, 1);/*Tx2path*/
	}
}
	...
	)
{
	...
	if ((pBeamformingInfo->beamformer_su_cnt > 0) && (BFerIdx < BEAMFORMER_ENTRY_NUM)) {
		...
		odm_write_1byte(p_dm_odm, REG_SND_PTCL_CTRL_8822C, 0xDB);	
				
		/*MAC address/Partial AID of Beamformer*/
		if (pBeamformerEntry->su_reg_index == 0) {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte(p_dm_odm, (REG_ASSOCIATED_BFMER0_INFO_8822C+i), pBeamformerEntry->MacAddr[i]);
		} else {
			for (i = 0; i < 6 ; i++)
				odm_write_1byte(p_dm_odm, (REG_ASSOCIATED_BFMER1_INFO_8822C+i), pBeamformerEntry->MacAddr[i]);
		}

		/*CSI report parameters of Beamformer*/
		Nc_index = halTxbf8822C_GetNrx(p_dm_odm);	/*for 8814A Nrx = 3(4 Ant), min=0(1 Ant)*/
		...
		odm_write_1byte(p_dm_odm, REG_SND_PTCL_CTRL_8822C+3, 0x70);
	...
	if ((pBeamformingInfo->beamformee_su_cnt > 0) && (BFeeIdx < BEAMFORMEE_ENTRY_NUM)) {
			...
			pBeamformeeEntry->su_reg_index == 0xFF)
		{
	...
	#endif
			halTxbf8822C_RfMode(p_dm_odm, pBeamformingInfo, BFeeIdx);
			...
			if (pBeamformeeEntry->su_reg_index == 0) {	
				odm_write_2byte(p_dm_odm, REG_TXBF_CTRL_8822C, STAid);	
				odm_write_1byte(p_dm_odm, REG_TXBF_CTRL_8822C+3, odm_read_1byte(p_dm_odm, REG_TXBF_CTRL_8822C+3)|BIT4|BIT6|BIT7);
			} else {
				odm_write_2byte(p_dm_odm, REG_TXBF_CTRL_8822C+2, STAid | BIT14 | BIT15 | BIT12);
			}	

			/*CSI report parameters of Beamformee*/
			if (pBeamformeeEntry->su_reg_index == 0) {
				/*Get BIT24 & BIT25*/
				u1Byte	tmp = odm_read_1byte(p_dm_odm, REG_ASSOCIATED_BFMEE_SEL_8822C+3) & 0x3;
			
				odm_write_2byte(p_dm_odm, REG_ASSOCIATED_BFMEE_SEL_8822C, STAid | BIT9);
			} else		
				odm_write_2byte(p_dm_odm, REG_ASSOCIATED_BFMEE_SEL_8822C+2, STAid | 0x0200);	/*Set BIT25*/
	...
	if ((pBeamformingInfo->beamformer_mu_cnt > 0) && (BFerIdx < BEAMFORMER_ENTRY_NUM)) {
		...
		if(pBeamformerEntry->BeamformEntryCap & BEAMFORM_CAP_VHT_MU_BFEE)
		{
			...
			/* Set partial AID */
			odm_write_2byte(p_dm_odm, (REG_ASSOCIATED_BFMER0_INFO_8822C+6), pBeamformerEntry->P_AID);
			...
			u1bTmp = odm_read_1byte(p_dm_odm, REG_RXFLTMAP0_8822C+1);
			u1bTmp |= 0x40;
			odm_write_1byte(p_dm_odm, REG_RXFLTMAP0_8822C+1, u1bTmp);
			/* Set 0x6A2[5:4] = 1 to NDPA and BF report poll */
			u1bTmp = odm_read_1byte(p_dm_odm, REG_RXFLTMAP1_8822C);
			u1bTmp |= 0x30;
			odm_write_1byte(p_dm_odm, REG_RXFLTMAP1_8822C, u1bTmp);
		
			/*CSI report parameters of Beamformer*/
			Nc_index = halTxbf8822C_GetNrx(p_dm_odm);	/* Depend on RF type */
	...
	if ((pBeamformingInfo->beamformee_mu_cnt > 0) && (BFeeIdx < BEAMFORMEE_ENTRY_NUM)) {
		...
		if((pBeamformeeEntry->BeamformEntryCap & BEAMFORM_CAP_VHT_MU_BFER) && pBeamformeeEntry->mu_reg_index == 0xFF)
		{
			...
			/* 0x42C[30] = 1 (0: from Tx desc, 1: from 0x45F) */
			u1bTmp = odm_read_1byte(p_dm_odm, REG_TXBF_CTRL_8822C+3);
			u1bTmp |= 0xD0; // Set bit 28, 30, 31 to 3b'111

			odm_write_1byte(p_dm_odm, REG_TXBF_CTRL_8822C+3, u1bTmp);

				if(AC_SIGMA_MODE == AC_SIGMA_APUT) {
					/* Set NDPA to VHT-1SS0 BW = 80M for 11ac logo r2*/
					odm_write_1byte(p_dm_odm, REG_NDPA_RATE_8822C, 0x2c); // VHT 1SS-0
					odm_write_1byte(p_dm_odm, REG_NDPA_OPT_CTRL_8822C, 0x12); //BW=80M
				}
				else
				{
					/* Set NDPA to 6M*/
					odm_write_1byte(p_dm_odm, REG_NDPA_RATE_8822C, 0x4); // 6M

					u1bTmp = odm_read_1byte(p_dm_odm, REG_NDPA_OPT_CTRL_8822C);
					u1bTmp &= 0xFC; // Clear bit 0, 1
					odm_write_1byte(p_dm_odm, REG_NDPA_OPT_CTRL_8822C, u1bTmp);
				}

			u4bTmp = odm_read_4byte(p_dm_odm, REG_SND_PTCL_CTRL_8822C);
			u4bTmp = ((u4bTmp & 0xFF0000FF) | 0x020200); // Set [23:8] to 0x0202
			odm_write_4byte(p_dm_odm, REG_SND_PTCL_CTRL_8822C, u4bTmp);	
	...
	)
{
	...
	if(pBeamformerEntry && pBeamformerEntry->BeamformEntryCap == BEAMFORMING_CAP_NONE) { //eric-txbf
		pBeamformerEntry = &pBeamformingInfo->BeamformerEntry[Idx];
		odm_write_1byte(p_dm_odm, REG_SND_PTCL_CTRL_8822C, 0xD8);	
#if (MU_BEAMFORMING_SUPPORT == 1)		
		...
		{ /*SU BFer */
			if(pBeamformerEntry->su_reg_index != 0xFF) {
				...
				if (pBeamformerEntry->su_reg_index == 0) {	
					odm_write_4byte(p_dm_odm, REG_ASSOCIATED_BFMER0_INFO_8822C, 0);
					odm_write_2byte(p_dm_odm, REG_ASSOCIATED_BFMER0_INFO_8822C+4, 0);
					odm_write_2byte(p_dm_odm, 0x6F4, 0);
				} else {
					odm_write_4byte(p_dm_odm, REG_ASSOCIATED_BFMER1_INFO_8822C, 0);
					odm_write_2byte(p_dm_odm, REG_ASSOCIATED_BFMER1_INFO_8822C+4, 0);
	...
	if(pBeamformeeEntry && pBeamformeeEntry->BeamformEntryCap == BEAMFORMING_CAP_NONE) { //eric-txbf
		halTxbf8822C_RfMode(p_dm_odm, pBeamformingInfo, Idx);
#if (MU_BEAMFORMING_SUPPORT == 1)		
		...
		{ /*SU BFee*/
			if(pBeamformeeEntry->su_reg_index != 0xFF) { 
				...
				if (pBeamformeeEntry->su_reg_index == 0) {	
					odm_write_2byte(p_dm_odm, REG_TXBF_CTRL_8822C, 0x0);	
					odm_write_1byte(p_dm_odm, REG_TXBF_CTRL_8822C+3, odm_read_1byte(p_dm_odm, REG_TXBF_CTRL_8822C+3)|BIT4|BIT6|BIT7);
					odm_write_2byte(p_dm_odm, REG_ASSOCIATED_BFMEE_SEL_8822C, 0);
				} else {
					odm_write_2byte(p_dm_odm, REG_TXBF_CTRL_8822C+2, 0x0 | BIT14 | BIT15 | BIT12);

					odm_write_2byte(p_dm_odm, REG_ASSOCIATED_BFMEE_SEL_8822C+2, 
					odm_read_2byte(p_dm_odm, REG_ASSOCIATED_BFMEE_SEL_8822C+2) & 0x60);
				}
	...
	)
{
	...
	if(pSoundingInfo->State == SOUNDING_STATE_SU_SOUNDDOWN) {
		...
		if (pBeamformEntry->su_reg_index == 0) {
			BeamCtrlReg = REG_TXBF_CTRL_8822C;
		} else {
			BeamCtrlReg = REG_TXBF_CTRL_8822C+2;
		...
		tmpVal = odm_read_2byte(p_dm_odm, REG_TXBF_CTRL_8822C);
		odm_write_2byte(p_dm_odm, REG_TXBF_CTRL_8822C, tmpVal|BIT15);
	} 
#if (MU_BEAMFORMING_SUPPORT == 1)	
	else {
	...
	)
{
	...
	if (pBeamEntry->BeamformEntryState == BEAMFORMING_ENTRY_STATE_PROGRESSING)
		halTxbf8822C_DownloadNDPA(Adapter, Idx);

	halTxbf8822C_FwTxBFCmd(Adapter);
#endif
}

...
#include "halmac_init_8812f.h"
#include "halmac_8812f_cfg.h"
#if HALMAC_PCIE_SUPPORT
#include "halmac_pcie_8812f.h"
#endif
#if HALMAC_SDIO_SUPPORT
#include "halmac_sdio_8812f.h"
...
#define RSVD_PG_CSIBUF_NUM		20
#define RSVD_PG_DLLB_NUM		(TX_FIFO_SIZE_8812F / 3 >> \
...
#define WLAN_RRSC_BITMAP	0x1FF

#define WLAN_RX_FILTER0		0xFFFFFFFF
#define WLAN_RX_FILTER2		0xFFFF
#define WLAN_RCR_CFG		0xE418220E
...
static struct halmac_rqpn HALMAC_RQPN_SDIO_8812F[] = {
...
/*PCIE RQPN Mapping*/
static struct halmac_rqpn HALMAC_RQPN_PCIE_8812F[] = {
	/* { mode, vo_map, vi_map, be_map, bk_map, mg_map, hi_map } */
	{HALMAC_TRX_MODE_NORMAL,
	 HALMAC_MAP2_NQ, HALMAC_MAP2_NQ, HALMAC_MAP2_LQ, HALMAC_MAP2_LQ,
	 HALMAC_MAP2_EXQ, HALMAC_MAP2_HQ},
	...
	{HALMAC_TRX_MODE_DELAY_LOOPBACK,
	 ...
	 HALMAC_MAP2_EXQ, HALMAC_MAP2_HQ},
	{HALMAC_TRX_MODE_CTC_SPEC_WMM,
	 HALMAC_MAP2_LQ, HALMAC_MAP2_LQ, HALMAC_MAP2_LQ, HALMAC_MAP2_LQ,
	 HALMAC_MAP2_EXQ, HALMAC_MAP2_HQ},
};

/*USB 2 Bulkout RQPN Mapping*/
static struct halmac_rqpn HALMAC_RQPN_2BULKOUT_8812F[] = {
	/* { mode, vo_map, vi_map, be_map, bk_map, mg_map, hi_map } */
	{HALMAC_TRX_MODE_NORMAL,
...
static struct halmac_rqpn HALMAC_RQPN_3BULKOUT_8812F[] = {
	/* { mode, vo_map, vi_map, be_map, bk_map, mg_map, hi_map } */
	{HALMAC_TRX_MODE_NORMAL,
	 HALMAC_MAP2_NQ, HALMAC_MAP2_NQ, HALMAC_MAP2_LQ, HALMAC_MAP2_LQ,
	 HALMAC_MAP2_HQ, HALMAC_MAP2_HQ},
...
static struct halmac_rqpn HALMAC_RQPN_4BULKOUT_8812F[] = {
	/* { mode, vo_map, vi_map, be_map, bk_map, mg_map, hi_map } */
	{HALMAC_TRX_MODE_NORMAL,
	 HALMAC_MAP2_NQ, HALMAC_MAP2_NQ, HALMAC_MAP2_LQ, HALMAC_MAP2_LQ,
	 HALMAC_MAP2_EXQ, HALMAC_MAP2_HQ},
...
#if HALMAC_PLATFORM_WINDOWS
/*SDIO Page Number*/
static struct halmac_pg_num HALMAC_PG_NUM_SDIO_8812F[] = {
	...
	{HALMAC_TRX_MODE_CTC_SPEC_WMM, 64, 64, 64, 0, 1},
};
#else
/*SDIO Page Number*/
static struct halmac_pg_num HALMAC_PG_NUM_SDIO_8812F[] = {
	...
	{HALMAC_TRX_MODE_CTC_SPEC_WMM, 64, 64, 64, 64, 1},
};
#endif

/*PCIE Page Number*/
static struct halmac_pg_num HALMAC_PG_NUM_PCIE_8812F[] = {
	/* { mode, hq_num, nq_num, lq_num, exq_num, gap_num} */
	{HALMAC_TRX_MODE_NORMAL, 16, 16, 64, 16, 1},
	{HALMAC_TRX_MODE_TRXSHARE, 64, 64, 64, 64, 1},
	{HALMAC_TRX_MODE_WMM, 64, 64, 64, 64, 1},
	{HALMAC_TRX_MODE_P2P, 64, 64, 64, 64, 1},
	{HALMAC_TRX_MODE_LOOPBACK, 64, 64, 64, 64, 1},
	{HALMAC_TRX_MODE_DELAY_LOOPBACK, 64, 64, 64, 64, 1},
	{HALMAC_TRX_MODE_CTC_SPEC_WMM, 16, 0, 64, 16, 1},
};

/*USB 2 Bulkout Page Number*/
static struct halmac_pg_num HALMAC_PG_NUM_2BULKOUT_8812F[] = {
	/* { mode, hq_num, nq_num, lq_num, exq_num, gap_num} */
	...
	{HALMAC_TRX_MODE_CTC_SPEC_WMM, 64, 64, 0, 0, 1},
};

/*USB 3 Bulkout Page Number*/
static struct halmac_pg_num HALMAC_PG_NUM_3BULKOUT_8812F[] = {
	/* { mode, hq_num, nq_num, lq_num, exq_num, gap_num} */
	{HALMAC_TRX_MODE_NORMAL, 64, 64, 64, 0, 1},
...
static struct halmac_pg_num HALMAC_PG_NUM_4BULKOUT_8812F[] = {
	/* { mode, hq_num, nq_num, lq_num, exq_num, gap_num} */
	{HALMAC_TRX_MODE_NORMAL, 64, 64, 64, 64, 1},
	{HALMAC_TRX_MODE_TRXSHARE, 64, 64, 64, 64, 1},
	{HALMAC_TRX_MODE_WMM, 64, 64, 64, 64, 1},
...
txdma_queue_mapping_8812f(struct halmac_adapter *adapter,
			  enum halmac_trx_mode mode);

static enum halmac_ret_status
priority_queue_cfg_8812f(struct halmac_adapter *adapter,
			 enum halmac_trx_mode mode);

static enum halmac_ret_status
set_trx_fifo_info_8812f(struct halmac_adapter *adapter,
			enum halmac_trx_mode mode);

static void
init_txq_ctrl_8812f(struct halmac_adapter *adapter);

static void
init_sifs_ctrl_8812f(struct halmac_adapter *adapter);

static void
init_rate_fallback_ctrl_8812f(struct halmac_adapter *adapter);

enum halmac_ret_status
mount_api_8812f(struct halmac_adapter *adapter)
{
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;

	adapter->chip_id = HALMAC_CHIP_ID_8812F;
	adapter->hw_cfg_info.efuse_size = EFUSE_SIZE_8812F;
	adapter->hw_cfg_info.eeprom_size = EEPROM_SIZE_8812F;
	adapter->hw_cfg_info.bt_efuse_size = BT_EFUSE_SIZE_8812F;
	adapter->hw_cfg_info.prtct_efuse_size = PRTCT_EFUSE_SIZE_8812F;
	adapter->hw_cfg_info.cam_entry_num = SEC_CAM_NUM_8812F;
	adapter->hw_cfg_info.tx_fifo_size = TX_FIFO_SIZE_8812F;
	adapter->hw_cfg_info.rx_fifo_size = RX_FIFO_SIZE_8812F;
	adapter->hw_cfg_info.ac_oqt_size = OQT_ENTRY_AC_8812F;
	adapter->hw_cfg_info.non_ac_oqt_size = OQT_ENTRY_NOAC_8812F;
	adapter->hw_cfg_info.usb_txagg_num = BLK_DESC_NUM;
	adapter->txff_alloc.rsvd_drv_pg_num = RSVD_PG_DRV_NUM;

	api->halmac_init_trx_cfg = init_trx_cfg_8812f;
	api->halmac_init_system_cfg = init_system_cfg_8812f;
	api->halmac_init_protocol_cfg = init_protocol_cfg_8812f;
	api->halmac_init_h2c = init_h2c_8812f;
	api->halmac_pinmux_get_func = pinmux_get_func_8812f;
	api->halmac_pinmux_set_func = pinmux_set_func_8812f;
	api->halmac_pinmux_free_func = pinmux_free_func_8812f;
	api->halmac_get_hw_value = get_hw_value_8812f;
	api->halmac_set_hw_value = set_hw_value_8812f;
	api->halmac_cfg_drv_info = cfg_drv_info_8812f;
	api->halmac_fill_txdesc_checksum = fill_txdesc_check_sum_8812f;
	api->halmac_init_low_pwr = init_low_pwr_8812f;
	api->halmac_pre_init_system_cfg = pre_init_system_cfg_8812f;

	api->halmac_init_wmac_cfg = init_wmac_cfg_8812f;
	api->halmac_init_edca_cfg = init_edca_cfg_8812f;

	if (adapter->intf == HALMAC_INTERFACE_SDIO) {
#if HALMAC_SDIO_SUPPORT
		adapter->sdio_hw_info.tx_addr_format = HALMAC_SDIO_AGG_MODE;
		api->halmac_init_interface_cfg = init_sdio_cfg_8812f;
		api->halmac_init_sdio_cfg = init_sdio_cfg_8812f;
		api->halmac_mac_power_switch = mac_pwr_switch_sdio_8812f;
		api->halmac_phy_cfg = phy_cfg_sdio_8812f;
		api->halmac_pcie_switch = pcie_switch_sdio_8812f;
		api->halmac_interface_integration_tuning = intf_tun_sdio_8812f;
		api->halmac_tx_allowed_sdio = tx_allowed_sdio_8812f;
		api->halmac_get_sdio_tx_addr = get_sdio_tx_addr_8812f;
		api->halmac_reg_read_8 = reg_r8_sdio_8812f;
		api->halmac_reg_write_8 = reg_w8_sdio_8812f;
		api->halmac_reg_read_16 = reg_r16_sdio_8812f;
		api->halmac_reg_write_16 = reg_w16_sdio_8812f;
		api->halmac_reg_read_32 = reg_r32_sdio_8812f;
		api->halmac_reg_write_32 = reg_w32_sdio_8812f;

		adapter->sdio_fs.macid_map_size = MACID_MAX_8812F << 1;
...
#if HALMAC_USB_SUPPORT
		api->halmac_mac_power_switch = mac_pwr_switch_usb_8812f;
		api->halmac_phy_cfg = phy_cfg_usb_8812f;
		api->halmac_pcie_switch = pcie_switch_usb_8812f;
		api->halmac_interface_integration_tuning = intf_tun_usb_8812f;
#endif
	} else if (adapter->intf == HALMAC_INTERFACE_PCIE) {
#if HALMAC_PCIE_SUPPORT
		api->halmac_mac_power_switch = mac_pwr_switch_pcie_8812f;
		api->halmac_phy_cfg = phy_cfg_pcie_8812f;
		api->halmac_pcie_switch = pcie_switch_8812f;
		api->halmac_interface_integration_tuning = intf_tun_pcie_8812f;
#endif
	} else {
...
/**
 * init_trx_cfg_8812f() - config trx dma register
 ...
 */
enum halmac_ret_status
init_trx_cfg_8812f(struct halmac_adapter *adapter, enum halmac_trx_mode mode)
{
	u8 value8;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	...
	status = txdma_queue_mapping_8812f(adapter, mode);
	...
	status = priority_queue_cfg_8812f(adapter, mode);
	if (status != HALMAC_RET_SUCCESS) {
		PLTFM_MSG_ERR("[ERR]priority queue cfg\n");
		return status;
	}

	status = init_h2c_8812f(adapter);
	if (status != HALMAC_RET_SUCCESS) {
			  ...
			  enum halmac_trx_mode mode)
{
	...
	if (adapter->intf == HALMAC_INTERFACE_SDIO) {
		cur_rqpn_sel = HALMAC_RQPN_SDIO_8812F;
	} else if (adapter->intf == HALMAC_INTERFACE_PCIE) {
		cur_rqpn_sel = HALMAC_RQPN_PCIE_8812F;
	} else if (adapter->intf == HALMAC_INTERFACE_USB) {
		if (adapter->bulkout_num == 2) {
			cur_rqpn_sel = HALMAC_RQPN_2BULKOUT_8812F;
		} else if (adapter->bulkout_num == 3) {
			cur_rqpn_sel = HALMAC_RQPN_3BULKOUT_8812F;
		} else if (adapter->bulkout_num == 4) {
			cur_rqpn_sel = HALMAC_RQPN_4BULKOUT_8812F;
		} else {
			 ...
			 enum halmac_trx_mode mode)
{
	...
	status = set_trx_fifo_info_8812f(adapter, mode);
	if (status != HALMAC_RET_SUCCESS) {
	...
	if (adapter->intf == HALMAC_INTERFACE_SDIO) {
		cur_pg_num = HALMAC_PG_NUM_SDIO_8812F;
	} else if (adapter->intf == HALMAC_INTERFACE_PCIE) {
		cur_pg_num = HALMAC_PG_NUM_PCIE_8812F;
	} else if (adapter->intf == HALMAC_INTERFACE_USB) {
		if (adapter->bulkout_num == 2) {
			cur_pg_num = HALMAC_PG_NUM_2BULKOUT_8812F;
		} else if (adapter->bulkout_num == 3) {
			cur_pg_num = HALMAC_PG_NUM_3BULKOUT_8812F;
		} else if (adapter->bulkout_num == 4) {
			cur_pg_num = HALMAC_PG_NUM_4BULKOUT_8812F;
		} else {
			PLTFM_MSG_ERR("[ERR]interface not support\n");
			return HALMAC_RET_NOT_SUPPORT;
		}
...
{
	u16 cur_pg_addr;
	u32 txff_size = TX_FIFO_SIZE_8812F;
	u32 rxff_size = RX_FIFO_SIZE_8812F;
	struct halmac_txff_allocation *info = &adapter->txff_alloc;

	if (info->rx_fifo_exp_mode == HALMAC_RX_FIFO_EXPANDING_MODE_1_BLOCK) {
		txff_size = TX_FIFO_SIZE_RX_EXPAND_1BLK_8812F;
		rxff_size = RX_FIFO_SIZE_RX_EXPAND_1BLK_8812F;
	} else if (info->rx_fifo_exp_mode ==
		   HALMAC_RX_FIFO_EXPANDING_MODE_2_BLOCK) {
		txff_size = TX_FIFO_SIZE_RX_EXPAND_2BLK_8812F;
		rxff_size = RX_FIFO_SIZE_RX_EXPAND_2BLK_8812F;
	} else if (info->rx_fifo_exp_mode ==
		   HALMAC_RX_FIFO_EXPANDING_MODE_3_BLOCK) {
		txff_size = TX_FIFO_SIZE_RX_EXPAND_3BLK_8812F;
		rxff_size = RX_FIFO_SIZE_RX_EXPAND_3BLK_8812F;
	} else if (info->rx_fifo_exp_mode ==
		   HALMAC_RX_FIFO_EXPANDING_MODE_4_BLOCK) {
		txff_size = TX_FIFO_SIZE_RX_EXPAND_4BLK_8812F;
		rxff_size = RX_FIFO_SIZE_RX_EXPAND_4BLK_8812F;
	}

	if (info->la_mode != HALMAC_LA_MODE_DISABLE) {
		txff_size = TX_FIFO_SIZE_LA_8812F;
		rxff_size = RX_FIFO_SIZE_8812F;
	}
...
/**
 * init_system_cfg_8812f() -  init system config
 * @adapter : the adapter of halmac
 * Author : Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
init_system_cfg_8812f(struct halmac_adapter *adapter)
{
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	u32 tmp = 0;
	u32 value32;
	...
	if (adapter->chip_ver == HALMAC_CHIP_VER_B_CUT) {
		...
		value32 = HALMAC_REG_R32(REG_ANAPAR_XTAL_1);
		value32 = value32 & ~(BIT_XTAL_DELAY_DIGI_V2);
		HALMAC_REG_W32(REG_ANAPAR_XTAL_1, value32);
	}
...
/**
 * init_protocol_cfg_8812f() - config protocol register
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
init_protocol_cfg_8812f(struct halmac_adapter *adapter)
{
	u32 max_agg_num;
	u32 max_rts_agg_num;
	u32 value32;
	...
	init_txq_ctrl_8812f(adapter);
	init_sifs_ctrl_8812f(adapter);
	init_rate_fallback_ctrl_8812f(adapter);
	...
	value32 = HALMAC_REG_R32(REG_RRSR);
	value32 = (value32 & ~(0xFFFFF)) | WLAN_RRSC_BITMAP;
	...
	value32 = BIT_CLEAR_RRSR_RSC_8812F(HALMAC_REG_R32(REG_RRSR));
	HALMAC_REG_W32(REG_RRSR, value32);
...
/**
 * init_h2c_8812f() - config h2c packet buffer
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
init_h2c_8812f(struct halmac_adapter *adapter)
{
	u8 value8;
	u32 value32;
	u32 h2cq_addr;
...
/**
 * init_edca_cfg_8812f() - init EDCA config
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
init_edca_cfg_8812f(struct halmac_adapter *adapter)
{
	u8 value8;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
...
/**
 * init_wmac_cfg_8812f() - init wmac config
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
init_wmac_cfg_8812f(struct halmac_adapter *adapter)
{
	u8 value8;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	enum halmac_ret_status status = HALMAC_RET_SUCCESS;
...
/**
 * pre_init_system_cfg_8812f() - pre-init system config
 * @adapter : the adapter of halmac
 * Author : KaiYuan Chang/Ivan Lin
 * Return : enum halmac_ret_status
 * More details of status code can be found in prototype document
 */
enum halmac_ret_status
pre_init_system_cfg_8812f(struct halmac_adapter *adapter)
{
	u32 value32;
	struct halmac_api *api = (struct halmac_api *)adapter->halmac_api;
	u8 enable_bb;

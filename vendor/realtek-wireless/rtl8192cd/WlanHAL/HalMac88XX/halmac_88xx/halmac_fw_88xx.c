		 ...
		 u32 size)
{
	...
	fwBufPtr = (u8 *)PLTFM_MALLOC(adapter->dlfw_pkt_size);
	if (NULL == fwBufPtr) {
		PLTFM_MSG_ERR("[ERR]Allocate buffer for FW download!!\n");
		return HALMAC_RET_FW_NO_MEMORY;
	}
	...
	while (residue_size != 0) {
		...
		PLTFM_MEMCPY(fwBufPtr, fw_bin + mem_offset, pkt_size);

		status = send_fwpkt_88xx(adapter, (u16)(src >> 7),
					 fwBufPtr, pkt_size);
		if (status != HALMAC_RET_SUCCESS) {
			PLTFM_MSG_ERR("[ERR]send fw pkt!!\n");
			goto dlfw_to_mem_88xx_done;
		}
...
dlfw_to_mem_88xx_done:

	PLTFM_FREE(fwBufPtr, adapter->dlfw_pkt_size);

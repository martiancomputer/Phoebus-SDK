...
boolean phydm_rfe_8195b(struct dm_struct *dm, u8 channel)
{
	...
	/* chip top mux - BBP dynamic control*/
	odm_set_bb_reg(dm, R_0x74, BIT(0), 0x1);
	odm_set_bb_reg(dm, R_0x74, BIT(2), 0x1);
...
__iram_odm_func__
void 
phydm_bb_reset_8195b(struct dm_struct *dm)
{
...
__iram_odm_func__
void
config_phydm_write_rf_reg_protect_8195b(struct dm_struct *dm, u8 rf_path, u32 reg_addr,
		    u32 bit_mask, u32 data)
{
	config_phydm_parameter_init_8195b(dm, ODM_PRE_RF_SET);
	config_phydm_write_rf_reg_8195b(dm, rf_path, reg_addr, bit_mask, data);
	config_phydm_parameter_init_8195b(dm, ODM_POST_RF_SET);
	//phydm_bb_reset_8195b(dm);
}
...
__iram_odm_func__
boolean
config_phydm_csimask_BGA_efem_8195b(struct dm_struct *dm,
				    u8 central_ch, enum channel_width bandwidth)
{
	PHYDM_DBG(dm, ODM_PHY_CONFIG, "[%s]===================>\n", __func__);
				  ...
				  u8 central_ch, enum channel_width bandwidth)
{
	...
	else if (dm->package_type == 1 && dm->rfe_type == 2)
		config_phydm_csimask_QFN128_8195b(dm, central_ch, bandwidth);
	else if (dm->package_type == 0 && dm->rfe_type == 0)
		config_phydm_csimask_BGA_8195b(dm, central_ch, bandwidth);
	else if (dm->package_type == 0 && dm->rfe_type == 2)
		config_phydm_csimask_BGA_efem_8195b(dm, central_ch, bandwidth);

	/* FFT Sample Freq Setting */
	r830 = reg830_8195b;
	if (dm->package_type == 0 && dm->rfe_type == 0) {
		ofst = 4;
		switch (bandwidth) {
		case CHANNEL_WIDTH_20: {
			switch (central_ch) {

...
void set_iqk_matrix_8723b(struct dm_struct *dm, u8 OFDM_index, u8 rf_path,
...
void set_cck_filter_coefficient(struct dm_struct *dm, u8 cck_swing_index)
{
	if (*dm->channel != 14) {
				    ...
				    u8 rf_path, u8 channel_mapped_index)
{
	...
	if (method == TXAGC) {
		...
		if (*dm->mp_mode == true) {
			...
			phy_set_tx_power_index_by_rate_section(adapter, rf_path, *dm->channel, CCK);
			phy_set_tx_power_index_by_rate_section(adapter, rf_path, *dm->channel, OFDM);
			phy_set_tx_power_index_by_rate_section(adapter, rf_path, *dm->channel, HT_MCS0_MCS7);
		}
#endif
...
void get_delta_swing_table_8723b(void *dm_void, u8 **temperature_up_a,
		...
		boolean config_path_b)
{
	...
	*	odm_set_rf_reg(dm, RF_PATH_A, RF_0x56, RFREGOFFSETMASK, 0x51000); */
	...
	/* leave IQK mode */
#if 0
	/*	odm_set_bb_reg(dm, REG_FPGA0_IQK, 0xffffff00, 0x00000000);
...
void _phy_path_a_fill_iqk_matrix8723b(struct dm_struct *dm, boolean is_iqk_ok,
...
void _phy_path_b_fill_iqk_matrix8723b(struct dm_struct *dm, boolean is_iqk_ok,
				      s32 result[][8], u8 final_candidate,
...
void odm_set_iqc_by_rfpath(struct dm_struct *dm, u32 rf_path)
{
	...
	if (cali_info->tx_iqc_8723b[PATH_S0][idx_0xc80][VAL] != 0x0 && cali_info->rx_iqc_8723b[PATH_S0][idx_0xc14][VAL] != 0x0 &&
	    cali_info->tx_iqc_8723b[PATH_S1][idx_0xc80][VAL] != 0x0 && cali_info->rx_iqc_8723b[PATH_S1][idx_0xc14][VAL] != 0x0) {
...
void _phy_save_adda_registers8723b(struct dm_struct *dm, u32 *adda_reg,
...
void _phy_save_mac_registers8723b(struct dm_struct *dm, u32 *mac_reg,
				  u32 *mac_backup)
{
	u32 i;
...
void _phy_reload_adda_registers8723b(struct dm_struct *dm, u32 *adda_reg,
...
void _phy_reload_mac_registers8723b(struct dm_struct *dm, u32 *mac_reg,
				    u32 *mac_backup)
{
	u32 i;
...
void _phy_path_adda_on8723b(struct dm_struct *dm, u32 *adda_reg,
...
void _phy_mac_setting_calibration8723b(struct dm_struct *dm, u32 *mac_reg,
				       u32 *mac_backup)
{
	u32 i = 0;
...
void _phy_path_a_stand_by8723b(struct dm_struct *dm)
{
...
void _phy_pi_mode_switch8723b(struct dm_struct *dm, boolean pi_mode)
{
	u32 mode;
...
boolean
phy_simularity_compare_8723b(struct dm_struct *dm, s32 result[][8], u8 c1,
			     u8 c2, boolean is2t)
{
...
void _phy_check_coex_status_8723b(struct dm_struct *dm, boolean beforek)
{
	u8 u1b_tmp;
	u16 count = 0;
	u8 h2c_parameter;
...
void _phy_iq_calibrate_8723b(struct dm_struct *dm, s32 result[][8], u8 t,
			     boolean is2T, u8 rf_path)
{
	void *adapter = dm->adapter;
	u32 i;
...
void _phy_lc_calibrate_8723b(struct dm_struct *dm, boolean is2T)
{
	u8 tmp_reg;
...
void phy_iq_calibrate_8723b(void *dm_void, boolean is_recovery)
{
	...
	if (is_recovery) {
		...
		if (dm->rf_calibrate_info.tx_lok[RF_PATH_A] == 0) {
			RF_DBG(dm, DBG_RF_IQK,
			       "%s => Restore path-A tx_lok result failed\n",
			       __func__);
			is_result = FAIL;
		} else {
...
#if 0
	/*	odm_set_bb_reg(dm, R_0x764, BIT(12), 0x0);
	*	odm_set_bb_reg(dm, R_0x764, BIT(11), 0x1); */
#endif
	...
	for (i = 0; i < 3; i++) {
		...
		if (i == 1) {
			is12simular = phy_simularity_compare_8723b(dm, result, 0, 1, true);
			if (is12simular) {
				final_candidate = 0;
				RF_DBG(dm, DBG_RF_IQK,
		...
		if (i == 2) {
			is13simular = phy_simularity_compare_8723b(dm, result, 0, 2, true);
			if (is13simular) {
				final_candidate = 0;
				RF_DBG(dm, DBG_RF_IQK,
			...
			is23simular = phy_simularity_compare_8723b(dm, result, 1, 2, true);
			if (is23simular) {
				final_candidate = 1;
				RF_DBG(dm, DBG_RF_IQK,
				  ...
				  boolean is_main)
{
	...
	_phy_set_rf_path_switch_8723b(dm, is_main, true);
#else

...
)
{
	...
	if (*(dm->mp_mode)) {
		...
		value32 = odm_get_bb_reg(dm, REG_S0_S1_PATH_SWITCH, MASKLWORD);
		...
		if (((*dm->is_1_antenna == true) && (*dm->rf_default_path == 1)) || (dm->support_interface == ODM_ITRF_USB)
		    || ((dm->support_ability & ODM_BB_ANT_DIV) && (fat_tab->rx_idle_ant  == AUX_ANT)))
			rf_path = RF_PATH_B;
	}
	#endif
	...
	cali_info->tx_iqc_8723b[PATH_S1][idx_0xc80][VAL] = value32;

	value32 = ((ele_C & 0x000003C0) >> 6) << 28;
	cali_info->tx_iqc_8723b[PATH_S1][idx_0xc94][VAL] =
		(cali_info->tx_iqc_8723b[PATH_S1][idx_0xc94][VAL] & (~(MASKH4BITS))) | value32;

	value32 = ((((iqk_result_s1_x * ele_D) >> 7) & 0x01) << 28) | (ele_A_ext << 31) | (ele_C_ext << 29) ;
	cali_info->tx_iqc_8723b[PATH_S1][idx_0xc4c][VAL] =
		(cali_info->tx_iqc_8723b[PATH_S1][idx_0xc4c][VAL] & (~(BIT(31) | BIT(29) | BIT(28)))) | value32;


	/* S0------------------------------------- */
	if (*(dm->mp_mode) || (dm->support_ability & ODM_BB_ANT_DIV) || ((*dm->is_1_antenna == true) && (*dm->rf_default_path == 1))
	    || (dm->support_interface == ODM_ITRF_USB) || (*dm->is_1_antenna == false)) {
		ele_A = 0;
		ele_C = 0;
		...
		value32 = (ele_D << 22) | ((ele_C & 0x3F) << 16) | ele_A;
		cali_info->tx_iqc_8723b[PATH_S0][idx_0xc80][VAL] = value32;

		value32 = ((ele_C & 0x000003C0) >> 6) << 28;
		cali_info->tx_iqc_8723b[PATH_S0][idx_0xc94][VAL] =
			(cali_info->tx_iqc_8723b[PATH_S0][idx_0xc94][VAL] & (~(MASKH4BITS))) | value32;

		value32 = ((((iqk_result_s0_x * ele_D) >> 7) & 0x01) << 28) | (ele_A_ext << 31) | (ele_C_ext << 29) ;
		cali_info->tx_iqc_8723b[PATH_S0][idx_0xc4c][VAL] =
			(cali_info->tx_iqc_8723b[PATH_S0][idx_0xc4c][VAL] & (~(BIT(31) | BIT(29) | BIT(28)))) | value32;
	}
	...
	if ((((iqk_result_s1_x != 0) && (rf_path == RF_PATH_A)) ||
		...
		case RF_PATH_A:
			...
			/* for 8723B S1 */
			odm_set_bb_reg(dm, REG_OFDM_0_XA_TX_IQ_IMBALANCE, MASKDWORD, cali_info->tx_iqc_8723b[PATH_S1][idx_0xc80][VAL]);
			odm_set_bb_reg(dm, REG_OFDM_0_XC_TX_AFE, MASKH4BITS, ((cali_info->tx_iqc_8723b[PATH_S1][idx_0xc94][VAL]) >> 28));

			value32 = odm_get_bb_reg(dm, REG_OFDM_0_ECCA_THRESHOLD, MASKDWORD);
			value32 = (value32 & (~(BIT(31) | BIT(29) | BIT(28)))) | (cali_info->tx_iqc_8723b[PATH_S1][idx_0xc4c][VAL] & (BIT(31) | BIT(29) | BIT(28)));
			odm_set_bb_reg(dm, REG_OFDM_0_ECCA_THRESHOLD, MASKDWORD, value32);
			break;
		case RF_PATH_B:
			/* wirte new elements A, C, D to regC80 and regC94, element B is always 0 */
			/* for 8723B S0 */
			odm_set_bb_reg(dm, REG_OFDM_0_XA_TX_IQ_IMBALANCE, MASKDWORD, cali_info->tx_iqc_8723b[PATH_S0][idx_0xc80][VAL]);
			odm_set_bb_reg(dm, REG_OFDM_0_XC_TX_AFE, MASKH4BITS, ((cali_info->tx_iqc_8723b[PATH_S0][idx_0xc94][VAL]) >> 28));

			value32 = odm_get_bb_reg(dm, REG_OFDM_0_ECCA_THRESHOLD, MASKDWORD);
			value32 = (value32 & (~(BIT(31) | BIT(29) | BIT(28)))) | (cali_info->tx_iqc_8723b[PATH_S0][idx_0xc4c][VAL] & (BIT(31) | BIT(29) | BIT(28)));
			odm_set_bb_reg(dm, REG_OFDM_0_ECCA_THRESHOLD, MASKDWORD, value32);
			break;
		default:
			break;
...
)
{
	...
	if (method == TXAGC) {
		...
		set_iqk_matrix_8723b(dm, final_ofdm_swing_index,
		...
		if (final_ofdm_swing_index > pwr_tracking_limit_ofdm) {   /* BBSwing higher then Limit */
			...
			set_iqk_matrix_8723b(dm, pwr_tracking_limit_ofdm,
				cali_info->iqk_matrix_reg_setting[channel_mapped_index].value[0][0],
				cali_info->iqk_matrix_reg_setting[channel_mapped_index].value[0][1],
				cali_info->iqk_matrix_reg_setting[channel_mapped_index].value[0][4],
				cali_info->iqk_matrix_reg_setting[channel_mapped_index].value[0][5]);
...
)
{
	...
	RF_DBG(dm, DBG_RF_IQK, "[IQK]S1 TX IQK!\n");
	/* save RF path */
	...
	odm_set_bb_reg(dm, R_0x764, BIT(12) | BIT(11), 0x1);
	/*	RF_DBG(dm,DBG_RF_IQK, "[IQK]0x764 when IQK= 0x%x\n", odm_get_bb_reg(dm, R_0x764, MASKDWORD)); */
	...
	odm_set_bb_reg(dm, R_0x764, BIT(12) | BIT(11), 0x3);
	/*	RF_DBG(dm,DBG_RF_IQK, "[IQK]0x764 after IQK= 0x%x\n", odm_get_bb_reg(dm, R_0x764, MASKDWORD)); */
	...
	RF_DBG(dm, DBG_RF_IQK, "[IQK]0xe90(before IQK)= 0x%x, 0xe98(afer IQK) = 0x%x\n",
...
)
{
	...
	RF_DBG(dm, DBG_RF_IQK, "[IQK]S1 RX IQK:Get TXIMR setting\n");
	/* 1 Get TXIMR setting */
	...
	*	odm_set_rf_reg(dm, RF_PATH_A, RF_TXPA_G2, RFREGOFFSETMASK, 0xf7fb7); */
	odm_set_rf_reg(dm, RF_PATH_A, RF_TXPA_G2, RFREGOFFSETMASK, 0xf7df7);
	/* S0 IQK PA off */
	odm_set_rf_reg(dm, RF_PATH_A, RF_0xed, 0x20, 0x1);
	/*	odm_set_rf_reg(dm, RF_PATH_A, RF_0x43, RFREGOFFSETMASK, 0x60fed); */
	odm_set_rf_reg(dm, RF_PATH_A, RF_0x43, RFREGOFFSETMASK, 0x60efd);
	...
	/* 1 RX IQK */
	RF_DBG(dm, DBG_RF_IQK, "[IQK]S1 RX IQK\n");
	...
	odm_set_bb_reg(dm, REG_RX_IQK_PI_A, MASKDWORD, 0x2816081f);
	...
	odm_set_rf_reg(dm, RF_PATH_A, RF_0x55, RFREGOFFSETMASK, 0x40207);
	...
	/* monitor image power before & after IQK */
	RF_DBG(dm, DBG_RF_IQK, "[IQK]0xea0(before IQK)= 0x%x, 0xea8(afer IQK) = 0x%x\n",
...
)
{
	...
	RF_DBG(dm, DBG_RF_IQK, "[IQK]S0 TX IQK!\n");
...
)
{
	...
	/* 1 Get TXIMR setting */
	RF_DBG(dm, DBG_RF_IQK, "[IQK]S0 RX IQK:Get TXIMR setting!\n");
	...
	*	odm_set_rf_reg(dm, RF_PATH_A, RF_TXPA_G2, RFREGOFFSETMASK, 0xf7fb7 ); */
	...
	/* 1 RX IQK */
	RF_DBG(dm, DBG_RF_IQK, "[IQK]S0 RX IQK\n");
	...
	/* PA, PAD setting */
	odm_set_rf_reg(dm, RF_PATH_A, RF_0xdd, RFREGOFFSETMASK, 0x5c);
	odm_set_rf_reg(dm, RF_PATH_A, RF_0x75, RFREGOFFSETMASK, 0xe1007);
	...
	odm_set_bb_reg(dm, REG_FPGA0_IQK, 0xffffff00, 0x000000);
	/*	PA/PAD controlled by 0x0 */
	odm_set_rf_reg(dm, RF_PATH_A, RF_0xdd, RFREGOFFSETMASK, 0x4c);
	...
	if (!(reg_eac & BIT(27)) &&		/* if Tx is OK, check whether Rx is OK */
		...
		RF_DBG(dm, DBG_RF_IQK, "[IQK]S0 Rx IQK fail!!\n");
...
)
{
	...
	if (final_candidate == 0xFF)
		...
		if ((X & 0x00000200) != 0)
			X = X | 0xFFFFFC00;
		TX0_A = (X * oldval_0) >> 8;
		RF_DBG(dm, DBG_RF_IQK, "[IQK]X = 0x%x, TX0_A = 0x%x, oldval_0 0x%x\n", X, TX0_A, oldval_0);
		...
		cali_info->rx_iqc_8723b[PATH_S1][idx_0xc14][KEY] = REG_OFDM_0_XA_RX_IQ_IMBALANCE;
		cali_info->rx_iqc_8723b[PATH_S1][idx_0xc14][VAL] = tmp0xc14;

		reg = (result[final_candidate][3] >> 6) & 0xF;
		tmp0xca0 = odm_get_bb_reg(dm, REG_OFDM_0_RX_IQ_EXT_ANTA, 0x0fffffff) | (reg << 28);

		cali_info->rx_iqc_8723b[PATH_S1][idx_0xca0][KEY] = REG_OFDM_0_RX_IQ_EXT_ANTA;
		cali_info->rx_iqc_8723b[PATH_S1][idx_0xca0][VAL] = tmp0xca0;
	}
}
...
)
{
	...
	if (final_candidate == 0xFF)
		...
		cali_info->rx_iqc_8723b[PATH_S0][idx_0xc14][KEY] = REG_OFDM_0_XA_RX_IQ_IMBALANCE;
		cali_info->rx_iqc_8723b[PATH_S0][idx_0xc14][VAL] = tmp0xc14;

		reg = (result[final_candidate][7] >> 6) & 0xF;
		tmp0xca0 = odm_get_bb_reg(dm, REG_OFDM_0_RX_IQ_EXT_ANTA, 0x0fffffff) | (reg << 28);

		cali_info->rx_iqc_8723b[PATH_S0][idx_0xca0][KEY] = REG_OFDM_0_RX_IQ_EXT_ANTA;
		cali_info->rx_iqc_8723b[PATH_S0][idx_0xca0][VAL] = tmp0xca0;
	}
}
...
)
{
	...
	if (rf_path) { /* S1: rf_path = 0, S0:rf_path = 1 */
		if ((cali_info->tx_iqc_8723b[PATH_S0][idx_0xc80][VAL] != 0x0) && (cali_info->rx_iqc_8723b[PATH_S0][idx_0xc14][VAL] != 0x0)) {

			RF_DBG(dm, DBG_RF_IQK, "[IQK]RF S0 IQC!!!\n");
			RF_DBG(dm, DBG_RF_IQK, "[IQK]0xc80 = 0x%x!!!\n", cali_info->tx_iqc_8723b[PATH_S0][idx_0xc80][VAL]);

			/* S0 TX IQC */
			odm_set_bb_reg(dm, cali_info->tx_iqc_8723b[PATH_S0][idx_0xc94][KEY], MASKH4BITS, (cali_info->tx_iqc_8723b[PATH_S0][idx_0xc94][VAL] >> 28));
			odm_set_bb_reg(dm, cali_info->tx_iqc_8723b[PATH_S0][idx_0xc80][KEY], MASKDWORD, cali_info->tx_iqc_8723b[PATH_S0][idx_0xc80][VAL]);
			odm_set_bb_reg(dm, cali_info->tx_iqc_8723b[PATH_S0][idx_0xc4c][KEY], BIT(31), (cali_info->tx_iqc_8723b[PATH_S0][idx_0xc4c][VAL] >> 31));
			odm_set_bb_reg(dm, cali_info->tx_iqc_8723b[PATH_S0][idx_0xc4c][KEY], BIT(29), ((cali_info->tx_iqc_8723b[PATH_S0][idx_0xc4c][VAL] & BIT(29)) >> 29));
			...
			RF_DBG(dm, DBG_RF_IQK, "[IQK]IQC S0 vaule invalid!!!\n");
			return false;
		}
	} else {
		if ((cali_info->tx_iqc_8723b[PATH_S1][idx_0xc80][VAL] != 0x0) && (cali_info->rx_iqc_8723b[PATH_S1][idx_0xc14][VAL] != 0x0)) {
			RF_DBG(dm, DBG_RF_IQK, "[IQK]RF S1 IQC!!!\n");
			RF_DBG(dm, DBG_RF_IQK, "[IQK]0xc80 = 0x%x!!!\n", cali_info->tx_iqc_8723b[PATH_S1][idx_0xc80][VAL]);

			/* S1 TX IQC */
			odm_set_bb_reg(dm, cali_info->tx_iqc_8723b[PATH_S1][idx_0xc94][KEY], MASKH4BITS, (cali_info->tx_iqc_8723b[PATH_S1][idx_0xc94][VAL] >> 28));
			odm_set_bb_reg(dm, cali_info->tx_iqc_8723b[PATH_S1][idx_0xc80][KEY], MASKDWORD, cali_info->tx_iqc_8723b[PATH_S1][idx_0xc80][VAL]);
			odm_set_bb_reg(dm, cali_info->tx_iqc_8723b[PATH_S1][idx_0xc4c][KEY], BIT(31), (cali_info->tx_iqc_8723b[PATH_S1][idx_0xc4c][VAL] >> 31));
			odm_set_bb_reg(dm, cali_info->tx_iqc_8723b[PATH_S1][idx_0xc4c][KEY], BIT(29), ((cali_info->tx_iqc_8723b[PATH_S1][idx_0xc4c][VAL] & BIT(29)) >> 29));
			...
			RF_DBG(dm, DBG_RF_IQK, "IQC S1 vaule invalid!!!\n");
			return false;
		}
	}
}
...
)
{
	...
	if (t == 0) {
		...
		RF_DBG(dm, DBG_RF_IQK, "[IQK]IQ Calibration for %s for %d times\n", (*dm->is_1_antenna ? "1ant" : "2ant"), t);
		if (*dm->is_1_antenna)
			RF_DBG(dm, DBG_RF_IQK, "[IQK]IQ Calibration for %s\n", (*dm->rf_default_path ? "S0" : "S1"));

		/* Save ADDA parameters, turn path A ADDA on */
		_phy_save_adda_registers8723b(dm, ADDA_REG, cali_info->ADDA_backup, IQK_ADDA_REG_NUM);
		_phy_save_mac_registers8723b(dm, IQK_MAC_REG, cali_info->IQK_MAC_backup);
		_phy_save_adda_registers8723b(dm, IQK_BB_REG_92C, cali_info->IQK_BB_backup, IQK_BB_REG_NUM);
	}
	RF_DBG(dm, DBG_RF_IQK, "[IQK]IQ Calibration for %s for %d times\n", (*dm->is_1_antenna ? "1ant" : "2ant"), t);
	if (*dm->is_1_antenna)
		RF_DBG(dm, DBG_RF_IQK, "[IQK]IQ Calibration for %s\n", (*dm->rf_default_path ? "S0" : "S1"));

	_phy_path_adda_on8723b(dm, ADDA_REG, true);
...
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
		_phy_pi_mode_switch8723b(adapter, true);
#else
		_phy_pi_mode_switch8723b(dm, true);
#endif
	}
#endif
	...
	path_sel_bb = odm_get_bb_reg(dm, R_0x948, MASKDWORD);
	/* save 0x764 GNT_BT */
	tmp0x764 = odm_get_bb_reg(dm, R_0x764, MASKDWORD);

	/*	path_sel_rf = odm_get_rf_reg(dm, RF_PATH_A, RF_0xb0, 0xfffff); */

	/* MAC settings */
	_phy_mac_setting_calibration8723b(dm, IQK_MAC_REG, cali_info->IQK_MAC_backup);
	/* BB setting */
	...
	/*	odm_set_bb_reg(dm, REG_FPGA0_XAB_RF_INTERFACE_SW, BIT10, 0x01);
...
#if 0
	/* LOK RF setting */
	odm_set_rf_reg(dm, RF_PATH_A, RF_0xed, 0x2, 0x1);
	odm_set_rf_reg(dm, RF_PATH_A, RF_0xef, 0x2, 0x1);
	odm_set_rf_reg(dm, RF_PATH_A, RF_0x56, RFREGOFFSETMASK, 0x00032);
	odm_set_rf_reg(dm, RF_PATH_A, RF_0x76, RFREGOFFSETMASK, 0x00032);
#endif
...
#if MP_DRIVER != 1
	if ((*dm->is_1_antenna == false) || ((*dm->is_1_antenna == true) && (dm->support_interface != ODM_ITRF_USB)
			&& (*dm->rf_default_path == 0)))
#endif
	{

		for (i = 0 ; i < retry_count ; i++) {
			path_aok = phy_path_a_iqk_8723b(dm);
			/*		if(path_aok == 0x03){ */
			if (path_aok == 0x01) {
				RF_DBG(dm, DBG_RF_IQK, "[IQK]S1 Tx IQK Success!!\n");
		...
		for (i = 0 ; i < retry_count ; i++) {
			path_aok = phy_path_a_rx_iqk_8723b(dm);
			if (path_aok == 0x03) {
				RF_DBG(dm, DBG_RF_IQK, "[IQK]S1 Rx IQK Success!!\n");
				/*				result[t][0] = (odm_get_bb_reg(dm, REG_TX_POWER_BEFORE_IQK_A, MASKDWORD)&0x3FF0000)>>16;
				 *				result[t][1] = (odm_get_bb_reg(dm, REG_TX_POWER_AFTER_IQK_A, MASKDWORD)&0x3FF0000)>>16; */
				...
				RF_DBG(dm, DBG_RF_IQK, "[IQK]S1Rx IQK Fail!!\n");
		}

		if (0x00 == path_aok)
			RF_DBG(dm, DBG_RF_IQK, "[IQK]S1 IQK failed!!\n");
	}
#endif
...
#if 0
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
	/*		_phy_path_a_stand_by8723b(adapter); */

	/*  Turn path B ADDA on */
	_phy_path_adda_on8723b(adapter, ADDA_REG, false, is2T);
#else
	/*		_phy_path_a_stand_by8723b(dm); */

	/*  Turn path B ADDA on */
	_phy_path_adda_on8723b(dm, ADDA_REG, false, is2T);
#endif
#endif
...
#endif
	{
		...
		for (i = 0 ; i < retry_count ; i++) {
			...
			/*		if(path_bok == 0x03){ */
			if (path_bok == 0x01) {
				RF_DBG(dm, DBG_RF_IQK, "[IQK]S0 Tx IQK Success!!\n");
		...
		for (i = 0 ; i < retry_count ; i++) {
			path_bok = phy_path_b_rx_iqk_8723b(dm);
			if (path_bok == 0x03) {
				RF_DBG(dm, DBG_RF_IQK, "[IQK]S0 Rx IQK Success!!\n");
				/*				result[t][0] = (odm_get_bb_reg(dm, REG_TX_POWER_BEFORE_IQK_A, MASKDWORD)&0x3FF0000)>>16;
				 *				result[t][1] = (odm_get_bb_reg(dm, REG_TX_POWER_AFTER_IQK_A, MASKDWORD)&0x3FF0000)>>16; */
	...
	RF_DBG(dm, DBG_RF_IQK, "[IQK]IQK:Back to BB mode, load original value!\n");
	odm_set_bb_reg(dm, REG_FPGA0_IQK, 0xffffff00, 0x000000);

	if (t != 0) {
		if (!cali_info->is_rf_pi_enable) {
			/* Switch back BB to SI mode after finish IQ Calibration. */
			/*			_phy_pi_mode_switch8723b(dm, false); */
		}
		/* Reload ADDA power saving parameters */
		_phy_reload_adda_registers8723b(dm, ADDA_REG, cali_info->ADDA_backup, IQK_ADDA_REG_NUM);
		/* Reload MAC parameters */
		_phy_reload_mac_registers8723b(dm, IQK_MAC_REG, cali_info->IQK_MAC_backup);
		_phy_reload_adda_registers8723b(dm, IQK_BB_REG_92C, cali_info->IQK_BB_backup, IQK_BB_REG_NUM);
		/* Reload RF path */
		odm_set_bb_reg(dm, R_0x948, MASKDWORD, path_sel_bb);
		/* reload 0x764 GNT_BT */
		odm_set_bb_reg(dm, R_0x764, MASKDWORD, tmp0x764);
	...
	RF_DBG(dm, DBG_RF_IQK, "[IQK]_phy_iq_calibrate_8723b() <==\n");
}
...
)
{
	...
	if (is_recovery && (!dm->is_in_hct_test)) {/* YJ,add for PowerTest,120405 */
		RF_DBG(dm, DBG_RF_INIT, "[IQK]phy_iq_calibrate_8723b: Return due to is_recovery!\n");
		_phy_reload_adda_registers8723b(dm, IQK_BB_REG_92C, cali_info->IQK_BB_backup_recover, 9);
		return;
	}
	...
	RF_DBG(dm, DBG_RF_IQK, "[IQK]S1 0xc80 = 0x%x, S0 0xc80 = 0x%x\n", cali_info->tx_iqc_8723b[PATH_S1][idx_0xc80][VAL], cali_info->tx_iqc_8723b[PATH_S0][idx_0xc80][VAL]);

	if ((*dm->is_1_antenna == true) && (*dm->rf_default_path == 0)) {	/* 1-ant, S1 */
		if (odm_set_iqc_by_rfpath(dm, 0)) {
			RF_DBG(dm, DBG_RF_IQK, "[IQK]1-ant S1 IQK value is reloaded!!!\n");
			is_reload_iqk = true;
		}
	} else if ((*dm->is_1_antenna == true) && (*dm->rf_default_path == 1)) {	/* 1-ant, S0 */
		if (odm_set_iqc_by_rfpath(dm, 1)) { /* S0 */
			RF_DBG(dm, DBG_RF_IQK, "[IQK]1-ant S0 IQK value is reloaded!!!\n");
			is_reload_iqk = true;
		}
	} else {	/* 2-ant */
		if ((cali_info->tx_iqc_8723b[PATH_S1][idx_0xc80][VAL] != 0x0) && (cali_info->rx_iqc_8723b[PATH_S1][idx_0xc14][VAL] != 0x0)) {
			RF_DBG(dm, DBG_RF_IQK, "[IQK]IQK has been done before.!!!\n");

			if ((path_sel_bb == 0x0) || (path_sel_bb == 0x200)) {	/* S1 */
				if (odm_set_iqc_by_rfpath(dm, 0)) {
					RF_DBG(dm, DBG_RF_IQK, "[IQK]2-ant S1 IQK value is reloaded!!!\n");
					is_reload_iqk = true;
				}
			} else {	/* S0 */
				if (odm_set_iqc_by_rfpath(dm, 1)) {
					RF_DBG(dm, DBG_RF_IQK, "[IQK]2-ant S0 IQK value is reloaded!!!\n");
					is_reload_iqk = true;
				}
			}
		}
	...
	RF_DBG(dm, DBG_RF_IQK, "[IQK]0x948 = 0x%x\n", path_sel_bb);
	/* IQK start!!!!!!!!!! */
	...
	for (i = 0; i < 3; i++) {
		_phy_iq_calibrate_8723b(dm, result, i);

		if (i == 1) {
			is12simular = phy_simularity_compare_8723b(dm, result, 0, 1);
			if (is12simular) {
				final_candidate = 0;
				RF_DBG(dm, DBG_RF_IQK, "[IQK]IQK: is12simular final_candidate is %x\n", final_candidate);
				break;
			}
		}
		...
		if (i == 2) {
			...
			if (is13simular) {
				final_candidate = 0;
				RF_DBG(dm, DBG_RF_IQK, "[IQK]IQK: is13simular final_candidate is %x\n", final_candidate);
			...
			if (is23simular) {
				final_candidate = 1;
				RF_DBG(dm, DBG_RF_IQK, "[IQK]IQK: is23simular final_candidate is %x\n", final_candidate);
			} else {
				for (i = 0; i < 8; i++)
					reg_tmp += result[3][i];
	...
	for (i = 0; i < 4; i++) {
		...
		regec4 = result[i][6];
		regecc = result[i][7];
		RF_DBG(dm, DBG_RF_IQK, "[IQK]IQK: rege94=%x rege9c=%x regea4=%x regeac=%x regeb4=%x regebc=%x regec4=%x regecc=%x\n ", rege94, rege9c, regea4, regeac, regeb4, regebc, regec4, regecc);
	}
	...
	_phy_save_adda_registers8723b(dm, IQK_BB_REG_92C, cali_info->IQK_BB_backup_recover, IQK_BB_REG_NUM);

	/* Restore RF path */
	odm_set_bb_reg(dm, R_0x948, MASKDWORD, path_sel_bb);
	RF_DBG(dm, DBG_RF_IQK, "[IQK]is1ant = %d, supprotinterface = %d, RFdefaultpath=%d.\n", *dm->is_1_antenna, dm->support_interface, *dm->rf_default_path);
 ...
 *	u32			AP_curve[PATH_NUM][APK_CURVE_REG_NUM]; */
	...
	RF_DBG(dm, DBG_RF_IQK, "==>_phy_ap_calibrate_8188e() delta %d\n", delta);
...
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
	_phy_save_mac_registers8723b(adapter, MAC_REG, MAC_backup);

	/* save AFE default value */
	_phy_save_adda_registers8723b(adapter, AFE_REG, AFE_backup, IQK_ADDA_REG_NUM);
#else
	_phy_save_mac_registers8723b(dm, MAC_REG, MAC_backup);

	/* save AFE default value */
	_phy_save_adda_registers8723b(dm, AFE_REG, AFE_backup, IQK_ADDA_REG_NUM);
#endif
	...
	for (path = 0; path < pathbound; path++) {
		...
		if (path == RF_PATH_A) {
			...
			for (index = 0; index < 11; index++) {
				...
				RF_DBG(dm, DBG_RF_IQK, "_phy_ap_calibrate_8188e() offset 0x%x value 0x%x\n", offset, odm_get_bb_reg(dm, offset, MASKDWORD));
		...
		RF_DBG(dm, DBG_RF_IQK, "_phy_ap_calibrate_8188e() offset 0xe70 %x\n", odm_get_bb_reg(dm, REG_RX_WAIT_CCA, MASKDWORD));
		...
		RF_DBG(dm, DBG_RF_IQK, "_phy_ap_calibrate_8188e() offset 0x800 %x\n", odm_get_bb_reg(dm, R_0x800, MASKDWORD));

		/* MAC settings */
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
		_phy_mac_setting_calibration8723b(adapter, MAC_REG, MAC_backup);
#else
		_phy_mac_setting_calibration8723b(dm, MAC_REG, MAC_backup);
#endif
		...
		for (index = 0; index < APK_BB_REG_NUM; index++) {
...
#if 1
			if (!cali_info->is_apk_thermal_meter_ignore) {
				...
				RF_DBG(dm, DBG_RF_IQK, "_phy_ap_calibrate_8188e() APK index %d tmp_reg 0x%x delta_V %d delta_offset %d\n", index, tmp_reg, delta_V, delta_offset);
			...
			do {
				odm_set_bb_reg(dm, REG_FPGA0_IQK, 0xffffff00, 0x800000);
				{
					...
					RF_DBG(dm, DBG_RF_IQK, "_phy_ap_calibrate_8188e() offset 0x%x value 0x%x\n", APK_offset[path], odm_get_bb_reg(dm, APK_offset[path], MASKDWORD));
					ODM_delay_ms(3);
				...
				RF_DBG(dm, DBG_RF_IQK, "_phy_ap_calibrate_8188e() offset 0xbd8[25:21] %x\n", tmp_reg);
...
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
	_phy_reload_mac_registers8723b(adapter, MAC_REG, MAC_backup);
#else
	_phy_reload_mac_registers8723b(dm, MAC_REG, MAC_backup);
#endif
...
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
	_phy_reload_adda_registers8723b(adapter, AFE_REG, AFE_backup, IQK_ADDA_REG_NUM);
#else
	_phy_reload_adda_registers8723b(dm, AFE_REG, AFE_backup, IQK_ADDA_REG_NUM);
#endif
	...
	for (path = 0; path < pathbound; path++) {
		...
		if (APK_result[path][1] > 6)
			APK_result[path][1] = 6;
		RF_DBG(dm, DBG_RF_IQK, "apk path %d result %d 0x%x \t", path, 1, APK_result[path][1]);
	}
	...
	cali_info->is_ap_kdone = true;

	RF_DBG(dm, DBG_RF_IQK, "<==_phy_ap_calibrate_8188e()\n");
}
...
)
{
	...
	if (cali_info->is_ap_kdone)
#endif
		return;

#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
	if (IS_92C_SERIAL(hal_data->VersionID))
		_phy_ap_calibrate_8723b(adapter, delta, true);
	else
#endif
	{
		/* For 88C 1T1R */
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
		_phy_ap_calibrate_8723b(adapter, delta, false);
#else
		_phy_ap_calibrate_8723b(dm, delta, false);
#endif
	}
}
...
)
{
	...
	RF_DBG(dm, DBG_RF_IQK, "==>_phy_digital_predistortion8723b()\n");

	RF_DBG(dm, DBG_RF_IQK, "_phy_digital_predistortion8723b for %s\n", (is2T ? "2T2R" : "1T1R"));
	...
	/* save MAC default value */
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
	_phy_save_mac_registers8723b(adapter, BB_REG, MAC_backup);
#else
	_phy_save_mac_registers8723b(dm, BB_REG, MAC_backup);
#endif
...
#else
			RF_backup[path][index] = odm_get_rf_reg(adapter, path, RF_REG[index], MASKDWORD);
#endif
	}
	...
	while (retry_count < DP_RETRY_LIMIT && !cali_info->is_dp_path_aok) {
		...
		if (tmp_reg < 0xf0 || tmp_reg > 0x105 || tmp_reg2 > 0xff) {
			...
			ODM_delay_ms(50);
			retry_count++;
			RF_DBG(dm, DBG_RF_IQK, "path A DPK retry_count %d 0xbe0[31:16] %x 0xbe8[31:16] %x\n", retry_count, tmp_reg, tmp_reg2);
		} else {
			RF_DBG(dm, DBG_RF_IQK, "path A DPK Sucess\n");
			cali_info->is_dp_path_aok = true;
			break;
		}
	}
	retry_count = 0;
	...
	if (cali_info->is_dp_path_aok) {
		...
		for (i = REG_PDP_ANT_A; i <= 0xb3c; i += 4) {
			...
			RF_DBG(dm, DBG_RF_IQK, "path A ofsset = 0x%x\n", i);
		}
	...
	if (is2T) {
		...
		while (retry_count < DP_RETRY_LIMIT && !cali_info->is_dp_path_bok) {
			/* ----read back measurement results---- */
			...
			if (tmp_reg < 0xf0 || tmp_reg > 0x105 || tmp_reg2 > 0xff) {
				...
				ODM_delay_ms(50);
				retry_count++;
				RF_DBG(dm, DBG_RF_IQK, "path B DPK retry_count %d 0xbf0[31:16] %x, 0xbf8[31:16] %x\n", retry_count, tmp_reg, tmp_reg2);
			} else {
				RF_DBG(dm, DBG_RF_IQK, "path B DPK Success\n");
				cali_info->is_dp_path_bok = true;
				break;
			}
		}

		/* DPP path B */
		if (cali_info->is_dp_path_bok) {
			/* DP setting */
			/* LUT by SRAM */
	...
	odm_set_rf_reg(dm, RF_PATH_A, RF_MODE1, MASKDWORD, 0x1000f);	/* standby mode */
	odm_set_rf_reg(dm, RF_PATH_A, RF_MODE2, MASKDWORD, 0x20101);		/* RF lpbk switches off */
...
)
{
	...
	if (cali_info->is_dp_done)
		return;
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)

	if (IS_92C_SERIAL(hal_data->VersionID))
		_phy_digital_predistortion8723b(adapter, true);
	else
#endif
	{
		/* For 88C 1T1R */
		_phy_digital_predistortion8723b(adapter, false);
	}
}
#endif
...
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
	return _phy_query_rf_path_switch_8723b(adapter, false);
#else

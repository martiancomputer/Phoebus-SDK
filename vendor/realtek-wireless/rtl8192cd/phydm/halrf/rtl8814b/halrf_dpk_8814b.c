...
void _dpk_init_8814b(struct dm_struct *dm, boolean start_dpk)
{
	...
	if (start_dpk) {
		...
		odm_write_4byte(dm, 0x1b60, 0x1F0e0000);
	...
	u8 gs_type)
{
	...
	if (gs_type == GSK4) {
		odm_write_4byte(dm, 0x1bd4, 0x00060001);
		odm_write_4byte(dm, 0x1b4c, 0x00090000);
	} else if (gs_type == GSK3) {
		odm_write_4byte(dm, 0x1bd4, 0x00060001);
...
#if 1
		RF_DBG(dm, DBG_RF_DPK, "[DPK][%x] (1) 1bfc=0x%x\n", gs_type, odm_get_bb_reg(dm, R_0x1bfc, MASKDWORD));
		RF_DBG(dm, DBG_RF_DPK, "[DPK][%x] (1) i=0x%x, q=0x%x, i^2+q^2=0x%x\n",
		       gs_type, i_val, q_val, i_val*i_val + q_val*q_val);
		RF_DBG(dm, DBG_RF_DPK, "[DPK][%x] (1) i=%d, q=%d, i^2+q^2=%d\n",
		       gs_type, i_val, q_val, i_val*i_val + q_val*q_val);
	...
	if (q_val >> 15 != 0)
		q_val = 0x10000 - q_val;
	RF_DBG(dm, DBG_RF_DPK, "[DPK][%x] (2) 1bfc=0x%x\n", gs_type, odm_get_bb_reg(dm, R_0x1bfc, MASKDWORD));
	
	odm_write_4byte(dm, 0x1b4c, 0x0);
#if 1
	RF_DBG(dm, DBG_RF_DPK, "[DPK][%x] (2) i=0x%x, q=0x%x, i^2+q^2=0x%x\n",
	       gs_type, i_val, q_val, i_val*i_val + q_val*q_val);
	...
	u8 gs_type)
{
	...
	gain = _dpk_pas_get_iq_8814b(dm, gs_type) >> 13; //offset 90db
	...
	u8 gs_type)
{
	...
	if (gs_type == GSK3) {
		...
		//RF_DBG(dm, DBG_RF_DPK, "[DPK][GSK3] old rxbb = 0x%x, gain_db =%s\n", tmp_rxbb, gain_db);		
		RF_DBG(dm, DBG_RF_DPK, "[DPK][GSK3] old rxbb = 0x%x, gain_db =%d\n", tmp_rxbb, gain_db);
		...
		if (gain_db > (-90-300)) { //> -0dB
			tmp_rxbb = tmp_rxbb - 2;
			fail = true;
			//RF_DBG(dm, DBG_RF_DPK, "[DPK][GSK3] Case 4\n");
#if 1
		} else if (gain_db < (-150-300)){ // <-6dB
			tmp_rxbb = tmp_rxbb + 2;
			fail = true;
		...
		RF_DBG(dm, DBG_RF_DPK, "[DPK][GSK3] new rxbb = 0x%x\n", tmp_rxbb);
	}
...
void _dpk_rfsetting_8814b(struct dm_struct *dm, u8 path)
{
	...
	odm_write_4byte(dm, 0x1b00, IQK_CMD_8814B | path << 1);
	odm_set_bb_reg(dm, 0x1bb8, BIT(20), 0x0);
	odm_set_bb_reg(dm, 0x1b60, 0x00ff0000, 0xe);
	//ODM_delay_ms(1);
	...
	if (*dm->band_type == ODM_BAND_2_4G) {
		...
		odm_set_rf_reg(dm, (enum rf_path)path, 0x88, 0x300, 0x1);
		odm_set_rf_reg(dm, (enum rf_path)path, 0x88, 0xc00, 0x0);		
		odm_set_rf_reg(dm, (enum rf_path)path, 0x8e, BIT(5), 0x0);
		odm_set_rf_reg(dm, (enum rf_path)path, 0x8e, 0x0000f, 0xb);
	} else {
		if (path == 0x0) {
...
#if 1
	RF_DBG(dm, DBG_RF_DPK, "[DPK][RF] new gain setting\n");	
	...
	RF_DBG(dm, DBG_RF_DPK, "[DPK][RF] RF_0x88 = %x\n",odm_get_rf_reg(dm, path, 0x88, 0xfffff));	
	RF_DBG(dm, DBG_RF_DPK, "[DPK][RF] RF_0x8e = %x\n",odm_get_rf_reg(dm, path, 0x8e, 0xfffff));	
		       ...
		       u8 group)
{
	...
	rxbb = odm_get_rf_reg(dm, (enum rf_path)path, RF_0x00, 0x003e0);
	tmp = (u8)(tx_agc_ori & 0x1f);
	tx_agc =  tmp - tx_agc_search;
	rxbb = rxbb + tx_agc_search / 2;
	odm_set_rf_reg(dm, (enum rf_path)path, RF_0x00, 0x003e0, rxbb);
	RF_DBG(dm, DBG_RF_DPK, "[DPK][DO_DPK]rxbb = 0x%x\n", rxbb);	
	RF_DBG(dm, DBG_RF_DPK, "[DPK][DO_DPK]rf0 = 0x%x\n", odm_get_rf_reg(dm, (enum rf_path)path, RF_0x00, 0xfffff));
	reg_1b67 = tx_agc;	
			 ...
			 u8 group)
{
	...
	u8 reg1b67 = 0x0;
	boolean fail = true;
	u8 threshold_low = 0x0;
	u8 threshold_high = 0x0;
...
#if 0 //  if antenna is not connected, reg1b67 < 0x12
	reg1b67 = odm_read_1byte(dm, R_0x1b67);
	if (reg1b67 <= threshold_low)
		is_fail = true;
	if (reg1b67 > threshold_high)
		is_fail = true;
	...
	u8 path)
{
	...
	if (tmp <= 0x2)
		dpk_info->thermal_dpk[path] = 0x0;
	else
		dpk_info->thermal_dpk[path] = tmp - 2;
...
boolean _do_dpk_8814b(void *dm_void)
{
	...
	for (path = 0; path < SS_8814B; path++) {
		RF_DBG(dm, DBG_RF_DPK, "[DPK] S%d DPK process start v11\n", path);
		_dpk_rfsetting_8814b(dm, path);
		gonext = true;
		dpk_step = 0x0;
		...
		case 0:
			...
			reg_1bfc = _dpk_pas_get_0x1bfc_8814b(dm);
			RF_DBG(dm, DBG_RF_DPK, "[DPK]0x%x, 1bfc = 0x%x\n", dpk_step, reg_1bfc);
 			break;
		case 1:	
#if 0
		...
		case 2:	
			...
			for(i = 0; i < 3; i++) {
				...
				if (dpk_info->tx_gain >= 0x1) {
					break;
				} else {
					rf0 = odm_get_rf_reg(dm, (enum rf_path)path, 0x00, 0xfffff);
					odm_set_rf_reg(dm, (enum rf_path)path, 0x00, 0xfffff, rf0 + 1);					
					RF_DBG(dm, DBG_RF_DPK, "[DPK]txbb + 1, rf0 =%x \n", rf0);
				}
			}
			dpk_step++;
			gl_retry_count++;
		...
		case 3:	
			...
			//dpd_gain= _dpk_pas_get_iq_8814b(dm, GSK4);
			//RF_DBG(dm, DBG_RF_DPK, "[DPK] dpd_gain = %d\n", dpd_gain);
	#if 0
			if (dpd_gain < 370954414) {//1.4
				KFAIL = true;					
				RF_DBG(dm, DBG_RF_DPK, "[DPK] DPD gain < 1.5dB happen, dpd_gain = %d\n", dpd_gain);
			}	
			if (dpd_gain > 489477284) {//2.6
				KFAIL = true;					
				RF_DBG(dm, DBG_RF_DPK, "[DPK] DPD gain > 2.5dB happen, dpd_gain = %d\n", dpd_gain);
			}
	#endif
			dpk_step++;			
			gonext = true;

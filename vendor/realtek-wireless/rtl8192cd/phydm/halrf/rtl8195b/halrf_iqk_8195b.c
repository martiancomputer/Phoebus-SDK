	...
	u32 *buf_size)
{
	...
	if (buf_size)
		*buf_size = IQK_INFO_RSVD_LEN_8195B;
}
...
void iqk_get_cfir_8195b(void *dm_void, u8 idx, u8 path, boolean debug)
{
	...
	for (i = 0; i < IQK_CFIR_NUM_8195B; i++) {
...
#if 1
	for (i = 0; i < IQK_CFIR_NUM_8195B; i++)
		RF_DBG(dm, DBG_RF_IQK, "[IQK]cfir_real[0][%d][%d][%x] = %2x\n",
		       path, idx, i, iqk_info->iqk_cfir_real[0][path][idx][i]);
	for (i = 0; i < IQK_CFIR_NUM_8195B; i++)
		RF_DBG(dm, DBG_RF_IQK, "[IQK]cfir_imag[0][%d][%d][%x] = %2x\n",
	...
	u8 path)
{
	...
	case 0:
		...
		RF_DBG(dm, DBG_RF_IQK, "[IQK]iqk_info->iqk_channel[1] = %2x\n",
		...
		for (i = 0; i < SS_8195B; i++) {
			...
			for (j = 0; j < 0x2; j++) {
				...
				RF_DBG(dm, DBG_RF_IQK, "[IQK]iqk_info->iqk_fail_report[0][%x][%x] = %2x\n",
							i, j, iqk_info->iqk_fail_report[1][i][j]);
				for (k = 0; k <= IQK_CFIR_NUM_8195B; k++) {
	...
	case 1: /*LOK backup*/
		...
		break;
	case 2: /*TXIQK backup*/
		iqk_get_cfir_8195b(dm, TX_IQK, path, false);
		break;
	case 3: /*RXIQK backup*/
		iqk_get_cfir_8195b(dm, RX_IQK, path, false);
		break;
	}
}
...
static u8 wlg_lna[6] = {0x0, 0x1, 0x2, 0x3, 0x4, 0x5};
static u8 wla_lna[6] = {0x0, 0x1, 0x2, 0x3, 0x4, 0x5};
	...
	)
{
	...
	if (!LOK_notready) {
		...
		_iqk_backup_iqk_8195b(dm, 0x1, path);
	} else
		...
		for (i = 0; i < 2; i++) {
			...
			for (j = 0; j < SS_8195B; j++) {
				...
				for (k = 0; k < 2; k++) {
					...
					iqk_info->gs_retry_count[i][j][k] = 0x0;
					for (m = 0; m < 8; m++) {
...
void iqk_reload_iqk_8195b(void *dm_void, boolean reset)
{
	...
	_iqk_reload_iqk_8195b(dm, reset);

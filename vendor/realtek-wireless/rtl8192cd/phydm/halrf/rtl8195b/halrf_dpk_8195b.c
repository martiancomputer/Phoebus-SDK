...
_dpk_check_nctl_done_8195b(struct dm_struct *dm, u8 path, u32 IQK_CMD)
{
	...
	if (!fail)
		RF_DBG(dm, DBG_RF_DPK,
		       "[DPK]S%d IQK_CMD =%x, one shot PASS!!!\n", path, IQK_CMD);
	else
		RF_DBG(dm, DBG_RF_DPK,
		       "[DPK]S%d IQK_CMD =%x, one shot Fail!!!\n", path, IQK_CMD);
	return fail;
	//return false;
}
...
u8 _dpk_get_thermal_8195b(void *dm_void, u8 path)
{
	...
	for (i =0; i< 3; i++){
		thermal_ready = odm_get_rf_reg(dm, RF_PATH_A, 0x42, BIT(17) | BIT(16));
		if(thermal_ready != 0x0)
			ODM_delay_us(50);
		else
			break;
	}
	...
	dpk_info->thermal_dpk[path] = (u8)odm_get_rf_reg(dm, (enum rf_path)path,
							 RF_0x42, 0x0FC00);
#else
	dpk_info->thermal_dpk[path] = (u8) halrf_get_thermal_8195b(dm);
#endif
...
void _dpk_rfsetting_8195b(struct dm_struct *dm, u8 rf_bandtype)
{
	...
	if (rf_bandtype == ODM_BAND_2_4G) {
		odm_set_rf_reg(dm, RF_PATH_A, 0xde, BIT(9), 0x1);
		odm_set_rf_reg(dm, RF_PATH_A, 0x86, 0xf000, 0xf);
		txagc_init = 0x1c;
		if (eethermal == 0x0)
			txagc_init = 0x1c;
	...
	RF_DBG(dm, DBG_RF_DPK, "0xde = %x\n", odm_get_rf_reg(dm, RF_PATH_A, 0xde, RFREGOFFSETMASK));	
	RF_DBG(dm, DBG_RF_DPK, "0x86 = %x\n", odm_get_rf_reg(dm, RF_PATH_A, 0x86, RFREGOFFSETMASK));
#endif
	ODM_sleep_ms(1);
}
...
boolean _dpk_one_shot_8195b(struct dm_struct *dm, u8 path, u8 action)
{
	...
	if (*dm->band_width == 2)
		odm_set_bb_reg(dm, R_0x1b8c, MASKDWORD, 0x4023);
	else
		odm_set_bb_reg(dm, R_0x1b8c, MASKDWORD, 0x4020);
...
void dpk_track_8195b(void *dm_void)
{
...
#else
	thermal_value[0] = (u8) halrf_get_thermal_8195b(dm);
#endif
	...
	if(*dm->channel > 144)
		offset[0] = delta_dpk[0] / DPK_THRESHOLD_8195B_5GH;
	else

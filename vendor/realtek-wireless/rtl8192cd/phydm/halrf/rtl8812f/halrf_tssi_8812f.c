	...
	void *dm_void)
{
	...
	u8 channel = *dm->channel, thermal = 0;
	s8 j;
	u8 rate = phydm_get_tx_rate(dm);
	u32 thermal_offset_tmp = 0, thermal_offset_index = 0x10, thermal_tmp, tmp, i;
	...
	void *dm_void, u8 path)
{
	...
	if (path == RF_PATH_B)
		if (odm_get_bb_reg(dm, R_0x45a4, MASKDWORD) != 0) {
		return 0;
		}
#endif

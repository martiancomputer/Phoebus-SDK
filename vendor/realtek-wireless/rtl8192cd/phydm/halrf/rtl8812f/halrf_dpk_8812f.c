...
/*8812F DPK ver:0xa 20191226*/

static u32
_btc_wait_indirect_reg_ready_8812f(
	struct dm_struct *dm)
{
	u32 delay_count = 0;
	...
	u16 reg_addr)
{
	...
	/* wait for ready bit before access 0x1700 */
	_btc_wait_indirect_reg_ready_8812f(dm);
	...
	u32 reg_value)
{
	...
	odm_write_4byte(dm, 0x1700, 0xc00F0000 | reg_addr);
		...
		val = _btc_read_indirect_reg_8812f(dm, reg_addr);
	...
	boolean is_before_k)
{
	...
	if (is_before_k) {
		...
		dpk_info->gnt_value = _btc_read_indirect_reg_8812f(dm, 0x38);
		
		/*force GNT control to WL*/
		odm_set_mac_reg(dm, R_0x70, BIT(26), 0x1);
		/*force GNT_WL=1, GNT_BT=0*/
		_btc_write_indirect_reg_8812f(dm, 0x38, 0xFF00, 0x77);
#if 0
		...
		RF_DBG(dm, DBG_RF_DPK, "[DPK] set 0x70/0x38 = 0x%x/0x%x\n",
		       ...
		       _btc_read_indirect_reg_8812f(dm, 0x38));
#endif
	} else {
		_btc_write_indirect_reg_8812f(dm, 0x38, MASKDWORD, dpk_info->gnt_value);
...
void _backup_mac_bb_registers_8812f(
	struct dm_struct *dm,
	u32 *reg,
	u32 *reg_backup,
	u32 reg_num)
...
void _backup_rf_registers_8812f(
	struct dm_struct *dm,
	u32 *rf_reg,
	u32 rf_reg_backup[][2])
{
	...
	for (i = 0; i < DPK_RF_REG_NUM_8812F; i++) {
...
void _reload_mac_bb_registers_8812f(
	struct dm_struct *dm,
	u32 *reg,
	u32 *reg_backup,
	u32 reg_num)
...
void _reload_rf_registers_8812f(
	struct dm_struct *dm,
	u32 *rf_reg,
	u32 rf_reg_backup[][2])
{
	u32 i, rf_reg_8f[DPK_RF_PATH_NUM_8812F] = {0x0};
	...
	for (i = 0; i < DPK_RF_PATH_NUM_8812F; i++) {
...
void _dpk_mac_bb_setting_8812f(
	struct dm_struct *dm)
{
	struct dm_dpk_info *dpk_info = &dm->dpk_info;

	_dpk_tx_pause_8812f(dm);
...
void _dpk_bb_path_setting_8812f(
	struct dm_struct *dm,
	u8 path)
{
#if 0
	...
	struct dm_struct *dm)
{
	...
	for (path = 0; path < DPK_RF_PATH_NUM_8812F; path++) {
		odm_set_bb_reg(dm, R_0x1b00, MASKDWORD, 0x8 | (path << 1));
		odm_set_bb_reg(dm, R_0x1b60, MASKDWORD, 0x1F0D0000); /*txagc bnd*/
		odm_set_bb_reg(dm, R_0x1b44, 0x00007000, 0x5); /*GL = val*0.5+1*/
		odm_set_bb_reg(dm, R_0x1b20, 0xc0000000, 0x3); /*CFIR to TX*/
	}

	odm_set_bb_reg(dm, R_0x1b00, 0x0000000f, 0xc);

	odm_write_1byte(dm, 0x1bb4, 0x00);
	odm_write_1byte(dm, 0x1bb6, 0x00);
	odm_set_bb_reg(dm, 0x1bb8, MASKDWORD, 0x20202020);
	odm_set_bb_reg(dm, 0x1bbc, MASKDWORD, 0x20202020);
	odm_set_bb_reg(dm, 0x1bc0, MASKDWORD, 0x20202020);
	odm_set_bb_reg(dm, 0x1bc4, MASKDWORD, 0x20202020);
	odm_set_bb_reg(dm, 0x1bc8, MASKDWORD, 0x04040404);
	odm_set_bb_reg(dm, 0x1bcc, MASKDWORD, 0x04040404);
	odm_set_bb_reg(dm, 0x1bd0, MASKDWORD, 0x04040404);
#if 0
	if (dpk_info->dpk_bw == 3) { /*20M*/
		odm_write_1byte(dm, 0x1bb4, 0xe0);  /*rec_sram_dly*/
		odm_write_1byte(dm, 0x1bb6, 0xe0);  /*tpg_dly*/
		odm_set_bb_reg(dm, 0x1bb8, MASKDWORD, 0x60e06060);
		odm_set_bb_reg(dm, 0x1bbc, MASKDWORD, 0x60e06060);
		odm_set_bb_reg(dm, 0x1bc0, MASKDWORD, 0x60e06060);
		odm_set_bb_reg(dm, 0x1bc4, MASKDWORD, 0x20e02020);
		odm_set_bb_reg(dm, 0x1bc8, MASKDWORD, 0x04080404);
		odm_set_bb_reg(dm, 0x1bcc, MASKDWORD, 0x04080404);
		odm_set_bb_reg(dm, 0x1bd0, MASKDWORD, 0x04080404);
	} else {
		odm_write_1byte(dm, 0x1bb4, 0x00);
		odm_write_1byte(dm, 0x1bb6, 0x00);
		odm_set_bb_reg(dm, 0x1bb8, MASKDWORD, 0x20202020);
	...
	u8 path)
{
	...
	RF_DBG(dm, DBG_RF_DPK, "[DPK] S%d txidx_offset = 0x%x\n",
	       path, txidx_offset);
#endif
	if (*dm->band_type == ODM_BAND_2_4G) { /*2G*/
		/*TXAGC for gainloss*/
		odm_set_rf_reg(dm, (enum rf_path)path,
			       RF_0x00, RFREG_MASK, 0x50010 + txidx_offset);
		/*ATT Gain*/
		...
		if (path == RF_PATH_A)/*TXAGC for gainloss*/
			odm_set_rf_reg(dm, (enum rf_path)path,
				       RF_0x00, RFREG_MASK, 0x50013 + txidx_offset);
		else
		odm_set_rf_reg(dm, (enum rf_path)path,
				       RF_0x00, RFREG_MASK, 0x50014 + txidx_offset);
		/*ATT Gain*/
	...
	return odm_get_rf_reg(dm, (enum rf_path)path, RF_0x00, RFREG_MASK);
}

void _dpk_sync_dc_20m_8812f(
	struct dm_struct *dm)
{
	odm_set_bb_reg(dm, R_0x1b00, 0x0000000f, 0xc); /*subpage 2*/

	/*disable MDPD*/
	odm_write_1byte(dm, 0x1b04, 0x80);
	odm_write_1byte(dm, 0x1b0b, 0x00);
	odm_write_1byte(dm, 0x1b5c, 0x80);
	odm_write_1byte(dm, 0x1b63, 0x00);

	/*set DC_i*/
	odm_write_1byte(dm, 0x1bf8, 0xff);
	odm_write_1byte(dm, 0x1bf9, 0xfa);
	odm_write_1byte(dm, 0x1bfa, 0x41);
	odm_write_1byte(dm, 0x1bfb, 0x80);
	odm_write_1byte(dm, 0x1bfb, 0x00);

	/*set DC_q*/
	odm_write_1byte(dm, 0x1bf8, 0xf1);
	odm_write_1byte(dm, 0x1bf9, 0xe2);
	odm_write_1byte(dm, 0x1bfa, 0x51);
	odm_write_1byte(dm, 0x1bfb, 0x80);
	odm_write_1byte(dm, 0x1bfb, 0x00);

	/*set back threshold*/
	odm_write_1byte(dm, 0x1bf8, 0x28);
	odm_write_1byte(dm, 0x1bf9, 0xb4);
	odm_write_1byte(dm, 0x1bfa, 0x00);
	odm_write_1byte(dm, 0x1bfb, 0x00);

	/*fmod=20m*/
	odm_write_1byte(dm, 0x1bf4, 0x10);

	/*RF_DBG(dm, DBG_RF_DPK, "[DPK] -Sync DC\n");*/
}

void _dpk_sync_path_8812f(
	struct dm_struct *dm,
	u8 path)
{
	u8 sync_done = 0x0;
	...
	odm_write_1byte(dm, 0x1bf5, 0x00);
	odm_write_1byte(dm, 0x1bf6, 0x02);
	odm_write_1byte(dm, 0x1bf7, 0x15 + path * 0x10);
	odm_write_1byte(dm, 0x1bb4, 0xe0);
	odm_write_1byte(dm, 0x1bb6, 0xe0);
	
	odm_write_1byte(dm, 0x1bb7, 0x00 + path);
	odm_write_1byte(dm, 0x1bb5, 0x10); /*rec_sram_one_shot*/
	odm_write_1byte(dm, 0x1bb5, 0x00);

	odm_set_bb_reg(dm, R_0x1bd4, 0x001f0000, 0x0);
	ODM_delay_us(20);
	sync_done = (u8)odm_get_bb_reg(dm, R_0x1bfc, BIT(31));
	while (sync_done != 0x1 && count < 2000) {
		ODM_delay_us(20);
		sync_done = (u8)odm_get_bb_reg(dm, R_0x1bfc, BIT(31));
		count++;
	}

	/*RF_DBG(dm, DBG_RF_DPK, "[DPK] -Sync S%d (count=%d)\n", path, count);*/

	if (count == 2000)
		RF_DBG(dm, DBG_RF_DPK, "[DPK] Sync one-shot over 40ms!!!!\n");

	odm_write_1byte(dm, 0x1bf7, 0x00);
}

void _dpk_lms_dc_20m_8812f(
	struct dm_struct *dm)
{
	/*set DC_i*/
	odm_write_1byte(dm, 0x1bf8, 0xaf);
	odm_write_1byte(dm, 0x1bf9, 0xfb);
	odm_write_1byte(dm, 0x1bfa, 0x41);
	odm_write_1byte(dm, 0x1bfb, 0x80);
	odm_write_1byte(dm, 0x1bfb, 0x00);

	/*set DC_q*/
	odm_write_1byte(dm, 0x1bf8, 0x43);
	odm_write_1byte(dm, 0x1bf9, 0xf4);
	odm_write_1byte(dm, 0x1bfa, 0x51);
	odm_write_1byte(dm, 0x1bfb, 0x80);
	odm_write_1byte(dm, 0x1bfb, 0x00);
	...
	/*RF_DBG(dm, DBG_RF_DPK, "[DPK] -LMS DC\n");*/
}

void _dpk_lms_path_8812f(
	struct dm_struct *dm,
	u8 path)
{
	u8 sync_done = 0x0;
	...
	odm_write_1byte(dm, 0x1b04 + path * 0x58, 0x80);
	odm_write_1byte(dm, 0x1b07 + path * 0x58, 0x30);
	odm_write_1byte(dm, 0x1bb7, 0x6c + path);

	odm_set_bb_reg(dm, R_0x1bd4, 0x001f0000, 0x5);
	ODM_delay_us(20);
	sync_done = (u8)odm_get_bb_reg(dm, R_0x1bfc, BIT(16));
	while (sync_done != 0x1 && count < 2000) {
		ODM_delay_us(20);
		sync_done = (u8)odm_get_bb_reg(dm, R_0x1bfc, BIT(16));
		count++;
	...
	/*RF_DBG(dm, DBG_RF_DPK, "[DPK] -LMS S%d (count=%d)\n", path, count);*/

	if (count == 2000)
		RF_DBG(dm, DBG_RF_DPK, "[DPK] LMS one-shot over 40ms!!!!\n");
}

void _dpk_mdpd_gl_path_8812f(
	struct dm_struct *dm,
	u8 path)
{
	odm_write_1byte(dm, 0x1bb7, 0x00);

	odm_write_1byte(dm, 0x1b09 + path * 0x58, 0x04);
	odm_write_1byte(dm, 0x1b0b + path * 0x58, 0xb0);

	odm_write_1byte(dm, 0x1bb5, 0x04 << path);
	odm_write_1byte(dm, 0x1bb7, 0x08);

	odm_write_1byte(dm, 0x1bf7, 0x10 + path * 0x10);

	odm_write_1byte(dm, 0x1b04 + path * 0x58, 0x80);
	odm_write_1byte(dm, 0x1b05 + path * 0x58, 0xff);
	odm_write_1byte(dm, 0x1b06 + path * 0x58, 0x3f);
	odm_write_1byte(dm, 0x1b07 + path * 0x58, 0x31);

	odm_set_bb_reg(dm, R_0x1b00, 0x0000000f, 0x8 | (path << 1));

	/*RF_DBG(dm, DBG_RF_DPK, "[DPK] -MDPD GL S%d\n", path);*/
}

void _dpk_gl_20m_8812f(
	struct dm_struct *dm)
{
	u8 sync_done = 0x0;
	u16 count = 0;
	...
	/*trigger PAS*/
	odm_write_1byte(dm, 0x1bcc, 0x12); /*ItQt*/
	odm_write_1byte(dm, 0x1bce, 0x20);
	odm_write_1byte(dm, 0x1b59, 0x8f);

	odm_write_1byte(dm, 0x1b40, 0x02); /*PA scan mode*/
	odm_write_1byte(dm, 0x1b4c, 0x01); /*one-shot*/
	odm_write_1byte(dm, 0x1b4c, 0x00);

	odm_set_bb_reg(dm, R_0x1bd4, 0x001f0000, 0x7);
	ODM_delay_us(20);
	sync_done = (u8)odm_get_bb_reg(dm, R_0x1bfc, BIT(0));
	while (sync_done != 0x1 && count < 2000) {
		ODM_delay_us(20);
		sync_done = (u8)odm_get_bb_reg(dm, R_0x1bfc, BIT(0));
		count++;
	...
	if (count == 2000)
		RF_DBG(dm, DBG_RF_DPK, "[DPK] triggrt PAS one-shot over 40ms!!!!\n");

	/*gen-1 LUT*/
	odm_write_1byte(dm, 0x1b40, 0x03); /*LUT gen mode*/
	odm_write_1byte(dm, 0x1b49, 0x80);
	odm_write_1byte(dm, 0x1b56, 0xc0);
	odm_write_1byte(dm, 0x1b54, 0x02);
	odm_write_1byte(dm, 0x1b4c, 0x01); /*one-shot*/
	odm_write_1byte(dm, 0x1b4c, 0x00);
	...
	if (count == 2000)
		RF_DBG(dm, DBG_RF_DPK, "[DPK] Gen-1 LUT one-shot over 40ms!!!!\n");

	odm_write_1byte(dm, 0x1b54, 0x00);
	odm_write_1byte(dm, 0x1bcc, 0x00); /*ItQt*/
	odm_write_1byte(dm, 0x1bce, 0x00);

	/*gain loss*/
	odm_write_1byte(dm, 0x1b40, 0x04); /*gain loss mode*/
	odm_write_1byte(dm, 0x1b4c, 0x01); /*one-shot*/
	odm_write_1byte(dm, 0x1b4c, 0x00);
	...
	/*RF_DBG(dm, DBG_RF_DPK, "[DPK] -Do GL (count=%d)\n", count);*/

	if (count == 2000)
		RF_DBG(dm, DBG_RF_DPK, "[DPK] GL main one-shot over 40ms!!!!\n");
}

void _dpk_tpg_src_path_8812f(
	struct dm_struct *dm,
	u8 path)
{
	u32 reg_1b00 = 0x0;

	if (odm_get_bb_reg(dm, 0x1e2c, 0xffff) == 0x0400) { /*Tx = A or B only*/
		reg_1b00 = odm_get_bb_reg(dm, 0x1b00, MASKDWORD);	
		odm_set_bb_reg(dm, 0x1b00, 0x0000000f, 0xc);
		odm_write_1byte(dm, 0x1bb5, 0x04 << path);
		RF_DBG(dm, DBG_RF_DPK, "[DPK] Set TPG Src S%d\n", path);
		odm_set_bb_reg(dm, 0x1b00, MASKDWORD, reg_1b00);
	}
}

void _dpk_one_shot_20m_8812f(
	struct dm_struct *dm,
	u8 path,
	u8 action)
{
	RF_DBG(dm, DBG_RF_DPK, "[DPK] one-shot for S%d %s (20M):\n", path, 
		action == 1 ? "GL" : (action == 2 ? "DO_DPK" :
	       (action == 3 ? "DPK_ON" : "DAGC")));

	switch (action) {
	case DAGC:
		_dpk_sync_dc_20m_8812f(dm);
		_dpk_sync_path_8812f(dm, path);
		break;

	case DO_DPK:
		_dpk_sync_dc_20m_8812f(dm);
		_dpk_sync_path_8812f(dm, path);
		_dpk_lms_dc_20m_8812f(dm);
		_dpk_lms_path_8812f(dm, path);
		break;
	...
	case GAIN_LOSS:
		...
		_dpk_lms_dc_20m_8812f(dm);
		_dpk_lms_path_8812f(dm, path);
		_dpk_mdpd_gl_path_8812f(dm, path);
		_dpk_gl_20m_8812f(dm);
		break;
	...
	u8 action)
{
	...
	/*_btc_set_gnt_wl_bt_8812f(dm, true);*/
	...
	/*_btc_set_gnt_wl_bt_8812f(dm, false);*/

	odm_write_1byte(dm, 0x1b10, 0x0);
}

u16 _dpk_dgain_read_8812f(
	struct dm_struct *dm,
	u8 path)
{
	u16 dgain = 0x0;
	...
	u8 check)
{
	...
	if (check == GAIN_CHK) {
		_dpk_one_shot_8812f(dm, path, DAGC);
		dgain = _dpk_dgain_read_8812f(dm, path);
		...
		loss = _dpk_pas_read_8812f(dm, path, LOSS_CHK);
		...
		if ((loss_db - 3870) > 1000) { /*GL > 10dB*/
			...
			RF_DBG(dm, DBG_RF_DPK, "[DPK] GLoss < 1.5dB happen!!\n");
			result = 4;
			return result;
		}  else
			return result;
	...
	u8 loss_only)
{
	...
	u8 tmp_txagc = 0, tmp_pga = 0, i = 0;
	u8 goout = 0, limited_pga = 0, agc_cnt = 0, gl_idx;
	...
	do {
		...
		case 0: /*Gain check first*/
			...
			if (loss_only)
				...
				i = _dpk_agc_chk_8812f(dm, path, limited_pga,
						       GAIN_CHK);
		...
		case 3: /*GL > criterion*/
			...
			if (gl_idx > 0x3)
				tmp_txagc = tmp_txagc - 0x2;
			else
				tmp_txagc = tmp_txagc - 0x1;
		...
		case 5: /*Loss check*/
			_dpk_one_shot_8812f(dm, path, GAIN_LOSS);
			i = _dpk_agc_chk_8812f(dm, path, limited_pga, LOSS_CHK);

			gl_idx = _dpk_gainloss_result_8812f(dm, path);
			RF_DBG(dm, DBG_RF_DPK, "[DPK][AGC] tmp GL_idx = 0x%x\n", gl_idx);

#if (DPK_PAS_DBG_8812F)
			_dpk_pas_read_8812f(dm, path, PAS_READ);
#endif
			if (i == 0)
				goout = 1;
			break;
...
boolean _dpk_coef_iq_check_8812f(
	struct dm_struct *dm,
	u16 coef_i,
	u16 coef_q)
{
	...
	u8 path)
{
	...
	dpk_info->coef[path][0] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x040400F0);
	dpk_info->coef[path][1] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x080400F0);
	dpk_info->coef[path][2] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x010400F0);
	dpk_info->coef[path][3] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x050400F0);
	dpk_info->coef[path][4] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x090400F0);
	dpk_info->coef[path][5] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x020400F0);
	dpk_info->coef[path][6] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x060400F0);
	dpk_info->coef[path][7] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x0A0400F0);
	dpk_info->coef[path][8] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x030400F0);
	dpk_info->coef[path][9] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x070400F0);
	dpk_info->coef[path][10] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x0B0400F0);
	dpk_info->coef[path][11] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x0C0400F0);
	dpk_info->coef[path][12] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x100400F0);
	dpk_info->coef[path][13] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x0D0400F0);
	dpk_info->coef[path][14] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x110400F0);
	dpk_info->coef[path][15] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x0E0400F0);
	dpk_info->coef[path][16] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x120400F0);
	dpk_info->coef[path][17] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x0F0400F0);
	dpk_info->coef[path][18] = _dpk_coef_transfer_8812f(dm);
	odm_set_bb_reg(dm, R_0x1bd4, MASKDWORD, 0x130400F0);
	dpk_info->coef[path][19] = _dpk_coef_transfer_8812f(dm);

}

u8 _dpk_coef_read_8812f(
	void *dm_void,
	u8 path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	...
	for (addr = 0; addr < 20; addr++) {
		...
		if (_dpk_coef_iq_check_8812f(dm, coef_i, coef_q)) {
			result = 0;
			break;
		}
	}
...
#if (DPK_COEF_DBG_8812F)
		RF_DBG(dm, DBG_RF_DPK,
		       "[DPK][coef_w] 0x%x = 0x%08x\n", tmp_reg, coef);
#endif
	}
}

void _dpk_coef_default_8812f(
	void *dm_void,
	u8 path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	...
	u8 result)
{
	...
	if (result) /*check PASS*/
		odm_write_1byte(dm, R_0x1b67, (u8)dpk_txagc);	
	else
	...
	_dpk_coef_write_8812f(dm, path, result);
}

u32 _dpk_gainloss_8812f(
	struct dm_struct *dm,
	u8 path)
{
	struct dm_dpk_info *dpk_info = &dm->dpk_info;

	u8 k = 0, tx_agc_search = 0x0, result[5] = {0x0};
	s8 txidx_offset = 0x0;
	u32 tx_agc;

	dpk_info->thermal_dpk[path] = _dpk_thermal_read_8812f(dm, path);
	RF_DBG(dm, DBG_RF_DPK, "[DPK][GL] S%d thermal = 0x%x\n", 
	       path, dpk_info->thermal_dpk[path]);

	_dpk_rf_setting_8812f(dm, path);

	_dpk_tpg_src_path_8812f(dm, path);

	_dpk_one_shot_8812f(dm, path, DAGC);
	_dpk_dgain_read_8812f(dm, path);
	_dpk_dc_corr_check_8812f(dm, path);

	_dpk_tpg_src_path_8812f(dm, path);
#if 1
	_dpk_pas_agc_8812f(dm, path, false, true);
#else
	_dpk_one_shot_8812f(dm, path, GAIN_LOSS);
#if (DPK_PAS_DBG_8812F)
	_dpk_pas_read_8812f(dm, path, PAS_READ);
...
#endif
	tx_agc = odm_get_rf_reg(dm, (enum rf_path)path, RF_0x00, 0x0001f);

	tx_agc_search = _dpk_gainloss_result_8812f(dm, path);
#if 0
	RF_DBG(dm, DBG_RF_DPK,
	...
	if (DPK_PAS_DBG_8812F)
		_dpk_pas_read_8812f(dm, path, PAS_READ);
#endif
	tx_agc = tx_agc - tx_agc_search;

	RF_DBG(dm, DBG_RF_DPK, "[DPK][GL] S%d GL_idx= 0x%x, txagc_index= 0x%x\n",
	       path, tx_agc_search, tx_agc);

	return tx_agc;
}

u8 _dpk_by_path_8812f(
	struct dm_struct *dm,
	u32 tx_agc,
	u8 path)
{
	...
	RF_DBG(dm, DBG_RF_DPK,
	       ...
	       odm_get_rf_reg(dm, (enum rf_path)path, RF_0x8f, RFREG_MASK));
#endif
	_dpk_one_shot_8812f(dm, path, DO_DPK);
	...
	u8 path)
{
	...
	tmp_gs = (odm_get_bb_reg(dm, R_0x1bfc, 0x0FFF0000) * 910) >> 10;
	/*910 = 0x5b * 10*/
	...
	dpk_info->dpk_gs[path] = (u16)tmp_gs;
	...
	u8 path)
{
	...
	_dpk_one_shot_8812f(dm, path, DPK_ON);

	odm_set_bb_reg(dm, R_0x1b00, 0x0000000f, 0x8 | (path << 1));
	odm_set_bb_reg(dm, R_0x1b20, 0xc0000000, 0x0); /* disable CFIR to TX*/

	_dpk_cal_gs_8812f(dm, path);
	...
	u8 path)
{
	...
	if (!is_fail)
		if (_dpk_coef_read_8812f(dm, path))
			result = 1; /*check PASS*/
		else
			result = 0; /*check FAIL*/
	else
	...
	_dpk_fill_result_8812f(dm, dpk_txagc, path, result);
	return result;
}

void _dpk_result_reset_8812f(
	struct dm_struct *dm)
{
	struct dm_dpk_info *dpk_info = &dm->dpk_info;
	...
	u8 path)
{
	...
	u32 dpk_txagc = 0;
       ...
       "[DPK] =========== S%d DPK Start ===========\n", path);		
		...
		dpk_txagc = _dpk_gainloss_8812f(dm, path);

		dpk_fail = _dpk_by_path_8812f(dm, dpk_txagc, path);

			RF_DBG(dm, DBG_RF_DPK,
	       "[DPK] =========== S%d DPK Finish ==========\n", path);

		if (_dpk_check_fail_8812f(dm, dpk_fail, dpk_txagc, path))
				break;
		}
...
void _dpk_path_select_8812f(
	struct dm_struct *dm)
{
	struct dm_dpk_info *dpk_info = &dm->dpk_info;

#if (DPK_PATH_A_8812F)
	_dpk_calibrate_8812f(dm, RF_PATH_A);
#endif

#if (DPK_PATH_B_8812F)
	_dpk_calibrate_8812f(dm, RF_PATH_B);
#endif
#if (DPK_PATH_A_8812F)
	_dpk_on_8812f(dm, RF_PATH_A);
#endif
#if (DPK_PATH_B_8812F)
	_dpk_on_8812f(dm, RF_PATH_B);
#endif
}

void _dpk_result_summary_8812f(
	struct dm_struct *dm)
{
	struct dm_dpk_info *dpk_info = &dm->dpk_info;
	...
	void *dm_void)
{
	...
	u32 mac_reg_backup[DPK_MAC_REG_NUM_8812F];
	u32 bb_reg_backup[DPK_BB_REG_NUM_8812F];
	u32 rf_reg_backup[DPK_RF_REG_NUM_8812F][DPK_RF_PATH_NUM_8812F];

	u32 mac_reg[DPK_MAC_REG_NUM_8812F] = {R_0x520, R_0x550};
	u32 bb_reg[DPK_BB_REG_NUM_8812F] = {
		R_0x820, R_0x824, R_0x1c3c, R_0x1d58, R_0x1864,
		R_0x4164, R_0x180c, R_0x410c, R_0x186c, R_0x416c,
		R_0x1a14, R_0x1e70, R_0x80c, R_0x18a4, R_0x41a4,
		R_0x1e7c};
	u32 rf_reg[DPK_RF_REG_NUM_8812F] = {
		RF_0x0, RF_0x1a, RF_0x55, RF_0x63, RF_0x87, RF_0x8f, RF_0xde};
	...
	if (dm->ext_pa || dm->ext_pa_5g) {
		...
		RF_DBG(dm, DBG_RF_DPK,
		       ...
		       DPK_VER_8812F, dm->cut_version, dm->package_type);
	}

	_dpk_information_8812f(dm);

	_backup_mac_bb_registers_8812f(dm, mac_reg, mac_reg_backup,
				       DPK_MAC_REG_NUM_8812F);
	_backup_mac_bb_registers_8812f(dm, bb_reg, bb_reg_backup,
				       DPK_BB_REG_NUM_8812F);
	_backup_rf_registers_8812f(dm, rf_reg, rf_reg_backup);

	_dpk_mac_bb_setting_8812f(dm);
	_dpk_afe_setting_8812f(dm, true);
	_dpk_manual_txagc_8812f(dm, true);
	_dpk_pre_setting_8812f(dm);

	_dpk_result_reset_8812f(dm);
	_dpk_path_select_8812f(dm);
	_dpk_result_summary_8812f(dm);

	_dpk_manual_txagc_8812f(dm, false);
	_dpk_afe_setting_8812f(dm, false);

	dpk_enable_disable_8812f(dm);

	_reload_rf_registers_8812f(dm, rf_reg, rf_reg_backup);
	_reload_mac_bb_registers_8812f(dm, bb_reg, bb_reg_backup,
				       DPK_BB_REG_NUM_8812F);
	_reload_mac_bb_registers_8812f(dm, mac_reg, mac_reg_backup,
				       DPK_MAC_REG_NUM_8812F);
	...
	void *dm_void)
{
	...
	/*get thermal meter*/
	for (path = 0; path < DPK_RF_PATH_NUM_8812F; path++) {
		thermal_value[path] = _dpk_thermal_read_8812f(dm, path);

	...
	enum halrf_lna_set type)
{
	...
	if (type == HALRF_LNA_DISABLE) {
		...
		odm_set_rf_reg(dm, RF_PATH_A, RF_0x33, 0xfffff, 0x00003); /*select Rx mode*/
		odm_set_rf_reg(dm, RF_PATH_A, RF_0x3d, 0xfffff, 0x00000);
		odm_set_rf_reg(dm, RF_PATH_A, RF_0x3e, 0xfffff, 0x0001e); /*disable LNA*/
		odm_set_rf_reg(dm, RF_PATH_A, RF_0x3f, 0xfffff, 0xd23f6); 
		...
		odm_set_rf_reg(dm, RF_PATH_A, RF_0x33, 0xfffff, 0x00003); /*select Rx mode*/
		odm_set_rf_reg(dm, RF_PATH_A, RF_0x3d, 0xfffff, 0x00000);
		odm_set_rf_reg(dm, RF_PATH_A, RF_0x3e, 0xfffff, 0x0001f); /*back to normal*/
		odm_set_rf_reg(dm, RF_PATH_A, RF_0x3f, 0xfffff, 0xd23f6); 
		odm_set_rf_reg(dm, RF_PATH_A, RF_0xef, 0x80000, 0x0);
	}
}
	...
	/* CCK Follow path-A and lower CCK index means higher power. */
		...
		case RF_PATH_A:
			if (final_ofdm_swing_index > pwr_tracking_limit_ofdm) {
				...
				if (cali_info->absolute_ofdm_swing_idx[rf_path] >= 8) {
				/*Add 1db at BBswing,because TXAGC full at 110 deg */
					cali_info->remnant_ofdm_swing_idx[rf_path] =
						(final_ofdm_swing_index -
						 pwr_tracking_limit_ofdm -1) * 2;
			...
			if (final_cck_swing_index > pwr_tracking_limit_cck) {
					if (cali_info->absolute_cck_swing_idx[rf_path] >= 8) {
				/*Add 1db at BBswing,because TXAGC full at 110 deg */
						cali_info->remnant_cck_swing_idx =
						(final_cck_swing_index -
						 pwr_tracking_limit_cck -1) * 2;
...
void
_phy_realtime_calibrate_8721d(struct dm_struct *dm, boolean is2T)
{
	u8 tmp_reg, bb_clk;
	u32 rf_amode = 0, rf_bmode = 0, aac_reg, rt_cal, cnt;

	RF_DBG(dm, DBG_RF_LCK, "ORIGIN VCO_CV = 0x%x, LOCKDET = 0xA8 = 0x%x\n",
	       odm_get_rf_reg(dm, RF_PATH_A, 0xA2, 0xff),
	       odm_get_rf_reg(dm, RF_PATH_A, 0xA8, 0xfffff));
	RF_DBG(dm, DBG_RF_LCK, "RTK:Start!!!\n");
	...
	/*3. backup RF0x18*/
	aac_reg = odm_get_rf_reg(dm, RF_PATH_A, 0xa3, RFREGOFFSETMASK);
	rt_cal = odm_get_rf_reg(dm, RF_PATH_A, 0xc1, RFREGOFFSETMASK);

	/*4. Start LCK: RF reg18 bit15=1*/
	odm_set_rf_reg(dm, RF_PATH_A, 0xa3, 0xfffff, 0x6b40c);
	odm_set_rf_reg(dm, RF_PATH_A, 0xc0, 0x80000, 0x1);
	odm_set_rf_reg(dm, RF_PATH_A, 0xc1, 0x00800, 0x1);
	odm_set_rf_reg(dm, RF_PATH_A, 0xc1, 0x00004, 0x1);

	ODM_delay_ms(5);

	/*5. Recover channel number*/
	odm_set_rf_reg(dm, RF_PATH_A, 0xa3, RFREGOFFSETMASK, aac_reg);
	odm_set_rf_reg(dm, RF_PATH_A, 0xc1, RFREGOFFSETMASK, rt_cal);
	
	RF_DBG(dm, DBG_RF_LCK, "PATH A 0xC2 = 0x%x, 0xC3 = 0x%x\n",
	       odm_get_rf_reg(dm, RF_PATH_A, 0xC2, 0xfffff),
	       odm_get_rf_reg(dm, RF_PATH_A, 0xC3, 0xfffff));
	RF_DBG(dm, DBG_RF_LCK, "PATH A VCO_CV=0xA2 = 0x%x, 0xA8 = 0x%x\n",
	       odm_get_rf_reg(dm, RF_PATH_A, 0xA2, 0xfffff),
	       odm_get_rf_reg(dm, RF_PATH_A, 0xA8, 0xfffff));
	...
	RF_DBG(dm, DBG_RF_LCK, "RTSK:Stop!!!\n");
}
...
phy_iq_calibrate_8721d(void *dm_void, boolean is_recovery)
{
	...
	if (final_candidate < 4) {
		...
		cali_info->iqk_matrix_reg_setting[0].is_iqk_done = true;
	}
...
phy_lc_calibrate_8721d(void *dm_void)
{
	...
	RF_DBG(dm, DBG_RF_LCK, "LCK start!!!\n");
	/*PwrTracking trigger RTK instead of LCK*/
	/*_phy_lc_calibrate_8721d(dm, false);*/
	_phy_realtime_calibrate_8721d(dm, false);
...
void phy_poll_lck_8721d(struct dm_struct *dm)
{
	...
	RF_DBG(dm, DBG_RF_LCK, "[LCK]lck_counter = %d\n", counter);
}

void phy_aac_8721d(struct dm_struct *dm)
{
	/*AAC: Amplitude Auto Control!*/
	odm_set_rf_reg(dm, RF_PATH_A, RF_0xa3, 0xfffff, 0x6B408);
	odm_set_rf_reg(dm, RF_PATH_A, RF_0xa3, 0xfffff, 0x6B40C);
	ODM_delay_us(200);

	RF_DBG(dm, DBG_RF_LCK, "[LCK]AAC end!\n");
}

void phy_x2k_8721d(struct dm_struct *dm)
{
	odm_set_rf_reg(dm, RF_PATH_A, RF_0xca, 0xfffff, 0xD4E20);
	ODM_delay_ms(1);
	RF_DBG(dm, DBG_RF_LCK, "[LCK]X2K end!\n");
}

void phy_x2_check_8721d(struct dm_struct *dm)
{
	u32 busy;
	/*check X2K, X2K_busy == regC8[7]*/
	busy = odm_get_rf_reg(dm, RF_PATH_A, RF_0xc8, 0x00080);
	if (busy) {
		RF_DBG(dm, DBG_RF_LCK, "[LCK]X2K_busy! begin X2K check!\n");
		odm_set_rf_reg(dm, RF_PATH_A, RF_0xc8, 0x40, 0x1);
		odm_set_rf_reg(dm, RF_PATH_A, RF_0xc8, 0x80, 0x0);
		odm_set_rf_reg(dm, RF_PATH_A, RF_0xca, 0x80000, 0x0);
		odm_set_rf_reg(dm, RF_PATH_A, RF_0xc8, 0x40, 0x0);
		odm_set_rf_reg(dm, RF_PATH_A, RF_0xc8, 0x40000, 0x0);
	} else {
		RF_DBG(dm, DBG_RF_LCK, "[LCK]X2K check end!!!!!!!\n");
	}
}

void phy_syn_calibrate_part1_8721d(struct dm_struct *dm)
{
	RF_DBG(dm, DBG_RF_LCK, "[LCK]phy_syn_calibrate_part11!\n");
	phy_aac_8721d(dm);
	/*LCK_part1*/
	odm_set_rf_reg(dm, RF_PATH_A, RF_CHNLBW, 0xfffff, 0x8401);
}

void phy_syn_calibrate_part2_8721d(struct dm_struct *dm)
{
	/*LCK_part2*/
	phy_poll_lck_8721d(dm);
	odm_set_rf_reg(dm, RF_PATH_A, RF_CHNLBW, 0xfffff, 0x401);
	phy_x2k_8721d(dm);
	phy_x2_check_8721d(dm);
	RF_DBG(dm, DBG_RF_LCK, "[LCK]phy_syn_calibrate_part2!syn done!\n");
}

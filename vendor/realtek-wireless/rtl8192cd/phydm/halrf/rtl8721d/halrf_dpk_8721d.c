...
/*8721D DPK v03 20191225 by shirley*/
...
u8 _dpk_pas_gain_check_8721d(struct dm_struct *dm)
{
	u8 result = 0, txagc_backoff;
	u32 loss = 0, gain = 0;
	s32 loss_db = 0, gain_db = 0;

	txagc_backoff = odm_get_bb_reg(dm, R_0xbdc, 0x0000000f) & 0xf;
...
#if (DPK_GAINLOSS_DBG_8721D)
	RF_DBG(dm, DBG_RF_DPK, "[DPK] txagc_backoff = 0x%x\n", txagc_backoff);
	...
	if (gain == 0 && loss == 0) {
		RF_DBG(dm, DBG_RF_DPK, "[DPK][Error]Gainloss func failed!!!\n");
		return result;
	} else if (txagc_backoff < 0x6) {
		/*GL > 6dB*/
		RF_DBG(dm, DBG_RF_DPK, "[DPK][GL_Chk] GL > 5dB happen!!\n");
		result = 3;
		return result;
	} else if (txagc_backoff == 0xa) {
		/*GL < 2dB*/
...
void _dpk_pas_agc_8721d(struct dm_struct *dm, u8 path)
{
	u8 tmp_txagc, tmp_pga, i = 0;
	u8 goout = 0, limited_pga = 0;
	u8 agc_cnt = 0, txagc_backoff;
	...
	do {
		...
		case 0: /*one-shot*/
			...
			RF_DBG(dm, DBG_RF_DPK,
			       "[DPK][AGC]count%d: TXAGC=0x%x, PGA=0x%x\n",
			       agc_cnt, tmp_txagc, tmp_pga);
			...
			i = _dpk_pas_gain_check_8721d(dm);
	...
	} while (!goout && !limited_pga && (agc_cnt < 6));
}
	...
	for (k = 30; k > 0; k -= 2) {
		...
		if (DPK_PULSE_DBG_8721D) {
			...
			RF_DBG(dm, DBG_RF_DPK, "[DPK] Insert PAS[%02d] = 0x%08x\n",
			       k, (dpk_info->tmp_pas_i[k] << 16) | (u16)dpk_info->tmp_pas_q[k]);
#else
	...
	if (DPK_PULSE_DBG_8721D) {
		for (k = 0; k < 32 ; k++)
			RF_DBG(dm, DBG_RF_DPK, "[DPK] S%d DPK PAS(interpolated) = 0x%x\n",
			       RF_PATH_A, (dpk_info->tmp_pas_i[k] << 16) | (u16)dpk_info->tmp_pas_q[k]);
	}
}
...
u8 _dpk_gainloss_8721d(struct dm_struct *dm)
{
	...
	_dpk_pas_agc_8721d(dm, RF_PATH_A);
...
void _dpk_set_tx_agc_8721d(void *dm_void,	u8 tx_agc_search)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	u32 reg_b68, gain;
	s32 gain_db, loop_gain;
	u8 tx_agc, txagc_ori, temp_pga;
	s8 txagc_backoff = 0, pwsf_idx = 0;

	txagc_ori = odm_get_rf_reg(dm, RF_PATH_A, RF_0x00, 0x00001f);	
	temp_pga = odm_get_rf_reg(dm, RF_PATH_A, RF_0x8f, BIT(14)|BIT(13));
	...
	if (*dm->band_type == ODM_BAND_2_4G) 
		pwsf_idx = (tx_agc - 0x19 + 0x1) & 0x1f;
	else
		pwsf_idx = (tx_agc - 0x19 + 0x2) & 0x1f;
	/*store pwsf offset[14:10]*/
	//reg_b68 = odm_get_bb_reg(dm, R_0xb68, MASKDWORD) | ((pwsf_idx & 0x1f) << 10);
	odm_set_rf_reg(dm, RF_PATH_A, RF_0x00, RFREG_MASK, (0x50000 | tx_agc));

	odm_set_bb_reg(dm, R_0xb68, 0x7c00, pwsf_idx);

	RF_DBG(dm, DBG_RF_DPK, "[DPK] pwsf_idx = 0x%x, tx_agc = 0x%x\n",
	       pwsf_idx, tx_agc);
	RF_DBG(dm, DBG_RF_DPK, "[DPK] 0xb68 = 0x%x, rf0 = 0x%x\n",
	...
	gain = _dpk_pas_get_iq_8721d(dm, GAIN_CHK);
	gain_db = 3 * _dpk_log2base(gain) - 6020;
	loop_gain = gain_db + 80 * txagc_backoff;
	RF_DBG(dm, DBG_RF_DPK, "[DPK] gain_db = %d.%02d, loopgain = %d.%02d\n",
	       gain_db / 100, -gain_db % 100,
	       loop_gain / 100, -loop_gain % 100);

	if(loop_gain < -860) {
		if (temp_pga == 0x0)
			temp_pga = 0x1;
		else if (temp_pga == 0x1 || temp_pga == 0x2)
			temp_pga = 0x3;
		odm_set_rf_reg(dm, RF_PATH_A, RF_0x8f, BIT(14)|BIT(13), temp_pga);
	}
	RF_DBG(dm, DBG_RF_DPK, "PGA_gain = 0x%x, 0x8f = 0x%x\n", temp_pga,
	       odm_get_rf_reg(dm, RF_PATH_A, RF_0x8f, 0xfffff));
}
u32 _dpd_calibrate_8721d(void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct dm_dpk_info *dpk_info = &dm->dpk_info;
	u32 result;
	u8 channel, mid_band = 0;
	s8 pwsf;
#if 0
	u32 reg_b68;
...
#endif
#if 0
	/*DPK change to normal mode for promote 5GMB TX EVM */
	channel = (u8)odm_get_rf_reg(dm, RF_PATH_A, RF_0x18, 0x000ff);
	pwsf = (u8)odm_get_bb_reg(dm, R_0xb68, 0x7c00) - 1;
	if(channel > 64 && channel < 150) {
		mid_band = 1;
		odm_set_bb_reg(dm, R_0xb68, 0x7c00, pwsf & 0x1f);
	}
	/*DPK change to normal mode at 5GMB*/
#endif
	if(dpk_info->is_dpk_by_channel && (!mid_band)) {
...
void _dpk_calibrate_by_group_8721d(struct dm_struct *dm,	u8 path)
{
	...
	for (group = 0; group < DPK_GROUP_NUM_8721D; group++) {
		...
		for (retry_cnt = 0; retry_cnt < 4; retry_cnt++) {
			...
			ODM_delay_ms(1);
			_dpk_set_tx_agc_8721d(dm, tx_agc_search);

			dpk_fail = _dpd_calibrate_8721d(dm);
...
void _dpk_calibrate_by_channel_8721d(struct dm_struct *dm,	u8 path)
{
	...
	for (retry_cnt = 0; retry_cnt < 2; retry_cnt++) {
		...
		RF_DBG(dm, DBG_RF_DPK,
		       ...
		       path, channel);
		if(retry_cnt > 0) {
			last_txagc = odm_get_rf_reg(dm, (enum rf_path)path,
						   RF_0x00, 0x0001f) + 1;
			odm_set_rf_reg(dm, RF_PATH_A, 0x00, 0x0001f, last_txagc);
		}
...
{
#ifdef CONFIG_FT
	printf("<======FT TEST bypass DPK track======>\n");
	return;
#endif

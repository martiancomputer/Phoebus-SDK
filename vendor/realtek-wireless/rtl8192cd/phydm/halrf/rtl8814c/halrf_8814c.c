...
void halrf_rf_lna_setting_8814c(struct dm_struct *dm_void,
...
#if 0
boolean get_mix_mode_tx_agc_bb_swing_offset_8814c(void *dm_void,
...
u8 _halrf_tssi_rate_to_driver_rate_8814c(
...
u8 _halrf_driver_rate_to_tssi_rate_8814c(
	void *dm_void, u8 rate)
{
	u8 tssi_rate = 0;
	struct dm_struct *dm = (struct dm_struct *)dm_void;
...
void _backup_bb_registers_8814c(
	void *dm_void,
	u32 *reg,
	u32 *reg_backup,
	u32 reg_num)
...
void _reload_bb_registers_8814c(
	void *dm_void,
	u32 *reg,
	u32 *reg_backup,
	u32 reg_num)
	...
	u32 bb_reg_num)
{
	...
	for (i = 0; i < mac_reg_num /*MAC_REG_NUM_8814C*/; i++){
		mac_backup[i] = odm_read_4byte(dm, backup_mac_reg[i]);
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "[TSSI]Backup mac addr = %x, value =% x\n", backup_mac_reg[i], mac_backup[i]);
	}
	for (i = 0; i < bb_reg_num /*BB_REG_NUM_8814C*/; i++){
		bb_backup[i] = odm_read_4byte(dm, backup_bb_reg[i]);		
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "[TSSI]Backup bbaddr = %x, value =% x\n", backup_bb_reg[i], bb_backup[i]);
	}
	return;
}

void _restore_mac_bb_8814c(
	struct dm_struct *dm,
	u32 *mac_backup,
	u32 *bb_backup,
	u32 *backup_mac_reg,
	...
	u32 bb_reg_num)
{
	...
	for (i = 0; i < mac_reg_num /* MAC_REG_NUM_8814C */; i++) {
		...
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "[TSSI]restore mac = %x, value = %x\n",backup_mac_reg[i],mac_backup[i]);
	}
	for (i = 0; i < bb_reg_num /* BB_REG_NUM_8814C */ ; i++) {
		odm_set_bb_reg(dm, backup_bb_reg[i], MASKDWORD, bb_backup[i]);		
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "[TSSI]restore bb = %x, value = %x\n",backup_bb_reg[i],bb_backup[i]);
	}	

}

void _backup_rf_registers_8814c(
	void *dm_void,
	u8 e_rf_path,
	u32 *reg,
	u32 *reg_backup,
...
void _reload_rf_registers_8814c(
	void *dm_void,
	u8 e_rf_path,
	u32 *reg,
	u32 *reg_backup,
...
void _backup_syn_registers_8814c(
	void *dm_void,
...
void _reload_syn_registers_8814c(
	void *dm_void,
	enum rf_syn syn_path,
	u32 *reg,
	u32 *reg_backup,
...
u32 _halrf_get_efuse_tssi_offset_8814c(
...
#if 1
void _halrf_parse_tssi_diff_from_efuse_8814c(
	void *dm_void,
	u16 address,
	u8 path,
	u16 tssi_diff_idx,
	u8 high_low_bit)
{
	...
	odm_efuse_logical_map_read(dm, 1, address, &pg);
	pg = pg & 0xff;
	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		"======>%s   efuse[0x%x]=0x%02x\n", __func__, address, pg);
	...
	if (high_low_bit == 2) {
		pg_tmp = pg & 0xf0;
		if (pg_tmp & 0x80)
			pg_tmp = (pg_tmp >> 4) | 0xfffffff0;
		else
			pg_tmp = pg_tmp >> 4;
	
		tssi->tssi_de_diff_efuse[path][tssi_diff_idx] = (s8)(pg_tmp & 0xff);
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"tssi->tssi_efuse_diff[%d][%d]=0x%x\n", path, tssi_diff_idx, tssi->tssi_de_diff_efuse[path][tssi_diff_idx]);
		tssi_diff_idx++;
	}

	pg_tmp = pg & 0xf;
	if (pg_tmp & 0x8)
		pg_tmp = pg_tmp | 0xfffffff0;
...
s8 _halrf_parse_tssi_diff_from_flash_8814c(
	void *dm_void,
	u8 diff_value,
	u8 high_low)
{
	...
	diff_value = diff_value & 0xff;
	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		"======>%s   0x%02x\n", __func__, diff_value);

	if (high_low) {
		diff_value_tmp = diff_value & 0xf0;
		if (diff_value_tmp & 0x80)
			diff_value_tmp = (diff_value_tmp >> 4) | 0xfffffff0;
		else
			diff_value_tmp = diff_value_tmp >> 4;

		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"high bit diff_value_tmp=0x%x\n", diff_value_tmp);
	} else {
		diff_value_tmp = diff_value & 0xf;
		if (diff_value_tmp & 0x8)
			diff_value_tmp = diff_value_tmp | 0xfffffff0;

		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"low bit diff_value_tmp=0x%x\n", diff_value_tmp);
	}

	return (s8)diff_value_tmp;
}
#endif

s32 _halrf_get_efuse_tssi_de_diff_offset_8814c(
	void *dm_void, u8 path)
{
	...
	if (channel >= 1 && channel <= 14 && band_width == CHANNEL_WIDTH_20)
		offset_index = 0;
	else if (channel >= 36 && channel <= 64 && band_width == CHANNEL_WIDTH_20)
		offset_index = 1;
	else if (channel >= 36 && channel <= 64 && band_width == CHANNEL_WIDTH_80)
		offset_index = 2;
	else if (channel >= 36 && channel <= 64 && band_width == CHANNEL_WIDTH_80_80)
		offset_index = 3;
	else if (channel >= 100 && channel <= 144 && band_width == CHANNEL_WIDTH_20)
		offset_index = 4;
	else if (channel >= 100 && channel <= 144 && band_width == CHANNEL_WIDTH_80)
		offset_index = 5;
	else if (channel >= 100 && channel <= 144 && band_width == CHANNEL_WIDTH_80_80)
		offset_index = 6;
	else if (channel >= 149 && channel <= 177 && band_width == CHANNEL_WIDTH_20)
		offset_index = 7;
	else if (channel >= 149 && channel <= 177 && band_width == CHANNEL_WIDTH_80)
		offset_index = 8;
	else if (channel >= 149 && channel <= 177 && band_width == CHANNEL_WIDTH_80_80)
		offset_index = 9;
	else
		return 0;

	offset = tssi->tssi_de_diff_efuse[path][offset_index];
...
s32 _halrf_get_efuse_tssi_de_diff_offset_8814c_bybw(
	void *dm_void, u8 path, u8 band_width)
{
...
void _halrf_driver_flash_to_tssi_de_8814c(
	void *dm_void)
{
#if (DM_ODM_SUPPORT_TYPE & ODM_AP)
	struct dm_struct *dm = (struct dm_struct *)dm_void;
...
void _halrf_driver_flash_to_tssi_de_diff_8814c(
	void *dm_void)
{
#if 1
#if (DM_ODM_SUPPORT_TYPE & ODM_AP)
	...
	/*AP 2G 20M CH7*/
	tssi->tssi_de_diff_efuse[RF_PATH_A][0] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_20BW1S_OFDM1T_A[6], high);
	tssi->tssi_de_diff_efuse[RF_PATH_B][0] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_20BW1S_OFDM1T_B[6], high);
	tssi->tssi_de_diff_efuse[RF_PATH_C][0] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_20BW1S_OFDM1T_C[6], high);
	tssi->tssi_de_diff_efuse[RF_PATH_D][0] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_20BW1S_OFDM1T_D[6], high);

	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		"priv->pmib->dot11RFEntry.pwrdiff_20BW1S_OFDM1T[6]=0x%x 0x%x 0x%x 0x%x\n",
		priv->pmib->dot11RFEntry.pwrdiff_20BW1S_OFDM1T_A[6],
		priv->pmib->dot11RFEntry.pwrdiff_20BW1S_OFDM1T_B[6],
		priv->pmib->dot11RFEntry.pwrdiff_20BW1S_OFDM1T_C[6],
		priv->pmib->dot11RFEntry.pwrdiff_20BW1S_OFDM1T_D[6]);

	for (i = 0; i < MAX_PATH_NUM_8814C; i++)
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"AP 2G 20M CH7 TSSI DE diff tssi->tssi_de_diff_efuse[%d][0]=0x%x\n", i, tssi->tssi_de_diff_efuse[i][0]);

	/*AP 5G 20M CH36*/
	tssi->tssi_de_diff_efuse[RF_PATH_A][1] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_A[35], high);
	tssi->tssi_de_diff_efuse[RF_PATH_B][1] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_B[35], high);
	tssi->tssi_de_diff_efuse[RF_PATH_C][1] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_C[35], high);
	tssi->tssi_de_diff_efuse[RF_PATH_D][1] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_D[35], high);

	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		"priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T[35]=0x%x 0x%x 0x%x 0x%x\n",
		priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_A[35],
		priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_B[35],
		priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_C[35],
		priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_D[35]);

	for (i = 0; i < MAX_PATH_NUM_8814C; i++)
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"AP 5G 20M CH36 TSSI DE diff tssi->tssi_de_diff_efuse[%d][1]=0x%x\n", i, tssi->tssi_de_diff_efuse[i][1]);

	/*AP 5G 80M CH36*/
	tssi->tssi_de_diff_efuse[RF_PATH_A][2] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_A[35], high);
	tssi->tssi_de_diff_efuse[RF_PATH_B][2] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_B[35], high);
	tssi->tssi_de_diff_efuse[RF_PATH_C][2] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_C[35], high);
	tssi->tssi_de_diff_efuse[RF_PATH_D][2] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_D[35], high);

	/*AP 5G 160M CH36*/
	tssi->tssi_de_diff_efuse[RF_PATH_A][3] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_A[35], low);
	tssi->tssi_de_diff_efuse[RF_PATH_B][3] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_B[35], low);
	tssi->tssi_de_diff_efuse[RF_PATH_C][3] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_C[35], low);
	tssi->tssi_de_diff_efuse[RF_PATH_D][3] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_D[35], low);

	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		"priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S[35]=0x%x 0x%x 0x%x 0x%x\n",
		priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_A[35],
		priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_B[35],
		priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_C[35],
		priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_D[35]);

	for (i = 0; i < MAX_PATH_NUM_8814C; i++) {
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"AP 5G 80M CH36 TSSI DE diff tssi->tssi_de_diff_efuse[%d][2]=0x%x\n", i, tssi->tssi_de_diff_efuse[i][2]);
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"AP 5G 160M CH36 TSSI DE diff tssi->tssi_de_diff_efuse[%d][3]=0x%x\n", i, tssi->tssi_de_diff_efuse[i][3]);
	}

	/*AP 5G 20M CH100*/
	tssi->tssi_de_diff_efuse[RF_PATH_A][4] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_A[99], high);
	tssi->tssi_de_diff_efuse[RF_PATH_B][4] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_B[99], high);
	tssi->tssi_de_diff_efuse[RF_PATH_C][4] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_C[99], high);
	tssi->tssi_de_diff_efuse[RF_PATH_D][4] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_D[99], high);

	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		"priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T[99]=0x%x 0x%x 0x%x 0x%x\n",
		priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_A[99],
		priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_B[99],
		priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_C[99],
		priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_D[99]);

	for (i = 0; i < MAX_PATH_NUM_8814C; i++)
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"AP 5G 20M CH100 TSSI DE diff tssi->tssi_de_diff_efuse[%d][4]=0x%x\n", i, tssi->tssi_de_diff_efuse[i][4]);

	/*AP 5G 80M CH100*/
	tssi->tssi_de_diff_efuse[RF_PATH_A][5] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_A[99], high);
	tssi->tssi_de_diff_efuse[RF_PATH_B][5] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_B[99], high);
	tssi->tssi_de_diff_efuse[RF_PATH_C][5] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_C[99], high);
	tssi->tssi_de_diff_efuse[RF_PATH_D][5] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_D[99], high);

	/*AP 5G 160M CH100*/
	tssi->tssi_de_diff_efuse[RF_PATH_A][6] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_A[99], low);
	tssi->tssi_de_diff_efuse[RF_PATH_B][6] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_B[99], low);
	tssi->tssi_de_diff_efuse[RF_PATH_C][6] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_C[99], low);
	tssi->tssi_de_diff_efuse[RF_PATH_D][6] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_D[99], low);

	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		"priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S[99]=0x%x 0x%x 0x%x 0x%x\n",
		priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_A[99],
		priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_B[99],
		priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_C[99],
		priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_D[99]);
	
	for (i = 0; i < MAX_PATH_NUM_8814C; i++) {
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"AP 5G 80M CH100 TSSI DE diff tssi->tssi_de_diff_efuse[%d][5]=0x%x\n", i, tssi->tssi_de_diff_efuse[i][5]);

		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"AP 5G 160M CH100 TSSI DE diff tssi->tssi_de_diff_efuse[%d][6]=0x%x\n", i, tssi->tssi_de_diff_efuse[i][6]);
	}

	/*AP 5G 20M CH149*/
	tssi->tssi_de_diff_efuse[RF_PATH_A][7] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_A[148], high);
	tssi->tssi_de_diff_efuse[RF_PATH_B][7] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_B[148], high);
	tssi->tssi_de_diff_efuse[RF_PATH_C][7] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_C[148], high);
	tssi->tssi_de_diff_efuse[RF_PATH_D][7] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_D[148], high);

	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		"priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T[148]=0x%x 0x%x 0x%x 0x%x\n",
		priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_A[148],
		priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_B[148],
		priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_C[148],
		priv->pmib->dot11RFEntry.pwrdiff_5G_20BW1S_OFDM1T_D[148]);

	for (i = 0; i < MAX_PATH_NUM_8814C; i++)
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"AP 5G 20M CH149 TSSI DE diff tssi->tssi_de_diff_efuse[%d][7]=0x%x\n", i, tssi->tssi_de_diff_efuse[i][7]);

	/*AP 5G 80M CH149*/
	tssi->tssi_de_diff_efuse[RF_PATH_A][8] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_A[148], high);
	tssi->tssi_de_diff_efuse[RF_PATH_B][8] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_B[148], high);
	tssi->tssi_de_diff_efuse[RF_PATH_C][8] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_C[148], high);
	tssi->tssi_de_diff_efuse[RF_PATH_D][8] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_D[148], high);

	/*AP 5G 160M CH149*/
	tssi->tssi_de_diff_efuse[RF_PATH_A][9] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_A[148], low);
	tssi->tssi_de_diff_efuse[RF_PATH_B][9] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_B[148], low);
	tssi->tssi_de_diff_efuse[RF_PATH_C][9] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_C[148], low);
	tssi->tssi_de_diff_efuse[RF_PATH_D][9] = _halrf_parse_tssi_diff_from_flash_8814c(dm, priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_D[148], low);

	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		"priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S[148]=0x%x 0x%x 0x%x 0x%x\n",
		priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_A[148],
		priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_B[148],
		priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_C[148],
		priv->pmib->dot11RFEntry.pwrdiff_5G_80BW1S_160BW1S_D[148]);
	
	for (i = 0; i < MAX_PATH_NUM_8814C; i++) {
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"AP 5G 80M CH149 TSSI DE diff tssi->tssi_de_diff_efuse[%d][8]=0x%x\n", i, tssi->tssi_de_diff_efuse[i][8]);

		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"AP 5G 160M CH149 TSSI DE diff tssi->tssi_de_diff_efuse[%d][9]=0x%x\n", i, tssi->tssi_de_diff_efuse[i][9]);
	}
#endif
#endif
...
void _halrf_tssi_set_de_by_thermal_8814c(void *dm_void, u8 rf_path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct dm_rf_calibration_struct *cali_info = &(dm->rf_calibrate_info);
	struct _hal_rf_ *rf = &dm->rf_table;
	...
	*/
	u32 addr_d[4][MAX_PATH_NUM_8814C] = { {0x18a8, 0x1eec, 0x1ef4, 0x1efc},
						 {0x18f4, 0x41f4, 0x52f4, 0x53f4},
						 {0x18f4, 0x41f4, 0x52f4, 0x53f4}, 
						 {0x18f4, 0x41f4, 0x52f4, 0x53f4}	};

	u32 addr_d_bitmask[4][MAX_PATH_NUM_8814C] = { {0xff000000, 0x3fc00000, 0x3fc00000, 0x3fc00000},
	...
	*/
	u32 addr_cck_d[2][MAX_PATH_NUM_8814C] = { {0x18e8, 0x41e8, 0x52e8, 0x53e8},
							 				  {0x18f8, 0x41f8, 0x52f8, 0x53f8} };
	
	u32 addr_cck_d_bitmask[2][MAX_PATH_NUM_8814C] = { {0x01fe0000, 0x01fe0000, 0x01fe0000, 0x01fe0000},
							 		 				  {0x00ff, 0x00ff, 0x00ff, 0x00ff} };
	...
	RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "======>%s, BW = %d ", __func__, band_width); /*@20M/40M/80M = 0/1/2*/
	...
	if (dm->rf_calibrate_info.txpowertrack_control == 4) {
		...
		for (i = 0; i < MAX_PATH_NUM_8814C; i++) {
			...
			#else
			for (b = 0; b<2; b++)
				odm_set_bb_reg(dm, addr_cck_d[b][i], addr_cck_d_bitmask[b][i], 0x0);

			for (b = 0; b<4; b++)
				odm_set_bb_reg(dm, addr_d[b][i], addr_d_bitmask[b][i], 0x0);
			#endif
		}
		return;
	}
...
#endif
	/* CCK */
	tssi_de_tmp = tssi->tssi_efuse[rf_path][offset_index] +
			phydm_get_tssi_trim_de(dm, rf_path) +
			cali_info->absolute_ofdm_swing_idx[rf_path];

	if (tssi_de_tmp > 127)
		tssi_de_tmp = 127;
	else if (tssi_de_tmp < -128)
		tssi_de_tmp = -128;

	for ( b = 0; b < 2; b++) {
		odm_set_bb_reg(dm, addr_cck_d[b][rf_path], addr_cck_d_bitmask[b][rf_path],
			(tssi_de_tmp & 0xff));
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"CCK 0x%x[%x] tssi_de_tmp(%d) = tssi->tssi_efuse[%d][%d](%d) + phydm_get_tssi_trim_de(dm, %d)(%d) + _halrf_get_efuse_tssi_de_diff_offset_8814c(dm, %d)(%d) + cali_info->absolute_ofdm_swing_idx[%d](%d)\n",
			addr_cck_d[b][rf_path], addr_cck_d_bitmask[b][rf_path], tssi_de_tmp, 
			rf_path, offset_index, tssi->tssi_efuse[rf_path][offset_index],
			rf_path, phydm_get_tssi_trim_de(dm, rf_path),
			rf_path, 0,
			rf_path, cali_info->absolute_ofdm_swing_idx[rf_path]);
	}

	tssi_offest_de = (u32)_halrf_get_efuse_tssi_offset_8814c(dm, rf_path);


	for ( b = 0; b<4; b++) {
		
		tssi_de_tmp = tssi_offest_de + phydm_get_tssi_trim_de(dm, rf_path)
			+ _halrf_get_efuse_tssi_de_diff_offset_8814c_bybw(dm, rf_path, b)
			+ cali_info->absolute_ofdm_swing_idx[rf_path];
		...
		odm_set_bb_reg(dm, addr_d[b][rf_path], addr_d_bitmask[b][rf_path], (tssi_de_tmp & 0xff));
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"OFDM 0x%x[%x] tssi_de_tmp(%d) = tssi_offest_de(%d) + phydm_get_tssi_trim_de(dm, %d)(%d) + _halrf_get_efuse_tssi_de_diff_offset_8814c(dm, %d)(%d) + cali_info->absolute_ofdm_swing_idx[%d](%d)\n",
			addr_d[b][rf_path], addr_d_bitmask[b][rf_path], tssi_de_tmp,
			tssi_offest_de,
			rf_path, phydm_get_tssi_trim_de(dm, rf_path),
			rf_path, _halrf_get_efuse_tssi_de_diff_offset_8814c_bybw(dm, rf_path, b),
			rf_path, cali_info->absolute_ofdm_swing_idx[rf_path]);
	}
	...
	u8 rf_path)
{
	...
	/* CCK */
	/*
	u32 big_a_cck_reg[4] = {0x1880, 0x4180, 0x5280, 0x5380};
	u32 big_a_cck_bit_mask[4] = {0x1fff, 0x1fff, 0x1fff, 0x1fff};
	*/
...
u8 halrf_get_tssi_result_8814c(
	void *dm_void,
	u8 rf_path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
...
void _halrf_get_tssi_normal_driver_result_8814c(
	void *dm_void,
	u8 rf_path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	...
	*/
	...
	txagc_tmp = tssi->tssi_trk_txagc_offset[rf_path];

	tssi->tssi_trk_txagc_offset[rf_path] = tssi->tssi_trk_txagc_offset[rf_path] +
		tssi->delta_tssi_txagc_offset[rf_path];
		
	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		"tssi_trk_txagc_offset[%d](%d) = tssi_trk_txagc_offset[%d](%d) + delta_tssi_txagc_offset[%d](%d)\n",
		rf_path, tssi->tssi_trk_txagc_offset[rf_path], rf_path, txagc_tmp,
		rf_path, tssi->delta_tssi_txagc_offset[rf_path]);
	...
	u8 rf_path)
{
	...
	*/
	...
	odm_set_bb_reg(dm, tssi_trigger[rf_path], 0x10000000, 0x0);
	for (i = 0; i < 300; i++)
		ODM_delay_us(1); /* add for fix tssi on*/
	odm_set_bb_reg(dm, tssi_trigger[rf_path], 0x10000000, 0x1);
	...
	for (i = 0; odm_get_bb_reg(dm, tssi_result[rf_path], 0x10000) == 0; i++) {
		...
		if (i >= 300) {
			...
			/*_reload_bb_registers_8814c(dm, bb_reg, bb_reg_backup, 1);*/
			rf->is_tssi_in_progress = 0;
			return 1;
		}
	}
	...
	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		"tssi_trk_txagc_offset[%d](%d) = tssi_trk_txagc_offset[%d](%d) + s_delta_tssi_code(%d)\n",
		rf_path, tssi->tssi_trk_txagc_offset[rf_path], rf_path, txagc_tmp,
		s_delta_tssi_code);
	...
	u8 rf_path)
{
	...
	if (!tssi->thermal_trigger) {
		halrf_trigger_tssi_8814c(dm, rf_path);
	} else {
		_halrf_get_tssi_normal_driver_result_8814c(dm, rf_path);
	}
...
void halrf_calculate_tssi_codeword_8814c(
	void *dm_void,
	u8 rf_path)
{
	...
	*/
	...
	for (i = 0; i < TSSI_CODE_NUM; i++) {
		rate = _halrf_tssi_rate_to_driver_rate_8814c(dm, i);
...
void halrf_set_tssi_codeword_8814c(
...
u8 halrf_get_tssi_codeword_8814c(
	void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct _hal_rf_ *rf = &dm->rf_table;
	...
	tssi_rate = _halrf_driver_rate_to_tssi_rate_8814c(dm, ODM_MGN_MCS7);
...
u32 halrf_query_tssi_value_8814c(
	void *dm_void)
{
	s32 tssi_codeword = 0;
	...
	tssi_rate = _halrf_driver_rate_to_tssi_rate_8814c(dm, rate);
...
u32 halrf_set_tssi_value_8814c(
...
void halrf_tssi_get_efuse_8814c(
	void *dm_void)
{
#if (DM_ODM_SUPPORT_TYPE & ODM_AP)
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	...
	_halrf_driver_flash_to_tssi_de_8814c(dm);
	_halrf_driver_flash_to_tssi_de_diff_8814c(dm);
	...
	j = 0;
	for (i = 0x10; i <= 0x1a; i++) {
		odm_efuse_logical_map_read(dm, 1, i, &pg);
		tssi->tssi_efuse[0][j] = (s8)pg;
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
	...
	/*TSSI DE diff path s0*/
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x1c, RF_PATH_A, 0, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x31, RF_PATH_A, 1, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x32, RF_PATH_A, 2, 2);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x33, RF_PATH_A, 4, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x34, RF_PATH_A, 5, 2);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x35, RF_PATH_A, 7, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x36, RF_PATH_A, 8, 2);

	/*path s1*/
	j = 0;
	for (i = 0x3a; i <= 0x44; i++) {
		odm_efuse_logical_map_read(dm, 1, i, &pg);
		tssi->tssi_efuse[1][j] = (s8)pg;
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
	...
	/*TSSI DE diff path s1*/
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x46, RF_PATH_B, 0, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x5b, RF_PATH_B, 1, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x5c, RF_PATH_B, 2, 2);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x5d, RF_PATH_B, 4, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x5e, RF_PATH_B, 5, 2);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x5f, RF_PATH_B, 7, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x60, RF_PATH_B, 8, 2);

	/*path s2*/
	j = 0;
	for (i = 0x64; i <= 0x6e; i++) {
		odm_efuse_logical_map_read(dm, 1, i, &pg);
		tssi->tssi_efuse[2][j] = (s8)pg;
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
	...
	/*TSSI DE diff path s2*/
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x70, RF_PATH_C, 0, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x85, RF_PATH_C, 1, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x86, RF_PATH_C, 2, 2);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x87, RF_PATH_C, 4, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x88, RF_PATH_C, 5, 2);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x89, RF_PATH_C, 7, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x8a, RF_PATH_C, 8, 2);

	/*path s3*/
	j = 0;
	for (i = 0x8e; i <= 0x98; i++) {
		odm_efuse_logical_map_read(dm, 1, i, &pg);
		tssi->tssi_efuse[3][j] = (s8)pg;
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
	...
	/*TSSI DE diff path s3*/
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0x9a, RF_PATH_D, 0, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0xaf, RF_PATH_D, 1, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0xb0, RF_PATH_D, 2, 2);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0xb1, RF_PATH_D, 4, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0xb2, RF_PATH_D, 5, 2);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0xb3, RF_PATH_D, 7, 1);
	_halrf_parse_tssi_diff_from_efuse_8814c(dm, 0xb4, RF_PATH_D, 8, 2);
#endif

}

void halrf_get_efuse_thermal_pwrtype_8814c(
	void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct dm_rf_calibration_struct *cali_info = &(dm->rf_calibrate_info);
	...
	void *dm_void)
{
	...
	*/
	...
	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
				"======>%s channel=%d   band_width=%d\n",
				__func__, channel, band_width);
	...
	if (dm->rf_calibrate_info.txpowertrack_control == 4) {
...
#else
		for (i = 0; i < MAX_PATH_NUM_8814C; i++) {
			odm_set_bb_reg(dm, txagc_offset[i], 0x7f,
				(cali_info->default_txagc_index & 0x7f));
		}
		...
		for (i = 0; i < MAX_PATH_NUM_8814C; i++) {
			...
			for (b = 0; b < 2; b++) {
				odm_set_bb_reg(dm, addr_cck_d[b][i], addr_cck_d_bitmask[b][i],
					(tmp & 0xff));

				RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
					"==>%s CCK 0x%x[%x] tssi_trim(%d)\n",
					__func__,
					addr_cck_d[b][i], addr_cck_d_bitmask[b][i],
					(tmp & 0xff)
					);
			}
			...
			tmp = phydm_get_tssi_trim_de(dm, i);
			for (b = 0; b < 4; b++) {
				odm_set_bb_reg(dm, addr_d[b][i], addr_d_bitmask[b][i], (tmp & 0xff));

				RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
					"==>%s 0x%x[%x] tssi_offset(%d)\n",
					__func__,
					addr_d[b][i], addr_d_bitmask[b][i],
					(tmp & 0xff)
					);
			}
	...
	for (i = 0; i < MAX_PATH_NUM_8814C; i++) {
		...
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"==>%s CCK 0x%x[%x] tssi_offset(%d)=tssi_efuse(%d)+tssi_trim(%d)+tssi_diff(%d) path=%d\n",
			__func__,
		...
		tssi_offest_de = (s8)_halrf_get_efuse_tssi_offset_8814c(dm, i);

		#if 0
		tmp = tssi_offest_de + phydm_get_tssi_trim_de(dm, i)
			+ _halrf_get_efuse_tssi_de_diff_offset_8814c(dm, i);
		...
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
			"==>%s 0x%x[%x] tssi_offset(%d)=tssi_efuse(%d)+tssi_trim(%d)+tssi_diff(%d) path=%d\n",
			__func__,
			addr_d[i], addr_d_bitmask[i],
			(s8)(tmp & 0xff),
			tssi_offest_de,
			phydm_get_tssi_trim_de(dm, i),
			_halrf_get_efuse_tssi_de_diff_offset_8814c(dm, i), i);
		#else
		for (b = 0; b < 4; b++) {
			tmp = tssi_offest_de + phydm_get_tssi_trim_de(dm, i)
				+ _halrf_get_efuse_tssi_de_diff_offset_8814c_bybw(dm, i, b);
			...
			RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
				...
				tssi_offest_de,
				phydm_get_tssi_trim_de(dm, i),
				_halrf_get_efuse_tssi_de_diff_offset_8814c_bybw(dm, i, b), i);
		}		
		#endif
	}
}
	...
	void *dm_void, u32 tssi_de, u8 path)
{
	...
	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		"==>%s tssi_de(%d) tssi_de_tmp(%d), BW = %d\n",
		__func__,
		tssi_de, tssi_de_tmp, band_width);
	...
	odm_set_bb_reg(dm, addr_cck_d[band_width][path], addr_cck_d_bitmask[band_width][path],
		(tmp & 0xff));
	
	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		"==>%s CCK 0x%x[%x] tssi_offset(%d)=tssi_de_tmp(%d)+tssi_trim(%d)\n",
		__func__,
		addr_cck_d[band_width][path], addr_cck_d_bitmask[band_width][path],
		(tmp & 0xff),
		tssi_de_tmp,
	...
	odm_set_bb_reg(dm, addr_d[band_width][path], addr_d_bitmask[band_width][path], (tmp & 0xff));

	RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
		"==>%s 0x%x[%x] tssi_offset(%d)=tssi_de_tmp(%d)+tssi_trim(%d)\n",
		__func__,
		addr_d[band_width][path], addr_d_bitmask[band_width][path],
		(tmp & 0xff),
		tssi_de_tmp,
		phydm_get_tssi_trim_de(dm, path));
#endif
...
void halrf_tssi_clean_de_8814c(
	void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct _hal_rf_ *rf = &dm->rf_table;
...
u32 halrf_tssi_trigger_de_8814c(
	void *dm_void, u8 path)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct _hal_rf_ *rf = &dm->rf_table;
	...
	void *dm_void, u8 direct_do)
{
	...
	u32 backup_num = 12;
#else
	u32 bb_reg[21] = {R_0x1c3c, R_0x2dbc, R_0x820, R_0x1e2c, R_0x1e28,
			R_0x1800, R_0x4100, R_0x5200, R_0x5300, R_0x1b00,
			R_0x1bcc, R_0x1d08, R_0x900, R_0x908, R_0x90c,
			R_0x910, R_0x914, R_0x938, R_0x940, R_0xa58,
			R_0x1e70};
	u32 bb_reg_backup[21] = {0};
	u32 bb_backup_num = 21;

	
	u32 mac_reg[1] = {0x520};
	u32 mac_reg_backup[1];
	u32 mac_backup_num = 1;
#endif
	...
	u32 dck_cck_offset_mask = 0x7fc00000; /* alll paths are the same */
	...
	/* _backup_bb_registers_8814c(dm, bb_reg, bb_reg_backup, backup_num); */

	_backup_mac_bb_8814c(dm, mac_reg_backup, bb_reg_backup, mac_reg, bb_reg, mac_backup_num, bb_backup_num);

	/* MACBB register setting */
	odm_write_1byte(dm, REG_TXPAUSE, 0xff);

	RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "[TSSI][DCK] Set REG_TXPAUSE!!!\n");
	...
	for (i = 0; i < MAX_PATH_NUM_8814C; i++) {
		...
		{
			status_check = odm_get_rf_reg(dm, rf_path, 0x00, 0x30000); /* bit 17-16 */

			RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "[TSSI][DCK] check rf0 status = %d \n",
					status_check);
	...
	for (i = RF_PATH_A; i < MAX_PATH_NUM_8814C; i++) {
		...
		if (channel >= 1 && channel <= 14) {
			...
			for (k = 0; k < retry_time; k++) {
				...
				odm_set_bb_reg(dm, dck_cck_offset[i], dck_cck_offset_mask, tmp);
	...
	/* _reload_bb_registers_8814c(dm, bb_reg, bb_reg_backup, backup_num); */

	_restore_mac_bb_8814c(dm, mac_reg_backup, bb_reg_backup, mac_reg, bb_reg, mac_backup_num, bb_backup_num);
...
void odm_pwrtrack_method_set_pwr8814c(void *dm_void,
				      enum pwrtrack_method method,
				      u8 rf_path, u8 tx_pwr_idx_offset)
{
	...
	if (method == CLEAN_MODE) { /*use for mp driver clean power tracking status*/
		...
		RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "offset(%d) = cali_info->default_txagc_index(%d) + cali_info->absolute_ofdm_swing_idx[rf_path](%d)\n",
		       txagc_tmp, cali_info->default_txagc_index, cali_info->absolute_ofdm_swing_idx[rf_path]);
		...
		halrf_do_get_tssi_8814c(dm, rf_path);
		_halrf_tssi_set_de_by_thermal_8814c(dm, rf_path);
	}
}
				   ...
				   u8 rf_path, u8 channel_mapped_index)
{
	...
	odm_pwrtrack_method_set_pwr8814c(dm, method, rf_path,
					 tx_pwr_idx_offset);
}
...
void get_delta_swing_table_8814c_path_cd(void *dm_void,
				 u8 **temperature_up_c,
...
void halrf_dack_setbb_8814c(struct dm_struct *dm)
{
	/*BB setting*/
...
u32 halrf_pathsel1_8814c(struct dm_struct *dm, u8 path)
{
	switch (path) {
	case 0:
		return 0x1800;
...
u32 halrf_pathsel2_8814c(struct dm_struct *dm, u8 path)
{
	switch (path) {
	case 0:
		return 0x2800;
...
void halrf_dack_check_reset_8814c(struct dm_struct *dm, u8 path)
{
	...
	p = halrf_pathsel1_8814c(dm, path);
	pr = halrf_pathsel2_8814c(dm, path);
	if (odm_get_bb_reg(dm, pr + 0x18, 0xffffff80) != 0x0 ||
	    odm_get_bb_reg(dm, pr + 0x1c, 0x1f) != 0x0 ||
	    odm_get_bb_reg(dm, pr + 0x44, 0xffffff80) != 0x0 ||
	    odm_get_bb_reg(dm, pr + 0x48, 0x1f) != 0x0) {
	    	RF_DBG(dm, DBG_RF_DACK, "[DACK]S%d DACK reset!!!!!",path);
	    	odm_set_bb_reg(dm, p + 0xb8, BIT(26) | BIT(25), 0x0);
	    	odm_set_bb_reg(dm, p + 0xd4, BIT(26) | BIT(25), 0x0);

	    	odm_set_bb_reg(dm, p + 0xb0, BIT(0), 0x0);
		ODM_delay_us(10);
		odm_set_bb_reg(dm, p + 0xb0, BIT(0), 0x1);
	    	odm_set_bb_reg(dm, p + 0xcc, BIT(0), 0x0);
		ODM_delay_us(10);
		odm_set_bb_reg(dm, p + 0xcc, BIT(0), 0x1);
	}
}


void halrf_dack1_8814c(struct dm_struct *dm, u8 path, u32 *iadc, u32 *qadc,
	              u32 *v1, u32 *v2)
{
	u8 i;
...
void halrf_dack2_8814c(struct dm_struct *dm, u8 path, u32 *idac, u32 *qdac,
		       u32 v1, u32 v2)
{
	u8 i, c;
	u32 p, pr, idc, qdc;
...
void halrf_dack_reload_8814c(struct dm_struct *dm)
{
	u8 i;
	u32 p;
	
	for (i = 0; i < 4; i++) {
		p = halrf_pathsel1_8814c(dm, i);
//		odm_write_4byte(dm, p + 0x10, 0x0005c304);
...
void halrf_do_rxbb_dck_8814c(void *dm_void)
{
	...
	RF_DBG(dm, DBG_RF_RFK, "[RXDCK]RXBB DCK start!!!!!!!\n");
	for (path = 0; path < 4; path++) {
		
		bk_rf0 = odm_get_rf_reg(dm, (enum rf_path)path, 0x00, 0xfffff);
		RF_DBG(dm, DBG_RF_IQK, "[DACK]bk rf0 = %x\n", odm_get_rf_reg(dm, (enum rf_path)path, 0x00, 0xfffff));
		odm_set_rf_reg(dm, (enum rf_path)path, 0x00, 0xfffff, 0x30000);
		RF_DBG(dm, DBG_RF_IQK, "[DACK]set rf0 = %x\n", odm_get_rf_reg(dm, (enum rf_path)path, 0x00, 0xfffff));
	...
	odm_set_bb_reg(dm, 0x1d70, 0xffffffff, 0x22222222);
	odm_set_bb_reg(dm, 0x1d70, 0xffffffff, 0x20202020);
	odm_set_rf_reg(dm, (enum rf_path)path, 0x00, 0xfffff, bk_rf0);
	RF_DBG(dm, DBG_RF_IQK, "[RXDCK]RXBB DCK end!!!!!!!\n");
...
void halrf_dac_cal_8814c(void *dm_void)
{
	...
	u32 bp[DACK_REG_8814C];
	u32 bp_reg[DACK_REG_8814C] = {0x180c, 0x410c, 0x520c, 0x530c, 0x1c3c,
				      ...
				      0x53bc};
	u32 bp_rf[DACK_RF_8814C][4];
	u32 bp_rfreg[DACK_RF_8814C] = {0x8f};

	count++;
	RF_DBG(dm, DBG_RF_DACK, "[DACK]count = %d", count);
	RF_DBG(dm, DBG_RF_DACK, "[DACK]DACK start!!!!!!!!!!!!!!!!!!!!!!!!!!!");
	halrf_bp(dm, bp_reg, bp, DACK_REG_8814C);
	halrf_bprf(dm, bp_rfreg, bp_rf,DACK_RF_8814C, 4);
	halrf_dack_setbb_8814c(dm);
	for (i = 0; i < 4; i++) {
		halrf_dack1_8814c(dm, i, iadc, qadc, &v1, &v2);
		halrf_dack2_8814c(dm, i, idac, qdac, v2, v1);
	}
	halrf_dack_reload_8814c(dm);
	halrf_reload_bp(dm, bp_reg, bp, DACK_REG_8814C);
	halrf_reload_bprf(dm,bp_rfreg, bp_rf, DACK_RF_8814C, 4);
	for (i = 0; i < 4; i++)
		halrf_dack_check_reset_8814c(dm, i);
	for (i = 0; i < 4; i++)
...
void _phy_lc_calibrate_8814c(struct dm_struct *dm)
{
...
void _phy_rtc_8814c(struct dm_struct *dm)
{
	...
	config_phydm_write_rf_syn_8814b(dm, RF_SYN1, 0xc, RFREG_MASK, tmp0xc | BIT(2));
#if 0
	cv_offset = config_phydm_read_syn_reg_8814c(dm, RF_SYN0, 0xd, 0xf0000);
	RF_DBG(dm, DBG_RF_LCK, "[LCK]SYN0 CV_offset = %x\n", cv_offset);
	cv_offset = config_phydm_read_syn_reg_8814c(dm, RF_SYN1, 0xd, 0xf0000);
...
void configure_txpower_track_8814c(struct txpwrtrack_cfg *config)
{
	...
	config->average_thermal_num = AVG_THERMAL_NUM_8814C;
	config->rf_path_count = MAX_PATH_NUM_8814C;
	config->thermal_reg_addr = RF_T_METER_8814C;

	config->odm_tx_pwr_track_set_pwr = odm_tx_pwr_track_set_pwr8814c;
	config->do_iqk = do_iqk_8814c;
	config->phy_lc_calibrate = halrf_lck_trigger;
	config->do_tssi_dck = halrf_tssi_dck;
	config->get_delta_swing_table = get_delta_swing_table_8814c;
	config->get_delta_swing_table8814only = get_delta_swing_table_8814c_path_cd;
}

#if ((DM_ODM_SUPPORT_TYPE & ODM_AP) || (DM_ODM_SUPPORT_TYPE == ODM_CE))
void phy_set_rf_path_switch_8814c(struct dm_struct *dm, boolean is_main)
#else
void phy_set_rf_path_switch_8814c(void *adapter, boolean is_main)
#endif
{
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
...
#if ((DM_ODM_SUPPORT_TYPE & ODM_AP) || (DM_ODM_SUPPORT_TYPE == ODM_CE))
boolean _phy_query_rf_path_switch_8814c(struct dm_struct *dm)
#else
boolean _phy_query_rf_path_switch_8814c(void *adapter)
#endif
{
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
...
#if ((DM_ODM_SUPPORT_TYPE & ODM_AP) || (DM_ODM_SUPPORT_TYPE == ODM_CE))
boolean phy_query_rf_path_switch_8814c(struct dm_struct *dm)
#else
boolean phy_query_rf_path_switch_8814c(void *adapter)
#endif
{
#if DISABLE_BB_RF
	return true;
#endif
#if ((DM_ODM_SUPPORT_TYPE & ODM_AP) || (DM_ODM_SUPPORT_TYPE == ODM_CE))
	return _phy_query_rf_path_switch_8814c(dm);
#else
	return _phy_query_rf_path_switch_8814c(adapter);
#endif
}
...
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	u32 tssi_trigger[4] = {0x18a4, 0x41a4, 0x52a4, 0x53a4}; //8814c
	u32 rf_path;
	...
	if (dm->support_ic_type & ODM_RTL8814C) {		
		for(rf_path = 0; rf_path < 4; rf_path++) {
			odm_set_bb_reg(dm, tssi_trigger[rf_path], 0x10000000, 0x0);
			RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
				"[TSSI]======>%s Reset tssi trigger bit to 0\n", __func__);
		}			
	}
}

#endif /*(RTL8814C_SUPPORT == 0)*/

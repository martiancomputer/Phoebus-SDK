...
void phydm_txbf_rfmode(void *dm_void, u8 su_bfee_cnt, u8 mu_bfee_cnt)
{
	...
	if (dm->support_ic_type == ODM_RTL8812F) {
		if (su_bfee_cnt > 0 || mu_bfee_cnt > 0) {
			#define MAX_RF_WRITE_RTY	3

			/*Path A ==================*/
			/*RF mode table write enable*/
			for (i = 0; i < MAX_RF_WRITE_RTY; i++) {
				odm_set_rf_reg(dm, RF_PATH_A, RF_0xef, BIT(19), 0x1);
				if (odm_get_rf_reg(dm, RF_PATH_A, RF_0xef, BIT(19)) == 0x1)
					break;
			}
			/*Select RX mode*/
			for (i = 0; i < MAX_RF_WRITE_RTY; i++) {
				odm_set_rf_reg(dm, RF_PATH_A, RF_0x33, 0xF, 3);
				if (odm_get_rf_reg(dm, RF_PATH_A, RF_0x33, 0xF) == 3)
					break;
			}
			/*Set Table data*/
			for (i = 0; i < MAX_RF_WRITE_RTY; i++) {
				odm_set_rf_reg(dm, RF_PATH_A, RF_0x3e, 0x3, 0x3);
				if (odm_get_rf_reg(dm, RF_PATH_A, RF_0x3e, 0x3) == 0x3)
					break;
			}
			/*Set Table data*/
			for (i = 0; i < MAX_RF_WRITE_RTY; i++) {
				odm_set_rf_reg(dm, RF_PATH_A, RF_0x3f, 0xfffff,
					       0x61AFE);
				if(odm_get_rf_reg(dm, RF_PATH_A, RF_0x3f, 0xfffff) == 0x61AFE)
					break;
			}
			if (i == MAX_RF_WRITE_RTY)
				printk("[%s] RF%d 0x%x write failed!!\n", __func__, RF_PATH_A, RF_0x3f);
			/*RF mode table write disable*/
			for (i = 0; i < MAX_RF_WRITE_RTY; i++) {
				odm_set_rf_reg(dm, RF_PATH_A, RF_0xef, BIT(19), 0x0);
				if (odm_get_rf_reg(dm, RF_PATH_A, RF_0xef, BIT(19)) == 0x0)
					break;
			}
			...
			for (i = 0; i < MAX_RF_WRITE_RTY; i++) {
				odm_set_rf_reg(dm, RF_PATH_B, RF_0xef, BIT(19), 0x1);
				if (odm_get_rf_reg(dm, RF_PATH_B, RF_0xef, BIT(19)) == 0x1)
					break;
			}
			/*Select RX mode*/
			for (i = 0; i < MAX_RF_WRITE_RTY; i++) {
				odm_set_rf_reg(dm, RF_PATH_B, RF_0x33, 0xF, 3);
				if (odm_get_rf_reg(dm, RF_PATH_B, RF_0x33, 0xF) == 3)
					break;
			}
			/*Set Table data*/
			for (i = 0; i < MAX_RF_WRITE_RTY; i++) {
				odm_set_rf_reg(dm, RF_PATH_B, RF_0x3f, 0xfffff,
				       0xD86BF);
				if (odm_get_rf_reg(dm, RF_PATH_B, RF_0x3f, 0xfffff) == 0xD86BF)
					break;
			}
			if (i == MAX_RF_WRITE_RTY)
				printk("[%s] RF%d 0x%x write failed!!\n", __func__, RF_PATH_B, RF_0x3f);
			/*RF mode table write disable*/
			for (i = 0; i < MAX_RF_WRITE_RTY; i++) {
				odm_set_rf_reg(dm, RF_PATH_B, RF_0xef, BIT(19), 0x0);
				if (odm_get_rf_reg(dm, RF_PATH_B, RF_0xef, BIT(19)) == 0x0)
					break;
			}
		}

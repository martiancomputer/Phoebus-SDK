...
DEFINE_TIMER_CALLBACK(phydm_adaptive_soml_callback);
#endif
#endif
...
#if (DM_ODM_SUPPORT_TYPE == ODM_AP)
DEFINE_TIMER_CALLBACK(pre_phydm_adaptive_soml_callback);
#endif
...
void phydm_adaptive_soml_timers_usb(void *dm_void, u8 state)
{
	...
	if (state == INIT_SOML_TIMMER) {
		...
		odm_initialize_timer(dm, &soml_tab->phydm_adaptive_soml_timer,
				     (void *)GET_TIMER_CALLBACK(pre_phydm_adaptive_soml_callback), NULL,
				     "phydm_adaptive_soml_timer_usb");
#else
		odm_initialize_timer(dm, &soml_tab->phydm_adaptive_soml_timer,
				     (void *)pre_phydm_adaptive_soml_callback, NULL,
				     "phydm_adaptive_soml_timer_usb");
#endif
...
#if (DM_ODM_SUPPORT_TYPE == ODM_AP)
		odm_initialize_timer(dm, &soml_tab->phydm_adaptive_soml_timer,
				     (void *)GET_TIMER_CALLBACK(phydm_adaptive_soml_callback), NULL,

...
const u32 array_mp_8814c_mac_reg[] = {

};

void
odm_read_and_config_mp_8814c_mac_reg(struct dm_struct *dm)
{
	u32	i = 0;
	u8	c_cond;
	boolean	is_matched = true, is_skipped = false;
	u32	array_len =
			sizeof(array_mp_8814c_mac_reg) / sizeof(u32);
	u32	*array = (u32 *)array_mp_8814c_mac_reg;
	...
	while ((i + 1) < array_len) {
		...
		if (v1 & (BIT(31) | BIT(30))) {/*positive & negative condition*/
			...
			if (is_matched)
				odm_config_mac_8814c(dm, v1, (u8)v2);
		}
		i = i + 2;
	}
}
...
u32
odm_get_version_mp_8814c_mac_reg(void)
{
		return 13;
}

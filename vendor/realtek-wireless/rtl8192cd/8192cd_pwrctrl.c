...
int rtw_sdio_suspend(struct device *dev)
{
	...
	if (rtk_timer_pending(&pshare->beacon_timer))
		rtk_del_timer_sync(&pshare->beacon_timer);
	if (rtk_timer_pending(&pshare->ps_timer))
		rtk_del_timer_sync(&pshare->ps_timer);
	if (rtk_timer_pending(&pshare->xmit_check_timer))
		rtk_del_timer_sync(&pshare->xmit_check_timer);
	if (rtk_timer_pending(&pshare->LED_Timer))
		rtk_del_timer_sync(&pshare->LED_Timer);
#if (BEAMFORMING_SUPPORT == 1) && (defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8822BE))
	if ((GET_CHIP_VER(priv)== VERSION_8192E) || (GET_CHIP_VER(priv)== VERSION_8822B)) {
		if (rtk_timer_pending(&pBeamformingInfo->BeamformingTimer))
			rtk_del_timer_sync(&pBeamformingInfo->BeamformingTimer);
	}
#endif
...
void rtw_lock_suspend_timeout(struct rtl8192cd_priv *priv, unsigned int timeout)
{
	...
	if (!priv->pshare->ps_timer_expires || time_after(expires, priv->pshare->ps_timer_expires)) {
		rtk_mod_timer(&priv->pshare->ps_timer, expires);
...
#endif
#if defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8821CE)
	/* Set H2C Cmd to FW To leave PS */
	if ((GET_CHIP_VER(priv) == VERSION_8192E) || (GET_CHIP_VER(priv) == VERSION_8821C)) {
...
void rtw_ap_start_fw_ps(struct rtl8192cd_priv *priv, u4Byte en_32k, u4Byte reason)
{
	...
	if ((GET_CHIP_VER(priv) == VERSION_8188E) || (GET_CHIP_VER(priv) == VERSION_8822B) || (GET_CHIP_VER(priv) == VERSION_8821C)) {

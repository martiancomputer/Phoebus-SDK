	...
	int initial_gain)
{
	...
	if (!priv->pmib->dot11DFSEntry.disable_tx)
		odm_write_1byte(p_dm_odm, 0x522, 0xFF); //REG_TXPAUSE �אּ0x522

	// Turn off CCA
	if((GET_CHIP_VER(priv) == VERSION_8814A) || (GET_CHIP_VER(priv) == VERSION_8822B)){
		odm_set_bb_reg(p_dm_odm, 0x838, BIT1, 0x1); //838[1] �]��1
	}
	else{
		odm_set_bb_reg(p_dm_odm, 0x838, BIT3, 0x1); //838[3] �]��1
	}
	...
	// Turn off 3-wire
	odm_set_bb_reg(p_dm_odm, 0xC00, BIT1|BIT0, 0x0); //c00[1:0] �g0

	// pts value = 128, 256, 512, 1024
	odm_set_bb_reg(p_dm_odm, 0x910, BIT14|BIT15, 0x1); //910[15:14]�]��1, ��256�I
	odm_set_bb_reg(p_dm_odm, 0x910, BIT12|BIT13, 0x1); //910[13:12]�]��1, avg 8 ��
	...
	// Turn on 3-wire
	odm_set_bb_reg(p_dm_odm, 0xc00, BIT1|BIT0, 0x3); //c00[1:0] �g3
	...
	//Turn on CCA
	if((GET_CHIP_VER(priv) == VERSION_8814A) || (GET_CHIP_VER(priv) == VERSION_8822B)){
		odm_set_bb_reg(p_dm_odm, 0x838, BIT1, 0); //838[1] �]��0
	}
	else{
		odm_set_bb_reg(p_dm_odm, 0x838, BIT3, 0); //838[3] �]��0
	}
	...
	// Resume TX Queue
	if (!priv->pmib->dot11DFSEntry.disable_tx)
		odm_write_1byte(p_dm_odm, 0x522, 0x00); //REG_TXPAUSE �אּ0x522
...
void dfs_radar_distinguish_for_8814c(struct rtl8192cd_priv *priv, void *dm_void)
{	
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct ccx_info *ccx = &dm->dm_ccx_info;
	
	//----- syn0 --------//
	unsigned int reg_2e08, reg_2e24, reg_2e28; //PW
	unsigned int reg_2e2c, reg_2e30, reg_2e34; //PRI
	unsigned int reg_2e0c, reg_2e10, reg_2e20; //Loct.
	//----- syn1 --------//
	unsigned int reg_2e90, reg_2ea4, reg_2ea8; //PW
	unsigned int reg_2eac, reg_2eb0, reg_2eb4; //PRI
	unsigned int reg_2e94, reg_2e98, reg_2ea0; //Loct.
	//----- common ------//
	unsigned int reg_2e18;			   // mode PRI


	
	unsigned int pw_rpt_14c_set[12]={0}, pri_rpt_14c_set[12]={0}, pri_rpt_14c_mode=0;
	signed char loct_rpt_14c_set[12]={0};
	unsigned char i, j, dc_cnt = 0, short_cnt_th = 0, long_cnt_th = 0, loop_cnt = 0, radar_class_tmp = 0;
	unsigned int loct_diff_14c = 0, radar_class_num = 0, radar_class_cnt[10] = {0}, pw_diff_14c = 0;
	unsigned int pri_cnt_final = 0, pri_cnt_temp = 0;
	...
	/*for MIB*/
	unsigned int pw_lowth_14c[10], pw_uppth_14c[10], pri_rpt_lowth_14c[10], pri_rpt_uppth_14c[10];
	//unsigned int loct_diff_th_14c = 0, pw_diff_th_14c = 0, pw_cnt_th_14c = 0 ,cnt_th_14c = 0, pri_cnt_th_14c = 0, pri_vaild_max = 0 ,pri_vaild_min = 32767;
	unsigned int loct_diff_th_14c = 0, pw_diff_th_14c = 0, cnt_th_14c = 0, pri_vaild_max = 0 ,pri_vaild_min = 32767;
	unsigned char pri_cnt_th = 0, pri_var_offset = 0;

	//pw_cnt_th_14c = priv->pshare->rf_ft_var.pw_cnt_th;
	loct_diff_th_14c = priv->pshare->rf_ft_var.loct_diff_th;
	pw_diff_th_14c = priv->pshare->rf_ft_var.pw_diff_th;	
	pri_cnt_th = priv->pshare->rf_ft_var.pri_cnt_th;
	pri_var_offset = priv->pshare->rf_ft_var.pri_var_offset;

	/*read pw/pri/lctn report*/
	if (priv->seg1_flag_14c== 1) {
		//panic_printk("\n SYN 1 \n");

		/* PW : Wordlength : U(8,0) , unit: 400ns*/
		reg_2e90 = phy_query_bb_reg(priv, 0x2e90, 0xffffffff);
		pw_rpt_14c_set[0]=(unsigned int)(reg_2eac& 0x000000ff);
		pw_rpt_14c_set[1]=(unsigned int)((reg_2eac & 0x0000ff00)>>8);
		pw_rpt_14c_set[2]=(unsigned int)((reg_2eac & 0x00ff0000)>>16);
		pw_rpt_14c_set[3]=(unsigned int)((reg_2e90 & 0xff000000)>>24);
		reg_2ea4 = phy_query_bb_reg(priv, 0x2ea4, 0xffffffff);
		pw_rpt_14c_set[4]=(unsigned int)(reg_2ea4 & 0x000000ff);
		pw_rpt_14c_set[5]=(unsigned int)((reg_2ea4 & 0x0000ff00)>>8);
		pw_rpt_14c_set[6]=(unsigned int)((reg_2ea4 & 0x00ff0000)>>16);
		pw_rpt_14c_set[7]=(unsigned int)((reg_2ea4 & 0xff000000)>>24);
		reg_2ea8 = phy_query_bb_reg(priv, 0x2ea8, 0xffffffff);
		pw_rpt_14c_set[8]=(unsigned int)(reg_2ea8 & 0x000000ff);
		pw_rpt_14c_set[9]=(unsigned int)((reg_2ea8 & 0x0000ff00)>>8);
		pw_rpt_14c_set[10]=(unsigned int)((reg_2ea8 & 0x00ff0000)>>16);
		pw_rpt_14c_set[11]=(unsigned int)((reg_2ea8 & 0xff000000)>>24);

		/* PRI : Wordlength : U(8,0) , unit: 25us */
		reg_2eac = phy_query_bb_reg(priv, 0x2eac, 0xffffffff);
		pri_rpt_14c_set[0]=(unsigned int)(reg_2eac & 0x000000ff);
		pri_rpt_14c_set[1]=(unsigned int)((reg_2eac & 0x0000ff00)>>8);
		pri_rpt_14c_set[2]=(unsigned int)((reg_2eac & 0x00ff0000)>>16);
		pri_rpt_14c_set[3]=(unsigned int)((reg_2eac & 0xff000000)>>24);
		reg_2eb0 = phy_query_bb_reg(priv, 0x2eb0, 0xffffffff);
		pri_rpt_14c_set[4]=(unsigned int)(reg_2eb0 & 0x000000ff);
		pri_rpt_14c_set[5]=(unsigned int)((reg_2eb0 & 0x0000ff00)>>8);
		pri_rpt_14c_set[6]=(unsigned int)((reg_2eb0 & 0x00ff0000)>>16);
		pri_rpt_14c_set[7]=(unsigned int)((reg_2eb0 & 0xff000000)>>24);
		reg_2eb4 = phy_query_bb_reg(priv, 0x2eb4, 0xffffffff);
		pri_rpt_14c_set[8]=(unsigned int)(reg_2eb4 & 0x000000ff);
		pri_rpt_14c_set[9]=(unsigned int)((reg_2eb4 & 0x0000ff00)>>8);
		pri_rpt_14c_set[10]=(unsigned int)((reg_2eb4 & 0x00ff0000)>>16);
		pri_rpt_14c_set[11]=(unsigned int)((reg_2eb4 & 0xff000000)>>24);

		pri_rpt_14c_mode = (unsigned int)(reg_2e18 & 0x00000fff);

		/* Location : Wordlength : S(8,0) , unit: MHz */
		reg_2e94 = phy_query_bb_reg(priv, 0x2e94, 0xffffffff);
		loct_rpt_14c_set[0]=(signed char)(reg_2e94 & 0x000000ff);
		loct_rpt_14c_set[1]=(signed char)((reg_2e94 & 0x0000ff00)>>8);
		loct_rpt_14c_set[2]=(signed char)((reg_2e94 & 0x00ff0000)>>16);
		loct_rpt_14c_set[3]=(signed char)((reg_2e94 & 0xff000000)>>24);
		reg_2e98 = phy_query_bb_reg(priv, 0x2e98, 0xffffffff);
		loct_rpt_14c_set[4]=(signed char)(reg_2e98 & 0x000000ff);
		loct_rpt_14c_set[5]=(signed char)((reg_2e98 & 0x0000ff00)>>8);
		loct_rpt_14c_set[6]=(signed char)((reg_2e98 & 0x00ff0000)>>16);
		loct_rpt_14c_set[7]=(signed char)((reg_2e98 & 0xff000000)>>24);
		reg_2ea0 = phy_query_bb_reg(priv, 0x2ea0, 0xffffffff);
		loct_rpt_14c_set[8]=(signed char)(reg_2ea0 & 0x000000ff);
		loct_rpt_14c_set[9]=(signed char)((reg_2ea0 & 0x0000ff00)>>8);
		loct_rpt_14c_set[10]=(signed char)((reg_2ea0 & 0x00ff0000)>>16);
		loct_rpt_14c_set[11]=(signed char)((reg_2ea0 & 0xff000000)>>24);
		...
		/* PW : Wordlength : U(8,0) , unit: 400ns*/
		reg_2e08 = phy_query_bb_reg(priv, 0x2e08, 0xffffffff);
		pw_rpt_14c_set[0]=(unsigned int)(reg_2e08& 0x000000ff);
		pw_rpt_14c_set[1]=(unsigned int)((reg_2e08 & 0x0000ff00)>>8);
		pw_rpt_14c_set[2]=(unsigned int)((reg_2e08 & 0x00ff0000)>>16);
		pw_rpt_14c_set[3]=(unsigned int)((reg_2e08 & 0xff000000)>>24);
		reg_2e24 = phy_query_bb_reg(priv, 0x2e24, 0xffffffff);
		pw_rpt_14c_set[4]=(unsigned int)(reg_2e24 & 0x000000ff);
		pw_rpt_14c_set[5]=(unsigned int)((reg_2e24 & 0x0000ff00)>>8);
		pw_rpt_14c_set[6]=(unsigned int)((reg_2e24 & 0x00ff0000)>>16);
		pw_rpt_14c_set[7]=(unsigned int)((reg_2e24 & 0xff000000)>>24);
		reg_2e28 = phy_query_bb_reg(priv, 0x2e28, 0xffffffff);
		pw_rpt_14c_set[8]=(unsigned int)(reg_2e28 & 0x000000ff);
		pw_rpt_14c_set[9]=(unsigned int)((reg_2e28 & 0x0000ff00)>>8);
		pw_rpt_14c_set[10]=(unsigned int)((reg_2e28 & 0x00ff0000)>>16);
		pw_rpt_14c_set[11]=(unsigned int)((reg_2e28 & 0xff000000)>>24);

		/* PRI : Wordlength : U(8,0) , unit: 25.6us */
		reg_2e2c = phy_query_bb_reg(priv, 0x2e2c, 0xffffffff);
		pri_rpt_14c_set[0]=(unsigned int)(reg_2e2c & 0x000000ff);
		pri_rpt_14c_set[1]=(unsigned int)((reg_2e2c & 0x0000ff00)>>8);
		pri_rpt_14c_set[2]=(unsigned int)((reg_2e2c & 0x00ff0000)>>16);
		pri_rpt_14c_set[3]=(unsigned int)((reg_2e2c & 0xff000000)>>24);
		reg_2e30 = phy_query_bb_reg(priv, 0x2e30, 0xffffffff);
		pri_rpt_14c_set[4]=(unsigned int)(reg_2e30 & 0x000000ff);
		pri_rpt_14c_set[5]=(unsigned int)((reg_2e30 & 0x0000ff00)>>8);
		pri_rpt_14c_set[6]=(unsigned int)((reg_2e30 & 0x00ff0000)>>16);
		pri_rpt_14c_set[7]=(unsigned int)((reg_2e30 & 0xff000000)>>24);
		reg_2e34 = phy_query_bb_reg(priv, 0x2e34, 0xffffffff);
		pri_rpt_14c_set[8]=(unsigned int)(reg_2e34 & 0x000000ff);
		pri_rpt_14c_set[9]=(unsigned int)((reg_2e34 & 0x0000ff00)>>8);
		pri_rpt_14c_set[10]=(unsigned int)((reg_2e34 & 0x00ff0000)>>16);
		pri_rpt_14c_set[11]=(unsigned int)((reg_2e34 & 0xff000000)>>24);
		/* Mode PRI Wordlength : U(12,0) , unit: us */
		pri_rpt_14c_mode = (unsigned int)(reg_2e18 & 0x0fff00000);

		/* Location : Wordlength : S(8,0) , unit: MHz */
		reg_2e0c = phy_query_bb_reg(priv, 0x2e0c, 0xffffffff);
		loct_rpt_14c_set[0]=(signed char)(reg_2e0c & 0x000000ff);
		loct_rpt_14c_set[1]=(signed char)((reg_2e0c & 0x0000ff00)>>8);
		loct_rpt_14c_set[2]=(signed char)((reg_2e0c & 0x00ff0000)>>16);
		loct_rpt_14c_set[3]=(signed char)((reg_2e0c & 0xff000000)>>24);
		reg_2e10 = phy_query_bb_reg(priv, 0x2e10, 0xffffffff);
		loct_rpt_14c_set[4]=(signed char)(reg_2e10 & 0x000000ff);
		loct_rpt_14c_set[5]=(signed char)((reg_2e10 & 0x0000ff00)>>8);
		loct_rpt_14c_set[6]=(signed char)((reg_2e10 & 0x00ff0000)>>16);
		loct_rpt_14c_set[7]=(signed char)((reg_2e10 & 0xff000000)>>24);
		reg_2e20 = phy_query_bb_reg(priv, 0x2e20, 0xffffffff);
		loct_rpt_14c_set[8]=(signed char)(reg_2e20 & 0x000000ff);
		loct_rpt_14c_set[9]=(signed char)((reg_2e20 & 0x0000ff00)>>8);
		loct_rpt_14c_set[10]=(signed char)((reg_2e20 & 0x00ff0000)>>16);
		loct_rpt_14c_set[11]=(signed char)((reg_2e20 & 0xff000000)>>24);
	...
	for (i = 0; i < 12; i++) {
		priv->pw_14c_rslt[i] = 0;
		priv->loct_14c_rslt[i] = 0;
		priv->pri_14c_rslt[i] = 0;
	}
	
	priv->pri_14c_rslt_mode = 0;
	...
	// In 8814c, a50[23:20] = N => The short_pulse_cnt_th is N+2
	if (priv->radar_type) 
		loop_cnt = long_cnt_th + 2;
	else
		loop_cnt = short_cnt_th  + 2;
	...
	if (!(priv->pshare->rf_ft_var.pw_pri_cnt_debug)) 
		cnt_th_14c = loop_cnt / 2;
	else 
		cnt_th_14c= priv->pshare->rf_ft_var.loop_cnt_th;	

	
	
	/*Calculate the difference between each element (1st Not calculated)*/
	for(i = 0; i < loop_cnt; i++){
		priv->loct_14c_rslt[i] = loct_rpt_14c_set[i];
	}

	for(i = 1; i < loop_cnt; i++){
		loct_diff_14c = loct_diff_14c + abs(priv->loct_14c_rslt[i] - priv->loct_14c_rslt[i-1]);
	}
	/*The value may less than the average deviation, since the variable type is int (not float)*/
	
	for(i = 0; i < loop_cnt; i++) {
		if (-4 < loct_rpt_14c_set[i] && loct_rpt_14c_set[i] < 4)
			dc_cnt++;
	}
	...
	if (dc_cnt >= 3){
		loct_diff_th_14c += 15;
		pw_diff_th_14c +=3;
		/* Due to half of pri rpts are 0*/
		cnt_th_14c = ((cnt_th_14c/2) > 1) ? (cnt_th_14c/2) : 2; 
	}
	...
	if (loct_diff_14c < loct_diff_th_14c)
		priv->loct_flag = 1;
	...
	/*radar type classification*/
	priv->pri_14c_rslt_mode = pri_rpt_14c_mode;


	/* Added by Big Bill */
	/* Radar type classification */
	pw_lowth_14c[0] = priv->pshare->rf_ft_var.pw_lth1;
	pw_uppth_14c[0] = priv->pshare->rf_ft_var.pw_uth1;
	pw_lowth_14c[1] = priv->pshare->rf_ft_var.pw_lth2;
	pw_uppth_14c[1] = priv->pshare->rf_ft_var.pw_uth2;
	pw_lowth_14c[2] = priv->pshare->rf_ft_var.pw_lth3;
	pw_uppth_14c[2] = priv->pshare->rf_ft_var.pw_uth3;
	pw_lowth_14c[3] = priv->pshare->rf_ft_var.pw_lth4;
	pw_uppth_14c[3] = priv->pshare->rf_ft_var.pw_uth4;
	pw_lowth_14c[4] = priv->pshare->rf_ft_var.pw_lth5;
	pw_uppth_14c[4] = priv->pshare->rf_ft_var.pw_uth5;
	pw_lowth_14c[5] = priv->pshare->rf_ft_var.pw_lth6;
	pw_uppth_14c[5] = priv->pshare->rf_ft_var.pw_uth6;

	pri_rpt_lowth_14c[0] = priv->pshare->rf_ft_var.pri_lth1;
	pri_rpt_uppth_14c[0] = priv->pshare->rf_ft_var.pri_uth1;
	pri_rpt_lowth_14c[1] = priv->pshare->rf_ft_var.pri_lth2;
	pri_rpt_uppth_14c[1] = priv->pshare->rf_ft_var.pri_uth2;
	pri_rpt_lowth_14c[2] = priv->pshare->rf_ft_var.pri_lth3;
	pri_rpt_uppth_14c[2] = priv->pshare->rf_ft_var.pri_uth3;
	pri_rpt_lowth_14c[3] = priv->pshare->rf_ft_var.pri_lth4;
	pri_rpt_uppth_14c[3] = priv->pshare->rf_ft_var.pri_uth4;
	pri_rpt_lowth_14c[4] = priv->pshare->rf_ft_var.pri_lth5;
	pri_rpt_uppth_14c[4] = priv->pshare->rf_ft_var.pri_uth5;
	pri_rpt_lowth_14c[5] = priv->pshare->rf_ft_var.pri_lth6;
	pri_rpt_uppth_14c[5] = priv->pshare->rf_ft_var.pri_uth6;

	if (priv->pshare->rf_ft_var.manual_dfs_regdomain == 1) {
		pw_lowth_14c[6] = priv->pshare->rf_ft_var.pw_lth7;
		pw_uppth_14c[6] = priv->pshare->rf_ft_var.pw_uth7;
		pri_rpt_lowth_14c[6] = priv->pshare->rf_ft_var.pri_lth7;
		pri_rpt_uppth_14c[6] = priv->pshare->rf_ft_var.pri_uth7;
		radar_class_num = 7;
		if (dc_cnt >= 3) {
			pw_lowth_14c[0] = 0;
			pw_lowth_14c[1] = 0;
		...
		pw_lowth_14c[6] = priv->pshare->rf_ft_var.pw_lth7;
		pw_uppth_14c[6] = priv->pshare->rf_ft_var.pw_uth7;
		pw_lowth_14c[7] = priv->pshare->rf_ft_var.pw_lth8;
		pw_uppth_14c[7] = priv->pshare->rf_ft_var.pw_uth8;
		pri_rpt_lowth_14c[6] = priv->pshare->rf_ft_var.pri_lth7;
		pri_rpt_uppth_14c[6] = priv->pshare->rf_ft_var.pri_uth7;
		pri_rpt_lowth_14c[7] = priv->pshare->rf_ft_var.pri_lth8;
		pri_rpt_uppth_14c[7] = priv->pshare->rf_ft_var.pri_uth8;
		radar_class_num = 8;
		if (dc_cnt >= 3) {
			if ((priv->pmib->dot11RFEntry.dot11channel >= 52) && (priv->pmib->dot11RFEntry.dot11channel <= 64)) {
				pw_lowth_14c[0] = 0;
	...
	if (priv->radar_type == 0) {
		for (i = 0; i < loop_cnt; i++) {
			priv->pw_14c_rslt[i] = pw_rpt_14c_set[i];
			priv->pri_14c_rslt[i] = pri_rpt_14c_set[i];
			//Idealistically, #PW = #PRI + 1
			for ( j = 0; j < radar_class_num ; j++) {
				if ( pw_lowth_14c[j] <= priv->pw_14c_rslt[i]
					&& priv->pw_14c_rslt[i] <= pw_uppth_14c[j] 
					&& pri_rpt_lowth_14c[j] <= priv->pri_14c_rslt[i] 
					&& priv->pri_14c_rslt[i] <= pri_rpt_uppth_14c[j]){
					radar_class_cnt[j]++;	
					if (priv->pri_14c_rslt[i] > pri_vaild_max)
						pri_vaild_max = priv->pri_14c_rslt[i];
					if (priv->pri_14c_rslt[i] < pri_vaild_min)
						pri_vaild_min = priv->pri_14c_rslt[i];
				}
			}		
		}
		for(j = 1; j < loop_cnt; j++)
			pw_diff_14c = pw_diff_14c + abs(priv->pw_14c_rslt[j] - priv->pw_14c_rslt[j-1]);
			/*The value may less than the average deviation, since the variable type is int (not float)*/
	}	
	else {
	// PRI of long pulse is not accurate enough
		for (i = 0; i < loop_cnt; i++) {
			...
			//Idealistically, #PW = #PRI + 1
			for ( j = 0; j < radar_class_num ; j++) {
				if ( pw_lowth_14c[j] <= priv->pw_14c_rslt[i] && priv->pw_14c_rslt[i] <= pw_uppth_14c[j] )
					radar_class_cnt[j]++;					
			}		
		}
	}
	...
	for (i = 0; i < radar_class_num ; i++) {
		if ( radar_class_cnt[i] > cnt_th_14c ) {
			if (!(priv->radar_type)) {
				if (dc_cnt >= 3 || pw_diff_14c < pw_diff_th_14c)
					priv->pulsewidth_flag = 1;
				/*
				if (priv->pri_14b_rslt != 0)
					priv->pulseinterval_flag = 1;
	...
	for (i = 0; i < (loop_cnt - 1); i++) {
		for (j = 0; j < (loop_cnt - 1); j++) {
			if ((i == j) || (priv->pri_14c_rslt[i] <= 4))
				continue;
			if ((priv->pri_14c_rslt[i] <= (priv->pri_14c_rslt[j] + pri_var_offset)) && (priv->pri_14c_rslt[i] >= (priv->pri_14c_rslt[j] - pri_var_offset))
				&& (priv->pri_14c_rslt[i] <= pri_vaild_max) && (priv->pri_14c_rslt[i] >= pri_vaild_min))
				pri_cnt_temp++;
		}
		if (pri_cnt_temp > pri_cnt_final)
			pri_cnt_final = pri_cnt_temp;
	...
	priv->nhm_avg_flag = 0;
	priv->nhm_cnt = (priv->nhm_cnt >= 9) ? 0 : (priv->nhm_cnt+1);
	//panic_printk("nhm_cnt = %d!,nhm_dfs_cnt = %d!\n", priv->nhm_cnt, priv->nhm_dfs_cnt);
	priv->nhm_dfs_cnt_all[priv->nhm_cnt] = priv->nhm_dfs_cnt;
	priv->nhm_acc = 0;
	for (i = 0; i< 10; i++) {
		priv->nhm_acc+=priv->nhm_dfs_cnt_all[i];
		//panic_printk("nhm_dfs_cnt_all[%d] = %d!, priv->nhm_acc =  %d!\n",
		//	i, priv->nhm_dfs_cnt_all[i], priv->nhm_acc);
	}
	if (priv->nhm_acc > 25) {
		priv->nhm_avg_flag = 1;
		//panic_printk("priv->nhm_avg_flag = %d!\n", priv->nhm_avg_flag);
	}
			
	if (priv->pshare->rf_ft_var.dfs_print_hist_report) {
		if (priv->nhm_dfs_flag || priv->nhm_avg_flag) {
			panic_printk("Enable NHMxDFS or NHMavgDFS\n");
			panic_printk("NHMxDFS cnt = %d!, nhm_acc = %d!\n",
				priv->nhm_dfs_cnt, priv->nhm_acc);
		} else {
			panic_printk("\n");
			if (priv->pulsewidth_flag && priv->pulseinterval_flag && priv->loct_flag ) 
				panic_printk("Radar type %d has detected!!\n", (radar_class_tmp + 1));
			...
			if (priv->radar_type == 1)
				panic_printk("long_pulse_flag raise up!\n");
			panic_printk("The current mib information of 8814c: ([Lower bound / Upper bound])\n");
			//PW Threshold
			if (dc_cnt >= 6)
				panic_printk("Relax for DC cnt!\n");
			
			panic_printk("pw_th1 : [%d, %d], pw_th2 : [%d, %d], pw_th3 : [%d, %d]\n",
				pw_lowth_14c[0], pw_uppth_14c[0], pw_lowth_14c[1], pw_uppth_14c[1], pw_lowth_14c[2], pw_uppth_14c[2]);
			panic_printk("pw_th4 : [%d, %d], pw_th5 : [%d, %d], pw_th6 : [%d, %d] \n",
				pw_lowth_14c[3], pw_uppth_14c[3], pw_lowth_14c[4], pw_uppth_14c[4], pw_lowth_14c[5], pw_uppth_14c[5]); 
			if (priv->pshare->rf_ft_var.manual_dfs_regdomain == 1)
				panic_printk("pw_th7 : [%d, %d]\n",pw_lowth_14c[6], pw_uppth_14c[6]);
			if (priv->pshare->rf_ft_var.manual_dfs_regdomain == 2)
				panic_printk("pw_th7 : [%d, %d], pw_th8 : [%d, %d]\n",
				pw_lowth_14c[6], pw_uppth_14c[6],pw_lowth_14c[7], pw_uppth_14c[7]);
			//PRI Threshold
			panic_printk("pri_th1 : [%d, %d], pri_th2 : [%d, %d], pri_th3 : [%d, %d]\n",
				pri_rpt_lowth_14c[0], pri_rpt_uppth_14c[0], pri_rpt_lowth_14c[1], pri_rpt_uppth_14c[1], pri_rpt_lowth_14c[2], pri_rpt_uppth_14c[2]);
			panic_printk("pri_th4 : [%d, %d], pri_th5 : [%d, %d], pri_th6 : [%d, %d]\n",
				pri_rpt_lowth_14c[3], pri_rpt_uppth_14c[3],pri_rpt_lowth_14c[4], pri_rpt_uppth_14c[4], pri_rpt_lowth_14c[5], pri_rpt_uppth_14c[5]);
			if (priv->pshare->rf_ft_var.manual_dfs_regdomain == 1)
				panic_printk("pri_th7 : [%d, %d]\n",pri_rpt_lowth_14c[6], pri_rpt_uppth_14c[6]);
			if (priv->pshare->rf_ft_var.manual_dfs_regdomain == 2)
				panic_printk("pri_th7 : [%d, %d], pri_th8 : [%d, %d]\n",
				pri_rpt_lowth_14c[6], pri_rpt_uppth_14c[6],pri_rpt_lowth_14c[7], pri_rpt_uppth_14c[7]);
			//Diff Threshold
			panic_printk("loct_diff_th_14c = %d\n", loct_diff_th_14c);
			panic_printk("pw_diff_th_14c = %d\n", pw_diff_th_14c);

			//Advanced PRI Threshold
			if (!(priv->idle_flag)) {
				panic_printk("pri_var_offset = %d (Relax for TP mode)\n", pri_var_offset);
				panic_printk("pri_cnt_th = %d (Relax for TP mode)\n", pri_cnt_th);
			} else {
			...
			panic_printk("cnt_th_14c (half of loop cnt) = %d\n", cnt_th_14c);


			panic_printk("\n");
			panic_printk("The current report of 8814c:\n");
			panic_printk("pw_rpt(1-12) = [%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d]\n",
				     pw_rpt_14c_set[0], pw_rpt_14c_set[1], pw_rpt_14c_set[2], pw_rpt_14c_set[3], pw_rpt_14c_set[4], pw_rpt_14c_set[5], pw_rpt_14c_set[6], pw_rpt_14c_set[7],pw_rpt_14c_set[8], pw_rpt_14c_set[9], pw_rpt_14c_set[10], pw_rpt_14c_set[11]);
			panic_printk("loct_rpt(1-12) = [%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d]\n",
				     loct_rpt_14c_set[0], loct_rpt_14c_set[1], loct_rpt_14c_set[2], loct_rpt_14c_set[3], loct_rpt_14c_set[4], loct_rpt_14c_set[5], loct_rpt_14c_set[6], loct_rpt_14c_set[7], loct_rpt_14c_set[8], loct_rpt_14c_set[9], loct_rpt_14c_set[10], loct_rpt_14c_set[11]);
			panic_printk("pri_rpt(1-12) = [%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d]\n",
				     pri_rpt_14c_set[0], pri_rpt_14c_set[1], pri_rpt_14c_set[2], pri_rpt_14c_set[3], pri_rpt_14c_set[4], pri_rpt_14c_set[5], pri_rpt_14c_set[6], pri_rpt_14c_set[7], pri_rpt_14c_set[8], pri_rpt_14c_set[9], pri_rpt_14c_set[10], pri_rpt_14c_set[11]);
			panic_printk("pri_rpt_mode = [%d]\n",pri_rpt_14c_mode);
			
			panic_printk("\n");
			panic_printk("loop_cnt = %d\n",loop_cnt);
			panic_printk("loct_diff_14c = %d\n", loct_diff_14c);
			panic_printk("pw_diff_14c = %d\n", pw_diff_14c);
...
void rtl8192cd_dfs_det_chk(struct rtl8192cd_priv *priv)
{
	...
	struct ccx_info *ccx = &dm->dm_ccx_info;
	unsigned int regf98_value = 0;
	unsigned int reg2e00_value;
#if defined(CONFIG_WLAN_HAL_8814BE) || defined(CONFIG_WLAN_HAL_8814CR)
	unsigned int reg2e20_value;  //8814B
	unsigned int reg2e88_value;  //8814C
#endif
	...
	int i, PSD_report_right[20]={0}, PSD_report_left[20]={0};
	...
	if ((GET_CHIP_VER(priv) == VERSION_8814B)) {
		...
		else if (priv->seg1_flag_14c == 1){
			reg2e88_value = phy_query_bb_reg(priv, 0x2e88, 0xffffffff);
			short_pulse_cnt_cur = (u16)((reg2e88_value & 0x000ff800) >> 11);
			long_pulse_cnt_cur = (u16)((reg2e88_value & 0x0fc00000) >> 22);
...
#if defined(CONFIG_WLAN_HAL_8814BE)
	if ((GET_CHIP_VER(priv) == VERSION_8814B)) {
		if (priv->seg1_flag == 1 || priv->seg1_flag_14c == 1) {
			DFS_tri_short_pulse = (reg2e20_value & BIT(20)) ? 1 : 0;
			DFS_tri_long_pulse = (reg2e20_value & BIT(28)) ? 1 : 0;
		}else if (priv->seg1_flag_14c == 1){
			DFS_tri_short_pulse = (reg2e88_value & BIT(20)) ? 1 : 0;
			DFS_tri_long_pulse = (reg2e88_value & BIT(28)) ? 1 : 0;
		} else {
	...
	if((GET_CHIP_VER(priv) == VERSION_8814B) && (((RTL_R16(REG_SYS_CFG1) >> 12) & 0xf) == 0x8)  && (((RTL_R32(REG_SYS_STATUS1) >> 28) & 0xf) == 0x8)) {
		...
		if(priv->pshare->rf_ft_var.dfs_radar_diff_on && priv->pshare->rf_ft_var.dfs_det_off == 0) {
			if(priv->pulse_flag_hist[priv->mask_idx])
				dfs_radar_distinguish_for_8814c(priv,dm);
		}
	}else
#endif
...
//#if defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_WLAN_HAL_8822BE)
	...
	if ((GET_CHIP_VER(priv) == VERSION_8814B)) {
		if (priv->seg1_flag == 1) {
			...
			reg_dfsrpt_value = phy_query_bb_reg(priv, 0x2e88, 0xffffffff);
			if ((reg_dfsrpt_value & BIT(20)) || (reg_dfsrpt_value & BIT(28))) {
				odm_set_bb_reg(dm, 0xa6c, BIT(0), 0);
				odm_set_bb_reg(dm, 0xa6c, BIT(0), 1);
			}
...
void rtl8192cd_dfs_dynamic_setting(struct rtl8192cd_priv *priv)
{
	...
	if ((GET_CHIP_VER(priv) == VERSION_8814B)  || (GET_CHIP_VER(priv) == VERSION_8812F)) {
		if ((priv->idle_flag == 1)) {
			short_pulse_cnt_th_cur = 10;	// Real cnt in 8812F
			...
			else if (((GET_CHIP_VER(priv) == VERSION_8814B) && (((RTL_R16(REG_SYS_CFG1) >> 12) & 0xf) == 0x8)  && (((RTL_R32(REG_SYS_STATUS1) >> 28) & 0xf) == 0x8))){
				short_pulse_cnt_th_cur = 8;	// Real cnt in 8814c
				long_pulse_cnt_th_cur = 6;	// Real cnt in 8814c
			}
			else {

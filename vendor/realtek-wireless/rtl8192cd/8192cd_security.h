...
type 12 is used only with cipher suite selector values
    00-0F-AC:9  (GCMP-256),
...
typedef struct _CHANNEL_CHANGE {
    unsigned char EventId;
    unsigned char IsMoreEvent;
    unsigned char channel;
    unsigned char ReasonCode; //0: unknown, 1: ACS, 2: DFS
} CHANNEL_CHANGE;
...
typedef	union _DOT11_RSN_CAPABILITY{
	...
	struct
	{
		...
		unsigned short Reserved2:3; //B13 B14 B15
		unsigned short PBAC:1; //B12
		unsigned short SppAmsduRequired:1; //B11
		unsigned short SppAmsduCapable:1; //B10
		unsigned short PeerKey:1; //B9
		unsigned short Reserved:1; //B8
#else
		unsigned short PreAuthentication:1; // B0
		unsigned short NoPairwise:1; // B1
		unsigned short PtksaReplayCounter:2; // B3 B2

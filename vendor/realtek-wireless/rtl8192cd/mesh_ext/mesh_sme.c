...
{
	init_mesh_peer_link_timer(priv);

#ifdef MESH_BOOTSEQ_AUTH
	init_mesh_auth_timer(priv);
#endif
...
#ifdef RTK_MESH_AODV_STANDALONE_TIMER
static __always_inline void mesh_standalone_timer_expire(unsigned long task_priv)
{
    ...
    aodv_expire(priv);
    rtk_mod_timer(&priv->mesh_expire_timer, jiffies + MESH_AODV_EXPIRE_TO);
}
DEFINE_TIMER_CALLBACK(mesh_standalone_timer_expire);
...
void mesh_CSA_expire(DRV_PRIV *priv) {
	...
	if(priv->mesh_CSA_state == MESH_CSA_ONGOING) {
		if(priv->mesh_CSA_expire) {
			...
			if(priv->pmib->dot11RFEntry.dot11channel != priv->mesh_CSA_channel) {
	...
	else if(priv->mesh_CSA_state == MESH_CSA_CROSSBAND_ONGOING) {
		if(priv->mesh_CSA_expire) {
			priv->mesh_CSA_expire--;
		}
		else {
...
int mesh_CSA_check(DRV_PRIV *priv, struct rx_frinfo *pfrinfo)
{
    ...
    UINT32 swchnl_precedence = 0;
    int ret = 0;
	int mesh_csa_enable = 0;

	if(priv->bss11v_priv_sc) {
		if(GET_MIB(priv)->meshCSA.mesh_csa_enable || GET_MIB(priv->bss11v_priv_sc)->meshCSA.mesh_csa_enable)
			mesh_csa_enable = 1;
	}
	else {
		if(GET_MIB(priv)->meshCSA.mesh_csa_enable)
			mesh_csa_enable = 1;
	}

	if(!IS_ROOT_INTERFACE(priv) || !mesh_csa_enable)
		return ret;
    ...
    {
        if (swchnl_channel == priv->pmib->dot11RFEntry.dot11channel) {
			printk("Detect CSA same channel from other MAP\n");
            return ret;
        }

        if(priv->mesh_CSA_state == MESH_CSA_NONE && priv->mesh_CSA_blocking == 0)
        {
			if(swchnl_mode & RTK_MESH_CSA_CROSSBAND) {
				if(!priv->bss11v_priv_sc)
					return ret;
				...
				priv->mesh_CSA_state = MESH_CSA_CROSSBAND_READY;

				priv->bss11v_priv_sc->mesh_swchnl_ttl = swchnl_ttl - 1;
	            priv->bss11v_priv_sc->mesh_swchnl_flag = swchnl_flag & ~BIT1; /*erase initiator bit*/
	            priv->bss11v_priv_sc->mesh_swchnl_reason = swchnl_reason;
	            priv->bss11v_priv_sc->mesh_swchnl_precedence = swchnl_precedence;
	            priv->bss11v_priv_sc->mesh_swchnl_offset = swchnl_offset;
	            priv->bss11v_priv_sc->mesh_CSA_mode = swchnl_mode & ~RTK_MESH_CSA_CROSSBAND;
	            priv->bss11v_priv_sc->mesh_CSA_channel= swchnl_channel;
				priv->bss11v_priv_sc->mesh_CSA_state = MESH_CSA_READY;

	            printk("Detect Crossband CSA from other MAP: swchnl=%u, mode=%d, offset=%u\n",
 ...
 *	@retval	void
 */
static __always_inline void mesh_auth_timer(unsigned long pVal)
{
...
restart:
    ...
    if (!(list_empty(phead)))
        rtk_mod_timer(&priv->mesh_auth_timer, jiffies + MESH_TIMER_TO);

    RESTORE_INT(flags);
}
DEFINE_TIMER_CALLBACK(mesh_auth_timer);

void init_mesh_auth_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->mesh_auth_timer), mesh_auth_timer, (unsigned long)priv, 0);
}
 ...
 *	@retval	void
 */
static __always_inline void mesh_peer_link_timer(unsigned long pVal)
{
#ifndef SMP_SYNC
    unsigned long		flags;
#endif
    ...
    if (!(list_empty(phead)))
        rtk_mod_timer(&priv->mesh_peer_link_timer, jiffies + MESH_TIMER_TO);

    RESTORE_INT(flags);
}
DEFINE_TIMER_CALLBACK(mesh_peer_link_timer);

void init_mesh_peer_link_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->mesh_peer_link_timer), mesh_peer_link_timer, (unsigned long)priv, 0);
}
...
int start_MeshPeerLink(DRV_PRIV *priv, struct rx_frinfo *pfrinfo, struct stat_info *pstat, UINT16 cap)
{
    ...
    if (0 >= MESH_PEER_LINK_CAP_NUM(priv) || 2 > priv->up_time)
    {
        return FAIL;
    }
    ...
    if (NULL == pstat)
    {
        ...
        //check sa
        if (priv->pmib->dot1180211sInfo.mesh_acl_mode == ACL_ALLOW)	// 1: positive check, only STAs which are in acl_list can be connected.
            res = FAIL;
        else
            res = SUCCESS;
        ...
        while(plist != phead)
        {
            ...
            if (!memcmp((void *)pfrinfo->sa, paclnode->addr, MACADDRLEN))
            {
                ...
                if (paclnode->mode & ACL_DENY)   // deny
                {
                    res = FAIL;
                    break;
                }
...
reInitial:
        ...
        if (!(rtk_timer_pending(&priv->mesh_peer_link_timer)))		// start timer if stop
            rtk_mod_timer(&priv->mesh_peer_link_timer, jiffies + MESH_TIMER_TO);
    ...
    if (0 < cap)  	// ActivePeerLinkOpen (line 10-14, page 113,  D1.0)
    {
        ...
        if (!(rtk_timer_pending(&priv->mesh_auth_timer)))	// start timer if stop
            rtk_mod_timer(&priv->mesh_auth_timer, jiffies + MESH_TIMER_TO);
...
unsigned int OnProbeRsp_MP(DRV_PRIV *priv, struct rx_frinfo *pfrinfo)
{
    ...
    if (1 == GET_MIB(priv)->dot1180211sInfo.mesh_enable)
    {
        ...
        p = get_ie(pframe + WLAN_HDR_A3_LEN + _PROBERSP_IE_OFFSET_, _MESH_ID_IE_, (int *)&len,
                   pfrinfo->pktlen - WLAN_HDR_A3_LEN - _PROBERSP_IE_OFFSET_);
        if(NULL == p ||
              (!rtk_timer_pending(&priv->ss_timer) &&
                  (len != strlen(GET_MIB(priv)->dot1180211sInfo.mesh_id) ||
...
int mesh_open(DRV_PRIV *priv, struct net_device *dev)
{
...
#if defined(RTK_MESH_AODV_STANDALONE_TIMER)
	rtk_timer_setup(&(priv->mesh_expire_timer), mesh_standalone_timer_expire, (unsigned long)priv, 0);
#ifdef __KERNEL__
	rtk_mod_timer(&priv->mesh_expire_timer, jiffies + MESH_AODV_EXPIRE_TO);
#else
	rtk_mod_timer(&priv->mesh_expire_timer, MESH_AODV_EXPIRE_TO);
#endif
#endif //RTK_MESH_AODV_STANDALONE_TIMER
    ...
    #if defined(RTK_MESH_AODV_STANDALONE_TIMER)
		rtk_timer_setup(&(priv->mesh_priv_sc->mesh_expire_timer), mesh_standalone_timer_expire, (unsigned long)priv, 0);
    #ifdef __KERNEL__
		rtk_mod_timer(&priv->mesh_priv_sc->mesh_expire_timer, jiffies + MESH_AODV_EXPIRE_TO);
    #else
		rtk_mod_timer(&priv->mesh_priv_sc->mesh_expire_timer, MESH_AODV_EXPIRE_TO);
    #endif
    #endif //RTK_MESH_AODV_STANDALONE_TIMER

	}
#endif  // CONFIG_RTL_MESH_CROSSBAND
...
#if defined(RTK_MESH_AODV_STANDALONE_TIMER)
    if (rtk_timer_pending(&priv->mesh_expire_timer))
       rtk_del_timer_sync(&priv->mesh_expire_timer);
#if defined(CONFIG_RTL_MESH_CROSSBAND)
    if (IS_DRV_OPEN(priv->mesh_priv_sc) && GET_MIB(priv->mesh_priv_sc)->dot1180211sInfo.mesh_enable) {
        if (rtk_timer_pending(&priv->mesh_priv_sc->mesh_expire_timer))
            rtk_del_timer_sync(&priv->mesh_priv_sc->mesh_expire_timer);
    }
#endif
#endif

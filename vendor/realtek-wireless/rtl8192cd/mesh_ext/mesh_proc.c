...
#endif
{
	...
	if(pstat->isEth == 170)
	{ 
		PRINT_SINGL_ARG("    isEth: ", 1, "%d"); 
	}
	else if(pstat->isEth == 85)
	{
		PRINT_SINGL_ARG("    isEth: ", 0, "%d");
	}
	else
	{
		PRINT_SINGL_ARG("    isEth: ", 0, "%d");
...
#endif
{
	...
	if (1 == GET_MIB(priv)->dot1180211sInfo.mesh_enable) {
		...
		PRINT_SINGL_ARG("      tx_packets:    ", (unsigned long long)priv->mesh_stats.tx_packets, "%llu");
		PRINT_SINGL_ARG("      tx_bytes:      ", (unsigned long long)priv->mesh_stats.tx_bytes, "%llu");
		PRINT_SINGL_ARG("      tx_errors:     ", (unsigned long long)priv->mesh_stats.tx_errors, "%llu");
		PRINT_SINGL_ARG("      rx_packets:    ", (unsigned long long)priv->mesh_stats.rx_packets, "%llu");
		PRINT_SINGL_ARG("      rx_bytes:      ", (unsigned long long)priv->mesh_stats.rx_bytes, "%llu");
		PRINT_SINGL_ARG("      rx_errors:     ", (unsigned long long)priv->mesh_stats.rx_errors, "%llu");
		PRINT_SINGL_ARG("      rx_crc_errors: ", (unsigned long long)priv->mesh_stats.rx_crc_errors, "%llu");
	...
	PRINT_SINGL_ARG("      State:        ", priv->mesh_CSA_state, "%u");
...
#endif
{
    ...
    if (1 == GET_MIB(priv)->dot1180211sInfo.mesh_enable) {
		...
		PRINT_ONE("    path2NexthopMAC: ---", "%s", 1);
        PRINT_ONE("    path2Metric: ---", "%s", 1);
        PRINT_ONE("    path2Hopcount: ---", "%s", 1);
        ...
        {        
            ...
            {			
                ...
                /* Second Path */
                PRINT_ARRAY_ARG("    path2NexthopMAC: ", ptable->path2NexthopMAC, "%02x", MACADDRLEN);
                PRINT_SINGL_ARG("    path2Metric: ", ptable->path2Metric, "%u");
                PRINT_SINGL_ARG("    path2Hopcount: ", ptable->path2Hopcount, "%u");  
                /* End of Second Path */

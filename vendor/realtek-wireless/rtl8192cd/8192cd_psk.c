...
static void ReSendEAPOL(struct rtl8192cd_priv *priv, struct stat_info *pstat);
	...
	*usOutLen = 0;
	...
	if ((priv->pmib->dot1180211AuthEntry.dot11EnablePSK & PSK_WPA2)
		...
		if (priv->pmib->dot1180211AuthEntry.dot11IEEE80211W == NO_MGMT_FRAME_PROTECTION) {
			...
			dot11RSNCapability.field.PBAC = 0; //PBAC
			...
			dot11RSNCapability.field.MFPC = 1; // MFPC
			dot11RSNCapability.field.PBAC = 1; //PBAC
		}
#endif
...
#ifdef CONFIG_IEEE80211R
			if (!FT_ENABLE || (priv->pmib->dot1180211AuthEntry.dot11EnablePSK & PSK_WPA3))
#endif
				ulIELength += 2;	// add PMDID Count (2bytes)
		}
#endif
...
#else
					ulIELength += sizeof(dot11RSNIGTKSuite) + 2; // add PMDID Count (2bytes)
#endif
			}
		}
#endif
...
#ifdef CONFIG_IEEE80211W
	rtk_sha256_prf(keyin, keyinlen, label, data, sizeof(data), tmp, keyoutlen);
#else
...
{
	unsigned char data[2 * ETHER_ADDRLEN + 2 * KEY_NONCE_LEN], tmpPTK[128]={0};
...
#ifdef CONFIG_IEEE80211W
	if (use_sha256) {
		//PMFDEBUG("rtk_sha256_prf\n");
		rtk_sha256_prf(keyin, keyinlen, (unsigned char*)PMK_EXPANSION_CONST, data, sizeof(data),
				   tmpPTK, keyoutlen);
	} else
#endif
	{
...
//			AES_UnWRAP(EapolKeyMsgRecvd.Octet + KeyDataPos, keylen + 8, kek, keklen, tmp2);
		...
		AES_UnWRAP(EapolKeyMsgRecvd.Octet + KeyDataPos, Message_KeyDataLength(EapolKeyMsgRecvd), kek, keklen, tmp2, sizeof(tmp2));
//------------------------- 2005-08-01
...
#endif
		case DOT11_EVENT_CHANNEL_CHANGE:
		{
			unsigned char* tmp = (unsigned char*)data;
			CHANNEL_CHANGE channel_change;
			memset(&channel_change, 0, sizeof(CHANNEL_CHANGE));
			channel_change.EventId = event;
			channel_change.channel = *tmp;
			channel_change.ReasonCode = *(tmp+1);
			DEBUG_INFO("[GENERAL_EVENT]Channel Change to CH%d, reason_code = %d\n", channel_change.channel, channel_change.ReasonCode);
			if (priv->pshare->wlangeneral_pid > 0)
			{
				DOT11_EnQueue((unsigned long)priv, priv->pgeneralevent_queue, (unsigned char*)&channel_change, sizeof(CHANNEL_CHANGE));
			}
			break;
		}
		default:
...
void ToDrv_SetIE(struct rtl8192cd_priv *priv) 
{
	...
	if (priv->wpa_global_info->AuthInfoElement.Length <= 0) {
		DEBUG_ERR("[%s] Unexpected size\n", __func__);
		return;
	}
...
static __always_inline void ResendTimeout(unsigned long task_psta)
{
	...
	ReSendEAPOL(priv, pstat);
}
DEFINE_TIMER_CALLBACK(ResendTimeout);
...
static void reset_sta_info(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	if (rtk_timer_pending(&pInfo->resendTimer)) {
		rtk_del_timer(&pInfo->resendTimer);
	}
	...
	rtk_timer_setup(&(pInfo->resendTimer), ResendTimeout, (unsigned long)pstat, 0);
...
static inline void RTL_SET_BE32(u8 *a, u32 val)
{
        a[0] = (val >> 24) & 0xff;
        a[1] = (val >> 16) & 0xff;
        a[2] = (val >> 8) & 0xff;
...
#define RSN_SELECTOR_PUT(a, val) RTL_SET_BE32((u8 *) (a), (val))
...
static void ReSendEAPOL(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	if (++pstat->wpa_sta_info->resendCnt > MAX_RESEND_NUM) {
			...
			if (pstat->wpa_sta_info->state == PSK_STATE_PTKINITDONE &&
				...
				if (priv->wpa_global_info->GKeyDoneStations == 0 && !priv->wpa_global_info->GkeyReady) {
					...
					if (priv->pmib->dot1180211AuthEntry.dot11GKRekeyTime)
						rtk_mod_timer(&priv->wpa_global_info->GKRekeyTimer, jiffies + RTL_SECONDS_TO_JIFFIES(priv->pmib->dot1180211AuthEntry.dot11GKRekeyTime));
		...
		// Unlock earlier to avoild calling spin_unlock_bh inside spin_lock_irqsave
		SMP_UNLOCK_PSK_RESEND(flags);
...
static __always_inline void GKRekeyTimeout(unsigned long task_priv)
{
...
DEFINE_TIMER_CALLBACK(GKRekeyTimeout);

static void SendEAPOL(struct rtl8192cd_priv *priv, struct stat_info *pstat, int resend)
{
	volatile OCTET_STRING	IV, RSC, KeyID, MIC, KeyData, EAPOLMsgSend, EapolKeyMsgSend;
	unsigned char	my_11w = priv->pmib->dot1180211AuthEntry.dot11IEEE80211W;
	unsigned char	my_sha256 = priv->pmib->dot1180211AuthEntry.dot11EnableSHA256;
...
//???
			...
			if ( support_wpa3(priv) && priv->pmib->dot1180211AuthEntry.dot11TDBitmap != 0 ) {
				char td_bitmp = BIT(0);

				key_data_pos = wpa_add_kde(key_data_pos,
					WFA_KEY_DATA_TRANSITION_DISABLE,
					&priv->pmib->dot1180211AuthEntry.dot11TDBitmap,
					1, NULL, 0);
 			}
#endif
...
#ifdef ENABLE_SAE_H2E
			if ( support_wpa3(priv) && priv->pmib->dot1180211AuthEntry.dot11SAEPwe != 0 ) {
				u8 tmp[2];
				size_t flen, frlen=0;

				flen = construct_rsnxe_ie(tmp);
				key_data_pos = set_ie(key_data_pos,
					_RSNX_IE_, flen, tmp, &frlen);
			}
#endif
...
//	pskb->cb[0] = 7;	// use highest priority to xmit
	...
	rtk_mod_timer(&pstat->wpa_sta_info->resendTimer, jiffies + RESEND_TIME);
}
...
void ClientSendEAPOL(struct rtl8192cd_priv *priv, struct stat_info *pstat, int resend)
{
		...
		if (!pStaInfo->clientHndshkDone) {
			if (pStaInfo->clientHndshkProcessing < fourWaystep3) {
				...
				if ( support_wpa3(priv) && priv->pmib->dot1180211AuthEntry.dot11SAEPwe != 0 ) {
					...
					flen = construct_rsnxe_ie(tmp);
					h2e_ie = pGblInfo->AuthInfoElement.Octet +
						pGblInfo->AuthInfoElement.Length;

					set_ie(h2e_ie, _RSNX_IE_, flen,
						tmp, &pGblInfo->AuthInfoElement.Length);

					Message_setKeyDataLength(EapolKeyMsgSend,
						pGblInfo->AuthInfoElement.Length);

					Message_setKeyData(EapolKeyMsgSend,
						pGblInfo->AuthInfoElement);
	...
	#ifdef CONFIG_IEEE80211R_CLI
		if(FT_ENABLE) {
			panic_printk("FT CLI mode:  Extended rtk_mod_timer\n");
			rtk_mod_timer(&pStaInfo->resendTimer, jiffies + RTL_SECONDS_TO_JIFFIES(5));
		} else
	#endif
			rtk_mod_timer(&pStaInfo->resendTimer, jiffies + RESEND_TIME);
	}
	//--------------------------------------------------------------send_packet
}
#endif // CLIENT_MODE
...
static void UpdateGK(struct rtl8192cd_priv *priv)
{
	...
	if (pGblInfo->GTKAuthenticator && (pGblInfo->GTKRekey || pGblInfo->GInitAKeys)) {
		...
		if (rtk_timer_pending(&pGblInfo->GKRekeyTimer))
			rtk_del_timer_sync(&pGblInfo->GKRekeyTimer);
...
#ifdef CTC_WIFI_DIAG
				ctcwifi_assoc_err(priv, pstat->cmn_info.mac_addr, "Authentication failed [4-2: ERROR_NONEEQUL_REPLAYCOUNTER]\n");
#endif
				break;
			}
...
#ifdef ENABLE_SAE_H2E
			if ( pstat->with_h2e )  {
				OCTET_STRING tmpKeyData = Message_KeyData(pStaInfo->EapolKeyMsgRecvd, 
					Message_KeyDataLength(pStaInfo->EapolKeyMsgRecvd));

				unsigned int status, reason = unspec_reason;

				if ( sae_check_imd_4way(priv, pstat, 
					tmpKeyData.Octet, tmpKeyData.Length, &status) < 0 ) {

					DRV_RT_TRACE(priv, DRV_DBG_CONN_INFO, DRV_DBG_SERIOUS, 
						"SAE : no rsnxe for h2e sta, reason=%d\n", reason);
...
#ifdef CTC_WIFI_DIAG
				ctcwifi_assoc_err(priv, pstat->cmn_info.mac_addr, "Authentication failed [4-2: ERROR_MIC_FAIL]\n");
#endif
#if defined(CONFIG_WLAN_STATS_EXTENTION)
				priv->ext_stats.total_mic_fail++;
				priv->ext_stats.total_psk_fail++;
...
#endif
#ifdef RTK_WLAN_EVENT_INDICATE
				rtk_wlan_event_indicate(priv->dev->name, WIFI_PEER_CONNECT_FAIL, pstat->cmn_info.mac_addr, RSN_MIC_failure);
#endif
...
cont_msg:
			...
			// delete resend timer
			SMP_LOCK_PSK_RESEND(flags);
			if (rtk_timer_pending(&pStaInfo->resendTimer)){
				rtk_del_timer(&pStaInfo->resendTimer);
			}
			SMP_UNLOCK_PSK_RESEND(flags);
...
#ifdef CTC_WIFI_DIAG
    			    ctcwifi_assoc_err(priv, pstat->cmn_info.mac_addr, "Authentication failed [4-2 in akmsm_PTKINITNEGOTIATING: ignore older 4-2 packets!]\n");
#endif
    			    break;
                }
			}
...
#ifdef CTC_WIFI_DIAG
				ctcwifi_assoc_err(priv, pstat->cmn_info.mac_addr, "Authentication failed [4-4: RSN_MIC_failure]\n");
#endif
...
//				global->portSecure = TRUE;
				...
				if (pGblInfo->GKeyDoneStations == 0 && !pGblInfo->GkeyReady) {
					...
					// start groupkey rekey timer
					if (priv->pmib->dot1180211AuthEntry.dot11GKRekeyTime)
						rtk_mod_timer(&pGblInfo->GKRekeyTimer, jiffies + RTL_SECONDS_TO_JIFFIES(priv->pmib->dot1180211AuthEntry.dot11GKRekeyTime));
				}
#if 0
				if (global->RSNVariable.PMKCached) {
...
#ifdef CTC_WIFI_DIAG
				ctcwifi_assoc_err(priv, pstat->cmn_info.mac_addr, "Authentication failed [2-2 ERROR_LARGER_REPLAYCOUNTER]\n");
#endif
#if defined(CONFIG_WLAN_STATS_EXTENTION)
				priv->ext_stats.total_psk_fail++;
#endif
...
#ifdef CTC_WIFI_DIAG
				ctcwifi_assoc_err(priv, pstat->cmn_info.mac_addr, "Authentication failed [2-way handshake failed! (2-2: MIC error)]\n");
#endif
	...
	if (Message_KeyType(pStaInfo->EapolKeyMsgRecvd) == type_Pairwise) {
		...
		if(FT_ENABLE && pStaInfo->isFT) {
					...
					if ( *pGTK_KDE == WPA2_ELEMENT_ID ) {
						...
						if(priv->bss_support_pmf){
							...
							if(!memcmp((pGTK_KDE+8+gtk_len), IGTK_KDE_TYPE, sizeof(IGTK_KDE_TYPE))){
								...
								dump_hex("IGTK",priv->pmib->dot11IGTKTable.dot11EncryptKey.dot11TTKey.skey);
							#endif
							}
							else{
			...
			if ( pstat->with_h2e )  {
				...
				if ( sae_check_imd_4way(priv, pstat,
					tmpKeyData.Octet, tmpKeyData.Length, &status) < 0 ) {
					DEBUG_ERR("SAE: no rsnxe for h2e sta:%p, %d\n",
...
void psk_init(struct rtl8192cd_priv *priv)
{
	...
	if (OPMODE & WIFI_AP_STATE) {
		...
		rtk_timer_setup(&(pGblInfo->GKRekeyTimer), GKRekeyTimeout, (unsigned long)priv, 0);
	}
#if 0
...
// button 2009.05.21
	...
	case DOT11_EVENT_REASSOCIATION_IND:
		...
		if (OPMODE & WIFI_AP_STATE) {
			...
			rtk_mod_timer(&pstat->wpa_sta_info->resendTimer, jiffies + WAIT_EAP_TIME);
#endif
		break;
	...
	unsigned char *pbuf, unsigned int limit, unsigned int *status)
{
	...
	// check rsnxe
	p = get_ie(pbuf, _RSNX_IE_, &len, limit);
	if ( !p ) {
		DEBUG_INFO("Err: no rsnxe found \n");
		return 0;
	}

	if ((len+2) > 16 ) {
		DEBUG_ERR("Err: rsnxe too big \n");
		return -1;
	}

	len += 2; // with ie header

	if (memcmp(pstat->rsnxe_ie, p, len) != 0) {
		DEBUG_ERR("Err: rsnxe_ie not match \n");
		return -1;
	}

	for (i = 0; i < len; i++) {
		DEBUG_ERR(" %02X : %02X \n", *(p+i), pstat->rsnxe_ie[i] );
	}

	DEBUG_ERR("compare rsnxe done OK !! \n");
	...
	unsigned char *pmkid_in)
{
	...
	pos += MACADDRLEN;
	rtk_sha256_prf(r0kh->pmk_r0, PMK_LEN, (unsigned char *)FT_PMKR1_CONST,
	...
	if (rtk_sha256_vector(4, data_vec, len_vec, tmpBuf)) {
...
void derive_ft_ptk(struct r1_key_holder *r1kh, unsigned char *snonce, unsigned char *anonce, unsigned char *keyout, unsigned int keyoutlen)
{
	...
	pos += MACADDRLEN;
	rtk_sha256_prf(r1kh->pmk_r1, PMK_LEN, (unsigned char *)FT_PMK_EXPANSION_CONST, 
...
static void CalcFTPTK(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *keyout, int keyoutlen)
{
	...
	*pos++ = R0KH_ID_LEN;
	...
	pos += MACADDRLEN;
	rtk_sha256_prf(pStaInfo->PMK, PMK_LEN, (unsigned char*)FT_PMKR0_CONST, data, pos - data, tmpBuf, 48);	
	...
	len_vec[1] = PMKID_LEN;
	if (rtk_sha256_vector(2, data_vec, len_vec, tmpBuf)) {
		DEBUG_ERR("Error: sha256 fail\n");
		return;
	}
	...
	pos += MACADDRLEN;
	rtk_sha256_prf(pmk_r0, PMK_LEN, (unsigned char *)FT_PMKR1_CONST,
	...
	pos += MACADDRLEN;
	rtk_sha256_prf(pmk_r1, PMK_LEN, (unsigned char *)FT_PMK_EXPANSION_CONST,
    ...
    unsigned int r0kh_id_len, unsigned char *pmk_r0_name, unsigned char *cur_pmk_r1_name)
{
	...
	*pos++ = r0kh_id_len;
	...
	rtk_sha256_prf(pmkBuf, PMK_LEN, (unsigned char*)FT_PMKR0_CONST, data, pos - data, tmpBuf, 48);	
	memcpy(pmk_r0, tmpBuf, PMK_LEN);
	...
	if(r0kh_id_len < MACADDRLEN*2 || !rtl_string_to_hex(r0kh_id, bssid_buf, MACADDRLEN*2)) {
        ...
        memcpy(pos, pstat->cmn_info.mac_addr, MACADDRLEN);
        pos += MACADDRLEN;
        rtk_sha256_prf(pmk_r0, PMK_LEN, (unsigned char *)FT_PMKR1_CONST, data, pos - data, tmpBuf, 32);
        memcpy(pmk_r1, tmpBuf, PMK_LEN);
...
int ft_check_ft_auth_rsp(struct rtl8192cd_priv *priv, unsigned char *pbuf, unsigned int limit, unsigned int *status, struct rx_frinfo *pfrinfo)
{
	...
	if (IEEE8021X_FUN) {
		...
		memcpy(pos, GET_MY_HWADDR, MACADDRLEN);
		pos += MACADDRLEN;
		rtk_sha256_prf(r0kh->pmk_r0, PMK_LEN, (unsigned char *)FT_PMKR1_CONST, data, pos - data, tmpBuf, 32);
		memcpy(cur_pmk_r1, tmpBuf, PMK_LEN);

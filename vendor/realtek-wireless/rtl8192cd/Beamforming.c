...
DECLARE_TIMER_CALLBACK(beamform_SoundingTimerCallback);
DECLARE_TIMER_CALLBACK(beamform_SoundingTimeout);
...
#ifdef CONFIG_WLAN_HAL_8812FE
	if(GET_CHIP_VER(priv) == VERSION_8812F) {
		Beamforming_NDPARate_8812F(priv, 0, 0, priv->pshare->rf_ft_var.force_ndpa_rate);
	}		
#endif
...
)
{
	...
	if (priv->pshare->rf_ft_var.set_ndp_rate)
		return priv->pshare->rf_ft_var.set_ndp_rate;
#endif
		...
		if((GET_CHIP_VER(priv) == VERSION_8822B) || (GET_CHIP_VER(priv) == VERSION_8812F) || (GET_CHIP_VER(priv) == VERSION_8814B)) {
			...
			if (pBeamInfo->beamformee_mu_cnt >= 2) {
				if (!priv->mu_bkup_agg_num) {
					priv->mu_bkup_agg_num = RTL_R16(0x4ca);
#ifdef CONFIG_WLAN_HAL_8814BE
					if(GET_CHIP_VER(priv) == VERSION_8814B){
						RTL_W16(0x4ca, 0x3f3f);
						priv->mu_bkup_reg_63e = RTL_R16(0x63e);
						RTL_W16(0x63e, 0x1313);
						config_phydm_mu_anapar_8814b(ODMPTR, pBeamInfo->beamformee_mu_cnt);
					}else
#endif
				...
				if(GET_CHIP_VER(priv) == VERSION_8812F){
					// use TBTT RA
					//printk("[%s] use TBTT RA mode \n", __func__);
					phydm_ra_mode_selection(ODMPTR,  1 );
					if(pEntry->pSTA->IOTPeer == HT_IOT_PEER_REALTEK){
						//close reg and do IQK, DPK
						//printk("[%s] close reg and do Tssi_Off_Flow\n", __func__);
						RTL_W32(0x1e7c, RTL_R32(0x1e7c) & (~BIT30));
						Tssi_Disable_Flow(priv);
			...
			if (GET_CHIP_VER(priv) == VERSION_8814B && priv->pmib->dot11RFEntry.adaptive_ant) {
				unsigned int ant_map = orSTABitMap(&priv->pshare->ant_Bitmap);
				if (!ant_map) {
					if (!priv->ant_adapt_mu && pBeamInfo->beamformee_mu_cnt >= 2) {
						priv->ant_adapt_mu = 1;
		...
		if(pBeamformerEntry == NULL)
		{
			...
			if(pBeamformerEntry == NULL) {
				...
				if(IS_VXD_INTERFACE(priv)) {
					...
					if(reuse_idx < BEAMFORMER_ENTRY_NUM) {
...
//	pBeamTimerInfo->Idx = Idx;
	...
	{
		rtk_mod_timer(&pBeamInfo->BeamformingTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(pBeamTimerInfo->BeamPeriod));
	}
	else
	{
	...
	)
{
	...
	if(pBeamTimerInfo->Mode == SOUNDING_SW_VHT_TIMER || pBeamTimerInfo->Mode == SOUNDING_SW_HT_TIMER)
	{
		rtk_del_timer(&pBeamInfo->BeamformingTimer);

		if ((GET_CHIP_VER(priv) == VERSION_8822B) || GET_CHIP_VER(priv) == VERSION_8812F) {
			rtk_del_timer(&pBeamInfo->BFSoundingTimeoutTimer);
...
#ifdef CONFIG_WLAN_HAL_8192EE
	if ((OPMODE & WIFI_AP_STATE) && (priv->pshare->soundingLock) && (GET_CHIP_VER(priv)== VERSION_8192E)) {
		pEntry->Sounding_En = 0;
		PHYDM_DBG(ODMPTR, DBG_TXBF, "%s, %d\n", __FUNCTION__, __LINE__);
		return FALSE;
	}
			...
			if (pBeamformingTimerInfo->Mode == SOUNDING_SW_VHT_TIMER || pBeamformingTimerInfo->Mode == SOUNDING_SW_HT_TIMER) {
				if (shortenSoundingPeriod(priv)) {
					if (pBeamformingInfo->BeamformingPeriodState == 1 || pBeamformingInfo->BeamformingPeriodState == 2)
						rtk_mod_timer(&pBeamformingInfo->BeamformingTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(pBeamformingTimerInfo->BeamPeriod/100));
					else   // pBeamformingInfo->BeamformingPeriodState == 3
						rtk_mod_timer(&pBeamformingInfo->BeamformingTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(pBeamformingTimerInfo->BeamPeriod/200));
				}
				else
					rtk_mod_timer(&pBeamformingInfo->BeamformingTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(pBeamformingTimerInfo->BeamPeriod));
...
static void Beamforming_SWTimerCallback(unsigned long task_priv)
#else
static __always_inline void Beamforming_SWTimerCallback(unsigned long task_priv)
#endif
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;
	do_beamforming(priv);
}
DEFINE_TIMER_CALLBACK(Beamforming_SWTimerCallback);
	...
	)
{
	...
	if((GET_CHIP_VER(priv)== VERSION_8822B) || (GET_CHIP_VER(priv)== VERSION_8812F) || (GET_CHIP_VER(priv) == VERSION_8814B)) {
		...
		rtk_timer_setup(&(pBeamInfo->BeamformingTimer), beamform_SoundingTimerCallback, (unsigned long)priv, 0);

		rtk_timer_setup(&(pBeamInfo->BFSoundingTimeoutTimer), beamform_SoundingTimeout, (unsigned long)priv, 0);
		beamform_InitSoundingVars(priv);

		if (priv->pshare->rf_ft_var.txbf_iqgen_keep_on) {
#ifdef CONFIG_WLAN_HAL_8812FE
			if (GET_CHIP_VER(priv) == VERSION_8812F) {
				printk("[%s] phydm_txbf_rfmode(1)\n", __func__);
				phydm_txbf_rfmode(ODMPTR, 1, 1);
			}
#endif
		}
	}
	else
	{
		...
		rtk_timer_setup(&(pBeamInfo->BeamformingTimer), Beamforming_SWTimerCallback, (unsigned long)priv, 0);
#elif defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
		rtk_timer_setup(&(pBeamInfo->BeamformingTimer), timer_event_timer_fn, (unsigned long)(&(pBeamInfo->BeamformingTimer_event)), 0);
	...
	)
{
	...
	rtk_del_timer(&pBeamformingInfo->BeamformingTimer);
	if ((GET_CHIP_VER(priv) == VERSION_8822B) || (GET_CHIP_VER(priv) == VERSION_8812F)) {
		rtk_del_timer(&pBeamformingInfo->BFSoundingTimeoutTimer);
	}
		...
		if((GET_CHIP_VER(priv)== VERSION_8822B) || (GET_CHIP_VER(priv)== VERSION_8812F) || (GET_CHIP_VER(priv)== VERSION_8814B)) {
			...
			//if(pBeamInfo->BeamformCap & (BEAMFORMER_CAP_HT_EXPLICIT|BEAMFORMER_CAP_VHT_SU|BEAMFORM_CAP_VHT_MU_BFER))
			{
				if(pBeamInfo->SoundingInfoV2.State == SOUNDING_STATE_NONE)
				{
					...
					rtk_mod_timer(&pBeamInfo->BeamformingTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(2000)); // Start sounding after 2 sec.
				}
			}
		}
	}
	...
	)
{
        ...
        priv->pmib->dot11RFEntry.adaptive_ant) 
    {
        unsigned int ant_map = orSTABitMap(&priv->pshare->ant_Bitmap);
        if (!ant_map) {
            if (priv->ant_adapt_mu && pBeamformingInfo->beamformee_mu_cnt < 2) {
                priv->ant_adapt_mu = 0;
	...
	)
{
	...
	if(pbuf)
	{
		...
		txinsn.fr_len = 0;
		txinsn.fixed_rate = 1;	
		if (priv->pshare->rf_ft_var.set_bfrp_rate)
			txinsn.tx_rate = priv->pshare->rf_ft_var.set_bfrp_rate;
		else
...
{
	if (priv->pshare->rf_ft_var.set_bfrp_rate)
		return priv->pshare->rf_ft_var.set_bfrp_rate;
...
void Tssi_Enable_Flow(	struct rtl8192cd_priv *priv){
	RTL_W32(0x1c38, 0xf7d5005e);
	RTL_W32(0x1860, 0x2);
	RTL_W32(0x1830, 0x700b8041);
	RTL_W32(0x1830, 0x701f0042);
	RTL_W32(0x1830, 0x702f0042);
	RTL_W32(0x1830, 0x703f0042);
	RTL_W32(0x1830, 0x704f0042);
	RTL_W32(0x1830, 0x705b8041);
	RTL_W32(0x1830, 0x706f0042);
	RTL_W32(0x4130, 0x700b8041);
	RTL_W32(0x4130, 0x701f0042);
	RTL_W32(0x4130, 0x702f0042);
	RTL_W32(0x4130, 0x703f0042);
	RTL_W32(0x4130, 0x704f0042);
	RTL_W32(0x4130, 0x705b8041);
	RTL_W32(0x4130, 0x706f0042);
	RTL_W32(0x1830, 0x707b8041);
	RTL_W32(0x1830, 0x708b8041);
	RTL_W32(0x1830, 0x709b8041);
	RTL_W32(0x1830, 0x70ab8041);
	RTL_W32(0x1830, 0x70bb8041);
	RTL_W32(0x1830, 0x70cb8041);
	RTL_W32(0x1830, 0x70db8041);
	RTL_W32(0x1830, 0x70eb8041);
	RTL_W32(0x1830, 0x70fb8041);
	RTL_W32(0x4130, 0x707b8041);
	RTL_W32(0x4130, 0x708b8041);
	RTL_W32(0x4130, 0x709b8041);
	RTL_W32(0x4130, 0x70ab8041);
	RTL_W32(0x4130, 0x70bb8041);
	RTL_W32(0x4130, 0x70cb8041);
	RTL_W32(0x4130, 0x70db8041);
	RTL_W32(0x4130, 0x70eb8041);
	RTL_W32(0x4130, 0x70fb8041);
}

void Tssi_Disable_Flow( struct rtl8192cd_priv *priv){
	RTL_W32(0x1c38, 0xffa1005e);
	RTL_W32(0x1830, 0x700b8041);
	RTL_W32(0x1830, 0x70144041);
	RTL_W32(0x1830, 0x70244041);
	RTL_W32(0x1830, 0x70344041);
	RTL_W32(0x1830, 0x70444041);
	RTL_W32(0x1830, 0x705b8041);
	RTL_W32(0x1830, 0x70644041);
	RTL_W32(0x4130, 0x700b8041);
	RTL_W32(0x4130, 0x70144041);
	RTL_W32(0x4130, 0x70244041);
	RTL_W32(0x4130, 0x70344041);
	RTL_W32(0x4130, 0x70444041);
	RTL_W32(0x4130, 0x705b8041);
	RTL_W32(0x4130, 0x70644041);
	RTL_W32(0x1830, 0x707b8041);
	RTL_W32(0x1830, 0x708b8041);
	RTL_W32(0x1830, 0x709b8041);
	RTL_W32(0x1830, 0x70ab8041);
...
#if defined(CONFIG_WLAN_HAL_8812FE)
		if (GET_CHIP_VER(priv) == VERSION_8812F && priv->pmib->dot11RFEntry.dot11channel>=149) {
			if(STAGrpCnt ){
				priv->pshare->isSTAGrp =1;				
			}else{
				priv->pshare->isSTAGrp =0;
			}
		}
#endif
	}
...
#if (MU_BEAMFORMING_SUPPORT == 1)	
            pBeamInfo->BeamformeeEntry[idx].SoundPeriod = priv->pshare->rf_ft_var.mutime;
#endif
		}
	}

#if (MU_BEAMFORMING_SUPPORT == 1)	
    pBeamInfo->SoundingInfoV2.MinSoundingPeriod = priv->pshare->rf_ft_var.mutime;
#endif
}
...
static void do_beamform_sounding(struct rtl8192cd_priv *priv)
{
	...
	if (priv->pshare->rf_ft_var.disable_snd == 1) {
		if (!priv->pshare->rf_ft_var.manual_snd) {
			rtk_mod_timer(&pBeamInfo->BeamformingTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(1000));
			return;
		}
		else
	...
	do
	{
		if(pSoundingInfo->State == SOUNDING_STATE_INIT)
		{
			...
			else if(rtStatus == RT_STATUS_FAILURE)
			{
				...
				if(WaitCnt < 500)
				{
					...
					rtk_mod_timer(&pBeamInfo->BeamformingTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(2));
				}
				else
				{
					WaitCnt=0;
...
#endif
				TimeoutPeriod = priv->pshare->rf_ft_var.MUsndperiod;
				bSetTimeoutTimer = TRUE;
			}
		}
		...
		if(bSetTimeoutTimer)
		{
			rtk_mod_timer(&pBeamInfo->BFSoundingTimeoutTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(TimeoutPeriod)); //eric-mu
			break;
...
static __always_inline void beamform_SoundingTimerCallback(unsigned long task_priv)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;

	do_beamform_sounding(priv);
}
DEFINE_TIMER_CALLBACK(beamform_SoundingTimerCallback);

static __always_inline void beamform_SoundingTimeout(unsigned long task_priv)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;
	PRT_BEAMFORMING_INFO 		pBeamInfo = &(priv->pshare->BeamformingInfo);
	PRT_SOUNDING_INFOV2			pSoundingInfo = &(pBeamInfo->SoundingInfoV2);
	...
	if(!beamform_IsLastSoundingPeer(priv))
	{
		...
		)
		{
			...
			if(NextSoundPeriod)
				rtk_mod_timer(&pBeamInfo->BeamformingTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(NextSoundPeriod));
			else
				do_beamform_sounding(priv);
		}
		else
...
DEFINE_TIMER_CALLBACK(beamform_SoundingTimeout);
...
#ifdef RTK_AC_SUPPORT	
	mimo = get_sta_vht_mimo_mode(pstat);
#endif

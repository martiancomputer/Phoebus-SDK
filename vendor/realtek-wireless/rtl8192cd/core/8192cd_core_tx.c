...
#if !defined(MCAST2UI_REFINE) && !defined(CONFIG_TX_BCAST2UNI)
 ...
 */
	...
	if (skb) {
#ifdef CMCC_CTC_QOS
	    if ((skb->cb[33] == 0x22) && (skb->cb[34] != 0x22)) {
			priv->ext_stats.cb_data1_drop++;
		}
#endif
...
int rtl88XX_signin_txdesc_amsdu(struct rtl8192cd_priv *priv, struct tx_insn* txcfg)
{
    ...
    if (txcfg->aggre_en == FG_AGGRE_MSDU_LAST) {
#ifdef SW_TXQ_ATF
		swq_atf_calc_hwq_pktsz(pdescinfo, TRUE);
#endif
...
int rtl8192cd_signin_txdesc_amsdu(struct rtl8192cd_priv *priv, struct tx_insn* txcfg)
{
	...
	pbuf = ((struct sk_buff *)txcfg->pframe)->data;
	tx_len = ((struct sk_buff *)txcfg->pframe)->len;
	tmpphyaddr = get_physical_addr(priv, pbuf, tx_len, DMA_TO_DEVICE);
#ifdef USE_RTL8186_SDK
	rtl_cache_sync_wback(priv, (unsigned long)pbuf, tx_len, DMA_TO_DEVICE);
#endif
...
#if !defined(USE_RTL8186_SDK) && CONFIG_WLAN_NOT_HAL_EXIST
	rtl_cache_sync_wback(priv, (unsigned long)bus_to_virt(dma_txhead[*tx_head]-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), sizeof(struct tx_desc), DMA_TO_DEVICE);
#endif
...
//	pdescinfo->len = txcfg->fr_len;
	...
	if (RSVQ_ENABLE && IS_RSVQ(q_num))
		...
		rtl_cache_sync_wback(priv, (unsigned long)bus_to_virt(priv->amsdu_first_dma_desc-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), sizeof(struct tx_desc), DMA_TO_DEVICE);
#endif
		tx_poll(priv, q_num);
	}
...
//    }
    ...
    if (txcfg->privacy) {
       ...
       txcfg->use_sw_enc = UseSwCrypto(priv, txcfg->pstat, FALSE, 0);
#else
       txcfg->use_sw_enc = UseSwCrypto(priv, txcfg->pstat, FALSE);
#endif		
    }
...
//        }
    ...
    if (txcfg->aggre_en == FG_AGGRE_MSDU_FIRST) {
		...
		if (pskb->cb[_SKB_CB_FLAGS] & _SKB_CB_FLAGS_AMSDU_SEG_LIST) {
			...
			while (pskb) {
				next_skb = pskb->next;
				ieee8022_llc_snap_encapsulate(pskb, pstat->cmn_info.mac_addr);
				txcfg->pframe = pskb;
				txcfg->aggre_en = (next_skb ? FG_AGGRE_MSDU_MIDDLE : FG_AGGRE_MSDU_LAST);
				if (txcfg->aggre_en == FG_AGGRE_MSDU_MIDDLE) {
					if (pskb->len & 0x3)
						skb_put(pskb, 4-(pskb->len & 0x3));
				}
	...
	if (txcfg->one_txdesc) {
		...
		if (GET_TX_EARLY_MODE) {
			...
			pdesc->Dword10 = set_desc(get_physical_addr(priv, txcfg->phdr-8,
				(get_desc(pdesc->Dword7)&TX_TxBufSizeMask), DMA_TO_DEVICE));
...
/*
#ifdef USE_RTL8186_SDK
	rtl_cache_sync_wback(priv, get_desc(pdesc->Dword8), (get_desc(pdesc->Dword7)&TX_TxBufferSizeMask), DMA_TO_DEVICE);
#endif
*/
	...
	pdesc->Dword10 = set_desc(get_physical_addr(priv, pskb->data,
		(get_desc(pdesc->Dword7)&0x0fff), DMA_TO_DEVICE));
...
/*
#ifndef USE_RTL8186_SDK
	rtl_cache_sync_wback(priv, dma_txhead[*tx_head], sizeof(struct tx_desc), DMA_TO_DEVICE);
#else
	rtl_cache_sync_wback(priv, get_desc(pdesc->Dword8), (get_desc(pdesc->Dword7)&TX_TxBufferSizeMask), DMA_TO_DEVICE);
#endif
*/
	...
	rtl_cache_sync_wback(priv, (unsigned long)bus_to_virt(dma_txhead[*tx_head]-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), sizeof(struct tx_desc), DMA_TO_DEVICE);
#endif
	rtl_cache_sync_wback(priv, (unsigned long)bus_to_virt(get_desc(pdesc->Dword10)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), (get_desc(pdesc->Dword7)&TX_TxBufSizeMask), DMA_TO_DEVICE);
...
one_txdesc:
	...
	rtl_cache_sync_wback(priv, (unsigned long)bus_to_virt(get_desc(pfrstdesc->Dword10)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), (get_desc(pfrstdesc->Dword7)&TX_TxBufSizeMask), DMA_TO_DEVICE);
...
#ifndef USE_RTL8186_SDK
	rtl_cache_sync_wback(priv, (unsigned long)bus_to_virt(pfrst_dma_desc-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), sizeof(struct tx_desc), DMA_TO_DEVICE);
#endif
...
#endif
{
	...
	{
		int len = sizeof(struct wlan_ethhdr_t)-offset;
		
		if (aggre_en > FG_AGGRE_MSDU_FIRST) {
			unsigned short protocol = ntohs(*(u16 *)(hdr + ETH_ALEN*2));
			if (protocol + WLAN_ETHHDR_LEN <= WLAN_MAX_ETHFRM_LEN)
				len -= 2;	// Ignore length check on AMSDU_[MIDDLE|LAST]
		}
			
		idx = pstat->tx_sc_amsdu_idx_cache;
		for (i = TX_SC_ENTRY_NUM; i < MAX_TXSC_ENTRY; i++) {
			if (!rtk_memcmp(hdr+offset, (unsigned char *)(&ptxsc_entry[idx].ethhdr)+offset, len)) {
...
void rtl8192cd_skb_recycle(unsigned long task_priv)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;

	skb_queue_recycle(priv->pshare->skb_recycle_queue);
	priv->pshare->skb_recycle_trigger = 0;
}
#endif
	...
	while (tail != head)
	{
		...
		if (IS_PCIBIOS_TYPE) {
			if (_RESERVED_FRAME_TYPE_ != pdescinfo->type) {
				dma_unmap_single(&priv->pshare->pdev->dev, pdescinfo->paddr, pdescinfo->len, DMA_TO_DEVICE);
			}
		}
#endif

#ifdef SW_TXQ_ATF
		swq_atf_calc_hwq_pktsz(pdescinfo, FALSE);
#endif
...
#endif
		if (pdescinfo->type == _PRE_ALLOCHDR_)
		{
#ifdef AP_SWPS_OFFLOAD		
...
#ifdef SMP_SKB_RECYCLE_OFFLOAD
						skb_queue_tail(priv->pshare->skb_recycle_queue, (struct sk_buff *)(pdescinfo->buf_pframe[cnt]));
#ifdef ENABLE_RTL_SKB_STATS
						rtl_atomic_dec(&pdescinfo->priv->rtl_tx_skb_cnt);
#endif

#else // !SMP_SKB_RECYCLE_OFFLOAD
#ifdef ENABLE_RTL_SKB_STATS	
						rtl_kfree_skb(pdescinfo->priv, (struct sk_buff *)(pdescinfo->buf_pframe[cnt]), _SKB_TX_IRQ_);
#else
						rtl_kfree_skb(priv, (struct sk_buff *)(pdescinfo->buf_pframe[cnt]), _SKB_TX_IRQ_);
#endif
#endif // SMP_SKB_RECYCLE_OFFLOAD
					} else
						priv->ext_stats.freeskb_err++;
#else
// for debug ------------
...
#if defined(CONFIG_NET_PCI) && !defined(USE_RTL8186_SDK)
					if (IS_PCIBIOS_TYPE) {
						dma_unmap_single(&priv->pshare->pdev->dev, pdescinfo->buf_paddr_amsdu[cnt], pdescinfo->buf_len_amsdu[cnt], DMA_TO_DEVICE);
					}
#endif
...
#ifdef SMP_SKB_RECYCLE_OFFLOAD
							skb_queue_tail(priv->pshare->skb_recycle_queue, (struct sk_buff *)(pdescinfo->buf_pframe_amsdu[cnt]));
#ifdef ENABLE_RTL_SKB_STATS
							rtl_atomic_dec(&pdescinfo->priv->rtl_tx_skb_cnt);
#endif
...
#ifdef SMP_SKB_RECYCLE_OFFLOAD
	if (recycleCnt) {
		if (!priv->pshare->skb_recycle_trigger) {
			priv->pshare->skb_recycle_trigger = 1;
			smp_call_function_single_async(SKB_RECYCLE_CPU, &priv->pshare->skb_recycle_csd);
		}
	}
#endif

	return (recycleCnt ? 1:0);
}
...
#ifndef WIFI_RX_PROCESS_ALL_QUEUE_OFFLOAD
	...
	if (txcfg->pstat)
	{
...
#endif			
		{
				...
				txcfg->privacy == _WEP_104_PRIVACY_)
			{
				...
				picvdesc->Dword10 = set_desc(get_physical_addr(priv, picv, txcfg->icv, DMA_TO_DEVICE));
			...
			else if (txcfg->privacy == _CCMP_PRIVACY_)
			{
				...
				// set TxBufferAddr
				pmicdesc->Dword10= set_desc(get_physical_addr(priv, pmic, txcfg->mic, DMA_TO_DEVICE));
...
init_deschead:
	...
	for (i=0; i<flush_num; i++)
		rtl_cache_sync_wback(priv, flush_addr[i], flush_len[i], DMA_TO_DEVICE);
    ...
    if (is_MCS_rate(txcfg->tx_rate))    // HT rates
    {
        ...
        if (txcfg->pstat && (txcfg->pstat->aggre_mthd & AGGRE_MTHD_MPDU) && txcfg->aggre_en) {
            ...
            if (txcfg->pstat && txcfg->pstat->ADDBA_ready[TID] && !txcfg->pstat->low_tp_disable_ampdu) {
				...
				if (priv->pshare->rf_ft_var.ampdu_density != 0xff) {
                ...
                } else if (txcfg->privacy) {
#if 1   /* TODO: for test two STA TP*/
#if defined(CONFIG_WLAN_HAL_8814AE) 			
					if ((GET_CHIP_VER(priv) == VERSION_8814A))
					{
						if ((txcfg->pstat->ht_cap_buf.ampdu_para & _HTCAP_AMPDU_SPC_MASK_) >> _HTCAP_AMPDU_SPC_SHIFT_)
							pdesc_data->ampduDensity = ((txcfg->pstat->ht_cap_buf.ampdu_para & _HTCAP_AMPDU_SPC_MASK_) >> _HTCAP_AMPDU_SPC_SHIFT_);
						else if (txcfg->pstat->ht_cap_buf.support_mcs[2])	// for R7800
							pdesc_data->ampduDensity = _HTCAP_AMPDU_SPC_NORES_;
						else
							pdesc_data->ampduDensity = _HTCAP_AMPDU_SPC_16_US_;
					} else
#endif	
					...
					if ((GET_CHIP_VER(priv) == VERSION_8822B)||(GET_CHIP_VER(priv) == VERSION_8198F)||(GET_CHIP_VER(priv) == VERSION_8822C)||(GET_CHIP_VER(priv) == VERSION_8812F)||(GET_CHIP_VER(priv)==VERSION_8814B))
						pdesc_data->ampduDensity = _HTCAP_AMPDU_SPC_2_US_;
					else
#endif	
                    ...
                    pdesc_data->ampduDensity = _HTCAP_AMPDU_SPC_16_US_;
#else
                    pdesc_data->ampduDensity = _HTCAP_AMPDU_SPC_4_US_;
#endif
                } else {
#ifdef CONFIG_WLAN_HAL_8814BE
					if (GET_CHIP_VER(priv) == VERSION_8814B
						&& txcfg->pstat->IOTPeer == HT_IOT_PEER_NETGEAR 
						&& txcfg->pstat->ht_cap_buf.support_mcs[3])
						pdesc_data->ampduDensity = _HTCAP_AMPDU_SPC_QUAR_US_;
					else
#endif
						pdesc_data->ampduDensity = ((txcfg->pstat->ht_cap_buf.ampdu_para & _HTCAP_AMPDU_SPC_MASK_) >> _HTCAP_AMPDU_SPC_SHIFT_);
                }
				...
				if (GET_CHIP_VER(priv) == VERSION_8814A) {
					if (priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_2G) {
						...
						if ((txcfg->aggre_en == FG_AGGRE_MSDU_FIRST) && (pdesc_data->maxAggNum > 10))
				...
				if (GET_CHIP_VER(priv) == VERSION_8814B) {
					if ((txcfg->aggre_en == FG_AGGRE_MSDU_FIRST) && (pdesc_data->maxAggNum > 0xf))
						pdesc_data->maxAggNum = 0xf;	// 0x1e
				}
#endif
#if 1 /*eric-8822 tp*/
        ...
        if (txcfg->q_num == MANAGE_QUE_NUM) {
            ...
            if (GetFrameSubType(txcfg->phdr) == WIFI_PROBERSP) {
                ...
                pdesc_data->dataRtyLmt = 16;
                    ...
                    (((struct sk_buff *)txcfg->pframe)->cb[_SKB_CB_FLAGS] & _SKB_CB_FLAGS_URGENT)) {
            	pdesc_data->dataRtyLmt = 16;
            } 
            else {
...
void rtl8192cd_signin_txdesc(struct rtl8192cd_priv *priv, struct tx_insn* txcfg)
{
	...
	tmpphyaddr = get_physical_addr(priv, pbuf, actual_size, DMA_TO_DEVICE);
#else
	tmpphyaddr = get_physical_addr(priv, pbuf, tx_len, DMA_TO_DEVICE);
#endif
...
#endif
				//TxBufferAddr
				picvdesc->Dword8 = set_desc(get_physical_addr(priv, picv, txcfg->icv, DMA_TO_DEVICE));
...
#endif
				// set TxBufferAddr
				pmicdesc->Dword8= set_desc(get_physical_addr(priv, pmic, txcfg->mic, DMA_TO_DEVICE));
...
if((priv->pshare->rf_ft_var.txforce != 0xff || (txcfg->pstat && txcfg->pstat->force_rate))
	...
	if(GetFrameSubType(txcfg->phdr) == WIFI_RTS ){
		desc_data.navUseHdr = _TRUE;
	}
#endif
#ifdef WLAN_HAL_HW_TX_SHORTCUT_HDR_CONV
		...
		if (OFFLOAD_ENABLE(priv)) {
			if (GetFrameType(pwlhdr) == WIFI_DATA) {
				...
				else if(txcfg->q_num >= BK_QUEUE && txcfg->q_num <= VO_QUEUE) {
...
#ifdef WPA3_ENT
				desc_data.macId = (priv->vap_init_seq*2+HW_ENC_GROUP_CIPHER_OFFSET);
#else
...
#ifdef AUTH_SAE
			if(priv->pshare->rf_ft_var.gcmpforce)
				desc_data.secType = _GCMP256_PRIVACY_;
#endif
			if ( txcfg->use_sw_enc ) {				
        ...
        if ((txcfg->privacy == _CCMP_PRIVACY_) && (txcfg->use_sw_enc == FALSE)) {
            desc_data.secType = txcfg->privacy;
            desc_data.swCrypt = FALSE;
            desc_data.icv     = 0;
            desc_data.mic     = 0;
...
#ifdef MCAST2UI_REFINE
						memcpy((void *)&txcfg->pstat->tx_sc_ent[idx].ethhdr.daddr, &(((struct sk_buff *)txcfg->pframe)->cb[10]), MACADDRLEN);
#endif
        			}
					else
#endif
	...
	if (txcfg->privacy == _WAPI_SMS4_)
	{
		...
		if (txcfg->privacy && txcfg->use_sw_enc)
		{
            if (txcfg->privacy == _TKIP_PRIVACY_ ||
...
#ifdef WPA3_ENT
			else if(txcfg->privacy == _GCMP256_PRIVACY_){

				unsigned char pllc = NULL;

				if((i == 0)&&(txcfg->llc != 0))
					pllc = pwlhdr + txcfg->hdr_len + 8;
				...
				rtk_gcmp_encrypt(priv, pwlhdr, pllc, pbuf, txcfg->fr_len, pmic, txcfg->isPMF);
			}
			else if(txcfg->privacy == _BIP_GMAC256_PRIVACY_){
...
swenc_end:
    ...
    /* cliff test */
	if ( !(OPMODE & WIFI_MP_STATE) &&  priv->assoc_num > 4 ) {
#ifdef USE_OUT_SRC
    ...
    if (txcfg->aggre_en == FG_AGGRE_MSDU_FIRST) {
        priv->amsdu_len = desc_data.hdrLen + desc_data.llcLen + desc_data.frLen;
        if (txcfg->privacy && (txcfg->use_sw_enc == FALSE))
            priv->amsdu_len += (desc_data.icv + desc_data.mic + desc_data.iv);
    }
#endif
...
init_deschead:
    ...
    if (txcfg->aggre_en == FG_AGGRE_MSDU_FIRST) {
		...
		if (pskb->cb[_SKB_CB_FLAGS] & _SKB_CB_FLAGS_AMSDU_SEG_LIST) {
			...
			while (pskb) {
				next_skb = pskb->next;
				ieee8022_llc_snap_encapsulate(pskb, txcfg->pstat->cmn_info.mac_addr);
				txcfg->pframe = pskb;
				txcfg->aggre_en = (next_skb ? FG_AGGRE_MSDU_MIDDLE : FG_AGGRE_MSDU_LAST);
				if (txcfg->aggre_en == FG_AGGRE_MSDU_MIDDLE) {
					if (pskb->len & 0x3)
...
__IRAM_IN_865X unsigned int get_skb_priority(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info *pstat, int is_11s)
{
	...
	if (parsing) {
		if(pstat && pstat->cmn_info.aid ==priv->pshare->rf_ft_var.txbuf_manual_aid) {
			pri = priv->pshare->rf_ft_var.txbuf_manual;
...
service_priority_abort:
...
#endif // FORCE_QOS_SUPPORT
		{
			...
			if ((protocol[0] == 0x08) && (protocol[1] == 0x00))
			{
				...
				pri = skb->cb[0];
#endif
			else if (protocol[0] == 0x86 && protocol[1] == 0xdd) //IPv6
				pri = (skb->data[14] & 0x0e) >> 1;
...
#endif
		if (skb->cb[_SKB_CB_FLAGS] & _SKB_CB_FLAGS_ICMP) {
			pri = 7; // VO, ICMP packets go through VO queue for the low latency purpose.
		}
	}
...
static inline void schedule_txdsr_tasklet(struct rtl8192cd_priv *priv)
{
#ifdef TX_PROCESS_REFINE
...
#ifdef CONFIG_WLAN_HAL_8814BE
	if (GET_CHIP_VER(priv)== VERSION_8814B && priv->pshare->curr_band == BAND_2G)
	{
		smp_call_function_single_async(TX_DSR_CPU_2G, &(priv->pshare->txq_tasklet_csd));
	} else
#endif
#endif
	{
...
int amsdu_timer_add(struct rtl8192cd_priv *priv, struct stat_info *pstat, int tid, int from_timeout)
{
	...
	if (priv->pmib->dot11nConfigEntry.dot11nAMSDUSendTimeout) {
...
static int check_txdesc(struct rtl8192cd_priv *priv, struct tx_insn* txcfg)
{
	...
	if (txcfg->fr_type == _SKB_FRAME_TYPE_)
	{
				...
				)
				{
					...
					if (!pstat->ADDBA_ready[priority]) {
						if ((pstat->ADDBA_req_num[priority] < 5) && !pstat->ADDBA_sent[priority]) {
							...
							if (GET_CHIP_VER(priv) == VERSION_8814B) {
								SMP_UNLOCK_XMIT(flags);
								prepare_ADDBAreq(priv, pstat, priority);
								SMP_LOCK_XMIT(flags);
							} else
#endif
						...
						if (pstat->ADDBA_sent[priority])
							pstat->ADDBA_accu[priority]++;
					}
				}
			}
			...
			)
		{
...
#endif
			// If ICMP data packets go through MGNT queue, the TX tput is downgraded 
			// because STA (e.g., AX210) discards data packets due to the inconsistent SNs.
			if ((pskb->cb[_SKB_CB_FLAGS] & (_SKB_CB_FLAGS_URGENT | _SKB_CB_FLAGS_ICMP)) == _SKB_CB_FLAGS_URGENT) {
				txcfg->q_num = MGNT_QUEUE;
			} else if (txcfg->aggre_en < FG_AGGRE_MSDU_FIRST) {
				if (txcfg->q_num) { //MGNT queue = 0 , other queue is > 0
					//txcfg->q_num = pskb->cb[_SKB_CB_QNUM];
					priority = pskb->cb[_SKB_CB_PRIORITY];
				} else {
			...
			if ((txcfg->aggre_en == 0) && (pskb->cb[2] != (char)0xff)) {
...
#endif
			if ((pstat->aggre_mthd & AGGRE_MTHD_MPDU) && txcfg->aggre_en)
			{
				//panic_printk("%s %d pstat->ADDBA_ready[priority]=%d, priority=%d\n",__func__,__LINE__,pstat->ADDBA_ready[priority],priority);
				if (!pstat->ADDBA_ready[priority]) {
					if ((pstat->ADDBA_req_num[priority] < 5) && !pstat->ADDBA_sent[priority]) {
		...
		else
		{
			...
			} else if (priv->pmib->dot1180211AuthEntry.dot11PrivacyAlgrthm) {
				DEBUG_ERR("TX DROP: group key not set yet!\n");
				return FALSE;
			}
    ...
    // start from 8198F, MBSSID extend to 16
...
//	PHCI_TX_DMA_MANAGER_88XX	ptx_dma;
	...
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		...
		if ((check_txdesc(priv, txcfg)) == FALSE) // this will only happen in errorous forwarding
		{
			...
			if (txcfg->fr_type == _SKB_FRAME_TYPE_) {
#ifdef CMCC_CTC_QOS
				if ( (((struct sk_buff *)txcfg->pframe)->cb[33] == 0x22) && (((struct sk_buff *)txcfg->pframe)->cb[34]!= 0x22) ) {
					priv->ext_stats.cb_data1_drop++;
				}
#endif
				rtl_kfree_skb(priv, (struct sk_buff *)txcfg->pframe, _SKB_TX_);
		...
		if (txcfg->aggre_en < FG_AGGRE_MSDU_FIRST) {
...
#endif
			{
				...
				if (txcfg->fr_type==_SKB_FRAME_TYPE_ && q_num_t>=BK_QUEUE && q_num_t<=VO_QUEUE)
				{
					...
					if (pstat_t)
					{
						int pri_t = ((struct sk_buff *)txcfg->pframe)->cb[_SKB_CB_PRIORITY];
...
#ifdef GBWC
/*static*/ int rtl8192cd_tx_gbwc(struct rtl8192cd_priv *priv, struct stat_info	*pstat, struct sk_buff *skb, struct tx_insn *txcfg)
{
...
#ifdef RTK_STA_BWC
#ifdef TRX_VI_DATA_LOG
int rtl8192cd_tx_sta_bwc(struct rtl8192cd_priv *priv, struct stat_info	*pstat, struct sk_buff *skb, struct tx_insn *txcfg)
#else
int rtl8192cd_tx_sta_bwc(struct rtl8192cd_priv *priv, struct stat_info	*pstat, struct sk_buff *skb)
#endif
{
	unsigned int tx_count, length = 0;
	BOOLEAN ret;
...
#ifdef TX_AMSDU_REFINE
	if (skb->cb[_SKB_CB_FLAGS] & _SKB_CB_FLAGS_AMSDU_SEG_LIST) {
	...
	tx_count = pstat->sta_bwc_tx_cnt + length;
...
*/
	...
	rtl_cache_sync_wback(priv, (unsigned long)bus_to_virt(dma_txhead[*tx_head]-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), sizeof(struct tx_desc), DMA_TO_DEVICE);
#endif
	rtl_cache_sync_wback(priv, (unsigned long)bus_to_virt(get_desc(pdesc->Dword8)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), (get_desc(pdesc->Dword7)&TX_TxBufSizeMask), DMA_TO_DEVICE);
...
one_txdesc:
	...
	rtl_cache_sync_wback(priv, (unsigned long)bus_to_virt(get_desc(pfrstdesc->Dword8)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), (get_desc(pfrstdesc->Dword7)&TX_TxBufSizeMask), DMA_TO_DEVICE);
...
#ifdef SBWC
#ifdef TRX_VI_DATA_LOG
static int rtl8192cd_tx_sbwc(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct sk_buff *skb, struct tx_insn *txcfg)
#else
	...
	if (pstat)
	{
		...
		if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
			if (priv->pmib->gbwcEntry.GBWCMode)
				if (rtl8192cd_tx_gbwc(priv, pstat, skb, txcfg) == TX_PROCEDURE_CTRL_STOP)
				{
					goto stop_proc;
				}
		}
		...
		#ifdef SBWC
		#ifdef TRX_VI_DATA_LOG
		if (rtl8192cd_tx_sbwc(priv, pstat, skb, txcfg) == TX_PROCEDURE_CTRL_STOP)
		#else
...
#ifdef TRX_VI_DATA_LOG
		if (rtl8192cd_tx_sta_bwc(priv, pstat, skb, txcfg) == TX_PROCEDURE_CTRL_STOP)
#else
...
#endif
#ifdef DOT11K
		if (priv->pmib->dot11StationConfigEntry.dka_pkt_timeout)
		{
			if ((priv->pmib->dot11StationConfigEntry.dka_pkt_type & BIT0) &&
				(skb->cb[_SKB_CB_FLAGS] & _SKB_CB_FLAGS_ARP))
				pstat->dka_pkt_timeout = priv->pmib->dot11StationConfigEntry.dka_pkt_timeout;
			else if ((priv->pmib->dot11StationConfigEntry.dka_pkt_type & BIT1) &&
				(skb->cb[_SKB_CB_FLAGS] & _SKB_CB_FLAGS_ICMP))
				pstat->dka_pkt_timeout = priv->pmib->dot11StationConfigEntry.dka_pkt_timeout;
		}
#endif
	}
			...
			)
		{
...
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
			if (OFFLOAD_ENABLE(priv)) {
				/* already do rtl8192cd_tx_queueDsr in this function */
				checkAvailableH2D_TXBD(priv, q_num);
			} else
...
#ifdef SW_TXQ_RSVD_DESC
				if (!pstat->ADDBA_ready[ptxsc_entry->pktpri]
#ifdef SW_TXQ_ATF
					&& !priv->pshare->swq_atf_state
#endif
				) {
					int max_queue_cnt = CURRENT_NUM_TX_DESC >> 3;
					if (priv->pshare->swq_skb_queue_cnt[q_num - 1] >= max_queue_cnt)
						num_swq_rsvd = max_queue_cnt;
					else
						num_swq_rsvd = priv->pshare->swq_skb_queue_cnt[q_num - 1];
...

			{
				...
				if(txcfg->aggre_en >= FG_AGGRE_MSDU_FIRST){
					...
					if (txcfg->privacy == _TKIP_PRIVACY_ && txcfg->aggre_en == FG_AGGRE_MSDU_LAST)
			...
			if (txcfg->aggre_en == FG_AGGRE_MSDU_MIDDLE || txcfg->aggre_en == FG_AGGRE_MSDU_LAST) {
				...
				tx_sum_up(priv, pstat, txcfg);
#ifdef ISSUE_RTS_BY_SW
				if(priv->pshare->rf_ft_var.RTS_pkt_intvl && priv->net_stats.tx_packets % priv->pshare->rf_ft_var.RTS_pkt_intvl == 0){
					issue_RTS(priv, pstat->hwaddr);
				}
#endif
				SNMP_MIB_INC(dot11TransmittedFragmentCount, 1);
			...
			if(txcfg->aggre_en >= FG_AGGRE_MSDU_FIRST)
				...
				if ((pstat->aggre_mthd & AGGRE_MTHD_MPDU) && txcfg->aggre_en) {
					if (!pstat->ADDBA_ready[(int)skb->cb[1]]) {
						...
						if (pstat->ADDBA_sent[(int)skb->cb[1]])
							pstat->ADDBA_accu[(int)skb->cb[1]]++;
					}
				}
#ifndef TXSC_HDR 
#ifdef TX_SCATTER
...
static __always_inline void check_tcp_ack_timeout(unsigned long task_priv)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;
	struct list_head *phead, *plist;
	struct stat_info *pstat;
	...
	if (IS_DRV_OPEN(priv))
		rtk_mod_timer(&priv->tcpack_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(1));
}
DEFINE_TIMER_CALLBACK(check_tcp_ack_timeout);

void init_tcpack_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->tcpack_timer), check_tcp_ack_timeout, (unsigned long)priv, 0);
}
...
#if defined(A4_STA) && defined(RTK_MULTI_AP)
static int easymesh_1905_mcast_2_unicst(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info *sa_stat)
{
	struct list_head *phead, *plist;
	struct stat_info *pstat;
...
#ifdef CONFIG_IEEE80211W
		if (privacy == _NO_PRIVACY_ || (privacy == _CCMP_PRIVACY_ && !UseSwCrypto(priv, pstat, FALSE, 0)))
#else
		if (privacy == _NO_PRIVACY_ || (privacy == _CCMP_PRIVACY_ && !UseSwCrypto(priv, pstat, FALSE)))
#endif
		{
			if (!skb_cloned(skb) && (newskb = skb_clone(skb, GFP_ATOMIC))) {
...
ucast_next_sta:
		...
		if (plist == plist->next)
			break;
	} // end while((plist = asoc_list_get_next(priv, plist)) != phead)
...
#endif // A4_STA && RTK_MULTI_AP

#ifdef A4_STA
static int arp_2_unicst(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info *sa_stat)
{
	struct list_head *phead, *plist;
	struct stat_info *pstat;
	struct sk_buff *newskb;
	...
	unsigned long flags = 0;
#endif
	BOOLEAN a3_sta_exist = FALSE;

#ifdef MCAST2UI_REFINE
	privacy = get_mcast_encrypt_algthm(priv);
#ifdef CONFIG_IEEE80211W
	bmc_hwenc = (privacy == _NO_PRIVACY_) || (privacy == _CCMP_PRIVACY_ && !UseSwCrypto(priv, NULL, TRUE, 0));
#else
	bmc_hwenc = (privacy == _NO_PRIVACY_) || (privacy == _CCMP_PRIVACY_ && !UseSwCrypto(priv, NULL, TRUE));
#endif
#endif
	...
	while ((plist = asoc_list_get_next(priv, plist)) != phead) {
		...
		// Don't send ucast to A3 STAs, send bcast instead to avoid arp storm
		if (!(pstat->state & WIFI_A4_STA)) {
			a3_sta_exist = TRUE;
			continue;
		}
		...
		if ((plist->next == phead) && !skb_cloned(skb) && !a3_sta_exist) {
			asoc_list_unref(priv, pstat);
#ifdef MCAST2UI_REFINE
			memcpy(&skb->cb[10], pstat->cmn_info.mac_addr, 6);
#else
...
#ifdef RTK_MULTI_AP
			if (IS_1905_MCAST(skb->data))
				return easymesh_1905_mcast_2_unicst(priv, skb, sa_stat);
#endif
			if (IS_ARP_PROTO(skb->data))
				return arp_2_unicst(priv, skb, sa_stat);
		}
    }
#endif // A4_STA

	// all multicast managment packet try do m2u
	if (isSpecialFloodMac(priv,skb) || /* IS_MDNSV4_MAC(skb->data) || IS_MDNSV6_MAC(skb->data) || */
		IS_IGMP_PROTO(skb->data) || isICMPv6Mng(skb) || IS_ICMPV6_PROTO(skb->data)|| isMDNS(skb->data)) {
...
//#ifdef VIDEO_STREAMING_REFINE
	...
	if(m2uCnt == 0){
		...
		else
		{
#if 1
			int idx;
			unsigned char alreadyInTable=0;

			//find if the mc macaddr is in the table
			for(idx=0; idx<MAX_UNKNOWN_MC_ENTRY; idx++){
				if (!memcmp(priv->unknown_mc[idx].mcaddr, origDest, 6)){ // find if the DA of unkown_mc_pkt is in table
					priv->unknown_mc[idx].mc_pkts++;
					//priv->unknown_mc[idx].timeout =5;
					alreadyInTable = 1;
					break;
				}
			}
			if(alreadyInTable == 0){
				//find the usable entry id for unknown pkt and init Entry
				for (idx=0; idx<MAX_UNKNOWN_MC_ENTRY; idx++) {
					if (priv->unknown_mc[idx].timeout == 0) {
						memcpy(priv->unknown_mc[idx].mcaddr, origDest, 6);
						priv->unknown_mc[idx].mc_pkts = 1;
						priv->unknown_mc[idx].timeout = 5;
						break;
					}
				}
			}
			if (idx == MAX_UNKNOWN_MC_ENTRY) {
				priv->unknown_others_mc_pkt++;
				if(priv->unknown_others_mc_pkt > priv->pshare->rf_ft_var.mc2u_drop_unknown) {
			...
			// if (unknown pkts count > threshold) this second, then drop the other  pkts.
			if (priv->unknown_mc[idx].mc_pkts > priv->pshare->rf_ft_var.mc2u_drop_unknown) {
				DEBUG_WARN("TX DROP: %s %d !\n", __FUNCTION__,__LINE__);
				priv->ext_stats.tx_drops++;
				rtl_kfree_skb(priv, skb, _SKB_TX_);
				priv->unknown_mc[idx].timeout =5;
...
//#endif
	...
	while ((plist = asoc_list_get_next(priv, plist)) != phead) {
		...
		if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE)
		{
			int q_num = BE_QUEUE;	// use BE queue to send multicast

			if (priv->stop_tx_mcast2uni) {
				rtl8192cd_tx_queueDsr(priv, q_num);

				if ((priv->stop_tx_mcast2uni == 1)
					&& (getAvailableTXBD(priv, q_num) > (CURRENT_NUM_TX_DESC/4)))
				{
					priv->stop_tx_mcast2uni = 0;
				}
#if defined(CONFIG_RTL865X_ETH_PRIV_SKB) || defined(CONFIG_RTL_ETH_PRIV_SKB)
#if defined(CONFIG_RTL_ETH_RECYCLED_SKB)
				else if ((priv->stop_tx_mcast2uni == 2) && (rtl_free_eth_skb_num() > ETH_SKB_FREE_TH))
#else
...
#ifdef SW_TXQ_ATF
#define SWQ_ATF_OFF		0
#define SWQ_ATF_ON		1
#define SWQ_ATF_DELAY	2

void swq_atf_schedule(struct rtl8192cd_priv *priv)
{
	struct stat_info *pstat = NULL;
	int i = 0, total_sta_num = 0, hwq_txtime = 0;
	int nobudget_sta_num = 0, nopkt_sta_num = 0, max_txrate = 0, max_enqnum = 0;

	// ATF is on while the total number of associated stations > 0
	if (priv->pshare->total_assoc_num == 0) {
		if (priv->pshare->swq_atf_state)
			priv->pshare->swq_atf_state = SWQ_ATF_OFF;
		return;
	}

	// Dsetermine the re-schedule is needed or not.
	max_enqnum = priv->pshare->rf_ft_var.swq_max_enqueue_len;
	...
	for (i = 0; i < NUM_STAT; i++) {
		...
		if (pstat->swq.atf_base_enqnum > 0)
			max_enqnum -= pstat->swq.atf_base_enqnum;
		else
			max_txrate += pstat->swq.atf_txrate;

		// HW packet size (bytes) / data rate * retry ratio / 100
		pstat->swq.atf_hwq_txtime = rtl_atomic_read(&pstat->swq.atf_hwpkt_bytes) / 125 /
						pstat->swq.atf_txrate * pstat->swq.atf_retry_ratio / 100;
		hwq_txtime += pstat->swq.atf_hwq_txtime;

		if (pstat->swq.atf_state == SWQ_ATF_DELAY)
			continue;

		if (pstat->state & WIFI_SLEEP_STATE)
			continue;

		if (pstat->tx_avarage <= MBPS_TO_BIT(1))
			continue;

		if (pstat->swq.atf_budget <= 0) {
			nobudget_sta_num++;
		} else if (pstat->swq.atf_tx_bytes == pstat->swq.atf_tx_bytes_prev && pstat->swq.atf_cnt == 0) {
			nopkt_sta_num++;
		}
		pstat->swq.atf_tx_bytes_prev = pstat->swq.atf_tx_bytes;
	...
	if (priv->pshare->swq_atf_state == SWQ_ATF_DELAY) {
		if (hwq_txtime <= priv->pshare->rf_ft_var.swq_atf_delay_lo_th)
			goto do_schedule;

		priv->pshare->swq_atf_delay_cnt++;
		goto update_param;
	}

	if (nobudget_sta_num == total_sta_num) {
		priv->pshare->swq_atf_sched_nobudget_cnt++;
	} else if (++priv->pshare->swq_atf_force_cnt >= priv->pshare->rf_ft_var.swq_atf_force_max) {
		priv->pshare->swq_atf_sched_force_cnt++;
	} else if (nobudget_sta_num > 0 && total_sta_num == (nobudget_sta_num + nopkt_sta_num)) {
		if (++priv->pshare->swq_atf_nopkt_cnt >= priv->pshare->rf_ft_var.swq_atf_nopkt_max) {
			priv->pshare->swq_atf_sched_nopkt_cnt++;
		} else {
			priv->pshare->swq_atf_nosched_nopkt_cnt++;
			return;
		}
	} else {
		if (nobudget_sta_num > 0) {
			priv->pshare->swq_atf_nopkt_cnt = 0;
			priv->pshare->swq_atf_nosched_pkt_cnt++;
		} else {
			priv->pshare->swq_atf_nosched_budget_cnt++;
		}
		return;
	}
...
do_schedule:
	...
	if (hwq_txtime >= priv->pshare->rf_ft_var.swq_atf_delay_hi_th) {
		priv->pshare->swq_atf_state = SWQ_ATF_DELAY;
		goto update_param;
	}

	priv->pshare->swq_atf_force_cnt = 0;
	priv->pshare->swq_atf_nopkt_cnt = 0;
	if (max_enqnum < 0)
		max_enqnum = 0;

	// allocate a new budget for each STA
	for (i = 0; i < NUM_STAT; i++) {
		if (priv->pshare->aidarray[i] == NULL || priv->pshare->aidarray[i]->used == FALSE)
			continue;
		...
		// Enqueue: the max qlen is based on the TX rate.
		if (pstat->swq.atf_base_enqnum > 0) {
			pstat->swq.atf_enqnum = pstat->swq.atf_base_enqnum;
		} else {
			pstat->swq.atf_enqnum = max_enqnum * pstat->swq.atf_txrate / max_txrate;
			if (pstat->swq.atf_enqnum == 0)
				pstat->swq.atf_enqnum = 1;
		}

		if (pstat->swq.atf_hwq_txtime >= priv->pshare->rf_ft_var.swq_atf_sta_hi_th) {
			pstat->swq.atf_state = SWQ_ATF_DELAY;
			pstat->swq.atf_budget = 0;
		} else if (pstat->swq.atf_hwq_txtime < priv->pshare->rf_ft_var.swq_atf_sta_lo_th) {
			pstat->swq.atf_state = SWQ_ATF_ON;
		}

		// Dequeue: reset budget if remaining, add a new quota.
		if (pstat->swq.atf_budget > 0) {
			pstat->swq.atf_budget = 0;
		}

		pstat->swq.atf_quota = pstat->swq.atf_base_quota * pstat->swq.atf_txrate * 125 * 100 / pstat->swq.atf_retry_ratio;
		if (pstat->swq.atf_state != SWQ_ATF_DELAY)
			pstat->swq.atf_budget += pstat->swq.atf_quota;
	}

	priv->pshare->swq_atf_state = SWQ_ATF_ON;
	return;
...
void swq_atf_calc_hwq_pktsz(struct tx_desc_info *pdescinfo, bool is_add)
{
	int i;
	...
	if (is_add) {
		if (pdescinfo->amsdu_num == 0) {
			if (pdescinfo->buf_type[0] == _SKB_FRAME_TYPE_)
				pdescinfo->swq_atf_signin_pktsz = pdescinfo->len + pdescinfo->buf_len[0] + pdescinfo->buf_len[1];
			else
				pdescinfo->swq_atf_signin_pktsz = 0;
		} else {
			pdescinfo->swq_atf_signin_pktsz = pdescinfo->len;
			for (i = 0; i < pdescinfo->amsdu_num; i++) {
				pdescinfo->swq_atf_signin_pktsz += pdescinfo->buf_len_amsdu[i];
			}
		}
		rtl_atomic_add(pdescinfo->swq_atf_signin_pktsz, &pstat->swq.atf_hwpkt_bytes);
	} else {
		rtl_atomic_sub(pdescinfo->swq_atf_signin_pktsz, &pstat->swq.atf_hwpkt_bytes);
	}
}
#endif /* SW_TXQ_ATF */
...
__inline__ static unsigned char rtl8192cd_swq_settimer(struct rtl8192cd_priv *priv, UINT32 timer_timeout)
{
    ...
    if(timer_timeout == 0) { // from deltimer
        ...
        if(priv->pshare->swq_use_hw_timer) {
            ...
            if(set_timer) {
                priv->pshare->swq_current_timeout = timer_timeout;
                rtk_mod_timer(&priv->pshare->swq_sw_timer, (unsigned long)timer_timeout);
            }
        }
    }
    return 0;
...
void rtl8192cd_swq_deltimer(struct rtl8192cd_priv *priv, struct stat_info* pstat, int qnum)
{
    unsigned char set_timer = 0;
...
__inline__ static int rtl8192cd_swq_dequeue_amsdu(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char q_num, unsigned int dqnum, unsigned char releaseALL, int max_size)
{
	...
	int count = 0, qlen;
	unsigned short protocol;
	unsigned int is_first=1, total_len=0, total_num=0;
	unsigned int amsdu_merge, agg_cnt = 1;
	struct tx_insn tx_insn;
	struct net_device *wdsDev=NULL;
	int standalone;
	struct sk_buff *pskb_first, *pskb_prev;	
	int avail_txbd_num, req_txbd;
	int recycleCnt = 0;
#ifdef SW_TXQ_ATF
	int deq_pktsz = 0;
#endif
	...
	amsdu_merge = priv->pshare->rf_ft_var.amsdu_merge;
#if defined(CONFIG_RTL_OFFLOAD_DRIVER) && defined(TX_AMSDU_REFINE)
	req_txbd = 7;
#else
	req_txbd = 4;
#endif
	avail_txbd_num = getAvailableTXBD(priv, q_num) - req_txbd;
	...
	while (count < dqnum)
	{
...
#endif
			{
				protocol = ntohs(*(u16 *)(pskb->data + ETH_ALEN*2));
				if ((protocol + WLAN_ETHHDR_LEN <= WLAN_MAX_ETHFRM_LEN) ||
					(!skb_cloned(pskb) && skb_headroom(pskb) >= sizeof(struct llc_snap)))
					standalone = 0;
				else
					standalone = 1;
			}
		...
		if ((standalone == 1) || (is_first && (!next_skb || skb_tailroom(pskb) < 3))) {
			standalone = -1;
			//Normal packet without AMSDU
			txcfg->aggre_en = 0;
			avail_txbd_num--;
			...
			if (next_skb) {
				if (next_skb->cb[_SKB_CB_FLAGS] & _SKB_CB_FLAGS_URGENT) {
					standalone = 1;
				} else
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
				if (OFFLOAD_ENABLE(priv)) {
...
#endif
				{
					protocol = ntohs(*(u16 *)(next_skb->data + ETH_ALEN*2));
					if ((protocol + WLAN_ETHHDR_LEN <= WLAN_MAX_ETHFRM_LEN) ||
						(!skb_cloned(next_skb) && skb_headroom(next_skb) >= sizeof(struct llc_snap)))
						standalone = 0;
					else
						standalone = 1;
				}
				if ((agg_cnt < amsdu_merge)
					&& !is_first
					&& next_skb->len < 128
					&& !skb_cloned(pskb)
					) {
					int required_len, padding;
					int merge_cnt;
					
					required_len = next_skb->len + sizeof(struct llc_snap) + 3;
					if ((total_len + required_len < max_size)
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
						&& (pskb->len + required_len < 2100)	// limit the total length of merged subframe
#endif
						&& (skb_tailroom(pskb) > required_len)
...
subframe_merge:
						...
						// merge next skb
						next_skb = skb_dequeue(&pstat->swq.swq_queue[q_num]);
						...
						if (protocol + WLAN_ETHHDR_LEN > WLAN_MAX_ETHFRM_LEN) {
							...
							pethhdr = (struct wlan_ethhdr_t *)next_skb->data;
							pmsdu_hdr = (struct wlan_ethhdr_t *)skb_tail_pointer(pskb);
							if (pethhdr->daddr[0] & 0x1) {
								copyMACAddr(pmsdu_hdr->daddr, pstat->cmn_info.mac_addr);
								copyMACAddr(pmsdu_hdr->saddr, pethhdr->saddr);
							} else
								memcpy(pmsdu_hdr, pethhdr, ETH_ALEN*2);
							pmsdu_hdr->type = htons(next_skb->len - WLAN_ETHHDR_LEN + sizeof(struct llc_snap));
							eth_2_llc(pethhdr, (struct llc_snap *)((unsigned long)pmsdu_hdr+WLAN_ETHHDR_LEN));
							memcpy((u8 *)pmsdu_hdr+WLAN_ETHHDR_LEN+sizeof(struct llc_snap), next_skb->data+WLAN_ETHHDR_LEN,
								next_skb->len-WLAN_ETHHDR_LEN);
							skb_put(pskb, next_skb->len+sizeof(struct llc_snap));
						} else
							memcpy(skb_put(pskb, next_skb->len), next_skb->data, next_skb->len);
						...
						priv->pshare->swq_skb_queue_cnt[q_num - 1]--;
#endif
						if (next_skb->len < SWQ_AMSDU_SMLPKT_LEN)
							pstat->sml_big_pkt_diff--;
						else
							pstat->sml_big_pkt_diff++;
...
//#endif
#ifdef SMP_SKB_RECYCLE_OFFLOAD
						skb_queue_tail(priv->pshare->skb_recycle_queue, next_skb);
#ifdef ENABLE_RTL_SKB_STATS
						rtl_atomic_dec(&priv->rtl_tx_skb_cnt);
#endif
						recycleCnt++;
#else
						rtl_kfree_skb(priv, next_skb, _SKB_TX_);
#endif
						...
						if (next_skb) {
							...
							if ((agg_cnt < amsdu_merge) && next_skb->len < 128) {
								required_len = next_skb->len + sizeof(struct llc_snap) + 3;
								if ((total_len + required_len < max_size)
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
									&& (pskb->len + required_len < 2100)	// limit the total length of merged subframe
						...
						priv->net_stats.tx_packets += merge_cnt;
#if defined(CONFIG_RTL8672) || defined(CONFIG_WLAN_STATS_EXTENTION)
						priv->ext_stats.tx_ucast_pkts_cnt += merge_cnt;
#endif
#ifdef SW_TXQ_ATF
						pstat->swq.atf_cnt -= merge_cnt;
#endif
						pstat->tx_pkts2 += merge_cnt;
					}
				}
			}
			...
			if (!next_skb || (standalone == 1)
					...
					|| (skb_tailroom(next_skb) < 3)	// reserved tailroom for 4-bytes alignment padding
					|| (total_num+1 >= priv->pmib->dot11nConfigEntry.dot11nCurAMSDUSendNum)
					|| (amsdu_merge && agg_cnt >= amsdu_merge)
					|| (count+1 >= dqnum)
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
					|| (OFFLOAD_ENABLE(priv) && avail_txbd_num <= 1)
#endif
				) {
				if (is_first) {
					//Normal packet without AMSDU
		...
		if (!priv->pmib->dot11OperationEntry.disable_amsdu_txsc)
		{
			pskb->cb[_SKB_CB_AMSDU_TXSC] = q_num | (txcfg->aggre_en << 4);
#ifdef TX_AMSDU_REFINE
			if (txcfg->aggre_en) {
				if (txcfg->aggre_en == FG_AGGRE_MSDU_FIRST) {
					pskb_first = pskb;
...
#ifdef SW_TXQ_ATF
		pstat->swq.atf_cnt -= count;
		if (priv->pshare->swq_atf_state) {
			pstat->swq.atf_budget -= deq_pktsz;
			pstat->swq.atf_tx_bytes += deq_pktsz;
		}
#endif
...
__inline__ static int rtl8192cd_swq_dequeue(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char qnum, unsigned int dqnum, unsigned char releaseALL)
{
...
#ifdef SW_TXQ_ATF
		|| (priv->pshare->swq_atf_state == SWQ_ATF_DELAY)
		|| (priv->pshare->swq_atf_state == SWQ_ATF_ON && pstat->swq.atf_budget <= 0)
#endif
		)
		return 0;
	...
	if (pstat->AMSDU_AMPDU_support && !(pstat->state & WIFI_SLEEP_STATE)) {
		...
		else if (priv->pshare->rf_ft_var.amsdusmallpkts && pstat->sml_big_pkt_diff > 1 && (pstat->AMSDU_AMPDU_support & STA_RX_AMSDU_SUPPORT))
	...
	while (1)
	{
		...
		if(rtl8192cd_swq_bdfull(priv, req_txbd, qnum))
			break;
...
#ifdef SW_TXQ_ATF
		deq_pktsz += tmpskb->len;
		pri = tmpskb->cb[_SKB_CB_PRIORITY];
#endif
	...
	if (count) {
		...
		if (priv->pshare->swq_atf_state) {
			if (!pstat->ADDBA_ready[pri])
				pstat->swq.atf_budget -= count * pstat->swq.atf_ack_penalty;
			pstat->swq.atf_budget -= deq_pktsz;
			pstat->swq.atf_tx_bytes += deq_pktsz;
		}
#endif
...
__inline__ static UINT32 get_swq_tri_time_using_hw_timer(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info *pstat)
{
    ...
    if ((priv->pshare->swq_numActiveSTA > priv->pshare->rf_ft_var.swq_turbo_sta_th) &&
...
__inline__ static int rtl8192cd_swq_enqueue(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info	*pstat)
{
...
#ifdef SW_TXQ_ATF
	if (priv->pshare->swq_atf_state && pstat->swq.atf_cnt >= pstat->swq.atf_enqnum) {
	...
	if(priv->pshare->swq_use_hw_timer) {
		...
		if(pstat->muPartner_num){
			if(pstat->swq.swq_en[q_num]) {
				...
				for (pn = 0; pn < pstat->muPartner_num; ++pn) {
					psta_mu = pstat->muPartner[pn];
...
#endif
#if defined(SUPPORT_TX_SWQ_AMSDU) || defined(SW_TXQ_ATF)
		{
			if(!pstat->swq.swq_en[q_num]) {
				pstat->swq.swq_en[q_num] = 1; /*enable swq*/
				pstat->swq.swq_prev_timeout[q_num] = 1;
...
#else
		{
			...
			else if((assoc_num > assoc_num_low) || (KBPS_TO_BIT(5500) < pstat->tx_avarage && pstat->tx_avarage < MBPS_TO_BIT(155))) {
				...
				if(change) {
					if(pstat->swq.swq_en[q_num]) {
						...
						if(pstat->swq.swq_empty[q_num] == 0) {
							...
							priv->pshare->swq_numActiveSTA++;
							if(priv->pshare->swq_numActiveSTA == (priv->pshare->rf_ft_var.swq_turbo_sta_th+1)) {
								turbo_swq_setting(priv);
								priv->pshare->swq_turbo_time = priv->up_time;
							}
						}

						if(priv->pshare->swq_max_xmit[q_num])
							rtl8192cd_swq_dequeue(priv, pstat, q_num, priv->pshare->swq_max_xmit[q_num], SWQ_DEQ_LIMIT);
						else
	...
	if(change != 1) {
		...
		if (pBeamInfo->beamformee_mu_cnt >= 2 && pstat->muPartner_num) {
            ...
            if ((NUM_TX_DESC - getAvailableTXBD(priv, q_num)) < priv->pshare->rf_ft_var.mu_bdth) {
                need_fire = 1;
            }
            else {
			qlen = skb_queue_len(&pstat->swq.swq_queue[q_num]);
		...
		if(need_deque_mu) {
				...
				if (priv->pshare->rf_ft_var.dqnum){
					if(qlen >= psta[0]->mu_deq_num && qlen2 >= psta[1]->mu_deq_num){
					    ...
					    {						
							...
							if(count > 0 && count2 > 0) {
								if(count == count2) {
									if(count >= priv->pmib->dot11nConfigEntry.dot11nCurAMSDUSendNum){
										sta_deq_num[0] =  priv->pmib->dot11nConfigEntry.dot11nCurAMSDUSendNum;
										sta_deq_num[1] =  priv->pmib->dot11nConfigEntry.dot11nCurAMSDUSendNum;
										count-= priv->pmib->dot11nConfigEntry.dot11nCurAMSDUSendNum;
										count2-= priv->pmib->dot11nConfigEntry.dot11nCurAMSDUSendNum;
									}else{
										sta_deq_num[0] = count;
            ...
            unsigned int cur_amsdu = priv->pmib->dot11nConfigEntry.dot11nCurAMSDUSendNum;
            ...
            for (i = 0; i < psta_num; i++) {
                if (psta[i]->mu_deq_num <= deq_num_min)
                    ...
                    deq_num[i] = psta[i]->mu_deq_num / deq_num_min;

                if (psta[i]->aggre_mthd == AGGRE_MTHD_MPDU_AMSDU)
                    deq_num[i] *= cur_amsdu;
            ...
            while (!done) 
            {
                ...
                unsigned int dq_cnt = 0;
                if ((bd_num = getAvailableTXBD(priv, q_num)) < deq_num_all + 5) {
                    rtl8192cd_tx_queueDsr(priv, q_num);
                    
                    bd_num = getAvailableTXBD(priv, q_num);
                    if (bd_num < deq_num_all + 5) {
                        break;
                    }
                }
                ...
                for (i = 0; i < psta_num; i++) {
                    if (count[i] >= deq_num[i]) {
                        dq_cnt = rtl8192cd_swq_dequeue(priv, psta[i], q_num, deq_num[i], SWQ_DEQ_LIMIT);
                        if (dq_cnt == deq_num[i]) {
                            count[i] -= deq_num[i];
                            if (count[i] < deq_num[i]) {
                                qlen = skb_queue_len(&psta[i]->swq.swq_queue[q_num]);
                                if (qlen + count[i] >= psta[i]->mu_deq_num)
                                    count[i] = psta[i]->mu_deq_num;
                                else if (qlen + count[i] >= deq_num[i])
                                    count[i] = deq_num[i];
                                else
                                    done = 1;
                            }
...
static __always_inline void do_swq_timeout(unsigned long task_priv)
{
#if (MU_BEAMFORMING_SUPPORT == 1)
	int i, pn;
#endif
...
#ifdef SW_TXQ_ATF
	if (priv->pshare->rf_ft_var.swq_atf_en) {
		swq_atf_schedule(priv);
	} else if (priv->pshare->swq_atf_state) {
		priv->pshare->swq_atf_state = SWQ_ATF_OFF;
	}
#endif
...
#endif
#ifdef SW_TXQ_ATF
						if (priv->pshare->swq_max_xmit[qnum] && priv->pshare->swq_atf_state)
							rtl8192cd_swq_dequeue(priv_this, pstat, qnum, priv->pshare->swq_max_xmit[qnum], SWQ_DEQ_LIMIT);
						else
#endif
#ifdef CTC_WMM_LATENCY
						if ((priv->pshare->rf_ft_var.ctc_spec_wmm) && (priv->pshare->swq_max_xmit[qnum]))
							rtl8192cd_swq_dequeue(priv_this, pstat, qnum, priv->pshare->swq_max_xmit[qnum], SWQ_DEQ_LIMIT);
						else
#endif
...
#ifdef SW_TXQ_ATF
						if (priv->pshare->swq_max_xmit[qnum] && priv->pshare->swq_atf_state)
							pstat->swq.q_TOCount[qnum] += rtl8192cd_swq_dequeue(priv_this, pstat, qnum, priv->pshare->swq_max_xmit[qnum], SWQ_DEQ_LIMIT);
						else
#endif
	...
	rtk_tasklet_clear_triggered(&(priv->pshare->swq_tasklet));
#ifdef SMP_LOAD_BALANCE_SUPPORT
	if (priv->pshare->rf_ft_var.swq_hw_to)
		RTL_W32(TC4_CTRL, BIT26 | BIT25 | BIT24 | (RTL_MICROSECONDS_TO_GTIMERCOUNTER(priv->pshare->rf_ft_var.swq_hw_to) & 0x00FFFFFF));
	else
...
{
	do_swq_timeout(task_priv);
}
#else  // !(__OSK__)
void rtl8192cd_swq_timeout_tasklet_cb(unsigned long task_priv)
{
	do_swq_timeout(task_priv);
}

static __always_inline void rtl8192cd_swq_timeout_timer_cb(unsigned long task_priv)
{
	do_swq_timeout(task_priv);
}
DEFINE_TIMER_CALLBACK(rtl8192cd_swq_timeout_timer_cb)

void init_swq_sw_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->pshare->swq_sw_timer), rtl8192cd_swq_timeout_timer_cb, (unsigned long)priv, 0);
}
#endif // __OSK__
#endif //SW_TX_QUEUE
...
int __rtl8192cd_start_xmit_pci(struct sk_buff *skb, struct net_device *dev, int tx_flag)
{
	...
	if ((passThruWanIdx != -1) && (dev==wlan_device[passThruWanIdx].priv->pWlanDev ||skb->dev==wlan_device[passThruWanIdx].priv->pWlanDev))
	{
#ifdef __ECOS
...
//#endif
	...
	if (!priv->pshare->rf_ft_var.mc2u_disable) {
		if ((OPMODE & WIFI_AP_STATE) && IS_MCAST(skb->data) && (tx_flag != TX_NO_MUL2UNI))
		{
			if (IP_MCAST_MAC(skb->data)
			...
			/*jim 20160415*/
				...
				|| IS_BCAST_MAC(skb->data)
				#endif
				#if defined(A4_STA) && defined(MCAST2UI_REFINE)
...
check_swq_enq:
...
#endif
	{
        ...
        }else if(tx_flag == TX_NO_MUL2UNI && pstat == NULL && (OPMODE & WIFI_AP_STATE)){
                if(priv->pmib->miscEntry.a4_enable){
	                if(!IS_MCAST(da)) {/*just send one 3-address multi/broadcast  for all clients*/
	                    pstat = a4_sta_lookup(priv, da);
	                }
	...
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
    	if (pstat)
        {
            ...
            if (!(pstat->state & WIFI_SLEEP_STATE) &&
                (skb->cb[_SKB_CB_FLAGS] & (_SKB_CB_FLAGS_URGENT | _SKB_CB_FLAGS_DIRECT_TX)))
                swq_out = 0;
        }
	}
...
#ifdef CONFIG_WLAN_HAL_8814BE
	if ((GET_CHIP_VER(priv) == VERSION_8814B) && (priv->pmib->miscEntry.ping_toggle))
	{
		struct iphdr* iph;
		unsigned short proto;
		proto = *((unsigned short *)(skb->data + 2 * ETH_ALEN));
		if (proto == __constant_htons(ETH_P_IP))	
		{
			iph = (struct iphdr *)(skb->data + ETH_HLEN); 
			if (iph->protocol ==  IPPROTO_ICMP)
			{
				skb->cb[_SKB_CB_PRIORITY] = 7;
				skb->cb[_SKB_CB_QNUM] = VO_QUEUE;
				goto xmit_out;	
			}	
		}		
	}
...
#ifdef CMCC_CTC_QOS
			&& (!priv->pshare->force_qos_timeout || ((skb->cb[33] == 0x22) && (skb->cb[34] != 0x22)) )
#endif
		) {//swq_en=1
#ifndef SMP_LOAD_BALANCE_SUPPORT
...
int rtl8192cd_udma_start_xmit(struct sk_buff *skb, struct net_device *dev){
	...
	if((skb!=NULL) && (*((UINT16*)(skb->data+(ETH_ALEN<<1))) == __constant_htons(ETH_P_8021Q))){
		...
		// xmit to root interface
		{
			...
			if(IS_DRV_OPEN(priv) &&  get_assoc_sta_num(priv, 0) && (0 == priv->pmib->miscEntry.func_off) && (1 == priv->pmib->vlan.vlan_enable) && (tag_vid == priv->pmib->vlan.vlan_id)){
		...
		// xmit to vap interface
		{
			...
			for(i=0;i<RTL8192CD_NUM_VWLAN;i++){
				...
				if( IS_DRV_OPEN(priv_tmp) && get_assoc_sta_num(priv_tmp, 0) && (0 == priv_tmp->pmib->miscEntry.func_off) && (1 == priv_tmp->pmib->vlan.vlan_enable) && (tag_vid == priv_tmp->pmib->vlan.vlan_id)){
...
int rtl8192cd_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
...
#if (!defined(CONFIG_RTK_SOC_RTL8198D) && ((defined(CONFIG_RTL8672) || defined(CONFIG_LUNA_G3_SERIES)) && defined(__LINUX_4_4_140__)) || defined(__LINUX_4_16__))
	...
	if (priv->pmib->miscEntry.pkt_dump & PKT_DUMP_TX)
		pkt_dump(priv, skb, (unsigned char *)__FUNCTION__);
#endif	
	...
	skb->cb[33] = 0;
	skb->cb[34] = 0;
	if (priv->pshare->rf_ft_var.force_qos & FORCE_QOS_ENABLE){
		skb->cb[33] = 0x22;

#ifdef CMCC_CTC_QOS
		if (skb->cb[0] && skb->cb[0] < 8) {
			priv->pshare->force_qos_timeout = 10;
			priv->ext_stats.cb_data1++;
			skb->cb[34] = skb->cb[0];
		} else if ((priv->pshare->rf_ft_var.force_qos & (FORCE_QOS_ENABLE || FORCE_NON_QOS_TO_BK))
				&& (priv->pshare->force_qos_timeout > 0)) {
			//set to BK
			skb->cb[34] = 0x22;
		}
#else
...
skip_check_eap:
	...
	skb->cb[_SKB_CB_FLAGS] = check_pkt_type(skb);
	...
	if (priv->pshare->swq_cnt + skb_qlen >= priv->pshare->rf_ft_var.swq_max_enqueue_len) {
		...
		dev_kfree_skb_any(skb);
		return 0; 
	} else if (priv->pshare->swq_cnt == 0 && skb_qlen == 0) {
		unsigned int used_txbd_num = 0, qnum;
		for (qnum = BK_QUEUE; qnum <= VO_QUEUE; qnum++)
			used_txbd_num += getUsedTXBD(priv, qnum);

		if (used_txbd_num <= priv->pshare->rf_ft_var.direct_tx_bdth) {
			priv->pshare->direct_xmit_num++;
			skb->cb[_SKB_CB_FLAGS] |= _SKB_CB_FLAGS_DIRECT_TX;
			goto direct_xmit;
		}
	}
#endif
	...
	if (rtk_txdata_queue_enqueue(priv, &priv->pshare->skb_xmit_queue[index], skb) == 1){
		priv->ext_stats.txdata_queue_overflow_drop_pkt++;
		priv->ext_stats.tx_drops++;
		dev_kfree_skb_any(skb);
	} else {

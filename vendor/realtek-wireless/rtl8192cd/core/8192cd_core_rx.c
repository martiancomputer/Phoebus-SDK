...
#endif
#else
#if defined(CONFIG_WIRELESS_LAN_MODULE) || defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)   || (defined(CONFIG_RTL8672) && defined(__LINUX_3_18__))
#define __MIPS16
#else
#define __MIPS16			__attribute__ ((mips16))
#endif
...
volatile unsigned long long G24_rx_bytes = 0;
volatile unsigned long long G24_tx_bytes = 0;
#endif
...
int check_groupIP_in_m2u_list(struct rtl8192cd_priv *priv, struct sk_buff *skb)
{
	int ret = FALSE;
...
{
#ifdef __LINUX_3_7__
	struct rtnl_link_stats64 *pnet_stats = &(priv->net_stats);
#else
...
)
{
	...
	if (pfrinfo->gather_flag == GATHER_MIDDLE || pfrinfo->gather_flag == GATHER_LAST)
		return FALSE;
	pktlen = pfrinfo->total_len;
#else
...
#endif
			if (nextskb == NULL) {
				rtl_kfree_skb(priv, joinskb, _SKB_RX_);
				return NULL;
			}
...
SBWC_forward_check_free_skb:
	count_rx_data_drops(priv, get_pfrinfo(pskb));
...
void process_netif_rxq(unsigned long task_priv)
{
	...
	while (skb_queue_len(&priv->pshare->netif_rx_queue) > 0) {
		if (count > priv->pshare->rf_ft_var.limit_proc_all_queue)
			break;
		
		pskb = skb_dequeue(&priv->pshare->netif_rx_queue);
		if (pskb == NULL)
			break;
	...
	if (skb_queue_len(&priv->pshare->netif_rx_queue))
...
{
	*(unsigned long *)&(pskb->cb[_SKB_CB_RX_STA]) = (unsigned long)pstat;
	skb_queue_tail(&priv->rx_data_queue, pskb);
	priv->pshare->rxq_ind_seq++;
...
void __rtl_netif_rx(struct rtl8192cd_priv *priv, struct sk_buff *pskb, struct stat_info *pstat)
#else
void rtl_netif_rx(struct rtl8192cd_priv *priv, struct sk_buff *pskb, struct stat_info *pstat)
#endif
{
...
/* Hotspot 2.0 Release 1 */
...
#endif
	{
		...
		if (GET_ROOT(priv)->pmib->multi_ap.multiap_vlan_enable) {
			if (*((unsigned short*)(pskb->data+(ETH_ALEN<<1))) == __constant_htons(ETH_P_8021Q) || *((unsigned short*)(pskb->data+(ETH_ALEN<<1))) == __constant_htons(ETH_P_8021AD))
			{
	...
	if (priv->pmib->miscEntry.pkt_dump & PKT_DUMP_RX)
		pkt_dump(priv, pskb, (unsigned char *)__FUNCTION__);
#endif

#ifdef DOT11K
	if (priv->pmib->dot11StationConfigEntry.dka_pkt_timeout)
	{
		unsigned char pkt_type = check_pkt_type(pskb);
		if ((priv->pmib->dot11StationConfigEntry.dka_pkt_type & BIT0) &&
			(pkt_type & _SKB_CB_FLAGS_ARP))
...
/* Hotspot 2.0 Release 1 */
	...
	if ((OPMODE & WIFI_STATION_STATE) &&
	...
	#elif defined(CONFIG_RTL_XDSL_WIFI_HWACCELERATION)
		{
			...
			if(rtk_xdsl_process_wifi_rx(pskb))
			{
				netif_rx(pskb);
			}
		}
	#elif (defined(CONFIG_RG_WLAN_HWNAT_ACCELERATION)||defined(CONFIG_RTK_FC_WLAN_HWNAT_ACCELERATION)) && !defined(CONFIG_ARCH_LUNA_SLAVE)
    {
#ifdef CONFIG_RTK_MESH
...
#endif
		{
			...
			RCU_READ_UNLOCK();
#endif
#if defined(CONFIG_HWNAT_NETIF_RX_QUEUE_SUPPORT)
			skb_queue_tail(&priv->pshare->hwnat_netif_rx_queue, pskb);
#elif defined(SMP_RX_LOAD_BALANCE) && (SMP_RX_LB_LEVEL == 1)
			skb_queue_tail(&priv->pshare->netif_rx_queue, pskb);
			if (rtk_tasklet_set_triggered(&(priv->pshare->rxq_tasklet)))
				smp_call_function_single_async(SMP_RX_LB_OFFLOAD_CPU, &(priv->pshare->rxq_tasklet_csd));
#else
			rtk_hwnat_netif_rx(pskb);
        ...
        if (pnewskb) {
            ...
            if (rx_gather) {
                ...
                if (subfr_len <= 0) {
                    ...
                    count_rx_data_drops(priv, get_pfrinfo(pnewskb));
            ...
            if (OPMODE & WIFI_AP_STATE)
            {
                ...
                if (priv->pmib->miscEntry.a4_enable) {
                    ...
                    if (!IS_MCAST(pfrinfo->sa) && !IS_BSSID(priv, pfrinfo->sa)) {
...
// Reuse current rc_timer
static __always_inline void reorder_ctrl_timer_reuse(struct reorder_ctrl_timer* rc_timer)
{
	list_del_init(&rc_timer->list);  // only remove it from priv->pshare->rc_timer_list
}

static __always_inline struct reorder_ctrl_timer *reorder_ctrl_timer_alloc(struct rtl8192cd_priv *priv, struct stat_info *pstat, int tid)
{
	struct reorder_ctrl_entry *rc_entry = &(pstat->rc_entry[tid]);
	struct reorder_ctrl_timer *prc_timer = NULL;
	int idx = 0;

	for (idx = 0; idx < RC_TIMER_NUM; idx++) {
		if (priv->pshare->rc_timer_pool[idx].pstat == NULL) {
			/* Found unused rc_timer */
	...
	if (idx == RC_TIMER_NUM) {  // Failed to find unused rc_timer
...
static __always_inline void reorder_ctrl_timer_list_add(struct rtl8192cd_priv *priv, struct reorder_ctrl_timer *prc_timer)
{
	// Update timeout
...
__IRAM_IN_865X
#endif
static unsigned long do_reorder_ctrl_timeout(struct rtl8192cd_priv *priv)
{
	...
	while (phead && (plist != phead)) {
		...
		if (time_after_eq(current_time, prc_timer->timeout)) {
			...
			if (SN_LESS(win_start, rc_entry->last_seq) || (win_start == rc_entry->last_seq)) {
				// Still have reordered pkts, renew timeout of rc_timer.
				// Update timeout of rc_timer directly instead of deleting rc_timer and add
				// a new one back, since we are traversing priv->pshare->rc_timer_list
				prc_timer->timeout = jiffies + ((OPMODE & WIFI_STATION_STATE) ? priv->pshare->rc_timer_tick_cli : priv->pshare->rc_timer_tick);
				if (!prc_timer->timeout)
					prc_timer->timeout = 1;
				...
				// All reordered pkts are out, delete rc_timer
				reorder_ctrl_timer_del(rc_entry->rc_timer);
				rc_entry->rc_timer = NULL;
				rc_entry->start_rcv = FALSE;
			}
...
__IRAM_IN_865X
#endif
static struct reorder_ctrl_timer *reorder_ctrl_timer_add(struct rtl8192cd_priv *priv, struct stat_info *pstat, int tid)
{
	struct reorder_ctrl_timer *prc_timer = NULL;
	unsigned long next_timeout = 0;

	// Process time-outed reordered packets
	if (rtk_timer_pending(&priv->pshare->rc_sys_timer)) {
		rtk_del_timer(&priv->pshare->rc_sys_timer);
	}
	next_timeout = do_reorder_ctrl_timeout(priv);

	// Try to allocate a new rc_timer from pool
	prc_timer = reorder_ctrl_timer_alloc(priv, pstat, tid);

	// Update timeout of prc_timer and add prc_timer to priv->pshare->rc_timer_list
	if (prc_timer) {
		reorder_ctrl_timer_list_add(priv, prc_timer);

		// Update next_timeout for rc_sys_timer
		if (next_timeout == 0 || time_before(prc_timer->timeout, next_timeout))
			next_timeout = prc_timer->timeout;
	}
	...
	if (next_timeout > 0) {
		...
		rtk_mod_timer(&priv->pshare->rc_sys_timer, next_timeout);
	}
...
__IRAM_IN_865X
#endif
static void reorder_ctrl_timer_update(struct rtl8192cd_priv *priv, struct stat_info *pstat, int tid)
{
	struct reorder_ctrl_entry *rc_entry = &(pstat->rc_entry[tid]);
	struct reorder_ctrl_timer *prc_timer = rc_entry->rc_timer;
	unsigned long next_timeout = 0;

	// We attempt to reuse current rc_timer. Hence, we just remove it from priv->pshare->rc_timer_list
	// but keep pstat->rc_entry[tid]->rc_timer pointed to current rc_timer object. Removing the rc_timer
	// object from the list prevents the rc_timer from being processed by do_reorder_ctrl_timeout().
	reorder_ctrl_timer_reuse(prc_timer);
...
__IRAM_IN_865X
#endif
static __always_inline void reorder_ctrl_timeout(unsigned long task_priv)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;
	unsigned long flags = 0;
	unsigned long next_timeout = 0;
	...
	process_hwnat_netif_rx_queue(priv);
#endif
}
DEFINE_TIMER_CALLBACK(reorder_ctrl_timeout);

void init_rc_sys_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->pshare->rc_sys_timer), reorder_ctrl_timeout, (unsigned long)priv, 0);
}
...
int check_win_seqment(unsigned short win_start, unsigned short win_end, unsigned short seq)
{
	...
	if (win_start < win_end) {
		if (win_start < seq && seq < win_end)
			return 3;
		else {
			middle = (win_end + ((0xfff - win_end) + win_start) / 2) & 0xfff;
			if (middle > win_end) {
				if (win_end <= seq && seq < middle)
					return 4;
				else
					return 1;
			} else {
				if (middle < seq && seq < win_start)
					return 1;
				else
					return 4;
			}
		...
		if (win_start < seq || seq < win_end) {
			...
			middle = win_end + (win_start - win_end) / 2;
			if (seq > middle)
				return 1;
			else
				return 4;
...
int reorder_ctrl_check(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct rx_frinfo *pfrinfo)
{
	...
	else
	{
		if (segment == 1) {
			...
			// If there is no queued reordered pkts, end reorder_ctrl_check
			...
			// Process queued reordered pkts until reach next hole in sequence or cleanup all queued pkts
			win_start = SN_NEXT(win_start);
			while (rc_entry->packet_q[win_start & (win_size - 1)]) {
				reorder_ctrl_pktout(priv, rc_entry->packet_q[win_start & (win_size - 1)], pstat,
						rc_entry->frinfo_q[win_start & (win_size - 1)]);
			...
			if (!SN_LESS(rc_entry->last_seq, win_start)) {
				// Still have reordered pkts, renew timeout of rc_timer
				reorder_ctrl_timer_update(priv, pstat, tid);
			} else {
				// All reordered pkts are out, delete rc_timer
				reorder_ctrl_timer_del(rc_entry->rc_timer);
				rc_entry->rc_timer = NULL;
...
#endif
			{
				if (SN_LESS(rc_entry->last_seq, seq))
					rc_entry->last_seq = seq;
				if (rc_entry->rc_timer == NULL) {
					// Get the first reordered pkts, try to add a rc_timer
					if (reorder_ctrl_timer_add(priv, pstat, tid) == NULL)
			...
			if (rc_entry->rc_timer != NULL) {
				// At least half of the queued reordered pkts are out, renew timeout of rc_timer
				reorder_ctrl_timer_update(priv, pstat, tid);
			} else {
				// It's wiered that we reach here ... try to add a rc_timer
				if (reorder_ctrl_timer_add(priv, pstat, tid) == NULL)
...
#ifdef CLIENT_MODE
inline unsigned char check_packet_number_overflow(union PN48 rxPN, union PN48 curPN)
{
	if ((curPN.val48 > 0xffffffff0000) &&
		 (rxPN.val48 <= 0xffff)) {
		return 1;
	}
	return 0;
...
inline unsigned char mcast_packet_number_check(struct rtl8192cd_priv *priv, unsigned char *pframe, unsigned int hdr_len)
{
	...
	if(get_mcast_encrypt_algthm(priv) == _CCMP_PRIVACY_) {
		...
		if((rxPN.val48 <= pCurRxPN->val48) &&
			!check_packet_number_overflow(rxPN, *pCurRxPN)) {
        ...
        (GetFragNum(pframe) == 0) && (GetMFrag(pframe) == 0))
    {
        ...
        if (pstat->rx_privacy == privacy)
        {
            ...
            if (pfrinfo->to_fr_ds == 3 && priv->pmib->miscEntry.a4_enable) {
                ...
                if (memcmp(GetAddr4Ptr(pframe), prxsc_entry->rx_wlanhdr.addr4, 6))
                    goto shouldnot_rxsc;
                if (memcmp(BSSID, GetAddr4Ptr(pframe), MACADDRLEN)) {
...
#if defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0))
	int vid = 0;
	vid = get_80211frameVlanID(priv, pstat, pfrinfo);
#endif
#endif
                ...
                )
				{
					...
					if(vid)
					{
						...
						if(wlan_dev == NULL)
						{
							...
							if(wlan_dev == NULL) {
								count_rx_data_drops(priv, pfrinfo);
								DEBUG_ERR("RX DROP: vlan interface not found!!(vid=%d)\n", vid);
								rtl_kfree_skb(priv, pfrinfo->pskb, _SKB_RX_);
								return 1;
							}
						}
...
static struct rx_frinfo *defrag_frame_main(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
	...
	static const u8 pae_group_addr[ETH_ALEN] __aligned(2) = { 0x01, 0x80, 0xC2, 0x00, 0x00, 0x03};  // IEEE Std 802.1X PAE address: 01-80-C2-00-00-03
...
#endif
	if (!GetPrivacy(pframe) && (priv->pmib->dot1180211AuthEntry.dot11PrivacyAlgrthm != _NO_PRIVACY_))
	{
		unsigned short proto;
		...
		if (amsdu ||
			(!(
#if (defined(RTK_MULTI_AP) && (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0)))
			((proto == __constant_htons(ETH_P_8021Q)) && (*((UINT16 *)(pframe + hdr_len + sizeof(struct wlan_llc_t) + 7)) == __constant_htons(LIB1X_ETHER_EAPOL_TYPE))) ||
#endif
			(proto == 0x888e)))
			)
		{
		...
		if ((
			...
			&& !isEqualMACAddr(pfrinfo->da, GET_MY_HWADDR))
		{
			if(isEqualMACAddr(pfrinfo->da, pae_group_addr))
			{
				DEBUG_INFO("Replace PAE group addr with wlan mac.\n");
				memcpy(pfrinfo->da, GET_MY_HWADDR, ETH_ALEN);
			}
			else
			{
				count_rx_data_drops(priv, pfrinfo);
				DEBUG_ERR("RX DROP: EAPOL packet with malformed Mac address.\n");
				goto free_skb_in_defrag;
			}
		}
	}
	...
	if (GetMFrag(pframe))
	{
		...
		if (pfrinfo->gather_flag == GATHER_FIRST) {
			count_rx_data_drops(priv, pfrinfo);
			DEBUG_ERR("RX DROP: NOT support AMSDU defrag!\n");
			goto free_skb_in_defrag;
		}
#endif
		...
		if(memcmp(rxmic, tkipmic, 8))
		{
			...
			if (OPMODE & WIFI_AP_STATE)
			{
				...
				if ((pfrinfo->to_fr_ds == 3) &&
					...
					goto free_skb_in_defrag;
#endif
				if (pstat->skipFirstTkipMic) {
					pstat->skipFirstTkipMic--;
					goto free_skb_in_defrag;
				}
...
char rtw_verify_eapol(struct rtl8192cd_priv *priv,
				unsigned char *da,
				unsigned char *msg,
				int len)
{
	...
	struct lib1x_eapol_pkt_hdr *eap_hdr;

	eapol = (struct lib1x_eapol *) (msg + ETHER_HDRLEN);

	if (eapol->packet_type == LIB1X_EAPOL_EAPPKT) {
		eap_hdr = (struct lib1x_eapol_pkt_hdr *) (eapol + 1);
		/* legal eap code as follows :
			...
			4       Failure */
		if (eap_hdr->code > 5) {
			printk("!!!!!! eapol : code = %d, len = %d \n",
				eap_hdr->code, eap_hdr->length);
			err = 1;
		}
	}
#else
	if (memcmp(BSSID, da, MACADDRLEN) != 0) {
		err = 1;
		printk("!!!!! wrong da : %02X-%02X-%02X-%02X-%02X-%02X \n",
...
static int process_datafrme(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
	...
	if (OPMODE & WIFI_AP_STATE)
	{
		...
		if ((QOS_ENABLE) && (APSD_ENABLE) && (pstat->QosEnabled) && (pstat->apsd_bitmap & 0x0f) &&
			...
			(GetFrameSubType(pframe) == (WIFI_QOS_DATA))) {
			process_APSD_dz_queue(priv, pstat, pfrinfo->tid);
		}
#endif
...
//			if (get_free_memory() < FREE_MEM_LOWER_BOUND) {
			...
			RCU_READ_UNLOCK();
				...
				{
					if(!IS_BCAST2(da) && (check_si_mem(priv) == 0))
					{
						count_rx_data_drops(priv, pfrinfo);
				        goto mcast_netif_rx;
					}
					...
					if (pnewskb) {
						...
						else
						{
							...
							if(dontBcast2otherSta){
								...
								{
#if defined(CONFIG_BOLOCK_IPTV_FROM_LAN_SERVER) && defined(CONFIG_RTK_L34_FLEETCONNTRACK_ENABLE)
									if((block_iptv_from_lanWlan_server) && check_groupIP_in_m2u_list(priv, pnewskb))
									{
										//in same interface, block iptv data from a sta to other stations  
										dev_kfree_skb_any(pnewskb);
									}
									else
#endif
...
mcast_netif_rx:
		...
		else
		{
			...
			if (*(unsigned short *)(SKB_MAC_HEADER(pskb) + MACADDRLEN*2) ==
				__constant_htons(0x888e))
			{
				if (rtw_verify_eapol(priv, da, pskb->data, pskb->len))
				{
					count_rx_data_drops(priv, pfrinfo);
					return FAIL;
				}
				...
				struct rx_frinfo *inputPfrinfo)
{
	...
	static volatile unsigned long long prev_rx_bytes = 0;
	static volatile unsigned long long prev_tx_bytes = 0;
	unsigned long j, diff;
	unsigned long long rxb = 0; /* Current rx bytes */
	unsigned long long txb = 0; /* Current tx bytes */
	unsigned long long totalb = 0; /* Total tx/rx bytes */
#ifdef __LINUX_3_7__
	...
	if(priv->pshare->use_hal)  { //This is 8192ER 2.4GHz traffic
		ns = &(priv->net_stats);
		if(ns != NULL) {
			G24_rx_bytes = (unsigned long long)ns->rx_bytes;
			G24_tx_bytes = (unsigned long long)ns->tx_bytes;
		} else {
...
extern struct rtk_timer_list G5_rx_drop_timer;
				rtk_mod_timer(&G5_rx_drop_timer, jiffies+HZ);
			}
			prev_jiffies = j;
			prev_rx_bytes = rxb;
			prev_tx_bytes = txb;
	...
	if (is_qos_data(pframe)) {
		if ((OPMODE & WIFI_AP_STATE) && (QOS_ENABLE)) {
			if ((pfrinfo->tid == 7) || (pfrinfo->tid == 6)) {
				...
				priv->ext_stats.rx_vi_pkts++;
				priv->ext_stats.rx_vi_bytes += pfrinfo->pktlen;
#endif
...
#ifdef ALLOW_MGNT_FRAME_WITH_TO_DS
#ifdef MBSSID
		return rtl8192cd_rx_dispatch_mgmt_adhoc(&priv, pfrinfo, vap_idx);
#else
		return rtl8192cd_rx_dispatch_mgmt_adhoc(&priv, pfrinfo);
#endif // MBSSID
#else  // !(ALLOW_MGNT_FRAME_WITH_TO_DS)
		goto out;
#endif // ALLOW_MGNT_FRAME_WITH_TO_DS
	}
...
/*#ifdef A4_STA
	...
	if (OPMODE & WIFI_AP_STATE) {
#ifdef A4_STA
        if (priv->pmib->miscEntry.a4_enable && IS_BSSID(priv, pfrinfo->sa)){
            // Illegal address: pfrinfo->sa == our mac
            priv->ext_stats.rx_data_drops++;
            DEBUG_WARN("RX DROP: Illegal address, to_fr_ds=%d, Addr1=%pM, Addr2=%pM, Addr3=%pM, Addr4=%pM\n",
            pfrinfo->to_fr_ds, GetAddr1Ptr(pframe), GetAddr2Ptr(pframe), GetAddr3Ptr(pframe), GetAddr4Ptr(pframe));
            goto free_skb_in_defrag;
        }
#endif //A4_STA
	...
	if(pfrinfo->to_fr_ds == 3 && (priv->pmib->dot11WdsInfo.wdsEnabled || priv->pmib->dot1180211sInfo.mesh_enable)) {
		if (pstat == NULL){
			count_rx_data_drops(priv, pfrinfo);
			DEBUG_WARN("RX Drop: WDS/MESH rx data with pstat == NULL\n");
			goto free_skb_in_defrag;
		} 
	} else
#endif
...
int rtl8192cd_rx_dispatch_offload(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
	...
	pfrinfo->retry = GetRetry(pfrinfo->rxpkt_info->frame_ctrl);
#if defined(CONFIG_RTL8672) || defined(CONFIG_WLAN_STATS_EXTENTION)
	pfrinfo->bcast = IS_BCAST2(pfrinfo->da);
	pfrinfo->mcast = IS_MCAST(pfrinfo->da);
#endif
...
{
	unsigned char	*sa, *da, *myhwaddr, *pframe=NULL, is_amsdu=0;
	unsigned int	frtype;
...
#endif
	pframe = get_pframe(pfrinfo);
	is_amsdu = is_qos_data(pframe)?(*GetQosControl(pframe) & BIT(7)):0;
...
#if defined(CONFIG_WLAN_HAL_8814BE) && (defined(CONFIG_RTL9607C) || defined(_CORTINA_)) && !defined(SMP_RX_LOAD_BALANCE)
#ifdef RING_BASED_DATA_FRAME_QUEUE
			if (rtk_data_queue_enqueue(priv_intf, &priv_intf->data_queue, pfrinfo)) {
				return 1;
			}
#else // !(RING_BASED_DATA_FRAME_QUEUE)
			SMP_LOCK_RX_DATA(x);
...
//		printk("pfrinfo->hdr_len == 0\n");
	...
	check_unassoc_sta_match(priv, pfrinfo);
#endif
...
//#endif
    ...
    if ((frtype == WIFI_DATA_TYPE) && (ODMPTR->support_ability & ODM_BB_ADAPTIVE_SOML))
    {
		//panic_printk(" rate %d, len %d\n", pfrinfo->rx_rate, pfrinfo->pktlen);

		int rate = 0;
		if (pfrinfo->rx_rate >= VHT_RATE_ID)
		{
			rate = pfrinfo->rx_rate - VHT_RATE_ID + 12 + 32;
		}
		else if (pfrinfo->rx_rate >= HT_RATE_ID)
		{
			rate = pfrinfo->rx_rate - HT_RATE_ID + 12;
		}
...
void rtl88XX_ipc_rx_isr(struct rtl8192cd_priv *priv, struct sk_buff *pskb)
{
			...
			q_num	= 0;	
				...
				if (RT_STATUS_SUCCESS != GET_HAL_INTERFACE(priv)->QueryRxDescHandler(priv, q_num, pskb->data, &rx_desc_status)) {
					...
					if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
						if (IS_PCIBIOS_TYPE) {
							dma_unmap_single(&priv->pshare->pdev->dev, phw->rx_infoL[cur_host_idx].paddr, (RX_BUF_LEN - sizeof(struct rx_frinfo)), DMA_FROM_DEVICE);
						}
					}
#endif
					pfrinfo = NULL;
				...
				if (rx_desc_status.CRC32) {
...
#ifdef TRX_VI_DATA_LOG
					if (is_qos_data(get_pframe(pfrinfo)) && ((pfrinfo->tid == 5) || (pfrinfo->tid == 4)))	// count Rx VI error pkts
						priv->ext_stats.rx_vi_error_pkts++;
#endif
					goto rx_reuse;
				}
					...
					if (rx_desc_status.C2HPkt) {
						...
						if (OFFLOAD_ENABLE(priv)) {
							...
							GET_HAL_INTERFACE(priv)->C2HPacketHandler(priv, get_pframe(pfrinfo), pfrinfo->pktlen);
							SMP_LOCK_RECV(y);
...
void _count_icv_error(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
	unsigned char *pframe = get_pframe(pfrinfo);
	unsigned char to_fr_ds = get_tofr_ds(pframe);
	unsigned char *sa;
	struct stat_info *pstat = NULL;
	
	if ((OPMODE & WIFI_AP_STATE) && (to_fr_ds == 0x02)) {
		sa = get_sa(pframe);
		pstat = get_stainfo(priv, sa);
		if (pstat) {
			pstat->rx_icverr_pkts++;
		}
	}
}
...
void rtl88XX_rx_isr(struct rtl8192cd_priv *priv)
{
	...
	while (cur_host_idx != hw_idx)
	{
        ...
        if ( RT_STATUS_SUCCESS != GET_HAL_INTERFACE(priv)->QueryRxDescHandler(priv, q_num, pskb->data, &rx_desc_status) ) {
			...
			if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
				if (IS_PCIBIOS_TYPE) {
					dma_unmap_single(&priv->pshare->pdev->dev, phw->rx_infoL[cur_host_idx].paddr, (RX_BUF_LEN-sizeof(struct rx_frinfo)-64), DMA_FROM_DEVICE);
				}
			}
#endif
            priv->ext_stats.rx_reuse++;
		...
		if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
			if (IS_PCIBIOS_TYPE) {
				...
				if (rx_desc_status.FS == 0x01 && rx_desc_status.LS == 0x01)
					size = sizeof(RX_DESC_88XX) + rx_desc_status.DRV_INFO_SIZE + rx_desc_status.SHIFT + rx_desc_status.PKT_LEN;
				else
					size = rx_desc_status.RXBuffSize;
				dma_unmap_single(&priv->pshare->pdev->dev, phw->rx_infoL[cur_host_idx].paddr, size, DMA_FROM_DEVICE);
			}
		}
#endif
...
#if	0 /*eric-8822 check csi*/
{
	...
	if(frtype_tmp == Type_Action_No_Ack) {
		...
		if (rx_desc_status.FS && !rx_desc_status.C2HPkt) {
#ifdef CONFIG_ADAPTIVE_SOML
			//soml new feature
...
#endif // !defined(RX_BUFFER_GATHER)
#if defined(__OSK__) && defined(CONFIG_RTL8672)
		if(AvailableMemSize < (100*1024) && (!rx_desc_status.C2HPkt))
...
#endif
        {
            ...
            if (!(rx_desc_status.FS == 0x01 && rx_desc_status.LS == 0x01)) {
				if (rx_desc_status.FS == 0x01 && priv->pshare->gather_state == GATHER_STATE_NO) {
					...
					pfrinfo->total_len = rx_desc_status.PKT_LEN;
				...
				pfrinfo->total_len = pfrinfo->pktlen;
			}
...
#endif
            {
				...
				if (OFFLOAD_ENABLE(priv)) {
					...
					pfrinfo->rxdma_info = (struct d2h_rxdma_info *)get_pframe(pfrinfo);
					pfrinfo->rxpkt_info = (struct d2h_rxpkt_info *)(get_pframe(pfrinfo)+sizeof(struct d2h_rxdma_info));
				
					skb_shift_size = sizeof(struct d2h_rxdma_info) + sizeof(struct d2h_rxpkt_info) + pfrinfo->rxpkt_info->dummy_len;
					if (pfrinfo->rxpkt_info->pkt_type == RXPKTINFO_TYPE_RAW)
						skb_shift_size += sizeof(RX_DESC_88XX);
					skb_shift_size += sizeof(RX_DESC_88XX);
					skb_reserve(pskb, skb_shift_size);
	...
	if (IS_SUPPORT_HW_FILL_MACID(priv)) {
			...
			if (!pfrinfo->physt) {
				pfrinfo->rf_info.rssi = 0;
			} else if (priv->pshare->rf_ft_var.spec_env_test_en & BIT2) {
				pfrinfo->rf_info.rssi = 40;
				pfrinfo->rf_info.mimorssi[0] = 0;
				pfrinfo->rf_info.mimorssi[1] = 0;
			} else {
#if defined(RX_BUFFER_GATHER)
...
#ifdef RX_BUFFER_GATHER
			if (pfrinfo->gather_flag && pfrinfo->gather_flag != GATHER_FIRST)
				free_page = 0xFFFFFFFF;
			else
#endif
			if (priv->pshare->rf_ft_var.rx_min_rsvd_mem)
	...
	//-------------------------------------------------------------------------------------------
...
#endif // defined(MP_TEST)
			{
                ...
                if (rx_desc_status.FS == 0x01 && rx_desc_status.LS == 0x01) {
                    ...
                    skb_used_size = 64 + sizeof(struct rx_frinfo) + pfrinfo->pktlen;
                }
#else
                skb_used_size = 64 + sizeof(struct rx_frinfo) + SIZE_RXDESC_88XX + 
                    pfrinfo->driver_info_size + pfrinfo->rxbuf_shift + pfrinfo->pktlen + _CRCLNG_;
                ...
                if (skb_used_size <= RX_BUF_LEN) {
                    ...
                    if (rx_desc_status.ICVERR) {
						rx_pkt_exception_88XX(priv, rx_desc_status.CRC32, rx_desc_status.ICVERR, rx_desc_status.RX_RATE);
						_count_icv_error(priv, pfrinfo);
#ifdef TRX_VI_DATA_LOG
						pframe = get_pframe(pfrinfo);
						if (is_qos_data(pframe)) {
							pfrinfo->tid = GetTid(pframe) & 0x07;
							if ((pfrinfo->tid == 5) || (pfrinfo->tid == 4))	// count Rx VI error pkts
								priv->ext_stats.rx_vi_error_pkts++;
						}
#endif
#if !defined(DELAY_REFILL_RX_BUF)	/*we create this, but we do not free it!*/
...
#endif
#if 0	/*Fix: bridge_handle_frame core dump, epc = 0x00000006*/
						rtl_kfree_skb(priv, pskb, _SKB_RX_);
						reuse = 0;
#endif
					}
...
rx_reuse:
		...
		if (cmp_flags)
		{
			...
			// restore skb->data as first getting
			pskb->data = pskb->head + pfrinfo->def_headroom;
			skb_reset_tail_pointer(pskb);
...
//            memset(bus_to_virt(phw->rx_infoL[cur_host_idx].paddr), 0, RX_BUF_LEN - offset);
        ...
        rtl_cache_sync_wback(priv, (unsigned long)(bus_to_virt(phw->rx_infoL[cur_host_idx].paddr)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), 
            RX_BUF_LEN - offset, DMA_FROM_DEVICE);
#endif  //CONFIG_ENABLE_CCI400
#else
...
rx_done:
		...
		if (cur_host_idx == hw_idx) {
			...
			hw_idx = cur_q->hw_idx;
			update = 1;
		} else if (priv->pmib->miscEntry.update_rxbd_th && (n % priv->pmib->miscEntry.update_rxbd_th == 0)) {
			update = 1;
		}
...
void rtl8192cd_rx_isr(struct rtl8192cd_priv *priv)
{
...
#endif //CONFIG_WLAN_HAL
	{
		...
		rtl_cache_sync_wback(priv, (unsigned long)prxdesc, sizeof(struct rx_desc), DMA_FROM_DEVICE);
#else
		...
		while (1)
		{
			...
			#if defined(CONFIG_NET_PCI) && !defined(USE_RTL8186_SDK)
			if (IS_PCIBIOS_TYPE) {
				dma_unmap_single(&priv->pshare->pdev->dev, phw->rx_infoL[tail].paddr, (cmd & RX_PktLenMask), DMA_FROM_DEVICE);
			}
			#endif
	...
	//					printk("pkt in\n");
						...
						if (priv->pshare->gather_state != GATHER_STATE_NO) {
							...
							if (priv->pshare->gather_state == GATHER_STATE_LAST) {
								if (!search_first_segment(priv, &pfrinfo))
									...
									pfrinfo->total_len = (cmd & RX_PktLenMask) - _CRCLNG_;
									pskb = pfrinfo->pskb;
									pfrinfo_update = 1;
#if defined(RX_BUFFER_GATHER_REORDER)
									priv->pshare->gather_list_p = NULL;
                        ...
                        reuse = 0;
	                        ...
	                        if (pfrinfo->gather_flag & GATHER_FIRST){
								...
								flush_rx_list_inq(priv, pfrinfo);
#endif
#if !defined(NOT_RTK_BSP) || !defined(RX_BUFFER_GATHER_REORDER)
	                            rtl_kfree_skb(priv, pskb, _SKB_RX_);
	                            reuse = 0;
	...
	rx_reuse:
		...
		if(pfrinfo && (priv->pshare->gather_state != GATHER_STATE_NO)) {
...
#endif	/*	defined(DELAY_REFILL_RX_BUF)	*/
			{
...
#if defined(CONFIG_NET_PCI) && !defined(USE_RTL8186_SDK)
				if (IS_PCIBIOS_TYPE) {
					phw->rx_infoL[tail].paddr = get_physical_addr(priv, pskb->data, (RX_BUF_LEN - sizeof(struct rx_frinfo)), DMA_FROM_DEVICE);
				}
#endif
				...
				//rtl_cache_sync_wback(priv, (unsigned long)bus_to_virt(phw->rx_infoL[tail].paddr), RX_BUF_LEN - sizeof(struct rx_frinfo)-64, DMA_FROM_DEVICE);
#else
				rtl_cache_sync_wback(priv, (unsigned long)bus_to_virt(phw->rx_infoL[tail].paddr-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), RX_BUF_LEN - sizeof(struct rx_frinfo)-64, DMA_FROM_DEVICE);
#endif

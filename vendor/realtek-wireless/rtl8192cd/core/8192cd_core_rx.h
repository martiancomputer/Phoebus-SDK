	...
	if (pfrinfo->physt) {
...

			{
				if (pfrinfo->rf_info.mimorssi[0] != 0 || pfrinfo->rf_info.mimorssi[1] != 0) {
					...
					memcpy(&pstat->rf_info.mimosq[0], &pfrinfo->rf_info.mimosq[0], 4);
#ifdef USE_OUT_SRC
					if (IS_OUTSRC_CHIP(priv)) {
						for (i = 0; i < 4; i++) {
    ...
    )
{
	...
	phw->rx_infoL[i].paddr	= get_physical_addr(priv, pskb->data, (RX_BUF_LEN - offset), DMA_FROM_DEVICE) + CONFIG_LUNA_SLAVE_PHYMEM_OFFSET_HAL;
#else
    phw->rx_infoL[i].paddr  = get_physical_addr(priv, pskb->data, (RX_BUF_LEN - offset), DMA_FROM_DEVICE);
#endif

    *pBufAddr   = (u4Byte)pskb->data;
#if 0    
    *pBufLen    = RX_BUF_LEN - offset;
#else
    ...
    *pBufLen    = RX_BUF_LEN - sizeof(struct rx_frinfo) - 64;
    ...
    //rtl_cache_sync_wback(priv, bus_to_virt(phw->rx_infoL[i].paddr), RX_BUF_LEN - offset, DMA_FROM_DEVICE);     
#else
	...
	if (OFFLOAD_ENABLE(priv))
		rtl_cache_sync_wback(priv, (unsigned long)(bus_to_virt(phw->rx_infoL[i].paddr)-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), 
			2600 - offset, DMA_FROM_DEVICE);
	else
#endif
	if ((priv->pshare->veriwave_sta_num) && (NULL != priv->pshare->highTP_found_pstat) &&
			(!(priv->pshare->highTP_found_pstat->AMSDU_AMPDU_support & STA_TX_AMSDU_SUPPORT))) {
		_dma_cache_wback((unsigned long)(bus_to_virt(phw->rx_infoL[i].paddr) - CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), 1800);
	} else {

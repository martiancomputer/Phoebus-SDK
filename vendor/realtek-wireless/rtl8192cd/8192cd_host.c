...
struct pcie_para ePHY[][PHY_PARAM_SIZE] = {
	...
	{0xff,0xff,0xffff}}, //8685 25M clk
      {{0, 0, 0x404c},	{0, 1, 0x16a3},	{0, 2, 0x6340},	{0, 3, 0x370d},	
        {0, 4, 0x4644}, {0, 5, 0x8109}, {0, 6, 0x2081}, {0, 7, 0x5440},
	{0, 8, 0x9000},	{0, 9, 0x0ccc},	{0, 0xa, 0x4437},	{0, 0xb, 0x0230}, 	
	{0, 0xc, 0x0021},	{0, 0xd, 0x0000},	{0, 0xe, 0x0000},	{0, 0x1f, 0x0000}, 
	...
	{	
		...
		{ 0, 0x07, 0x52b3 }, { 0, 0x08, 0x5285 }, { 0, 0x09, 0x6300 }, { 0, 0x0b, 0x0009 },
		{ 0, 0x0c, 0x0800 }, { 0, 0x0e, 0x0093 }, { 0, 0x20, 0x0105 }, { 0, 0x21, 0x1000 },
		{0xff,0xff,0xffff}
	}, // 8685G P0
	{		
		{ 1, 0x00, 0x8a51 }, { 1, 0x02, 0x26f9 }, { 1, 0x03, 0x6bcd }, { 1, 0x06, 0x104a }, 		
		{ 1, 0x07, 0x52b3 }, { 1, 0x08, 0x5285 }, { 1, 0x09, 0x6300 }, { 1, 0x0b, 0x0009 },
		{ 1, 0x0c, 0x0800 }, { 1, 0x0e, 0x0093 }, { 1, 0x20, 0x0105 }, { 1, 0x21, 0x1000 },
		{0xff,0xff,0xffff}
	}, // 8685G P1
	{		
	...
	{		
		...
		{ 1, 0x06, 0x1088 }, { 1, 0x07, 0x52b3 }, { 1, 0x08, 0x5285 }, { 1, 0x09, 0x6300 },
		{ 1, 0x0b, 0x0009 }, { 1, 0x0c, 0x0800 }, { 1, 0x0e, 0x0093 }, { 1, 0x20, 0x0105 },
		{ 1, 0x21, 0x1000 },
		{0xff,0xff,0xffff}
	} // 9607C P1
...
static void __pcie_param_fixup(void)
{
	...
	if (rev == CHIP_REV_ID_C) {
		printk("IC-C v007 (2025.03.25)\n");
		if (ePHY[CLK25_9607C_P0][0].port==0) {
			struct pcie_para g2phy[PHY_PARAM_SIZE] = {
				{0, 0x01, 0xa852 },{0, 0x06, 0x0017 },{0, 0x08, 0x3591 },{0, 0x09, 0x520c },
				{0, 0x0a, 0xf670 },{0, 0x0b, 0xa90d },{0, 0x0d, 0xe720 },{0, 0x0e, 0x1010 },
				{0, 0x1c, 0x2001 },{0, 0x1e, 0x66eb },{0, 0x20, 0xd4a4 },{0, 0x21, 0x485a },
				{0, 0x23, 0x0b66 },{0, 0x24, 0x4f0c },{0, 0x29, 0xf0f3 },{0, 0x2b, 0xa0a1 },
				{0, 0x09, 0x500c },{0, 0x09, 0x520c },

				{0, 0x41, 0xa849 },{0, 0x46, 0x0017 },{0, 0x48, 0x3591 },{0, 0x49, 0x520c },
				{0, 0x4a, 0xf650 },{0, 0x4b, 0xa90d },{0, 0x4d, 0xe720 },{0, 0x4e, 0x1010 },
				{0, 0x5c, 0x2001 },{0, 0x60, 0xd4a6 },{0, 0x61, 0x586a },{0, 0x63, 0x0b66 },
				{0, 0x69, 0xf0f3 },{0, 0x6b, 0xa0a1 },{0, 0x6f, 0x5046 },
		...
		if (ePHY[CLK25_9607C_P1][0].port==1) {
			struct pcie_para g1phy[PHY_PARAM_SIZE] = {
				{1, 0x01, 0xa852 },{1, 0x06, 0x0017 },{1, 0x08, 0x3591 },{1, 0x09, 0x520c },
				{1, 0x0a, 0xf670 },{1, 0x0b, 0xa90d },{1, 0x0d, 0xe720 },{1, 0x0e, 0x1010 },
				{1, 0x1c, 0x2001 },{1, 0x1e, 0x66eb },{1, 0x20, 0xd4a4 },{1, 0x21, 0x485a },
				{1, 0x23, 0x0b66 },{1, 0x24, 0x4f0c },{1, 0x29, 0xf0f3 },{1, 0x2b, 0xa0a1 },
				{1, 0x09, 0x500c },{1, 0x09, 0x520c },

				{0xff, 0xff, 0xffff}
			};

			BUG_ON(sizeof(ePHY[CLK25_9607C_P1])!=sizeof(g1phy));
			memcpy(ePHY[CLK25_9607C_P1], g1phy, sizeof(g1phy));
		}
	}
	else if (rev == CHIP_REV_ID_B) {
		printk("IC-B v1.3 (2021.06.09)\n");
		if (ePHY[CLK25_9607C_P0][0].port==0) {
			struct pcie_para g2phy[PHY_PARAM_SIZE] = {
				...
				{ 0, 0x4d, 0xef16 },{ 0, 0x4e, 0x0000 },{ 0, 0x4f, 0x000c },{ 0, 0x5b, 0xaea1 },
				{ 0, 0x5e, 0x28eb },{ 0, 0x60, 0x94aa },{ 0, 0x61, 0x88ff },{ 0, 0x62, 0x0093 },
				{ 0, 0x67, 0x011a },{ 0, 0x6f, 0x65bd },{ 0, 0x49, 0x500c },{ 0, 0x49, 0x520c },
				{ 0xff, 0xff, 0xffff }
			};
...
static int use_plat_pcie_reset(void) {
	uint32  chipId,rev,subType  = 0;
    rtk_switch_version_get(&chipId,&rev,&subType);
	return (chipId == RTL9603CVD_CHIP_ID);
}
#endif
...
#ifdef CONFIG_RTL9603CVD_SERIES
	if (use_plat_pcie_reset()) {
		extern int bsp_pcie_reset(int portnum);
		return bsp_pcie_reset(portnum) ? FAIL : SUCCESS;
	}
#endif

...
//    unsigned long int l_out, r_out;
	...
	{
		if (tx)
			rtl_cache_sync_wback(priv, (unsigned long)message, (num_blocks*4), DMA_TO_DEVICE);
...
unsigned int rtk_crc32(unsigned char *buf, int len)
{
	unsigned char *p;
	unsigned int  crc;
...
do_tkip_decrypt:
	...
	crc = rtk_crc32(pframe + hdr_len + 8, fr_len - hdr_len - 8 - 4);
...
#if defined(_DEBUG_RTL8192CD_) && !defined(__ECOS)
{
	...
	// now, let's check if icv is correct!
	crc = rtk_crc32(pframe+hdr_len+4, fr_len-hdr_len-4-4);

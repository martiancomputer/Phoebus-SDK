	...
	*usOutLen = 0;
	...
	if ( priv->pmib->dot1180211AuthEntry.dot11EnablePSK & PSK_WPA) {
		...
		for (ulIndex = 0; ulIndex < priv->wpa_global_info->NumOfUnicastCipher; ulIndex++) {
			...
			pDot11RSNPairwiseSuite->dot11RSNIESuite[usSuitCount].Type = priv->wpa_global_info->UnicastCipher[priv->wpa_global_info->NumOfUnicastCipher-1-ulIndex];
			usSuitCount++;
        }
	...
	if ((priv->pmib->dot1180211AuthEntry.dot11EnablePSK & PSK_WPA2) 
		...
		*/
		...
		if(OPMODE & WIFI_STATION_STATE) {
			...
			} else if ( ((my_11w == REQ_PMF)&& (peer_11w == OPT_PMF))||
...
/* kernel minmax.h defines MIN(a,b); this file defines its own MIN() function */
#undef MIN

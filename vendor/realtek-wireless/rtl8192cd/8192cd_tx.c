...
//	printk("data lens: %d\n", txLength );
	...
	for (ih=0; ih<DescNum; ih++) {
		...
		// Clear all status
		memset(pdesc, 0, 36);
//		rtl_cache_sync_wback(priv, phw->txcmd_desc_dma_addr[*tx_head], 32, DMA_TO_DEVICE);
		...
		rtl_cache_sync_wback(priv, (unsigned long)priv->pshare->txcmd_buf, txLength, DMA_TO_DEVICE);
...
//			*(ppdesc) |= set_desc(BIT(31));
		...
		rtl_cache_sync_wback(priv, (unsigned long)bus_to_virt(phw->txcmd_desc_dma_addr[*tx_head]-CONFIG_LUNA_SLAVE_PHYMEM_OFFSET), sizeof(struct tx_desc), DMA_TO_DEVICE);
#endif
	...
	if (timer_timeout == 0) {
		if (priv->pshare->atm_swq_current_timeout) {
			...
			if (priv->pshare->atm_swq_use_hw_timer) {
				...
				if (rtk_timer_pending(&priv->pshare->atm_swq_sw_timer))
        ...
        if(priv->pshare->atm_swq_use_hw_timer) {
            ...
            if (set_timer) {
                priv->pshare->atm_swq_current_timeout = timer_timeout;
                rtk_mod_timer(&priv->pshare->atm_swq_sw_timer, (unsigned long)timer_timeout);
            }
        }
    }
}
...
static __always_inline void do_atm_swq_timeout(unsigned long task_priv)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;
	UINT32 tri_time = 0;
	unsigned long x = 0;
...
{
	do_atm_swq_timeout(task_priv);
}

void rtl8192cd_atm_swq_timeout_timer_cb(unsigned long task_priv)
{
	do_atm_swq_timeout(task_priv);
}
DEFINE_TIMER_CALLBACK(rtl8192cd_atm_swq_timeout_timer_cb);

void init_atm_swq_sw_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->pshare->atm_swq_sw_timer), rtl8192cd_atm_swq_timeout_timer_cb, (unsigned long)priv, 0);
}
...
int __rtl8192cd_start_xmit_usb(struct sk_buff *skb, struct net_device *dev, int tx_flag)
{
	...
	if (dev->base_addr) {
		...
		if (priv->pmib->dot11WdsInfo.wdsPure) {
			priv->ext_stats.tx_drops++;
#ifdef TRX_VI_DATA_LOG
			if (skb->cb[_SKB_CB_QNUM] == VI_QUEUE)
				priv->ext_stats.tx_vi_drop_pkts++;
#endif

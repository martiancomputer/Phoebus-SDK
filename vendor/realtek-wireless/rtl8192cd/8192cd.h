...
/* PHOEBUS 6.18: forward declarations.
 * These tags were first mentioned inside function-pointer parameter lists
 * (struct hci_ops_s etc). In C that gives the tag *prototype scope*, so each
 * one became a distinct incomplete type and every call through those ops was
 * a type mismatch. GCC used to warn ('declared inside parameter list will not
 * be visible outside...'); GCC 14 makes the resulting mismatch an error.
 * Declaring them at file scope first makes all uses refer to the same type.
 */
...
//#define MAX_AP_CHANNEL_NUM          8 // move to wifi_common.h
...
#define panic_printk_cont(fmt, args...)    printk(KERN_CONT fmt, ##args)
#elif !defined(CONFIG_PANIC_PRINTK)
#define panic_printk		scrlog_printk
#define panic_printk_cont	scrlog_printk
#endif
#endif
...
typedef struct _RT_BEAMFORMING_INFO {
	...
	struct rtk_timer_list			BeamformingTimer;
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	struct timer_event_entry		BeamformingTimer_event;
#endif
	struct rtk_timer_list			BFSoundingTimeoutTimer;
...
enum DRV_NHM_CLM_ACTION {
	DRV_NHM_CLM_SCAN = 0,
	DRV_NHM_CLM_ACS  = 1,
};

typedef struct _rt_ch_load_info_ {
	u1Byte free_time;
	u1Byte interence_time;
	u1Byte dut_time;
	u1Byte other_dut_time;
	//u1Byte ch_utilization;         //include noise
} RT_CH_LOAD_INFO, *P_RT_CH_LOAD_INFO;

typedef struct _rt_ch_load_ss_info_ {
	...
	u1Byte nhm_pwr;
	//u1Byte ch_utilization;         //include noise
} RT_CH_LOAD_SS_INFO, *P_RT_CH_LOAD_SS_INFO;
...
typedef enum _WLAN_Support_Ability_Definition
{
    ...
    WLAN_RF_DPK_SETTING_SUPPORT = BIT2,
}WLAN_ABILITY_E;
...
enum led_type {
	...
	LEDTYPE_SW_CUSTOM2_LED0_WPS = 61,	// WIFI_LED_SHARE_WITH_WPS
	LEDTYPE_SW_MAX,
};
...
enum _HT_CURRENT_TX_INFO_ {
	...
	TX_USE_80M_80M_MODE	= BIT(4),
	SUPPORT_NSS_2		= BIT(6),
	SUPPORT_NSS_3		= BIT(7),
	SUPPORT_NSS_4		= BIT(6) | BIT(7)
};
...
enum ACL_MODE {
	ACL_DISABLED = 0,  // ACL is disabled
	ACL_ALLOW = 1,     // NL80211_ACL_POLICY_DENY_UNLESS_LISTED, allow the stations which are listed in ACL
	ACL_DENY = 2       // NL80211_ACL_POLICY_ACCEPT_UNLESS_LISTED, deny stations which are listed in ACL
};
...
enum {
	WIFI_PEER_CONNECT_SUCCESS = 11,
	WIFI_PEER_CONNECT_FAIL = 12,
	WIFI_PEER_DISCONNECT = 13,
	WIFI_DUT_DISCONNECT = 14,
	WIFI_PEER_EXPIRE_DISCONNECT = 15,
...
struct unknown_mcast_info {
	//int				used;
	unsigned char		mcaddr[MACADDRLEN];
	unsigned long 		timeout; 
	unsigned int		mc_pkts;
};
#endif
...
// 48 is SIZE_TXDESC_88XX
	u1Byte  hal_hw_desc[64] __attribute__ ((aligned (4))); //extend to MAX size, SIZE_TXDESC_88XX_MAX
...
#ifdef RTK_MULTI_AP
//set local steering list
struct local_steering_list{
    unsigned char	addr[MACADDRLEN];
};
#endif
...
struct assoc_control_block_list {
	...
	int got_assoc_req; //default value=0, will be set to 1 when get 1st assoc req
};
#endif
...
struct dot11k_cca_measurement_req
{
	unsigned char channel;
...
struct dot11k_cca_measurement_report
{
	unsigned char channel;
	unsigned long long start_time; 
	unsigned short measure_duration;
...
struct dot11k_rpi_measurement_req
{
	unsigned char channel;
	unsigned long long start_time; 
	unsigned short measure_duration;
...
struct dot11k_rpi_measurement_report
{
	unsigned char channel;
	unsigned long long start_time; 
	unsigned short measure_duration;
...
struct dot11k_stat_info 
{
	...
	unsigned char rm_nb_rpt_req_dialog_token;
	unsigned char rm_nb_rpt_req_ssid_len;
	unsigned char rm_nb_rpt_req_ssid[WLAN_SSID_MAXLEN + 1];
#endif
};

struct dot11k_sm_stat_info 
{
	/* for issuing CCA measurement request and getting report from other STA */
	unsigned char cca_dialog_token;
	unsigned char cca_expire_count;
	struct dot11k_cca_measurement_req cca_req;
	enum MEASUREMENT_RESULT cca_result;
	struct dot11k_cca_measurement_report cca_report;

	/* for issuing RPI measurement request and getting report from other STA */
	unsigned char rpi_dialog_token;
	unsigned char rpi_expire_count;
	struct dot11k_rpi_measurement_req rpi_req;
	enum MEASUREMENT_RESULT rpi_result;
	struct dot11k_rpi_measurement_report rpi_report;
};
...
struct dot11k_priv
{
    ...
    struct rtk_timer_list delay_measure_timer;
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
    struct timer_event_entry	delay_measure_timer_event;
#endif
    unsigned int measure_time_hi;
    unsigned int measure_time_lo;
...
#ifdef AP_NEIGHBOR_INFO
#define NB_RPT_AGING 120

enum rm_phy_type {
	RM_PHY_TYPE_UNSPECIFIED = 0,
	RM_PHY_TYPE_FHSS = 1,
	RM_PHY_TYPE_DSSS = 2,
	RM_PHY_TYPE_IRBASEBAND = 3,
	RM_PHY_TYPE_OFDM = 4,
	RM_PHY_TYPE_HRDSSS = 5,
	RM_PHY_TYPE_ERP = 6,
	RM_PHY_TYPE_HT = 7,
	RM_PHY_TYPE_DMG = 8,
	RM_PHY_TYPE_VHT = 9,
...
#endif
#endif /*end defined(DOT11K) || defined(HAPD_11K) */
...
#define STA_RX_AMSDU_SUPPORT	BIT(0)
#define STA_TX_AMSDU_SUPPORT	BIT(1)
...
struct stat_info {
...
#if defined(INCLUDE_WPA_PSK) || defined(WIFI_HAPD) || defined(RTK_NL80211) || defined(AUTH_SAE)
	unsigned char		is_sae_sta;
#endif

#ifdef AUTH_SAE
	//struct sae_data 	sae; /* save PMK */
	/* when rx peer's SAE confirm save PMK here; clean these when release_stainfo */
	unsigned char		sae_pmk[LEN_PMK];
	unsigned char		sae_pmkid[LEN_PMKID];
	unsigned char		pmkid_caching_idx;
	unsigned char		with_h2e;
	unsigned char		rsnxe_ie[16];
#endif
...
#ifdef DOT11K
    struct rtk_timer_list       rm_timer;

    unsigned long		cca_link_time_bkup;
    unsigned long		rpi_link_time_bkup;
#endif
...
#ifdef RTK_NL80211
	unsigned char		has_pmkid;
	unsigned char		assoc_type;
	unsigned char		in_cfg80211_del_station;
#endif
    ...
    struct dot11k_sm_stat_info sm;
    unsigned int trx_1sec_pkt_delta;
    unsigned int dka_pkt_timeout; // detect keep-alive packets timeout
#endif
#ifdef CONFIG_IEEE80211V_CLI
   struct	dot11v_stat_info wnm;
	...
	unsigned long		rx_icverr_pkts;
	unsigned long		rx_conti_icverr_pkts;
	unsigned long		rx_conti_icverr_times;
	unsigned int		leave_conti_icverr;
	/******************************************************************
	 * from here to end, data will be backup when doing FAST_RECOVERY *
	 ******************************************************************/
...
#endif
	unsigned char		skipFirstTkipMic;
	unsigned char		rssi;
	unsigned char		sq;
	unsigned char		rx_rate;
	unsigned char		rx_bw;
...
//#if !defined(USE_OUT_SRC) || defined(_OUTSRC_COEXIST)
	...
	unsigned int		rx_pkts2;
	unsigned int		tcpack_pkts;
	unsigned int		mc2uc_pkts;
	unsigned int		mc2uc_pkts_pre;
...
//#endif
   ...
   unsigned char      tv_auto_support; /*0: not support, 1:support*/
	...
	struct rtk_timer_list		SA_timer;	//SA_timer: Security Association timer
...
//	UINT8		IOTPeer;			// Enum value.	HT_IOT_PEER_E
	...
	unsigned long long prev_tx_byte;
	unsigned int total_rx_time;
	unsigned long long prev_rx_byte;
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
...
struct extra_stats {
	...
	unsigned long       txdata_queue_overflow_drop_pkt;
#endif
#ifdef CONFIG_SPECIAL_ENV_TEST
	...
	unsigned long long	pre_rx_packets;
	...
	unsigned long		last_ch_utilization;
	...
	unsigned long		tx_vi_pkts;
	unsigned long		tx_vi_bytes;
	unsigned long		tx_vi_error_pkts;
	unsigned long		tx_vi_drop_pkts;
	unsigned long		tx_vi_retry_pkts;
	unsigned long		rx_vi_pkts;
	unsigned long		rx_vi_bytes;
	unsigned long		rx_vi_error_pkts;
	unsigned long		rx_vi_drop_pkts;
#endif
	...
	unsigned long       si_mem_drop;

#ifdef CMCC_CTC_QOS
	unsigned long       cb_data1;
	unsigned long       cb_data1_drop;
#endif
};
...
struct ss_res {
	...
	struct bss_desc_ext	bss_ext[MAX_BSS_NUM];
	...
	unsigned char		prev_target_channel;
	unsigned char		target_macaddr[MACADDRLEN];
	unsigned int		RCR_backup;
	unsigned int		original_channel;
	unsigned int		unassoc_sta_count;
...
struct rf_finetune_var {
	...
	unsigned char		dig_rssi_ofst;
	unsigned int		manual_dig_fa_th;	//calculate in hex format. fa[0]= manual_dig_fa_th[9:0]; fa[1]= manual_dig_fa_th[19:10], fa[2]= manual_dig_fa_th[31:20]
...
//#endif
...
#endif
	unsigned int		cochannel_enhance;
...
#endif
	unsigned int 		mc2u_drop_unknown; // the threshold of #pkts in each time unit
	...
	u1Byte				dcbackoff;
	BOOLEAN 			txbuf_merge;
	u1Byte				txbuf_manual_aid;
	u1Byte				txbuf_manual;
	unsigned int			IPTV_pkt_th;
	BOOLEAN 			ctc_spec_wmm;
	...
	// for 8814b and 8812f and 8814c mib
	#if 0
	unsigned char		pw_lowth1;
	unsigned char		pw_uppth1;
	unsigned char		pw_lowth2;
	unsigned int		pw_uppth2;
	unsigned char		pw_cnt_th;
	unsigned char		pri_rpt_lowth1;
	unsigned int		pri_rpt_uppth1;
	#endif
	...
	int			swq_aggnum;
	int			swqturboaggnum;
	int			swqSEaggnum;	//SE: special Environment, include veriwave ane testcenter throughput test
	int			swq_turbo_sta_th;
	int			swqmaxturbotime;
	int			manual_max_xmit_enabled;
	int			swq_def_max_xmit;
	int			manual_swq_max_xmit[8];
	int			timeout_thd;
	int			timeout_thd2;
	int			timeout_thd3;
...
#ifdef SW_TXQ_ATF
	int					swq_atf_en;
	int					swq_atf_base_qta;
	int					swq_atf_delay_hi_th;
	int					swq_atf_delay_lo_th;
	int					swq_atf_sta_hi_th;
	int					swq_atf_sta_lo_th;
	int					swq_atf_force_max;
	int					swq_atf_nopkt_max;
#endif /* SW_TXQ_ATF */
#endif /* SW_TX_QUEUE */
	int             	rtsdrop_timer;
...
// power constraint
	...
	unsigned char		ndpa_retry;

	unsigned char		mu_hotfix;
    unsigned char       mu_amsdu;
	unsigned int        mu_bdth;
	...
	unsigned char		txbf_iqgen_keep_on;
#endif
#if (MU_BEAMFORMING_SUPPORT == 1)
	unsigned int		qtime;
	unsigned int		qlmt;
	...
	unsigned char		set_bfrp_rate;
	unsigned char		force_mu;
	unsigned int		force_mutp_th;
#endif
	...
	unsigned char       resetba_vw_force; //1: enable this feature, 0: disable
...
//	unsigned char		mp_dig_enable;
	...
	unsigned char		initial_pa;
	unsigned char		secondary_pa;
#ifdef RTK_STA_BWC
...
#endif
#ifdef __KERNEL__
	unsigned int        mcst_min_rsvd_mem;
#endif
...
#ifdef CONFIG_SPECIAL_ENV_TEST
	unsigned char		spec_env_test_en; //bit 0: seperate seq number, bit 1: amsdu delay, bit 7: treat Unknown STA as Veriwave STA
	unsigned int		spec_env_test_fa;
	unsigned int		spec_pktsz;
	unsigned char       veriwave_interference;
	unsigned int        spec_latency_sta_num;
	unsigned int        spec_latency_swqturboaggnum;
	unsigned int        spec_latency_swqSEaggnum;
	unsigned char		vw_sifs_bk_en;
#endif
	...
	unsigned int		swq_hw_to; //0: default 1000us;	N: N us
	unsigned int		direct_tx_bdth;
#endif
	...
	unsigned char		led_wps_mode;		// use for WIFI_LED_SHARE_WITH_WPS
...
unsigned char dc_bcn_mode;
	...
	unsigned char		amsdu_merge;
	unsigned char		amsdu_merge_default;
	unsigned char		manual_amsdu_merge;
#endif
	unsigned char		os_test_en;	//otoscope test mode enable  
#ifdef ISSUE_RTS_BY_SW
	unsigned int		RTS_pkt_intvl;
	unsigned int		RTS_duration;
	unsigned int		RTS_rate;
	unsigned char		RTS_Qnum;
#endif
#ifdef FORCE_QOS_SUPPORT
	unsigned char		force_qos;  /*bit0: enable/disable, bit1: set others pkts to BK queue*/
#endif
};


#ifdef FORCE_QOS_SUPPORT
#define FORCE_QOS_ENABLE       0x01  /* Enable Force-QoS mechanism */
#define FORCE_NON_QOS_TO_BK    0x02  /* Force pkts with skb->cb[0] != 1 ~ 7 be xmitted by BK queue */
#endif
...
struct sta_mac_rssi_detail {
	...
	unsigned char 			Entry;
	unsigned char 			status;		
	unsigned long 				time_stamp; // jiffies time of last frame
};
...
#if STA_CONTROL_ALGO == STA_CONTROL_ALGO2
    struct rtk_timer_list   stactrl_timer;
#endif
...
struct mclone_sta_info {
	...
	struct rtk_timer_list	reauth_timer;
	int			reauth_count;
	struct rtk_timer_list	reassoc_timer;
	int			reassoc_count;
	int 			auth_seq;
	unsigned int		opmode;
...
struct channel_utilization_info {
    ...
    int                     chbusytime;
    struct rtk_timer_list   cu_cntdwn_timer;
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
...
#ifdef CONFIG_RTL_NEW_AUTOCH
enum acs_setting{
	STORE_DEFAULT_NHM_SETTING = 0,
	RESTORE_DEFAULT_NHM_SETTING = 1,
	ACS_NHM_SETTING = 2,
};

struct acs_info {
	u1Byte		step;
	u1Byte		nhm_cnt[30][10]; /*the length of array must be more than available channel number (2.4G & 5G)*/
	...
	/* jgr3-Series, for storing previous setting */
	u4Byte		reg0x1e40;
	u4Byte		reg0x1e44;
	u4Byte		reg0x1e48;	
	u4Byte		reg0x1e60;
...
// temp mark by Eric
	...
	struct rtk_timer_list xmit_check_timer;
#if !defined(CONFIG_PCI_HCI) && !defined(SMP_LOAD_BALANCE_SUPPORT)
	...
	struct rtk_timer_list chk_h2c_buf_timer;
#endif
	...
	struct rtk_timer_list beacon_timer;
	...
	u8 ps_ctrl;
	u8 pwr_state;
	struct rtk_timer_list ps_timer;
	...
	struct rtk_timer_list PktAging_timer;
#endif
...
#ifdef TX_PROCESS_REFINE
	rtk_call_single_data_t	txq_tasklet_csd;
#endif
#ifdef WIFI_RX_PROCESS_ALL_QUEUE_OFFLOAD
	struct rtk_tasklet	rxq_tasklet;
	rtk_call_single_data_t	rxq_tasklet_csd;
#endif
#ifdef SMP_RX_LOAD_BALANCE
	struct rtk_tasklet	rxq_tasklet;
	rtk_call_single_data_t	rxq_tasklet_csd;
	unsigned long			rxq_ind_seq;
	unsigned long			rxq_proc_seq;
#endif
#ifndef __ECOS
...
#if defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_WLAN_HAL_8192FE) || defined(CONFIG_WLAN_HAL_8197F) || defined(CONFIG_WLAN_HAL_8198F)
...
/*cfg p2p cfg p2p*/	
	...
	rtk_call_single_data_t   swq_tasklet_csd;
	#endif
	#endif // __KERNEL__
#endif // SW_TX_QUEUE
#if defined SMP_LOAD_BALANCE_SUPPORT
	rtk_call_single_data_t   smp_assign_expire_timer_csd;
#endif

#ifdef SMP_SKB_RECYCLE_OFFLOAD
	struct sk_buff_head		*skb_recycle_queue;
	struct tasklet_struct	skb_recycle_tasklet;
	rtk_call_single_data_t	skb_recycle_csd;
	unsigned char			skb_recycle_trigger;
#endif
	...
	struct sk_buff_head		netif_rx_queue;
#endif
...
#ifdef RTK_NL80211
	unsigned int			scanned_time;
#endif

#ifdef CTC_WIFI_DIAG
	struct rtk_timer_list		ctc_diag_tx_bcn_timer;
#endif
	// for SW LED
	struct rtk_timer_list		LED_Timer;
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	...
	unsigned char			LED_ToggleForce;	/*RTL_LED_ON : 0, RTL_LED_OFF : 1 */


	int				ther_offset;
	unsigned int			ther_parameter;
	unsigned int			ther_tp_level;
	int				ther_cps[MAX_THER_TP_LEVEL];
...
#ifdef CTC_2G_ACS
	atomic_t                ss_score_timeout_count;
	atomic_t                autoch_score_timeout;
#endif
#ifdef USE_OUT_SRC	
	RT_CH_LOAD_INFO         ch_info;
	RT_CH_LOAD_SS_INFO      ch_ss_info[SUPPORT_CH_NUM];
#endif
	...
	unsigned short		spec_env_latency_test;
	unsigned short		macbook_sta_num;
	unsigned short		broadcom_4T4R_sta_num;
	unsigned short		intel_sta_num;
	unsigned char		veriwave_sifs_bk_status;
...
#ifdef CMCC_CTC_QOS
	unsigned int		force_qos_timeout;
#endif
	...
	struct rtk_timer_list		DPKTimer;
	...
	// MP DIG
#ifdef CONFIG_RTL_92D_SUPPORT
	struct rtk_timer_list		MP_DIGTimer;
#endif
...
//#endif
	...
	unsigned long long		reset_monitor_rx_pkt_cnt;
#endif
	...
	unsigned long long		writeport_total_time[MAX_XMITBUF_PKT];
	unsigned long long		writeport_avg_time[MAX_XMITBUF_PKT];
#endif // SDIO_STATISTICS_TIME
#endif
#endif // CONFIG_SDIO_HCI
#endif // CONFIG_USB_HCI || CONFIG_SDIO_HCI
...
//arx parameters
...
#if defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_WLAN_HAL_8197F) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8198F) || defined(CONFIG_WLAN_HAL_8192FE) || defined(CONFIG_WLAN_HAL_8814BE) || defined(CONFIG_WLAN_HAL_8822CE) || defined(CONFIG_WLAN_HAL_8812FE)
	UINT64					mp_rx_mac_ok;
	...
	// monitor Tx and Rx
	unsigned long long		tx_packets_total;
	unsigned long long		rx_packets_total;
	unsigned long long		tx_packets_pre;
	unsigned long long		rx_packets_pre;
	unsigned long long		rx_packets_pre2;
	unsigned long long		tx_packets_pre2;
	...
	unsigned long long		wifi_rx_pkts[3];
	unsigned long long		wifi_tx_pkts[3];
...
//	unsigned int			pkt_in_hiQ;
	...
	unsigned int			direct_xmit_num;
#endif

	atomic_t				tx_recycle_trigger;

#ifdef CONFIG_HWNAT_NETIF_RX_QUEUE_SUPPORT
    struct sk_buff_head	hwnat_netif_rx_queue;
#endif

	struct rtk_timer_list		rc_sys_timer;
    ...
    struct rtk_timer_list       swq_sw_timer;
...
#endif
#ifdef SW_TXQ_ATF
	unsigned int				swq_atf_state;
	unsigned int				swq_atf_force_cnt;
	unsigned int				swq_atf_nopkt_cnt;

	unsigned int				swq_atf_nosched_budget_cnt;
	unsigned int				swq_atf_nosched_pkt_cnt;
	unsigned int				swq_atf_nosched_nopkt_cnt;

	unsigned int				swq_atf_sched_nobudget_cnt;
	unsigned int				swq_atf_sched_force_cnt;
	unsigned int				swq_atf_sched_nopkt_cnt;
	unsigned int				swq_atf_delay_cnt;
#endif /* SW_TXQ_ATF */
#endif /* SW_TX_QUEUE */

#ifdef RTK_ATM
    UINT32                      atm_swq_current_timeout;
    struct rtk_timer_list       atm_swq_sw_timer;
...
//	STA_BITMAP			has_2r_sta; // Used when AP is 2T2R. bitmap of 2R aid
    ...
    unsigned char			fw_download_status;
#endif
...
//	unsigned short			F90_cnt;
	...
	struct rtk_timer_list	swAntennaSwitchTimer;
	...
	unsigned long long	TXByteCnt_R;
	unsigned long long	TXByteCnt_L;
	unsigned long long	RXByteCnt_R;
	unsigned long long	RXByteCnt_L;
	...
	struct rtk_timer_list	vxd_ibss_beacon; //timer to issue beacon for vxd_adhoc
#endif
	...
	int					ada_stats_ratio;
	struct rtk_timer_list	ada_stats_timer;
#ifdef SMP_SYNC
	...
	unsigned int ant_flag;
	STA_BITMAP ant_Bitmap;
#endif
};
...
#ifdef CONFIG_CMCC_FEATURE_SUPPORT
#define NR_STA_CAP 128
struct sta_max_cap {
	unsigned char	addr[ETH_ALEN];
	unsigned char	mode;
	unsigned char	bw:6;		// 0: 20M, 1: 40M, 4: 80M, 8: 160M, 16: 80_80
	unsigned char	assoc:2;	// 0: unassociated, 1: associated
	unsigned long	timestamp;	// record jiffies
};
#endif
...
typedef struct rtl8192cd_priv {
	 ...
	 */
	struct rtk_timer_list		ch52_timer;
	struct rtk_timer_list		ch56_timer;
	struct rtk_timer_list		ch60_timer;
	struct rtk_timer_list		ch64_timer;
	struct rtk_timer_list		ch100_timer;
	struct rtk_timer_list		ch104_timer;
	struct rtk_timer_list		ch108_timer;
	struct rtk_timer_list		ch112_timer;
	struct rtk_timer_list		ch116_timer;
	struct rtk_timer_list		ch120_timer;
	struct rtk_timer_list		ch124_timer;
	struct rtk_timer_list		ch128_timer;
	struct rtk_timer_list		ch132_timer;
	struct rtk_timer_list		ch136_timer;
	struct rtk_timer_list		ch140_timer;
	struct rtk_timer_list		ch144_timer;
	struct rtk_timer_list		ch149_timer;
	struct rtk_timer_list		ch153_timer;
	struct rtk_timer_list		ch157_timer;
	struct rtk_timer_list		ch161_timer;
	struct rtk_timer_list		ch165_timer;
...
#ifdef RTK_MULTI_AP
			spinlock_t			local_steering_list_lock;
#endif
#endif
...
#ifdef RTK_MULTI_AP
	struct local_steering_list disallowed_list[MAX_TRANS_LIST_NUM];
	unsigned char disallowed_list_bitmask[(MAX_TRANS_LIST_NUM + 7)/8];
#endif
	bool 				startCounting;
...
#endif // STA_CONTROL_BAND_TRANSITION
#ifdef SUPPORT_MONITOR
	struct rtk_timer_list		chan_switch_timer;
	...
	unsigned int			pcie_stuck;
	struct work_struct		reset_hangup_work;
#endif
#ifdef CONFIG_WLAN_HAL_8814AE
	...
	unsigned long long		tx_packets_prev;
	unsigned long long		rx_packets_prev;

#ifdef WDS
#ifdef __LINUX_3_7__
	struct rtnl_link_stats64 wds_stats[NUM_WDS];
#else
...
#ifdef CONFIG_RTK_MESH
    struct rtk_timer_list   mesh_expire_timer;
    struct wlan_ethhdr_t    ethhdr;
...
#ifdef __LINUX_3_7__
    struct rtnl_link_stats64 mesh_stats;
#else
	...
	struct rtk_timer_list		mesh_auth_timer;		///< for unestablish (And establish to unestablish) MP mesh_auth_hdr
	...
	struct rtk_timer_list		mesh_peer_link_timer;	///< for unestablish (And establish to unestablish) MP mesh_unEstablish_hdr
	...
	unsigned char		hw_serial_mac[MACADDRLEN];  /* bridge mac */
	int					ss_serial_timer;
	int					ss_serial_cnt;
	char				ss_serial_no[16][65];
	unsigned char		ss_serial_mac[16][MACADDRLEN];
#if defined(__LINUX_2_6_27__)
	...
	struct rtk_timer_list		waiMCastKeyUpdateTimer;
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
...
#endif
#ifdef P2P_SUPPORT
	struct rtk_timer_list		p2p_listen_timer_t;
	struct rtk_timer_list		p2p_search_timer_t;
	...
	struct sk_buff_head		rx_data_queue;
#endif
	...
	struct rtk_timer_list		frag_to_filter;
	unsigned int			frag_to;

	struct rtk_timer_list		expire_timer;
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
...
#if (BEAMFORMING_SUPPORT == 1)
	struct rtk_timer_list		txbf_timer;
	struct rtk_timer_list		txbf_swtimer;
#endif
	...
	struct rtk_timer_list		ss_timer;	//ss_timer: site_survey timer
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	...
	unsigned int			ProbeReqEntryOccupied;	
	unsigned int			ProbeReqEntryNum;	
#if defined(WLAN_DIAGNOSTIC)
	struct sta_mac_rssi_detail     unassoc_sta[MAX_UNASSOC_STA];
	unsigned int			UnassocSTAEntryOccupied;	
	unsigned int			UnassocSTAEntryNum;	
#endif

#ifdef RTK_SMART_ROAMING
	struct rtk_timer_list		send_timer_wlan0;
	struct rtk_timer_list		send_timer_wlan1;
#if defined(MBSSID)
	struct rtk_timer_list		send_timer_wlan0_vap[RTL8192CD_NUM_VWLAN];
	struct rtk_timer_list		send_timer_wlan1_vap[RTL8192CD_NUM_VWLAN];
#endif
	...
	/* ******************************************* */
#if defined(CONFIG_RTL_TRIBAND_SUPPORT)
	struct rtk_timer_list		send_timer_wlan2;
#if defined(MBSSID)
	struct rtk_timer_list		send_timer_wlan2_vap[RTL8192CD_NUM_VWLAN];
#endif
#endif
	...
	unsigned char			wlanid;   // 0: wlan0, 1: wlan1
	...
	struct rtk_timer_list		multiap_backhaul_steer_timer;
	struct rtk_timer_list		multiap_unassoc_sta_timer;

#if (EASYMESH_VERSION >= MULTI_AP_VERSION(2,0,0))
	struct rtk_timer_list		multiap_cac_process_timer;
#endif
...
#if defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_92C_SUPPORT)
	struct rtk_timer_list		add_RATid_timer;
	struct rtk_timer_list		add_rssi_timer;
#endif
	struct rtk_timer_list		add_ps_timer;
#endif // CONFIG_PCI_HCI

#if defined(CONFIG_RTL_92D_SUPPORT) && defined(CONFIG_RTL_NOISE_CONTROL)
	struct rtk_timer_list		dnc_timer;
#endif
...
//	unsigned char			*pBeaconErp;			// ptr of ERP field in beacon buf
	...
	unsigned int			bkp_avail_chnl[MAX_CHANNEL_NUM];		// all available channel we can use
	unsigned int			bkp_avail_chnl_num;		// record the number
#ifdef MBSSID
	...
	// for MIC check
	struct rtk_timer_list		MIC_check_timer;
	struct rtk_timer_list		assoc_reject_timer;
	...
	struct rtk_timer_list		idle_timer;
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	struct timer_event_entry	idle_timer_event;
#endif
#ifdef DFS
	struct rtk_timer_list		dfs_cntdwn_timer;
#endif

#ifdef HS2_SUPPORT
	struct rtk_timer_list		disassoc_timer;
#endif
	unsigned int			join_req_ongoing;
	int						join_index;
...
//	unsigned int			rxMlcstDataNumInPeriod_pre;
...
#if (MU_BEAMFORMING_SUPPORT == 1)
	unsigned short			mu_bkup_agg_num;
	unsigned short			mu_bkup_reg_63e;
#endif
	...
	unsigned long			cnt_icverr_deauth;

#ifdef CONFIG_CMCC_FEATURE_SUPPORT
	struct sta_max_cap		sta_cap_table[NR_STA_CAP];
#endif
...
#ifdef DFS
	struct rtk_timer_list		DFS_timer;			/* timer for radar detection */
	struct rtk_timer_list		DFS_TXPAUSE_timer;	/* timer for checking whether restarting TX or not*/
	struct rtk_timer_list		ch_avail_chk_timer;	/* timer for channel availability check */
	struct rtk_timer_list		dfs_chk_timer;		/* timer for dfs trigger */
	struct rtk_timer_list		dfs_det_chk_timer;	/* timer for channel busy check under dfs */
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	...
	unsigned char 			nhm_dfs_cnt_all[10];
	unsigned char 			nhm_cnt;
	unsigned char 			nhm_acc;
	unsigned char			nhm_avg_flag;
	...
	signed int			loct_14b_rslt[8];
	/*8814C*/
	unsigned char			seg1_flag_14c;
	unsigned int			pw_14c_rslt[12];
	unsigned int			pri_14c_rslt[12];
	unsigned int			pri_14c_rslt_mode;
	signed int			loct_14c_rslt[12];
#endif
...
#ifdef GBWC
	struct rtk_timer_list		GBWC_timer;
...
#ifdef SBWC
	struct rtk_timer_list		SBWC_timer;
#endif

#ifdef RTK_STA_BWC
	struct rtk_timer_list		sta_bwc_timer;
#endif
...
//	unsigned int			ampdu_first_dma_desc;
...
#ifdef CHECK_RX_HANGUP
	unsigned long long		rx_packets_pre1;
	unsigned long long		rx_packets_pre2;
	...
	struct unknown_mcast_info		unknown_mc[MAX_UNKNOWN_MC_ENTRY];
	unsigned int					unknown_others_mc_pkt;
#endif
...
//#endif
    ...
    unsigned char tx_cpu_turbo;
#endif
#endif
#ifdef SMART_ADAPTIVE_ANT
    unsigned char ant_adapt_mu;
#endif
...
#define TBTT(priv)		(priv->pmib->dot11StationConfigEntry.dot11BeaconPeriod ? priv->pmib->dot11StationConfigEntry.dot11BeaconPeriod : 100)
...
#define PENDING_REAUTH_TIMER	((ACTIVE_ID > 0) ? \
			rtk_timer_pending(&priv->pshare->mclone_sta[ACTIVE_ID-1].reauth_timer) : \
					rtk_timer_pending(&priv->reauth_timer))
#define DELETE_REAUTH_TIMER	((ACTIVE_ID > 0) ? \
			rtk_del_timer(&priv->pshare->mclone_sta[ACTIVE_ID-1].reauth_timer) : \
					rtk_del_timer(&priv->reauth_timer))
#define PENDING_REASSOC_TIMER	((ACTIVE_ID > 0) ? \
			rtk_timer_pending(&priv->pshare->mclone_sta[ACTIVE_ID-1].reassoc_timer) : \
					rtk_timer_pending(&priv->reassoc_timer))
#define DELETE_REASSOC_TIMER	((ACTIVE_ID > 0) ? \
			rtk_del_timer(&priv->pshare->mclone_sta[ACTIVE_ID-1].reassoc_timer) : \
					rtk_del_timer(&priv->reassoc_timer))

#define MOD_REAUTH_TIMER(t)	((ACTIVE_ID > 0) ? \
			rtk_mod_timer (&priv->pshare->mclone_sta[ACTIVE_ID-1].reauth_timer, jiffies+t) : \
					rtk_mod_timer(&priv->reauth_timer, jiffies+t))
#define MOD_REASSOC_TIMER(t)	((ACTIVE_ID > 0) ? \
			rtk_mod_timer (&priv->pshare->mclone_sta[ACTIVE_ID-1].reassoc_timer, jiffies+t) : \
					rtk_mod_timer(&priv->reassoc_timer, jiffies+t))
#else
...
#define PENDING_REAUTH_TIMER	(rtk_timer_pending(&priv->reauth_timer))
#define DELETE_REAUTH_TIMER	(rtk_del_timer(&priv->reauth_timer))
#define PENDING_REASSOC_TIMER	(rtk_timer_pending(&priv->reassoc_timer))
#define DELETE_REASSOC_TIMER	(rtk_del_timer(&priv->reassoc_timer))
#define MOD_REAUTH_TIMER(t)	(rtk_mod_timer(&priv->reauth_timer, jiffies+t))
#define MOD_REASSOC_TIMER(t)	(rtk_mod_timer(&priv->reassoc_timer, jiffies+t))
#endif

...
struct rtk_timer_list	*timer_wlan0;
struct rtk_timer_list	*timer_wlan1;
...
struct rtk_timer_list	*timer_wlan2;
...
DECLARE_TIMER_CALLBACK(send_roam_info);

static void sr_timer_rec_add(void) {
    if (wlan0_used) {
        if (!wlan0_deleted) {
            rtk_del_timer_sync(timer_wlan0);
            wlan0_deleted = 1;
        }
        if (wlan0_deleted) {
            rtk_add_timer(timer_wlan0);
            wlan0_deleted = 0;
        }
    }
    ...
    if (wlan1_used) {
        if (!wlan1_deleted) {
            rtk_del_timer_sync(timer_wlan1);
            wlan1_deleted = 1;
        }
        if (wlan1_deleted) {
            rtk_add_timer(timer_wlan1);
            wlan1_deleted = 0;
        }
    }
    ...
    if (wlan2_used) {
        if (!wlan2_deleted) {
            rtk_del_timer_sync(timer_wlan2);
            wlan2_deleted = 1;
        }

        if (wlan2_deleted) {
          rtk_add_timer(timer_wlan2);
          wlan0_deleted = 0;
        }
    }
#endif
	...
	for (j = 0; j < RTL8192CD_NUM_VWLAN; j++) {
		if (wlan0_vap_used[j]) {
			if (!wlan0_vap_delete[j]) {
				rtk_del_timer_sync(send_timer_wlan0_vap[j]);
				wlan0_vap_delete[j] = 1;
			}
			if (wlan0_vap_delete[j]) {
				rtk_add_timer(send_timer_wlan0_vap[j]);
		...
		if(wlan1_vap_used[j]) {
			if (!wlan1_vap_delete[j]) {
				rtk_del_timer_sync(send_timer_wlan1_vap[j]);
				wlan1_vap_delete[j] = 1;
			}
			if (wlan1_vap_delete[j]) {
				rtk_add_timer(send_timer_wlan1_vap[j]);
...
void timer_ready(struct rtl8192cd_priv *priv)
{
	if ((!strcmp(priv->dev->name, "wlan0")) && (wlan0_used != 1)){
		rtk_timer_setup(&(priv->send_timer_wlan0), send_roam_info, (unsigned long)priv, 0);
		rtk_timer_set_expires(&(priv->send_timer_wlan0), jiffies + RTL_SECONDS_TO_JIFFIES(5));
		priv->wlanid = 0;
		priv->vapid = 0;
		...
		rtk_timer_setup(&(priv->send_timer_wlan1), send_roam_info, (unsigned long)priv, 0);
		rtk_timer_set_expires(&(priv->send_timer_wlan1), jiffies + RTL_SECONDS_TO_JIFFIES(4));
		priv->wlanid = 1;
		priv->vapid = 0;
		...
		rtk_timer_setup(&(priv->send_timer_wlan2), send_roam_info, (unsigned long)priv, 0);
		rtk_timer_set_expires(&(priv->send_timer_wlan2), jiffies + RTL_SECONDS_TO_JIFFIES(4));
		priv->wlanid = 2;
		priv->vapid = 0;
...
void vap_timer_ready(struct rtl8192cd_priv *priv, int i) {
	...
	if (!strcmp(priv->dev->name, "wlan0")) {
		if (netif_running(GET_VAP_PRIV(priv, i)->dev) && (wlan0_vap_used[i] != 1)) {
			rtk_timer_setup(&(priv->send_timer_wlan0_vap[i]), send_roam_info, (unsigned long)(GET_VAP_PRIV(priv, i)), 0);
			rtk_timer_set_expires(&(priv->send_timer_wlan0_vap[i]), jiffies + RTL_SECONDS_TO_JIFFIES(5));
		...
		if (netif_running(GET_VAP_PRIV(priv, i)->dev) && (wlan1_vap_used[i] != 1)) {
			rtk_timer_setup(&(priv->send_timer_wlan1_vap[i]), send_roam_info, (unsigned long)(GET_VAP_PRIV(priv, i)), 0);
			rtk_timer_set_expires(&(priv->send_timer_wlan1_vap[i]), jiffies + RTL_SECONDS_TO_JIFFIES(4));
...
{
    if (!strcmp(priv->dev->name, "wlan0")) {
    	rtk_del_timer_sync(&priv->send_timer_wlan0);
    	wlan0_used = 0;
        wlan0_deleted = 1;
		panic_printk("[%s] %s timer deleted!\n", __FUNCTION__, priv->dev->name);
    }
	else if (!strcmp(priv->dev->name, "wlan1")) {
    	rtk_del_timer_sync(&priv->send_timer_wlan1);
        wlan1_used = 0;
        wlan1_deleted = 1;
		panic_printk("[%s] %s timer deleted!\n", __FUNCTION__, priv->dev->name);
    }
        ...
        rtk_del_timer_sync(&priv->send_timer_wlan2);
        wlan2_used = 0;
        wlan2_deleted = 1;
		panic_printk("[%s] %s timer deleted!\n", __FUNCTION__, priv->dev->name);
    }
		...
		int i = priv->vap_id;
		rtk_del_timer_sync(&GET_ROOT(priv)->send_timer_wlan0_vap[i]);
		wlan0_vap_used[i] = 0;
		wlan0_vap_delete[i] = 1;
		panic_printk("[%s] %s timer deleted!\n", __FUNCTION__, priv->dev->name);
	}
		...
		int i = priv->vap_id;
		rtk_del_timer_sync(&GET_ROOT(priv)->send_timer_wlan1_vap[i]);
		wlan1_vap_used[i] = 0;
		wlan1_vap_delete[i] = 1;
		panic_printk("[%s] %s timer deleted!\n", __FUNCTION__, priv->dev->name);
	}
...
static __always_inline void send_roam_info(unsigned long task_priv)
{
	unsigned long flags;
...
#if defined(CONFIG_IEEE80211V) && defined(WLAN_DIAGNOSTIC)
	...
	if(vapsr_enable != 1) {
		if ((!strcmp(priv->dev->name, "wlan0")) && (!rtk_timer_pending(&priv->send_timer_wlan0))) {
			rtk_mod_timer(&priv->send_timer_wlan0, jiffies+RTL_SECONDS_TO_JIFFIES(5));
			return;
		}
		if ((!strcmp(priv->dev->name, "wlan1")) && (!rtk_timer_pending(&priv->send_timer_wlan1))) {
			rtk_mod_timer(&priv->send_timer_wlan1, jiffies+RTL_SECONDS_TO_JIFFIES(4));
			return;
		}
#if defined(CONFIG_RTL_TRIBAND_SUPPORT)
	    if ((!strcmp(priv->dev->name, "wlan2")) && (!rtk_timer_pending(&priv->send_timer_wlan2))) {
	        rtk_mod_timer(&priv->send_timer_wlan2, jiffies+RTL_SECONDS_TO_JIFFIES(4));
			return;
		}
#endif
	}
#ifdef MBSSID
	if(vapsr_enable == 1) {
		if ((priv->dev->name[4] == '0') && (!rtk_timer_pending(&priv->send_timer_wlan0_vap[i]))) {
			rtk_mod_timer(&root_priv->send_timer_wlan0_vap[i], jiffies+RTL_SECONDS_TO_JIFFIES(5));
		}
		if ((priv->dev->name[4] == '1') && (!rtk_timer_pending(&priv->send_timer_wlan1_vap[i]))) {
			rtk_mod_timer(&root_priv->send_timer_wlan1_vap[i], jiffies+RTL_SECONDS_TO_JIFFIES(4));
		}
#if defined(CONFIG_RTL_TRIBAND_SUPPORT)
	    if ((priv->dev->name[4] == '2') && (!rtk_timer_pending(&priv->send_timer_wlan2_vap[i]))) {
			rtk_mod_timer(&root_priv->send_timer_wlan2_vap[i], jiffies+RTL_SECONDS_TO_JIFFIES(4));
		}
#endif
	}
#endif
}
DEFINE_TIMER_CALLBACK(send_roam_info);
...
void notify_new_sta(struct rtl8192cd_priv *priv, unsigned char *mac, int type, unsigned char rssi)
{
	...
	if(priv->pmib->miscEntry.telco_selected == TELCO_CMCC){
		...
		if(priv->pmib->rlr_profile.roaming_switch && priv->pmib->rlr_profile.roaming_qos){
			...
			rtk_eventd_netlink_send(rtk_eventd_pid, nl_eventd_sk, event, NULL, send_buf, offset);
#endif
...
void construct_netlink_send(struct rtl8192cd_priv *priv)
{
	...
	unsigned char auto_channel_backup_tmp = 0; //always set it to 0
	memcpy(send_buf+offset,&auto_channel_backup_tmp,sizeof(unsigned char));
	...
	if(priv->bss11v_priv_sc) {
		ap_rate = (priv->pshare->total_tp + priv->bss11v_priv_sc->pshare->total_tp) * 100 / (priv_max_tp(priv) + priv_max_tp(priv->bss11v_priv_sc));
		ap_stanum = htonl(priv->pshare->total_sta_num + priv->bss11v_priv_sc->pshare->total_sta_num);
	}
	else {
...
void construct_netlink_send_extend(struct rtl8192cd_priv *priv)
{
	...
	if (1 == GET_MIB(priv)->dot1180211sInfo.mesh_enable && netif_running(priv->mesh_dev)
		...
		offset += sizeof(unsigned char);
		memcpy(send_buf+offset, &(priv->pmib->dot11OperationEntry.hwaddr), MACADDRLEN);
		offset+=MACADDRLEN;
		...
		for(i = 0; i < tbl_sz; i++) {		 
			if(priv->pathsel_table->entry_array[i].dirty != 0) {			
				...
				offset+=MACADDRLEN;
				memcpy(send_buf+offset, &(ptable->path2NexthopMAC), MACADDRLEN);
				offset+=MACADDRLEN;
				path_num++;
			}
		}
		...
		send_buf[offset] = SR_MESH_MPINFO;
		offset += sizeof(unsigned char);
		mpinfo_num_offset = offset;
		offset += sizeof(mpinfo_num);
		...
		if (!list_empty(phead)) {
        	...
        	while (plist != phead) {
				...
				memcpy(send_buf+offset, &(pstat->rssi), sizeof(unsigned char));
				offset+=sizeof(unsigned char);
				unsigned char isEth = 0;
				if(pstat->isEth == 170) isEth = 1;
				memcpy(send_buf+offset, &(isEth), sizeof(unsigned char));
				offset+=sizeof(unsigned char);
				mpinfo_num++;
				plist = plist->next;
        	}
		}
		mpinfo_num = htonl(mpinfo_num);
		memcpy(send_buf+mpinfo_num_offset,&mpinfo_num,sizeof(mpinfo_num));
	}
#endif

	send_buf[offset] = SR_ENABLE;
	offset+=sizeof(unsigned char);
	memcpy(send_buf+offset,&(SMART_ROAMING_ENABLE),sizeof(unsigned char));
	offset+=sizeof(unsigned char);
...
void smart_roaming_csa_expire_check(struct rtl8192cd_priv *priv)
{
	...
	if(get_assoc_sta_num(priv, 1)) csa_state |= BIT0;

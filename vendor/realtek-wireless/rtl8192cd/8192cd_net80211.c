...
#if defined(WIFI_HAPD) || defined(RTK_NL80211)  // TO_BE_REMOBED
...
#endif // defined(WIFI_HAPD) || defined(RTK_NL80211)
...
int	rtl_wpas_join(struct rtl8192cd_priv *priv, int bss_num)
{
	...
	//rtk_mod_timer(&priv->WPAS_timer, jiffies + 300);
...
#endif // (WIFI_WPAS || RTK_NL80211)

#ifdef WIFI_HAPD
static void rtl_net80211_authmode(struct net_device *dev, int authtype, int encmode, 
									int pskenable, int dot11802_1x)
{
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);

	HAPD_MSG("rtl_net80211_authmode +++\n");
		...
		case IEEE80211_PARAM_AUTHMODE:		/* authentication mode */
			...
			case IEEE80211_AUTH_NONE:
				{//Disable
					...
					pskenable = 0;
					dot11802_1x = 0;
					rtl_net80211_authmode(dev, authtype, encmode, pskenable, dot11802_1x);
					break;
				}
...
int rtl_net80211_setoptie(struct net_device *dev, unsigned char *ie, unsigned int length)
{
	...
	if ((ie[0] == _WPS_IE_) && (!memcmp(ie+2, WSC_IE_OUI, 4))) {
		priv->pmib->wscEntry.assoc_ielen = length;
		memcpy((void *)priv->pmib->wscEntry.assoc_ie, ie, length);
	...
	memcpy(priv->pmib->dot11RsnIE.rsnie, ie, length);
	priv->pmib->dot11RsnIE.rsnielen = length;
...
int rtl_hapd_config(struct net_device *dev, struct rtk_hapd_config *config)
{
	...
	priv->pmib->dot11OperationEntry.dot11RTSThreshold		  	= (config->rtsthres == -1) ? 2347 : config->rtsthres;
	priv->pmib->dot11OperationEntry.dot11FragmentationThreshold = (config->fragthres == -1) ? 2346 : config->fragthres;
...
#ifdef WIFI_WPAS
static inline int rtl_wpas_config_2G(struct rtl8192cd_priv *priv)
{
...
static inline int rtl_wpas_config_5G(struct rtl8192cd_priv *priv)
{
...
static int rtl_wpas_config(struct rtl8192cd_priv *priv, unsigned char bandmode, unsigned char phymode)
{
...
#endif  // WIFI_HAPD || RTK_NL80211  // TO_BE_REMOBED

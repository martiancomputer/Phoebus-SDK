...
DECLARE_TIMER_CALLBACK(rtl8192cd_xmit_check_timer);
void rtl8192eu_xmit_tasklet(unsigned long data);
...
int _rtw_init_xmit_priv(struct rtl8192cd_priv *priv)
{
	...
	rtk_timer_setup(&(pshare->xmit_check_timer), rtl8192cd_xmit_check_timer, (unsigned long)priv, 0);

	// init xmit tasklet
	tasklet_init(&pshare->xmit_tasklet, rtl8192eu_xmit_tasklet, (unsigned long)priv);
...
#if 0//defined(CONFIG_RTL_TRIBAND_SUPPORT)
...
static __always_inline void rtl8192cd_xmit_check_timer(unsigned long task_priv)
{
...
tcpack_out:
	...
	rtk_mod_timer(&pshare->xmit_check_timer, jiffies+msecs_to_jiffies(USB_TXAGG_TIMEOUT));
#else
	rtk_mod_timer(&pshare->xmit_check_timer, jiffies+msecs_to_jiffies(10));
#endif
}
DEFINE_TIMER_CALLBACK(rtl8192cd_xmit_check_timer);
...
int rtl8192cd_signin_txdesc_usb(struct rtl8192cd_priv *priv, struct tx_insn* txcfg, struct wlan_ethhdr_t *pethhdr)
{
	...
	#if 0//defined(CONFIG_RTL_TRIBAND_SUPPORT)
		//printk("==> %s:%d, pxmitbuf->pkt_offset=%d\n", __func__, __LINE__, pxmitbuf->pkt_offset);
	#else

...
struct rtl8192cd_priv *if_priv[NUM_WLAN_IFACE] = {};
...
struct rtl8192cd_priv *if_priv_stactrl[NUM_WLAN_IFACE];
#endif
...
#if (defined(CONFIG_RTL8672) || defined(CONFIG_LUNA_G3_SERIES)) && !defined(__OSK__) && defined(IFF_DOMAIN_WLAN)
#define RTK_IFF_DOMAIN_WLAN IFF_DOMAIN_WLAN
#endif
...
struct _device_info_ wlan_device[] =
{
    ...
    #if defined(CONFIG_NET_PCI) && defined(CONFIG_RTL_MULTI_WIFI_MODULES)
    	...
    	{(MESH_NUM_CFG<<MESH_SHIFT) |(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_PCI_BIOS<<TYPE_SHIFT) | ACCESS_SWAP_MEM, 0, 0, 0, NULL},
    	#endif
    #elif defined(CONFIG_NET_PCI) && !defined(CONFIG_RTL_TRIBAND_SUPPORT) && !defined(CONFIG_RTK_SOC_RTL8198D)
    	{(MESH_NUM_CFG<<MESH_SHIFT) |(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_PCI_BIOS<<TYPE_SHIFT) | ACCESS_SWAP_MEM, 0, 0, 0, NULL},
    	{(MESH_NUM_CFG<<MESH_SHIFT) |(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_PCI_BIOS<<TYPE_SHIFT) | ACCESS_SWAP_MEM, 0, 0, 0, NULL},
    	{(MESH_NUM_CFG<<MESH_SHIFT) |(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_PCI_BIOS<<TYPE_SHIFT) | ACCESS_SWAP_MEM, 0, 0, 0, NULL},
	#elif defined(CONFIG_USE_PCIE_SLOT_0) && defined(CONFIG_USE_PCIE_SLOT_1) && defined(CONFIG_RTK_SOC_RTL8198D)
		#if (defined(CONFIG_RTL_5G_SLOT_0) && defined(CONFIG_BAND_2G_ON_WLAN0)) || \
					(defined(CONFIG_RTL_5G_SLOT_1) && defined(CONFIG_BAND_5G_ON_WLAN0))
		  ...
		  #endif
		#else
					//5G in wlan1/slot1, 2G in wlan0/slot0 or 5G in wlan0/slot0, 2G in wlan1/slot1
					{(MESH_NUM_CFG<<MESH_SHIFT) |(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_PCI_DIRECT<<TYPE_SHIFT) | ACCESS_SWAP_MEM, BSP_PCIE1_D_CFG0, BSP_PCIE1_D_MEM, BSP_PCIE1_IRQ, NULL },
					{(MESH_NUM_CFG<<MESH_SHIFT) |(WDS_NUM_CFG<<WDS_SHIFT) | (TYPE_PCI_DIRECT<<TYPE_SHIFT) | ACCESS_SWAP_MEM, BSP_PCIE0_D_CFG0, BSP_PCIE0_D_MEM, BSP_PCIE_IRQ, NULL},
		#endif
	#elif defined(CONFIG_USE_PCIE_SLOT_0) && defined(CONFIG_USE_PCIE_SLOT_1) // for 9607/8685/8277/G3 series
...
#else
#ifndef __OSK__
/* notifier_call must return int -- the kernel inspects the return value
 * (NOTIFY_DONE/NOTIFY_OK/...). Returning void here was a real ABI mismatch. */
...
static struct sk_buff_head g_skb_recycle_queue;
#endif

#ifndef __ECOS
static int wlan_index = 0;  // Number of wlan interfaces has been registered
...
static void free_aid_obj(struct rtl8192cd_priv *priv)
{
	unsigned int i;
...
#ifdef PRE_ALLOC_STA_BUF
static int prealloc_aid_obj(struct rtl8192cd_priv *priv)
{
	unsigned int i;
	int retval = 0;

	struct stat_info *tmp_stat_info[NUM_STAT];
	unsigned char fake_mac[6] = "123456";

	// Init linked-lists used by stat_info
	for (i = 0; i < NUM_STAT; i++)
		INIT_LIST_HEAD(&(priv->stat_hash[i]));

	INIT_LIST_HEAD(&priv->pshare->rc_timer_list);

	// Pre-allocate stat_info
	for (i = 0; i < PRE_ALLOC_NUM_STAT; i++) {
		if ((tmp_stat_info[i] = alloc_stainfo(priv, fake_mac, -1)) == NULL) {
			printk("Failed to pre-allocate %dth stat_info\n", i + 1);
			retval = 1;
			break;
		}
	}

	// Set stat_info as unused (this deosn't kfree stat_info)
	while (i-- > 0) {
		free_stainfo(priv, tmp_stat_info[i]);
	}

	// Kfree pre-allocated stat_info if any error
	if (retval) {
		printk("Kfree pre-allocated stat_info\n", i);
		free_aid_obj(priv);
	} else {
		printk("Pre-allocate %d stat_info sucessfully\n", PRE_ALLOC_NUM_STAT);
	}

	return retval;
}
#endif // PRE_ALLOC_STA_BUF
...
#ifdef PRE_ALLOC_STA_BUF
	// Only pre-allocate aid_obj (sta_info) on root interface
	if (ret == 0) {
		if (ret = prealloc_aid_obj(wlan_device[wlan_index].priv)) {
			printk("Prealloc aid_obj fail!\n");
		}
	}
#endif
...
void smp_assign_expire_timer(void *_priv)
{
    ...
    rtk_mod_timer_pinned(&priv->expire_timer, jiffies + EXPIRE_TO);
#else
...
)
{
	...
	unsigned char TXPAUSE_CHECK_FLAG = 0;

#if (CONFIG_WLAN_HAL_8192FE) && !defined(CONCURRENT_MODE)
	TXPAUSE_CHECK_FLAG = BIT_MAC_STOPBCNQ;
#else
	if (GET_CHIP_VER(priv) == VERSION_8192F)
		TXPAUSE_CHECK_FLAG = BIT_MAC_STOPBCNQ;
	else
		TXPAUSE_CHECK_FLAG = BIT_MAC_STOPBCNQ | BIT_STOP_BCN_HI_MGT;
#endif
...
#endif
#if defined(CONFIG_IEEE80211V) && defined(RTK_MULTI_AP)
                    if ((priv->pshare->bcnDOk_priv->pmib->dot11OperationEntry.opmode & WIFI_AP_STATE)){
                        if(priv->pshare->bcnDOk_priv->pmib->wnmEntry.dot11vBssTransEnable)
                            process_BssTransReq(priv->pshare->bcnDOk_priv);
                    }
#endif
				}
#else // !MBSSID
				...
				if ((OPMODE & WIFI_AP_STATE)) {
					if ( TRUE == GET_HAL_INTERFACE(priv)->GetInterruptHandler(priv, HAL_INT_TYPE_TBDER) ) {
						...
						if (priv->ext_stats.is_init_beacon_finished  &&
								...
								!(RTL_R8(TXPAUSE) & TXPAUSE_CHECK_FLAG))){
...
#if !defined(RING_BASED_DATA_FRAME_QUEUE) && !defined(SMP_RX_LOAD_BALANCE)
    if (caseRxRDU) {
#ifdef CONFIG_RTL8190_PRIV_SKB
#ifdef CONCURRENT_MODE
...
#endif // !(RING_BASED_DATA_FRAME_QUEUE) && !(SMP_RX_LOAD_BALANCE)
...

{
    ...
    if (_FALSE == GET_HAL_INTERFACE(priv)->InterruptRecognizedHandler(priv, NULL, 0)) {
    	...
    	// don't print for non-RTK platforms, interrupt line may be shared among devices
			...
			DEBUG_INFO("NULL Wlan Interrupt !?\n");
			DEBUG_INFO("Reg_HISR0:  %08x, Reg_HISR1: %08x\n", RTL_R32(REG_HISR0), RTL_R32(REG_HISR1));//should back up halisr to printk
			DEBUG_INFO("Reg_HIMR0:  %08x, Reg_HISM1: %08x\n", RTL_R32(REG_HIMR0), RTL_R32(REG_HIMR1));
			DEBUG_INFO("IntMask0:   %08x, IntMask1:      %08x\n",_GET_HAL_DATA(priv)->IntMask[0], _GET_HAL_DATA(priv)->IntMask[1]);
...
int_retry_process:
	{
        ...
        // Note: If there has skbs in rx_queue (even though no Rx interrupt is occurred), we still need to
        // ececute InterruptRxHandle() to take those skbs out of rx_queue.
        if (TRUE == caseRxRDU || TRUE == caseRxOK || TRUE == caseRxFOVW || (cur_q->host_idx != cur_q->hw_idx)) {
...
#else // !(SMP_SYNC) && !defined(__ECOS)
	if (OPMODE & WIFI_MP_STATE)
		rtl8192cd_tx_dsr((unsigned long)priv);
#endif // (SMP_SYNC) || (__ECOS)
	else
#endif
	if (
...
#ifdef __LINUX_3_7__
void rtl8192cd_get_stats64(struct net_device *dev, struct rtnl_link_stats64 *storage)
{
	struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	...
	if (dev->base_addr == 0) {
		...
		if (idx < 0) {
			memset(&priv->wds_stats[NUM_WDS-1], 0, sizeof(struct rtnl_link_stats64));
			SMP_UNLOCK(flags);
			return;
		}
		...
		if (netif_running(dev) && IS_DRV_OPEN(priv)) {
			...
			if (pstat == NULL) {
				...
				memset(&priv->wds_stats[idx], 0, sizeof(struct rtnl_link_stats64));
			} else {
				...
				memcpy(storage, &priv->wds_stats[idx], sizeof(struct rtnl_link_stats64));
			}
		}
	...
	if (dev->base_addr == 1) {
		if (priv->mesh_dev == dev) {
			memcpy(storage, &priv->mesh_stats, sizeof(struct rtnl_link_stats64));
		}
	...
	memcpy(storage, &priv->net_stats, sizeof(struct rtnl_link_stats64));
...
static int rtl8192cd_init_sw(struct rtl8192cd_priv *priv)
{
	...
	if (((priv->pmib->dot11StationConfigEntry.dot11RegDomain == DOMAIN_MKK) ||
		...
		if (priv->pmib->dot11BssType.net_work_type & WIRELESS_11A) {
			...
			if (!rtk_timer_pending(&priv->ch52_timer))
				InsertChannel(priv->NOP_chnl, &priv->NOP_chnl_num, 52);
			if (!rtk_timer_pending(&priv->ch56_timer))
				InsertChannel(priv->NOP_chnl, &priv->NOP_chnl_num, 56);
			if (!rtk_timer_pending(&priv->ch60_timer))
				InsertChannel(priv->NOP_chnl, &priv->NOP_chnl_num, 60);
			if (!rtk_timer_pending(&priv->ch64_timer))
	...
	if (!priv->pmib->dot11DFSEntry.disable_DFS &&
		((rtk_timer_pending(&priv->ch52_timer) && (priv->pmib->dot11RFEntry.dot11channel == 52)) ||
		 (rtk_timer_pending(&priv->ch56_timer) && (priv->pmib->dot11RFEntry.dot11channel == 56)) ||
		 (rtk_timer_pending(&priv->ch60_timer) && (priv->pmib->dot11RFEntry.dot11channel == 60)) ||
		 (rtk_timer_pending(&priv->ch64_timer) && (priv->pmib->dot11RFEntry.dot11channel == 64)) ||
		 (rtk_timer_pending(&priv->ch100_timer) && (priv->pmib->dot11RFEntry.dot11channel == 100)) ||
		 (rtk_timer_pending(&priv->ch104_timer) && (priv->pmib->dot11RFEntry.dot11channel == 104)) ||
		 (rtk_timer_pending(&priv->ch108_timer) && (priv->pmib->dot11RFEntry.dot11channel == 108)) ||
		 (rtk_timer_pending(&priv->ch112_timer) && (priv->pmib->dot11RFEntry.dot11channel == 112)) ||
		 (rtk_timer_pending(&priv->ch116_timer) && (priv->pmib->dot11RFEntry.dot11channel == 116)) ||
		 (rtk_timer_pending(&priv->ch120_timer) && (priv->pmib->dot11RFEntry.dot11channel == 120)) ||
		 (rtk_timer_pending(&priv->ch124_timer) && (priv->pmib->dot11RFEntry.dot11channel == 124)) ||
		 (rtk_timer_pending(&priv->ch128_timer) && (priv->pmib->dot11RFEntry.dot11channel == 128)) ||
		 (rtk_timer_pending(&priv->ch132_timer) && (priv->pmib->dot11RFEntry.dot11channel == 132)) ||
		 (rtk_timer_pending(&priv->ch136_timer) && (priv->pmib->dot11RFEntry.dot11channel == 136)) ||
		 (rtk_timer_pending(&priv->ch140_timer) && (priv->pmib->dot11RFEntry.dot11channel == 140)) ||
		 (rtk_timer_pending(&priv->ch144_timer) && (priv->pmib->dot11RFEntry.dot11channel == 144)) ||
		 (rtk_timer_pending(&priv->ch149_timer) && (priv->pmib->dot11RFEntry.dot11channel == 149)) ||
		 (rtk_timer_pending(&priv->ch153_timer) && (priv->pmib->dot11RFEntry.dot11channel == 153)) ||
		 (rtk_timer_pending(&priv->ch157_timer) && (priv->pmib->dot11RFEntry.dot11channel == 157)) ||
		 (rtk_timer_pending(&priv->ch161_timer) && (priv->pmib->dot11RFEntry.dot11channel == 161)) ||
		 (rtk_timer_pending(&priv->ch165_timer) && (priv->pmib->dot11RFEntry.dot11channel == 165)))) {
...
&& (GET_VXD_PRIV(priv)->simple_config_status==0)
...
#endif
					{
						...
						if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
...
#endif
#else // !((RTL8190_ISR_RX) && (RTL8190_DIRECT_RX))
							...
							memset(&(priv->pshare->rxq_tasklet_csd), 0, sizeof(rtk_call_single_data_t));
							priv->pshare->rxq_tasklet_csd.func = smp_assign_tasklet;
							priv->pshare->rxq_tasklet_csd.info = (void *)&priv->pshare->rxq_tasklet.tasklet;
#endif
#ifdef SMP_RX_LOAD_BALANCE
#if (SMP_RX_LB_LEVEL == 1)
							rtk_tasklet_init(&priv->pshare->rxq_tasklet, process_netif_rxq, (unsigned long)priv);
							...
							priv->pshare->rxq_tasklet_csd.func = smp_assign_tasklet;
							priv->pshare->rxq_tasklet_csd.info = (void *)&priv->pshare->rxq_tasklet.tasklet;
#endif // SMP_RX_LOAD_BALANCE
#endif // (RTL8190_ISR_RX) && (RTL8190_DIRECT_RX)
							rtk_tasklet_init(&priv->pshare->tx_tasklet, rtl8192cd_tx_dsr, (unsigned long)priv);
#ifdef TX_PROCESS_REFINE
							memset(&(priv->pshare->txq_tasklet_csd), 0, sizeof(rtk_call_single_data_t));
							priv->pshare->txq_tasklet_csd.func = smp_assign_tasklet;
							priv->pshare->txq_tasklet_csd.info = (void *)&priv->pshare->tx_tasklet.tasklet;
#endif
#ifndef __ECOS
...
#if 1 //def CONFIG_WIFI_LED_SHARED
...
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
		if (GET_HCI_TYPE(priv) == RTL_HCI_USB || GET_HCI_TYPE(priv) == RTL_HCI_SDIO) {
			rtk_mod_timer(&priv->pshare->xmit_check_timer, jiffies + RTL_SECONDS_TO_JIFFIES(1));
		}
#endif
	...
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		...
		/* To set the DMA address for both RX/TX ring */
		{
			...
			for (i=0; i<NUM_RX_DESC_IF(priv); i++) {
				phw->rx_descL_dma_addr[i] = get_physical_addr(priv, (void *)(&phw->rx_descL[i]),
					sizeof(struct rx_desc), DMA_TO_DEVICE);
			}
#endif
			...
			while (tx_desc_array[txDescRingIdx] != 0) {
				...
				for (i=0; i<CURRENT_NUM_TX_DESC; i++) {
					tx_desc_dma_ptr[i] = get_physical_addr(priv, (void *)(&(tx_desc_ptr[i])),
						sizeof(struct tx_desc), DMA_TO_DEVICE);
				}
#endif
        ...
        skb_queue_head_init(&priv->pshare->hwnat_netif_rx_queue);
#endif
#if defined(SMP_RX_LOAD_BALANCE) && (SMP_RX_LB_LEVEL == 1)
        skb_queue_head_init(&priv->pshare->netif_rx_queue);
#endif
	...
	if(!CHIP_VER_92X_SERIES(priv))
		...
		if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
...
#endif
				{
	            	rtk_tasklet_init(&priv->pshare->swq_tasklet, rtl8192cd_swq_timeout_tasklet_cb, (unsigned long)priv);
#if defined(SMP_LOAD_BALANCE_SUPPORT) && defined(CONFIG_RTL9607C) && defined(CONCURRENT_MODE)
					// init csd structure required for smp_call_function_single_async()
					memset(&(priv->pshare->swq_tasklet_csd), 0, sizeof(rtk_call_single_data_t));
					priv->pshare->swq_tasklet_csd.func = smp_assign_tasklet;
					priv->pshare->swq_tasklet_csd.info = (void *)&priv->pshare->swq_tasklet.tasklet;
#endif
				}
#if defined(SMP_LOAD_BALANCE_SUPPORT)
				memset(&(priv->pshare->smp_assign_expire_timer_csd), 0, sizeof(rtk_call_single_data_t));
			...
			{
				init_swq_sw_timer(priv);
			}
		}
		reset_swq_max_xmit(priv);
#endif
...
/*cfg p2p cfg p2p*/
    ...
    if(priv->p2pPtr){
        init_p2p_find_timer(priv);
        init_P2P_search_timer(priv);

        #ifdef RTK_NL80211
        rtk_p2p_init_remain_on_ch_timer(priv);
        rtk_p2p_init_scan_deny_timer(priv);
        #endif //CONFIG_P2P
    }
	...
	*/
	...
	INIT_LIST_HEAD(&(priv->delsta_list));
	INIT_LIST_HEAD(&(priv->sleep_list));
	INIT_LIST_HEAD(&(priv->defrag_list));
	INIT_LIST_HEAD(&(priv->wakeup_list));
#if defined(SMP_RX_LOAD_BALANCE) && (SMP_RX_LB_LEVEL == 2)
	skb_queue_head_init(&priv->rx_data_queue);
#endif
#ifdef RING_BASED_DATA_FRAME_QUEUE
	...
	spin_lock_init(&(priv->delsta_list_lock));
	...
	_rtw_mutex_init(&(priv->delsta_list_lock));
	...
	// construct operation and basic rates set
	{
		...
		if (priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_5G)
		{
			DEBUG_INFO("Only 2.4G will use NHM-ACS currently\n");
...
#ifdef __ECOS
	memcpy(priv->hw_serial_mac, GET_MY_HWADDR, MACADDRLEN);
#else
	RCU_READ_LOCK();
	br_port = GET_BR_PORT(priv->dev);
	if (br_port)
#if defined(__LINUX_2_6__) || defined(__OSK__)
		memcpy(priv->hw_serial_mac, br_port->br->dev->dev_addr, MACADDRLEN);
#else
		memcpy(priv->hw_serial_mac, priv->dev->br_port->br->dev.dev_addr, MACADDRLEN);
#endif
	RCU_READ_UNLOCK();
#endif
#endif //CONFIG_RTK_WPS_MESH
...
/* No idea how to process AMSDU shortcut with MESH or WDS, so disable AMSDU shortcut when MESH or WDS enabled */
	...
	if (priv->pmib->dot1180211AuthEntry.dot11EnablePSK) {
		...
		else if(priv->pmib->dot11nConfigEntry.dot11nCurAMSDUSendNum > WLAN_HAL_TX_AMSDU_MAX_NUM ){
			priv->pmib->dot11nConfigEntry.dot11nCurAMSDUSendNum = WLAN_HAL_TX_AMSDU_MAX_NUM;
		}
#endif
	} else {
	...
	rtk_multi_ap_init_backhaul_steer_timer(priv);

	rtk_multi_ap_init_unassoc_sta_timer(priv);
	...
	if (priv->pmib->multi_ap.multiap_bss_type)
		priv->pmib->miscEntry.a4_enable = 1;
#endif
	...
	if (priv->pmib->dot11StationConfigEntry.dot11RadioMeasurementActivated) {
		rm_init_delay_measure_timer(priv);
	}
#endif
	...
	if(priv->pmib->dot11StationConfigEntry.dot11DesiredSSIDLen==2 &&
		...
		priv->is_ssid_ps=0;
#endif
#ifdef WIFI_LOGO_WPA3_MARVELL_STA_WKARD
	if(!memcmp(priv->pmib->dot11StationConfigEntry.dot11DesiredSSID, "Wi-Fi-4.2.1", 11) ||
		!memcmp(priv->pmib->dot11StationConfigEntry.dot11DesiredSSID, "Wi-Fi-4.2.7", 11))
		priv->use_marvell_sta=1;
	else
		priv->use_marvell_sta=0;
#endif
	...
	if((OPMODE & WIFI_STATION_STATE) && IS_ROOT_INTERFACE(priv))
		priv->pmib->miscEntry.ss_to = 90;
	else
		priv->pmib->miscEntry.ss_to = SS_TO_MS;
#endif

	if (priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G) {
		if (priv->pmib->dot11nConfigEntry.dot11nUse40M == CHANNEL_WIDTH_20)
			priv->pshare->rf_ft_var.tri_time_rx = 14;
		else
			priv->pshare->rf_ft_var.tri_time_rx = 7;
	}
...
static int rtl8192cd_tx_desc_free(struct rtl8192cd_priv *priv){
    ...
    {
        if ((i++) > 10){
            printk("Warning: Free txdesc_skb maybe failed\n");
            break;
        }
        delay_ms(5);
    }
...
#ifdef SW_TXQ_ATF
			swq_atf_calc_hwq_pktsz(&tx_info[i], FALSE);
#endif
			...
			if (IS_HAL_CHIP(priv)) {
				if (tx_info[i].buf_pframe[0] && (tx_info[i].buf_type[0] == _SKB_FRAME_TYPE_)) { // should be buf_paddr
					...
					if (IS_PCIBIOS_TYPE && (tx_info[i].buf_len[0]!=0 && tx_info[i].buf_paddr[0]!=0))
						dma_unmap_single(&priv->pshare->pdev->dev, tx_info[i].buf_paddr[0],(tx_info[i].buf_len[0])&0xffff, DMA_TO_DEVICE);
#endif
#ifdef MP_TEST
					...
					{
#ifdef ENABLE_RTL_SKB_STATS
						rtl_kfree_skb(tx_info[i].priv, tx_info[i].buf_pframe[0], _SKB_TX_);
#else
...
#ifdef WLAN_HAL_TX_AMSDU
				for (cnt = 0; cnt < tx_info[i].amsdu_num; cnt++) {
					if (tx_info[i].buf_type_amsdu[cnt] == _SKB_FRAME_TYPE_ && tx_info[i].buf_pframe_amsdu[cnt]) {
#if defined(CONFIG_NET_PCI) && !defined(USE_RTL8186_SDK)
						if (IS_PCIBIOS_TYPE) {
							dma_unmap_single(&priv->pshare->pdev->dev, tx_info[i].buf_paddr_amsdu[cnt], tx_info[i].buf_len_amsdu[cnt], DMA_TO_DEVICE);
						}
#endif
#ifdef ENABLE_RTL_SKB_STATS
						rtl_kfree_skb(tx_info[i].priv, (struct sk_buff *)(tx_info[i].buf_pframe_amsdu[cnt]), _SKB_TX_);
#else
						rtl_kfree_skb(priv, (struct sk_buff *)(tx_info[i].buf_pframe_amsdu[cnt]), _SKB_TX_);
#endif
					}
				}
#endif
...
#ifdef CONFIG_NET_PCI
					if (IS_PCIBIOS_TYPE)
						dma_unmap_single(&priv->pshare->pdev->dev, tx_info[i].paddr, (tx_info[i].len), DMA_TO_DEVICE);
#endif
...
#ifdef CONFIG_WLAN_HAL
		if (IS_HAL_CHIP(priv)) {
			ptx_dma->tx_queue[halQnum].hw_idx = ptx_dma->tx_queue[halQnum].host_idx = 0;
		} else if(CONFIG_WLAN_NOT_HAL_EXIST)
#endif
		{
			*get_txtail_addr(phw, j) = *get_txhead_addr(phw, j) = 0;
		}
	} // TX descriptor Free
...
static int rtl8192cd_stop_sw(struct rtl8192cd_priv *priv)
{
	...
	SMP_UNLOCK(flags);
	if (rtk_timer_pending(&priv->frag_to_filter))
	{
		rtk_del_timer_sync(&priv->frag_to_filter);
	}
...
#endif
	{
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
		if (GET_HCI_TYPE(priv) == RTL_HCI_USB || GET_HCI_TYPE(priv) == RTL_HCI_SDIO) {
			if (rtk_timer_pending(&priv->pshare->xmit_check_timer))
				rtk_del_timer_sync(&priv->pshare->xmit_check_timer);
		}
#endif
#ifdef CONFIG_USB_HCI
		if (GET_HCI_TYPE(priv) == RTL_HCI_USB) {
			...
			if (rtk_timer_pending(&priv->pshare->beacon_timer))
				rtk_del_timer_sync(&priv->pshare->beacon_timer);
#endif
		}
#endif // CONFIG_USB_HCI
#ifdef CONFIG_SDIO_HCI
		...
		if (rtk_timer_pending(&priv->tcpack_timer))
			rtk_del_timer_sync(&priv->tcpack_timer);
#endif

		if (rtk_timer_pending(&priv->expire_timer))
			rtk_del_timer_sync(&priv->expire_timer);
#ifdef 	SW_ANT_SWITCH
		if (rtk_timer_pending(&priv->pshare->swAntennaSwitchTimer))
			rtk_del_timer_sync(&priv->pshare->swAntennaSwitchTimer);
#endif
		if (rtk_timer_pending(&priv->pshare->rc_sys_timer))
			rtk_del_timer_sync(&priv->pshare->rc_sys_timer);
#if 0
		if (rtk_timer_pending(&priv->pshare->phw->tpt_timer))
			rtk_del_timer_sync(&priv->pshare->phw->tpt_timer);
#endif
	...
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		if (rtk_timer_pending(&priv->ps_timer))
			rtk_del_timer_sync(&priv->ps_timer);
	}
#endif

#ifdef SDIO_AP_OFFLOAD
	if (rtk_timer_pending(&priv->pshare->ps_timer))
		rtk_del_timer_sync(&priv->pshare->ps_timer);
#endif

//#if (BEAMFORMING_SUPPORT == 1)
//	if (priv->pmib->dot11RFEntry.txbf) {
//		if (rtk_timer_pending(&priv->txbf_timer))
//			rtk_del_timer_sync(&priv->txbf_timer);
//		if (rtk_timer_pending(&priv->txbf_swtimer))
//			rtk_del_timer_sync(&priv->txbf_swtimer);
//	}
//#endif
		 ...
		 * CAUTION !! These statement meshX(virtual interface) ONLY, Maybe modify....
		 */
		if (rtk_timer_pending(&priv->mesh_peer_link_timer))
			rtk_del_timer_sync(&priv->mesh_peer_link_timer);

#ifdef MESH_BOOTSEQ_AUTH
		if (rtk_timer_pending(&priv->mesh_auth_timer))
			rtk_del_timer_sync(&priv->mesh_auth_timer);
#endif
#endif

#if defined(CONFIG_RTL_92D_SUPPORT) && defined(DPK_92D)
		if (GET_CHIP_VER(priv) == VERSION_8192D){
			if (rtk_timer_pending(&priv->pshare->DPKTimer))
				rtk_del_timer_sync(&priv->pshare->DPKTimer);
		}
#endif

#ifdef RTK_ATM
		if (priv->pshare->atm_swq_use_hw_timer == 0 && rtk_timer_pending(&priv->pshare->atm_swq_sw_timer)) {
			rtk_del_timer_sync(&priv->pshare->atm_swq_sw_timer);
		}
#endif
	...
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
        if(priv->pshare->swq_use_hw_timer) {
            ...
            if (rtk_timer_pending(&priv->pshare->swq_sw_timer))
                rtk_del_timer_sync(&priv->pshare->swq_sw_timer);
        }
    }
#endif

#if defined(MULTI_STA_REFINE)
		if (rtk_timer_pending(&priv->pshare->PktAging_timer))
			rtk_del_timer_sync(&priv->pshare->PktAging_timer);
#endif

#ifdef ADAPTIVITY_STATS_DEBUG
		if (rtk_timer_pending(&priv->pshare->ada_stats_timer))
			rtk_del_timer_sync(&priv->pshare->ada_stats_timer);
#endif
	}
	...
	if (IS_VXD_INTERFACE(priv)){
		...
		if (rtk_timer_pending(&priv->pshare->vxd_ibss_beacon))
			rtk_del_timer_sync(&priv->pshare->vxd_ibss_beacon);
	}
#endif
#endif

	if (rtk_timer_pending(&priv->ss_timer))
		rtk_del_timer_sync(&priv->ss_timer);
	if (rtk_timer_pending(&priv->MIC_check_timer))
		rtk_del_timer_sync(&priv->MIC_check_timer);
	if (rtk_timer_pending(&priv->assoc_reject_timer))
		rtk_del_timer_sync(&priv->assoc_reject_timer);
#if defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_92C_SUPPORT)
	#ifdef CONFIG_PCI_HCI
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		// to avoid add RAtid fail
		if (rtk_timer_pending(&priv->add_RATid_timer))
			rtk_del_timer_sync(&priv->add_RATid_timer);
...
#ifdef CONFIG_PCI_HCI
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		if (rtk_timer_pending(&priv->add_ps_timer))
			rtk_del_timer_sync(&priv->add_ps_timer);
	}
#endif
...
#ifdef CLIENT_MODE
	if (rtk_timer_pending(&priv->reauth_timer))
		rtk_del_timer_sync(&priv->reauth_timer);
	if (rtk_timer_pending(&priv->reassoc_timer))
		rtk_del_timer_sync(&priv->reassoc_timer);
	if (rtk_timer_pending(&priv->idle_timer))
		rtk_del_timer_sync(&priv->idle_timer);
#endif
...
#ifdef RTK_MULTI_AP
	if (rtk_timer_pending(&priv->multiap_backhaul_steer_timer))
		rtk_del_timer_sync(&priv->multiap_backhaul_steer_timer);

	if (rtk_timer_pending(&priv->multiap_unassoc_sta_timer))
		rtk_del_timer_sync(&priv->multiap_unassoc_sta_timer);
...
SMP_LOCK(flags);
...
#ifdef GBWC
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		if (rtk_timer_pending(&priv->GBWC_timer))
			rtk_del_timer_sync(&priv->GBWC_timer);
...
#ifdef SBWC
	if (rtk_timer_pending(&priv->SBWC_timer))
		rtk_del_timer_sync(&priv->SBWC_timer);
#endif

#ifdef RTK_STA_BWC
	if (rtk_timer_pending(&priv->sta_bwc_timer))
		rtk_del_timer_sync(&priv->sta_bwc_timer);
#endif

#ifdef INCLUDE_WPA_PSK
	if (rtk_timer_pending(&priv->wpa_global_info->GKRekeyTimer))
		rtk_del_timer_sync(&priv->wpa_global_info->GKRekeyTimer);
#endif
...
#endif
	{
		...
		if (priv->pmib->dot11RFEntry.txbf) {
			...
			if (priv->pshare->rf_ft_var.txbf_iqgen_keep_on) {
#ifdef defined(CONFIG_WLAN_HAL_8812FE)
				if (GET_CHIP_VER(priv) == VERSION_8812F) {
					printk("[%s] phydm_txbf_rfmode(0)\n", __func__);
					phydm_txbf_rfmode(ODMPTR, 0, 0);
				}
#endif
			}
		}
...
#ifdef DFS
		SMP_UNLOCK(flags);
		if (rtk_timer_pending(&priv->DFS_TXPAUSE_timer))
			rtk_del_timer_sync(&priv->DFS_TXPAUSE_timer);

		if (rtk_timer_pending(&priv->DFS_timer))
			rtk_del_timer_sync(&priv->DFS_timer);

		if (rtk_timer_pending(&priv->ch_avail_chk_timer))
			rtk_del_timer_sync(&priv->ch_avail_chk_timer);

		if (rtk_timer_pending(&priv->dfs_chk_timer))
			rtk_del_timer_sync(&priv->dfs_chk_timer);

		if (rtk_timer_pending(&priv->dfs_det_chk_timer))
			rtk_del_timer_sync(&priv->dfs_det_chk_timer);
		...
		if (rtk_timer_pending(&priv->ch52_timer) &&
			...
			rtk_del_timer_sync(&priv->ch52_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 52);
		}

		if (rtk_timer_pending(&priv->ch56_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch56_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 56);
		}

		if (rtk_timer_pending(&priv->ch60_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch60_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 60);
		}

		if (rtk_timer_pending(&priv->ch64_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch64_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 64);
		}

		if (rtk_timer_pending(&priv->ch100_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch100_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 100);
		}

		if (rtk_timer_pending(&priv->ch104_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch104_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 104);
		}

		if (rtk_timer_pending(&priv->ch108_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch108_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 108);
		}

		if (rtk_timer_pending(&priv->ch112_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch112_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 112);
		}

		if (rtk_timer_pending(&priv->ch116_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch116_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 116);
		}

		if (rtk_timer_pending(&priv->ch120_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch120_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 120);
		}

		if (rtk_timer_pending(&priv->ch124_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch124_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 124);
		}

		if (rtk_timer_pending(&priv->ch128_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch128_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 128);
		}

		if (rtk_timer_pending(&priv->ch132_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch132_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 132);
		}

		if (rtk_timer_pending(&priv->ch136_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch136_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 136);
		}

		if (rtk_timer_pending(&priv->ch140_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch140_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 140);
		}

		if (rtk_timer_pending(&priv->ch144_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch144_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 144);
		}

		if (rtk_timer_pending(&priv->ch149_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch149_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 149);
		}

		if (rtk_timer_pending(&priv->ch153_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch153_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 153);
		}

		if ((priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch157_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 157);
		}

		if (rtk_timer_pending(&priv->ch161_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch161_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 161);
		}

		if (rtk_timer_pending(&priv->ch165_timer) &&
			(priv->pmib->dot11DFSEntry.disable_DFS)) {
			rtk_del_timer_sync(&priv->ch165_timer);
			RemoveChannel(priv, priv->NOP_chnl, &priv->NOP_chnl_num, 165);
		}
		SMP_LOCK(flags);
...
#if defined(WIFI_RX_PROCESS_ALL_QUEUE_OFFLOAD) || defined(SMP_RX_LOAD_BALANCE)
	...
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		for (i=0; i<NUM_RX_DESC_IF(priv); i++)
		{
			if (phw->rx_infoL[i].pbuf != NULL) {
				...
				if (IS_PCIBIOS_TYPE)
					dma_unmap_single(&priv->pshare->pdev->dev, phw->rx_infoL[i].paddr, (RX_BUF_LEN - sizeof(struct rx_frinfo)), DMA_FROM_DEVICE);
#endif
				...
				if (priv->reset_hangup) {
					...
					if (priv->pshare->skb_queue.qlen < priv->pshare->skb_queue.qmax)
						rtk_queue_tail(priv, &priv->pshare->skb_queue, (struct sk_buff*)(phw->rx_infoL[i].pbuf));
					else
		...
		if (IS_HAL_CHIP(priv)) {
		    ...
		    if (IS_PCIBIOS_TYPE) {
				...
				#endif
					dma_unmap_single(&priv->pshare->pdev->dev, get_desc(cur_txbd->TXBD_ELE[1].Dword1), 128*sizeof(unsigned int), DMA_TO_DEVICE);
		    }
		} else if(CONFIG_WLAN_NOT_HAL_EXIST)
#endif
		{
				...
				if(GET_CHIP_VER(priv)== VERSION_8812E) {
					...
					if (phw->tx_descB->Dword10 != 0)
						...
						dma_unmap_single(&priv->pshare->pdev->dev, get_desc(phw->tx_descB->Dword10),
							128*sizeof(unsigned int), DMA_TO_DEVICE);
				} else
#endif
				{
					#ifdef NOT_RTK_BSP
					...
					#endif
						dma_unmap_single(&priv->pshare->pdev->dev, get_desc(phw->tx_descB->Dword8), 128*sizeof(unsigned int), DMA_TO_DEVICE);
				}
		    }
		}
...
#ifdef CONFIG_HWNAT_NETIF_RX_QUEUE_SUPPORT
		free_skb_queue(priv, &priv->pshare->hwnat_netif_rx_queue);
#endif
#if defined(SMP_RX_LOAD_BALANCE) && (SMP_RX_LB_LEVEL == 1)
		free_skb_queue(priv, &priv->pshare->netif_rx_queue);
#endif
...
#if defined(CHECK_HANGUP) && !defined(PRE_ALLOC_STA_BUF)
	if (!priv->reset_hangup)
		free_aid_obj(priv);
#endif
...
//			priv->pmib->dot11OperationEntry.keep_rsnie = 1;
	...
	_rtw_mutex_free(&(priv->delsta_list_lock));
							  ...
							  u8 rate, u8 bandwidth, u8 channel)
{
	...
	if (!priv->pshare->rf_ft_var.disable_txpwrlmt) {
			...
			// This causes wrong chidx_20 and chidx_40 when looking up pwr lmt table
			if(priv->pshare->CurrentChannelBW==1)
				channel = channel-2;

			// Unnecessary to reload pwr lmt table unless switching channel
...
static __always_inline void sdio_power_save_timer(unsigned long task_priv)
{
	...
	printk("acli: get %d %d %d\n", priv->pshare->total_assoc_num, act, sts);
        ...
        switch ( ACT_STS(act, sts) )
        {
        	  ...
        	  case ACT_STS(RTW_ACT_ASSOC, RTW_STS_SUSPEND):
        	     ...
        	     rtk_mod_timer(&priv->pshare->ps_timer, jiffies + POWER_DOWN_T0 + POWER_DOWN_T0);
        	     goto out;
        	     break;
...
out1:
       rtk_mod_timer(&priv->pshare->ps_timer, jiffies + POWER_DOWN_T0);

out:
       priv->pshare->ps_ctrl = RTW_ACT_IDLE;
       return;
}
DEFINE_TIMER_CALLBACK(sdio_power_save_timer);

void init_sdio_power_save_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->pshare->ps_timer), sdio_power_save_timer, (unsigned long)priv, 0);
}
...
void hard_code_8814b_mibs(struct rtl8192cd_priv *priv)
{
	...
	priv->pshare->rf_ft_var.auto_amsdu_vw = 1;
#if 0
...
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
	if (OFFLOAD_ENABLE(priv)) {
		if (priv->pshare->fw_download_status)
			return 0;
	}
#endif
...
priv->pshare->curr_band =0; //init curr_band value
	...
	#if defined(CONFIG_RTL_CUSTOM_PASSTHRU)
	if ((passThruWanIdx != -1) && (dev==wlan_device[passThruWanIdx].priv->pWlanDev)) {
		if(priv->drv_state&DRV_STATE_OPEN)
		{
			netif_start_queue(dev);
			return 0;
...
do_hw_init:
	...
	rtk_wlan_manager_nl_hook(priv, priv->pshare->curr_band, priv->if_id);
#endif
...
#if defined(UNIVERSAL_REPEATER) && defined(MULTI_MAC_CLONE) && defined(CONFIG_WLAN_HAL)
	if(priv->pmib->ethBrExtInfo.macclone_enable && IS_HAL_CHIP(priv) && IS_VXD_INTERFACE(priv))
	{
		pu1Byte     macAddr;
		macAddr = GET_MIB(priv)->dot11OperationEntry.hwaddr;
		memcpy(macAddr ,(GET_MIB(GET_ROOT(priv)))->dot11OperationEntry.hwaddr, MACADDRLEN);
		macAddr[4] ^= macAddr[3] ^ 0xca;
		macAddr[5] ^= macAddr[3] ^ 0xac;
		macAddr[0] |= BIT1;
		eth_hw_addr_set(priv->dev, macAddr);
...
//------------------- david+2006-06-30
...
#ifdef CTC_WIFI_DIAG
		ctcwifi_priv_set(priv);
		rtk_timer_setup(&(priv->pshare->ctc_diag_tx_bcn_timer),
				ctcwifi_diag_tx_bcn_timer_func, (unsigned long)priv, 0);
#endif /* CTC_WIFI_DIAG */
	...
	if (OFFLOAD_ENABLE(priv)) {
		...
		dc_if_update(priv, 0);
		//priv->pshare->rf_ft_var.dc_bcn_mode=1;
	}
#endif
...
/*cfg p2p cfg p2p rm*/
...
#ifdef CLIENT_MODE
	init_reauth_timer(priv);

	init_reassoc_timer(priv);

	init_idle_timer(priv);

#ifdef DFS
	init_dfs_cntdwn_timer(priv);
#endif
#endif // CLIENT_MODE
	...
	for (i=0; i<priv->pshare->mclone_num_max; i++) {
	    if (priv->pshare->mclone_sta[i].isTimerInit == 0) {// if it is used, do not re-init
			...
			init_mclone_reauth_timer(priv, i);

			init_mclone_reassoc_timer(priv, i);
		}
	}
#endif
	...
	init_frag_to_filter_timer(priv);
...
#endif
	{
		...
		if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
			...
			init_PCIe_power_save_timer(priv);
			rtk_mod_timer(&priv->ps_timer, jiffies + POWER_DOWN_T0);
//			priv->ps_ctrl = 0x11;
#else // !(PCIE_POWER_SAVING)
			init_RF_MIMO_check_timer(priv);
			// Move rf_status initialization to init_sw to avoid state is wrong when calling in phy_iq_calibrate
			//priv->pshare->rf_status = get_rf_mimo_mode(priv);
			rtk_mod_timer(&priv->ps_timer, jiffies + IDLE_T0);
#endif // PCIE_POWER_SAVING
		}
#endif // PCIE_POWER_SAVING || RF_MIMO_SWITCH

#ifdef SDIO_AP_OFFLOAD
		init_sdio_power_save_timer(priv);
#ifdef MP_TEST
		if (!priv->pshare->rf_ft_var.mp_specific)
#endif
		{
		...
		init_expire_timer(priv);

#ifdef 	SW_ANT_SWITCH
		init_swAntennaSwitchTimer(priv);
#endif

#if defined(SMP_LOAD_BALANCE_SUPPORT) && defined(__LINUX_3_15__) // smp_call_function_single_async() is support when kernel >= 3.15
		...
		if (priv->pmib->miscEntry.tcpack_acc)
		{
			init_tcpack_timer(priv);
			rtk_mod_timer(&priv->tcpack_timer, jiffies + RTL_SECONDS_TO_JIFFIES(5));
		}
#endif
		...
		if (IS_HAL_CHIP(priv) || (GET_CHIP_VER(priv)==VERSION_8812E)) {
			init_PktAging_timer(priv);
			rtk_mod_timer(&priv->pshare->PktAging_timer, jiffies + RTL_SECONDS_TO_JIFFIES(10));
		}
#endif
	...
	// for MIC check
	init_MIC_check_timer(priv);
	init_assoc_reject_timer(priv);
...
#ifdef GBWC
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		init_GBWC_timer(priv);
		if (priv->pmib->gbwcEntry.GBWCMode != GBWC_MODE_DISABLE)
			rtk_mod_timer(&priv->GBWC_timer, jiffies + GBWC_TO);
	}
#endif

#ifdef SBWC
	if (OPMODE & WIFI_AP_STATE) {
		init_SBWC_timer(priv);
		rtk_mod_timer(&priv->SBWC_timer, jiffies + SBWC_TO);
	}
#endif

#ifdef RTK_STA_BWC
	init_sta_bwc_timer(priv);
	if (priv->pshare->rf_ft_var.sta_bwc_en)
		rtk_mod_timer(&priv->sta_bwc_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(priv->pshare->rf_ft_var.sta_bwc_to));
#endif
	...
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		// to avoid add RAtid fail
#if defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_92C_SUPPORT)
		init_add_RATid_timer(priv);
		init_add_rssi_timer(priv);
#endif
		init_add_ps_timer(priv);
	}
#endif
...
#endif
	{
		...
		if (!priv->pmib->dot11DFSEntry.disable_DFS &&
			...
			/* Init priv->DFS_TXPAUSE_timer */
			init_DFS_TXPAUSE_timer(priv, IGNORE_DRIVER_STATUS);
...
#ifdef __KERNEL__
#ifndef CONFIG_RTL8192CD_MODULE
#ifndef __LINUX_3_9__
		// defined in linux/mm/page_alloc.c before kernel 3.9
		extern int min_free_kbytes;
#endif // __LINUX_3_9__

		/* 6.x: the driver's own extern is inside #ifndef __LINUX_3_9__ (dead
		 * here); min_free_kbytes is still a kernel global, declare it. */
		extern int min_free_kbytes;
		unsigned int mcst_min_mem = ((unsigned int)min_free_kbytes) * 7 / 6;

		if ((priv->pshare->rf_ft_var.mcst_min_rsvd_mem) && (priv->pshare->rf_ft_var.mcst_min_rsvd_mem < mcst_min_mem)) {
			printk("Default mcst_min_rsvd_mem (%u KB) is lower than 1.167 * kernel's min_free_kbytes (%u KB), reset mcst_min_rsvd_mem to %u KB\n",
					priv->pshare->rf_ft_var.mcst_min_rsvd_mem, mcst_min_mem, mcst_min_mem);
			priv->pshare->rf_ft_var.mcst_min_rsvd_mem = mcst_min_mem;
		}

#endif // CONFIG_RTL8192CD_MODULE
#endif // __KERNEL__
	...
	if (IS_VAP_INTERFACE(priv)) {
		if((priv->pmib->miscEntry.vap_enable)&&(OPMODE == WIFI_STATION_STATE)) {
            if(
    			...
    			rtk_mod_timer(&priv->ss_timer, jiffies + RTL_SECONDS_TO_JIFFIES(5));
            }else if(
#if defined(CUSTOMIZE_WLAN_IF_NAME)
            ...
            !strcmp(priv->dev->name,"wlan0-va2")||!strcmp(priv->dev->name,"wlan1-va2")
                ...
                STADEBUG("open ,start ss_timer\n");
    			rtk_mod_timer(&priv->ss_timer, jiffies + RTL_SECONDS_TO_JIFFIES(15));
            }
		}
	}else
#endif
...
//	if (IS_VXD_INTERFACE(priv) && !GET_ROOT_PRIV(priv)->pmib->miscEntry.func_off)
	if (IS_VXD_INTERFACE(priv))
    {
		if (OPMODE == WIFI_STATION_STATE) {
			...
			if (GET_ROOT(priv)->pmib->dot11DFSEntry.CAC_enable && (rtk_timer_pending(&GET_ROOT(priv)->ch_avail_chk_timer) ||
				...
				if ((GET_ROOT(priv)->pmib->dot11StationConfigEntry.dot11RegDomain == DOMAIN_ETSI) &&
					...
					{
						rtk_mod_timer(&priv->ss_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(1500));
					}
					else
						rtk_mod_timer(&priv->ss_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(5000) + CH_AVAIL_CHK_TO_CE);
#else
					rtk_mod_timer(&priv->ss_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(5000) + CH_AVAIL_CHK_TO_CE);
#endif
				else
					...
					if(priv->pmib->dot11StationConfigEntry.sc_enabled  && priv->simple_config_status>0)
					{
						...
						rtk_mod_timer(&priv->ss_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(5000) + CH_AVAIL_CHK_TO);
#else
					rtk_mod_timer(&priv->ss_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(5000) + CH_AVAIL_CHK_TO);
#endif
			}
...
#endif
            {
                ...
                {   /*if mesh is doing auto channel scanning, delay 15 sec*/
                    rtk_mod_timer(&priv->ss_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(15000));
                }
                else
#endif
                {
					...
					if(priv->pmib->dot11StationConfigEntry.sc_enabled && priv->simple_config_status>0)
					{
					    ...
					    rtk_mod_timer(&priv->ss_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(5000));
#else
				    rtk_mod_timer(&priv->ss_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(1500));
#endif
                }
	...
	if (IS_VXD_INTERFACE(priv) && (OPMODE & WIFI_ADHOC_STATE)) {
		init_vxd_ibss_beacon_timer(priv);
		//rtk_mod_timer(&priv->pshare->vxd_ibss_beacon, jiffies + RTL_MILISECONDS_TO_JIFFIES(5000));
	}
#endif // UNIVERSAL_REPEATER
#endif // RTK_NL80211
...
#endif
#if defined(CONFIG_VERIWAVE_CHECK) && defined(CONFIG_SPECIAL_ENV_TEST)
		priv->pshare->veriwave_sifs_bk_status = !!(RTL_R16(REG_TX_PTCL_CTRL) & BIT12);
#endif
	}
...
int  rtl8192cd_set_hwaddr(struct net_device *dev, void *addr)
{
	...
	for (i=0; i<NUM_WDS; i++)
		if (priv->wds_dev[i])
			eth_hw_addr_set(priv->wds_dev[i], p);
#endif
#ifdef CONFIG_RTK_MESH
	if(NUM_MESH > 0)
		if (priv->mesh_dev)
			eth_hw_addr_set(priv->mesh_dev, p);
#endif
...
//#ifdef PCIE_POWER_SAVING
	...
	if (OFFLOAD_ENABLE(priv)) {
		if (IS_ROOT_INTERFACE(priv))
			priv->pshare->fw_download_status = 0;
	}
#endif
...
static void MDL_DEVINIT set_mib_default(struct rtl8192cd_priv *priv)
{
	...
	priv->pmib->dot11dCountry.dot11CountryCodeToRegDomain = 1;
#endif
	...
	{
#ifdef DFS
		// Init timers for each DFS channel
		init_timer_for_each_DFS_channel(priv);
#endif
...
#if defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8822CE) || defined(CONFIG_WLAN_HAL_8812FE) || defined(CONFIG_WLAN_HAL_8814BE) || defined(CONFIG_WLAN_HAL_8192FE)
	if ((GET_CHIP_VER(priv) == VERSION_8814A) || (GET_CHIP_VER(priv) == VERSION_8822B) || (GET_CHIP_VER(priv) == VERSION_8822C) || (GET_CHIP_VER(priv) == VERSION_8812F) || (GET_CHIP_VER(priv) == VERSION_8814B) || (GET_CHIP_VER(priv) == VERSION_8192F)){
#if defined(CONFIG_USE_PCIE_SLOT_1) && defined(CONFIG_USE_PCIE_SLOT_0)
//=========================
//SLOT0=5G, wlan0=pcie0, wlan1=pcie1
#ifdef CONFIG_RTL_5G_SLOT_0
...
#elif defined(CONFIG_SLOT_0_RFE_TYPE_16)
			priv->pmib->dot11RFEntry.rfe_type = 16;
#elif defined(CONFIG_SLOT_0_RFE_TYPE_17)
			priv->pmib->dot11RFEntry.rfe_type = 17;
#elif defined(CONFIG_SLOT_0_RFE_TYPE_18)
		...
		if(priv->pshare->wlandev_idx == (1^WLANIDX))
		{
			...
			priv->pmib->dot11RFEntry.rfe_type = 11;
#elif defined(CONFIG_SLOT_1_RFE_TYPE_12)
			priv->pmib->dot11RFEntry.rfe_type = 12;
#elif defined(CONFIG_SLOT_1_RFE_TYPE_13)
			priv->pmib->dot11RFEntry.rfe_type = 13;
#elif defined(CONFIG_SLOT_1_RFE_TYPE_14)
			priv->pmib->dot11RFEntry.rfe_type = 14;
#elif defined(CONFIG_SLOT_1_RFE_TYPE_16)
			priv->pmib->dot11RFEntry.rfe_type = 16;
#elif defined(CONFIG_SLOT_1_RFE_TYPE_17)
			priv->pmib->dot11RFEntry.rfe_type = 17;
#elif defined(CONFIG_SLOT_1_RFE_TYPE_18)
			priv->pmib->dot11RFEntry.rfe_type = 18;
#else
			priv->pmib->dot11RFEntry.rfe_type = 0;
#endif
...
#if 0//defined(CONFIG_WLAN_HAL_8192FE)
	if (GET_CHIP_VER(priv)==VERSION_8192F) {
		priv->pmib->dot11RFEntry.rfe_type = 3;
	}
#endif
...
#if defined(CONFIG_WLAN_HAL)
		|| (IS_HAL_CHIP(priv) && IS_SUPPORT_TX_AMSDU(priv))
#endif
		) {
#ifndef CONFIG_RTL_MULTI_WIFI_MODULES
		AMSDU_ENABLE = 2;
...
#ifdef SW_TX_QUEUE
		priv->pshare->rf_ft_var.swqSEaggnum = 32;
		priv->pshare->rf_ft_var.swq_turbo_sta_th = 3;
		// priv->pshare->rf_ft_var.tri_time_rx = 16;
		priv->pshare->rf_ft_var.swq_aggnum = 64;
#endif
#ifdef CONFIG_SPECIAL_ENV_TEST
		priv->pshare->rf_ft_var.spec_latency_sta_num = 4;
		priv->pshare->rf_ft_var.spec_latency_swqSEaggnum = 2;
#endif
	}
...
#if defined(CONFIG_WLAN_HAL) && defined(SW_TXQ_ATF)
	if (!IS_HAL_CHIP(priv)) {
		priv->pshare->rf_ft_var.swq_atf_en = 0;
	}
#endif

#ifdef CONFIG_RTL8672
	// For MIPS platform, driver cannot rely on RX_BUFFER_GATHER
	// to handle 8k amsdu, since MIPS has slow speed on memory
	// processing. Hence, set MAX_RX_BUF_LEN = 8800 for 5G.
	if (RX_BUF_LEN < 7900)
		priv->pmib->dot11nConfigEntry.dot11nAMSDURecvMax = 0;
#elif defined(_CORTINA_)
	// ARM platform, on the other hand, has better performance on
	// handling rx buffer gather. Hence, set MAX_RX_BUF_LEN=3712
	// is enough for handling 8k amsdu for 5G.
	if (priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G)
		priv->pmib->dot11nConfigEntry.dot11nAMSDURecvMax = 1;
	else
		priv->pmib->dot11nConfigEntry.dot11nAMSDURecvMax = 0;
#endif
...
/* Low power consumption in idle mode */
...
#if defined(CTC_WMM_LATENCY)
	if ((priv->pshare->rf_ft_var.ctc_spec_wmm) &&
			(priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G)) {
		priv->pshare->rf_ft_var.txbuf_merge = 1;
	}
#endif
    ...
    if (GET_CHIP_VER(priv) == VERSION_8814B) {
		...
		priv->pshare->rf_ft_var.cochannel_enhance = 1;
	}
#endif
	...
	if (GET_CHIP_VER(priv) == VERSION_8814A) {
		...
		if (GET_CHIP_VER(priv) == VERSION_8814B) {
			...
			priv->pshare->rf_ft_var.amsdu_merge_default = priv->pshare->rf_ft_var.amsdu_merge;
#endif
		}
#endif
...
#if !defined(__LINUX_2_6_42__) || (defined(__LINUX_3_0__) && !defined(__LINUX_3_2__))
...
#endif
#ifdef __LINUX_3_7__
        .ndo_get_stats64          = rtl8192cd_get_stats64,
#else
...
#endif // defined(__LINUX_2_6__) && !defined(CONFIG_COMPAT_NET_DEV_OPS)
...
int InitWlanBuffer88XX(struct rtl8192cd_priv *priv)
{
	...
	_GET_HAL_DATA(priv)->rxBDNum = RX_Q_RXBD_NUM_IF(priv);
...
static __always_inline void disable_G5_rx_drop(unsigned long data)
{
...
DEFINE_TIMER_CALLBACK(disable_G5_rx_drop);

struct rtk_timer_list G5_rx_drop_timer;
void init_G5_rx_drop_timer(void) {
	rtk_timer_setup(&(G5_rx_drop_timer), disable_G5_rx_drop, 0UL, 0);
	printk("===========================================\n");
...
#endif
	struct rtl8192cd_priv *Adapter = priv;
        ...
        case VERSION_8814B:
	        ...
	        if(OFFLOAD_ENABLE(priv)){
	             ...
	             if(priv->pshare->rf_ft_var.manual_pe_enable){
	                h2d_trx_dma_size = H2D_DESC_8814B_DCOFLD_PEOFLD_DMA_SIZE_MAX + PAGE_SIZE /*alignment*/ + PAGE_SIZE /*FWDL*/;
	                _GET_HAL_DATA(priv)->rxBDNum = 0;
	                _GET_HAL_DATA(priv)->txBDNum = TOTAL_NUM_TXBD_NO_BCN_DCOFLD_PEOFLD_8814B;
	                dma_size_max = H2D_DESC_8814B_DCOFLD_PEOFLD_DMA_SIZE_MAX;
	             }else
#endif
	            {
...
#if defined(CONFIG_PE_ENABLE)
	        if(priv->pshare->rf_ft_var.manual_pe_enable){
	            rx_dma_size = DESC_DMA_RXSIZE_PAGE_PEOFLD_8814B;
	            tx_dma_size = DESC_DMA_TXSIZE_PAGE_PEOFLD_8814B;
	            _GET_HAL_DATA(priv)->rxBDNum = 0; //tmp
	            _GET_HAL_DATA(priv)->txBDNum = TOTAL_NUM_TXBD_NO_BCN_PEOFLD_8814B;
	        }else
#endif
	        {
...
/*cfg p2p cfg p2p*/
	...
	if (wdev->priv) {
		...
		if (vap_idx >= 0)
		{
			...
			if(vap_idx == (RTL8192CD_NUM_VWLAN - 1))
				snprintf(dev->name, sizeof(dev->name), "tmp.phy%s", wdev->priv->dev->name+4);
			else
...
#elif defined(CONFIG_USE_PCIE_SLOT_0) && defined(CONFIG_USE_PCIE_SLOT_1) && defined(CONFIG_NET_PCI)
		// Assign device name and ID when using PCI_BIOS
	#if (defined(CONFIG_BAND_2G_ON_WLAN0) && defined(CONFIG_RTL_5G_SLOT_0)) || \
		(defined(CONFIG_BAND_5G_ON_WLAN0) && defined(CONFIG_RTL_5G_SLOT_1))
		strncpy(dev->name, (wlan_index == 0 ? "wlan1" : "wlan0"), (sizeof(dev->name)-1));
	#else
		strncpy(dev->name, (wlan_index == 0 ? "wlan0" : "wlan1"), (sizeof(dev->name)-1));
	#endif
#else
		// Assign device name and let kernel determine device ID when using PCI_DIRECT
...
#ifdef __LINUX_2_6_25__
		regs = ioremap(pciaddr, pmem_len);
#else
...
#if !defined(NOT_RTK_BSP)
{
		...
		{
			unsigned int vendor_deivce_id = 0;
			unsigned long config_base;

			if(dev->base_addr==0xba000000)
			{
				vendor_deivce_id= *((volatile unsigned int *)(0xb8b30000));
			}
			else if(dev->base_addr==0xb9000000)
			{
				vendor_deivce_id= *((volatile unsigned int *)(0xb8b10000));
			}
			else
			{
				vendor_deivce_id= *((volatile unsigned int *)dev->base_addr);
			}
			DEBUG_INFO("   vendor_deivce_id=%x\n", vendor_deivce_id);
			if (vendor_deivce_id == ((unsigned int)((0x8193<<16)|PCI_VENDOR_ID_REALTEK)))
		...
		if( (vendor_id==0x10ec) && ((device_id==0x818c) || (device_id==0xf192)))
		{
			...
			pci_write_config_dword( pdev , 0x724  , 0x4868100); //For Noise isolation.
			pci_read_config_dword( pdev , 0x724  , &conf_add_724);
			DEBUG_INFO("conf_addr+0x724: %x\n", conf_add_724);
			// Patch End
...
#endif // CONFIG_NET_PCI
	{
        ...
        if (((priv->pshare->type>>TYPE_SHIFT) & TYPE_MASK) == TYPE_PCI_DIRECT)
        {
            ...
            if (BSP_PCIE0_D_CFG0 == wdev->conf_addr)
            {
                ...
                #elif defined(CONFIG_RTL_8198F)
                ///////need to add 98F PCIE init //yllin8814B
				#else
			...
			{
				unsigned int vendor_deivce_id, sub_vendor_deivce_id;
				unsigned long config_base;

				config_base = wdev->conf_addr;
				vendor_deivce_id = *((volatile unsigned int *)(config_base+0));
				sub_vendor_deivce_id = *((volatile unsigned int *)(config_base+0x2c));
				...
				if (
                    #if defined(CONFIG_RTL_92D_SUPPORT)
					(vendor_deivce_id != ((unsigned int)((0x8193<<16)|PCI_VENDOR_ID_REALTEK))) &&
                    #endif
					(vendor_deivce_id != ((unsigned int)((0x8191<<16)|PCI_VENDOR_ID_REALTEK))) &&
					(vendor_deivce_id != ((unsigned int)((0x8171<<16)|PCI_VENDOR_ID_REALTEK))) &&
					(vendor_deivce_id != ((unsigned int)((0x8178<<16)|PCI_VENDOR_ID_REALTEK))) &&
					(vendor_deivce_id != ((unsigned int)((0x8174<<16)|PCI_VENDOR_ID_REALTEK))) &&
					(vendor_deivce_id != ((unsigned int)((0x8176<<16)|PCI_VENDOR_ID_REALTEK)))
                    #ifdef CONFIG_RTL_88E_SUPPORT
					&& (vendor_deivce_id != ((unsigned int)((0x8179<<16)|PCI_VENDOR_ID_REALTEK)))
                    #endif
					#ifdef CONFIG_RTL_8723B_SUPPORT
					&& (vendor_deivce_id != ((unsigned int)((0xb723<<16)|PCI_VENDOR_ID_REALTEK)))
					#endif
                    #ifdef CONFIG_RTL_8812_SUPPORT
					&& (vendor_deivce_id != ((unsigned int)((0x8812<<16)|PCI_VENDOR_ID_REALTEK)))
                    #endif
                    #if defined(CONFIG_WLAN_HAL_8192EE)
					&& (vendor_deivce_id != ((unsigned int)((0x8170<<16)|PCI_VENDOR_ID_REALTEK)))
					&& (vendor_deivce_id != ((unsigned int)((0x818b<<16)|PCI_VENDOR_ID_REALTEK)))
                    #endif
                    #if defined(CONFIG_WLAN_HAL_8814AE)
                    && (vendor_deivce_id != ((unsigned int)((0x8813<<16)|PCI_VENDOR_ID_REALTEK)))
                    #endif
                    #if defined(CONFIG_WLAN_HAL_8822BE)
                    && (vendor_deivce_id != ((unsigned int)((0xb822<<16)|PCI_VENDOR_ID_REALTEK)))
                    && (sub_vendor_deivce_id != ((unsigned int)((0xb822<<16)|PCI_VENDOR_ID_REALTEK)))
                    #endif
                    #if defined(CONFIG_WLAN_HAL_8822CE)
                    && (vendor_deivce_id != ((unsigned int)((0xc822<<16)|PCI_VENDOR_ID_REALTEK)))
                    && (sub_vendor_deivce_id != ((unsigned int)((0xc822<<16)|PCI_VENDOR_ID_REALTEK)))
                    #endif
                    #if defined(CONFIG_WLAN_HAL_8812FE)
                    && (vendor_deivce_id != ((unsigned int)((0xf812<<16)|PCI_VENDOR_ID_REALTEK)))
                    && (sub_vendor_deivce_id != ((unsigned int)((0xf812<<16)|PCI_VENDOR_ID_REALTEK)))
                    #endif
					#if defined(CONFIG_WLAN_HAL_8821CE)
					&& (vendor_deivce_id != ((unsigned int)((0xb821<<16)|PCI_VENDOR_ID_REALTEK)))
					#endif
                    #if defined(CONFIG_WLAN_HAL_8814BE)
                    && (vendor_deivce_id != ((unsigned int)((0xb814<<16)|PCI_VENDOR_ID_REALTEK)))
                    #endif
					#if defined(CONFIG_WLAN_HAL_8814CR)
                    && (vendor_deivce_id != ((unsigned int)((0xc814<<16)|PCI_VENDOR_ID_REALTEK)))
                    #endif
#if defined(CONFIG_WLAN_HAL_8192FE)
                    && (vendor_deivce_id != ((unsigned int)((0x818c<<16)|PCI_VENDOR_ID_REALTEK)))
                    && (vendor_deivce_id != ((unsigned int)((0xf192<<16)|PCI_VENDOR_ID_REALTEK)))
					#endif
					) {
...
#endif
#ifdef CONFIG_RTL_88E_SUPPORT
                if (vendor_deivce_id == ((unsigned int)((0x8179<<16)|PCI_VENDOR_ID_REALTEK))) {
                    //					DEBUG_INFO("\n found 8188E !!! \n");
                    priv->pshare->version_id = VERSION_8188E;
                } else
#endif
#ifdef CONFIG_RTL_8812_SUPPORT
                if (vendor_deivce_id == ((unsigned int)((0x8812<<16)|PCI_VENDOR_ID_REALTEK)))
                {
                    DEBUG_INFO("\n found 8812 !!! \n");
                    priv->pshare->version_id = VERSION_8812E;
#ifdef DROP_RXPKT
...
#endif
#ifdef CONFIG_RTL_8723B_SUPPORT
				if (vendor_deivce_id == ((unsigned int)((0xb723<<16)|PCI_VENDOR_ID_REALTEK)))
				{
					DEBUG_INFO("\n found 8723B !!! \n");
					priv->pshare->version_id = VERSION_8723B;
				}
...
#endif
#if defined(CONFIG_WLAN_HAL_8192EE)
                if (vendor_deivce_id == ((unsigned int)((0x8170<<16)|PCI_VENDOR_ID_REALTEK))
                    || vendor_deivce_id == ((unsigned int)((0x818b<<16)|PCI_VENDOR_ID_REALTEK))) {
                    priv->pshare->version_id = VERSION_8192E;
#ifdef __OSK__
					priv->pmib->dot11RFEntry.phyBandSelect = PHY_BAND_2G;
...
#endif
#if defined(CONFIG_WLAN_HAL_8814AE)
                if (vendor_deivce_id == ((unsigned int)((0x8813<<16)|PCI_VENDOR_ID_REALTEK))) {
                    priv->pshare->version_id = VERSION_8814A;
                    DEBUG_INFO("\n found 8814A !!! \n");
					#if defined(CONFIG_OPENWRT_SDK)
...
#if defined(CONFIG_WLAN_HAL_8822BE)
                if ((vendor_deivce_id == ((unsigned int)((0xb822<<16)|PCI_VENDOR_ID_REALTEK))) || (sub_vendor_deivce_id == ((unsigned int)((0xb822<<16)|PCI_VENDOR_ID_REALTEK)))) {
                    priv->pshare->version_id = VERSION_8822B;
                    DEBUG_INFO("\n found 8822B !!! \n");
                    printk("\n found 8822B !!! \n");
                }
...
#endif
#if defined(CONFIG_WLAN_HAL_8822CE)
                if ((vendor_deivce_id == ((unsigned int)((0xc822<<16)|PCI_VENDOR_ID_REALTEK))) || (sub_vendor_deivce_id == ((unsigned int)((0xc822<<16)|PCI_VENDOR_ID_REALTEK)))) {
                    priv->pshare->version_id = VERSION_8822C;
                    DEBUG_INFO("\n found 8822C !!! \n");
                    printk("\n found 8822C !!! \n");
                }
...
#endif
#if defined(CONFIG_WLAN_HAL_8812FE)
                if ((vendor_deivce_id == ((unsigned int)((0xf812<<16)|PCI_VENDOR_ID_REALTEK))) || (sub_vendor_deivce_id == ((unsigned int)((0xf812<<16)|PCI_VENDOR_ID_REALTEK)))) {
                    priv->pshare->version_id = VERSION_8812F;
                    DEBUG_INFO("\n found 8812F !!! \n");
                    printk("\n found 8812F !!! \n");
                }
...
#endif
#if defined(CONFIG_WLAN_HAL_8821CE)
				if (vendor_deivce_id == ((unsigned int)((0xb821<<16)|PCI_VENDOR_ID_REALTEK))) {
									priv->pshare->version_id = VERSION_8821C;
									DEBUG_INFO("\n found 8821C !!! \n");
									printk("\n found 8821C !!! \n");
				}
...
#endif
#if defined(CONFIG_WLAN_HAL_8814BE)
                if (vendor_deivce_id == ((unsigned int)((0xb814<<16)|PCI_VENDOR_ID_REALTEK))) {
                    priv->pshare->version_id = VERSION_8814B;
                    DEBUG_INFO("\n found 8814B !!! \n");
                    printk("\n found 8814B !!! \n");
                }
...
#endif
#if defined(CONFIG_WLAN_HAL_8814CR)
                if (vendor_deivce_id == ((unsigned int)((0xc814<<16)|PCI_VENDOR_ID_REALTEK))) {
                    priv->pshare->version_id = VERSION_8814B;
                    DEBUG_INFO("\n found 8814C !!! \n");
                    printk("\n found 8814C !!! \n");
                }
...
#if defined(CONFIG_WLAN_HAL_8192FE)
				if (vendor_deivce_id == ((unsigned int)((0x818c<<16)|PCI_VENDOR_ID_REALTEK)) ||
					 vendor_deivce_id == ((unsigned int)((0xf192<<16)|PCI_VENDOR_ID_REALTEK))) {
					priv->pshare->version_id = VERSION_8192F;
					DEBUG_INFO("\n found 8192F !!!\n");
				}
		...
		if (GET_CHIP_VER(priv)==VERSION_8821C) {
		   ...
		   if (GET_CHIP_VER(priv)==VERSION_8192F) {
			...
			REG32(0xb8b10078) = 0x162800;
	                ...
	                // Patch for 92F Anti-Nosie from Environment.
		        // REG32(0xb8b10724)=0x4868100; //For Noise isolation.
		        REG32(wdev->conf_addr+0x724) = 0x4868100; //For Noise isolation.
		        DEBUG_INFO("wdev->conf_addr+0x724: %x, val: %x\n", wdev->conf_addr+0x724, REG32(wdev->conf_addr+0x724));
		        // Patch End
#endif
			   Wlan_HAL_Link(priv);
		   }
...
register_driver:
...
#endif
#else // defined(__LINUX_2_6__) && !defined(CONFIG_COMPAT_NET_DEV_OPS)
  #if defined(CONFIG_RTL_TRIBAND_SUPPORT)
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
...
#if (defined(CONFIG_RTL8672) || defined(CONFIG_LUNA_G3_SERIES)) && !defined(__OSK__)
	dev->priv_flags = RTK_IFF_DOMAIN_WLAN;
#ifdef __LINUX_5_10__
	...
	if(vap_idx < 0) {
		...
		{
			priv->dev = dev;
			{	/* 6.x: netdev->dev_addr is const; compose then set */
				unsigned char __a[6];
				memcpy(__a, GET_ROOT(priv)->pmib->dot11Bss.bssid, 6);
				__a[4] = __a[4] + 1;
				eth_hw_addr_set(dev, __a);
			}
...
#if defined(CONFIG_RTK_L34_ENABLE) && defined(CONFIG_RG_WLAN_HWNAT_ACCELERATION) && !defined(CONFIG_ARCH_LUNA_SLAVE)
    int wlanIdx, devIdx;
#if defined(RTK_NL80211)
...
if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
	...
	dev->rtk_priv_flags = RTK_IFF_DOMAIN_WLAN;
...
#ifdef AUTH_SAE
		panic_printk("[%s][%d] init sae_peer_table \n ", __FUNCTION__, __LINE__);
		...
		if(rc == HASH_TABLE_FAILED)
		{
			printk("init_hash_table \"sae_peer_table\" error!!\n");
		}
		...
		if(rc == HASH_TABLE_FAILED)
		{
			printk("init_hash_table \"sae_blacklist_table\" error!!\n");
		}
		...
		//printk("\n\n\n\n\nin_atomic():%d, descbuf:%d, amsdu:%d, IS_PCIBIOS_TYPE:%d\n\n\n\n\n", in_atomic(), DESC_DMA_PAGE_SIZE_MAX_HAL_IF(priv), DESC_DMA_PAGE_SIZE_HAL_FOR_AMSDU, IS_PCIBIOS_TYPE);
#ifdef CONFIG_PCI_HCI
		if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
#ifdef  CONFIG_WLAN_HAL
			if (IS_HAL_CHIP(priv)) {
#if defined(CONFIG_RTL_8198F) || defined(CONFIG_WLAN_HAL_8814BE) //98F platfrorm (2 chip: 98F wifi+8814B wifi) or 8814B wifi (1 chip)
...
//#ifdef CONFIG_RTL_92D_DMDP
		if_priv[priv->pshare->wlandev_idx] = priv;
//#endif

#if defined(STA_CONTROL) && STA_CONTROL_ALGO == STA_CONTROL_ALGO3	//20170103
		if_priv_stactrl[priv->pshare->wlandev_idx] = priv;
#endif
...
#endif
#ifdef SMP_SKB_RECYCLE_OFFLOAD
		priv->pshare->skb_recycle_queue = &g_skb_recycle_queue;
		tasklet_init(&priv->pshare->skb_recycle_tasklet, rtl8192cd_skb_recycle, (unsigned long)priv);

		memset(&priv->pshare->skb_recycle_csd, 0, sizeof(rtk_call_single_data_t));
		priv->pshare->skb_recycle_csd.func = smp_assign_tasklet;
		priv->pshare->skb_recycle_csd.info = (void *)&priv->pshare->skb_recycle_tasklet;
#endif
		INIT_WORK(&priv->reset_hangup_work, reset_hangup_work);
	}
...
void rtl8192cd_init_one_cfg80211(struct rtknl *rtk)
{
	...
	printk("=>rtl8192cd_init_one");
	rtl8192cd_init_one(NULL, NULL, &wlan_device[wlan_index], rtk->num_vif);

#ifdef PRE_ALLOC_STA_BUF
	// Only pre-allocate aid_obj (sta_info) on root interface
	if (rtk->num_vif == 0) {
		if (prealloc_aid_obj(wlan_device[wlan_index].priv))
			printk("Prealloc aid_obj fail!\n");
	}
#endif
}
...
void rtl8192cd_deinit_one(struct rtl8192cd_priv *priv)
{
	...
	cancel_work_sync(&priv->reset_hangup_work);
...
#ifdef __LINUX_5_6__
struct proc_ops wlan_custom_Passthru_proc_ops = {
...
#else  // !(__LINUX_5_6__)
struct file_operations wlan_custom_Passthru_proc_ops = {
...
#ifdef CONFIG_RTL_PROC_NEW
	resPassThruWlan = proc_create_data("custom_Passthru_wlan", 0, NULL, &wlan_custom_Passthru_proc_ops, NULL);
...
#ifdef PRE_ALLOC_STA_BUF
	// Only pre-allocate aid_obj (sta_info) on root interface
	ret = prealloc_aid_obj(wlan_device[wlan_index].priv);
	if (ret) {
		printk("Prealloc aid_obj fail!\n");
		goto error;
	}
...
static void rtk_dev_shutdown(struct pci_dev *pdev)
{
	force_stop_wlan_hw();
}
...
static int realtek_register_driver(int index)
{
	int rc = 0;

	_DEBUG_INFO("wlan_index:%d %d, %d, 0x%lx, 0x%lx, %d\n", index,
			(((wlan_device[index].type >> TYPE_SHIFT) & TYPE_MASK) == TYPE_PCI_BIOS),
			wlan_device[index].type, wlan_device[index].base_addr,wlan_device[index].conf_addr,
			wlan_device[index].irq);

	if ((((wlan_device[index].type >> TYPE_SHIFT) & TYPE_MASK) == TYPE_PCI_BIOS) ||
			(((wlan_device[index].type >> TYPE_SHIFT) & TYPE_MASK) == TYPE_PCI_BIOS_DT)) {
...
#endif
#endif // defined(CONFIG_NET_PCI) || defined(CONFIG_RTL_8198F)
#endif // defined(CONFIG_PCI_HCI) && !defined(CONFIG_RTL_TRIBAND_SUPPORT)

#ifdef CONFIG_USB_HCI
			if (wlan_device[index].base_addr == ((0x8191 << 16) | USB_VENDER_ID_REALTEK)) {
...
#ifdef CONFIG_SDIO_HCI
			if (wlan_device[index].base_addr == ((0x8179 << 16) | 0x024c)) {
...
#ifdef __DRAYTEK_OS__
		wlan_device[index].base_addr = base_addr;
		wlan_device[index].type = (TYPE_PCI_DIRECT<<TYPE_SHIFT);
#endif

#ifdef CONFIG_WLAN_HAL_8198F
		rc = platform_driver_register(&rtk_wlan_8198F_driver);
		printk("[%s(%d)] wlan_index = %x rc = %x !!!!!\n", __func__, __LINE__, index, rc);
		//return rc;
#else // !(CONFIG_WLAN_HAL_8198F)
		rc = rtl8192cd_init_one(NULL, NULL, &wlan_device[index], -1);
		if (rc)
		...
		// Only pre-allocate aid_obj (sta_info) on root interface
		if (rc == 0) {
			if (rc = prealloc_aid_obj(wlan_device[index].priv)) {
				printk("Prealloc aid_obj fail!\n");
			}
		}
#endif
		...
		if (rc == 0) {
			for (i = 0; i < RTL8192CD_NUM_VWLAN; i++) {
				rc = rtl8192cd_init_one(NULL, NULL, &wlan_device[index], i);
				if (rc != 0) {
		...
		if (rc == 0) {
			...
			struct rtl8192cd_priv *priv = wlan_device[index].priv;
...
#endif // (CONFIG_RTL8672 && CONFIG_RTL_92C_SUPPORT)
	}
...
#if defined(CONFIG_RTK_BOARDID)
	if (!(BOARD_WIFI_TYPE(boot_board_id) & BOARD_WIFI_WIFI5)) {
		printk("boot_board_id without WiFi5, don't initial rtl8192cd Driver\n");
		return 0;
	}
#endif

#if defined(CONFIG_RTL9607C)
	printk("%s(%d): SIZE=%d %x/%x/%d\n",__func__,__LINE__,ARRAY_SIZE(wlan_device),
			wlan_device[0].conf_addr, wlan_device[0].base_addr, wlan_device[0].irq);
...
#endif
	{
		wlan_device[0].conf_addr = BSP_PCIE1_D_CFG0;
		wlan_device[0].base_addr = BSP_PCIE1_D_MEM;
		wlan_device[0].irq = BSP_PCIE1_IRQ;
	}
	printk("%s(%d): SIZE=%d %x/%x/%d\n",__func__,__LINE__,ARRAY_SIZE(wlan_device),
			wlan_device[0].conf_addr, wlan_device[0].base_addr, wlan_device[0].irq);
#endif
...
//------------------------------------
...
#ifndef GREEN_HILL
#if defined(SVN_REV)
	panic_printk("%s - version %d.%d.%d.%d(%s)(SVN:%s)\n", DRV_NAME, DRV_VERSION_H, DRV_VERSION_L, DRV_VERSION_SUBL, DRV_VERSION_SUBSUBL, DRV_RELDATE, SVN_REV);
#elif defined(GIT_SHA)
	panic_printk("%s - version %d.%d.%d.%d(%s)(Git SHA-1 ID:%s)\n", DRV_NAME, DRV_VERSION_H, DRV_VERSION_L, DRV_VERSION_SUBL, DRV_VERSION_SUBSUBL, DRV_RELDATE, GIT_SHA);
#else
	printk("%s driver version %d.%d.%d.%d(%s)\n", DRV_NAME, DRV_VERSION_H, DRV_VERSION_L, DRV_VERSION_SUBL, DRV_VERSION_SUBSUBL, DRV_RELDATE);
#endif
#endif
...
#ifdef __KERNEL__
#if (defined(CONFIG_NET_PCI) && !defined(CONFIG_USB_HCI) && !defined(CONFIG_SDIO_HCI) && !defined(CONFIG_RTL_TRIBAND_SUPPORT))
	if (wlan_index < ARRAY_SIZE(wlan_device)) {
		rc = realtek_register_driver(wlan_index);
	}
#else
	for (; wlan_index < ARRAY_SIZE(wlan_device); wlan_index++) {
		rc = realtek_register_driver(wlan_index);
	}
#endif
#else // __OSK__
	...
	if (rc == 0)
		printk("[%s(%d)] Register %d interface(s) sucessfully\n", __func__, __LINE__, wlan_index);
	else
		printk("[%s(%d)] Register interface failed\n", __func__, __LINE__, wlan_index);
...
#ifdef SMP_SKB_RECYCLE_OFFLOAD
	skb_queue_head_init(&g_skb_recycle_queue);
#endif
...
//------------------------------------
	...
	rtk_netlink_exit();
#elif defined(CONFIG_RTK_WLAN_MANAGER)
	rtk_wlan_manager_nl_deinit();
#endif

#ifdef SMP_SKB_RECYCLE_OFFLOAD
	skb_queue_recycle(&g_skb_recycle_queue);
#endif
}
#else // not __KERNEL__
...
struct net_device *get_shortcut_dev(unsigned char *da)
{
	...
	bool veriwave = ((if_priv[0] != NULL) ? is_veriwave_testing(if_priv[0]) : false) ||
		((if_priv[1] != NULL) ? is_veriwave_testing(if_priv[1]) : false);
#endif
	...
	for (i=0; i<sizeof(wlan_device)/sizeof(struct _device_info_); i++) {
		if (wlan_device[i].priv && (wlan_device[i].priv != priv)) {
			if (wlan_device[i].priv->pmib->dot11OperationEntry.opmode & WIFI_AP_STATE
				...
				snprintf((char *)tmpbuf, sizeof(tmpbuf), "%02x%02x%02x%02x%02x%02xforce",
...
//	int temp;
...
#endif
	{
		if (wlan_device[i].priv) {
			...
			/* avoid triggering recovery from check_hangup after stop hw */
			if (timer_pending(&priv->expire_timer.timer))
				timer_delete_sync(&priv->expire_timer.timer);

...
void init_band_trans_neighbor_report(struct rtl8192cd_priv *priv)
{
	...
	neighbor_report->bssinfo.field.ap_reachability = 3;
	neighbor_report->bssinfo.field.security = 1;
	neighbor_report->bssinfo.field.key_scope = 1;
	...
	// Very High Throughput or High Throughput
	if (priv->pmib->dot11BssType.net_work_type & WIRELESS_11AC)
		neighbor_report->bssinfo.field.very_high_tp = 1;
...
unsigned char * construct_temp_neighbor_report_ie(struct rtl8192cd_priv *priv, unsigned char *pbuf, unsigned int *frlen, unsigned char * hwaddr, unsigned char channel)
{
    struct dot11k_neighbor_report report;

	memset(&report, 0, sizeof(struct dot11k_neighbor_report));
    memcpy(report.bssid, hwaddr, MACADDRLEN);
    report.bssinfo.value = 0x000E;
    report.channel = channel;
    report.op_class = rm_get_op_class(priv, channel);
    report.phytype = 0;

    report.bssinfo.field.cap_spectrum = 1;
    report.bssinfo.field.cap_qos = 1;
    report.bssinfo.field.cap_apsd = 1;
    report.bssinfo.field.cap_rm = 1;      
    report.bssinfo.field.cap_delay_ba = 1;
    report.bssinfo.field.cap_im_ba = 1;
#ifdef CONFIG_IEEE80211V
    if(WNM_ENABLE) {
        report.subelemnt.subelement_id = _WNM_BSS_TRANS_CANDIDATE_PREFRENCE_;
        report.subelemnt.len = 1;
        report.subelemnt.preference = 255;
    }
#endif
    report.bssinfo.value = cpu_to_le32(report.bssinfo.value);
    pbuf = set_ie(pbuf, _NEIGHBOR_REPORT_IE_, sizeof(struct dot11k_neighbor_report), (unsigned char*)&report, frlen);
        ...
        unsigned char dialog_token, unsigned char *ssid, unsigned char ssid_len)
{
	...
	if (priv->stactrl.stactrl_priv_sc != NULL) {
		struct rtl8192cd_priv *priv_sc = priv->stactrl.stactrl_priv_sc;
		if (!ssid || (ssid_len == SSID_LEN && !memcmp(ssid, SSID, SSID_LEN)))
    ...
    for(i = 0; i < MAX_NEIGHBOR_REPORT; i++) {
        ...
        if(frlen + neighbor_size > MAX_REPORT_FRAME_SIZE)
            break;
        if ((priv->rm_neighbor_report[i].channel == (GET_MIB(priv))->dot11RFEntry.dot11channel) &&
            !memcmp(priv->rm_neighbor_report[i].bssid, (GET_MIB(priv))->dot11StationConfigEntry.dot11Bssid, MACADDRLEN))
            continue;
...
int rm_construct_neighbor_report(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo, unsigned char channel)
{
    ...
    if(i == MAX_NEIGHBOR_REPORT)  /*not found and no empty slot, find the oldest neighbor and replace it*/
    {
        for(i = 0; i < MAX_NEIGHBOR_REPORT; i++)
        {
            ...
            {
                if((oldest_idx < 0) || (oldest_aging < priv->rm_neighbor_info[i].aging)) {
...
#endif
{
    ...
    {        
        ...
        if(command == 1)   /*add*/
        {
            ...
            if ((report.channel == (GET_MIB(priv))->dot11RFEntry.dot11channel) &&
                !memcmp(report.bssid, (GET_MIB(priv))->dot11StationConfigEntry.dot11Bssid, MACADDRLEN)) {
                error_code = 3;
                goto end;
            }
    ...
    if(error_code == 1)
		...
		DEBUG_INFO("\nWarning: Skip the command because the MAC address and channel is the same.\n");
    return count;
#endif
}
#endif

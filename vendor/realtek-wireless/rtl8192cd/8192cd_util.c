...
#include "dual_cpu.h"
#endif
#if defined(USE_PID_NOTIFY) && defined(__LINUX_2_6_27__)
...
/********************/
    ...
    if (BW == CHANNEL_WIDTH_80 && (WirelessMode & WIRELESS_MODE_AC_5G))
		bCurTxBW80MHz = TRUE;
	...
	if (priv->pshare->WlanSupportAbility & WLAN_BEAMFORMING_SUPPORT){
		...
		#ifdef RTK_AC_SUPPORT		
				if ( priv->pmib->dot11RFEntry.txbfer == 1 && pstat->vht_cap_len && (cpu_to_le32(pstat->vht_cap_buf.vht_cap_info) & (BIT(SU_BFEE_S))))
				pstat->cmn_info.bf_info.vht_beamform_cap |= BEAMFORMING_VHT_BEAMFORMEE_ENABLE;
				if ( priv->pmib->dot11RFEntry.txbfee == 1 && pstat->vht_cap_len && (cpu_to_le32(pstat->vht_cap_buf.vht_cap_info) & (BIT(SU_BFER_S))))
...
void init_stainfo(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	INIT_LIST_HEAD(&pstat->delsta_list);
...
#ifdef DOT11K
	rm_init_sta_beacon_expire_timer(pstat);
#endif
	...
	pstat->skipFirstTkipMic = pmib->dot11StationConfigEntry.skipFirstTkipMic;
...
void free_sta_skb(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	for (i=0; i<8; i++) {
		rc_entry = &pstat->rc_entry[i];
		rc_entry->start_rcv = FALSE;
		for (j=0; j<RC_ENTRY_NUM; j++) {
			if (rc_entry->packet_q[j]) {
				pskb = rc_entry->packet_q[j];
				#if defined(SUPPORT_RX_AMSDU_AMPDU) && defined(RX_BUFFER_GATHER_REORDER)
				pfrinfo = rc_entry->frinfo_q[j];
				if (pfrinfo && pfrinfo->gather_flag == GATHER_FIRST) {
					flush_rx_list_inq(priv,pfrinfo);
					rc_entry->frinfo_q[j] = NULL;
				} 
				#endif
				rtl_kfree_skb(priv, pskb, _SKB_RX_);
				rc_entry->packet_q[j] = NULL;
			}
		}
		if (rc_entry->rc_timer) {
...
void release_stainfo(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
...
#ifdef WDS
		pstat->state &= (WIFI_WDS | WIFI_DELETE_STATE);
#else
    ...
    if (rtk_timer_pending(&pstat->rm_timer))
        rtk_del_timer_sync(&pstat->rm_timer);
...
#ifdef INCLUDE_WPA_PSK
	if (rtk_timer_pending(&pstat->wpa_sta_info->resendTimer)) {
		rtk_del_timer(&pstat->wpa_sta_info->resendTimer);
	}
#endif
...
int del_station(struct rtl8192cd_priv *priv, struct stat_info *pstat, int send_disconnect, int reason)
{
	...
	// Clear assoc in sta_cap_entry
	if (priv->pmib->dot11RFEntry.cmcc_probe_collect_enable)
		update_sta_cap_table_due_to_disconnect(priv, pstat);
#endif
...
int	free_stainfo(struct rtl8192cd_priv *priv, struct stat_info *pstat)
{
	...
	wapiAssert(pstat->wapiInfo==NULL);
#endif
	DEBUG_ERR("pstat can not be freed (%d, %02X%02X%02X%02X%02X%02X)\n", i, 
	    pstat->cmn_info.mac_addr[0], pstat->cmn_info.mac_addr[1], pstat->cmn_info.mac_addr[2],
		pstat->cmn_info.mac_addr[3], pstat->cmn_info.mac_addr[4], pstat->cmn_info.mac_addr[5]);
	return	FAIL;
}
...
unsigned short get_80211frameVlanID(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct rx_frinfo *pfrinfo)
{
...
static struct channel_list reg_channel_5g_full_band[] = {
...
#ifdef ETSI_BAND4_SUPPORT
	/* ETSI */		{{36,40,44,48,52,56,60,64,100,104,108,112,116,120,124,128,132,136,140,149,153,157,161,165},24},
#else
	...
	/* RUSSIAN */	{{36,40,44,48,52,56,60,64,132,136,140,144,149,153,157,161,165},17},
	...
	/* MX */		{{52,56,60,64,100,104,108,112,116,132,136,140,144,149,153,157,161,165},18},
};
...
int is_available_channel(struct rtl8192cd_priv *priv, unsigned char channel) {
	...
	if(priv->pmib->dot11DFSEntry.disable_DFS == 1) {
		if((channel >= 52 && channel <= dfs_channel))
			return 0;
	}
#endif	
	//CH144 cannot be used if not support 11ac 
	if(channel == 144 && !(priv->pmib->dot11BssType.net_work_type & WIRELESS_11AC))
		return 0;
...
void cnt_assoc_num(struct rtl8192cd_priv *priv, struct stat_info *pstat, int act, char *func)
{
	...
	if (act == INCREASE) {
		if (priv->assoc_num <= NUM_STAT) {
			...
			{
				if(priv->pshare->veriwave_sta_num == 0 && priv->pshare->rf_ft_var.veriwave_interference) {
			...
			if (pstat->IOTPeer == HT_IOT_PEER_APPLE && pstat->ht_cap_buf.support_mcs[2] && !pstat->ht_cap_buf.support_mcs[3])
			{
				priv->pshare->macbook_sta_num++;  
			}

			if (pstat->IOTPeer == HT_IOT_PEER_BROADCOM && pstat->ht_cap_buf.support_mcs[3] && !pstat->ht_cap_buf.support_mcs[4])
			{
				priv->pshare->broadcom_4T4R_sta_num++;
			}

			if (pstat->IOTPeer == HT_IOT_PEER_INTEL)
				priv->pshare->intel_sta_num++;
			...
			if (IS_HAL_CHIP(priv)) {
				...
				DEBUG_WARN("[%s][%s][%d] set MACID %d\n", func, priv->dev->name, pstat->cmn_info.aid, REMAP_AID(pstat));
			}
#endif
	...
	if(priv->pshare->rf_ft_var.manual_pe_enable){
		...
		rtl_ipc_tx_handler(CMD_ADD_STA,&stat_pe,GET_STAT_INFO_PE_SYNC_SIZE(stat_pe));
					...
					{
						if (get_sta_vht_mimo_mode(pstat) > RF_2T2R)
						{	
							setSTABitMap(&priv->pshare->ant_Bitmap, REMAP_AID(pstat));
							if (!priv->pshare->ant_flag)
							{
		...
		if (priv->assoc_num > 0) {
			...
			if (pstat->ht_cap_len) {
				...
				clearSTABitMap(&priv->switch_20_sta, pstat->cmn_info.aid);
#if defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL) || defined(CONFIG_RTL_8723B_SUPPORT)
	...
	if(priv->pshare->rf_ft_var.manual_pe_enable){
		...
		rtl_ipc_tx_handler(CMD_DEL_STA,&stat_pe,GET_STAT_INFO_PE_SYNC_SIZE(stat_pe));
				...
				{
					if (get_sta_vht_mimo_mode(pstat) > RF_2T2R)				
						clearSTABitMap(&priv->pshare->ant_Bitmap, REMAP_AID(pstat));
				}	
#endif
#endif
		} else {
...
void update_all_wifi_txrx_pkts(struct rtl8192cd_priv *rootPriv)
{
	...
	unsigned long long rx_pkt=0, tx_pkt=0;
...
void update_wifi_allitf_txrx_stats(struct rtl8192cd_priv *rootPriv)
{
	...
	rx_pkt = RX_PACKETS_COUNTER;
	tx_pkt = TX_PACKETS_COUNTER;
	rx_bytes = (unsigned long long)priv->net_stats.rx_bytes;
	tx_bytes = (unsigned long long)priv->net_stats.tx_bytes;  
...
#ifdef GBWC
static __always_inline void rtl8192cd_GBWC_timer(unsigned long task_priv)
{
...
DEFINE_TIMER_CALLBACK(rtl8192cd_GBWC_timer);

void init_GBWC_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->GBWC_timer), rtl8192cd_GBWC_timer, (unsigned long)priv, 0);
}
#endif  // GBWC

#ifdef SBWC
static __always_inline void rtl8192cd_SBWC_timer(unsigned long task_priv)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;
	struct sk_buff *pskb;
	SBWC_MODE mode;
...
DEFINE_TIMER_CALLBACK(rtl8192cd_SBWC_timer);

void init_SBWC_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->SBWC_timer), rtl8192cd_SBWC_timer, (unsigned long)priv, 0);
}
#endif  // SBWC

#ifdef RTK_STA_BWC
static __always_inline void rtl8192cd_sta_bwc_timer(unsigned long task_priv)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;
	struct stat_info	*pstat;
#if !defined(RTK_STA_BWC_DIRECT_XMIT)
...
DEFINE_TIMER_CALLBACK(rtl8192cd_sta_bwc_timer);

void init_sta_bwc_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->sta_bwc_timer), rtl8192cd_sta_bwc_timer, (unsigned long)priv, 0);
}
#endif  // RTK_STA_BWC
...
unsigned char fw_was_full(struct rtl8192cd_priv *priv)
{
	...
	while (plist != phead) {
		pstat = list_entry(plist, struct stat_info, asoc_list);
		plist = plist->next;
		if (pstat->cmn_info.ra_info.disable_ra && pstat->expire_to)
		{
			is_full = 1;
			goto exit;
		}
...
static __always_inline void TxPktBuf_AgingTimer(unsigned long task_priv)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;
	if (!(priv->drv_state & DRV_STATE_OPEN))
		return;
...
//					panic_printk("%s, Free %d pkts of MAC ID:%d\n", __FUNCTION__, Q_pkt[minIdx], Q_aid[minIdx]);
	...
	if(priv->pshare->rf_ft_var.msta_refine&1 )
		rtk_mod_timer(&priv->pshare->PktAging_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(10));
}
DEFINE_TIMER_CALLBACK(TxPktBuf_AgingTimer);

void init_PktAging_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->pshare->PktAging_timer), TxPktBuf_AgingTimer, (unsigned long)priv, 0);
}
#endif  // MULTI_STA_REFINE

#ifdef CONFIG_PCI_HCI
static __always_inline void add_ps_timer(unsigned long task_priv)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;
	struct stat_info *pstat = NULL;
	...
	if (set_timer)
		rtk_mod_timer(&priv->add_ps_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(50));	// 50 ms
}
DEFINE_TIMER_CALLBACK(add_ps_timer);

void init_add_ps_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->add_ps_timer), add_ps_timer, (unsigned long)priv, 0);
}
...
void checkBandwidth(struct rtl8192cd_priv *priv)
{
    ...
    if (rtk_timer_pending(&priv->ss_timer) || priv->ss_req_ongoing)
        return;
...
#if defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_92C_SUPPORT)
#ifdef CONFIG_PCI_HCI
static __always_inline void add_RATid_timer(unsigned long task_priv)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;
	struct stat_info *pstat = NULL;
	unsigned int set_timer = 0;
	...
	if (set_timer)
		rtk_mod_timer(&priv->add_RATid_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(50));	// 50 ms
}
DEFINE_TIMER_CALLBACK(add_RATid_timer);

void init_add_RATid_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->add_RATid_timer), add_RATid_timer, (unsigned long)priv, 0);
}
...
#endif // CONFIG_PCI_HCI
#endif // defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_92C_SUPPORT)
...
void ap_offload_deactivate(struct rtl8192cd_priv *priv, int reason)
{
	...
	if (!priv->pshare->offload_prohibited) {
		...
		rtk_del_timer(&priv->pshare->ps_timer);
		...
		if (pBackup->sta[i]) {
			...
			if (!SWCRYPTO && pstat->dot11KeyMapping.keyInCam == TRUE) {
				...
				if (pstat->dot11KeyMapping.dot11Privacy) {
					if (pstat->dot11KeyMapping.dot11EncryptKey.dot11TTKeyLen + pstat->dot11KeyMapping.dot11EncryptKey.dot11TMicKeyLen > sizeof(key_combo)) {
						printk("[%s] dot11KeyMapping dot11TTKeyLen(%d)+dot11TMicKeyLen(%d) > sizeof(key_combo)\n",
						__func__, pstat->dot11KeyMapping.dot11EncryptKey.dot11TTKeyLen, pstat->dot11KeyMapping.dot11EncryptKey.dot11TMicKeyLen);
						BUG_ON(1);
					}
					memcpy(key_combo,
...
#endif // CONFIG_RTL_HW_WAPI_SUPPORT
		{
			if (priv->pmib->dot11GroupKeysTable.dot11EncryptKey.dot11TTKeyLen + priv->pmib->dot11GroupKeysTable.dot11EncryptKey.dot11TMicKeyLen > sizeof(key_combo)) {
				printk("[%s] dot11GroupKeysTable dot11TTKeyLen(%d)+dot11TMicKeyLen(%d) > sizeof(key_combo)\n",
				__func__, priv->pmib->dot11GroupKeysTable.dot11EncryptKey.dot11TTKeyLen, priv->pmib->dot11GroupKeysTable.dot11EncryptKey.dot11TMicKeyLen);
				BUG_ON(1);
			}
			memcpy(key_combo,
...
void merge_pool(struct rtl8192cd_priv *priv)
{
	...
	while (1) {
		...
		buf = get_priv_skb_buf(if_priv[next_idx]);
		if (buf == NULL)
			break;
...
void split_pool(struct rtl8192cd_priv *priv)
{
	...
	while (1) {		
		...
		priv_buf = (struct priv_skb_buf *)(((unsigned long)buf) - offset);
		priv_buf->root_priv = if_priv[next_idx];
		release_buf_to_poll(if_priv[next_idx],
...
void update_RAMask_to_FW(struct rtl8192cd_priv *priv, int forceUpdate)
{
	...
	while (pstat) {
		if (forceUpdate || (!pstat->cmn_info.bw_mode && pstat->tx_bw)) {
#if defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_RTL_8723B_SUPPORT)
			if (GET_CHIP_VER(priv) == VERSION_8812E || GET_CHIP_VER(priv) == VERSION_8723B) {
				UpdateHalRAMask8812(priv, pstat, 3);
			}
...
void txrpt_handler_8812(struct rtl8192cd_priv *priv, struct tx_rpt *report, struct stat_info	*pstat)
{
    ...
    if(!pstat->cmn_info.ra_info.disable_ra)
	{
		...
		// ACK Duration: SIFS + ACK (12B) + SIFS.
		// 	CCK: 10 + 12*8/1 + 10 = 116.
		// 	2G: 10 + 12*8/24 + 10 = 24.
		// 	5G: 16 + 12*8/24 + 16 = 36.
		// ACK Penalty = ACK Duration * data rate / 8 (Bytes)
		if (is_CCK_rate(pstat->current_tx_rate)) {
			pstat->swq.atf_ack_penalty = 116 * pstat->swq.atf_txrate / 8;
		} else if (priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G) {
			pstat->swq.atf_ack_penalty = 24 * pstat->swq.atf_txrate / 8;
		} else {
			pstat->swq.atf_ack_penalty = 36 * pstat->swq.atf_txrate / 8;
		}
#endif /* SW_TXQ_ATF */
	}
}
...
#ifdef CTC_WIFI_DIAG
	if ((report->txfail != 0) && (pstat))
		ctcwifi_assoc_err(priv, pstat->cmn_info.mac_addr, "WiFi frames losing [tx_fail increases]\n");
#endif
...
#if (MU_BEAMFORMING_SUPPORT == 1)
void txretry_handler_MU(struct rtl8192cd_priv *priv, struct tx_retry *retry, unsigned char macid_idx)
{
	int i;
	PRT_BEAMFORMING_INFO		pBeamInfo = &(priv->pshare->BeamformingInfo);
	PRT_BEAMFORMING_ENTRY		pEntry = NULL;
	...
	if(priv->pshare->sta_query_retry_macid[macid_idx] < 0x80 || priv->pshare->sta_query_retry_macid[macid_idx] > 0x80+29) {
		panic_printk("Wrong macid=%d\n", priv->pshare->sta_query_retry_macid[macid_idx]);
		return;
	}
	...
	{
		pEntry = &(pBeamInfo->BeamformeeEntry[i]); 
		if(pEntry->mu_reg_index == gid_to_pos1_index[priv->pshare->sta_query_retry_macid[macid_idx] - 0x80] && pEntry->pSTA) {

			pEntry->pSTA->cur_tx_retry_pkts = retry->retry_pkt_macid[macid_idx];
			pEntry->pSTA->cur_tx_retry_cnt = retry->retry_cnt_macid[macid_idx];

			pEntry->pSTA->total_tx_retry_pkts += pEntry->pSTA->cur_tx_retry_pkts;
			pEntry->pSTA->total_tx_retry_cnt += pEntry->pSTA->cur_tx_retry_cnt;
			priv->ext_stats.tx_retrys += pEntry->pSTA->cur_tx_retry_cnt;
			return;
		}
	}
...
void txretry_handler(struct rtl8192cd_priv *priv, struct tx_retry *retry)
{
	...
	{
#if(MU_BEAMFORMING_SUPPORT == 1)
		if(priv->pshare->sta_query_retry_macid[macid_idx] >= 0x80 && priv->pshare->sta_query_retry_macid[macid_idx] != 0xff) {
			txretry_handler_MU(priv, retry, macid_idx);
		}
		else
#endif
		{
...
void rtl8192cd_cu_stop(struct rtl8192cd_priv *priv) {
    ...
    if(stop_cu) {
        if (rtk_timer_pending(&priv->pshare->cu_info.cu_cntdwn_timer)) {
            rtk_del_timer_sync(&priv->pshare->cu_info.cu_cntdwn_timer);
        }
...
#endif
        rtk_mod_timer(&priv->pshare->cu_info.cu_cntdwn_timer, jiffies + CU_TO);
        priv->pshare->cu_info.cu_enable = 1;
    }
}

#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
static void rtl8192cd_cu_cntdwn_timer(unsigned long task_priv)
#else
static __always_inline void rtl8192cd_cu_cntdwn_timer(unsigned long task_priv)
#endif
{
    struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;
    int val;
    ...
    {
        rtk_mod_timer(&priv->pshare->cu_info.cu_cntdwn_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(10));
    }
    else
    {
...
DEFINE_TIMER_CALLBACK(rtl8192cd_cu_cntdwn_timer);

void init_cu_cntdwn_timer(struct rtl8192cd_priv *priv)
{
#if defined(CONFIG_PCI_HCI)
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		rtk_timer_setup(&(priv->pshare->cu_info.cu_cntdwn_timer), rtl8192cd_cu_cntdwn_timer, (unsigned long)priv, 0);
	} else
#endif
#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	{
		rtk_timer_setup(&(priv->pshare->cu_info.cu_cntdwn_timer), timer_event_timer_fn, (unsigned long)(&(priv->pshare->cu_info.cu_cntdwn_timer_event)), 0);
		INIT_TIMER_EVENT_ENTRY(&priv->pshare->cu_info.cu_cntdwn_timer_event, rtl8192cd_cu_cntdwn_timer, (unsigned long)priv);
	}
#endif
	;
}
#endif  // 0 //defined(HS2_SUPPORT) || defined(DOT11K) || defined(CH_LOAD_CAL)
...
//			            rtk_mod_timer(ptimer, jiffies + 1);
		...
		)
{
		...
		totaltp = priv->ext_stats.rx_avarage;
#ifdef UNIVERSAL_REPEATER  
		totaltp += GET_VXD_PRIV(priv)->ext_stats.rx_avarage;
#endif
#ifdef MBSSID
		for(i=0; i<RTL8192CD_NUM_VWLAN; i++)
			totaltp += priv->pvap_priv[i]->ext_stats.rx_avarage;
#endif
...
static __always_inline void RF_MIMO_check_timer(unsigned long task_priv)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;
	enum rf_type rf_mimo = RF_TYPE_MAX;

#ifdef MCR_WIRELESS_EXTEND
	return;
#else
	rf_mimo = get_rf_mimo_mode(priv);

	if (!(priv->drv_state & DRV_STATE_OPEN))
		return;

	if ((rf_mimo != RF_2T2R) && (rf_mimo != RF_3T3R))
		return;
	...
	|| (IS_DRV_OPEN(GET_VXD_PRIV(priv)))
		...
		if (rf_mimo != priv->pshare->rf_status) {
			set_MIMO_Mode(priv, rf_mimo);
		}

		return;	
	}

	// If rf_mode is disabled and rf mimo mode has been reset to default,
	// start next timer directly.
	if ((priv->pshare->rf_ft_var.rf_mode == 255) &&
			(get_rf_mimo_mode(priv) == priv->pshare->rf_status))
		goto end;

	/* Change rf mimo mode in terms of rf_mode settings */
	switch (priv->pshare->rf_ft_var.rf_mode) {
		case 1:
		...
		case 4:
			...
			break;
		case 0: /* auto */
			/* Reset to default rf mimo mode if a STA is connected */
			if (priv->pshare->total_assoc_num) {
				if (rf_mimo != priv->pshare->rf_status) {
					set_MIMO_Mode(priv, rf_mimo);
				}
			} else {
#if 0 // #if defined(WIFI_11N_2040_COEXIST_EXT)
			...
			break;
		default: /* 255: disable rf mimo mode */
			if (rf_mimo != priv->pshare->rf_status) {
				set_MIMO_Mode(priv, rf_mimo);
			}
			break;
...
DEFINE_TIMER_CALLBACK(RF_MIMO_check_timer);

void init_RF_MIMO_check_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->ps_timer), RF_MIMO_check_timer, (unsigned long)priv, 0);
}
...
#define dprintf printk
	...
	if (strlen(data) == 0) {
		...
		printk("mimo 0: auto\n");
		printk("mimo 255: disable and reset to default\n");
		return 0;
	}
	if (mode == 0x01) {
	...
	} else if (mode == 0xff) { /* disabled */
		MIMO_Mode_Switch(priv, get_rf_mimo_mode(priv));
		priv->pshare->rf_ft_var.rf_mode = 0xff;
	} else {
...
//#ifdef CONFIG_RTK_VLAN_SUPPORT
	...
	*(unsigned int *)&(arp_skb->cb[40]) = 0x86518192;
	...
	SMP_LOCK_XMIT(x);
	if(__rtl8192cd_start_xmit(arp_skb, priv->dev, TX_NO_MUL2UNI)) {
...
void copy_wps_ie(struct rtl8192cd_priv *priv, unsigned char *wps_ie, unsigned char mgmt_type)
{
	...
	if (mgmt_type == MGMT_BEACON) {
		...
		#define TAG_SELECTED_REGISTRAR		0x1041
			...
			int attr_len;
			if (search_wsc_tag(wps_ie+6, TAG_SELECTED_REGISTRAR, wps_ie_len-6, &attr_len))		// found
				priv->pshare->rf_ft_var.led_wps_mode = 1;
			else
...
struct tm *get_current_timeInfo ( struct tm * const	tmp, int tz)
{
	...
	const time64_t * const	timep = &(time_stamp.tv_sec);
	static int padding_digit = 0;
	static struct timespec64 time_stamp_prev;

	ktime_get_real_ts64(&time_stamp);
	if(time_stamp.tv_sec == time_stamp_prev.tv_sec && time_stamp.tv_nsec == time_stamp_prev.tv_nsec) {
		if(padding_digit < 9)
			padding_digit++;
	} else {
		padding_digit = 0;
		memcpy(&time_stamp_prev, &time_stamp, sizeof(struct timespec64));
	}

	memset(tmp, 0, sizeof(struct tm));
	time64_to_tm(*timep, SECSPERHOUR * tz, tmp);
	tmp->tm_yday = (time_stamp.tv_nsec/1000)/1000 + padding_digit;  // using tm_yday to store msec ...
...
struct tm *get_current_timeInfo ( struct tm * const	tmp, int tz)
{
	...
	const time_t * const	timep = &(time_stamp.tv_sec);
	static int padding_digit=0;
	...
	tmp->tm_yday = time_stamp.tv_usec/1000 + padding_digit;  // using tm_yday to store msec ...

	return tmp;
}
#endif // __LINUX_4_20__
#endif // STA_ASSOC_STATISTIC
...
void stop_monitor_mode(struct rtl8192cd_priv * priv)
{
	...
	if (rtk_timer_pending(&priv->chan_switch_timer))
		rtk_del_timer_sync(&priv->chan_switch_timer);
			...
			case CHANNEL_WIDTH_40:
				...
				if(pstat->bw_cnt[CHANNEL_WIDTH_20] == dybw_thrd) {
					*(unsigned int *)pstat->bw_cnt=0;
					...
					setSTABitMap(&priv->switch_20_sta, pstat->cmn_info.aid);
#ifdef CONFIG_RTL_OFFLOAD_DRIVER
...
static __always_inline void adaptivity_stats_timer(unsigned long task_priv)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)task_priv;
	unsigned int ada_stats_to = priv->pshare->rf_ft_var.ada_stats_to;
#ifdef SMP_SYNC
...
DEFINE_TIMER_CALLBACK(adaptivity_stats_timer);

void init_ada_stats_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->pshare->ada_stats_timer), adaptivity_stats_timer, (unsigned long)priv, 0);
}
#endif  // ADAPTIVITY_STATS_DEBUG

#if defined(_CORTINA_)
#if !defined(__LINUX_4_4_211__) || (defined(__LINUX_4_5__) && !defined(__LINUX_4_6__))  // ver. < 4.4.211 or 4.5 < ver. < 4.6
...
#endif  // (kernel_ver < 4.4.211) || (4.5.0 <= kernel_ver < 4.6.0)
#endif  // _CORTINA_
...
int check_si_mem(struct rtl8192cd_priv *priv)
{
#ifdef __KERNEL__
	unsigned int mem_low_free;
	if(priv->pshare->rf_ft_var.mcst_min_rsvd_mem){
		unsigned long freeram, freehigh;
		freeram = nr_free_pages();
		freehigh = nr_free_highpages();

		#define K(x) ((x) << (PAGE_SHIFT - 10))
		mem_low_free = K(freeram - freehigh);
		if(mem_low_free < priv->pshare->rf_ft_var.mcst_min_rsvd_mem)
		{
			priv->ext_stats.si_mem_drop++;
		    DEBUG_ERR("System Memory reach oom-killer threshold!\n");
			return 0;
		}
	}
#endif
...
void smp_assign_tasklet(void *tasklet)
{
	tasklet_hi_schedule((struct tasklet_struct *)tasklet);
}

#ifdef SMP_SKB_RECYCLE_OFFLOAD
void skb_queue_recycle(struct sk_buff_head *skb_queue)
{
	...
	while (skb_queue_len(skb_queue)) {
		spin_lock_irqsave(&skb_queue->lock, flags);
		skb_queue_splice_init(skb_queue, &list);
...
void hapd_OnRadioMeasurementReport(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pframe, int frame_len){
	int len;
    unsigned char element_id;
...
#ifdef AP_NEIGHBOR_INFO
void hapd_rm_tx_nb_rpt_resp(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *buf, int *len)
{
	unsigned int i;
	unsigned char dialog_token = 0, match_bss_cnt = 0, *ptr_nb_rpt = NULL;
	struct dot11k_neighbor_report nb_rpt;
	//struct ap_neighbor_info *ap_neighbor = NULL;
	struct ap_neighbor_info_entry *ent = NULL;
	unsigned long sec;

	if ((!priv) || (!buf) || (!len)) {
		DEBUG_ERR("RM: invalid params. priv: %p buf: %p len: %p\n", priv, buf, len);
		return;
	}

	if (!priv->pmib->miscEntry.ap_neighbor_info_enable) {
		DEBUG_INFO("RM: ap neighbor info is not enabled.\n");
		return;
	}

	dialog_token = buf[2];
	if (dialog_token != pstat->rm.rm_nb_rpt_req_dialog_token) {
		DEBUG_INFO("RM: dialog token mismatch.\n");
		return;
	}

	ptr_nb_rpt = buf + 3;
	//ap_neighbor = &(priv->pshare->ap_neighbor);

	for (i = 0; i < MAX_AP_NEIGHBOR_INFO_NUM; i++) {
		ent = &(priv->pshare->ap_neighbor.ap_neighbor_info_ent[i]);
		if (ent->valid != 1)
			break;

		if ((pstat->rm.rm_nb_rpt_req_ssid_len != strlen(ent->ssid))
			|| memcmp(ent->ssid, pstat->rm.rm_nb_rpt_req_ssid, pstat->rm.rm_nb_rpt_req_ssid_len))
			continue;

		if (jiffies/HZ >= ent->sec)
			sec = jiffies/HZ - ent->sec;
		else
			sec = jiffies/HZ + ~(unsigned long)0/HZ - ent->sec;
		...
		memcpy(nb_rpt.bssid, ent->mac, 6);

		nb_rpt.bssinfo.value = 0x3;
		if (ent->standard & WIRELESS_11N)
			nb_rpt.bssinfo.value |= BIT11;
		if (ent->standard & WIRELESS_11AC)
			nb_rpt.bssinfo.value |= BIT12;
		nb_rpt.bssinfo.value = cpu_to_le32(nb_rpt.bssinfo.value);

		nb_rpt.channel = ent->channel;
		nb_rpt.op_class = rm_get_op_class(priv, nb_rpt.channel);

		if (ent->standard & WIRELESS_11AC)
			nb_rpt.phytype = RM_PHY_TYPE_VHT;
		else if (ent->standard & WIRELESS_11N)
			nb_rpt.phytype = RM_PHY_TYPE_HT;
		else if (priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_5G)
			nb_rpt.phytype = RM_PHY_TYPE_OFDM;
		else if (priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G) {
			if (ent->standard & WIRELESS_11G)
				nb_rpt.phytype = RM_PHY_TYPE_ERP;
			else
				nb_rpt.phytype = RM_PHY_TYPE_HRDSSS;
		}

		ptr_nb_rpt[0] = _NEIGHBOR_REPORT_IE_;
		ptr_nb_rpt[1] = 13;
		memcpy(ptr_nb_rpt+2, &nb_rpt, 13);
		ptr_nb_rpt += 15;
	}

	if (match_bss_cnt > 0)
		(*len) = WLAN_HDR_A3_LEN + 3 + match_bss_cnt * 15;
...
int wpa2_ie_check(
	...
	unsigned char **ie_ptr_pmksa, unsigned char *pmkid_cnt)
{
...
#if defined(DOT11K) || defined(HAPD_11K) || defined(RTK_MULTI_AP)
...
void addEntryBlockList(struct rtl8192cd_priv *priv, const struct assoc_control_block_list *list)
{
	...
	priv->block_list[i].got_assoc_req = 0;

	priv->block_list_bitmask[i>>3] |= (1<<(i&7));

	//DOT11VDEBUG
	panic_printk("ADDED BLOCK LIST STA (%d) :[%02x][%02x][%02x][%02x][%02x][%02x] (%u sec)\n",
...
void deleteEntryBlockList(struct rtl8192cd_priv *priv, const unsigned char* macAddress)
{
	...
	for (i = 0 ; i < MAX_TRANS_LIST_NUM; i++) {
		...
		if(0 == memcmp(macAddress, priv->block_list[i].addr, MACADDRLEN)) {
			...
			//DOT11VDEBUG
			panic_printk("REMOVE BLOCK LIST STA (%d) :[%02x][%02x][%02x][%02x][%02x][%02x] \n",
								i, priv->block_list[i].addr[0], priv->block_list[i].addr[1], priv->block_list[i].addr[2],
								priv->block_list[i].addr[3], priv->block_list[i].addr[4], priv->block_list[i].addr[5]);
			priv->block_list_bitmask[i>>3] &= ~(1<<(i&7));
	...
	for (i = 0 ; i < MAX_TRANS_LIST_NUM; i++) {
		if ((priv->block_list_bitmask[i>>3] & (1<<(i&7))) != 0) {
			if (priv->block_list[i].timer == 0) {
				priv->block_list_bitmask[i>>3] &= ~(1<<(i&7));
				//DOT11VDEBUG
				panic_printk("BLOCK LIST STA EXPIRES (%d) :[%02x][%02x][%02x][%02x][%02x][%02x] \n",
								i, priv->block_list[i].addr[0], priv->block_list[i].addr[1], priv->block_list[i].addr[2],
								priv->block_list[i].addr[3], priv->block_list[i].addr[4], priv->block_list[i].addr[5]);
				memset(&priv->block_list[i], 0, sizeof(struct assoc_control_block_list));
			} else {
...
int update_assoc_control_block_list(struct rtl8192cd_priv *priv, unsigned char *tmpbuf)
{
        ...
        if (control == 0x00) { // block STA
                ...
                //DOT11VTRACE
                panic_printk("invalid control code: %02x!\n", control);
        }

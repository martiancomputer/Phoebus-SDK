...
#endif // CONFIG_PHYDM_ANTENNA_DIVERSITY
}
...
int gothrough_brsrc(struct rtl8192cd_priv *priv, struct sk_buff *pskb, struct stat_info *pstat)
{
...
#endif
	{
		...
		if (TRUE
			...
			&& netif_running(cached_dev)
#if defined(CONFIG_RTK_GUEST_ZONE) && defined(__KERNEL__)
			&& (GET_BR_PORT(cached_dev) && GET_BR_PORT(cached_dev)->zone_type == GET_BR_PORT(pskb->dev)->zone_type)
#endif
			) 
		{
#if defined(CONFIG_RTK_GUEST_ZONE) && defined(__KERNEL__)
...
#if defined(__ECOS) || defined(RTK_129X_PLATFORM) || defined(__OSK__) || defined(__LINUX_3_14__)
...
#elif (MAX_RX_BUF_LEN < PAGE_SIZE)
			new_skb = __dev_alloc_skb(RX_BUF_LEN, GFP_ATOMIC | GFP_DMA);
#else
...
)
{
    ...
    u1Byte  bitmap_i,bitmap_val,shift_i,bitmap_valid,handle_macid,qos_case;
    pu1Byte reportbody = pBuf+SWPSRPT_BITMAP_SIZE; //rxdesc+bitmap
    struct stat_info * pstat;
    u1Byte PSflag;
    u2Byte reprepare_seq;
    u4Byte dump_i=0;
	u1Byte bitmap_valid_size = NUM_STAT /2 ;
    ...
    for(bitmap_i=0;bitmap_i<bitmap_valid_size;bitmap_i++){    
        ...
        for(shift_i=0;shift_i<8;shift_i++){
            ...
            if(bitmap_valid !=0x0){//have new report
                ...
                if(pstat){
                    ...
                    pstat->RPT_DropByID_SEQ[qos_case] = *(reportbody+handle_macid*SWPSRPT_ENTRY_SIZE+qos_case*4) + ((*(reportbody+handle_macid*SWPSRPT_ENTRY_SIZE+1+qos_case*4) & 0xf) << 8);
                    pstat->RPT_TxDone_SEQ[qos_case] = ((*(reportbody+handle_macid*SWPSRPT_ENTRY_SIZE+2+qos_case*4)) << 4) + ((*(reportbody+handle_macid*SWPSRPT_ENTRY_SIZE+1+qos_case*4) & 0xf0) >> 4);
                    pstat->RPT_DropBit = (*(reportbody+handle_macid*SWPSRPT_ENTRY_SIZE+3+qos_case*4) & (BIT0|BIT1|BIT2|BIT3));
                    PSflag = (*(reportbody+handle_macid*SWPSRPT_ENTRY_SIZE+3+qos_case*4) & BIT7) >> 7;
                    //GDEBUG("handle_macid=%d, pstat->RPT_TxDone_SEQ[%d]=%x\n",handle_macid,qos_case,pstat->RPT_TxDone_SEQ[qos_case]);//yllinSWPS
                    ...
                    if(PSflag && (pstat->RPT_PSflag==0)){ //after this client goto sleep, first time get report 
                        ...
                        if(pstat->RPT_TxDone_SEQ[qos_case] == MAX_SWPS_SEQ) //max=4095
                            ...
                            reprepare_seq = (pstat->RPT_TxDone_SEQ[qos_case]+1) & MAX_SWPS_SEQ;

						 
                             GDEBUG("macid %d qos%d : reprepare_seq=%x,swps_seq=%x.\n",handle_macid,qos_case,reprepare_seq,pstat->SWPS_seq[qos_case]);
						
                        if(pstat->SWPS_seq[qos_case]!=reprepare_seq)
                            pstat->RPT_reprepare_SEQ[qos_case] = reprepare_seq;
                        else
                            { 
                              GDEBUG("macid %d AC %d has no pkt need to reprepare.\n",handle_macid,qos_case);
						    } //debug
                        ...
                        GDEBUG("pstat->SWPS_last_seq[%d]=%x\n",qos_case,pstat->SWPS_last_seq[qos_case]); //yllinSWPS
                        ...
                        if(pstat->SWPS_last_seq[qos_case]!=0){
                            ...
                            if(pstat->SWPS_last_seq[qos_case] >= pstat->RPT_reprepare_SEQ[qos_case]){
                                ...
                                if((pstat->SWPS_last_seq[qos_case] - pstat->RPT_reprepare_SEQ[qos_case]) > (MAX_SWPS_SEQ/2)){
                                    ...
                                    GDEBUG("report drop seq >>> swps seq, no pkt to prepare, set last seq = 0....\n");//debug    
...
#endif                            
                            GDEBUG("pstat->reprepare_num[%d]=%x\n",qos_case,pstat->reprepare_num[qos_case]); //yllinSWPS
                            GDEBUG("pstat->SWPS_last_seq[%d]=%x\n",qos_case,pstat->SWPS_last_seq[qos_case]);
                            GDEBUG("pstat->RPT_reprepare_SEQ[%d]=%x\n",qos_case,pstat->RPT_reprepare_SEQ[qos_case]);
                        }else
                            { 
                                GDEBUG("last seq = 0....maybe RPT is faster than INT, or just no packet to prepare\n");
							}//debug    
...
#endif
		if (pskb) {
			txinsn.q_num   = pri_to_qnum(priv,pskb->cb[1]);	//WMMPS B.Z
...
void process_all_queues(struct rtl8192cd_priv *priv)
{
	...
	while (count < priv->pshare->rf_ft_var.limit_proc_all_queue) {
		pskb = skb_dequeue(&priv->rx_data_queue);
		if (pskb == NULL)
			break;
		
		__rtl_netif_rx(priv, pskb, (struct stat_info *)(*(unsigned long *)&pskb->cb[_SKB_CB_RX_STA]));
		priv->pshare->rxq_proc_seq++;
		count++;
	}
}
#endif
#else // SMP_RX_LOAD_BALANCE
void process_all_queues(struct rtl8192cd_priv *priv)
{
...
#ifdef SMP_RX_LOAD_BALANCE
	if (priv->pshare->rxq_proc_seq != priv->pshare->rxq_ind_seq)
...
__IRAM_IN_865X
static inline void schedule_rxq_tasklet(struct rtl8192cd_priv *priv)
{
#ifdef WIFI_RX_PROCESS_ALL_QUEUE_OFFLOAD
	...
	{
#ifdef WIFI_RX_PROCESS_ALL_QUEUE_OFFLOAD
		if (priv->pmib->dot11RFEntry.rx_data_offload != RX_DATA_OFFLOAD_DEFAULT_CPU) {
			smp_call_function_single_async(priv->pmib->dot11RFEntry.rx_data_offload,
					&(priv->pshare->rxq_tasklet_csd));
		} else
#endif
		{
...
#ifdef SMP_RX_LOAD_BALANCE
__IRAM_IN_865X
static inline void process_all_dz_queue(struct rtl8192cd_priv *priv)
{
	int i;
	...
	if (GET_ROOT(priv)->pmib->miscEntry.vap_enable) {
		for (i=0; i<RTL8192CD_NUM_VWLAN; i++) {
			if (IS_DRV_OPEN(priv->pvap_priv[i])) {
					if (!list_empty(&priv->pvap_priv[i]->wakeup_list))
						process_dzqueue(priv->pvap_priv[i]);
			}
		}
	}		
#endif
...
#if !defined(SMP_RX_LOAD_BALANCE)
	schedule_rxq_tasklet(priv);
#else
	process_all_dz_queue(priv);
#endif
...
void flush_rx_queue(struct rtl8192cd_priv *priv)
{
	...
	while ((pskb = skb_dequeue(&priv->rx_data_queue)) != NULL) {
...
static __inline__ unsigned char parse_ps_poll(struct rtl8192cd_priv *priv, struct stat_info *pstat) {
    ...
    if (hw_queue_num) {
...
#if defined(CONFIG_WLAN_HAL_8814AE) || defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8822CE) || defined(CONFIG_WLAN_HAL_8197F) || defined(CONFIG_WLAN_HAL_8198F) || defined(CONFIG_WLAN_HAL_8812FE)
			if (GET_CHIP_VER(priv) == VERSION_8814A || GET_CHIP_VER(priv) == VERSION_8822B || GET_CHIP_VER(priv) == VERSION_8822C || GET_CHIP_VER(priv) == VERSION_8197F || GET_CHIP_VER(priv) == VERSION_8198F || GET_CHIP_VER(priv) == VERSION_8812F) {
        ...
        {
            DEBUG_WARN("[%s][%s][%d] MACID sleep not support\n", __FUNCTION__, priv->dev->name, pstat->cmn_info.aid);
        }
    }    
    return 0;
...
// 2009.09.08
	...
	if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
		...
		if ((APSD_ENABLE) && (QOS_ENABLE) && pstat->QosEnabled)	
			txinsn.q_num   = pri_to_qnum(priv,pskb->cb[1]); //WMMPS M.U	
		else	
		{

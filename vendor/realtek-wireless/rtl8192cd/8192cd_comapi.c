			 ...
			 union iwreq_data *wrqu, char *essid)
{
	...
	sprintf(str, "ssid=%.*s\0", wrqu->essid.length, essid);
			...
			union iwreq_data *wrqu, char *extra)
{
	...
	//wait ss done
	wait_event_interruptible_timeout(priv->ss_wait,(priv->ss_req_ongoing == SS_INACTIVE),RTL_SECONDS_TO_JIFFIES(5));
	return 0;
}
#endif // WIFI_WPAS_CLI

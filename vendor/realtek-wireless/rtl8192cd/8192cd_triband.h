...
#define SMP_LOCK_DELSTA_LIST(__x__) \
	do { \
		if (priv->hci_type == RTL_HCI_PCIE) { \
			spin_lock_irqsave(&priv->delsta_list_lock, (__x__)); \
		} \
		else { \
			spin_lock_bh(&priv->delsta_list_lock); (void)(__x__); \
		} \
	} while(0)
...
#define SMP_UNLOCK_DELSTA_LIST(__x__) \
	do { \
		if (priv->hci_type == RTL_HCI_PCIE) { \
			spin_unlock_irqrestore(&priv->delsta_list_lock, (__x__)); \
		} \
		else { \
			spin_unlock_bh(&priv->delsta_list_lock); (void)(__x__); \
		} \
	} while(0)

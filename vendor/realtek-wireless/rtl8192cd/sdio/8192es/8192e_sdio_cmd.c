...
static __always_inline void timer_event_timer_fn(unsigned long __data)
{
...
DEFINE_TIMER_CALLBACK(timer_event_timer_fn);
...
u8 callback_update_sta_msr(struct rtl8192cd_priv *priv, struct cmd_header *pcmdhdr)
{
	...
	if ((pstat->state & WIFI_ASOC_STATE)
		...
		pcmdhdr->pstat->txpdrop_flag = ((DECREASE == param->opmode) ? 1:0);
	}
...
void rtw_offload_reinit_timer(struct rtl8192cd_priv *priv)
{
...
#endif
	rtk_mod_timer(&priv->pshare->xmit_check_timer, jiffies + msecs_to_jiffies(10));
	...
	rtk_mod_timer(&pBeamformingInfo->BeamformingTimer, jiffies + RTL_MILISECONDS_TO_JIFFIES(200));
#endif
}

...
static __always_inline void pre_rtl8192es_beacon_timer(unsigned long task_priv)
{
...
DEFINE_TIMER_CALLBACK(pre_rtl8192es_beacon_timer);
...
static void rtl8192es_beacon_timer(unsigned long task_priv)
{
	...
	if (pshare->offload_function_ctrl > RTW_PM_PREPROCESS) {
		rtk_mod_timer(&pshare->beacon_timer, jiffies+usecs_to_jiffies(beacon_interval));
		return;
	}
	...
	rtk_mod_timer(&pshare->beacon_timer, jiffies+usecs_to_jiffies(time_offset));
...
u8 rtw_init_drv_sw(struct rtl8192cd_priv *priv)
{
	...
	rtk_timer_setup(&(priv->pshare->beacon_timer), pre_rtl8192es_beacon_timer, (unsigned long)priv, 0);

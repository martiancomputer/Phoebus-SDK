...
static __always_inline void pre_rtl8188es_beacon_timer(unsigned long task_priv)
{
...
DEFINE_TIMER_CALLBACK(pre_rtl8188es_beacon_timer);
...
u8 rtw_init_drv_sw(struct rtl8192cd_priv *priv)
{
	...
	rtk_timer_setup(&(priv->pshare->beacon_timer), pre_rtl8188es_beacon_timer, (unsigned long)priv, 0);

...
#endif
#endif
#define PROBE_FRAME_SIZE (1024+24)
...
struct Dot11StationConfigEntry {
	...
	unsigned char	swTkipMic;
	unsigned char	skipFirstTkipMic;
	unsigned char	protectionDisabled;		// force disable protection
	unsigned char	olbcDetectDisabled;		// david, force disable olbc dection	
	unsigned char	nmlscDetectDisabled;		// hf, force disable no member legacy station condition detection
...
//#ifdef WLAN_DIAGNOSTIC
	...
	unsigned int    unassoc_sta_scan_ongoing;
...
//#endif
    ...
    /* below is for 802.11k spectrum management measurement*/
    unsigned char   dot11SMMeasurementActivated;
    ...
    unsigned int    dot11RadioMeasurementThreshold;
    unsigned int    dot11RMPktDeltaThreshold;
    unsigned int    dka_pkt_type;	// detect keep-alive packets type, bit0:arp, bit1: icmp
...
struct Dot1180211AuthEntry {
	...
	unsigned int	dot11GKRekeyTime;		// group key rekey time, 0 - disable
	unsigned int	dot11UKRekeyTime;		// unicast key rekey time, 0 - disable
	...
	unsigned char   dot11SAEPwe;   // 0: hunter-pecking 1: h2e 2: both
	unsigned char   dot11TDBitmap; // 1: SAE 2: 
...
//#endif
	unsigned char 	sta_leave_extra_condition;
	unsigned char	use_driver_acs;		//0: disable; 1: enable; greater than 1: acs is active now
};
...
// RTK_AC_SUPPORT
	...
	unsigned int	tssi1;
	unsigned int	tssi2;
	unsigned int	cmcc_probe_collect_enable;
	...
	unsigned char	mu_low_pwr_en;
	unsigned char	disable_mu_amsdu;
	...
	unsigned int    adaptive_ant;
	unsigned int    adaptive_ant_dbg;	
	unsigned int    adaptive_ant_rssi;	
    unsigned int    adaptive_ant_tolerance;		
};
...
struct bss_desc {
	...
	unsigned char	stage;	
    /*cfg p2p cfg p2p*/	
    #if	defined(WIFI_WPAS) || defined(RTK_NL80211) || defined(CONFIG_RTL_MULTI_WIFI_MODULES)
	#if 0
	...
	#endif
	#if defined(RTK_NL80211) || defined(CONFIG_RTL_MULTI_WIFI_MODULES)
	unsigned char			ht_info[32];
	unsigned char			ht_cap[32];
	#else
	struct ht_info_elmt ht_info;
	struct ht_cap_elmt ht_cap;
	#endif
	#endif
	#if	defined(WIFI_WPAS) || defined(RTK_NL80211) || defined(CONFIG_RTL_COMAPI_WLTOOLS) || defined(CONFIG_RTL_MULTI_WIFI_MODULES)
...
//#endif
	...
	unsigned int	autoch_score_timeout_default;
//#ifdef TCP_ACK_ACC
...
//#endif
	...
	unsigned int	ss_to; // Web SS timeout
	unsigned int	ss_igi; // initial gain for SS
	...
	unsigned int	pkt_dump;
	unsigned int	ping_toggle;
	...
	unsigned int	show_rev;
	unsigned int 	dc_debug;
	unsigned int	reset_hangup_dbg;
...
//#ifdef STA_CONTROL_BAND_TRANSITION
    ...
    char stactrl_btm_accepted_timeout;    // disassoc STA as timeout after BTM-Rsp with ACCEPT is received.
...
struct RoamingPolicyEntry {
	...
	unsigned char   roaming_force_11v;
};

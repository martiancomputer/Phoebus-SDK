...
struct rtk_timer_list	LED_TimerCFGING;
...
static void set_sw_LED0(struct rtl8192cd_priv *priv, int flag)
{
	...
	if(ledpwr_toggle_flag==0 && flag==RTL_LED_ON)//led power off
		return;
#endif
...
#else
static __always_inline void LED_Interval_timeout(unsigned long task_priv)
#endif
{
	...
	if (!(priv_led->drv_state & DRV_STATE_OPEN) && !(priv_led_o && (priv_led_o->drv_state & DRV_STATE_OPEN)))
	{
		//rtk_mod_timer(&priv->pshare->LED_Timer, jiffies + LED_NOBLINK_TIME);
		return;
	}
#else
	...
	if (priv->pshare->LED_ToggleForce < 2) {
		priv->pshare->LED_Toggle = priv->pshare->LED_ToggleForce;
		priv->pshare->LED_ToggleForce = 2;
...
#endif
    {
        if (!priv->pshare->set_led_in_progress) {
            ...
            if (GET_CHIP_VER(priv) == VERSION_8188E)
				...
				defined(CONFIG_WLAN_HAL_8822BE) || defined(CONFIG_WLAN_HAL_8812FE)
            if ((GET_CHIP_VER(priv) == VERSION_8192E) || (GET_CHIP_VER(priv) == VERSION_8192F) ||
					(GET_CHIP_VER(priv) == VERSION_8822B) || (GET_CHIP_VER(priv) == VERSION_8812F))
    ...
    if (LED_TYPE == LEDTYPE_SW_CUSTOM2_LED0_WPS && priv->pshare->rf_ft_var.led_wps_mode) {
        rtk_mod_timer(&priv->pshare->LED_Timer, jiffies + priv->pshare->LED_Interval);
    } else if (priv->pshare->LED_Toggle == priv->pshare->LED_ToggleStart) {
        rtk_mod_timer(&priv->pshare->LED_Timer, jiffies + priv->pshare->LED_Interval);
    } else {
        if 	(LED_TYPE == LEDTYPE_SW_CUSTOM1)
            rtk_mod_timer(&priv->pshare->LED_Timer, jiffies + priv->pshare->LED_Interval);
        else
            rtk_mod_timer(&priv->pshare->LED_Timer, jiffies + led_on_time);
    }

    priv->pshare->LED_Toggle = (priv->pshare->LED_Toggle + 1) % 2;
}
DEFINE_TIMER_CALLBACK(LED_Interval_timeout);
...
void enable_sw_LED(struct rtl8192cd_priv *priv, int init)
{
    ...
    if (GET_CHIP_VER(priv) == VERSION_8192E) {
        if (LED_TYPE != LEDTYPE_SW_LED_DO_NOTHING) {
            if (priv->pshare->set_led_in_progress)
                RTL_W32(LEDCFG, (RTL_R32(LEDCFG)&0xFFFFFF00));
            else
    ...
    if (GET_CHIP_VER(priv) == VERSION_8192F) {
        if (LED_TYPE != LEDTYPE_SW_LED_DO_NOTHING) {
            if (priv->pshare->set_led_in_progress)
                RTL_W32(LEDCFG, (RTL_R32(LEDCFG)&~(LED0CM_Mask|LED0DIS)) | BIT21 | LED0PL);
            else
    ...
    if (GET_CHIP_VER(priv) == VERSION_8881A) {
        ...
        writel(readl(IO_TYPE_CAST(0xb800350c)) | BIT(24), IO_TYPE_CAST(0xb800350c));
        #else
        setWlan5gLed(RTL_LED_OFF);/*tony 20150203: we should use gpio api, avoid gpio abnormal behavior.*/
        #endif
    } else
    #endif
...
#endif
	{
        ...
        priv->pshare->LED_ToggleStart = RTL_LED_OFF;
        priv->pshare->LED_ToggleForce = 2;
    ...
    if ((LED_TYPE == LEDTYPE_SW_ENABLE_TXRXDATA) ||
        (LED_TYPE == LEDTYPE_SW_ENABLETXRXDATA) ||
        (LED_TYPE == LEDTYPE_SW_CUSTOM2_LED0_WPS)) {
        if (!priv->pshare->set_led_in_progress) {
            set_sw_LED0(priv, RTL_LED_ON);
        } else {
            set_sw_LED0(priv, priv->pshare->set_led_setting);
        }

		set_sw_LED1(priv, RTL_LED_OFF);
		priv->pshare->LED_ToggleStart = RTL_LED_ON;
    } else if ((LED_TYPE == LEDTYPE_SW_LED2_GPIO10_ENABLETXRXDATA) ||
               (LED_TYPE == LEDTYPE_SW_LED2_GPIO10_ENABLETXRXDATA_92D)) {
        set_sw_LED2(priv, RTL_LED_ON);
        priv->pshare->LED_ToggleStart = RTL_LED_ON;
        ...
        if (priv->pshare->curr_band == BAND_5G) {
            ...
            set_sw_LED0(priv, RTL_LED_OFF);
            set_sw_LED1(priv, RTL_LED_ON);
        }
    }
        ...
        priv->pshare->LED_ToggleStart = RTL_LED_ON;
             ...
             (LED_TYPE == LEDTYPE_SW_LED2_GPIO10_LINKTXRX) ||
             (LED_TYPE == LEDTYPE_SW_LED2_GPIO8_LINKTXRXDATA)) {
        set_sw_LED2(priv, RTL_LED_OFF);
    }
	else if (LED_TYPE == LEDTYPE_SW_LED_DO_NOTHING) {
		priv->pshare->LED_ToggleStart = RTL_LED_ON;
		set_sw_LED0(priv, RTL_LED_ON);
	}
    else if (LED_TYPE == LEDTYPE_SW_CUSTOM2_LED0_WPS) {
		priv->pshare->LED_ToggleStart = RTL_LED_ON;
		set_sw_LED0(priv, RTL_LED_ON);
	}
    else {
	...
	if (init == 1) {
		...
		if (GET_HCI_TYPE(priv) == RTL_HCI_PCIE) {
			rtk_timer_setup(&(priv->pshare->LED_Timer), LED_Interval_timeout, (unsigned long)priv, 0);
		}
		#endif
        #if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
		if (GET_HCI_TYPE(priv) == RTL_HCI_USB || GET_HCI_TYPE(priv) == RTL_HCI_SDIO) {
			rtk_timer_setup(&(priv->pshare->LED_Timer), timer_event_timer_fn, (unsigned long)(&(priv->pshare->LED_Timer_event)), 0);
...
#ifdef SUPPORT_UCFGING_LED
static __always_inline void LED_Interval_timeout2(unsigned long task_priv)
{

#ifdef RTLWIFINIC_GPIO_CONTROL
		RTLWIFINIC_GPIO_write(5, LED_CFGING_Toggle);
#endif
	rtk_mod_timer(&LED_TimerCFGING, jiffies + LED_UCFGING_TIME);
	LED_CFGING_Toggle = (LED_CFGING_Toggle + 1) % 2;
}
DEFINE_TIMER_CALLBACK(LED_Interval_timeout2);

void StartCFGINGTimer(void)
{
	rtk_timer_setup(&(LED_TimerCFGING), LED_Interval_timeout2, 0UL, 0);
	LED_CFGING_Interval = LED_UCFGING_TIME;
	LED_CFGING_Toggle = 0;
	LED_CFGING_ToggleStart = RTL_LED_OFF;
	rtk_mod_timer(&LED_TimerCFGING, jiffies + LED_CFGING_Interval);
}
#endif  // SUPPORT_UCFGING_LED

void disable_sw_LED(struct rtl8192cd_priv *priv, int deltimer)
{
#ifdef CONFIG_WIFI_LED_SHARED
	if (!deltimer)
	{
...
void calculate_sw_LED_interval(struct rtl8192cd_priv *priv)
{
    ...
    if (LED_TYPE == LEDTYPE_SW_CUSTOM2_LED0_WPS) {
		if (priv->pshare->rf_ft_var.led_wps_mode) {
			priv->pshare->LED_Interval = LED_CUSTOM2_INTERVAL_TIME_WPS;
			return;
		}
	}
	...
	if (priv_led_o && (priv_led_o->drv_state & DRV_STATE_OPEN))
	{
        ...
        if (LED_TYPE == LEDTYPE_SW_CUSTOM1) {
            ...
            if (priv->pshare->LED_Interval == LED_NOBLINK_TIME)
                ...
                if ((LED_TYPE == LEDTYPE_SW_ENABLETXRXDATA) ||
					(LED_TYPE == LEDTYPE_SW_CUSTOM2_LED0_WPS))
                    priv->pshare->LED_ToggleForce = RTL_LED_ON;
                else
                    priv->pshare->LED_Interval = LED_NOBLINK_TIME;
            }
        }
...
#ifdef CONFIG_WIFI_LED_SHARED
		if (priv_led->link_status || (priv_led_o && priv_led_o->link_status))
#else
        if (priv->link_status)
#endif
            priv->pshare->LED_ToggleStart = RTL_LED_ON;
...
void control_wireless_led(struct rtl8192cd_priv *priv, int enable)
{
	...
	if (enable == 0) {
		priv->pshare->set_led_in_progress = 1;
		priv->pshare->set_led_setting = RTL_LED_OFF;
		set_sw_LED0(priv, RTL_LED_OFF);
		set_sw_LED1(priv, RTL_LED_OFF);
		set_sw_LED2(priv, RTL_LED_OFF);
	}
		...
		priv->pshare->set_led_in_progress = 1;
		priv->pshare->set_led_setting = RTL_LED_ON;
		set_sw_LED0(priv, RTL_LED_ON);
		set_sw_LED1(priv, RTL_LED_ON);
		set_sw_LED2(priv, RTL_LED_ON);
	}

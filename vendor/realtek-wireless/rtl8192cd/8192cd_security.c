...
extern struct rtk_timer_list	LED_TimerCFGING;
#endif
	...
	if(Set_Key.KeyType == DOT11_KeyType_Group)
	{
		...
		switch(Set_Key.EncType)
		{
			...
			case DOT11_ENC_CCMP:
				...
				break;
#ifdef WPA3_ENT
			case DOT11_ENC_GCMP256:
				set_ttkeylen(pEncryptKey, 32);
				set_tmickeylen(pEncryptKey, 24); //RTK_WPA3 ??
				set_aes_key(pEncryptKey, key_combo);

				pEncryptKey->dot11RXPN48.val48 = 0;

				DEBUG_INFO("going to set GCMP256-AES group key!\n");

					if (!SWCRYPTO) {
						if (set_gkey_to_cam)
						{
							retVal = CamDeleteOneEntry(priv, MULTICAST_ADD, Set_Key.KeyIndex, 0); //eric-?? only delete one entry for 256 bits
							if (retVal) {
								priv->pshare->CamEntryOccupied--;
								pmib->dot11GroupKeysTable.keyInCam = FALSE;
							}
							retVal = CamAddOneEntry(priv, MULTICAST_ADD, Set_Key.KeyIndex, DOT11_ENC_GCMP256<<2, 0, key_combo); //111: GCMP
							if (retVal) {
								priv->pshare->CamEntryOccupied++;
								pmib->dot11GroupKeysTable.keyInCam = TRUE;
							}
...
#ifdef WPA3_ENT
			if(pmib->dot11IGTKTable.dot11Privacy == DOT11_ENC_BIP_GMAC256){
				set_ttkeylen(pEncryptKey, 32);
				set_tmickeylen(pEncryptKey, 24); //RTK_WPA3 ??
				set_aes_key(pEncryptKey, key_combo);
			}
...
USE_GCMP:
			 ...
			 case DOT11_ENC_GCMP256:
				 ...
				 if(priv->pshare->rf_ft_var.keyforce)
				 memset(key_combo, priv->pshare->rf_ft_var.keyforce, 32); 

				 set_ttkeylen(pEncryptKey, 32);
				 set_tmickeylen(pEncryptKey, 24);
				 set_aes_key(pEncryptKey, key_combo);

				 panic_printk("going to set GCMP256 unicast key for sta %02X%02X%02X%02X%02X%02X, id=%d\n",
						 Set_Key.MACAddr[0], Set_Key.MACAddr[1], Set_Key.MACAddr[2],
						 Set_Key.MACAddr[3], Set_Key.MACAddr[4], Set_Key.MACAddr[5], pstat->keyid);
				...
				if (!SWCRYPTO) {
					...
					retVal = CamAddOneEntry(priv, Set_Key.MACAddr, Set_Key.KeyIndex, DOT11_ENC_GCMP256<<2, 0, key_combo);
					if (retVal) {
						priv->pshare->CamEntryOccupied++;
						if (pstat)	pstat->dot11KeyMapping.keyInCam = TRUE;
						assign_aggre_mthod(priv, pstat);
...
int __DOT11_Indicate_MIC_Failure(struct net_device *dev, struct stat_info *pstat)
{
	...
	if (priv->MIC_timer_on)
	{
		...
		rtk_mod_timer(&priv->assoc_reject_timer, jiffies + REJECT_ASSOC_PERIOD);
	...
	else
	{
		...
		rtk_mod_timer(&priv->MIC_check_timer, jiffies + MIC_TIMER_PERIOD);
...
static __always_inline void DOT11_Process_MIC_Timerup(unsigned long data)
{
...
DEFINE_TIMER_CALLBACK(DOT11_Process_MIC_Timerup);

void init_MIC_check_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->MIC_check_timer), DOT11_Process_MIC_Timerup, (unsigned long)priv, 0);
}

static __always_inline void DOT11_Process_Reject_Assoc_Timerup(unsigned long data)
{
	struct rtl8192cd_priv *priv = (struct rtl8192cd_priv *)data;
...
DEFINE_TIMER_CALLBACK(DOT11_Process_Reject_Assoc_Timerup);

void init_assoc_reject_timer(struct rtl8192cd_priv *priv)
{
	rtk_timer_setup(&(priv->assoc_reject_timer), DOT11_Process_Reject_Assoc_Timerup, (unsigned long)priv, 0);
}
...
int rtl8192cd_ioctl_daemonreq(struct net_device *dev, struct iw_point *data)
{
	...
	switch(req.EventId)
	{
		...
		case DOT11_EVENT_REQUEST_F_INCLUDE_WPS:
			ret = DOT11_DeQueue((unsigned long)priv, priv->pevent_queue, QueueData, &QueueDataLen);
			if (ret != 0) {
				DOT11_NO_EVENT noEvent = {0};
				noEvent.EventId = DOT11_EVENT_NO_EVENT;
				noEvent.IsMoreEvent = 0;
				if (copy_to_user((void *)data->pointer, (void *)(&noEvent), sizeof(DOT11_NO_EVENT))) {
					DEBUG_ERR("copy_to_user fail!\n");
					return -1;
				}
				data->length = sizeof(DOT11_NO_EVENT);
		...
		case DOT11_EVENT_GKEY_TSC:
			{
				...
				{
					if (data->length >= sizeof(DOT11_GKEY_TSC)){
						if (copy_to_user((void *)(data->pointer), &pGkey_TSC, sizeof(DOT11_GKEY_TSC))) {
							DEBUG_ERR("copy_to_user fail!\n");
							return -1;
						}
						data->length = sizeof(DOT11_GKEY_TSC);
					} else {
						DEBUG_ERR("copy_to_user fail: buffer size not enough!\n");
						return -1;
					}
				}
			}
...
/* Hotsport 2.0 Release 1 */
        ...
        case DOT11_EVENT_HS2_TSM_REQ:
			...
			rtk_mod_timer(&priv->disassoc_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(tsmreq.Disassoc_timer * 60 * 1000));
			}
			break;
		...
		case DOT11_EVENT_GET_IGTK_PN:
			{
				...
				*/
				//should consider endian issue
				pn._byte_.TSC0 =  pmib->dot11IGTKTable.dot11EncryptKey.dot11TXPN48._byte_.TSC0;
				pn._byte_.TSC1 =  pmib->dot11IGTKTable.dot11EncryptKey.dot11TXPN48._byte_.TSC1;
				pn._byte_.TSC2 =  pmib->dot11IGTKTable.dot11EncryptKey.dot11TXPN48._byte_.TSC2;
				pn._byte_.TSC3 =  pmib->dot11IGTKTable.dot11EncryptKey.dot11TXPN48._byte_.TSC3;
				pn._byte_.TSC4 =  pmib->dot11IGTKTable.dot11EncryptKey.dot11TXPN48._byte_.TSC4;
				pn._byte_.TSC5 =  pmib->dot11IGTKTable.dot11EncryptKey.dot11TXPN48._byte_.TSC5;
				if (data->length >= sizeof(union PN48)) {
					if (copy_to_user((void *)(data->pointer), &pn, sizeof(union PN48)))
					{
						DEBUG_ERR("copy_to_user fail!\n");
						return -1;
					}
					data->length = sizeof(union PN48);
				} else {
					DEBUG_ERR("copy_to_user fail: buffer size not enough!\n");
					return -1;
				}
		...
		case DOT11_EVENT_GENERAL_GET_EVENT:
			ret = DOT11_DeQueue((unsigned long)priv, priv->pgeneralevent_queue, QueueData, &QueueDataLen);
			if (ret != 0) {
				DOT11_NO_EVENT noEvent = {0};
				noEvent.EventId = DOT11_EVENT_NO_EVENT;
				noEvent.IsMoreEvent = 0;
		...
		case DOT11_EVENT_11K_NEIGHBOR_REPORT:
		{
			...
			if (val8 == 0) {
				val8 = 1;
				struct dot11k_neighbor_report neighbor_report = {0};
		...
		case DOT11_EVENT_UCFGING_LED:
			{
				...
				//panic_printk("LED_Configuring=%d\n", LED_Configuring);
					...
					if(LED_Configuring == 1){
						if (rtk_timer_pending(&priv->pshare->LED_Timer))
							rtk_del_timer_sync(&priv->pshare->LED_Timer);

							//init another timer for specific function
							StartCFGINGTimer();
					}else{
						//del another timer that specific function used
						if (rtk_timer_pending(&LED_TimerCFGING))
							rtk_del_timer_sync(&LED_TimerCFGING);

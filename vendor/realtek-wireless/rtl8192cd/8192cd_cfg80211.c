...
unsigned char is_WRT_scan_iface(const char* if_name)
{
	...
	if((strcmp(if_name, "tmp.phy0")==0) || (strcmp(if_name, "tmp.phy1")==0))
		return 1;
	else
#endif
		return 0;
...
void realtek_cfg80211_inform_bss(struct rtl8192cd_priv *priv)
{
	...
	pbuf = priv->bss_ext.WholeFrame;
	pbuflen = priv->bss_ext.WholeFrameLength;
	if(pbuflen == 0){
		NDEBUG2("CHK!!\n\n");
		return;
	}
...
//	}
	...
	for (i = 0; i < priv->site_survey->count; i++) {
		...
		pbuf = priv->site_survey->bss_ext[i].WholeFrame;
		pbuflen = priv->site_survey->bss_ext[i].WholeFrameLength;
		if(pbuflen == 0){
			NDEBUG2("CHK!!\n\n");
			continue;
		}
...
#if /*defined(OPENWRT_CC) || */ defined(__LINUX_3_18__)
...
{    
	...
	{	
		priv_root->ss_req_ongoing = SS_INACTIVE;
...
int event_indicate_cfg80211(struct rtl8192cd_priv *priv, unsigned char *mac, int event, unsigned char *extra)
{
	...
	struct wiphy *wiphy = priv->rtk->wiphy;
	unsigned short reason = WLAN_STATUS_SUCCESS;
...
#if /*defined(OPENWRT_CC) || */ defined(__LINUX_4_2__)
				if(extra) {
					if (*extra == _RSON_AUTH_NO_LONGER_VALID_)
						cfg80211_connect_result(priv->dev, NULL, NULL, 0, NULL, 0, WLAN_STATUS_UNSPECIFIED_FAILURE,	GFP_ATOMIC);
					else
...
#if /*defined(OPENWRT_CC) || */ defined(__LINUX_3_15__)
...
#if /*!defined(OPENWRT_CC) && */ !defined(__LINUX_4_0__)
		...
		case CFG80211_SCAN_ABORTED:
			{
				...
				if (priv->scan_req) 
				{
				#if defined(__LINUX_4_8__) || defined(CFG80211_FOR_BACKPORT)
...
static void rtk_get_band_capa(struct rtl8192cd_priv *priv,BOOLEAN *band_2gig ,BOOLEAN *band_5gig)
{
	...
	*band_5gig = false;
	...
	if ((GET_CHIP_VER(priv) == VERSION_8812E) || (GET_CHIP_VER(priv) == VERSION_8812F) || (GET_CHIP_VER(priv) == VERSION_8814A) || (GET_CHIP_VER(priv) == VERSION_8822B))
	{
...
#endif
			// We must keep dot11nUse40M to 40 MHz when conexist is configured even though
			// hostapd has set BW to 20 MHz due to a detection of OBSS before start_ap().
...
static void realtek_reset_security(struct rtl8192cd_priv *priv)
{
	NLENTER; 
...
void realtek_get_security(struct rtl8192cd_priv *priv, struct rtknl *rtk, struct cfg80211_crypto_settings crypto, int target_bss_idx)
{
    int i = 0;
	...
	case WLAN_AKM_SUITE_FT_8021X:
            NDEBUG("WLAN_AKM_SUITE_FT_8021X\n");				
            rtk->psk = 0;
            if (crypto.wpa_versions & NL80211_WPA_VERSION_1)
                rtk->wpa |= PSK_WPA;
            if (crypto.wpa_versions & NL80211_WPA_VERSION_2)
        ...
        case WLAN_AKM_SUITE_PSK_SHA256:
            NDEBUG("WLAN_AKM_SUITE_PSK_SHA256\n");
            rtk->psk = 1;
            rtk->sha256 = 1;
            if (crypto.wpa_versions & NL80211_WPA_VERSION_1)
                rtk->wpa |= PSK_WPA;
	...
	case WLAN_AKM_SUITE_8021X_SHA256:
            NDEBUG("WLAN_AKM_SUITE_8021X_SHA256\n");
            rtk->psk = 0;
            rtk->sha256 = 1;
            if (crypto.wpa_versions & NL80211_WPA_VERSION_1)
                rtk->wpa |= PSK_WPA;
...
void realtek_set_security_ap(struct rtl8192cd_priv *priv, struct rtknl *rtk, struct cfg80211_crypto_settings crypto)
{
	...
	// 802.1x + WEP is a vendor-specific security feature, so we cannot determine
	// whether 802.1x is required from crypto.akm_suites. However, if 802.1x + WEP
	// is configured, crypto.control_port_no_encrypt will be TRUE. So, we can rely
	// on this variable to determine whether dot118021xAlgrthm should be set to 1.
	if (rtk->cipher & (BIT(_WEP_40_PRIVACY_) | BIT(_WEP_104_PRIVACY_)))
		priv->pmib->dot118021xAuthEntry.dot118021xAlgrthm = ((crypto.control_port_no_encrypt) ? 1 : 0);

	realtek_set_security(priv, rtk);
}

void realtek_set_security_cli(struct rtl8192cd_priv *priv, struct rtknl *rtk, struct cfg80211_connect_params *sme, int target_bss_idx)
{
...
#ifdef CONFIG_RTL_WIFI_LED_SHARE_WITH_WPS
	priv->pshare->rf_ft_var.led_wps_mode = 0;
#endif
}
...
extern unsigned char Doll1X_SHA256_AKM_SUITE[];
extern unsigned char Doll1X_AKM_SUITE[];
...
{
	unsigned char *TAG_IE=NULL, *ExtCap_IE_Ptr=NULL , *tmp_IE_ptr=NULL;
	unsigned char *OSEN_IE=NULL;
	size_t TAG_LEN=0, ExtCap_IE_LEN=0,tmp_IE_Len=0 ;
	...
	DOT11_RSN_IE_SUITE *pDot11RSNIEGroupSuite;
	...
	TAG_IE = get_ie(BeaconBuf, _EXTENDED_CAP_IE_, &TAG_LEN, BeaconLen);
	if(TAG_IE) {
	...
	TAG_IE = get_ie(BeaconBuf, WLAN_EID_RRM_ENABLED_CAPABILITIES, &TAG_LEN, BeaconLen);
	if(TAG_IE) {
		if(TAG_LEN!=5){
			NDEBUG("[11K]IE should be 5,chk\n");
			priv->hapd_11k_ie_len = 0;
		}else{
			memcpy(priv->hapd_11k_ie,TAG_IE,TAG_LEN+2);
			priv->hapd_11k_ie_len = TAG_LEN+2;
			dump_hex("[11K enabled ;IE=]", priv->hapd_11k_ie, priv->hapd_11k_ie_len);
		}
	}
	else {
		priv->hapd_11k_ie_len = 0;
	...
	if(TAG_IE) {
		...
		/*GET RNS / WPA IE   --start*/
		/*parsing country code*/
		TAG_IE = get_ie(BeaconBuf, _RSN_IE_2_, &TAG_LEN, BeaconLen);
		if(TAG_IE) {
			NDEBUG("[_RSN_IE_2_] IE found,use it as DUT's RNS IE\n");
			...
			NDEBUG("[_RSN_IE_2_] IE not found  ,CHK!!!\n");
		}
		/*GET RNS / WPA IE   --end*/
	...
	TAG_IE = get_ie((unsigned char *)BeaconBuf, _RSNX_IE_, &TAG_LEN, BeaconLen);
	if(TAG_IE) {
	...
	/*parsing country code*/
	TAG_IE = get_ie(BeaconBuf, _COUNTRY_IE_, &TAG_LEN, BeaconLen);
	if(TAG_IE) {
	...
	if (TAG_IE) {
		...
		pDot11RSNIEGroupSuite = (DOT11_RSN_IE_SUITE *)TAG_IE;
		if (pDot11RSNIEGroupSuite->Type == DOT11_ENC_TKIP)
			priv->pmib->dot11GroupKeysTable.dot11Privacy = DOT11_ENC_TKIP;
		else if (pDot11RSNIEGroupSuite->Type == DOT11_ENC_CCMP)
		...
		for (i = 0 ; i < count ; ++i)
		{
			if (pDot11RSNIECountSuite->dot11RSNIESuite[i].Type == DOT11_ENC_TKIP)
				priv->pmib->dot1180211AuthEntry.dot11WPA2Cipher |= BIT1;
			else if (pDot11RSNIECountSuite->dot11RSNIESuite[i].Type == DOT11_ENC_CCMP)
				priv->pmib->dot1180211AuthEntry.dot11WPA2Cipher |= BIT3;
		}
			...
			for (i = 0 ; i < count ; ++i){
				...
				if (!memcmp(&pDot11RSNIECountSuite->dot11RSNIESuite[i],
					Doll1X_AKM_SUITE, OUI_LEN) ||
					!memcmp(&pDot11RSNIECountSuite->dot11RSNIESuite[i],
					Doll1X_SHA256_AKM_SUITE, OUI_LEN)){
					priv->pmib->dot118021xAuthEntry.dot118021xAlgrthm = 1;
				}
			}
			TAG_IE += (2 + count * sizeof(DOT11_RSN_IE_SUITE));
		...
		for(;;)
		{
...
#endif
			TAG_IE += sizeof(DOT11_RSN_IE_HEADER);
			...
			{
				if (pDot11RSNIECountSuite->dot11RSNIESuite[i].Type == DOT11_ENC_TKIP)
					priv->pmib->dot1180211AuthEntry.dot11WPACipher |= BIT1;
				else if (pDot11RSNIECountSuite->dot11RSNIESuite[i].Type == DOT11_ENC_CCMP)
					priv->pmib->dot1180211AuthEntry.dot11WPACipher |= BIT3;
			}
		}

	if (!priv->pmib->dot118021xAuthEntry.dot118021xAlgrthm) {
		if (priv->pmib->dot1180211AuthEntry.dot11WPACipher) priv->pmib->dot1180211AuthEntry.dot11EnablePSK |= PSK_WPA;
		if (priv->pmib->dot1180211AuthEntry.dot11WPA2Cipher) priv->pmib->dot1180211AuthEntry.dot11EnablePSK |= PSK_WPA2;
		if (priv->pmib->dot1180211AuthEntry.dot11EnablePSK)
		{
			if ((priv->pmib->dot1180211AuthEntry.dot11WPACipher & BIT3) || (priv->pmib->dot1180211AuthEntry.dot11WPA2Cipher & BIT3))
		...
		NDEBUG("[%s] <privacy> %d, <PrivacyAlgrthm> %d\n",__func__,priv->pmib->dot11GroupKeysTable.dot11Privacy,priv->pmib->dot1180211AuthEntry.dot11PrivacyAlgrthm);
#ifdef HS2_SUPPORT
		...
		if(TAG_IE) {
			if(memcmp(TAG_IE+2,OSEN_OUI,OUI_LEN)==0) {
				...
				*/
				...
				/*RSN cap , len=2*/
				OSEN_IE += ( 2  + sizeof(DOT11_RSN_IE_SUITE)*cipher_count);
				RSN_Cap_Ptr = (DOT11_RSN_CAPABILITY *)OSEN_IE;
	...
	ExtCap_IE_Ptr = parsing_specific_ie(BeaconBuf, _EXTENDED_CAP_IE_, 0, &ExtCap_IE_LEN, BeaconLen);
	...
	if(ExtCap_IE_Ptr) {
		...
		if(ExtCap_IE_LEN > 3) {
			...
			#else
			if(contend[3] & _UTC_TSF_OFFSET_){
				unsigned char *advt_ie=NULL, advt_ie_len=0;

				advt_ie = parsing_specific_ie(BeaconBuf, _ADVT_PROTO_IE_, 0, &advt_ie_len, BeaconLen);

				if(advt_ie) {
					memcpy(priv->pmib->hs2Entry.advt_proto_ie, advt_ie+2, TAG_LEN);
					priv->pmib->hs2Entry.advt_proto_ielen = advt_ie_len;
					NDEBUG("[ADVT]found\n");
				} else {
					priv->pmib->hs2Entry.advt_proto_ielen = 0;
					NDEBUG("[ADVT]not found\n");
				}
			} else {
				priv->pmib->hs2Entry.advt_proto_ielen = 0;			
				NDEBUG("[ADVT IE] not support\n");
			}
			#endif
			if(contend[3] & _INTERWORKING_SUPPORT_) {
				...
				interworking_ie = parsing_specific_ie(BeaconBuf, _INTERWORKING_IE_, 0, &interworking_ie_len, BeaconLen);
	...
	TAG_IE = parsing_specific_ie(BeaconBuf, _ROAM_IE_, 0, &TAG_LEN, BeaconLen);
	if(TAG_IE) {
	...
	/*Time Advt ID 69*/
	TAG_IE = parsing_specific_ie(BeaconBuf, _TIMEADVT_IE_, 0, &TAG_LEN, BeaconLen);
	if(TAG_IE) {
...
void realtek_set_ies_apmode(struct rtl8192cd_priv *priv, struct cfg80211_beacon_data *info)
{
	...
	beacon_buf = (unsigned char *)kmalloc(beacon_len+10, GFP_ATOMIC);
        if (beacon_buf != NULL)
	...
	#if 1 
		/*Sync TIME ZONE IE , BC FANG*/
		ie = parsing_specific_ie(info->proberesp_ies, _TIMEZONE_IE_, 0, &ie_len, info->proberesp_ies_len);

		if(ie) {
			memcpy(priv->pmib->hs2Entry.timezone_ie, ie+2, ie_len);
			priv->pmib->hs2Entry.timezone_ielen = ie_len;
			NDEBUG2(COLOR_BLUE"%s %d Time Zone IE found\n"COLOR_NONE,__func__,__LINE__);
		} else {
			NDEBUG2(COLOR_RED"%s %d Time Zone IE not found\n"COLOR_NONE,__func__,__LINE__);
		}
		/*Sync TIME ZONE IE , BC FANG*/
	#endif
	}
	...
	struct cfg80211_beacon_data *beacon, struct rtknl *rtk)
{
	...
	{		
		/*GET RNSEX IE(WPA3 R3)*/
		p = get_ie((unsigned char *)beacon->tail, _RSNX_IE_, &len, beacon->tail_len);
		if(p) {
...
#ifdef CONFIG_IEEE80211W
			if (!(dot11EnablePSK & (PSK_WPA2 | PSK_WPA3)) || !(dot11WPA2Cipher & BIT3))
			{
				dot11IEEE80211W = 0;
				dot11EnableSHA256 = 0;
			}
...
static int realtek_get_center_ch_2g(unsigned int ch, enum nl80211_chan_width width, unsigned int offset)
{
...
static int realtek_get_center_ch_5g(unsigned int ch, enum nl80211_chan_width width)
{
	int center_ch = ch;

	if (width == NL80211_CHAN_WIDTH_160) {
		if (ch % 4 == 0) {
			if (ch >= 36 && ch <= 64)
				center_ch = 50;
		...
		if (ch % 4 == 0) {
			...
			if (ch >= 149 && ch <= 153)
				...
				center_ch = 159;
			else if (ch >= 165 && ch <= 169)
				center_ch = 167;
			else if (ch >= 173 && ch <= 177)
				center_ch = 175;
		}
	}
...
static int realtek_get_center_ch(unsigned int ch, enum nl80211_chan_width width, unsigned int offset)
{
	// We assume that the control_ch is validated before entering this stage
	if (ch <= 14)
		return realtek_get_center_ch_2g(ch, width, offset);
	else
		return realtek_get_center_ch_5g(ch, width);
}

static int realtek_chbw_to_cfg80211_chan_def(struct wiphy *wiphy, struct net_device *dev, struct cfg80211_chan_def *chdef,
	enum nl80211_band band, unsigned int control_ch, enum nl80211_chan_width width, unsigned int offset)
{
	struct rtl8192cd_priv *priv = realtek_get_priv(wiphy, dev);
	int freq, cfreq, center_ch, ret = 0;
	struct ieee80211_channel *chan;

	NLENTER;

	memset(chdef, 0, sizeof(*chdef));

	// check the control_ch is valid
	freq = ieee80211_channel_to_frequency(control_ch, band);
	if (!freq)
		goto exit;

	center_ch = realtek_get_center_ch(control_ch, width, offset);

	cfreq = ieee80211_channel_to_frequency(center_ch, band);
	if (!cfreq)
		goto exit;
...
u8 rtw_cfg80211_ch_switch_notify(struct rtl8192cd_priv *priv)
{
	...
	struct wiphy *wiphy = priv->rtk->wiphy;
	unsigned int new_ch = priv->pmib->dot11RFEntry.dot11channel;
	unsigned char bw = priv->pshare->is_40m_bw;
	enum SECONDARY_CHANNEL_OFFSET offset = priv->pshare->offset_2nd_chan;
	enum nl80211_band band = IEEE80211_NUM_BANDS;
	int ret;
	...
	ret = realtek_chbw_to_cfg80211_chan_def(wiphy, priv->dev, &chdef, band, new_ch, bw, offset);
	if (!ret)
		return FAIL;
...
static int realtek_cfg80211_set_channel_with_self_acs_results(struct wiphy *wiphy,
	struct net_device *dev, struct cfg80211_chan_def *chandef)
{
	struct rtknl *rtk = wiphy_priv(wiphy);
	struct rtl8192cd_priv *priv = realtek_get_priv(wiphy, dev);
	unsigned int new_ch, minScore = 100, hostapd_ch;
	int ret = 0, i;
	struct cfg80211_chan_def new_chdef;
	enum SECONDARY_CHANNEL_OFFSET offset = HT_2NDCH_OFFSET_DONTCARE;

	NLENTER;

	new_ch = (priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G) ? 36 : 1;
	hostapd_ch = ieee80211_frequency_to_channel(chandef->chan->center_freq);
	priv->autoch_only_ss_clear_ch = 0; //reset

	// Find best channel (min score)
	for (i = 0; i < priv->available_chnl_num; i++) {
		if (minScore > priv->chnl_ss_score[i]) {
			minScore = priv->chnl_ss_score[i];
			new_ch = priv->available_chnl[i];
		}
	}

	// Find (best) 2nd offset
	if ((priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_2G) &&
		...
		else if (priv->chnl_ss_score[new_ch-4] > priv->chnl_ss_score[new_ch+4])
			offset = HT_2NDCH_OFFSET_ABOVE;
		else
			offset = HT_2NDCH_OFFSET_BELOW;
	}

	// Evaluate whether channel is valid. If not, use hostapd's settings.
	ret = realtek_chbw_to_cfg80211_chan_def(wiphy, dev, &new_chdef, chandef->chan->band, new_ch, chandef->width, offset);
	if (!ret) {
		NDEBUG3("[%s]Can't create new chandef, use hostapd chandef!\n", priv->dev->name);
		NDEBUG3("[%s]center_freq=[%u] channel=[%d] hw_value=[%u] bandwidth=[%d]\n", priv->dev->name,
				chandef->chan->center_freq, hostapd_ch, chandef->chan->hw_value, chandef->width);
		priv->pmib->dot11RFEntry.dot11channel = hostapd_ch;
		rtk_set_channel_mode(priv, chandef);
	} else {
		// Use driver's ACS result
		NDEBUG3("[%s]center_freq=[%u] channel=[%d] hw_value=[%u] bandwidth=[%d]\n", priv->dev->name,
...
static inline void set_pairwise_key_for_ibss(struct rtl8192cd_priv *priv, struct ieee80211req_key *wk)
{
	int i = 0;
	struct stat_info *pstat = NULL;
	...
	for(i=0; i<NUM_STAT; i++)
	{
		if(priv->pshare->aidarray[i] && (priv->pshare->aidarray[i]->used == TRUE)) 
		{
			...
			if(pstat)
			{
				...
				rtl_net80211_setkey(priv->dev, wk);
			}
		}
	}
}
...
#if 0
{
	...
	{
		if(!pairwise)
			set_pairwise_key_for_ibss(priv, &wk);	//or need to apply set_default_key
	}
#endif
				   ...
				   const u8 *mac_addr)			   
{
	...
	rtl_net80211_delkey(priv->dev, &wk);
...
#if /*defined(OPENWRT_CC) || */ defined(__LINUX_3_20__)
...
#endif
{
	...
	pstat->in_cfg80211_del_station = 1;

	// If station fallbacks to AUTHENTICATED from ASSOCIATED, do not
	// delete (i.e. calling free_stainfo()) this stat_info object.
	if ((list_empty(&pstat->asoc_list)) && (!(pstat->state & WIFI_DELETE_STATE)))
		goto realtek_cfg80211_del_station_end;
...
realtek_cfg80211_del_station_end:
	if (pstat != NULL)
		pstat->in_cfg80211_del_station = 0;
				    ...
				    struct station_parameters *params)
{
	...
	if(priv->pmib->dot1180211AuthEntry.dot11EnablePSK || priv->pmib->dot118021xAuthEntry.dot118021xAlgrthm)//OPENWRT_RADIUS 
		rtl_net80211_setmlme(priv->dev, &mlme);
...
static void realtek_cfg80211_set_rate_info(struct rate_info *r_info, unsigned int rate, unsigned char bw, unsigned char isSgi)
{
	...
	{
		r_info->legacy = ((rate&0x7f) / 2) * 10 /* in 100Kbps units */;
	}
...
static int realtek_cfg80211_set_wiphy_params(struct wiphy *wiphy, u32 changed)
{
	...
	if (changed & WIPHY_PARAM_FRAG_THRESHOLD)
		priv->pmib->dot11OperationEntry.dot11FragmentationThreshold = (wiphy->frag_threshold == -1) ? 2346 : wiphy->frag_threshold;
	if (changed & WIPHY_PARAM_RTS_THRESHOLD)
		priv->pmib->dot11OperationEntry.dot11RTSThreshold = (wiphy->rts_threshold == -1) ? 2347 : wiphy->rts_threshold;
...
#if defined(DFS)
	if (rtk_timer_pending(&GET_ROOT(priv)->ch_avail_chk_timer)) {
			  ...
			  struct cfg80211_scan_request *request)
{
	...
	// Count number of times that a scan is triggered by hostapd before start_ap()
	priv->pshare->scanned_time++;
...
start_scan:
	...
	if ((OPMODE & WIFI_AP_STATE) && (priv->pmib->dot11OperationEntry.use_driver_acs) && !is_WRT) {
		priv->ss_req_ongoing = SS_INACTIVE;
		priv->auto_channel = 1;
		priv->autoch_only_ss = 1; //Don't apply minChan to channel mib
		start_clnt_ss(priv);
	} else
		start_clnt_ss(priv);
...
#define IEEE80211_OFDM_RATE_54MB	0x6C
	...
	if (beacon->head_len && beacon->head_len > 36) {
		//Skip MAC header(24), timestamp(8), beacon interval(2), capability info(2)
		p = get_ie(beacon->head+36, _SUPPORTEDRATES_IE_, &len, beacon->head_len-36);
		if (p && len > 0) {
			p += 2;
			//Check OFDM rates
			while (--len >= 0) {
				if ((*(p+len) & ~(BIT(7))) == IEEE80211_OFDM_RATE_6MB ||
					(*(p+len) & ~(BIT(7))) == IEEE80211_OFDM_RATE_9MB ||
					(*(p+len) & ~(BIT(7))) == IEEE80211_OFDM_RATE_12MB ||
					(*(p+len) & ~(BIT(7))) == IEEE80211_OFDM_RATE_18MB ||
					(*(p+len) & ~(BIT(7))) == IEEE80211_OFDM_RATE_24MB ||
					(*(p+len) & ~(BIT(7))) == IEEE80211_OFDM_RATE_36MB ||
					(*(p+len) & ~(BIT(7))) == IEEE80211_OFDM_RATE_48MB ||
					(*(p+len) & ~(BIT(7))) == IEEE80211_OFDM_RATE_54MB) {
					is_b_only = false;
					break;
				}
			}
...
check_end:
	...
	if(GET_CHIP_VER(priv) == VERSION_8192F && priv->pmib->dot11nConfigEntry.dot11nCoexist) {
		priv->pshare->rf_ft_var.bws_enable = 1;
	}
	else
			   ...
			   struct cfg80211_ap_settings *info)
{
	...
	if (IS_ROOT_INTERFACE(priv)) {
		// Use driver's acs result
		if (priv->pmib->dot11OperationEntry.use_driver_acs) {
			if (priv->pmib->dot11nConfigEntry.dot11nCoexist) {
				// When starting root AP, if an auto channel selection is requested
				// alongside a 20/40 MHz coexistence configuration, hostapd triggers
				// scan twice, first for ACS and second for OBSS detection.
				if (priv->pshare->scanned_time == 2)
					realtek_cfg80211_set_channel_with_self_acs_results(wiphy, dev, &info->chandef);
				else
					realtek_cfg80211_set_channel(wiphy, dev, &info->chandef);
			} else {
				// If AP is configured with coexistence and hostapd has triggers
				// scan before starting AP, use driver's ACS results. Note that
				// HT Scan for BW > 40 MHz has been disabled in hostapd using the
				// [FORCE_BW] config, so once scan has occured before start_ap(),
				// it must be a scan for acs. For more info about FORCE_BW, please
				// refer commit a72a743 in hostapd.
				if (priv->pshare->scanned_time > 0)
					realtek_cfg80211_set_channel_with_self_acs_results(wiphy, dev, &info->chandef);
				else
					realtek_cfg80211_set_channel(wiphy, dev, &info->chandef);
			}
	...
	if(IS_ROOT_INTERFACE(priv) && (OPMODE&WIFI_AP_STATE) &&
			info->chandef.chan->dfs_state == NL80211_DFS_AVAILABLE) {
		...
		if (rtk_timer_pending(&priv->DFS_timer)) {
			...
			init_DFS_timer(priv, CHECK_DRIVER_STATUS);
		}
		rtk_mod_timer(&priv->DFS_timer, jiffies + RTL_SECONDS_TO_JIFFIES(1));

		if (rtk_timer_pending(&priv->dfs_det_chk_timer)) {
			rtk_del_timer(&priv->dfs_det_chk_timer);
		} else {
			init_dfs_det_chk_timer(priv, CHECK_DRIVER_STATUS);
		}
...
static int apply_acl_rules(struct rtl8192cd_priv *priv)
{
	...
	for (i=0; i<priv->pmib->dot11StationConfigEntry.dot11AclNum; i++)
	{
		...
		NDEBUG("[Drv]Sync MAC ACL entry[%d]: %02x:%02x:%02x:%02x:%02x:%02x,%s from MIB\n",i,
			...
			(paclnode->mode == ACL_ALLOW)? "Allowed":"Denied");
...
exit:
	...
	priv->pmib->dot11StationConfigEntry.dot11AclMode = ACL_DISABLED;
			       ...
			       const struct cfg80211_acl_data *params)
{
	...
	NDEBUG("Configure MAC ACL mode: %s stations which are listed in ACL.\n", (params->acl_policy & NL80211_ACL_POLICY_DENY_UNLESS_LISTED) ? "Allow" : "Deny");
	priv->pmib->dot11StationConfigEntry.dot11AclMode = (params->acl_policy & NL80211_ACL_POLICY_DENY_UNLESS_LISTED) ? ACL_ALLOW : ACL_DENY;
					 ...
					 u32 cac_time_ms)
{
	...
	priv->pshare->dfs_chan_def = (struct cfg80211_chan_def *)kzalloc(sizeof(struct cfg80211_chan_def), GFP_ATOMIC);
	if(priv->pshare->dfs_chan_def) {
	...
	priv->pshare->dfs_chan_def->chan = (struct ieee80211_channel *)kzalloc(sizeof(struct ieee80211_channel), GFP_ATOMIC);
	if(priv->pshare->dfs_chan_def->chan)
	...
	if (rtk_timer_pending(&priv->ch_avail_chk_timer)) {
		...
		init_ch_avail_chk_timer(priv, CHECK_DRIVER_STATUS);
	}
	rtk_mod_timer(&priv->ch_avail_chk_timer, jiffies + RTL_MILISECONDS_TO_JIFFIES(cac_time_ms));
	printk("*** [%s]Activate DFS-CAC for %d miliseconds ***\n",priv->dev->name,cac_time_ms);

	if (!rtk_timer_pending(&priv->DFS_TXPAUSE_timer)) {
		init_DFS_TXPAUSE_timer(priv, CHECK_DRIVER_STATUS);
	}
...
//=== check parameters
	...
	if (sme->bssid) {
		...
		bss_num = get_bss_by_bssid(priv, sme->bssid_hint, bssdb_count, bssdb);	
	...
	if (bss_num < 0)
	{
		...
		if (priv->site_survey->bss_ext[bss_num].WholeFrameLength <= PROBE_FRAME_SIZE) {
			priv->bss_ext.WholeFrameLength = priv->site_survey->bss_ext[bss_num].WholeFrameLength;
			memcpy(priv->bss_ext.WholeFrame, priv->site_survey->bss_ext[bss_num].WholeFrame, 
				priv->site_survey->bss_ext[bss_num].WholeFrameLength);
		}
	}

	priv->ss_req_ongoing = SS_INACTIVE; //found bss, no need to scan ...
...
#ifndef DIRECT_HAPD_RSN_IE	/*directly use all IEs(include RNS/WPA) from wpa_supplicant */
	if(priv->pmib->dot1180211AuthEntry.dot11EnablePSK)
		psk_init(priv);
		...
		struct wireless_dev *wdev, struct mgmt_frame_regs *upd)
{
	struct rtknl *rtk = wiphy_priv(wiphy);
	struct rtl8192cd_priv *priv = get_priv_from_wdev(rtk, wdev);
	unsigned int preq_mask = BIT(IEEE80211_STYPE_PROBE_REQ >> 4);
	unsigned int auth_mask = BIT(IEEE80211_STYPE_AUTH >> 4);

	if (OPMODE & WIFI_AP_STATE) {
		if (upd->interface_stypes & preq_mask)
			priv->probe_req_report = 1;
		else
			priv->probe_req_report = 0;
	}
...
void rtk_disable_monitor_mode(struct rtl8192cd_priv *priv)
{
	...
	if (rtk_timer_pending(&priv->chan_switch_timer))
		rtk_del_timer(&priv->chan_switch_timer);
}
#endif
...
static struct wireless_dev *realtek_cfg80211_add_iface(struct wiphy *wiphy,
						      const char *name,
#if /*defined(OPENWRT_CC) || */ defined(__LINUX_4_1__)
						      unsigned char name_assign_type,
#endif
						      enum nl80211_iftype type,
#if !defined(__LINUX_4_12__) && !defined(CPTCFG_VERSION)
						      u32 *flags,
#endif
				     ...
				     struct wireless_dev *wdev)
{
...
#if defined(__LINUX_4_12__) || defined(CFG80211_FOR_BACKPORT)
	wdev->nl_owner_dead = false;
#endif
	...
	// Call dev_close() to stop VAP net device, rather than deleting
	// the net device using unregister_netdev()
	lock_hold = rtnl_is_locked();
								...
								struct survey_info *survey)
{
	...
	if(!sband) {		
		...
		if (priv->nl80211_state == NL80211_OPEN && idx == 0) {
			priv->rtk->survey_info[idx].channel = priv->pmib->dot11RFEntry.dot11channel;
			read_noise_report(priv, 0);
		}

		if (!priv->rtk->survey_info[idx].channel) {
			return -ENOENT;
		}
	}
	...
	survey->filled = SURVEY_INFO_NOISE_DBM|SURVEY_INFO_CHANNEL_TIME|SURVEY_INFO_CHANNEL_TIME_BUSY|\
						SURVEY_INFO_CHANNEL_TIME_RX|SURVEY_INFO_CHANNEL_TIME_TX|SURVEY_INFO_IN_USE;
#endif
...

{
    ...
    if (rtk_timer_pending(&priv->p2pPtr->remain_on_ch_timer))
        rtk_del_timer(&priv->p2pPtr->remain_on_ch_timer);
	...
	rtk_mod_timer(&priv->p2pPtr->remain_on_ch_timer,jiffies + RTL_MILISECONDS_TO_JIFFIES(duration));
...
static __always_inline void realtek_cfg80211_RemainOnChExpire(unsigned long task_priv)
{
...
DEFINE_TIMER_CALLBACK(realtek_cfg80211_RemainOnChExpire);

void rtk_p2p_init_remain_on_ch_timer(struct rtl8192cd_priv *priv)
{
    rtk_timer_setup(&(priv->p2pPtr->remain_on_ch_timer), realtek_cfg80211_RemainOnChExpire, (unsigned long)priv, 0);
}
			 ...
			 int tx_ch, const u8 * mgmt_buf_from_cfg, int mgmt_buf_from_cfg_len)
{
	...
	if (priv->pshare->working_channel2 != tx_ch) {
		printk("chk, our working ch(%d) != tx_ch(%d) assigned by cfg\n", priv->pshare->working_channel2, tx_ch);
	}
	//rtk_set_scan_deny(priv,300);   // deny channel switch for 300 ms
	pbuf = txinsn.pframe = get_mgtbuf_from_poll(priv);
	if (pbuf == NULL)
	...
	category = frame_after_wlan_hrd[0];
	action = frame_after_wlan_hrd[1];
	tag = frame_after_wlan_hrd[7];
	NDEBUG2("category=[%d]; action=[%d]\n",category,action);

	if (category == WLAN_CATEGORY_PUBLIC || category == WLAN_CATEGORY_PROTECTED_DUAL_OF_ACTION) {
		...
		case WLAN_PA_GAS_INITIAL_RESP:
			...
			if (tag == 108) {
				protocol_id = frame_after_wlan_hrd[10];
				if (protocol_id == 0) {
...
/*HAPD__IEEE80211R*/
		...
		case WLAN_FT_ACTION_REQ:
			NDEBUG2("TX FT Action Req\n");
			break;
		case WLAN_FT_ACTION_RESP:
			NDEBUG2("TX FT Action Resp\n");
			break;
		case WLAN_FT_ACTION_CONFIRM:
			NDEBUG2("TX FT Action Confirm\n");
			break;
		case WLAN_FT_ACTION_ACK:
			NDEBUG2("TX FT Action Ack\n");
			break;
		default:
			NDEBUG2("unknown,dialog[%d]\n", action);
			break;
	...
	FTDEBUG("ft_state = %d\n", pstat->ft_state);
		...
		if(le16_to_cpu(mgmt->u.auth.auth_alg) == _AUTH_ALGM_SAE_){
			if(OPMODE & WIFI_AP_STATE){
				...
				pstat->is_sae_sta = 1;
				log("[issue_auth_resp]: auth_seq: %d, status: %d,len: %d,limt: %d",pstat->auth_seq,status,params->len,limit);
				issue_auth_resp(priv, pstat, status);
			}else if(OPMODE & WIFI_STATION_STATE){
				panic_printk("\n");
		...
		if (pstat->ft_state == state_imd_assoc) {
			...
			status = le16_to_cpu(mgmt->u.reassoc_resp.status_code);
			issue_asocrsp(priv, status, pstat, WIFI_ASSOCRSP, 0);
		} else
#endif /* CONFIG_IEEE80211R */
		if (pstat->has_pmkid) {
			status = le16_to_cpu(mgmt->u.reassoc_resp.status_code);
			if (pstat->assoc_type == WIFI_ASSOCREQ)
				issue_asocrsp(priv, status, pstat, WIFI_ASSOCRSP, 0);
			else
				issue_asocrsp(priv, status, pstat, WIFI_REASSOCRSP, 0);
		}
...
#if defined(AP_NEIGHBOR_INFO) && defined(RTK_MULTI_AP)
			if (mgmt->u.action.u.wme_action.action_code == WLAN_RRM_NEIGHBOR_REPORT_RESPONSE) {
				unsigned char new_buf[WLAN_HDR_A3_LEN+MAX_IE_LEN];
				int new_len = params->len;
				unsigned char *pframe_body;

				memcpy(new_buf, (unsigned char*)params->buf, params->len);
				pframe_body = new_buf + WLAN_HDR_A3_LEN;
				hapd_rm_tx_nb_rpt_resp(priv, pstat, pframe_body, &new_len);
				WARN_ON(new_len > WLAN_HDR_A3_LEN+MAX_IE_LEN);
				rtk_cfg80211_mgmt_tx(priv,tx_ch,new_buf, new_len);
			} else
#endif
			rtk_cfg80211_mgmt_tx(priv,tx_ch,params->buf, params->len);
		}else
...
#endif
{
	...
	// FIXME: Workaround for AX1500
#if defined(CONFIG_NET_PCI) && !(defined(CONFIG_USE_PCIE_SLOT_0) && defined(CONFIG_USE_PCIE_SLOT_1))
	snprintf(rtk->root_ifname, sizeof(rtk->root_ifname), "wlan1");
#elif defined(CONFIG_NET_PCI) && defined(CONFIG_USE_PCIE_SLOT_0) && defined(CONFIG_USE_PCIE_SLOT_1)
	// If using PCI_BIOS
	...
	snprintf(rtk->root_ifname, sizeof(rtk->root_ifname), "wlan%d", (idx == 0 ? 1 : 0));
	#else
	snprintf(rtk->root_ifname, sizeof(rtk->root_ifname), "wlan%d", (idx == 0 ? 0 : 1));
	#endif
#else
	// If using PCI_DIRECT

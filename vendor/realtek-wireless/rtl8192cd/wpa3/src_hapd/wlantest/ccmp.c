...
void ccmp_aad_nonce(const struct ieee80211_hdr *hdr, const u8 *data,
		  ...
		  const u8 *data, size_t data_len, size_t *decrypted_len)
{
	...
	if (rtk_aes_ccm_ad(tk, 16, nonce, 8, data + 8, mlen, aad, aad_len,
		       data + 8 + mlen, plain) < 0) {
		  ...
		  u8 *pn, int keyid, size_t *encrypted_len)
{
	...
	*pos++ = pn[0]; /* PN5 */
	...
	if (rtk_aes_ccm_ae(tk, 16, nonce, 8, frame + hdrlen, plen, aad, aad_len,
		       pos, pos + plen) < 0) {
		os_free(crypt);
		return NULL;
	}
		      ...
		      const u8 *data, size_t data_len, size_t *decrypted_len)
{
	...
	if (rtk_aes_ccm_ad(tk, 32, nonce, 16, data + 8, mlen, aad, aad_len,
		       data + 8 + mlen, plain) < 0) {
		u16 seq_ctrl = le_to_host16(hdr->seq_ctrl);
		      ...
		      u8 *qos, u8 *pn, int keyid, size_t *encrypted_len)
{
	...
	*pos++ = pn[0]; /* PN5 */
	...
	if (rtk_aes_ccm_ae(tk, 32, nonce, 16, frame + hdrlen, plen, aad, aad_len,
		       pos, pos + plen) < 0) {
		os_free(crypt);
		return NULL;
	}

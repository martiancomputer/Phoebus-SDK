...
void gcmp_aad_nonce(const struct ieee80211_hdr *hdr, const u8 *data,
 ...
 * @tk_len: length of @tk
 * @hdr: the mac header
 * @data: payload of the mac header, including the GCMP header and MIC
 * @data_len: length of @data, including GCMP header and MIC
		  ...
		  const u8 *data, size_t data_len, size_t *decrypted_len)
{
	...
	if (rtk_aes_gcm_ad(tk, tk_len, nonce, sizeof(nonce), m, mlen, aad, aad_len,
		       m + mlen, plain) < 0) {
 ...
 * @tk: the temporal key
 * @tk_len: length of @tk
 * @frame: the frame including mac header and payload, no GCMP header
 * @len: length of @frame
 * @hdrlen: length of the mac header
 * @qos: pointer to the QOS field of the frame
 * @pn: packet number
 * @keyid: key id
 * @encrypted_len: length of the encrypted frame including GCMP header and MIC
 */
		  ...
		  const u8 *pn, int keyid, size_t *encrypted_len)
{
	...
	*pos++ = pn[0]; /* PN5 */
	...
	if (rtk_aes_gcm_ae(tk, tk_len, nonce, sizeof(nonce), frame + hdrlen, plen,

...
struct pmkid_caching {
	unsigned char used[NUM_PMKID_CACHE];
...
typedef struct _wpa_global_info {
	...
	int					GTKRekey;
	int					GKeyFailure;
	struct rtk_timer_list			GKRekeyTimer;
} WPA_GLOBAL_INFO;
#endif
...
__PACK struct lib1x_eapol_pkt_hdr
{
 	unsigned char		code;
...
#ifdef RTK_NL80211
	unsigned char		commit_ie[256];
	unsigned int		commit_len;
#endif
} WPA_STA_INFO;
...
typedef struct _wpa_sta_info {
	...
	struct rtk_timer_list	resendTimer;
	struct rtl8192cd_priv	*priv;
	int					resendCnt;
	int					isGuest;

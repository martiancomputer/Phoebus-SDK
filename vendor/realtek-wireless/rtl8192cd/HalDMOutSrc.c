...
void DetectSTAExistance88XX(struct rtl8192cd_priv *priv, struct tx_rpt *report, struct stat_info *pstat)
{
...
#ifdef CTC_WIFI_DIAG
            ctcwifi_assoc_err(priv, pstat->cmn_info.mac_addr, "WiFi frames losing [STA is considered as disappeared]\n");
#endif
	...
	else if((GET_CHIP_VER(priv) == VERSION_8812F) || (GET_CHIP_VER(priv) == VERSION_8192F)){
		if(priv->pmib->dot11OperationEntry.sta_leave_extra_condition){
		unsigned int txpkts2_2sec = pstat->tx_pkts2 - pstat->tx_pkts2_pre2;
		if(txpkts2_2sec > 0){
			pstat->tx_conti_fail++;			
			DEBUG_WARN( "** tx_conti_fail=%d, (pkt, pkts)=(%d, %d) \n", pstat->tx_conti_fail, pstat->tx_pkts2, pstat->tx_pkts2_pre2);
		}

		pstat->tx_conti_fail_cnt += txpkts2_2sec;

		if((pstat->tx_conti_fail_cnt >= 100 && pstat->tx_conti_fail >= 3)
		||	((pstat->tx_conti_fail_cnt >= minTxFailCnt && priv->up_time >= (pstat->tx_last_good_time + 5)) && pstat->tx_conti_fail >= 5))
		{
			// This STA is considered as disappeared, so delete it.
			DEBUG_WARN( "** (tx_pkts2 or txfail) increase but txok is 0 for >5 times\n");
			DEBUG_WARN( "** tx_conti_fail_cnt=%d (min=%d,max=%d)\n", pstat->tx_conti_fail_cnt, minTxFailCnt, maxTxFailCnt);
			DEBUG_WARN( "** tx_last_good_time=%d, up_time=%d (Thr:5)\n", (int)pstat->tx_last_good_time, (int)priv->up_time );
			DEBUG_WARN( "AP is going to del_sta %02X:%02X:%02X:%02X:%02X:%02X\n", pstat->cmn_info.mac_addr[0],pstat->cmn_info.mac_addr[1],pstat->cmn_info.mac_addr[2],pstat->cmn_info.mac_addr[3],pstat->cmn_info.mac_addr[4],pstat->cmn_info.mac_addr[5] );
#ifdef CTC_WIFI_DIAG
			ctcwifi_assoc_err(priv, pstat->cmn_info.mac_addr, "WiFi frames losing [STA is considered as disappeared]\n");
#endif
...
void set_DIG_state(struct rtl8192cd_priv *priv, int state)
{
	...
	if (state) {
		...
		if(IS_OUTSRC_CHIP(priv) && priv->pshare->restore) {
			...
			//panic_printk("%s, 1-> %x, %x\n", __FUNCTION__, phydm_get_igi(ODMPTR, BB_PATH_A), priv->pshare->restore);
		}
#endif
		...
		if (priv->pshare->restore == 0) {
			...
			if(IS_OUTSRC_CHIP(priv)	) {
				priv->pshare->restore = phydm_get_igi(ODMPTR, BB_PATH_A);
...
#define CCK_TABLE_SIZE	33
 ...
 void set_CCK_swing_index(struct rtl8192cd_priv *priv, int CCK_index)
{
	short channel;
#ifdef MP_TEST
	if ((OPMODE & WIFI_MP_STATE) || priv->pshare->rf_ft_var.mp_specific)
...
// Client mode IOT issue, Button 2009.07.17
	...
	if (GET_CHIP_VER(priv)==VERSION_8188E) {
		...
		odm_ra_update_rate_info_8188e(ODMPTR, pstat->cmn_info.aid, pRAInfo->rate_id, pstat->tx_ra_bitmap, pRAInfo->sgi_enable);
#endif		
	} else
#endif
#if defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_92C_SUPPORT)
...
)
{
	...
	if (ODMPTR->support_ic_type & (ODM_IC_11AC_SERIES|ODM_IC_JGR3_SERIES))
	...
	if ((priv->up_time % 3) == 0)  {
		...
		if (ODMPTR->support_ic_type & ODM_IC_JGR3_SERIES)
...
enum asc_support_ic_type{
	acs_ic_11n_series = 0,
	acs_ic_11ac_series = 1,
	acs_ic_jgr3_series = 2,
	acs_ic_unknown = 3
};

enum asc_support_ic_type get_ic_type_acs(struct rtl8192cd_priv *priv)
{
	if (GET_CHIP_VER(priv) == VERSION_8188C ||
		...
		return acs_ic_11n_series;
	} else if (GET_CHIP_VER(priv) == VERSION_8814A) {
		return acs_ic_11ac_series;
	} else if (GET_CHIP_VER(priv) == VERSION_8814B) {
		return acs_ic_jgr3_series;
	}

	return acs_ic_unknown;
}
/*
void phydm_auto_channel_select_setting_ap(struct rtl8192cd_priv *priv, u4Byte setting)
{
    struct acs_info *acs = &priv->pshare->acs_info;
    u2Byte period = (SS_AUTO_CHNL_NHM_TO_VALUE * (1000/4));
    enum asc_support_ic_type type = get_ic_type_acs(priv);

    if (type == acs_ic_unknown) {
    	ACSDEBUG("chip not support nhm-acs");
    	return;
    }
    
	if (setting == STORE_DEFAULT_NHM_SETTING) {
        ACSDEBUG("STORE_DEFAULT_NHM_SETTING");
    
		if (type == acs_ic_11ac_series) {
            acs->reg0x990 = phy_query_bb_reg(priv, 0x990, 0xffffffff);
            acs->reg0x994 = phy_query_bb_reg(priv, 0x994, 0xffffffff);
            acs->reg0x998 = phy_query_bb_reg(priv, 0x998, 0xffffffff);
            acs->reg0x99c = phy_query_bb_reg(priv, 0x99c, 0xffffffff);
            acs->reg0x9a0 = phy_query_bb_reg(priv, 0x9a0, 0xff);
        } else if (type == acs_ic_11n_series) {
            acs->reg0x890 = phy_query_bb_reg(priv, 0x890, 0xffffffff);
            acs->reg0x894 = phy_query_bb_reg(priv, 0x894, 0xffffffff);
            acs->reg0x898 = phy_query_bb_reg(priv, 0x898, 0xffffffff);
            acs->reg0x89c = phy_query_bb_reg(priv, 0x89c, 0xffffffff);
            acs->reg0xe28 = phy_query_bb_reg(priv, 0xe28, 0xff);
        } else if (type == acs_ic_jgr3_series) {
        	acs->reg0x1e40 = phy_query_bb_reg(priv, 0x1e40, 0xffffffff);
            acs->reg0x1e44 = phy_query_bb_reg(priv, 0x1e44, 0xffffffff);
            acs->reg0x1e48 = phy_query_bb_reg(priv, 0x1e48, 0xffffffff);            
            acs->reg0x1e60 = phy_query_bb_reg(priv, 0x1e60, 0xffffffff);
            acs->reg0x1e5c = phy_query_bb_reg(priv, 0x1e5c, 0xff0000);
        }
    } else if (setting == RESTORE_DEFAULT_NHM_SETTING) {
        ACSDEBUG("RESTORE_DEFAULT_NHM_SETTING");
        
        if (type == acs_ic_11ac_series) {
            phy_set_bb_reg(priv, 0x990, 0xffffffff, acs->reg0x990);
            phy_set_bb_reg(priv, 0x994, 0xffffffff, acs->reg0x994);
            phy_set_bb_reg(priv, 0x998, 0xffffffff, acs->reg0x998);
            phy_set_bb_reg(priv, 0x99c, 0xffffffff, acs->reg0x99c);
            phy_set_bb_reg(priv, 0x9a0, 0xff, acs->reg0x9a0);
        } else if(type == acs_ic_11n_series) {
            phy_set_bb_reg(priv, 0x890, 0xffffffff, acs->reg0x890);
            phy_set_bb_reg(priv, 0x894, 0xffffffff, acs->reg0x894);
            phy_set_bb_reg(priv, 0x898, 0xffffffff, acs->reg0x898);
            phy_set_bb_reg(priv, 0x89c, 0xffffffff, acs->reg0x89c);
            phy_set_bb_reg(priv, 0xe28, 0xff, acs->reg0xe28); 
        } else if (type == acs_ic_jgr3_series) {
        	phy_set_bb_reg(priv, 0x1e40, 0xffffffff, acs->reg0x1e40);
            phy_set_bb_reg(priv, 0x1e44, 0xffffffff, acs->reg0x1e44);
            phy_set_bb_reg(priv, 0x1e48, 0xffffffff, acs->reg0x1e48);
            phy_set_bb_reg(priv, 0x1e60, 0xffffffff, acs->reg0x1e60);
            phy_set_bb_reg(priv, 0x1e5c, 0xff0000, acs->reg0x1e5c); 
        }
    } else if (setting == ACS_NHM_SETTING) {
        ACSDEBUG("ACS_NHM_SETTING (step=%d)", acs->step);
            
        if (type == acs_ic_11ac_series) {
        	//4 Set NHM period, 0x990[31:16]=0x61a8, Time duration for NHM unit: 4us, 0x61a8=100ms
            phy_set_bb_reg(priv, 0x990, 0xffff0000, period);            
			//4 Set NHM ignore_cca=1, ignore_txon=1, ccx_en=0
            phy_set_bb_reg(priv, 0x994, BIT8|BIT9|BIT10, 3);
            
            if (acs->step == 0) {
            	//4 Set IGI
                phy_set_bb_reg(priv, 0xc50, 0x7f, 0x3e);
                if (get_rf_mimo_mode(priv) != MIMO_1T1R)
					phy_set_bb_reg(priv, 0xe50, 0x7f, 0x3e);
				if (get_rf_mimo_mode(priv) == MIMO_3T4R) {
					phy_set_bb_reg(priv, 0x1850, 0x7f, 0x3e);
					phy_set_bb_reg(priv, 0x1a50, 0x7f, 0x3e);
				}
                    
                //4 Set ACS NHM threshold
                phy_set_bb_reg(priv, 0x998, 0xffffffff, 0x82786e64);
                phy_set_bb_reg(priv, 0x99c, 0xffffffff, 0xffffff8c);
                phy_set_bb_reg(priv, 0x9a0, 0xff, 0xff);
                phy_set_bb_reg(priv, 0x994, 0xffff0000, 0xffff);                
            } else if (acs->step == 1) {
                //4 Set IGI
                phy_set_bb_reg(priv, 0xc50, 0x7f, 0x2a);
                if (get_rf_mimo_mode(priv) != MIMO_1T1R)
					phy_set_bb_reg(priv, 0xe50, 0x7f, 0x2a);
				if (get_rf_mimo_mode(priv) == MIMO_3T4R) {
					phy_set_bb_reg(priv, 0x1850, 0x7f, 0x2a);
					phy_set_bb_reg(priv, 0x1a50, 0x7f, 0x2a);
				}

                //4 Set ACS NHM threshold
                phy_set_bb_reg(priv, 0x998, 0xffffffff, 0x5a50463c);
                phy_set_bb_reg(priv, 0x99c, 0xffffffff, 0xffffff64);                
            }
        } else if (type == acs_ic_11n_series) {
            //4 Set NHM period, 0x894[31:16]=0x61a8, Time duration for NHM unit: 4us, 0x61a8=100ms
			phy_set_bb_reg(priv, 0x894, 0xffff0000, period);
            //4 Set NHM ignore_cca=1, ignore_txon=1, ccx_en=0
            phy_set_bb_reg(priv, 0x890, BIT8|BIT9|BIT10, 3);
            
            if (acs->step == 0) {
                //4 Set IGI
                phy_set_bb_reg(priv, 0xc50, 0x7f, 0x3e);
                if (get_rf_mimo_mode(priv) != MIMO_1T1R)
					phy_set_bb_reg(priv, 0xc58, 0x7f, 0x3e);
            
                //4 Set ACS NHM threshold
                phy_set_bb_reg(priv, 0x898, 0xffffffff, 0x82786e64);
                phy_set_bb_reg(priv, 0x89c, 0xffffffff, 0xffffff8c);
                phy_set_bb_reg(priv, 0xe28, 0xff, 0xff);
                phy_set_bb_reg(priv, 0x890, 0xffff0000,0xffff);
                
            } else if (acs->step == 1) {
                //4 Set IGI
                phy_set_bb_reg(priv, 0xc50, 0x7f, 0x2a);
                if (get_rf_mimo_mode(priv) != MIMO_1T1R)
					phy_set_bb_reg(priv, 0xc58, 0x7f, 0x2a);
            
                phy_set_bb_reg(priv, 0x898, 0xffffffff, 0x5a50463c);
                phy_set_bb_reg(priv, 0x89c, 0xffffffff, 0xffffff64);
            }            
        } else if (type == acs_ic_jgr3_series) {
        	phy_set_bb_reg(priv, 0x1e40, 0xffff0000, period);
        	phy_set_bb_reg(priv, 0x1e60, 0xf00, 3);

        	if (acs->step == 0) {
				//4 Set IGI
				phy_set_bb_reg(priv, 0x1d70, 0x7f, 0x3e);
				phy_set_bb_reg(priv, 0x1d70, 0x7f00, 0x3e);
				phy_set_bb_reg(priv, 0x1d70, 0x7f0000, 0x3e);
				phy_set_bb_reg(priv, 0x1d70, 0x7f000000, 0x3e);
        		//4 Set ACS NHM threshold
        		phy_set_bb_reg(priv, 0x1e44, 0xffffffff, 0x82786e64);
                phy_set_bb_reg(priv, 0x1e48, 0xffffffff, 0xffffff8c);
                phy_set_bb_reg(priv, 0x1e5c, 0xff0000, 0xff);
                phy_set_bb_reg(priv, 0x1e60, 0xffff0000,0xffff);
        	} else if (acs->step == 1) {
        		//4 Set IGI
        		phy_set_bb_reg(priv, 0x1d70, 0x7f, 0x2a);
				phy_set_bb_reg(priv, 0x1d70, 0x7f00, 0x2a);
				phy_set_bb_reg(priv, 0x1d70, 0x7f0000, 0x2a);
				phy_set_bb_reg(priv, 0x1d70, 0x7f000000, 0x2a);
        		//4 Set ACS NHM threshold
        		phy_set_bb_reg(priv, 0x1e44, 0xffffffff, 0x5a50463c);
                phy_set_bb_reg(priv, 0x1e48, 0xffffffff, 0xffffff64);
        	}
        }
    }	
}

void nhm_trigger(struct rtl8192cd_priv *priv)
{
	enum asc_support_ic_type type = get_ic_type_acs(priv);
	u32 nhm_reg1 = 0;
	...
	phy_set_bb_reg(priv, nhm_reg1, BIT1, 0);
	phy_set_bb_reg(priv, nhm_reg1, BIT1, 1);
}
*/
void reset_fa_reg(struct rtl8192cd_priv *priv)
{
	...
	if (IS_OUTSRC_CHIP(priv))
	{
		extern void phydm_false_alarm_counter_reg_reset(void *dm_void);
...
/*
void phydm_get_nhm_statistics_ap(struct rtl8192cd_priv *priv, u4Byte idx)
{
    struct acs_info *acs = &priv->pshare->acs_info;
    u4Byte value32 = 0;
    u1Byte i;
    ...
    if (type == acs_ic_11n_series) {
        //4 Check if NHM result is ready        
        for (i = 0; i < 20; i++) {            
            delay_ms(1);

            if (phy_query_bb_reg(priv, 0x8b4, BIT17))
                break;
        }
        ...
        if (acs->step == 1) {
            ...
            value32 = phy_query_bb_reg(priv, 0x8dc, 0xffffffff);
            
            acs->nhm_cnt[idx][9] = (value32 & 0xff00) >> 8;
            acs->nhm_cnt[idx][8] = (value32 & 0xff);

            value32 = phy_query_bb_reg(priv, 0x8d8, 0xffffffff);

            acs->nhm_cnt[idx][7] = (value32 & 0xff000000) >> 24;
            acs->nhm_cnt[idx][6] = (value32 & 0xff0000) >> 16;
            acs->nhm_cnt[idx][5] = (value32 & 0xff00) >> 8;
        } else if (acs->step == 2) {
        
            value32 = phy_query_bb_reg(priv, 0x8d8, 0xffffffff);

            acs->nhm_cnt[idx][4] = phy_query_bb_reg(priv, 0x8dc, 0xff);            
            acs->nhm_cnt[idx][3] = (value32 & 0xff000000) >> 24;
            acs->nhm_cnt[idx][2] = (value32 & 0xff0000) >> 16;
            acs->nhm_cnt[idx][1] = (value32 & 0xff00) >> 8;
            acs->nhm_cnt[idx][0] = (value32 & 0xff);
        }
    } else if (type == acs_ic_11ac_series) {
        //4 Check if NHM result is ready        
        for (i = 0; i < 20; i++) {
			delay_ms(1);
			if (phy_query_bb_reg(priv, 0xfb4, BIT16))
				break;
        }
    
        if (acs->step == 1) {
            
            value32 = phy_query_bb_reg(priv, 0xfac, 0xffffffff);
            
            acs->nhm_cnt[idx][9] = (value32 & 0xff00) >> 8;
            acs->nhm_cnt[idx][8] = (value32 & 0xff);

            value32 = phy_query_bb_reg(priv, 0xfa8, 0xffffffff);     // ODM_REG_NHM_CNT3_TO_CNT0_11AC
            ...
            acs->nhm_cnt[idx][4] = phy_query_bb_reg(priv, 0xfac, 0xff);            
            acs->nhm_cnt[idx][3] = (value32 & 0xff000000) >> 24;
            acs->nhm_cnt[idx][2] = (value32 & 0xff0000) >> 16;
            acs->nhm_cnt[idx][1] = (value32 & 0xff00) >> 8;
            acs->nhm_cnt[idx][0] = (value32 & 0xff);
        ...
        for (i = 0; i < 20; i++) {
			delay_ms(1);
			if (phy_query_bb_reg(priv, 0x2d4c, BIT16))
				break;
        }
    
        if (acs->step == 1) {
            
            value32 = phy_query_bb_reg(priv, 0x2d44, 0xffffffff);
            
            acs->nhm_cnt[idx][9] = (value32 & 0xff00) >> 8;
            acs->nhm_cnt[idx][8] = (value32 & 0xff);

            value32 = phy_query_bb_reg(priv, 0x2d40, 0xffffffff);
            ...
            acs->nhm_cnt[idx][4] = phy_query_bb_reg(priv, 0x2d44, 0xff);            
            acs->nhm_cnt[idx][3] = (value32 & 0xff000000) >> 24;
            acs->nhm_cnt[idx][2] = (value32 & 0xff0000) >> 16;
            acs->nhm_cnt[idx][1] = (value32 & 0xff00) >> 8;
            acs->nhm_cnt[idx][0] = (value32 & 0xff);
...
*/
void query_fa_static(struct rtl8192cd_priv *priv, int idx)
{
#ifdef USE_OUT_SRC
...
#endif
	{
		...
		priv->chnl_ss_fa_count[idx] = ODMPTR->false_alm_cnt.cnt_all;
		priv->chnl_ss_cca_count[idx] = ODMPTR->false_alm_cnt.cnt_cca_all;		
	} 
#endif // USE_OUT_SRC
#if !defined(USE_OUT_SRC) || defined(_OUTSRC_COEXIST)
#ifdef _OUTSRC_COEXIST
	...
	{	
		...
		priv->chnl_ss_cca_count[idx] = priv->pshare->CCA_total_cnt;
	}

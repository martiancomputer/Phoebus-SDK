...
#ifdef RTK_MULTI_AP
void addEntryDisallowedList(struct rtl8192cd_priv *priv, const struct local_steering_list *list)
{
	int i, empty_slot;

	for(i = 0, empty_slot = -1; i < MAX_TRANS_LIST_NUM; i++)
	{
		if((priv->disallowed_list_bitmask[i>>3] & (1<<(i&7))) == 0) {
			if(empty_slot == -1)
				empty_slot = i;
		}else if(0 == memcmp(list->addr, priv->disallowed_list[i].addr, MACADDRLEN)) {
			break;
		}
	}
	...
	memcpy(priv->disallowed_list[i].addr, list->addr, MACADDRLEN);

	priv->disallowed_list_bitmask[i>>3] |= (1<<(i&7));

	DOT11VDEBUG("ADDED DISALLOWED LIST STA (%d) :[%02x][%02x][%02x][%02x][%02x][%02x] \n",
					i, priv->disallowed_list[i].addr[0], priv->disallowed_list[i].addr[1], priv->disallowed_list[i].addr[2],
					priv->disallowed_list[i].addr[3], priv->disallowed_list[i].addr[4], priv->disallowed_list[i].addr[5]);
}

void deleteEntryDisallowedList(struct rtl8192cd_priv *priv, const unsigned char* macAddress)
{
	int i;
	for (i = 0 ; i < MAX_TRANS_LIST_NUM; i++) {
		if((priv->disallowed_list_bitmask[i>>3] & (1<<(i&7))) == 0)
		...
		if(0 == memcmp(macAddress, priv->disallowed_list[i].addr, MACADDRLEN)) {

			DOT11VDEBUG("REMOVE DISALLOWED LIST STA (%d) :[%02x][%02x][%02x][%02x][%02x][%02x] \n",
								i, priv->disallowed_list[i].addr[0], priv->disallowed_list[i].addr[1], priv->disallowed_list[i].addr[2],
								priv->disallowed_list[i].addr[3], priv->disallowed_list[i].addr[4], priv->disallowed_list[i].addr[5]);
			priv->disallowed_list_bitmask[i>>3] &= ~(1<<(i&7));
			memset(&priv->disallowed_list[i], 0, sizeof(struct local_steering_list));
			break;
		}
	}
}

struct local_steering_list *getEntryDisallowedList(struct rtl8192cd_priv *priv, const unsigned char* macAddress){
	int i;
	for (i = 0 ; i < MAX_TRANS_LIST_NUM; i++) {
		if((priv->disallowed_list_bitmask[i>>3] & (1<<(i&7))) == 0)
			continue;
		...
		if(0 == memcmp(macAddress, priv->disallowed_list[i].addr, MACADDRLEN)) {
			return &priv->disallowed_list[i];
		}
	}
	return NULL;
}

void resetDisallowedList(struct rtl8192cd_priv *priv)
{
	int i;
	for(i = 0; i < (MAX_TRANS_LIST_NUM + 7)/8; i++)
		priv->disallowed_list_bitmask[i] = 0;
}
#endif
...
#ifdef RTK_MULTI_AP
	*(pbuf + 12) = entry->opclass; //Regulatory class
...
void process_BssTransReq(struct rtl8192cd_priv *priv)
{
	...
	for (i = 0; i < MAX_TRANS_LIST_NUM; i++) {
		...
		if(pstat) {
			...
			if(priv->transition_list[i].mode & BIT(7)){
...
void BssTrans_ExpiredTimer(struct rtl8192cd_priv *priv)
{
...
#ifdef RTK_MULTI_AP
	struct response_waiting_list *entry;
	unsigned char rcpi_threshold = priv->pmib->multi_ap.multiap_steering_rcpi_threshold;
	unsigned char cu_threshold = priv->pmib->multi_ap.multiap_cu_threshold;
	...
	for (i = 0; i < MAX_TRANS_LIST_NUM; i++) {
		...
		if(pstat) {
			...
			if((priv->transition_list[i].mode & BIT(7)) == 0){
				if(pstat->bssTransDisassocCountdown == 0)
					pstat->bssTransDisassocCountdown = priv->transition_list[i].disassoc_timer;
				if (pstat->bssTransTriggered != TRUE && 
					((pstat->rssi << 1) < rcpi_threshold || priv->ext_stats.ch_utilization > cu_threshold)) {
					send_bss_trans_event(priv, pstat, -1);
...
void set_staBssTransCap(struct stat_info *pstat, unsigned char *pframe, int frameLen, unsigned short ie_offset)
{
	...
	if(p != NULL) {
		ext_cap = p+2;
		if ((ie_len >= 3) && (ext_cap[2] & _WNM_BSS_TRANS_SUPPORT_)) {
			pstat->bssTransSupport = TRUE;
		} else {	// when there are 2 extended Capabilities IE (IOT issue with Fujitsu)
			p = p + ie_len + 2;
			if ((*p == _EXTENDED_CAP_IE_) && ((unsigned long)p + 5 <= (unsigned long)pframe + frameLen)) {
				ie_len = *(p+1);
				ext_cap = p+2;
				if ((ie_len >= 3) && (ext_cap[2] & _WNM_BSS_TRANS_SUPPORT_))
					pstat->bssTransSupport = TRUE;
...
#ifdef RTK_MULTI_AP
int update_local_steering_disallowed_list(struct rtl8192cd_priv *priv, unsigned char *tmpbuf)
{
	unsigned int flags;
	struct local_steering_list block_entry;
	int i;
	int local_steering_list_nr = tmpbuf[0];

	SAVE_INT_AND_CLI(flags);
	SMP_LOCK_LOCAL_STEERING_LIST(flags);
	
	resetDisallowedList(priv);
	
		for(i = 0; i < local_steering_list_nr; i++){
			memset(&block_entry, 0, sizeof(struct local_steering_list));
			memcpy(&block_entry.addr, tmpbuf+1+i*MACADDRLEN, MACADDRLEN);

			addEntryDisallowedList(priv, &block_entry);
	}

	SMP_UNLOCK_LOCAL_STEERING_LIST(flags);
	RESTORE_INT(flags);
...
void rtk_multi_ap_agent_steering_trigger(struct rtl8192cd_priv *priv)
{
	...
	while ((plist = asoc_list_get_next(priv, plist)) != phead) {
		...
		if (pstat->bssTransTriggered != TRUE && (pstat->rssi << 1) < priv->pmib->multi_ap.multiap_steering_rcpi_threshold
		    && getEntryDisallowedList(priv, pstat->cmn_info.mac_addr) == NULL) {
			send_bss_trans_event(priv, pstat, -1);
		}
		if (plist == plist->next) {
			break;
...
{	
	...
	if (entry) {
		diassoc_time = entry->disassoc_timer / TBTT(priv); // TUs transfer to TBTTs
	}
	...
	if(req_mode & (_WNM_PREFERRED_CANDIDATE_LIST_|_WNM_ABRIDGED_)) {	
		...
		if((preferred_BSSID && preferred_channel) && target_bassid_found == 0)
		{
	    	if(frlen + neighbor_size > MAX_REPORT_FRAME_SIZE)
	    		goto issue_wnm_bss_trans_fail;
	    	if(priv->pmib->roamingEntry.roaming_force_11v == 1)
	    		pbuf = construct_temp_neighbor_report_ie(priv, pbuf, &frlen, preferred_BSSID, preferred_channel);
			else if(priv->pmib->roamingEntry.roaming_force_11v == 0)
	    		goto issue_wnm_bss_trans_fail;
	    }
#endif
#endif // DOT11K
...
void OnBSSTransRsp(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pframe, int frame_len)
{
...
#if defined(RTK_MULTI_AP) || (defined(DOT11K) && defined(STA_CONTROL_BAND_TRANSITION))
	unsigned int flags;
#endif
#ifdef RTK_MULTI_AP
	...
	if (priv->stactrl.stactrl_status && priv->pmib->staControl.stactrl_band_transit) {
		...
		if (entry) {
			...
			// force disassociation if status_code != WNM_BSS_TM_ACCEPT
			TriggerBandTrans(priv, entry, ((status_code != WNM_BSS_TM_ACCEPT) ? 0 : priv->pmib->staControl.stactrl_btm_accepted_timeout));
...
void WNM_ActionHandler(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pframe, int frame_len)
{
	...
	#endif
	#ifdef CONFIG_MBO
		case _WNM_NOTIF_REQ_ACTION_ID_:

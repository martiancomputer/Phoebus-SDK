...
void sw_ant_div_rest_after_link(struct rtl8192cd_priv *priv)
{
	...
	priv->pshare->lastTxOkCnt = (unsigned long long)priv->net_stats.tx_bytes;
	priv->pshare->lastRxOkCnt = (unsigned long long)priv->net_stats.rx_bytes;
...
{
	unsigned long long  curTxOkCnt, curRxOkCnt;
	unsigned long long  CurByteCnt, PreByteCnt;
...
//1 3. Antenna Diversity
		...
		curTxOkCnt = (unsigned long long)priv->net_stats.tx_bytes - priv->pshare->lastTxOkCnt;
		curRxOkCnt = (unsigned long long)priv->net_stats.rx_bytes - priv->pshare->lastRxOkCnt;
		priv->pshare->lastTxOkCnt = (unsigned long long)priv->net_stats.tx_bytes;
		priv->pshare->lastRxOkCnt = (unsigned long long)priv->net_stats.rx_bytes;
